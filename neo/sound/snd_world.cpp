...
void idSoundWorldLocal::ProcessDemoCommand(idDemoFile* readDemo) {
  ...
  switch (dc) {
    case SCMD_STATE:
      ...
      // while the sound code is muted setting muted and going right in may not
      // be good enough here, as we async thread may already be in an async tick
      // (in which case we could still race to it)
      Sys_EnterCriticalSection();
    ...
    case SCMD_START: {
      ...
      EmitterForIndex(index)->StartSound(shader, (s_channelType)channel,
                                         diversity, shaderFlags);
    } break;
    case SCMD_MODIFY: {
      int channel;
      soundShaderParms_t parms;
...
===================
*/
float idSoundWorldLocal::CurrentShakeAmplitudeForPosition(
    const int time,
  ...
  for (int i = 1; i < emitters.Num(); i++) {
    ...
    amp +=
        FindAmplitude(sound, localTime, &listererPosition, SCHANNEL_ANY, true);
  }
  return amp;
}
...
void idSoundWorldLocal::MixLoop(int current44kHz,
      ...
      if (soundSystemLocal.efxloaded) {
        ...
        if (soundSystemLocal.s_muteEAXReverb.GetBool() ||
          ...
          if ((effect) && (effect->data) &&
              (memcpy(&EnvironmentParameters, effect->data,
                      effect->datasize))) {
            if (soundSystemLocal.s_muteEAXReverb.GetBool()) {
            ...
            if (soundSystemLocal.alEAXSet) {
              soundSystemLocal.alEAXSet(
                  &EAXPROPERTYID_EAX_FXSlot0, EAXREVERB_ALLPARAMETERS, 0,
                  &EnvironmentParameters, sizeof(EnvironmentParameters));
            }
          }
          listenerEnvironmentID = EnvironmentID;
        }
  ...
  if (idSoundSystemLocal::s_singleEmitter.GetInteger() > 0 &&
      idSoundSystemLocal::s_singleEmitter.GetInteger() < emitters.Num()) {
    ...
    if (sound && sound->playing) {
      ...
      for (j = 0; j < SOUND_MAX_CHANNELS; j++) {
        ...
        AddChannelContribution(sound, chan, current44kHz, numSpeakers,
                               finalMixBuffer);
      }
    }
    return;
  ...
  if (!idSoundSystemLocal::useOpenAL && enviroSuitActive) {
    soundSystemLocal.DoEnviroSuit(finalMixBuffer, MIXBUFFER_SAMPLES,
                                  numSpeakers);
  }
}
...
void idSoundWorldLocal::AVIOpen(const char* path, const char* name) {
  ...
  if (soundSystemLocal.snd_audio_hw->GetNumberOfSpeakers() == 6) {
    ...
    fpa[5] =
        fileSystem->OpenFileWrite(aviDemoPath + "channel_51_backright.raw");
  } else {
...
void idSoundWorldLocal::ResolveOrigin(const int stackDepth,
  ...
  for (int p = 0; p < numPortals; p++) {
    ...
    ResolveOrigin(stackDepth + 1, &newStack, otherArea,
                  dist + tlenLength + occlusionDistance, source, def);
  }
}
...
void idSoundWorldLocal::PlaceListener(const idVec3& origin,
  ...
  if (fpa[0]) {
    ...
    game44kHz = idMath::FtoiFast(gameMsec * ((1000.0f / 60.0f) / 16.0f) *
                                 0.001f * 44100.0f);
  } else {
...
void idSoundWorldLocal::ForegroundUpdate(int current44kHzTime) {
  ...
  for (j = 1; j < emitters.Num(); j++) {
    ...
    if (idSoundSystemLocal::s_drawSounds.GetInteger() && rw) {
      if (def->distance < def->maxDistance ||
          idSoundSystemLocal::s_drawSounds.GetInteger() > 1) {
        idBounds ref;
        ref.Clear();
        ...
        textPos[2] -= 8;
        rw->DrawText(va("%i", def->index), textPos, 0.1f, idVec4(1, 0, 0, 1),
                     listenerAxis);
        textPos[2] += 8;
        ...
        for (k = 0; k < SOUND_MAX_CHANNELS; k++) {
          ...
          const char* defaulted =
              chan->leadinSample->defaultSound ? "(DEFAULTED)" : "";
          sprintf(text, "%s (%i/%i %i/%i)%s", chan->soundShader->GetName(),
                  (int)def->distance, (int)def->realDistance, (int)min,
                  (int)max, defaulted);
  ...
  if (idSoundSystemLocal::s_showLevelMeter.GetInteger()) {
    const idMaterial* gui =
        declManager->FindMaterial("guis/assets/soundmeter/audiobg", false);
    if (gui) {
...
/*
 ...
 ===================
 */
void idSoundWorldLocal::WriteToSaveGameSoundShaderParams(
    idFile* saveGame,
  ...
  for (i = 1; i < num; i++) {
    ...
    while (channel >= 0) {
      if (channel > SOUND_MAX_CHANNELS) {
        common->Error(
            "idSoundWorldLocal::ReadFromSaveGame: channel > "
            "SOUND_MAX_CHANNELS");
      }
      ...
      if (soundSystemLocal.soundCache) {
        chan->leadinSample =
            soundSystemLocal.soundCache->FindSound(soundShader, false);
      } else {
...
/*
 ...
 ===================
 */
void idSoundWorldLocal::ReadFromSaveGameSoundShaderParams(
    idFile* saveGame,
    soundShaderParms_t* params) {
...
idSoundEmitter* idSoundWorldLocal::EmitterForIndex(int index) {
  ...
  if (index >= emitters.Num()) {
    common->Error("idSoundWorldLocal::EmitterForIndex: %i > %i", index,
                  emitters.Num());
  }
...
===============
*/
void idSoundWorldLocal::PlayShaderDirectly(const char* shaderName,
                                           int channel) {
  ...
  localSound->StartSound(shader, (channel == -1) ? SCHANNEL_ONE : channel,
                         diversity, SSF_GLOBAL);
...
void idSoundWorldLocal::CalcEars(int numSpeakers,
  ...
  if (numSpeakers == 6) {
    ...
    for (int i = 0; i < 6; i++) {
      ...
      ears[i] = (idSoundSystemLocal::s_dotbias6.GetFloat() + dot) /
                (1.0f + idSoundSystemLocal::s_dotbias6.GetFloat());
    ...
    // to the side
    dotBias +=
        (idSoundSystemLocal::s_spatializationDecay.GetFloat() - dotBias) *
        (1.0f - spatialize);

    ears[0] =
        (idSoundSystemLocal::s_dotbias2.GetFloat() + dot) / (1.0f + dotBias);
    ears[1] =
        (idSoundSystemLocal::s_dotbias2.GetFloat() - dot) / (1.0f + dotBias);
...
void idSoundWorldLocal::AddChannelContribution(idSoundEmitterLocal* sound,
  ...
  if (sample->defaultSound &&
      !idSoundSystemLocal::s_playDefaultSound.GetBool()) {
    return;
  }
  ...
  bool noOcclusion = (parms->soundShaderFlags & SSF_NO_OCCLUSION) ||
                     !idSoundSystemLocal::s_useOcclusion.GetBool();

  // speed goes from 1 to 0.2
  if (idSoundSystemLocal::s_slowAttenuate.GetBool() && slowmoActive &&
      !chan->disallowSlow) {
    maxd *= slowmoSpeed;
  }
  ...
  if (shader->leadinVolume &&
      current44kHz - chan->trigger44kHzTime < sample->LengthIn44kHzSamples()) {
  ...
  if (!global) {
    ...
    if (dlen >= maxd) {
      ...
      float frac = idMath::ClampFloat(0.0f, 1.0f,
                                      1.0f - ((dlen - mind) / (maxd - mind)));
  ...
  //
  if (idSoundSystemLocal::useOpenAL &&
      sound->removeStatus < REMOVE_STATUS_SAMPLEFINISHED) {
    if (!alIsSource(chan->openalSource)) {
      chan->openalSource = soundSystemLocal.AllocOpenALSource(
          chan,
          !chan->leadinSample->hardwareBuffer ||
              !chan->soundShader->entries[0]->hardwareBuffer || looping,
    ...
    if (alIsSource(chan->openalSource)) {
      ...
      if (global || omni) {
        ...
        alSourcef(chan->openalSource, AL_GAIN,
                  (volume) < (1.0f) ? (volume) : (1.0f));
      } else {
        alSourcei(chan->openalSource, AL_SOURCE_RELATIVE, AL_FALSE);
        alSource3f(chan->openalSource, AL_POSITION,
                   -spatializedOriginInMeters.y, spatializedOriginInMeters.z,
                   -spatializedOriginInMeters.x);
        alSourcef(chan->openalSource, AL_GAIN,
                  (volume) < (1.0f) ? (volume) : (1.0f));
      }
      alSourcei(chan->openalSource, AL_LOOPING,
                (looping && chan->soundShader->entries[0]->hardwareBuffer)
                    ? AL_TRUE
                    : AL_FALSE);
#if !defined(MACOS_X)
...
#endif
      alSourcef(chan->openalSource, AL_PITCH,
                (slowmoActive && !chan->disallowSlow) ? (slowmoSpeed) : (1.0f));
#if ID_OPENAL
      long lOcclusion = (enviroSuitActive ? -1150 : 0);
      if (soundSystemLocal.alEAXSet) {
        soundSystemLocal.alEAXSet(&EAXPROPERTYID_EAX_Source,
                                  EAXSOURCE_OCCLUSION, chan->openalSource,
                                  &lOcclusion, sizeof(lOcclusion));
      }
#endif
      if ((!looping && chan->leadinSample->hardwareBuffer) ||
        ...
        if (chan->triggered) {
          alSourcei(chan->openalSource, AL_BUFFER,
                    looping ? chan->soundShader->entries[0]->openalBuffer
                            : chan->leadinSample->openalBuffer);
        }
      } else {
        ...
        if (chan->triggered) {
          ...
          if (soundSystemLocal.alEAXSetBufferMode) {
            soundSystemLocal.alEAXSetBufferMode(
                3, &chan->openalStreamingBuffer[0],
                alGetEnumValue(ID_ALCHAR "AL_STORAGE_ACCESSIBLE"));
          }
          ...
          alGetSourcei(chan->openalSource, AL_BUFFERS_PROCESSED,
                       &finishedbuffers);
          alSourceUnqueueBuffers(chan->openalSource, finishedbuffers,
                                 &buffers[0]);
        ...
        for (j = 0; j < finishedbuffers; j++) {
          chan->GatherChannelSamples(
              ...
              MIXBUFFER_SAMPLES * sample->objectInfo.nChannels,
              alignedInputSamples);
          for (int i = 0;
               i < (MIXBUFFER_SAMPLES * sample->objectInfo.nChannels); i++) {
            if (alignedInputSamples[i] < -32768.0f)
              ...
              ((short*)alignedInputSamples)[i] =
                  idMath::FtoiFast(alignedInputSamples[i]);
          }
          alBufferData(
              buffers[j],
              chan->leadinSample->objectInfo.nChannels == 1
                  ? AL_FORMAT_MONO16
                  : AL_FORMAT_STEREO16,
              alignedInputSamples,
              MIXBUFFER_SAMPLES * sample->objectInfo.nChannels * sizeof(short),
              44100);
          chan->openalStreamingOffset += MIXBUFFER_SAMPLES;
        }

        if (finishedbuffers) {
          alSourceQueueBuffers(chan->openalSource, finishedbuffers,
                               &buffers[0]);
        }
      }
    ...
    if (slowmoActive && !chan->disallowSlow) {
      ...
      if (sample->objectInfo.nChannels == 2) {
        ...
        memset(alignedInputSamples, 0,
               sizeof(alignedInputSamples[0]) * MIXBUFFER_SAMPLES * 2);
      } else {
        slow.GatherChannelSamples(offset, MIXBUFFER_SAMPLES,
                                  alignedInputSamples);
      }
      ...
      if (sample->objectInfo.nChannels == 2) {
        ...
        // sample...
        chan->GatherChannelSamples(offset * 2, MIXBUFFER_SAMPLES * 2,
                                   alignedInputSamples);
      } else {
        chan->GatherChannelSamples(offset, MIXBUFFER_SAMPLES,
                                   alignedInputSamples);
      }
    }
    ...
    if (global || omni) {
      ...
      ears[3] =
          idSoundSystemLocal::s_subFraction.GetFloat() * volume;  // subwoofer

    } else {
      CalcEars(numSpeakers, spatializedOriginInMeters, listenerPos,
               listenerAxis, ears, spatialize);
    ...
    if (idSoundSystemLocal::s_clipVolumes.GetBool() &&
    ...
    if (numSpeakers == 6) {
      if (sample->objectInfo.nChannels == 1) {
        SIMDProcessor->MixSoundSixSpeakerMono(
            finalMixBuffer, alignedInputSamples, MIXBUFFER_SAMPLES, chan->lastV,
            ears);
      } else {
...
float idSoundWorldLocal::FindAmplitude(idSoundEmitterLocal* sound,
  ...
  for (i = 0; i < SOUND_MAX_CHANNELS; i++) {
    ...
    if (!listenerPosition) {
      ...
      if (listenerPosition && !(parms->soundShaderFlags & SSF_GLOBAL)) {
        ...
        if (dlen >= maxd) {
          ...
          float frac =
              idMath::ClampFloat(0, 1, 1.0f - ((dlen - mind) / (maxd - mind)));
          if (idSoundSystemLocal::s_quadraticFalloff.GetBool()) {
            frac *= frac;
          }
          volume *= frac;
    ...
    if (!listenerPosition && chan->parms.soundShaderFlags & SSF_NO_FLICKER) {
      ...
      int size =
          (looping ? chan->soundShader->entries[0]->LengthIn44kHzSamples()
                   : chan->leadinSample->LengthIn44kHzSamples());
      short* amplitudeData =
          (short*)(looping ? chan->soundShader->entries[0]->amplitudeData
                           : chan->leadinSample->amplitudeData);
      ...
      if (amplitudeData) {
        ...
        if (offset < size) {
          for (j = 0; j < (AMPLITUDE_SAMPLES); j++) {
            sourceBuffer[j] = j & 1 ? amplitudeData[(offset / 512) * 2]
                                    : amplitudeData[(offset / 512) * 2 + 1];
          }
        }
      } else {
...
void idSoundWorldLocal::FadeSoundClasses(const int soundClass,
  ...
  if (soundClass < 0 || soundClass >= SOUND_MAX_CLASSES) {
    common->Error("idSoundWorldLocal::FadeSoundClasses: bad soundClass %i",
                  soundClass);
  }

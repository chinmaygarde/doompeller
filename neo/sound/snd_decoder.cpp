...
int idWaveFile::ReadOGG(byte* pBuffer, int dwSizeToRead, int* pdwSizeRead) {
  ...
  do {
    int ret = ov_read(ov, bufferPtr, total >= 4096 ? 4096 : total,
                      Swap_IsBigEndian(), 2, 1, &ov->stream);
    if (ret == 0) {
      break;
    }
    if (ret < 0) {
...
class idSampleDecoderLocal : public idSampleDecoder {
 public:
  ...
  void Clear(void);
  int DecodePCM(idSoundSample* sample,
                int sampleOffset44k,
                int sampleCount44k,
                float* dest);
  int DecodeOGG(idSoundSample* sample,
                int sampleOffset44k,
                int sampleCount44k,
                float* dest);
...
void idSampleDecoder::Init(void) {
  ...
  decoderMemoryAllocator.SetFixedBlocks(
      idSoundSystemLocal::s_realTimeDecoding.GetBool() ? 10 : 1);
}
...
void idSampleDecoder::Free(idSampleDecoder* decoder) {
  idSampleDecoderLocal* localDecoder =
      static_cast<idSampleDecoderLocal*>(decoder);
...
void idSampleDecoderLocal::Decode(idSoundSample* sample,
  ...
  if (readSamples44k < sampleCount44k) {
    memset(dest + readSamples44k, 0,
           (sampleCount44k - readSamples44k) * sizeof(dest[0]));
  }
}
...
int idSampleDecoderLocal::DecodePCM(idSoundSample* sample,
  ...
  if (sample->nonCacheData == NULL) {
    assert(false);  // this should never happen ( note: I've seen that happen
                    // with the main thread down in idGameLocal::MapClear
                    // clearing entities - TTimo )
    failed = true;
    return 0;
  }

  if (!sample->FetchFromCache(sampleOffset * sizeof(short), &first, &pos, &size,
                              false)) {
    failed = true;
    return 0;
  }
  ...
  SIMDProcessor->UpSamplePCMTo44kHz(
      dest, (const short*)(first + pos), readSamples,
...
int idSampleDecoderLocal::DecodeOGG(idSoundSample* sample,
  ...
  do {
    float** samples;
    int ret =
        ov_read_float(&ogg, &samples,
                      totalSamples / sample->objectInfo.nChannels, &ogg.stream);
    if (ret == 0) {
      failed = true;
      break;
    }
    ...
    SIMDProcessor->UpSampleOGGTo44kHz(dest + (readSamples << shift), samples,
                                      ret, sample->objectInfo.nSamplesPerSec,

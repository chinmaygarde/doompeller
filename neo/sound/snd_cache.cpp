...
void idSoundCache::PrintMemInfo(MemInfo_t* mi) {
  ...
  for (i = 0; i < num - 1; i++) {
    for (j = i + 1; j < num; j++) {
      if (listCache[sortIndex[i]]->objectMemSize <
  ...
  for (i = 0; i < num; i++) {
    ...
    f->Printf("%s %s\n", idStr::FormatNumber(sample->objectMemSize).c_str(),
              sample->name.c_str());
  }
...
void idSoundSample::MakeDefault(void) {
  ...
  if (idSoundSystemLocal::useOpenAL) {
    ...
    alBufferData(
        openalBuffer,
        objectInfo.nChannels == 1 ? AL_FORMAT_MONO16 : AL_FORMAT_STEREO16,
        nonCacheData, objectMemSize, objectInfo.nSamplesPerSec);
    if (alGetError() != AL_NO_ERROR) {
      common->Error(
...
void idSoundSample::CheckForDownSample(void) {
  ...
  short* converted =
      (short*)soundCacheAllocator.Alloc(shortSamples * sizeof(short));
...
void idSoundSample::Load(void) {
  ...
  if (info.nChannels != 1 && info.nChannels != 2) {
    common->Warning("idSoundSample: %s has %i channels, using default",
                    name.c_str(), info.nChannels);
    fh.Close();
    MakeDefault();
    return;
  }
  ...
  if (info.wBitsPerSample != 16) {
    common->Warning(
        ...
        name.c_str(), info.wBitsPerSample);
    fh.Close();
    MakeDefault();
    return;
  }

  if (info.nSamplesPerSec != 44100 && info.nSamplesPerSec != 22050 &&
      info.nSamplesPerSec != 11025) {
    common->Warning(
        "idSoundCache: %s is %dHz, expected 11025, 22050 or 44100 Hz. Using "
        "default",
    ...
    if (objectInfo.wFormatTag == WAVE_FORMAT_TAG_PCM) {
      ...
      if (alIsBuffer(openalBuffer)) {
        ...
        if (alGetError() != AL_NO_ERROR) {
          ...
          amplitudeData = (byte*)soundCacheAllocator.Alloc(
              (objectSize / blockSize + 1) * 2 * sizeof(short));
          ...
          for (i = 0; i < objectSize; i += blockSize) {
            ...
            for (j = 0; j < Min(objectSize - i, blockSize); j++) {
              min = ((short*)nonCacheData)[i + j] < min
                        ? ((short*)nonCacheData)[i + j]
                        : min;
              max = ((short*)nonCacheData)[i + j] > max
                        ? ((short*)nonCacheData)[i + j]
                        : max;
            }
    ...
    if (objectInfo.wFormatTag == WAVE_FORMAT_TAG_OGG) {
#if defined(MACOS_X)
      if ((objectSize <
           ((int)objectInfo.nSamplesPerSec *
            idSoundSystemLocal::s_decompressionLimit.GetInteger()))) {
#else
      if ((alIsExtensionPresent(ID_ALCHAR "EAX-RAM") == AL_TRUE) &&
        ...
        if (alIsBuffer(openalBuffer)) {
          ...
          float* destData = (float*)soundCacheAllocator.Alloc(
              (LengthIn44kHzSamples() + 1) * sizeof(float));
          ...
          alBufferData(
              openalBuffer,
              objectInfo.nChannels == 1 ? AL_FORMAT_MONO16 : AL_FORMAT_STEREO16,
              destData, objectSize * sizeof(short), objectInfo.nSamplesPerSec);
          if (alGetError() != AL_NO_ERROR)
            common->Error(
                "idSoundCache: error loading data into OpenAL hardware buffer");
          else {
            ...
            for (i = 0; i < objectSize; i += blockSize) {
              ...
              int j;
              for (j = 0; j < Min(objectSize - i, blockSize); j++) {
                min = ((short*)destData)[i + j] < min
                          ? ((short*)destData)[i + j]
                          : min;
                max = ((short*)destData)[i + j] > max
                          ? ((short*)destData)[i + j]
                          : max;
              }
...
bool idSoundSample::FetchFromCache(int offset,
  ...
  if (objectSize == 0 || offset < 0 ||
      offset > objectSize * (int)sizeof(short) || !nonCacheData) {
    return false;
  }

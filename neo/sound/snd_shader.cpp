...
bool idSoundShader::SetDefaultText(void) {
  ...
  wavname = GetName();
  wavname.DefaultFileExtension(
      ".wav");  // if the name has .ogg in it, that will stay
...
bool idSoundShader::Parse(const char* text, const int textLength) {
  ...
  // deeper functions can set this, which will cause MakeDefault() to be called
  // at the end
  errorDuringParse = false;
...
bool idSoundShader::ParseShader(idLexer& src) {
  ...
  if (com_makingBuild.GetBool() || maxSamples <= 0 ||
  ...
  while (1) {
    ...
    // minimum number of sounds
      ...
      maxSamples =
          idMath::ClampInt(src.ParseInt(), SOUND_MAX_LIST_WAVS, maxSamples);
    }
    // description
    ...
    // leadinVolume is used to allow light breaking leadin sounds to be much
    // louder than the broken loop
    ...
    // onDemand can't be a parms, because we must track all references and
    // overrides would confuse it
    ...
    // the wave files
      ...
      if (soundSystemLocal.soundCache && numLeadins < maxSamples) {
        leadins[numLeadins] =
            soundSystemLocal.soundCache->FindSound(token.c_str(), onDemand);
        numLeadins++;
      }
  ...
  for (int k = 0; k < numLeadins; k++) {
    ...
    if (objectp) {
      common->Printf("      %5dms %4dKb %s (LEADIN)\n",
                     ...
                     (objectp->objectMemSize / 1024), objectp->name.c_str());
    }
  }

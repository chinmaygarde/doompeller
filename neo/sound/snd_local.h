...
// you need the OpenAL headers for build, even if AL is not enabled -
// http://www.openal.org/
#ifdef _WIN32
...
const int SOUND_DECODER_FREE_DELAY =
    1000 * MIXBUFFER_SAMPLES / USERCMD_MSEC;  // four seconds

const int PRIMARYFREQ = 44100;  // samples per second
const float SND_EPSILON =
    1.0f /
    32768.0f;  // if volume is below this, it will always multiply to zero
...
class idWaveFile {
 public:
  ...
  int OpenFromMemory(short* pbData,
                     int ulDataSize,
...
class idAudioHardware {
 public:
  ...
  // try to write as many sound samples to the device as possible without
  // blocking and prepare for a possible new mixing call returns wether there is
  // *some* space for writing available
...
class idSlowChannel {
  ...
  // functions
  void GenerateSlowChannel(FracTime& playPos,
                           int sampleCount44k,
                           float* finalBuffer);
 ...
 public:
  ...
  void GatherChannelSamples(int sampleOffset44k,
                            int sampleCount44k,
                            float* dest);
...
class idSoundChannel {
 public:
  ...
  soundShaderParms_t
      parms;  // combines the shader parms and the per-channel overrides
  ...
  float lastV[6];    // last calculated volume for each speaker, so we can
                     // smoothly fade
...
class idSoundEmitterLocal : public idSoundEmitter {
 public:
  ...
  virtual void UpdateEmitter(const idVec3& origin,
                             int listenerId,
                             const soundShaderParms_t* parms);

  // returns the length of the started sound in msec
  virtual int StartSound(const idSoundShader* shader,
                         const s_channelType channel,
                         float diversity = 0,
                         int shaderFlags = 0,
                         bool allowSlow = true /* D3XP */);

  // can pass SCHANNEL_ANY
  virtual void ModifySound(const s_channelType channel,
                           const soundShaderParms_t* parms);
  ...
  void OverrideParms(const soundShaderParms_t* base,
...
class idSoundWorldLocal : public idSoundWorld {
 public:
  ...
  virtual void FadeSoundClasses(const int soundClass,
                                const float to,
                                const float over);
  ...
  virtual void ReadFromSaveGameSoundChannel(idFile* saveGame,
                                            idSoundChannel* ch);
  virtual void ReadFromSaveGameSoundShaderParams(idFile* saveGame,
                                                 soundShaderParms_t* params);
  virtual void WriteToSaveGameSoundChannel(idFile* saveGame,
                                           idSoundChannel* ch);
  virtual void WriteToSaveGameSoundShaderParams(idFile* saveGame,
                                                soundShaderParms_t* params);
  ...
  void CalcEars(int numSpeakers,
                idVec3 realOrigin,
                idVec3 listenerPos,
                idMat3 listenerAxis,
                float ears[6],
                float spatialize);
  void AddChannelContribution(idSoundEmitterLocal* sound,
                              idSoundChannel* chan,
                              int current44kHz,
                              int numSpeakers,
  ...
  void AVIUpdate(void);
  void ResolveOrigin(const int stackDepth,
                     const soundPortalTrace_t* prevStack,
                     const int soundArea,
                     const float dist,
                     const idVec3& soundOrigin,
                     idSoundEmitterLocal* def);
  float FindAmplitude(idSoundEmitterLocal* sound,
                      const int localTime,
...
class idSoundSystemLocal : public idSoundSystem {
 public:
  idSoundSystemLocal() { isInitialized = false; }
  ...
  float*
      finalMixBuffer;  // points inside realAccum at a 16 byte aligned boundary
  ...
  s_stats
      soundStats;  // NOTE: updated throughout the code, not displayed anywhere
...
const int SCACHE_SIZE =
    MIXBUFFER_SAMPLES * 20;  // 1/2 of a second (aroundabout)
...
class idSoundSample {
 public:
  ...
  void Reload(
      bool force);  // reloads if timestamp has changed, or always if force
  void PurgeSoundSample();    // frees all data
  void CheckForDownSample();  // down sample if required
  bool FetchFromCache(int offset,
                      const byte** output,
                      int* position,
                      int* size,
                      const bool allowIO);
...
class idSampleDecoder {
 public:
  ...
  virtual void Decode(idSoundSample* sample,
                      int sampleOffset44k,
                      int sampleCount44k,
                      float* dest) = 0;

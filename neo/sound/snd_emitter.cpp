...
void idSoundChannel::Clear(void) {
  ...
  openalStreamingBuffer[0] = openalStreamingBuffer[1] =
      openalStreamingBuffer[2] = 0;
  lastopenalStreamingBuffer[0] = lastopenalStreamingBuffer[1] =
      lastopenalStreamingBuffer[2] = 0;
}
...
void idSoundChannel::ALStop(void) {
  if (idSoundSystemLocal::useOpenAL) {
    ...
    if (openalStreamingBuffer[0] && openalStreamingBuffer[1] &&
    ...
    if (lastopenalStreamingBuffer[0] && lastopenalStreamingBuffer[1] &&
        lastopenalStreamingBuffer[2]) {
      alGetError();
    ...
    for (i = 0; i < SOUND_MAX_CHANNELS; i++) {
      ...
      if (!(chan->parms.soundShaderFlags & SSF_LOOPING)) {
        ...
        if (soundWorld->slowmoActive && slow.IsActive()) {
          if (slow.GetCurrentPosition().time >=
              chan->leadinSample->LengthIn44kHzSamples() / 2) {
            chan->Stop();
      ...
      if (chan->decoder != NULL &&
          chan->decoder->GetLastDecodeTime() <
              current44kHzTime - SOUND_DECODER_FREE_DELAY) {
...
=====================
*/
void idSoundEmitterLocal::UpdateEmitter(const idVec3& origin,
                                        int listenerId,
...
=====================
*/
int idSoundEmitterLocal::StartSound(const idSoundShader* shader,
  ...
  if (idSoundSystemLocal::s_showStartSound.GetInteger()) {
    common->Printf("StartSound %ims (%i,%i,%s) = ", soundWorld->gameMsec, index,
                   (int)channel, shader->GetName());
  }
  ...
  if (channel != SCHANNEL_ANY) {
    for (i = 0; i < SOUND_MAX_CHANNELS; i++) {
      idSoundChannel* chan = &channels[i];
      if (chan->triggerState && chan->soundShader &&
  ...
  if (chan->parms.soundShaderFlags & SSF_LOOPING &&
    ...
    chan->trigger44kHzTime &=
        ~7;  // so we don't have to worry about the 22kHz and 11kHz expansions
...
===================
*/
void idSoundEmitterLocal::ModifySound(const s_channelType channel,
                                      const soundShaderParms_t* parms) {
  if (!parms) {
...
===================
*/
void idSoundEmitterLocal::FadeSound(const s_channelType channel,
                                    float to,
                                    float over) {
  if (idSoundSystemLocal::s_showStartSound.GetInteger()) {
  ...
  for (int i = 0; i < SOUND_MAX_CHANNELS; i++) {
    ...
    if (chan->channelFade.fadeEndVolume == to &&
        chan->channelFade.fadeEnd44kHz - chan->channelFade.fadeStart44kHz ==
            length44kHz) {
      continue;
    }

    // fade it
    chan->channelFade.fadeStartVolume =
        chan->channelFade.FadeDbAt44kHz(start44kHz);
...
float idSoundEmitterLocal::CurrentAmplitude(void) {
  ...
  ampTime = localTime;
  amplitude =
      soundWorld->FindAmplitude(this, localTime, NULL, SCHANNEL_ANY, false);
...
float idSlowChannel::GetSlowmoSpeed() {
  idSoundWorldLocal* sw =
      static_cast<idSoundWorldLocal*>(soundSystemLocal.GetPlayingSoundWorld());
...
void idSlowChannel::GenerateSlowChannel(FracTime& playPos,
  ...
  idSoundWorldLocal* sw =
      static_cast<idSoundWorldLocal*>(soundSystemLocal.GetPlayingSoundWorld());
  float in[MIXBUFFER_SAMPLES + 3], out[MIXBUFFER_SAMPLES + 3], *src, *spline,
      slowmoSpeed;
  ...
  lowpass.SetContinuitySamples(in_p[numSamples - 2], in_p[numSamples - 3],

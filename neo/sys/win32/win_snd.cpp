...
class idAudioBufferWIN32 : public idAudioBuffer {
 public:
  idAudioBufferWIN32(LPDIRECTSOUNDBUFFER apDSBuffer,
...
class idAudioHardwareWIN32 : public idAudioHardware {
 public:
  ...
  bool Initialize();
  bool InitializeSpeakers(byte* buffer,
                          int bufferSize,
                          dword dwPrimaryFreq,
                          dword dwPrimaryBitRate,
                          dword dwSpeakers);

  void SetPrimaryBufferFormat(dword dwPrimaryFreq,
                              dword dwPrimaryBitRate,
                              dword dwSpeakers);

  int Create(idWaveFile* pWaveFile, idAudioBuffer** ppiab);
  int Create(idAudioBuffer** ppSound,
             const char* strWaveFileName,
             dword dwCreationFlags = 0);
  int CreateFromMemory(idAudioBufferWIN32** ppSound,
                       byte* pbData,
                       ulong ulDataSize,
...
bool idAudioHardwareWIN32::Initialize(void) {
  ...
  SetPrimaryBufferFormat(PRIMARYFREQ, 16,
...
===============
*/
bool idAudioHardwareWIN32::InitializeSpeakers(byte* speakerData,
                                              int bufferSize,
                                              dword dwPrimaryFreq,
                                              dword dwPrimaryBitRate,
                                              dword dwSpeakers) {
  if (dwSpeakers == 2) {
    ...
    wfx.Format.nBlockAlign =
        wfx.Format.wBitsPerSample / 8 * wfx.Format.nChannels;
    wfx.Format.nAvgBytesPerSec =
        wfx.Format.nSamplesPerSec * wfx.Format.nBlockAlign;
    wfx.Format.cbSize = sizeof(WAVEFORMATEX);

    CreateFromMemory(&speakers, speakerData, bufferSize,
                     (waveformatextensible_t*)&wfx);
    ...
    waveFormatPCMEx.Format.nBlockAlign = waveFormatPCMEx.Format.wBitsPerSample /
                                         8 * waveFormatPCMEx.Format.nChannels;
    waveFormatPCMEx.Format.nAvgBytesPerSec =
        waveFormatPCMEx.Format.nSamplesPerSec *
        waveFormatPCMEx.Format.nBlockAlign;
    ...
    CreateFromMemory(&speakers, speakerData, bufferSize,
                     (waveformatextensible_t*)&waveFormatPCMEx);
...
void idAudioHardwareWIN32::SetPrimaryBufferFormat(dword dwPrimaryFreq,
  ...
  if (dwSpeakers == 6 &&
      (cfgSpeakers == DSSPEAKER_5POINT1 || cfgSpeakers == DSSPEAKER_SURROUND)) {
    WAVEFORMATEXTENSIBLE waveFormatPCMEx;
    ZeroMemory(&waveFormatPCMEx, sizeof(WAVEFORMATEXTENSIBLE));
    ...
    if (dwSpeakers == 6) {
      common->Printf(
          "sound: hardware reported unable to use multisound, defaulted to "
          "stereo\n");
    }
    WAVEFORMATEX wfx;
  ...
  byte* speakerData;
  bufferSize =
      MIXBUFFER_SAMPLES * sizeof(word) * numSpeakers * ROOM_SLICES_IN_BUFFER;
  speakerData = (byte*)Mem_Alloc(bufferSize);
  memset(speakerData, 0, bufferSize);

  InitializeSpeakers(speakerData, bufferSize, dwPrimaryFreq, dwPrimaryBitRate,
                     numSpeakers);
}
...
int idAudioHardwareWIN32::CreateFromMemory(idAudioBufferWIN32** ppSound,
  ...
  pWaveFile->OpenFromMemory((short*)pbData, ulDataSize,
                            (waveformatextensible_t*)pwfx);
...
===============
*/
bool idAudioHardwareWIN32::Lock(void** pDSLockedBuffer,
...
===============
*/
bool idAudioHardwareWIN32::Unlock(void* pDSLockedBuffer,
...
bool Sys_LoadOpenAL(void) {
  ...
  if (!hOpenAL) {
    common->Warning("LoadLibrary %s failed.",
                    idSoundSystemLocal::s_libOpenAL.GetString());
    return false;
  }
...
int idAudioBufferWIN32::FillBufferWithSound(LPDIRECTSOUNDBUFFER pDSB,
  ...
  if (dwWavDataRead == 0) {
    ...
    memset(pDSLockedBuffer,
           (byte)(m_pWaveFile->mpwfx.Format.wBitsPerSample == 8 ? 128 : 0),
...
bool idAudioBufferWIN32::GetCurrentPosition(ulong* pdwCurrentWriteCursor) {
  ...
  if (FAILED(
          hr = m_apDSBuffer->GetCurrentPosition(NULL, pdwCurrentWriteCursor))) {
    return false;
  }
  return true;
}

...
int Sys_GetSystemRam(void) {
  ...
  // HACK: For some reason, ullTotalPhys is sometimes off by a meg or two, so we
  // round up to the nearest 16 megs
...
int Sys_GetDriveFreeSpace(const char* path) {
  ...
  if (::GetDiskFreeSpaceEx(path, (PULARGE_INTEGER)&lpFreeBytesAvailable,
...
int Sys_GetVideoRam(void) {
  ...
  HRESULT hr = CoCreateInstance(CLSID_WbemLocator, 0, CLSCTX_SERVER,
  ...
  // Switch the security level to IMPERSONATE so that provider will grant access
  // to system-level objects.
  hr = CoSetProxyBlanket(spServices, RPC_C_AUTHN_WINNT, RPC_C_AUTHZ_NONE, NULL,
...
const int UNDECORATE_FLAGS =
    ...
    UNDNAME_NO_ALLOCATION_LANGUAGE | UNDNAME_NO_MEMBER_TYPE;
...
void Sym_Init(long addr) {
  ...
  GetModuleFileName((HMODULE)mbi.AllocationBase, moduleName,
                    sizeof(moduleName));
...
void Sym_GetFuncInfo(long addr, idStr& module, idStr& funcName) {
  ...
  for (s = m->symbols; s != NULL; s = s->next) {
    if (s->address == addr) {
      ...
      if (UnDecorateSymbolName(s->name, undName, sizeof(undName),
                               UNDECORATE_FLAGS)) {
        funcName = undName;
      } else {
        funcName = s->name;
...
void Sym_Init(long addr) {
  ...
  if (!SymLoadModule(processHandle, NULL, moduleName, NULL,

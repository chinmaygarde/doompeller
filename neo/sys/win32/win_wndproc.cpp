...
void WIN_Sizing(WORD side, RECT* rect) {
  if (!glConfig.isInitialized || glConfig.vidHeight <= 0 ||
      glConfig.vidWidth <= 0) {
    return;
  }
  ...
  switch (side) {
    case WMSZ_LEFT:
      ...
      rect->bottom =
          rect->top + (width * SCREEN_HEIGHT / SCREEN_WIDTH) + decoHeight;
      break;
    case WMSZ_RIGHT:
    ...
    case WMSZ_BOTTOMRIGHT:
      ...
      rect->right =
          rect->left + (height * SCREEN_WIDTH / SCREEN_HEIGHT) + decoWidth;
      break;
    case WMSZ_TOP:
    case WMSZ_TOPRIGHT:
    ...
    case WMSZ_BOTTOMLEFT:
      rect->bottom = rect->top + height + decoHeight;
      rect->left =
          rect->right - (height * SCREEN_WIDTH / SCREEN_HEIGHT) - decoWidth;
      break;
    case WMSZ_TOPLEFT:
      rect->top = rect->bottom - height - decoHeight;
      rect->left =

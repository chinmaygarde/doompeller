...
This is gross -- creating a window just to get a context to get the wgl
extensions
...
void GLW_CheckWGLExtensions(HDC hDC) {
  wglGetExtensionsStringARB =
      (PFNWGLGETEXTENSIONSSTRINGARBPROC)GLimp_ExtensionPointer(
          "wglGetExtensionsStringARB");
  ...
  wglSwapIntervalEXT =
      (PFNWGLSWAPINTERVALEXTPROC)GLimp_ExtensionPointer("wglSwapIntervalEXT");
  r_swapInterval.SetModified();  // force a set next frame

  // WGL_ARB_pixel_format
  wglGetPixelFormatAttribivARB =
      (PFNWGLGETPIXELFORMATATTRIBIVARBPROC)GLimp_ExtensionPointer(
          "wglGetPixelFormatAttribivARB");
  wglGetPixelFormatAttribfvARB =
      (PFNWGLGETPIXELFORMATATTRIBFVARBPROC)GLimp_ExtensionPointer(
          "wglGetPixelFormatAttribfvARB");
  wglChoosePixelFormatARB =
      (PFNWGLCHOOSEPIXELFORMATARBPROC)GLimp_ExtensionPointer(
          "wglChoosePixelFormatARB");

  // WGL_ARB_pbuffer
  wglCreatePbufferARB =
      (PFNWGLCREATEPBUFFERARBPROC)GLimp_ExtensionPointer("wglCreatePbufferARB");
  wglGetPbufferDCARB =
      (PFNWGLGETPBUFFERDCARBPROC)GLimp_ExtensionPointer("wglGetPbufferDCARB");
  wglReleasePbufferDCARB =
      (PFNWGLRELEASEPBUFFERDCARBPROC)GLimp_ExtensionPointer(
          "wglReleasePbufferDCARB");
  wglDestroyPbufferARB = (PFNWGLDESTROYPBUFFERARBPROC)GLimp_ExtensionPointer(
      "wglDestroyPbufferARB");
  wglQueryPbufferARB =
      (PFNWGLQUERYPBUFFERARBPROC)GLimp_ExtensionPointer("wglQueryPbufferARB");

  // WGL_ARB_render_texture
  wglBindTexImageARB =
      (PFNWGLBINDTEXIMAGEARBPROC)GLimp_ExtensionPointer("wglBindTexImageARB");
  wglReleaseTexImageARB = (PFNWGLRELEASETEXIMAGEARBPROC)GLimp_ExtensionPointer(
      "wglReleaseTexImageARB");
  wglSetPbufferAttribARB =
      (PFNWGLSETPBUFFERATTRIBARBPROC)GLimp_ExtensionPointer(
          "wglSetPbufferAttribARB");
}
...
static bool GLW_InitDriver(glimpParms_t parms) {
  ...
  if (wglChoosePixelFormatARB && parms.multiSamples > 1) {
    ...
    wglChoosePixelFormatARB(win32.hDC, iAttributes, fAttributes, 1,
                            &win32.pixelformat, &numFormats);
  } else {
  ...
  // get the full info
  DescribePixelFormat(win32.hDC, win32.pixelformat, sizeof(win32.pfd),
                      &win32.pfd);
...
static bool GLW_SetFullScreen(glimpParms_t parms) {
  ...
  // first make sure the user is not trying to select a mode that his
  // card/monitor can't handle
  matched = false;
  for (modeNum = 0;; modeNum++) {
    ...
    if ((int)devmode.dmPelsWidth >= parms.width &&
      ...
      if (parms.displayHz == 0 ||
          devmode.dmDisplayFrequency == parms.displayHz) {
        break;
      }
    }
  }
  ...
  if ((cdsRet = ChangeDisplaySettings(&dm, CDS_FULLSCREEN)) ==
  ...
  //
  // the exact mode failed, so scan EnumDisplaySettings for the next largest
  // mode
  //
  common->Printf("^3failed^0, ");
  ...
  for (modeNum = 0;; modeNum++) {
    ...
    if ((int)devmode.dmPelsWidth >= parms.width &&
        (int)devmode.dmPelsHeight >= parms.height &&
        devmode.dmBitsPerPel == 32) {
      if ((cdsRet = ChangeDisplaySettings(&devmode, CDS_FULLSCREEN)) ==
          DISP_CHANGE_SUCCESSFUL) {
        common->Printf("ok\n");
        win32.cdsFullscreen = true;
...
bool GLimp_Init(glimpParms_t parms) {
  ...
  // getting the wgl extensions involves creating a fake window to get a
  // context, which is pretty disgusting, and seems to mess with the AGP VAR
  // allocation
  GLW_GetWGLExtensionsWithFakeWindow();
...
bool GLimp_SetScreenParms(glimpParms_t parms) {
  ...
  bool ret =
      (ChangeDisplaySettings(&dm, parms.fullScreen ? CDS_FULLSCREEN : 0) ==
       DISP_CHANGE_SUCCESSFUL);
  SetWindowPos(win32.hWnd, parms.fullScreen ? HWND_TOPMOST : HWND_NOTOPMOST, x,
               y, w, h,
...
bool GLimp_SpawnRenderThread(void (*function)(void)) {
  ...
  win32.renderThreadHandle = CreateThread(
      ...
      (LPTHREAD_START_ROUTINE)
          GLimp_RenderThreadWrapper,  // LPTHREAD_START_ROUTINE lpStartAddr,

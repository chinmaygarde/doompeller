...
BOOL IsTaskMgrDisabled() {
  ...
  DWORD val = 0;
  DWORD len = 4;
  return RegQueryValueEx(hk, VAL_DisableTaskMgr, NULL, NULL, (BYTE*)&val,
                         &len) == ERROR_SUCCESS &&
         val == 1;
}
...
void DisableTaskKeys(BOOL bDisable, BOOL bBeep, BOOL bTaskMgr) {
  ...
  if (bDisable) {
    if (!g_hHookKbdLL) {
      g_hHookKbdLL =
          SetWindowsHookEx(WH_KEYBOARD_LL, MyTaskKeyHookLL, win32.hInstance, 0);
    }
  ...
  if (bTaskMgr) {
    HKEY hk;
    if (RegOpenKey(HKEY_CURRENT_USER, KEY_DisableTaskMgr, &hk) !=
        ERROR_SUCCESS) {
    ...
    if (bDisable) {
      ...
      DWORD val = 1;
      RegSetValueEx(hk, VAL_DisableTaskMgr, NULL, REG_DWORD, (BYTE*)&val,
                    sizeof(val));
    } else {

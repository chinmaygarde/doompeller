...
static GLboolean APIENTRY logAreTexturesResident(GLsizei n,
  ...
  fprintf(tr.logFile,
          "glAreTexturesResident %d 'const GLuint * textures' 'GLboolean * "
          "residences'\n",
          n);
...
static void APIENTRY logCopyTexImage1D(GLenum target,
  ...
  fprintf(tr.logFile, "glCopyTexImage1D %s %d %s %d %d %d %d\n",
          EnumString(target), level, EnumString(internalFormat), x, y, width,
          border);
...
static void APIENTRY logMap2d(GLenum target,
  ...
  // unknown type: "const GLdouble *" name: "points"
  fprintf(tr.logFile,
          "glMap2d %s %g %g %d %d %g %g %d %d 'const GLdouble * points'\n",
...
static void APIENTRY logMap2f(GLenum target,
  ...
  // unknown type: "const GLfloat *" name: "points"
  fprintf(tr.logFile,
          "glMap2f %s %g %g %d %d %g %g %d %d 'const GLfloat * points'\n",
          EnumString(target), u1, u2, ustride, uorder, v1, v2, vstride, vorder);
...
static void APIENTRY logPrioritizeTextures(GLsizei n,
  ...
  fprintf(tr.logFile,
          "glPrioritizeTextures %d 'const GLuint * textures' 'const GLclampf * "
          "priorities'\n",
          n);
...
static void APIENTRY logReadPixels(GLint x,
  ...
  fprintf(tr.logFile, "glReadPixels %d %d %d %d %s %s 'GLvoid * pixels'\n", x,
          y, width, height, EnumString(format), EnumString(type));
...
static void APIENTRY logStencilOp(GLenum fail, GLenum zfail, GLenum zpass) {
  fprintf(tr.logFile, "glStencilOp %s %s %s\n", EnumString(fail),
          EnumString(zfail), EnumString(zpass));
...
static void APIENTRY logTexImage1D(GLenum target,
  ...
  // unknown type: "const GLvoid *" name: "pixels"
  fprintf(tr.logFile,
          "glTexImage1D %s %d %d %d %d %s %s 'const GLvoid * pixels'\n",
          EnumString(target), level, internalformat, width, border,
          EnumString(format), EnumString(type));
...
static void APIENTRY logTexImage2D(GLenum target,
  ...
  // unknown type: "const GLvoid *" name: "pixels"
  fprintf(tr.logFile,
          "glTexImage2D %s %d %d %d %d %d %s %s 'const GLvoid * pixels'\n",
          EnumString(target), level, internalformat, width, height, border,
          EnumString(format), EnumString(type));
...
static void APIENTRY logTexSubImage1D(GLenum target,
  ...
  // unknown type: "const GLvoid *" name: "pixels"
  fprintf(tr.logFile,
          "glTexSubImage1D %s %d %d %d %s %s 'const GLvoid * pixels'\n",
          EnumString(target), level, xoffset, width, EnumString(format),
          EnumString(type));
...
static void APIENTRY logTexSubImage2D(GLenum target,
  ...
  // unknown type: "const GLvoid *" name: "pixels"
  fprintf(tr.logFile,
          "glTexSubImage2D %s %d %d %d %d %d %s %s 'const GLvoid * pixels'\n",
          EnumString(target), level, xoffset, yoffset, width, height,
          EnumString(format), EnumString(type));
  dllTexSubImage2D(target, level, xoffset, yoffset, width, height, format, type,
                   pixels);
}
...
static XVisualInfo* APIENTRY logChooseVisual(Display* dpy,
                                             int screen,
                                             int* attribList) {
...
static GLXContext APIENTRY logCreateContext(Display* dpy,
                                            XVisualInfo* vis,
  ...
  fprintf(tr.logFile,
          "glXCreateContext 'Display * dpy' 'XVisualInfo * vis' 'GLXContext "
...
static Bool APIENTRY logMakeCurrent(Display* dpy,
  ...
  // unknown type: "GLXContext" name: "ctx"
  fprintf(tr.logFile,
          "glXMakeCurrent 'Display * dpy' 'GLXDrawable drawable' 'GLXContext "
          "ctx'\n");

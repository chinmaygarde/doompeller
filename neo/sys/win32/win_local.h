...
#define WINDOW_STYLE \
  (WS_OVERLAPPED | WS_BORDER | WS_CAPTION | WS_VISIBLE | WS_THICKFRAME)
...
// add additional non keyboard / non mouse movement on top of the keyboard move
// cmd
...
typedef struct {
  ...
  bool mouseReleased;  // when the game has the console down or is doing a long
                       // operation
  ...
  // can know the exact time of an event (not really needed now that we use
  // async direct input)
  int sysMsgTime;

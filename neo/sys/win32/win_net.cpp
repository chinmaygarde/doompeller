...
idCVar net_port("net_port",
                "0",
                CVAR_SYSTEM | CVAR_INTEGER,
                "local IP port number");
idCVar net_forceLatency("net_forceLatency",
                        "0",
                        CVAR_SYSTEM | CVAR_INTEGER,
                        "milliseconds latency");
idCVar net_forceDrop("net_forceDrop",
                     "0",
                     CVAR_SYSTEM | CVAR_INTEGER,
                     "percentage packet loss");

idCVar net_socksEnabled("net_socksEnabled",
                        "0",
                        CVAR_SYSTEM | CVAR_ARCHIVE | CVAR_BOOL,
                        "");
idCVar net_socksServer("net_socksServer", "", CVAR_SYSTEM | CVAR_ARCHIVE, "");
idCVar net_socksPort("net_socksPort",
                     "1080",
                     CVAR_SYSTEM | CVAR_ARCHIVE | CVAR_INTEGER,
                     "");
idCVar net_socksUsername("net_socksUsername",
                         "",
                         CVAR_SYSTEM | CVAR_ARCHIVE,
                         "");
idCVar net_socksPassword("net_socksPassword",
                         "",
                         CVAR_SYSTEM | CVAR_ARCHIVE,
                         "");
...
=============
*/
static bool Net_ExtractPort(const char* src,
                            char* buf,
                            int bufsize,
                            int* port) {
  char* p;
...
=============
*/
static bool Net_StringToSockaddr(const char* s,
...
int NET_IPSocket(const char* net_interface, int port, netadr_t* bound_to) {
  ...
  if ((newsocket = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)) ==
      INVALID_SOCKET) {
    err = WSAGetLastError();
  ...
  if (bind(newsocket, (const struct sockaddr*)&address, sizeof(address)) ==
      SOCKET_ERROR) {
...
void NET_OpenSocks(int port) {
  ...
  if (h->h_addrtype != AF_INET) {
    common->Printf(
        "WARNING: NET_OpenSocks: gethostbyname: address type was not "
        "AF_INET\n");
    return;
  }
  address.sin_family = AF_INET;
  ...
  if (buf[1] == 2) {
    ...
    // send it
    if (send(socks_socket, (const char*)buf, 3 + ulen + plen, 0) ==
        SOCKET_ERROR) {
      err = WSAGetLastError();
      common->Printf("NET_OpenSocks: send: %s\n", NET_ErrorString());
      return;
...
bool Net_GetUDPPacket(int netSocket,
  ...
  fromlen = sizeof(from);
  ret =
      recvfrom(netSocket, data, maxSize, 0, (struct sockaddr*)&from, &fromlen);
  if (ret == SOCKET_ERROR) {
    err = WSAGetLastError();
  ...
  if (usingSocks && netSocket == ip_socket &&
      memcmp(&from, &socksRelayAddr, fromlen) == 0) {
    if (ret < 10 || data[0] != 0 || data[1] != 0 || data[2] != 0 ||
        data[3] != 1) {
      return false;
    }
    net_from.type = NA_IP;
...
void Net_SendUDPPacket(int netSocket,
  ...
  if (usingSocks && to.type == NA_IP) {
    ...
    *(short*)&socksBuf[8] = ((struct sockaddr_in*)&addr)->sin_port;
    ...
    ret = sendto(netSocket, socksBuf, length + 10, 0, &socksRelayAddr,
...
void Sys_InitNetworking(void) {
  ...
  if (!pAdapterInfo) {
    common->FatalError("Sys_InitNetworking: Couldn't malloc( %d )",
  ...
  if (GetAdaptersInfo(pAdapterInfo, &ulOutBufLen) == ERROR_BUFFER_OVERFLOW) {
    ...
    if (!pAdapterInfo) {
      common->FatalError("Sys_InitNetworking: Couldn't malloc( %ld )",
                         ulOutBufLen);
    }
  }
  ...
  if ((dwRetVal = GetAdaptersInfo(pAdapterInfo, &ulOutBufLen)) != NO_ERROR) {
    ...
    while (pAdapter) {
      common->Printf("Found interface: %s %s - ", pAdapter->AdapterName,
...
const char* Sys_NetAdrToString(const netadr_t a) {
  ...
  if (a.type == NA_LOOPBACK) {
    ...
    idStr::snPrintf(s, 64, "%i.%i.%i.%i:%i", a.ip[0], a.ip[1], a.ip[2], a.ip[3],
                    a.port);
  }
  return s;
}
...
bool idPort::GetPacket(netadr_t& from, void* data, int& size, int maxSize) {
  ...
  if (net_forceLatency.GetInteger() > 0 ||
      (udpPorts[bound_to.port] && udpPorts[bound_to.port]->recieveFirst)) {
    msg = udpPorts[bound_to.port]->recieveFirst;
    if (msg &&
        msg->time <= Sys_Milliseconds() - net_forceLatency.GetInteger()) {
      memcpy(data, msg->data, msg->size);
      size = msg->size;
      from = msg->address;
      udpPorts[bound_to.port]->recieveFirst =
          udpPorts[bound_to.port]->recieveFirst->next;
...
void idPort::SendPacket(const netadr_t to, const void* data, int size) {
  ...
  if (net_forceLatency.GetInteger() > 0 ||
      (udpPorts[bound_to.port] && udpPorts[bound_to.port]->sendFirst)) {
    assert(size <= MAX_UDP_MSG_SIZE);
    msg = udpPorts[bound_to.port]->udpMsgAllocator.Alloc();
    memcpy(msg->data, data, size);
    msg->size = size;
    ...
    for (msg = udpPorts[bound_to.port]->sendFirst;
      ...
      udpPorts[bound_to.port]->sendFirst =
          udpPorts[bound_to.port]->sendFirst->next;
...
bool idTCP::Init(const char* host, short port) {
  ...
  common->Printf("\"%s\" resolved to %i.%i.%i.%i:%i\n", host, address.ip[0],
                 address.ip[1], address.ip[2], address.ip[3], address.port);

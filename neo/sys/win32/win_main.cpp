...
idCVar Win32Vars_t::sys_cpustring("sys_cpustring",
                                  "detect",
                                  CVAR_SYSTEM | CVAR_INIT,
                                  "");
idCVar Win32Vars_t::in_mouse("in_mouse",
                             "1",
                             CVAR_SYSTEM | CVAR_BOOL,
                             "enable mouse input");
idCVar Win32Vars_t::win_allowAltTab("win_allowAltTab",
                                    "0",
                                    CVAR_SYSTEM | CVAR_BOOL,
                                    "allow Alt-Tab when fullscreen");
idCVar Win32Vars_t::win_notaskkeys("win_notaskkeys",
                                   "0",
                                   CVAR_SYSTEM | CVAR_INTEGER,
                                   "disable windows task keys");
idCVar Win32Vars_t::win_username("win_username",
                                 "",
                                 CVAR_SYSTEM | CVAR_INIT,
                                 "windows user name");
idCVar Win32Vars_t::win_xpos("win_xpos",
                             "3",
                             CVAR_SYSTEM | CVAR_ARCHIVE | CVAR_INTEGER,
                             "horizontal position of window");
idCVar Win32Vars_t::win_ypos("win_ypos",
                             "22",
                             CVAR_SYSTEM | CVAR_ARCHIVE | CVAR_INTEGER,
                             "vertical position of window");
idCVar Win32Vars_t::win_outputDebugString("win_outputDebugString",
                                          "0",
                                          CVAR_SYSTEM | CVAR_BOOL,
                                          "");
idCVar Win32Vars_t::win_outputEditString("win_outputEditString",
                                         "1",
                                         CVAR_SYSTEM | CVAR_BOOL,
                                         "");
idCVar Win32Vars_t::win_viewlog("win_viewlog",
                                "0",
                                CVAR_SYSTEM | CVAR_INTEGER,
                                "");
idCVar Win32Vars_t::win_timerUpdate(
    "win_timerUpdate",
    "0",
    CVAR_SYSTEM | CVAR_BOOL,
    "allows the game to be updated while dragging the window");
idCVar Win32Vars_t::win_allowMultipleInstances(
...
void Sys_CreateThread(xthread_t function,
  ...
  if (priority == THREAD_HIGHEST) {
    SetThreadPriority(
        ...
        THREAD_PRIORITY_HIGHEST);  //  we better sleep enough to do this
...
typedef struct CrtMemBlockHeader {
  struct _CrtMemBlockHeader*
      pBlockHeaderNext;  // Pointer to the block allocated just before this one:
  struct _CrtMemBlockHeader*
      pBlockHeaderPrev;  // Pointer to the block allocated just after this one
...
==================
*/
int Sys_AllocHook(int nAllocType,
                  void* pvData,
                  size_t nSize,
                  int nBlockUse,
                  long lRequest,
  ...
  switch (nAllocType) {
    ...
    case _HOOK_FREE:
      assert(pHead->gap[0] == 0xfd && pHead->gap[1] == 0xfd &&
             pHead->gap[2] == 0xfd && pHead->gap[3] == 0xfd);
...
void Sys_SetClipboardData(const char* string) {
  ...
  HMem =
      (char*)::GlobalAlloc(GMEM_MOVEABLE | GMEM_DDESHARE, strlen(string) + 1);
  if (HMem == NULL) {
    return;
  }
...
void Sys_DLL_Unload(int dllHandle) {
  ...
  if (FreeLibrary((HINSTANCE)dllHandle) == 0) {
    ...
    LPVOID lpMsgBuf;
    FormatMessage(
        FORMAT_MESSAGE_ALLOCATE_BUFFER, NULL, lastError,
...
static void Sys_AsyncThread(void* parm) {
  ...
  while (1) {
    ...
    if (r != WAIT_OBJECT_0) {
      OutputDebugString(
          "idPacketServer::PacketServerInterrupt: bad wait return");
    }
#endif
...
void Sys_StartAsyncThread(void) {
  ...
  Sys_CreateThread((xthread_t)Sys_AsyncThread, NULL, THREAD_ABOVE_NORMAL,
                   threadInfo, "Async", g_threads, &g_thread_count);
...
void Sys_Init(void) {
  ...
  cmdSystem->AddCommand("in_restart", Sys_In_Restart_f, CMD_FL_SYSTEM,
                        "restarts the input system");
#ifdef DEBUG
  cmdSystem->AddCommand("createResourceIDs", CreateResourceIDs_f, CMD_FL_TOOL,
  ...
  if (win32.osversion.dwPlatformId == VER_PLATFORM_WIN32_NT) {
    ...
    } else if (win32.osversion.dwMajorVersion == 5 &&
               win32.osversion.dwMinorVersion == 0) {
      win32.sys_arch.SetString("Win2K (NT)");
    } else if (win32.osversion.dwMajorVersion == 5 &&
               win32.osversion.dwMinorVersion == 1) {
    ...
    if (win32.osversion.dwMajorVersion == 4 &&
      ...
      if (win32.osversion.szCSDVersion[1] == 'C') {
        ...
        win32.sys_arch.SetString("Win95 (95)");
               ...
               win32.osversion.dwMinorVersion == 10) {
      // Win98
      if (win32.osversion.szCSDVersion[1] == 'A') {
        ...
        win32.sys_arch.SetString("Win98 (95)");
               ...
               win32.osversion.dwMinorVersion == 90) {
      // WinMe
  ...
  if (!idStr::Icmp(win32.sys_cpustring.GetString(), "detect")) {
    ...
    idLexer src(win32.sys_cpustring.GetString(),
                idStr::Length(win32.sys_cpustring.GetString()),
                "sys_cpustring");
    idToken token;
...
const char* GetExceptionCodeInfo(UINT code) {
  switch (code) {
    case EXCEPTION_ACCESS_VIOLATION:
      return "The thread tried to read from or write to a virtual address for "
    ...
    case EXCEPTION_DATATYPE_MISALIGNMENT:
      return "The thread tried to read or write data that is misaligned on "
             "hardware that does not provide alignment. For example, 16-bit "
    ...
    case EXCEPTION_FLT_DENORMAL_OPERAND:
      return "One of the operands in a floating-point operation is denormal. A "
             "denormal value is one that is too small to represent as a "
...
EXCEPTION_DISPOSITION __cdecl _except_handler(
  ...
  sprintf(msg,
          ...
          "If this text did not pop into your email client please copy and "
          "email it to programmers@idsoftware.com\n"
          "\n"
          "-= FATAL EXCEPTION =-\n"
          "\n"
          "%s\n"
          ...
          "\n"
          "%s\n",
          com_version.GetString(), ExceptionRecord->ExceptionCode,
          ExceptionRecord->ExceptionAddress,
          GetExceptionCodeInfo(ExceptionRecord->ExceptionCode),
          ContextRecord->Eax, ContextRecord->Ebx, ContextRecord->Ecx,
          ContextRecord->Edx, ContextRecord->Esi, ContextRecord->Edi,
          ContextRecord->Eip, ContextRecord->Esp, ContextRecord->Ebp,
          ContextRecord->EFlags, ContextRecord->SegCs, ContextRecord->SegSs,
          ContextRecord->SegDs, ContextRecord->SegEs, ContextRecord->SegFs,
          ContextRecord->SegGs, FPUFlags);
...
int WINAPI WinMain(HINSTANCE hInstance,
  ...
  if (win32.win_notaskkeys.GetInteger()) {
    DisableTaskKeys(TRUE, FALSE,
                    /*( win32.win_notaskkeys.GetInteger() == 2 )*/ FALSE);
  }
#endif
...
void idSysLocal::StartProcess(const char* exePath, bool doexit) {
  ...
  if (!CreateProcess(NULL, szPathOrig, NULL, NULL, FALSE, 0, NULL, NULL, &si,
                     &pi)) {

...
double Sys_ClockTicksPerSecond(void) {
  ...
  if (!ticks) {
    ...
    if (!RegOpenKeyEx(HKEY_LOCAL_MACHINE,
      ...
      ret = RegQueryValueEx(hKey, "~MHz", NULL, NULL, (LPBYTE)&ProcSpeed,
                            &buflen);
...
static void CPUID(int func, unsigned regs[4]) {
  ...
  __asm pusha __asm mov eax,
      func __asm __emit 00fh __asm __emit 0a2h __asm mov regEAX,
      eax __asm mov regEBX, ebx __asm mov regECX, ecx __asm mov regEDX,
      edx __asm popa
...
cpuid_t Sys_GetCPUId(void) {
  ...
  // check for Conditional Move (CMOV) and fast floating point comparison
  // (FCOMI) instructions
  if (HasCMOV()) {
    flags |= CPUID_CMOV;
  }
...
static bitFlag_t controlWordFlags[] = {
    {"Invalid operation", 0}, {"Denormalized operand", 1},
    {"Divide-by-zero", 2},    {"Numeric overflow", 3},
    {"Numeric underflow", 4}, {"Inexact result (precision)", 5},
    {"Infinity control", 12}, {"", 0}};
static char* precisionControlField[] = {
    "Single Precision (24-bits)", "Reserved", "Double Precision (53-bits)",
    "Double Extended Precision (64-bits)"};
static char* roundingControlField[] = {"Round to nearest", "Round down",
                                       "Round up", "Round toward zero"};
static bitFlag_t statusWordFlags[] = {
    {"Invalid operation", 0}, {"Denormalized operand", 1},
    {"Divide-by-zero", 2},    {"Numeric overflow", 3},
    {"Numeric underflow", 4}, {"Inexact result (precision)", 5},
    {"Stack fault", 6},       {"Error summary status", 7},
    {"FPU busy", 15},         {"", 0}};

...
typedef enum {
  ...
  CPUID_SSE3 =
      0x00100,  // Streaming SIMD Extentions 3 aka Prescott's New Instructions
  CPUID_ALTIVEC = 0x00200,  // AltiVec
  CPUID_HTT = 0x01000,      // Hyper-Threading Technology
  CPUID_CMOV = 0x02000,     // Conditional Move (CMOV) and fast floating point
                            // comparison (FCOMI) instructions
  CPUID_FTZ =
      0x04000,  // Flush-To-Zero mode (denormal results are flushed to zero)
  CPUID_DAZ = 0x08000  // Denormals-Are-Zero mode (denormal source operands are
                       // set to zero)
} cpuid_t;
...
class idPort {
 public:
  ...
  bool GetPacketBlocking(netadr_t& from,
                         void* data,
                         int& size,
                         int maxSize,
                         int timeout);
...
class idSys {
 public:
  ...
  virtual void DLL_GetFileName(const char* baseName,
                               char* dllName,
                               int maxLength) = 0;

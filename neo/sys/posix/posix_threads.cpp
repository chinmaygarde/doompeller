...
void Sys_WaitForEvent(int index) {
  ...
  if (signaled[index]) {
    ...
    waiting[index] = true;
    pthread_cond_wait(&event_cond[index],
                      &global_lock[MAX_LOCAL_CRITICAL_SECTIONS - 1]);
    waiting[index] = false;
  }
...
void Sys_CreateThread(xthread_t function,
  ...
  if (pthread_create((pthread_t*)&info.threadHandle, &attr,
                     (pthread_function_t)function, parms) != 0) {
...
void Sys_DestroyThread(xthreadInfo& info) {
  // the target thread must have a cancelation point, otherwise pthread_cancel
  // is useless
...
void Posix_StartAsyncThread() {
  if (asyncThread.threadHandle == 0) {
    Sys_CreateThread((xthread_t)Sys_AsyncThread, NULL, THREAD_NORMAL,
                     asyncThread, "Async", g_threads, &g_thread_count);
  } else {

...
const char* Sys_NetAdrToString(const netadr_t a) {
  ...
  if (a.type == NA_LOOPBACK) {
    ...
    idStr::snPrintf(s, sizeof(s), "%i.%i.%i.%i:%i", a.ip[0], a.ip[1], a.ip[2],
                    a.ip[3], a.port);
  }
  return s;
}
...
bool Sys_IsLANAddress(const netadr_t adr) {
  ...
  if (!num_interfaces) {
    return false;  // well, if there's no networking, there are no LAN
                   // addresses, right
  }
    ...
    if (ioctl(s, SIOCGIFADDR, ifr) < 0) {
      ...
      if (ifr->ifr_addr.sa_family != AF_INET) {
        ...
        if (ip == INADDR_LOOPBACK) {
          ...
          common->Printf("%d.%d.%d.%d", (unsigned char)ifr->ifr_addr.sa_data[2],
...
====================
*/
static int IPSocket(const char* net_interface,
                    int port,
                    netadr_t* bound_to = NULL) {
  int newsocket;
  ...
  if (bound_to) {
    ...
    if ((unsigned int)(getsockname(newsocket, (struct sockaddr*)&address,
                                   (socklen_t*)&len)) == -1) {
...
bool idPort::GetPacket(netadr_t& net_from, void* data, int& size, int maxSize) {
  ...
  fromlen = sizeof(from);
  ret = recvfrom(netSocket, data, maxSize, 0, (struct sockaddr*)&from,
                 (socklen_t*)&fromlen);
...
==================
*/
bool idPort::GetPacketBlocking(netadr_t& net_from,
                               void* data,
                               int& size,
                               int maxSize,
                               int timeout) {

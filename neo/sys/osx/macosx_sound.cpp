...
idCVar s_device("s_device",
                "-1",
                CVAR_SYSTEM | CVAR_ARCHIVE | CVAR_INTEGER,
                "Sound device to use. -1 for default device");
...
void idAudioHardwareOSX::Reset() {
  ...
  if (activeIOProc) {
    ...
    if (status != kAudioHardwareNoError) {
      common->Warning(
          "idAudioHardwareOSX::Reset: AudioDeviceStop failed. status: %s",
          ExtractStatus(status));
    }
    status = AudioDeviceRemoveIOProc(selectedDevice, DeviceIOProc);
    if (status != kAudioHardwareNoError) {
      common->Warning(
          "idAudioHardwareOSX::Reset: AudioDeviceRemoveIOProc failed. status "
          "%s\n",
          ExtractStatus(status));
    }
    activeIOProc = false;
...
OSStatus idAudioHardwareOSX::DeviceIOProc(AudioDeviceID inDevice,
  ...
  soundSystem->AsyncMix((int)inOutputTime->mSampleTime,
                        (float*)outOutputData->mBuffers[0].mData);
  Sys_LeaveCriticalSection();

  // doom mixes sound to -32768.0f 32768.0f range, scale down to -1.0f 1.0f
  SIMDProcessor->Mul(
      (Float32*)outOutputData->mBuffers[0].mData, 1.0f / 32768.0f,
      (Float32*)outOutputData->mBuffers[0].mData, MIXBUFFER_SAMPLES * 2);
...
bool idAudioHardwareOSX::Initialize() {
  ...
  if (status != kAudioHardwareNoError) {
    common->Warning(
        "AudioHardwareGetPropertyInfo kAudioHardwarePropertyDevices failed. "
        "status: %s",
        ExtractStatus(status));
    InitFailed();
    return false;
  ...
  status = AudioHardwareGetProperty(kAudioHardwarePropertyDevices, &size,
                                    deviceList);
  if (status != kAudioHardwareNoError) {
    common->Warning(
        "AudioHardwareGetProperty kAudioHardwarePropertyDevices failed. "
        "status: %s",
        ExtractStatus(status));
    free(deviceList);
    InitFailed();
  ...
  for (i = 0; i < deviceCount; i++) {
    size = 1024;
    status = AudioDeviceGetProperty(deviceList[i], 0, false,
                                    kAudioDevicePropertyDeviceName, &size, buf);
    if (status != kAudioHardwareNoError) {
      common->Warning(
          "AudioDeviceGetProperty kAudioDevicePropertyDeviceName %d failed. "
          "status: %s",
          i, ExtractStatus(status));
      free(deviceList);
      InitFailed();
      return false;
    }
    ...
    if (status != kAudioHardwareNoError) {
      common->Warning(
          "AudioDeviceGetProperty kAudioDevicePropertyDeviceManufacturer %d "
          "failed. status: %s",
          i, ExtractStatus(status));
      free(deviceList);
      InitFailed();
  ...
  status = AudioDeviceAddPropertyListener( selectedDevice, 0, false,
  kAudioDeviceProcessorOverload, DeviceListener, this ); if ( status !=
  kAudioHardwareNoError ) { common->Warning( "AudioDeviceAddPropertyListener
  kAudioDeviceProcessorOverload failed. status: %s", ExtractStatus( status ) );
          InitFailed();
          return;
  }
  */
  ...
  status = AudioDeviceGetProperty(selectedDevice, 0, false,
                                  kAudioDevicePropertyNominalSampleRate, &size,
                                  &sampleRate);
  if (status != kAudioHardwareNoError) {
    common->Warning(
        "AudioDeviceGetProperty %d kAudioDevicePropertyNominalSampleRate "
        "failed. status: %s",
        selectedDevice, ExtractStatus(status));
    InitFailed();
    return false;
  }
  ...
  if (sampleRate != PRIMARYFREQ) {
    ...
    status = AudioDeviceSetProperty(selectedDevice, NULL, 0, false,
                                    kAudioDevicePropertyNominalSampleRate, size,
                                    &sampleRate);
    if (status != kAudioHardwareNoError) {
      common->Warning(
          "AudioDeviceSetProperty %d kAudioDevicePropertyNominalSampleRate %g "
          "failed. status: %s",
          selectedDevice, sampleRate, ExtractStatus(status));
      InitFailed();
      return false;
    }
  }
  ...
  if (status != kAudioHardwareNoError) {
    common->Warning(
        "AudioDeviceGetProperty %d kAudioDevicePropertyBufferFrameSize "
        "failed.status: %s",
        selectedDevice, ExtractStatus(status));
    InitFailed();
    return false;
  ...
  if (status != kAudioHardwareNoError) {
    common->Warning(
        "AudioDeviceGetProperty %d kAudioDevicePropertyBufferFrameSizeRange "
        "failed. status: %s",
        selectedDevice, ExtractStatus(status));
    InitFailed();
    return false;
  }
  common->Printf("frame size allowed range: %g %g\n", frameSizeRange.mMinimum,
                 frameSizeRange.mMaximum);

  if (frameSizeRange.mMaximum < MIXBUFFER_SAMPLES) {
    common->Warning("can't obtain the required frame size of %d bits",
                    MIXBUFFER_SAMPLES);
    InitFailed();
    return false;
  }
  ...
  if (frameSize != (unsigned int)MIXBUFFER_SAMPLES) {
    ...
    if (status != kAudioHardwareNoError) {
      common->Warning(
          "AudioDeviceSetProperty %d kAudioDevicePropertyBufferFrameSize "
          "failed. status: %s",
          selectedDevice, ExtractStatus(status));
      InitFailed();
      return false;
  ...
  status = AudioDeviceGetProperty(
      selectedDevice, 0, false, kAudioDevicePropertyPreferredChannelsForStereo,
      &size, &channels);
  if (status != kAudioHardwareNoError) {
    common->Warning(
        "AudioDeviceGetProperty %d "
        "kAudioDevicePropertyPreferredChannelsForStereo failed. status: %s",
        selectedDevice, ExtractStatus(status));
    InitFailed();
    return false;
  }
  ...
  if (status != kAudioHardwareNoError) {
    common->Warning("AudioDeviceAddIOProc failed. status: %s",
                    ExtractStatus(status));
    InitFailed();
    return false;
  }
  ...
  if (status != kAudioHardwareNoError) {
    common->Warning("AudioDeviceStart failed. status: %s",
                    ExtractStatus(status));
    InitFailed();
    return false;
  }
  ...
  mixBufferSize =  dwSpeakers * dwSampleSize * dwPrimaryBitRate *
  ROOM_SLICES_IN_BUFFER / 8; mixBuffer = malloc( mixBufferSize ); memset(
...
void idAudioHardwareOSX::GetAvailableNominalSampleRates(void) {
  ...
  status = AudioDeviceGetPropertyInfo(
      selectedDevice, 0, false, kAudioDevicePropertyAvailableNominalSampleRates,
      &size, NULL);
  if (status != kAudioHardwareNoError) {
    common->Warning(
        "AudioDeviceGetPropertyInfo %d "
        "kAudioDevicePropertyAvailableNominalSampleRates failed. status: %s",
        selectedDevice, ExtractStatus(status));
    return;
  }
  ...
  for (i = 0; i < rangeCount; i++) {
    common->Printf("  %d: min %g max %g\n", i, rangeArray[i].mMinimum,
                   rangeArray[i].mMaximum);
  }

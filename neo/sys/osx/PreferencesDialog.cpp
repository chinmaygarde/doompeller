...
static idCVar r_stretched("r_stretched",
                          "0",
                          CVAR_ARCHIVE | CVAR_BOOL,
                          "Used stretched resolution");
...
const UInt32 kRes_Stretched = (1 << 0);  // set if the resolution is a stretched
                                         // mode (kCGDisplayModeIsStretched)
const UInt32 kRes_Safe =
    (1 << 1);  // еее╩(currently unused) set if the resolution is safe
...
struct PrefInfo {
  ...
  // The following are private data passed from GameDisplayPreferencesDialog()
  // to it's command handler.
  WindowRef window;
  ...
  std::list<Fixed>
      refreshRates;  // List of refresh rates available for the selected monitor
...
void Sys_DoPreferences(void) {
  ...
  if (fAlways ||
    ...
    if (CreateGameDisplayPreferencesDialog(&info, &prefWindow) == noErr) {
      if (RunGameDisplayPreferencesDialog(&info, prefWindow) == noErr) {
        ...
        cvarSystem->SetCVarInteger("r_screen",
                                   GetScreenIndexForDisplayID(info.displayID));
        cvarSystem->SetCVarInteger("r_minDisplayRefresh",
                                   (int)FixedToFloat(info.frequency));
        cvarSystem->SetCVarInteger("r_maxDisplayRefresh",
                                   (int)FixedToFloat(info.frequency));
      } else {
        Sys_Quit();
      }
  ...
  useOpenAL = CFPreferencesGetAppBooleanValue(
...
static int BuildResolutionList(CGDirectDisplayID inDisplayID,
                               Res* ioList,
  ...
  if (inDisplayID ==
      (CGDirectDisplayID)-1)  // special case, not associated with any display
  {
    ...
    for (i = 0; i < total; i++) {
      if (inCallback == NULL ||
          inCallback(inDisplayID, stdModes[i].width, stdModes[i].height, 32, 0))
        modes.insert(MakeRes(stdModes[i].width, stdModes[i].height, 32));
    }
  } else {
    CGDirectDisplayID displayID =
        inDisplayID ? inDisplayID : kCGDirectMainDisplay;
    ...
    for (i = 0; i < numModes; i++) {
      CFDictionaryRef modeRef =
          (CFDictionaryRef)CFArrayGetValueAtIndex(modeArrayRef, i);
      ...
      valueRef =
          (CFNumberRef)CFDictionaryGetValue(modeRef, kCGDisplayBitsPerPixel);
      ...
      if (CFDictionaryGetValueIfPresent(modeRef, kCGDisplayModeIsStretched,
                                        (const void**)&boolRef))
...
static void BuildRefreshRates(CGDirectDisplayID inDisplayID,
                              int inWidth,
                              int inHeight,
                              std::list<Fixed>* inList,
                              ValidModeCallbackProc inCallback) {
  CGDirectDisplayID displayID =
      inDisplayID ? inDisplayID : kCGDirectMainDisplay;
  ...
  for (int i = 0; i < numModes; i++) {
    ...
    if (width == inWidth && height == inHeight) {
      double freqDouble;
      valueRef =
          (CFNumberRef)CFDictionaryGetValue(modeRef, kCGDisplayRefreshRate);
...
static void BuildRefreshPopupButton(ControlRef inControl,
                                    std::list<Fixed>* inList) {
  ...
  // The menu has two permanent items - "Auto" & a divider line. Delete
  // everything else.
  DeleteMenuItems(menu, 3, CountMenuItems(menu) - 2);

  for (std::list<Fixed>::const_iterator iter = inList->begin();
       iter != inList->end(); ++iter) {
    float value = FixedToFloat(*iter);
    CFStringRef menuString =
        CFStringCreateWithFormat(kCFAllocatorDefault, 0, CFSTR("%g Hz"), value);
    InsertMenuItemTextWithCFString(menu, menuString, CountMenuItems(menu), 0,
                                   0);
  }
...
static SInt32 FindRefreshPopupMenuItem(std::list<Fixed>* inList,
                                       Fixed inFrequency) {
...
static void BuildResolutionPopupButton(ControlRef inControl,
                                       CGDirectDisplayID inDisplayID,
                                       ValidModeCallbackProc inCallback) {
  ...
  while (count--) {
    CFStringRef menuString = CFStringCreateWithFormat(
        kCFAllocatorDefault, 0, CFSTR("%d x %d %@"), resList[count].width,
        resList[count].height,
        (resList[count].resFlags & kRes_Stretched) ? CFSTR("(Stretched)")
                                                   : CFSTR(""));
    InsertMenuItemTextWithCFString(menu, menuString, 0, 0, 0);
    err = SetMenuItemProperty(menu, 1, kAppCreator, 'Res ',
...
static void GetResolutionFromPopupMenuItem(ControlRef inControl,
  ...
  err = GetMenuItemProperty(menu, inItem, kAppCreator, 'Res ', sizeof(res),
                            NULL, &res);
  if (!err) {
    *outX = res.width;
    *outY = res.height;
...
static void AdjustResolutionPopupMenu(ControlRef inControl,
                                      CGDirectDisplayID inDisplayID,
                                      bool isFullscreen,
                                      int& screenwidth,
                                      int& screenheight,
                                      int& screendepth,
                                      UInt32& screenResFlags) {
  int screenX = INT_MAX, screenY = INT_MAX;

  // In windowed mode, you have to disable resolutions that are larger than the
  // current screen size
  if (!isFullscreen) {
  ...
  for (item = 1; item <= count; item++) {
    GetResolutionFromPopupMenuItem(inControl, item, &resX, &resY, &depth,
                                   &resFlags);
    ...
    if (resX == screenwidth && resY == screenheight && depth == screendepth &&
        resFlags == screenResFlags)
  ...
  if (!IsPopupMenuItemEnabled(inControl, GetControlValue(inControl))) {
    for (item = 1; item <= count; item++) {
      if (IsPopupMenuItemEnabled(inControl, item)) {
        SetControlValue(inControl, item);
        GetResolutionFromPopupMenuItem(inControl, item, &screenwidth,
...
static void AdjustDisplayControls(PrefInfo* prefInfo) {
  ...
  if ((prefInfo->prefGameDisplayMode != kFullScreen)) {
    BuildResolutionPopupButton(prefInfo->resolutionPopup, (CGDirectDisplayID)-1,
                               prefInfo->callback);
    if (prefInfo->multiMonitor)
      EnableControl(prefInfo->chooseMonitorsBtn);
  } else {
    BuildResolutionPopupButton(prefInfo->resolutionPopup,
                               prefInfo->prefDisplayID, prefInfo->callback);
    if (prefInfo->multiMonitor)
      EnableControl(prefInfo->chooseMonitorsBtn);
  }
  AdjustResolutionPopupMenu(prefInfo->resolutionPopup, prefInfo->prefDisplayID,
                            prefInfo->prefGameDisplayMode == kFullScreen,
                            prefInfo->prefWidth, prefInfo->prefHeight,
                            prefInfo->prefDepth, prefInfo->prefResFlags);

  // Build new refresh popup and select appropriate rate
  BuildRefreshRates(prefInfo->prefDisplayID, prefInfo->prefWidth,
                    prefInfo->prefHeight, &prefInfo->refreshRates,
                    prefInfo->callback);
  BuildRefreshPopupButton(prefInfo->refreshRatePopup, &prefInfo->refreshRates);

  if (prefInfo->refreshRates.size() == 0) {  // No refresh rates, so pick Auto
    prefInfo->freqMenuIndex = 1;
    prefInfo->prefFrequency = 0;
  } else {
    prefInfo->freqMenuIndex = FindRefreshPopupMenuItem(&prefInfo->refreshRates,
                                                       prefInfo->prefFrequency);
    if (prefInfo->freqMenuIndex == 1)
      prefInfo->prefFrequency = 0;  // just in case FindRefreshPopupMenuItem
                                    // didn't find prefInfo->prefFrequency
  }
  SetControlValue(prefInfo->refreshRatePopup, prefInfo->freqMenuIndex);

  // Disable refresh rate if NOT fullscreen
  if ((prefInfo->prefGameDisplayMode != kFullScreen) ||
      (prefInfo->refreshRates.size() == 0))
...
static pascal OSStatus PrefHandler(EventHandlerCallRef inHandler,
                                   EventRef inEvent,
                                   void* inUserData) {
#pragma unused(inHandler)
  ...
  switch (cmd.commandID) {
    ...
    case kCmdInAWindow:
      ...
      SetControlValue(prefInfo->fullscreenBtn,
                      prefInfo->prefGameDisplayMode == kFullScreen);
      SetControlValue(prefInfo->inAWindowBtn,
                      1 - (prefInfo->prefGameDisplayMode == kFullScreen));
      if (prefInfo->prefGameDisplayMode == kFullScreen)
        EnableControl(prefInfo->refreshRatePopup);
      else
        DisableControl(prefInfo->refreshRatePopup);
    ...
    case kCmdChooseMonitors: {
      // PickMonitor((DisplayIDType*)&prefInfo->prefDisplayID,
      // prefInfo->window);
    ...
    case kCmdResolution: {
      ...
      GetResolutionFromPopupMenuItem(
          prefInfo->resolutionPopup, item, &prefInfo->prefWidth,
          &prefInfo->prefHeight, &prefInfo->prefDepth, &prefInfo->prefResFlags);

      // Adjust refresh menu
      BuildRefreshRates(prefInfo->prefDisplayID, prefInfo->prefWidth,
                        prefInfo->prefHeight, &prefInfo->refreshRates,
                        prefInfo->callback);
      BuildRefreshPopupButton(prefInfo->refreshRatePopup,
                              &prefInfo->refreshRates);
      prefInfo->freqMenuIndex = FindRefreshPopupMenuItem(
          &prefInfo->refreshRates, prefInfo->prefFrequency);
      if (prefInfo->freqMenuIndex == 1)
        prefInfo->prefFrequency = 0;  // just in case FindRefreshPopupMenuItem
                                      // didn't find prefInfo->prefFrequency
      SetControlValue(prefInfo->refreshRatePopup, prefInfo->freqMenuIndex);
    ...
    case kCmdRefreshRate: {
      // Keep prefInfo->prefFrequency updated for the other controls to
      // reference
...
static DEFINE_ONE_SHOT_HANDLER_GETTER(PrefHandler)
    ...
    OSStatus
    CreateGameDisplayPreferencesDialog(const GameDisplayInfo* inGDInfo,
                                       WindowRef* outWindow,
                                       ValidModeCallbackProc inCallback) {
  OSStatus err = noErr;
  ...
  Boolean result;
  Boolean keyFound;
  result = CFPreferencesGetAppBooleanValue(
      kPref_PrefsDialogAlways, kCFPreferencesCurrentApplication, &keyFound);
  ...
  // If DoPreferences is called at the start of the game, prefInfo.prefDisplayID
  // needs to be checked to see if it is still a valid display ID.
  ...
  GetControlByID(prefInfo.window, &kRefreshRatePopup,
                 &prefInfo.refreshRatePopup);
  assert(prefInfo.refreshRatePopup);
  GetControlByID(prefInfo.window, &kChooseMonitorsBtn,
                 &prefInfo.chooseMonitorsBtn);
  ...
  InstallWindowEventHandler(prefInfo.window, handler, 1, &cmdEvent, &prefInfo,
                            NULL);

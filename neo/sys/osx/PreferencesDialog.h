...
typedef struct {
  GameDisplayMode
      mode;  // Indicates if the game is in full screen mode or window mode.
  ...
  Fixed frequency;  // Screen refresh rate in MHz for full screen mode. If zero,
                    // then a default will be used.
  POINT windowLoc;  // Device-local coordinate of top left corner for window
                    // mode. Expressed as a Win32 POINT. Coordiantes may be
                    // CW_USEDEFAULT indicating no location has yet been
                    // established.
  unsigned long flags;  // kBlankingWindow, kDontRepositionWindow, etc.
  UInt32 resFlags;  // boolean bits to mark special modes for each resolution,
                    // e.g. stretched
} GameDisplayInfo;

typedef bool (*ValidModeCallbackProc)(CGDirectDisplayID displayID,
                                      int width,
                                      int height,
                                      int depth,
                                      Fixed freq);

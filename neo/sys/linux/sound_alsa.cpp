...
static idCVar s_alsa_pcm(
    "s_alsa_pcm",
    "default",
    CVAR_SYSTEM | CVAR_ARCHIVE,
    "which alsa pcm device to use. default, hwplug, hw.. see alsa docs");
static idCVar s_alsa_lib("s_alsa_lib",
                         "libasound.so.2",
                         CVAR_SYSTEM | CVAR_ARCHIVE,
                         "alsa client sound library");
...
bool idAudioHardwareALSA::DLOpen(void) {
  ...
  id_snd_asoundlib_version =
      (pfn_snd_asoundlib_version)dlsym(m_handle, "snd_asoundlib_version");
...
bool idAudioHardwareALSA::Initialize(void) {
  ...
  if ((err = id_snd_pcm_open(&m_pcm_handle, s_alsa_pcm.GetString(),
  ...
  if ((err = id_snd_pcm_hw_params_set_access(
           m_pcm_handle, hwparams, SND_PCM_ACCESS_RW_INTERLEAVED)) < 0) {
  ...
  if ((err = id_snd_pcm_hw_params_set_format(m_pcm_handle, hwparams,
  ...
  if ((err = id_snd_pcm_hw_params_set_channels(m_pcm_handle, hwparams,
                                               m_channels)) < 0) {
  ...
  if ((err = id_snd_pcm_hw_params_set_rate(m_pcm_handle, hwparams, PRIMARYFREQ,
                                           0)) < 0) {
    common->Printf(
        "failed to set 44.1KHz rate: %s - try ( +set s_alsa_pcm plughw:0 ? )\n",
        id_snd_strerror(err));
    InitFailed();
    return false;
  }

  // have enough space in the input buffer for our MIXBUFFER_SAMPLE feedings and
  // async ticks
  snd_pcm_uframes_t frames;
  frames = MIXBUFFER_SAMPLES + MIXBUFFER_SAMPLES / 3;
  if ((err = id_snd_pcm_hw_params_set_buffer_size_min(m_pcm_handle, hwparams,
                                                      &frames)) < 0) {
  ...
  // TODO: can use swparams to setup the device so it doesn't underrun but
  // rather loops over snd_pcm_sw_params_set_stop_threshold To get alsa to just
  // loop on underruns. set the swparam stop_threshold to equal buffer size. The
  // sound buffer will just loop and never throw an xrun.
...
void idAudioHardwareALSA::Write(bool flushing) {
  ...
  // write the max frames you can in one shot - we need to write it all out in
  // Flush() calls before the next Write() happens
  int pos =
      (int)m_buffer + (MIXBUFFER_SAMPLES - m_remainingFrames) * m_channels * 2;
  snd_pcm_sframes_t frames =
      id_snd_pcm_writei(m_pcm_handle, (void*)pos, m_remainingFrames);
  if (frames < 0) {
    if (frames != -EAGAIN) {
      Sys_Printf("snd_pcm_writei %d frames failed: %s\n", m_remainingFrames,
                 id_snd_strerror(frames));
    }
    return;
  }

...
#ifndef NO_ALSA
static idCVar s_driver("s_driver",
                       s_driverArgs[0],
                       CVAR_SYSTEM | CVAR_ARCHIVE,
                       "sound driver. 'best' will attempt to use alsa and "
                       "fallback to OSS if not available",
                       s_driverArgs,
                       idCmdSystem::ArgCompletion_String<s_driverArgs>);
#else
static idCVar s_driver("s_driver",
                       "oss",
                       CVAR_SYSTEM | CVAR_ARCHIVE | CVAR_ROM,
                       "sound driver. only OSS is supported in this build");
#endif
...
void idAudioHardwareOSS::ExtractOSSVersion(int version, idStr& str) const {
  sprintf(str, "%d.%d.%d", (version & 0xFF0000) >> 16, (version & 0xFF00) >> 8,
          version & 0xFF);
}
...
though OSS API docs (1.1) advertise AFMT_S32_LE, AFMT_S16_LE is the only output
format I've found in kernel emu10k1 headers

BSD NOTE: With the GNU library, you can use free to free the blocks that
memalign, posix_memalign, and valloc return. That does not work in BSD,
...
bool idAudioHardwareOSS::Initialize() {
  ...
  // open device ------------------------------------------------
  if ((m_audio_fd = open(s_device.GetString(), O_WRONLY | O_NONBLOCK, 0)) ==
      -1) {
    m_audio_fd = 0;
    common->Warning("failed to open sound device '%s': %s",
                    s_device.GetString(), strerror(errno));
    InitFailed();
    return false;
  }
  // make it blocking - so write overruns don't fail with 'Resource temporarily
  // unavailable'
  int flags;
  if ((flags = fcntl(m_audio_fd, F_GETFL)) == -1) {
    common->Warning("failed fcntl F_GETFL on sound device '%s': %s",
                    s_device.GetString(), strerror(errno));
    InitFailed();
    return false;
  }
  ...
  if (fcntl(m_audio_fd, F_SETFL, flags) == -1) {
    common->Warning("failed to clear O_NONBLOCK on sound device '%s': %s",
                    s_device.GetString(), strerror(errno));
    InitFailed();
    return false;
  }
  ...
  if (ioctl(m_audio_fd, OSS_SYSINFO, &si) == -1) {
    common->Printf(
        "ioctl SNDCTL_SYSINFO failed: %s\nthis ioctl is only available in "
        "OSS/Linux implementation. If you run OSS/Free, don't bother.",
        strerror(errno));
  } else {
  ...
  if (ioctl(m_audio_fd, SNDCTL_DSP_SETFMT, &m_sample_format) == -1) {
    common->Warning("ioctl SNDCTL_DSP_SETFMT %d failed: %s",
                    requested_sample_format, strerror(errno));
    InitFailed();
    return false;
  }
  if (m_sample_format != requested_sample_format) {
    common->Warning(
        "ioctl SNDCTL_DSP_SETFMT failed to get the requested sample format %d, "
        "got %d",
        requested_sample_format, m_sample_format);
    InitFailed();
    return false;
  }
  ...
  if (idSoundSystemLocal::s_numberOfSpeakers.GetInteger() != 6 &&
      idSoundSystemLocal::s_numberOfSpeakers.GetInteger() != 2) {
  ...
  if (ioctl(m_audio_fd, SNDCTL_DSP_CHANNELS, &m_channels) == -1) {
    common->Warning("ioctl SNDCTL_DSP_CHANNELS %d failed: %s",
  ...
  if (m_channels !=
      (unsigned int)idSoundSystemLocal::s_numberOfSpeakers.GetInteger()) {
    common->Warning(
        "ioctl SNDCTL_DSP_CHANNELS failed to get the %d requested channels, "
        "got %d",
        idSoundSystemLocal::s_numberOfSpeakers.GetInteger(), m_channels);
    if (m_channels != 2 &&
        idSoundSystemLocal::s_numberOfSpeakers.GetInteger() != 2) {
      // we didn't request 2 channels, some drivers reply 1 channel on error but
      // may still let us still get 2 if properly asked
      m_channels = 2;
      if (ioctl(m_audio_fd, SNDCTL_DSP_CHANNELS, &m_channels) == -1) {
        common->Warning("ioctl SNDCTL_DSP_CHANNELS fallback to 2 failed: %s",
                        strerror(errno));
        InitFailed();
        return false;
      }
  ...
  assert((int)m_channels ==
         idSoundSystemLocal::s_numberOfSpeakers.GetInteger());

  // sampling rate ------------------------------------------------
  m_speed = PRIMARYFREQ;
  if (ioctl(m_audio_fd, SNDCTL_DSP_SPEED, &m_speed) == -1) {
    common->Warning("ioctl SNDCTL_DSP_SPEED %d failed: %s", PRIMARYFREQ,
                    strerror(errno));
    InitFailed();
    return false;
  }
  // instead of an exact match, do a very close to
  // there is some horrible Ensonic ES1371 which replies 44101 for a 44100
  // request
  if (abs(m_speed - PRIMARYFREQ) > 5) {
    common->Warning(
        "ioctl SNDCTL_DSP_SPEED failed to get the requested frequency %d, got "
        "%d",
        PRIMARYFREQ, m_speed);
    InitFailed();
    return false;
  ...
  // output buffer ------------------------------------------------
  // allocate a final buffer target, the sound engine locks, writes, and we
  // write back to the device we want m_buffer_size ( will have to rename those
  // ) ROOM_SLICES_IN_BUFFER is fixed ( system default, 10 ) MIXBUFFER_SAMPLES
  // is the number of samples found in a slice each sample is m_channels *
  // sizeof( float ) bytes in AsyncUpdate we only write one block at a time, so
  // we'd only need to have a final mix buffer sized of a single block
  ...
  // toggle off before toggling on. that's what OSS source code samples
  // recommends
  int flag = 0;
  if (ioctl(m_audio_fd, SNDCTL_DSP_SETTRIGGER, &flag) == -1) {
    common->Warning("ioctl SNDCTL_DSP_SETTRIGGER 0 failed: %s",
                    strerror(errno));
  }
  flag = PCM_ENABLE_OUTPUT;
  if (ioctl(m_audio_fd, SNDCTL_DSP_SETTRIGGER, &flag) == -1) {
    common->Warning("ioctl SNDCTL_DSP_SETTRIGGER PCM_ENABLE_OUTPUT failed: %s",
                    strerror(errno));
  }
...
bool idAudioHardwareOSS::Flush(void) {
  ...
  m_freeWriteChunks =
      (ospace.bytes * MIXBUFFER_CHUNKS) / (MIXBUFFER_SAMPLES * m_channels * 2);
  if (m_writeChunks) {
...
rely on m_freeWriteChunks which has been set in Flush() before engine did the
mixing for this MIXBUFFER_SAMPLE
===============
*/
void idAudioHardwareOSS::Write(bool flushing) {
  ...
  if (!flushing && m_writeChunks) {
    ...
    // otherwise that last remaining chunk that was never flushed out to the
    // audio device has just been overwritten
    Sys_Printf(
        "idAudioHardwareOSS::Write: %d samples were overflowed and dropped\n",
  ...
  int pos = (int)m_buffer + (MIXBUFFER_CHUNKS - m_writeChunks) * m_channels *
                                2 * MIXBUFFER_SAMPLES / MIXBUFFER_CHUNKS;
  int len = Min(m_writeChunks, m_freeWriteChunks) * m_channels * 2 *
            MIXBUFFER_SAMPLES / MIXBUFFER_CHUNKS;
  assert(len > 0);

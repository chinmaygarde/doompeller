...
static int vidmode_MajorVersion = 0,
           vidmode_MinorVersion = 0;  // major and minor of XF86VidExtensions
...
void GLimp_SaveGamma() {
  ...
  XF86VidModeGetGammaRamp(dpy, scrnum, save_rampsize, save_red, save_green,
                          save_blue);
}
...
void GLimp_RestoreGamma() {
  ...
  XF86VidModeSetGammaRamp(dpy, scrnum, save_rampsize, save_red, save_green,
                          save_blue);
...
gamma ramp is generated by the renderer from r_gamma and r_brightness for 256
elements the size of the gamma ramp can not be changed on X (I need to confirm
this)
=================
*/
  ...
  if (dpy) {
    ...
    if (size > 256) {
      ...
      for (i = 0; i < size - 1; i++) {
        ...
        r_i = (int)floor(r_f);
        r_f -= (float)r_i;
        l_red[i] = (int)round((1.0f - r_f) * (float)red[r_i] +
                              r_f * (float)red[r_i + 1]);
        l_green[i] = (int)round((1.0f - r_f) * (float)green[r_i] +
                                r_f * (float)green[r_i + 1]);
        l_blue[i] = (int)round((1.0f - r_f) * (float)blue[r_i] +
                               r_f * (float)blue[r_i + 1]);
      }
...
**   I found out that on some hosts some operations would raise X errors
*(GLXUnsupportedPrivateRequest)
**   but those don't seem to be fatal .. so the default would be to just ignore
*them
**   our implementation mimics the default handler behaviour (not completely
*cause I'm lazy)
*/
...
#define ATTR_ALPHA_IDX 13
  ...
  // Get video mode list
  if (!XF86VidModeQueryVersion(dpy, &vidmode_MajorVersion,
                               &vidmode_MinorVersion)) {
    vidmode_ext = false;
  ...
  if (vidmode_ext) {
    ...
    if (a.fullScreen) {
      ...
      for (i = 0; i < num_vidmodes; i++) {
        if (a.width > vidmodes[i]->hdisplay || a.height > vidmodes[i]->vdisplay)
          continue;
  ...
  for (i = 0; i < 16; i++) {
    ...
    common->Printf(
        "Using %d/%d/%d Color bits, %d Alpha bits, %d depth, %d stencil "
        "display.\n",
        attrib[ATTR_RED_IDX], attrib[ATTR_GREEN_IDX], attrib[ATTR_BLUE_IDX],
        attrib[ATTR_ALPHA_IDX], attrib[ATTR_DEPTH_IDX],
  ...
  win =
      XCreateWindow(dpy, root, 0, 0, actualWidth, actualHeight, 0,
...
int Sys_GetVideoRam(void) {
  ...
  if (XNVCTRLQueryVersion(l_dpy, &major, &minor)) {
    ...
    if (XNVCTRLIsNvScreen(l_dpy, l_scrnum)) {
      if (XNVCTRLQueryAttribute(l_dpy, l_scrnum, 0, NV_CTRL_VIDEO_RAM,
                                &value)) {
        run_once = value / 1024;
        return run_once;
      } else {

...
// time mouse was last reset, we ignore the first 50ms of the mouse to allow
// settling of events
...
void Sys_InitInput(void) {
  ...
  assert(dpy);
  cmdSystem->AddCommand("in_clear", IN_Clear_f, CMD_FL_SYSTEM,
                        "reset the input keys");
  ...
  common->Printf(
      ...
      XkbMajorVersion, XkbMinorVersion, major_in_out, minor_in_out,
      ret ? "OK" : "Not compatible");
  if (ret) {
    ret = XkbQueryExtension(dpy, &opcode_rtrn, &event_rtrn, &error_rtrn,
...
static void Sys_XInstallGrabs(void) {
  ...
  XWarpPointer(dpy, None, win, 0, 0, 0, 0, glConfig.vidWidth / 2,
  ...
  XGrabPointer(dpy, win, False, MOUSE_MASK, GrabModeAsync, GrabModeAsync, win,
               None, CurrentTime);
...
static bool Sys_XRepeatPress(XEvent* event) {
  ...
  if (Sys_XPendingInput()) {
    ...
    if ((peekevent.type == KeyPress) &&
      ...
      lookupRet = XLookupString((XKeyEvent*)&peekevent, buf, sizeof(buf),
                                &keysym, NULL);
      if (lookupRet > 0) {
        ...
        // )
        Posix_QueEvent(SE_KEY, s_scantokey[peekevent.xkey.keycode], true, 0,
                       NULL);
        Posix_QueEvent(SE_KEY, s_scantokey[peekevent.xkey.keycode], false, 0,
                       NULL);
      }
    }
  }
    ...
    switch (event.type) {
      ...
      case MotionNotify:
        ...
        if (in_dgamouse.GetBool()) {
          ...
          // if we overflow here, we'll get a warning, but the delta will be
          // completely processed anyway
          Posix_AddMousePollEvent(M_DELTAX, dx);
          if (!Posix_AddMousePollEvent(M_DELTAY, dy))
            return;
        } else {
          // if it's a center motion, we've just returned from our warp
          // FIXME: we generate mouse delta on wrap return, but that lags us
          // quite a bit from the initial event..
...
unsigned char Sys_MapCharForKey(int _key) {
  ...
  // query the current keyboard group, must be passed as bit 13-14 in the
  // constructed XEvent see X Keyboard Extension library specifications
  ...
  lookupRet =
      XLookupString((XKeyEvent*)&event, buf, sizeof(buf), &keysym, NULL);
  if (lookupRet <= 0) {

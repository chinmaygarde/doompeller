...
typedef int (*cnv_func_t)(adev_p adev,
                          dmap_p dmap,
                          void** srcp,
                          int* srcl,
                          void** tgtp,
...
struct audio_driver {
  ...
  void (*output_block)(int dev,
                       oss_native_ulong buf,
                       int count,
                       int fragsize,
                       int intrflag);
  void (*start_input)(int dev,
                      oss_native_ulong buf,
                      int count,
                      int fragsize,
                      int intrflag);
  ...
  int (*copy_user)(int dev,
                   char* localbuf,
                   int localoffs,
                   WR_BUF_CONST snd_rw_buf* userbuf,
                   int useroffs,
                   int* len,
                   int max_space);
  ...
  void (*postprocess_write)(
      int dev); /* Device spesific postprocessing for written data */
  void (*preprocess_read)(
      int dev); /* Device spesific preprocessing for read data */
...
#define DMA_NOSOFTOSS \
  0x8000000 /* Don't install SoftOSS automatically for this device */
...
#define VMIX_MULTIFRAG \
  0x00000001 /* More than 2 fragments required (causes longer latencies) */
  int src_rate;
  int src_ratio;
  pid_t pid;
  char cmd[16];

...
void idInternalCVar::UpdateCheat(void) {
  ...
  if (flags & (CVAR_NOCHEAT | CVAR_INIT | CVAR_ROM | CVAR_ARCHIVE |
               CVAR_USERINFO | CVAR_SERVERINFO | CVAR_NETWORKSYNC)) {
    flags &= ~CVAR_CHEAT;
  } else {
    flags |= CVAR_CHEAT;
  }
...
class idCVarSystemLocal : public idCVarSystem {
 public:
  ...
  virtual void SetCVarString(const char* name,
                             const char* value,
                             int flags = 0);
  ...
  virtual void WriteFlaggedVariables(int flags,
                                     const char* setCmd,
                                     idFile* f) const;
...
void idCVarSystemLocal::Init(void) {
  ...
  cmdSystem->AddCommand("sets", SetS_f, CMD_FL_SYSTEM,
                        "sets a cvar and flags it as server info");
  cmdSystem->AddCommand("setu", SetU_f, CMD_FL_SYSTEM,
                        "sets a cvar and flags it as user info");
  cmdSystem->AddCommand("sett", SetT_f, CMD_FL_SYSTEM,
                        "sets a cvar and flags it as tool");
  cmdSystem->AddCommand("seta", SetA_f, CMD_FL_SYSTEM,
  ...
  cmdSystem->AddCommand("cvar_restart", Restart_f, CMD_FL_SYSTEM,
...
void idCVarSystemLocal::ListByFlags(const idCmdArgs& args, cvarFlags_t flags) {
  ...
  if (idStr::Icmp(args.Argv(argNum), "-") == 0 ||
      idStr::Icmp(args.Argv(argNum), "/") == 0) {
    if (idStr::Icmp(args.Argv(argNum + 1), "help") == 0 ||
        idStr::Icmp(args.Argv(argNum + 1), "?") == 0) {
      argNum = 3;
      show = SHOW_DESCRIPTION;
    } else if (idStr::Icmp(args.Argv(argNum + 1), "type") == 0 ||
               idStr::Icmp(args.Argv(argNum + 1), "range") == 0) {
      argNum = 3;
      show = SHOW_TYPE;
  ...
  switch (show) {
    ...
    case SHOW_DESCRIPTION: {
      ...
      for (i = 0; i < cvarList.Num(); i++) {
        cvar = cvarList[i];
        common->Printf(FORMAT_STRING S_COLOR_WHITE "%s\n",
                       cvar->nameString.c_str(),
                       CreateColumn(cvar->GetDescription(),
                                    NUM_DESCRIPTION_CHARS, indent, string));
      }
      break;
    }
    case SHOW_TYPE: {
      for (i = 0; i < cvarList.Num(); i++) {
        ...
        if (cvar->GetFlags() & CVAR_BOOL) {
          ...
          if (cvar->GetMinValue() < cvar->GetMaxValue()) {
            common->Printf(FORMAT_STRING S_COLOR_GREEN "int " S_COLOR_WHITE
                                                       "[%d, %d]\n",
                           cvar->GetName(), (int)cvar->GetMinValue(),
                           (int)cvar->GetMaxValue());
          } else {
          ...
          if (cvar->GetMinValue() < cvar->GetMaxValue()) {
            common->Printf(FORMAT_STRING S_COLOR_RED "float " S_COLOR_WHITE
                                                     "[%s, %s]\n",
                           cvar->GetName(), idStr(cvar->GetMinValue()).c_str(),
                           idStr(cvar->GetMaxValue()).c_str());
          } else {
            common->Printf(FORMAT_STRING S_COLOR_RED "float\n",
                           cvar->GetName());
          }
        } else if (cvar->GetValueStrings()) {
          common->Printf(FORMAT_STRING S_COLOR_WHITE "string " S_COLOR_WHITE
                                                     "[",
                         cvar->GetName());

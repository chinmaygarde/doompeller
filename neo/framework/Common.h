...
class idCommon {
 public:
  ...
  // otherwise pass the command line in a single string (without executable
  // name)
  ...
  // Prints message to the console, which may cause a screen update if
  // com_refreshOnPrint is set.
  ...
  // Prints WARNING %s message and adds the warning message to a queue for
  // printing later on.
  virtual void Warning(const char* fmt, ...)
      id_attribute((format(printf, 2, 3))) = 0;

  // Prints WARNING %s message in yellow that only shows up if the "developer"
  // cvar is set.
  ...
  // Fatal errors quit all the way to a system dialog box, which is appropriate
  // for static internal errors or cases where the system may be corrupted.

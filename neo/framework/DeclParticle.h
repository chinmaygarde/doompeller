...
typedef struct {
  ...
  idRandom random;
  idVec3
      origin;  // dynamic smoke particles can have individual origins and axis
  idMat3 axis;

  float age;  // in seconds, calculated as fraction * stage->particleLife
  idRandom originalRandom;   // needed so aimed particles can reset the random
                             // for another origin calculation
...
class idParticleStage {
 public:
  ...
  void Default();
  virtual int NumQuadsPerParticle()
      const;  // includes trails and cross faded animations
  ...
  int ParticleVerts(particleGen_t* g,
                    const idVec3 origin,
  ...
  bool randomDistribution;  // randomly orient the quad on emission ( defaults
                            // to true )
  bool entityColor;  // force color from render entity ( fadeColor is still
                     // valid )
  ...
  prtCustomPth_t
      customPathType;  // use custom C code routines for determining the origin
  ...
  idParticleParm
      rotationSpeed;  // half the particles will have negative rotation speeds

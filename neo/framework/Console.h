...
===============================================================================
        ...
        never be used to convey actual game information to the user, which
should always be done through a GUI.
...
class idConsole {
 public:
  ...
  // can't be combined with Init, because Init happens before renderer is
  // started
  virtual void LoadGraphics() = 0;

  virtual bool ProcessEvent(const struct sysEvent_s* event,
                            bool forceAccept) = 0;

  // the system code can release the mouse pointer when the console is active
  virtual bool Active(void) = 0;

  // clear the timers on any recent prints that are displayed in the notify
  // lines
  virtual void ClearNotifyLines(void) = 0;

  // some console commands, like timeDemo, will force the console closed before
  // they start

...
typedef struct {
  ...
  usercmd_t
      mapSpawnUsercmd[MAX_ASYNC_CLIENTS];  // needed for tracking delta angles
} mapSpawnData_t;

typedef enum { TD_NO, TD_YES, TD_YES_THEN_QUIT } timeDemo_t;

const int USERCMD_PER_DEMO_FRAME = 2;
const int CONNECT_TRANSMIT_TIME = 1000;
const int MAX_LOGGED_USERCMDS =
    60 * 60 * 60;  // one hour of single player, 15 minutes of four player
...
class idSessionLocal : public idSession {
 public:
  ...
  virtual const char* MessageBox(msgBoxType_t type,
                                 const char* message,
  ...
  virtual void DownloadProgressBox(backgroundDownload_t* bgl,
                                   const char* title,
  ...
  virtual bool CheckKey(const char* key,
                        bool netConnect,
  ...
  void DispatchCommand(idUserInterface* gui,

...
idCVar idSessionLocal::gui_configServerRate("gui_configServerRate",
                                            "0",
                                            CVAR_GUI | CVAR_ARCHIVE | CVAR_ROM |
                                                CVAR_INTEGER,
                                            "");
...
void idSessionLocal::StartMenu(bool playIntro) {
  ...
  if (fileSystem->HasD3XP()) {
    guiMainMenu->SetStateString(
        "game_list", common->GetLanguageDict()->GetString("#str_07202"));
  } else {
    guiMainMenu->SetStateString(
        "game_list", common->GetLanguageDict()->GetString("#str_07212"));
  }
...
void idSessionLocal::SetModsMenuGuiVars(void) {
  ...
  for (i = 0; i < list->GetNumMods(); i++) {
    guiActive->SetStateString(va("modsList_item_%i", i),
                              list->GetDescription(i));
...
#else
  guiMainMenu->SetStateString(
      "nightmare", cvarSystem->GetCVarBool("g_nightmare") ? "1" : "0");
#endif
  guiMainMenu->SetStateString("browser_levelshot",
                              "guis/assets/splash/pdtempa");
...
bool idSessionLocal::HandleSaveGameMenuCommand(idCmdArgs& args, int& icmd) {
  ...
  if (!idStr::Icmp(cmd, "deleteGame")) {
    ...
    if (choice >= 0 && choice < loadGameList.Num()) {
      fileSystem->RemoveFile(
          va("savegames/%s.save", loadGameList[choice].c_str()));
      fileSystem->RemoveFile(
          va("savegames/%s.tga", loadGameList[choice].c_str()));
      fileSystem->RemoveFile(
          va("savegames/%s.txt", loadGameList[choice].c_str()));
      SetSaveGameGuiVars();
      guiActive->StateChanged(com_frameTime);
    }
    return true;
  ...
  if (!idStr::Icmp(cmd, "updateSaveGameInfo")) {
    ...
    if (choice >= 0 && choice < loadGameList.Num()) {
      ...
      ID_TIME_T timeStamp;
      fileSystem->ReadFile(
          va("savegames/%s.save", loadGameList[choice].c_str()), NULL,
          &timeStamp);
...
void idSessionLocal::HandleRestartMenuCommands(const char* menuCommand) {
  ...
  for (icmd = 0; icmd < args.Argc();) {
    ...
    if (!idStr::Icmp(cmd, "restart")) {
      if (!LoadGame(
              GetAutoSaveName(mapSpawnData.serverInfo.GetString("si_map")))) {
...
void idSessionLocal::UpdateMPLevelShot(void) {
  ...
  fileSystem->FindMapScreenshot(cvarSystem->GetCVarString("si_map"), screenshot,
                                MAX_STRING_CHARS);
...
void idSessionLocal::HandleMainMenuCommands(const char* menuCommand) {
  ...
  for (icmd = 0; icmd < args.Argc();) {
    ...
    if (!idStr::Icmp(cmd, "startGame")) {
      cvarSystem->SetCVarInteger("g_skill",
                                 guiMainMenu->State().GetInt("skill"));
      if (icmd < args.Argc()) {
    ...
    if (!idStr::Icmp(cmd, "MAPScan")) {
      ...
      cvarSystem->SetCVarString("si_map",
                                (dict ? dict->GetString("path") : ""));
    ...
    if (!idStr::Icmp(cmd, "startMultiplayer")) {
      ...
      cvarSystem->SetCVarBool("net_LANServer",
                              guiActive->State().GetBool("server_type"));
      if (gui_configServerRate.GetInteger() > 0) {
        // guess the best rate for upstream, number of internet clients
        if (gui_configServerRate.GetInteger() == 5 ||
          ...
          if (n_clients > maxclients) {
            if (MessageBox(
                    MSG_OKCANCEL,
                    va(common->GetLanguageDict()->GetString("#str_04315"),
                       dedicated ? maxclients : Min(8, maxclients + 1)),
                    common->GetLanguageDict()->GetString("#str_04316"), true,
                    "OK")[0] == '\0') {
              continue;
            }
      ...
      if (!dedicated && !cvarSystem->GetCVarBool("net_LANServer") &&
        ...
        if (!MessageBox(MSG_YESNO,
                        common->GetLanguageDict()->GetString("#str_00100625"),
                        common->GetLanguageDict()->GetString("#str_00100626"),
                        true, "yes")[0]) {
          continue;
        }
      }
    ...
    if (!idStr::Icmp(cmd, "bind")) {
      if (args.Argc() - icmd >= 2) {
        ...
        if (idKeyInput::NumBinds(bind) >= 2 &&
            !idKeyInput::KeyIsBoundTo(key, bind)) {
...
#ifdef _WIN32
          MessageBox(MSG_OK, common->GetLanguageDict()->GetString("#str_04142"),
                     common->GetLanguageDict()->GetString("#str_04141"), true);
#else
          // a message that doesn't mention the windows control panel
          MessageBox(MSG_OK, common->GetLanguageDict()->GetString("#str_07230"),
                     common->GetLanguageDict()->GetString("#str_04141"), true);
#endif
        }
      }
        ...
        if (cvarSystem->GetCVarBool("s_useEAXReverb")) {
          ...
          switch (eax) {
            case 2:
              ...
              MessageBox(
                  MSG_OK, common->GetLanguageDict()->GetString("#str_07238"),
                  common->GetLanguageDict()->GetString("#str_07231"), true);
              break;
            case 1:
              // when you restart
              MessageBox(
                  MSG_OK, common->GetLanguageDict()->GetString("#str_04137"),
                  common->GetLanguageDict()->GetString("#str_07231"), true);
              break;
            case -1:
              ...
              // disabled
              MessageBox(
                  MSG_OK, common->GetLanguageDict()->GetString("#str_07233"),
                  common->GetLanguageDict()->GetString("#str_07231"), true);
              break;
            case 0:
              cvarSystem->SetCVarBool("s_useEAXReverb", false);
              // not available
              MessageBox(
                  MSG_OK, common->GetLanguageDict()->GetString("#str_07232"),
                  common->GetLanguageDict()->GetString("#str_07231"), true);
              break;
          }
        } else {
          ...
          // when you restart
          MessageBox(MSG_OK, common->GetLanguageDict()->GetString("#str_04137"),
                     common->GetLanguageDict()->GetString("#str_07231"), true);
        }
      }
      if (!vcmd.Icmp("drivar")) {
...
void idSessionLocal::MenuEvent(const sysEvent_t* event) {
  ...
  if (!menuCommand || !menuCommand[0]) {
    ...
    if (event->evType == SE_KEY && event->evValue2 == 1 &&
        event->evValue >= K_F1 && event->evValue <= K_F12) {
  ...
  switch (type) {
    ...
    case MSG_OK:
      guiMsg->SetStateString(
          "mid", common->GetLanguageDict()->GetString("#str_04339"));
    ...
    case MSG_ABORT:
      guiMsg->SetStateString(
          "mid", common->GetLanguageDict()->GetString("#str_04340"));
      guiMsg->SetStateString("visible_mid", "1");
      guiMsg->SetStateString("visible_left", "0");
      guiMsg->SetStateString("visible_right", "0");
      break;
    case MSG_OKCANCEL:
      guiMsg->SetStateString(
          "left", common->GetLanguageDict()->GetString("#str_04339"));
      guiMsg->SetStateString(
          "right", common->GetLanguageDict()->GetString("#str_04340"));
      guiMsg->SetStateString("visible_mid", "0");
    ...
    case MSG_YESNO:
      guiMsg->SetStateString(
          "left", common->GetLanguageDict()->GetString("#str_04341"));
      guiMsg->SetStateString(
          "right", common->GetLanguageDict()->GetString("#str_04342"));
      guiMsg->SetStateString("visible_mid", "0");
      guiMsg->SetStateString("visible_left", "1");
      guiMsg->SetStateString("visible_right", "1");
      break;
    ...
    case MSG_CDKEY:
      ...
      guiMsg->SetStateString("visible_hasxp",
                             fileSystem->HasD3XP() ? "1" : "0");
      ...
      if (bgl->completed) {
        ...
        if (dltotal) {
          ...
          if (lapsed < 2000) {
            ...
            sprintf(sETA, "%.0f sec",
                    ((float)dltotal / (float)dlnow - 1.0f) * lapsed / 1000);
            sprintf(sMsg, "%s / %s ( %s - %s )", sNow.c_str(), sTotal.c_str(),
                    sBW.c_str(), sETA.c_str());
          }
        } else {
          if (lapsed < 2000) {
            sMsg = sNow;
        ...
        if (dltotal) {
          guiMsg->SetStateString(
              "progress",
              va("%d", progress_start +
                           dlnow * (progress_end - progress_start) / dltotal));
        } else {
...
void idSessionLocal::HandleMsgCommands(const char* menuCommand) {
  ...
  if (idStr::Icmp(menuCommand, "mid") == 0 ||
      idStr::Icmp(menuCommand, "left") == 0) {
    guiActive = guiMsgRestore;
    guiMsgRestore = NULL;
    msgRunning = false;
    msgRetIndex = 0;
...
void idSessionLocal::HandleNoteCommands(const char* menuCommand) {
  ...
  if (idStr::Icmp(menuCommand, "note") == 0 && mapSpawned) {
    ...
    if (extended) {
      ...
      const idDecl* mapDecl = declManager->FindType(
          DECL_ENTITYDEF, mapSpawnData.serverInfo.GetString("si_map"), false);
      const idDeclEntityDef* mapInfo =
          static_cast<const idDeclEntityDef*>(mapDecl);
      ...
      if (count == 0) {
        ...
        for (i = 0; i < count; i++) {
          ...
          workName = fileName + "/";
          workName += guiTakeNotes->State().GetString(
              va("person_item_%i", person), "Nobody");
    ...
    for (i = 0; i < fileList.Num(); i++) {
      ...
      sprintf(str, "recordViewNotes \"%s\" \"%s\" \"%s\"\n", workName.c_str(),
              noteNum.c_str(), guiTakeNotes->State().GetString("note"));

      cmdSystem->BufferCommandText(CMD_EXEC_NOW, str);
      cmdSystem->ExecuteCommandBuffer();

      UpdateScreen();
      renderSystem->TakeScreenshot(renderSystem->GetScreenWidth(),
                                   renderSystem->GetScreenHeight(), shotName, 1,
                                   NULL);
    }
    noteNumber++;
...
void idSessionLocal::SetCDKeyGuiVars(void) {
  ...
  guiMainMenu->SetStateString(
      "str_d3key_state",
      common->GetLanguageDict()->GetString(va("#str_071%d", 86 + cdkey_state)));
  guiMainMenu->SetStateString(
      "str_xpkey_state",
      common->GetLanguageDict()->GetString(va("#str_071%d", 86 + xpkey_state)));
}

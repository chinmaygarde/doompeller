...
void SCR_DrawTextRightAlign(float& y, const char* text, ...) {
  ...
  va_end(argptr);
  renderSystem->DrawSmallStringExt(635 - i * SMALLCHAR_WIDTH, y + 2, string,
                                   colorWhite, true,
                                   localConsole.charSetShader);
  y += SMALLCHAR_HEIGHT + 4;
}
...
float SCR_DrawFPS(float y) {
  ...
  if (index > FPS_FRAMES) {
    ...
    renderSystem->DrawBigStringExt(635 - w, idMath::FtoiFast(y) + 2, s,
                                   colorWhite, true,
                                   localConsole.charSetShader);
  }
...
float SCR_DrawMemoryUsage(float y) {
  ...
  Mem_GetStats(allocs);
  SCR_DrawTextRightAlign(y, "total allocated memory: %4d, %4dkB", allocs.num,
                         allocs.totalSize >> 10);

  Mem_GetFrameStats(allocs, frees);
  SCR_DrawTextRightAlign(y, "frame alloc: %4d, %4dkB  frame free: %4d, %4dkB",
                         allocs.num, allocs.totalSize >> 10, frees.num,
                         frees.totalSize >> 10);
...
float SCR_DrawAsyncStats(float y) {
  ...
  if (idAsyncNetwork::server.IsActive()) {
    SCR_DrawTextRightAlign(y, "server delay = %d msec",
                           idAsyncNetwork::server.GetDelay());
    SCR_DrawTextRightAlign(y, "total outgoing rate = %d KB/s",
                           idAsyncNetwork::server.GetOutgoingRate() >> 10);
    SCR_DrawTextRightAlign(y, "total incoming rate = %d KB/s",
    ...
    for (i = 0; i < MAX_ASYNC_CLIENTS; i++) {
      ...
      outgoingCompression =
          idAsyncNetwork::server.GetClientOutgoingCompression(i);
      incomingCompression =
          idAsyncNetwork::server.GetClientIncomingCompression(i);

      if (outgoingRate != -1 && incomingRate != -1) {
        SCR_DrawTextRightAlign(y,
                               "client %d: out rate = %d B/s (% -2.1f%%), in "
                               "rate = %d B/s (% -2.1f%%)",
                               i, outgoingRate, outgoingCompression,
                               incomingRate, incomingCompression);
      }
    }
    ...
    if (outgoingRate != -1 && incomingRate != -1) {
      SCR_DrawTextRightAlign(
          y, "out rate = %d B/s (% -2.1f%%), in rate = %d B/s (% -2.1f%%)",
    ...
    SCR_DrawTextRightAlign(y, "packet loss = %d%%, client prediction = %d",
                           (int)idAsyncNetwork::client.GetIncomingPacketLoss(),
                           idAsyncNetwork::client.GetPrediction());

    SCR_DrawTextRightAlign(y, "predicted frames: %d",
...
float SCR_DrawSoundDecoders(float y) {
  ...
  while ((index = soundSystem->GetSoundDecoderInfo(index, decoderInfo)) != -1) {
    ...
    SCR_DrawTextLeftAlign(y, "%3d: %3d%% (%1.2f) %s: %s (%dkB)",
                          numActiveDecoders, percent, decoderInfo.lastVolume,
                          decoderInfo.format.c_str(), decoderInfo.name.c_str(),
                          decoderInfo.numBytes >> 10);
    numActiveDecoders++;
  }
  return y;
}
...
void idConsoleLocal::Init(void) {
  ...
  cmdSystem->AddCommand("clear", Con_Clear_f, CMD_FL_SYSTEM,
                        "clears the console");
  cmdSystem->AddCommand("conDump", Con_Dump_f, CMD_FL_SYSTEM,
...
bool idConsoleLocal::ProcessEvent(const sysEvent_t* event, bool forceAccept) {
  bool consoleKey;
  consoleKey =
      event->evType == SE_KEY && (event->evValue == Sys_GetConsoleKey(false) ||
                                  event->evValue == Sys_GetConsoleKey(true));
  ...
  if (event->evType == SE_CHAR) {
    ...
    if (event->evValue != Sys_GetConsoleKey(false) &&
        event->evValue != Sys_GetConsoleKey(true)) {
...
void idConsoleLocal::Linefeed() {
  ...
  for (i = 0; i < LINE_WIDTH; i++) {
    text[(current % TOTAL_LINES) * LINE_WIDTH + i] =
        (idStr::ColorIndex(C_COLOR_CYAN) << 8) | ' ';
  }
}
...
void idConsoleLocal::DrawInput() {
  ...
  if (consoleField.GetAutoCompleteLength() != 0) {
    autoCompleteLength =
        strlen(consoleField.GetBuffer()) - consoleField.GetAutoCompleteLength();
    ...
    if (autoCompleteLength > 0) {
      ...
      renderSystem->DrawStretchPic(
          2 * SMALLCHAR_WIDTH +
              consoleField.GetAutoCompleteLength() * SMALLCHAR_WIDTH,
          y + 2, autoCompleteLength * SMALLCHAR_WIDTH, SMALLCHAR_HEIGHT - 2, 0,
          0, 0, 0, whiteShader);
    }
  }
  ...
  consoleField.Draw(2 * SMALLCHAR_WIDTH, y, SCREEN_WIDTH - 3 * SMALLCHAR_WIDTH,
                    true, charSetShader);
}
...
void idConsoleLocal::DrawNotify() {
  ...
  for (i = current - NUM_CON_TIMES + 1; i <= current; i++) {
    ...
    for (x = 0; x < LINE_WIDTH; x++) {
      ...
      renderSystem->DrawSmallChar((x + 1) * SMALLCHAR_WIDTH, v,
                                  text_p[x] & 0xff, localConsole.charSetShader);
    }
...
void idConsoleLocal::DrawSolidConsole(float frac) {
  ...
  if (y < 1.0f) {
    ...
    renderSystem->DrawStretchPic(0, 0, SCREEN_WIDTH, y, 0, 1.0f - displayFrac,
                                 1, 1, consoleShader);
  }
  ...
  for (x = 0; x < i; x++) {
    renderSystem->DrawSmallChar(
        SCREEN_WIDTH - (i - x) * SMALLCHAR_WIDTH,
  ...
  if (display != current) {
    ...
    for (x = 0; x < LINE_WIDTH; x += 4) {
      renderSystem->DrawSmallChar((x + 1) * SMALLCHAR_WIDTH,
                                  idMath::FtoiFast(y), '^',
                                  localConsole.charSetShader);
    }
    y -= SMALLCHAR_HEIGHT;
  ...
  for (i = 0; i < rows; i++, y -= SMALLCHAR_HEIGHT, row--) {
    ...
    for (x = 0; x < LINE_WIDTH; x++) {
      ...
      renderSystem->DrawSmallChar((x + 1) * SMALLCHAR_WIDTH,
                                  idMath::FtoiFast(y), text_p[x] & 0xff,
                                  localConsole.charSetShader);
    }
  }

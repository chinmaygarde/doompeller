...
keyname_t keynames[] = {
    ...
    {"SEMICOLON", ';',
     "#str_07129"},  // because a raw semicolon separates commands
    {"APOSTROPHE", '\'',
     "#str_07130"},  // because a raw apostrophe messes with parsing
...
const char* idKeyInput::KeysFromBinding(const char* bind) {
  ...
  if (bind && *bind) {
    for (i = 0; i < MAX_KEYS; i++) {
      if (keys[i].binding.Icmp(bind) == 0) {
        if (keyName[0] != '\0') {
          idStr::Append(keyName, sizeof(keyName),
                        common->GetLanguageDict()->GetString("#str_07183"));
        }
  ...
  if (keyName[0] == '\0') {
    idStr::Copynz(keyName, common->GetLanguageDict()->GetString("#str_07133"),
                  sizeof(keyName));
  }
...
void idKeyInput::PreliminaryKeyEvent(int keynum, bool down) {
  ...
  if (down) {
    ...
    for (int i = 0; cheatCodes[i] != NULL; i++) {
      ...
      assert(l <= 16);
      if (idStr::Icmpn(lastKeys + 16 + (lastKeyIndex & 15) - l, cheatCodes[i],
                       l) == 0) {
...
void idKeyInput::Init(void) {
  ...
  cmdSystem->AddCommand("bind", Key_Bind_f, CMD_FL_SYSTEM,
                        "binds a command to a key",
                        idKeyInput::ArgCompletion_KeyName);
  cmdSystem->AddCommand(
      "bindunbindtwo", Key_BindUnBindTwo_f, CMD_FL_SYSTEM,
      "binds a key but unbinds it first if there are more than two binds");
  cmdSystem->AddCommand("unbind", Key_Unbind_f, CMD_FL_SYSTEM,
                        "unbinds any command from a key",
                        idKeyInput::ArgCompletion_KeyName);
  cmdSystem->AddCommand("unbindall", Key_Unbindall_f, CMD_FL_SYSTEM,
                        "unbinds any commands from all keys");
  cmdSystem->AddCommand("listBinds", Key_ListBinds_f, CMD_FL_SYSTEM,
                        "lists key bindings");
}

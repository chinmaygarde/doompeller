...
typedef enum { FS_READ = 0, FS_WRITE = 1, FS_APPEND = 2 } fsMode_t;
...
typedef enum { DLTYPE_URL, DLTYPE_FILE } dlType_t;
...
typedef enum { FILE_EXEC, FILE_OPEN } dlMime_t;

typedef enum { FIND_NO, FIND_YES, FIND_ADDON } findFile_t;
...
class idFileSystem {
 public:
  ...
  virtual const char* RelativePathToOSPath(
      const char* relativePath,
      const char* basePath = "fs_devpath") = 0;
  ...
  virtual idFile* OpenFileAppend(const char* filename,
                                 bool sync = false,
                                 const char* basePath = "fs_basepath") = 0;

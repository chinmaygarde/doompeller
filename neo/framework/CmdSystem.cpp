...
void idCmdSystemLocal::Init(void) {
  ...
  AddCommand("listSystemCmds", SystemList_f, CMD_FL_SYSTEM,
             "lists system commands");
  AddCommand("listRendererCmds", RendererList_f, CMD_FL_SYSTEM,
             "lists renderer commands");
  AddCommand("listSoundCmds", SoundList_f, CMD_FL_SYSTEM,
             "lists sound commands");
  ...
  for (prev = &commands; *prev; prev = &cmd->next) {
    ...
    if (idStr::Icmp(args.Argv(0), cmd->name) == 0) {
      ...
      *prev = cmd->next;
      ...
      if ((cmd->flags & (CMD_FL_CHEAT | CMD_FL_TOOL)) && session &&
          session->IsMultiplayer() &&
          !cvarSystem->GetCVarBool("net_allowCheats")) {
...
void idCmdSystemLocal::ArgCompletion_FolderExtension(
  ...
  if (string.Icmp(completionString) != 0) {
    ...
    for (extension = va_arg(argPtr, const char*); extension;
         extension = va_arg(argPtr, const char*)) {
...
void idCmdSystemLocal::ArgCompletion_DeclName(const idCmdArgs& args,
  ...
  for (i = 0; i < num; i++) {
    callback(idStr(args.Argv(0)) + " " +
             declManager->DeclByIndex((declType_t)type, i, false)->GetName());
  }
}

...
int idCompressor_BitStream::Compare(const byte* src1,
  ...
  if ((bitPtr1 & 7) == (bitPtr2 & 7) && maxBits > 16) {
    ...
    for (i = 0; i < maxBits; i++) {
      if (((src1[bitPtr1 >> 3] >> (bitPtr1 & 7)) ^
           (src2[bitPtr2 >> 3] >> (bitPtr2 & 7))) &
          1) {
        break;
      }
      bitPtr1++;
...
int idCompressor_RunLength::Write(const void* inData, int inLength) {
  ...
  while (readByte <= readLength) {
    ...
    for (nextBits = ReadBits(wordLength); nextBits == bits;
...
int idCompressor_RunLength_ZeroBased::Write(const void* inData, int inLength) {
  ...
  while (readByte <= readLength) {
    count = 0;
    for (bits = ReadBits(wordLength); bits == 0 && count < (1 << wordLength);
         bits = ReadBits(wordLength)) {
      count++;
    }
    if (count) {
...
void idCompressor_Huffman::AddRef(byte ch) {
  ...
  if (loc[ch] == NULL) { /* if this is the first transmission of this node */
    ...
    if (lhead->parent) {
      if (lhead->parent->left ==
          lhead) { /* lhead is guaranteed to by the NYT */
...
int idCompressor_Arithmetic::GetCurrentCount(void) {
  return (unsigned int)(((((long)code - low) + 1) * scale - 1) /
                        (((long)high - low) + 1));
}
...
void idCompressor_Arithmetic::RemoveSymbolFromStream(acSymbol_t* symbol) {
  ...
  while (true) {
    ...
    } else if ((low & AC_MSB2_MASK) == AC_MSB2_MASK &&
               (high & AC_MSB2_MASK) == 0) {
      code ^= AC_MSB2_MASK;
      low &= AC_MSB2_MASK - 1;
      high |= AC_MSB2_MASK;
...
bool idCompressor_LZSS::FindMatch(int startWord,
  ...
  for (i = hashTable[hash]; i >= bottom; i = hashNext[i]) {
    n = Compare(block, i * wordLength, block, startWord * wordLength,

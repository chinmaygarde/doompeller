...
typedef bool (*pureExclusionFunc_t)(const struct pureExclusion_s& excl,
                                    int l,
                                    const idStr& name);
...
bool excludeExtension(const pureExclusion_t& excl, int l, const idStr& name) {
  if (l > excl.extLen &&
      !idStr::Icmp(name.c_str() + l - excl.extLen, excl.ext)) {
    return true;
  }
  return false;
}
...
static pureExclusion_t pureExclusions[] = {
    ...
    {0, 0, "sound/levels/alphalabs2/uac_better_place.ogg", NULL,
     excludeFullName},
...
typedef enum { BINARY_UNKNOWN = 0, BINARY_YES, BINARY_NO } binaryStatus_t;
...
class idFileSystemLocal : public idFileSystem {
 public:
  ...
  virtual const char* RelativePathToOSPath(const char* relativePath,
                                           const char* basePath);
  virtual const char* BuildOSPath(const char* base,
                                  const char* game,
  ...
  virtual int ReadFile(const char* relativePath,
                       void** buffer,
                       ID_TIME_T* timestamp);
  virtual void FreeFile(void* buffer);
  virtual int WriteFile(const char* relativePath,
                        const void* buffer,
                        int size,
                        const char* basePath = "fs_savepath");
  virtual void RemoveFile(const char* relativePath);
  virtual idFile* OpenFileReadFlags(const char* relativePath,
                                    int searchFlags,
                                    pack_t** foundInPak = NULL,
  ...
  virtual void FindDLL(const char* basename,
  ...
  virtual int ValidateDownloadPakForChecksum(int checksum,
 ...
 private:
  ...
  bool loadedFileFromDir;  // set to true once a file was loaded from a
                           // directory - can't switch to pure anymore
  idList<int>
      restartChecksums;  // used during a restart to set things in right order
 ...
 private:
  ...
  int ListOSFiles(const char* directory,
                  const char* extension,
                  idStrList& list);
  FILE* OpenOSFile(const char* name,
                   const char* mode,
                   idStr* caseSensitiveName = NULL);
  ...
  // searches all the paks, no pure check
  pack_t* FindPakForFileChecksum(const char* relativePath,
                                 int fileChecksum,
                                 bool bReference);
  idFile_InZip* ReadFileFromZip(pack_t* pak,
  ...
  static size_t CurlWriteFunction(void* ptr,
                                  size_t size,
                                  size_t nmemb,
                                  void* stream);
  // curl_progress_callback in curl.h
  static int CurlProgressFunction(void* clientp,
                                  double dltotal,
                                  double dlnow,
                                  double ultotal,
                                  double ulnow);
...
idCVar idFileSystemLocal::fs_restrict("fs_restrict",
                                      "",
                                      CVAR_SYSTEM | CVAR_INIT | CVAR_BOOL,
                                      "");
idCVar idFileSystemLocal::fs_debug("fs_debug",
                                   "0",
...
idCVar idFileSystemLocal::fs_copyfiles(
    "fs_copyfiles",
    "0",
    CVAR_SYSTEM | CVAR_INIT | CVAR_INTEGER,
    "",
    0,
    4,
    idCmdSystem::ArgCompletion_Integer<0, 3>);
idCVar idFileSystemLocal::fs_basepath("fs_basepath",
                                      "",
                                      CVAR_SYSTEM | CVAR_INIT,
                                      "");
idCVar idFileSystemLocal::fs_savepath("fs_savepath",
                                      "",
                                      CVAR_SYSTEM | CVAR_INIT,
                                      "");
idCVar idFileSystemLocal::fs_cdpath("fs_cdpath",
                                    "",
                                    CVAR_SYSTEM | CVAR_INIT,
                                    "");
idCVar idFileSystemLocal::fs_devpath("fs_devpath",
                                     "",
                                     CVAR_SYSTEM | CVAR_INIT,
                                     "");
idCVar idFileSystemLocal::fs_game("fs_game",
                                  "",
                                  CVAR_SYSTEM | CVAR_INIT | CVAR_SERVERINFO,
                                  "mod path");
idCVar idFileSystemLocal::fs_game_base("fs_game_base",
                                       "",
                                       CVAR_SYSTEM | CVAR_INIT |
                                           CVAR_SERVERINFO,
...
#ifdef WIN32
idCVar idFileSystemLocal::fs_caseSensitiveOS("fs_caseSensitiveOS",
                                             "0",
...
idCVar idFileSystemLocal::fs_caseSensitiveOS("fs_caseSensitiveOS",
                                             ...
                                             "");
#endif
idCVar idFileSystemLocal::fs_searchAddons(
    "fs_searchAddons",
    "0",
    CVAR_SYSTEM | CVAR_BOOL,
...
================
*/
FILE* idFileSystemLocal::OpenOSFile(const char* fileName,
                                    const char* mode,
    ...
    for (i = 0; i < list.Num(); i++) {
      ...
      if (!entry.Icmp(fileName)) {
        ...
        if (fp) {
          ...
          common->Warning(
              "idFileSystemLocal::OpenFileRead: fs_caseSensitiveOS 1 could not "
              "open %s",
              entry.c_str());
        }
      }
...
===================
*/
const char* idFileSystemLocal::BuildOSPath(const char* base,
                                           const char* game,
                                           const char* relativePath) {
  ...
  if (fs_caseSensitiveOS.GetBool() || com_developer.GetBool()) {
    ...
    if (testPath.HasUpper()) {
      common->Warning("Non-portable: path contains uppercase characters: %s",
                      testPath.c_str());
...
const char* idFileSystemLocal::OSPathToRelativePath(const char* OSPath) {
  ...
  tempStr.ToLower();
  if ((strstr(tempStr, "//") || strstr(tempStr, "w:")) &&
      strstr(tempStr, "/doom/base/")) {
...
=====================
*/
const char* idFileSystemLocal::RelativePathToOSPath(const char* relativePath,
                                                    const char* basePath) {
...
bool idFileSystemLocal::FileIsInPAK(const char* relativePath) {
  ...
  if (!relativePath) {
    common->FatalError(
        "idFileSystemLocal::FileIsInPAK: NULL 'relativePath' parameter "
        "passed\n");
  }
  ...
  for (search = searchPaths; search; search = search->next) {
    ...
    if (search->pack && search->pack->hashTable[hash]) {
      ...
      if (serverPaks.Num()) {
        ...
        if (search->pack->pureStatus != PURE_NEVER &&
            !serverPaks.Find(search->pack)) {
...
============
*/
int idFileSystemLocal::ReadFile(const char* relativePath,
                                void** buffer,
                                ID_TIME_T* timestamp) {
  idFile* f;
  byte* buf;
...
============
*/
int idFileSystemLocal::WriteFile(const char* relativePath,
                                 const void* buffer,
                                 int size,
                                 const char* basePath) {
  idFile* f;
...
addonInfo_t* idFileSystemLocal::ParseAddonDef(const char* buf, const int len) {
  ...
  while (1) {
    ...
    int checksum;
    if (sscanf(token.c_str(), "0x%x", &checksum) != 1 &&
        sscanf(token.c_str(), "%x", &checksum) != 1) {
...
===============
*/
int idFileSystemLocal::AddUnique(const char* name,
                                 idStrList& list,
                                 idHashIndex& hashIndex) const {
  int i, hashKey;
...
===============
*/
void idFileSystemLocal::GetExtensionList(const char* extension,
                                         idStrList& extensionList) const {
  int s, e, l;
...
int idFileSystemLocal::GetFileList(const char* relativePath,
  ...
  for (search = searchPaths; search != NULL; search = search->next) {
    if (search->dir) {
      ...
      netpath =
          BuildOSPath(search->dir->path, search->dir->gamedir, relativePath);
      ...
      for (i = 0; i < pak->numfiles; i++) {
        ...
        for (j = 0; j < extensions.Num(); j++) {
          if (length >= extensions[j].Length() &&
              extensions[j].Icmp(name + length - extensions[j].Length()) == 0) {
            break;
          }
        }
        if (j >= extensions.Num()) {
...
idFileList* idFileSystemLocal::ListFiles(const char* relativePath,
  ...
  GetFileList(relativePath, extensionList, fileList->list, hashIndex,
              fullRelativePath, gamedir);
...
idFileList* idFileSystemLocal::ListFilesTree(const char* relativePath,
  ...
  GetFileListTree(relativePath, extensionList, fileList->list, hashIndex,
                  gamedir);
...
idModList* idFileSystemLocal::ListMods(void) {
  ...
  for (i = 0; i < list->mods.Num(); i++) {
    for (isearch = 0; isearch < 4; isearch++) {
      idStr descfile =
          BuildOSPath(search[isearch], list->mods[i], "description.txt");
...
bool idDEntry::Matches(const char* directory, const char* extension) const {
  if (!idDEntry::directory.Icmp(directory) &&
      !idDEntry::extension.Icmp(extension)) {
    return true;
  }
  return false;
}
...
===============
*/
int idFileSystemLocal::ListOSFiles(const char* directory,
                                   const char* extension,
                                   idStrList& list) {
  int i, j, ret;
  ...
  for (sp = fileSystemLocal.searchPaths; sp; sp = sp->next) {
    if (sp->pack) {
      if (com_developer.GetBool()) {
        sprintf(status, "%s (%i files - 0x%x %s", sp->pack->pakFilename.c_str(),
                sp->pack->numfiles, sp->pack->checksum,
...
void idFileSystemLocal::TouchFileList_f(const idCmdArgs& args) {
  ...
  idParser src(LEXFL_NOFATALERRORS | LEXFL_NOSTRINGCONCAT |
...
void idFileSystemLocal::AddGameDirectory(const char* path, const char* dir) {
  ...
  for (search = searchPaths; search; search = search->next) {
    ...
    if (search->dir->path.Cmp(path) == 0 &&
        search->dir->gamedir.Cmp(dir) == 0) {
      return;
    }
  }
...
void idFileSystemLocal::Startup(void) {
  ...
  if (restartChecksums.Num()) {
      ...
      if ((i = restartChecksums.FindIndex((*search)->pack->checksum)) != -1) {
        if (i == 0) {
          ...
          if (!aux) {
            ...
            if (fs_debug.GetBool()) {
              common->Printf(
                  "found pure checksum %x at index %d, but the end of search "
                  "path is reached\n",
            ...
            common->FatalError(
                "Failed to restart with pure mode restrictions for server "
                "connect");
          }
    ...
    // we could check if the game pak is actually present, but we would not be
    // restarting if there wasn't one @ first pure check
  ...
  cmdSystem->AddCommand("dirtree", DirTree_f, CMD_FL_SYSTEM,
                        "lists a folder with subfolders");
  cmdSystem->AddCommand("path", Path_f, CMD_FL_SYSTEM, "lists search paths");
  cmdSystem->AddCommand("touchFile", TouchFile_f, CMD_FL_SYSTEM,
                        "touches a file");
  cmdSystem->AddCommand("touchFileList", TouchFileList_f, CMD_FL_SYSTEM,
...
void idFileSystemLocal::SetRestrictions(void) {
  ...
  for (search = searchPaths; search; search = search->next) {
    if (search->pack) {
      ...
      if ((search->pack->checksum ^ 0x84268436u) !=
          (DEMO_PAK_CHECKSUM ^ 0x84268436u)) {
        common->FatalError("Corrupted %s: 0x%x",
                           search->pack->pakFilename.c_str(),
...
bool idFileSystemLocal::UpdateGamePakChecksums(void) {
  ...
  for (search = searchPaths; search; search = search->next) {
    ...
    for (pakFile = search->pack->hashTable[confHash]; pakFile;
         pakFile = pakFile->next) {
  ...
  if (!cvarSystem->GetCVarBool("net_serverAllowServerMod") &&
    ...
    common->Warning(
        "The current game code doesn't match pak files "
        "(net_serverAllowServerMod is off)");
    return false;
  }
...
int idFileSystemLocal::ValidateDownloadPakForChecksum(
  ...
  for (i = 0; i < testList.Num(); i++) {
    if (testList[i].Length() &&
        !testList[i].Icmpn(pak->pakFilename, testList[i].Length())) {
  ...
  if (i == testList.Num()) {
    common->Warning(
        "idFileSystem::ValidateDownloadPak: failed to extract relative path "
        "for %s",
        pak->pakFilename.c_str());
    return 0;
  }
...
fsPureReply_t idFileSystemLocal::SetPureServerChecksums(
  ...
  *missingGamePakChecksum = 0;
  ...
  while (pureChecksums[i]) {
    if (j < serverPaks.Num() && serverPaks[j]->checksum == pureChecksums[i]) {
      ...
      if (pack && pack->isNew) {
        ...
        if (fs_debug.GetBool()) {
          common->Printf(
              "pak %s checksumed 0x%x is a newly downloaded file. Restart "
              "required.\n",
              pack->pakFilename.c_str(), pack->checksum);
        }
        success = false;
        ...
        if (canPrepend) {
          ...
          if (fs_debug.GetBool()) {
            ...
            if (serverPaks.Find(pack)) {
              common->Printf(
                  "pak %s checksumed 0x%x is in the pure list at wrong index. "
                  "Current index is %d, found at %d\n",
                  pack->pakFilename.c_str(), pack->checksum, j,
                  serverPaks.FindIndex(pack));
            } else {
              common->Printf(
                  "pak %s checksumed 0x%x can't be added to pure list because "
                  "of search order\n",
                  pack->pakFilename.c_str(), pack->checksum);
            }
          }
  ...
  while (j < serverPaks.Num()) {
    ...
    if (fs_debug.GetBool()) {
      common->Printf(
          "pak %s checksumed 0x%x is an extra reference at the end of local "
          "pure list\n",
  ...
  if (_gamePakChecksum != gamePakChecksum) {
    ...
    if (pack->hashTable[dllHash]) {
      for (pakFile = pack->hashTable[dllHash]; pakFile;
           pakFile = pakFile->next) {
        if (!FilenameCompare(pakFile->name, dllName)) {
          gamePakChecksum = _gamePakChecksum;  // this will be used to extract
                                               // the DLL in pure mode FindDLL
          return PURE_RESTART;
        }
      }
    }
    common->Warning(
        "media is misconfigured. server claims pak '%s' ( 0x%x ) has media for "
        "us, but '%s' is not found\n",
...
=====================
*/
void idFileSystemLocal::SetRestartChecksums(
    const int pureChecksums[MAX_PURE_PAKS],
    int gamePakChecksum) {
  int i;
  pack_t* pack;
...
#ifdef WIN32
    fs_devpath.SetString(fs_cdpath.GetString()[0] ? fs_cdpath.GetString()
...
void idFileSystemLocal::Shutdown(bool reloading) {
  ...
  for (loop = searchPaths; loop;
       loop == searchPaths ? loop = addonPaks : loop = NULL) {
...
bool idFileSystemLocal::FileAllowedFromDir(const char* path) {
  ...
  if (strstr(path, "savegames") == path &&
      (!strcmp(path + l - 4, ".tga") || !strcmp(path + l - 4, ".txt") ||
       !strcmp(path + l - 5, ".save"))) {
    return true;
  }
  // screen shots
  ...
  // objective tgas
  if (strstr(path, "maps/game") == path && !strcmp(path + l - 4, ".tga")) {
    return true;
  }
...
===========
*/
idFile_InZip* idFileSystemLocal::ReadFileFromZip(pack_t* pak,
                                                 fileInPack_t* pakFile,
                                                 const char* relativePath) {
  unz_s* zfi;
  FILE* fp;
...
===========
*/
idFile* idFileSystemLocal::OpenFileReadFlags(const char* relativePath,
                                             int searchFlags,
                                             pack_t** foundInPak,
                                             bool allowCopyFiles,
                                             const char* gamedir) {
  ...
  if (!relativePath) {
    common->FatalError(
        "idFileSystemLocal::OpenFileRead: NULL 'relativePath' parameter "
        "passed\n");
  }
  ...
  for (search = searchPaths; search; search = search->next) {
    if (search->dir && (searchFlags & FSFLAG_SEARCH_DIRS)) {
      ...
      if (!loadedFileFromDir && !FileAllowedFromDir(relativePath)) {
        if (restartChecksums.Num()) {
          common->FatalError(
              "'%s' loaded from directory: Failed to restart with pure mode "
              "restrictions for server connect",
              relativePath);
        }
        common->DPrintf(
            "filesystem: switching to pure mode will require a restart. '%s' "
      ...
      if (allowCopyFiles && fs_copyfiles.GetInteger()) {
        ...
        idStr name;
        copypath =
            BuildOSPath(fs_savepath.GetString(), dir->gamedir, relativePath);
        ...
        switch (fs_copyfiles.GetInteger()) {
          ...
          case 2:
            ...
            if (isFromCDPath) {
              ...
              idStr sourcepath;
              sourcepath = BuildOSPath(fs_cdpath.GetString(), dir->gamedir,
                                       relativePath);
      ...
      for (pakFile = pak->hashTable[hash]; pakFile; pakFile = pakFile->next) {
        ...
        if (!FilenameCompare(pakFile->name, relativePath)) {
          ...
          if (!pak->referenced && !(searchFlags & FSFLAG_PURE_NOREF)) {
            ...
            if (fs_debug.GetInteger()) {
              common->Printf(
                  "idFileSystem::OpenFileRead: %s -> adding %s to referenced "
                  "paks\n",
...
idFile* idFileSystemLocal::OpenFileRead(const char* relativePath,
                                        bool allowCopyFiles,
                                        const char* gamedir) {
  return OpenFileReadFlags(relativePath,
                           FSFLAG_SEARCH_DIRS | FSFLAG_SEARCH_PAKS, NULL,
                           allowCopyFiles, gamedir);
}
...
================
*/
idFile* idFileSystemLocal::OpenFileByMode(const char* relativePath,
                                          fsMode_t mode) {
  if (mode == FS_READ) {
...
=================
*/
size_t idFileSystemLocal::CurlWriteFunction(void* ptr,
                                            size_t size,
                                            size_t nmemb,
                                            void* stream) {
...
#ifdef _WIN32
  return _write(static_cast<idFile_Permanent*>(bgl->f)->GetFilePtr()->_file,
                ptr, size * nmemb);
#else
  return fwrite(ptr, size, nmemb,
                static_cast<idFile_Permanent*>(bgl->f)->GetFilePtr());
#endif
}
...
=================
*/
int idFileSystemLocal::CurlProgressFunction(void* clientp,
                                            double dltotal,
                                            double dlnow,
                                            double ultotal,
                                            double ulnow) {
...
// use the low level read function, because fread may allocate memory
#if defined(WIN32)
      _read(static_cast<idFile_Permanent*>(bgl->f)->GetFilePtr()->_file,
            bgl->file.buffer, bgl->file.length);
#else
      fread(bgl->file.buffer, bgl->file.length, 1,
            static_cast<idFile_Permanent*>(bgl->f)->GetFilePtr());
#endif
      bgl->completed = true;
    } else {
      ...
      ret = curl_easy_setopt(session, CURLOPT_WRITEFUNCTION,
                             idFileSystemLocal::CurlWriteFunction);
      if (ret) {
        bgl->url.dlstatus = ret;
        bgl->url.status = DL_FAILED;
      ...
      ret = curl_easy_setopt(session, CURLOPT_PROGRESSFUNCTION,
                             idFileSystemLocal::CurlProgressFunction);
      if (ret) {
        bgl->url.dlstatus = ret;
        bgl->url.status = DL_FAILED;
...
void idFileSystemLocal::StartBackgroundDownloadThread() {
  if (!backgroundThread.threadHandle) {
    Sys_CreateThread((xthread_t)BackgroundDownloadThread, NULL, THREAD_NORMAL,
                     backgroundThread, "backgroundDownload", g_threads,
                     &g_thread_count);
    if (!backgroundThread.threadHandle) {
      common->Warning(
...
=================
*/
pack_t* idFileSystemLocal::FindPakForFileChecksum(const char* relativePath,
                                                  int findChecksum,
                                                  bool bReference) {
  searchpath_t* search;
  pack_t* pak;
...
=================
*/
void idFileSystemLocal::FindDLL(const char* name,
                                char _dllPath[MAX_OSPATH],
                                bool updateChecksum) {
  idFile* dllFile = NULL;
  char dllName[MAX_OSPATH];
  idStr dllPath;
  ...
  if (!dllFile) {
    if (!serverPaks.Num()) {
      ...
      dllFile = OpenFileReadFlags(
          dllName, FSFLAG_SEARCH_PAKS | FSFLAG_PURE_NOREF | FSFLAG_BINARY_ONLY,
          &inPak);
      if (dllFile) {
      ...
      // we are in pure mode. this path to be reached only for game DLL
      // situations with a code pak checksum given by server
      ...
      if (!pak) {
        ...
        // extract and copy
        for (pakFile = pak->hashTable[dllHash]; pakFile;
             pakFile = pakFile->next) {
          if (!FilenameCompare(pakFile->name, dllName)) {
...
bool idFileSystemLocal::HasD3XP(void) {
  ...
  for (i = 0; i < 4; i++) {
    pakfile =
        OpenExplicitFileRead(BuildOSPath(search[i], "d3xp", "pak000.pk4"));
    if (pakfile) {
      CloseFile(pakfile);
      d3xp = 1;
      return true;
...
idFile* idFileSystemLocal::MakeTemporaryFile(void) {
  FILE* f = tmpfile();
  if (!f) {
    common->Warning("idFileSystem::MakeTemporaryFile failed: %s",
                    strerror(errno));
    return NULL;
  }
  idFile_Permanent* file = new idFile_Permanent();
...
findFile_t idFileSystemLocal::FindFile(const char* path, bool scheduleAddons) {
  pack_t* pak;
  idFile* f = OpenFileReadFlags(
      path, FSFLAG_SEARCH_DIRS | FSFLAG_SEARCH_PAKS | FSFLAG_SEARCH_ADDONS,
      &pak);
  if (!f) {
    return FIND_NO;
  }
  ...
  if (scheduleAddons && pak->addon &&
      addonChecksums.FindIndex(pak->checksum) < 0) {
...
===============
*/
void idFileSystemLocal::FindMapScreenshot(const char* path,
                                          char* buf,
                                          int len) {
  idFile* file;
  idStr mapname = path;
  ...
  if (ReadFile(buf, NULL, NULL) == -1) {
    ...
    if (file) {
      ...
      file->Read(data, dlen);
      CloseFile(file);
      idStr::snPrintf(buf, len, "guis/assets/splash/addon/%s.tga",
                      mapname.c_str());

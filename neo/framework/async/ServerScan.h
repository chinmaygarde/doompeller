...
class idServerScan : public idList<networkServer_t> {
 public:
  ...
  typedef enum { IDLE = 0, WAIT_ON_INIT, LAN_SCAN, NET_SCAN } scan_state_t;
 ...
 private:
  ...
  static const int REPLY_TIMEOUT =
      999;  // how long should we wait for a reply from a game server
  static const int INCOMING_TIMEOUT =
      1500;  // when we got an incoming server list, how long till we decide the
             // list is done
  static const int REFRESH_START =
      10000;  // how long to wait when sending the initial refresh request

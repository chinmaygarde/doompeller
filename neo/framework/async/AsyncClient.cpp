...
void idAsyncClient::ConnectToServer(const netadr_t adr) {
  ...
  guiNetMenu->SetStateString(
      "status", va(common->GetLanguageDict()->GetString("#str_06749"),
...
void idAsyncClient::ConnectToServer(const char* address) {
  ...
  if (idStr::IsNumeric(address)) {
    ...
    if (serverNum < 0 || serverNum >= serverList.Num()) {
      session->MessageBox(
          MSG_OK,
          va(common->GetLanguageDict()->GetString("#str_06733"), serverNum),
    ...
    if (!Sys_StringToNetAdr(address, &adr, true)) {
      session->MessageBox(
          MSG_OK,
          va(common->GetLanguageDict()->GetString("#str_06734"), address),
          common->GetLanguageDict()->GetString("#str_06735"), true);
      return;
    }
  }
...
void idAsyncClient::ListServers(void) {
  ...
  for (i = 0; i < serverList.Num(); i++) {
    common->Printf("%3d: %s %dms (%s)\n", i,
                   ...
                   serverList[i].ping, Sys_NetAdrToString(serverList[i].adr));
  }
}
...
void idAsyncClient::RemoteConsole(const char* command) {
  ...
  if (active) {
    ...
    Sys_StringToNetAdr(idAsyncNetwork::clientRemoteConsoleAddress.GetString(),
                       &adr, true);
  }
...
void idAsyncClient::DuplicateUsercmds(int frame, int time) {
  ...
  for (i = 0; i < MAX_ASYNC_CLIENTS; i++) {
    idAsyncNetwork::DuplicateUsercmd(userCmds[previousIndex][i],
                                     userCmds[currentIndex][i], frame, time);
  }
}
...
void idAsyncClient::SendUsercmdsToServer(void) {
  ...
  if (idAsyncNetwork::verbose.GetInteger() == 2) {
    common->Printf(
        "sending usercmd to server: gameInitId = %d, gameFrame = %d, gameTime "
        "= %d\n",
  ...
  numUsercmds = idMath::ClampInt(
                    0, 10, idAsyncNetwork::clientUsercmdBackup.GetInteger()) +
                1;
...
void idAsyncClient::ProcessUnreliableServerMessage(const idBitMsg& msg) {
  ...
  switch (id) {
    ...
    case SERVER_UNRELIABLE_MESSAGE_GAMEINIT: {
      ...
      if (idAsyncNetwork::verbose.GetInteger()) {
        common->Printf(
            "received gameinit, gameInitId = %d, gameFrame = %d, gameTime = "
            "%d\n",
            gameInitId, gameFrame, gameTime);
      }
      ...
      if (pureWait) {
        guiNetMenu = uiManager->FindGui("guis/netmenu.gui", true, false, true);
        session->SetGUI(guiNetMenu, HandleGuiCommand);
        session->MessageBox(MSG_ABORT,
                            common->GetLanguageDict()->GetString("#str_04317"),
                            common->GetLanguageDict()->GetString("#str_04318"),
                            false, "pure_abort");
      } else {
        // load map
    ...
    case SERVER_UNRELIABLE_MESSAGE_SNAPSHOT: {
      ...
      game->ClientReadSnapshot(clientNum, snapshotSequence, snapshotGameFrame,
                               snapshotGameTime, numDuplicatedUsercmds,
                               aheadOfServer, msg);

      // read user commands of other clients from the snapshot
      for (last = NULL, i = msg.ReadByte(); i < MAX_ASYNC_CLIENTS;
           i = msg.ReadByte()) {
      ...
      if (clientState == CS_CONNECTED) {
        ...
        if (idAsyncNetwork::verbose.GetInteger()) {
          common->Printf(
              "received first snapshot, gameInitId = %d, gameFrame %d gameTime "
              "%d\n",
      ...
      if (gameTime < snapshotGameTime ||
        ...
        gameTimeResidual = idMath::ClampInt(
            -idAsyncNetwork::clientMaxPrediction.GetInteger(),
            idAsyncNetwork::clientMaxPrediction.GetInteger(), gameTimeResidual);
        clientPredictTime =
            idMath::ClampInt(-idAsyncNetwork::clientMaxPrediction.GetInteger(),
                             idAsyncNetwork::clientMaxPrediction.GetInteger(),
                             clientPredictTime);
      }
      ...
      clientPrediction -=
          (1 - (INTSIGNBITSET(aheadOfServer -
                              idAsyncNetwork::clientPrediction.GetInteger())
                << 1));
      clientPrediction = idMath::ClampInt(
          idAsyncNetwork::clientPrediction.GetInteger(),
          idAsyncNetwork::clientMaxPrediction.GetInteger(), clientPrediction);
      ...
      if (idAsyncNetwork::verbose.GetInteger() == 2) {
        common->Printf(
            "received snapshot, gameInitId = %d, gameFrame = %d, gameTime = "
            "%d\n",
            gameInitId, gameFrame, gameTime);
      }

      if (numDuplicatedUsercmds &&
          (idAsyncNetwork::verbose.GetInteger() == 2)) {
        common->Printf(
...
void idAsyncClient::ReadLocalizedServerString(const idBitMsg& msg,
  ...
  idStr::snPrintf(out, maxLen - 1, "%s",
                  common->GetLanguageDict()->GetString(out));
}
    ...
    switch (id) {
      case SERVER_RELIABLE_MESSAGE_CLIENTINFO: {
        ...
        if (checksum != srv_checksum) {
          common->DPrintf(
              "SERVER_RELIABLE_MESSAGE_CLIENTINFO %d (haveBase: %s): != "
              "checksums srv: 0x%x local: 0x%x\n",
              clientNum, haveBase ? "true" : "false", checksum, srv_checksum);
          info.Print();
        } else {
          common->DPrintf(
              "SERVER_RELIABLE_MESSAGE_CLIENTINFO %d (haveBase: %s): checksums "
              "ok 0x%x\n",
      ...
      case SERVER_RELIABLE_MESSAGE_DISCONNECT: {
        ...
        if (clientNum == idAsyncClient::clientNum) {
          ...
          session->MessageBox(
              MSG_OK, string,
              common->GetLanguageDict()->GetString("#str_04319"), true);
          session->StartMenu();
        } else {
          common->Printf("client %d %s\n", clientNum, string);
          cmdSystem->BufferCommandText(
              CMD_EXEC_NOW,
              va("addChatLine \"%s^0 %s\"",
                 sessLocal.mapSpawnData.userInfo[clientNum].GetString(
                     "ui_name"),
                 string));
      ...
      case SERVER_RELIABLE_MESSAGE_ENTERGAME: {
        ...
        game->SetUserInfo(clientNum, sessLocal.mapSpawnData.userInfo[clientNum],
                          true, false);
...
void idAsyncClient::ProcessChallengeResponseMessage(const netadr_t from,
  ...
  if (idStr::Icmp(cvarSystem->GetCVarString("fs_game_base"), serverGameBase) ||
    ...
    if (!fileSystem->HasD3XP() && (!idStr::Icmp(serverGameBase, "d3xp") ||
                                   !idStr::Icmp(serverGame, "d3xp"))) {
      common->Printf(
          "The server is running Doom3: Resurrection of Evil expansion pack. "
...
void idAsyncClient::ProcessConnectResponseMessage(const netadr_t from,
  ...
  clientPredictTime = clientPrediction =
      idMath::ClampInt(0, idAsyncNetwork::clientMaxPrediction.GetInteger(),
                       clientTime - lastConnectTime);
}
...
void idAsyncClient::ProcessDisconnectMessage(const netadr_t from,
  ...
  session->Stop();
  session->MessageBox(
      MSG_OK, common->GetLanguageDict()->GetString("#str_04320"), NULL, true);
  session->StartMenu();
}
...
void idAsyncClient::ProcessInfoResponseMessage(const netadr_t from,
  ...
  if (protocol != ASYNC_PROTOCOL_VERSION) {
    common->Printf("server %s ignored - protocol %d.%d, expected %d.%d\n",
                   Sys_NetAdrToString(serverInfo.adr), protocol >> 16,
                   protocol & 0xffff, ASYNC_PROTOCOL_MAJOR,
  ...
  common->Printf("%d: server %s - protocol %d.%d - %s\n", index,
                 Sys_NetAdrToString(serverInfo.adr), protocol >> 16,
                 protocol & 0xffff, serverInfo.serverInfo.GetString("si_name"));
}
...
void idAsyncClient::ProcessPrintMessage(const netadr_t from,
  ...
  if (opcode == SERVER_PRINT_GAMEDENY) {
    if (game_opcode == ALLOW_BADPASS) {
      retpass = session->MessageBox(
          MSG_PROMPT, common->GetLanguageDict()->GetString("#str_04321"),
          string, true, "passprompt_ok");
      ClearPendingPackets();
      guiNetMenu->SetStateString(
          "status", common->GetLanguageDict()->GetString("#str_04322"));
      if (retpass) {
        // #790
      ...
      session->MessageBox(MSG_OK, string,
                          common->GetLanguageDict()->GetString("#str_04323"),
                          true);
      ClearPendingPackets();
      cmdSystem->BufferCommandText(CMD_EXEC_NOW, "disconnect");
    }
    // ALLOW_NOTYET just keeps running as usual. The GUI has an abort button
  } else if (opcode == SERVER_PRINT_BADCHALLENGE &&
             clientState >= CS_CONNECTING) {
...
void idAsyncClient::ProcessServersListMessage(const netadr_t from,
  ...
  while (msg.GetRemaingData()) {
    ...
    c = msg.ReadByte();
    d = msg.ReadByte();
    serverList.AddServer(serverList.Num(),
                         va("%i.%i.%i.%i:%i", a, b, c, d, msg.ReadShort()));
  }
}
...
void idAsyncClient::ProcessAuthKeyMessage(const netadr_t from,
  ...
  if (clientState != CS_CONNECTING && !session->WaitingForGameAuth()) {
    common->Printf(
        "clientState != CS_CONNECTING, not waiting for game auth, authKey "
        "ignored\n");
    return;
  }
  ...
  if (authMsg == AUTHKEY_BADKEY) {
    ...
    switch (authBadStatus) {
      ...
      case AUTHKEY_BAD_BANNED:
        ...
        auth_msg = common->GetLanguageDict()->GetString(
            va("#str_0719%1d", 6 + key_index));
        auth_msg += "\n";
    ...
    if (clientState == CS_CONNECTING) {
      while (1) {
        ...
        retkey = session->MessageBox(
            MSG_CDKEY, auth_msg,
            common->GetLanguageDict()->GetString("#str_04325"), true);
        if (retkey) {
          if (session->CheckKey(retkey, true, valid)) {
            ...
            idAsyncNetwork::BuildInvalidKeyMsg(auth_msg, valid);
            session->MessageBox(
                MSG_OK, auth_msg.c_str(),
                common->GetLanguageDict()->GetString("#str_04327"), true);
            continue;
          }
        } else {
          cmdSystem->BufferCommandText(CMD_EXEC_NOW, "disconnect");
...
bool idAsyncClient::ValidatePureServerChecksums(const netadr_t from,
  ...
  fsPureReply_t reply = fileSystem->SetPureServerChecksums(
      inChecksums, inGamePakChecksum, missingChecksums,
  ...
  switch (reply) {
    ...
    case PURE_MISSING: {
      ...
      if (idAsyncNetwork::clientDownload.GetInteger() == 0) {
        // never any downloads
        idStr message = va(common->GetLanguageDict()->GetString("#str_07210"),
                           Sys_NetAdrToString(from));

        if (numMissingChecksums > 0) {
          message += va(common->GetLanguageDict()->GetString("#str_06751"),
                        numMissingChecksums, checksums.c_str());
        }
        if (missingGamePakChecksum) {
          message += va(common->GetLanguageDict()->GetString("#str_06750"),
                        missingGamePakChecksum);
        }

        common->Printf(message);
        cmdSystem->BufferCommandText(CMD_EXEC_NOW, "disconnect");
        session->MessageBox(MSG_OK, message,
                            common->GetLanguageDict()->GetString("#str_06735"),
                            true);
      } else {
        if (clientState >= CS_CONNECTED) {
        ...
        common->DPrintf("missing %d paks: %s\n",
                        numMissingChecksums + (missingGamePakChecksum ? 1 : 0),
                        checksums.c_str());
        if (missingGamePakChecksum) {
        ...
        // special case the code pak - if we have a 0 checksum then we don't
        // need to download it
    ...
    case PURE_NODLL:
      common->Printf(common->GetLanguageDict()->GetString("#str_07211"),
                     Sys_NetAdrToString(from));
      cmdSystem->BufferCommandText(CMD_EXEC_NOW, "disconnect");
      return false;
    default:
...
void idAsyncClient::ConnectionlessMessage(const netadr_t from,
                                          const idBitMsg& msg) {
  char string[MAX_STRING_CHARS *
              2];  // M. Quinn - Even Balance - PB packets can go beyond 1024
  ...
  if (!Sys_CompareNetAdrBase(from, serverAddress) &&
      (lastRconTime + 10000 < realTime ||
       !Sys_CompareNetAdrBase(from, lastRconAddress))) {
  ...
  if (idStr::Icmp(string, "authrequired") == 0) {
    ...
    if (idAsyncNetwork::LANServer.GetBool()) {
      common->Warning(
          "server %s requests master authorization for this client. Turning "
          "off LAN mode\n",
          Sys_NetAdrToString(from));
      idAsyncNetwork::LANServer.SetBool(false);
    }
...
void idAsyncClient::ProcessMessage(const netadr_t from, idBitMsg& msg) {
  ...
  if (!Sys_CompareNetAdrBase(from, channel.GetRemoteAddress()) ||
      id != serverId) {
...
void idAsyncClient::SetupConnection(void) {
  ...
  if (clientState == CS_CHALLENGING) {
    ...
    if (idAsyncNetwork::LANServer.GetBool()) {
      ...
      clientPort.SendPacket(idAsyncNetwork::GetMasterAddress(), msg.GetData(),
                            msg.GetSize());
    }
  } else {
    return;
  ...
  do {
    do {
      ...
      newPacket = clientPort.GetPacketBlocking(
          from, msgBuf, size, sizeof(msgBuf),
          USERCMD_MSEC - (gameTimeResidual + clientPredictTime) - 1);
      if (newPacket) {
        msg.Init(msgBuf, sizeof(msgBuf));
        msg.SetSize(size);
        msg.BeginReading();
  ...
  while (gameTimeResidual + clientPredictTime >= USERCMD_MSEC) {
    ...
    while (snapshotGameFrame < gameFrame) {
      ...
      bool lastPredictFrame =
          (snapshotGameFrame + 1 >= gameFrame &&
           gameTimeResidual + clientPredictTime < USERCMD_MSEC);

      // run client prediction
      gameReturn_t ret = game->ClientPrediction(
          clientNum, userCmds[snapshotGameFrame & (MAX_USERCMD_BACKUP - 1)],
          lastPredictFrame);
...
void idAsyncClient::HandleDownloads(void) {
  if (updateState == UPDATE_SENT && clientTime > updateSentTime + 2000) {
    ...
    if (showUpdateMessage) {
      session->MessageBox(
          MSG_OK, common->GetLanguageDict()->GetString("#str_04839"),
          common->GetLanguageDict()->GetString("#str_04837"), true);
      showUpdateMessage = false;
    }
    ...
    if (updateState == UPDATE_READY) {
      ...
      if (session->MessageBox(
              MSG_YESNO, updateMSG,
              common->GetLanguageDict()->GetString("#str_04330"), true,
              "yes")[0]) {
        if (!updateDirectDownload) {
          ...
          idFile_Permanent* f = static_cast<idFile_Permanent*>(
              fileSystem->OpenFileWrite(updateFile));
          dltotal = 0;
          dlnow = 0;
          ...
          session->DownloadProgressBox(
              &backgroundDownload, va("Downloading %s\n", updateFile.c_str()));
          updateState = UPDATE_DONE;
          if (backgroundDownload.url.status == DL_DONE) {
            ...
            if (session->MessageBox(
                    MSG_YESNO,
                    common->GetLanguageDict()->GetString("#str_04331"),
                    common->GetLanguageDict()->GetString("#str_04332"), true,
                    "yes")[0]) {
              ...
              session->MessageBox(
                  MSG_OK,
                  va(common->GetLanguageDict()->GetString("#str_04333"),
                     fullPath.c_str()),
                  common->GetLanguageDict()->GetString("#str_04334"), true);
            }
          } else {
            if (backgroundDownload.url.dlerror[0]) {
              common->Warning("update download failed. curl error: %s",
            ...
            session->MessageBox(
                MSG_OK, common->GetLanguageDict()->GetString("#str_04335"),
                common->GetLanguageDict()->GetString("#str_04336"), true);
      ...
      do {
        ...
        idFile_Permanent* f =
            static_cast<idFile_Permanent*>(fileSystem->MakeTemporaryFile());
        if (!f) {
        ...
        idStr dltitle;
        // "Downloading %s"
        sprintf(dltitle, common->GetLanguageDict()->GetString("#str_07213"),
                dlList[0].filename.c_str());
        if (numPaks > 1) {
          dltitle += va(" (%d/%d)", pakCount, numPaks);
        }
        if (totalDlSize) {
          progress_start =
              (int)((float)currentDlSize * 100.0f / (float)totalDlSize);
          progress_end = (int)((float)(currentDlSize + dlList[0].size) *
                               100.0f / (float)totalDlSize);
        } else {
          progress_start = 0;
          progress_end = 100;
        }
        session->DownloadProgressBox(&backgroundDownload, dltitle,
        ...
        if (backgroundDownload.url.status == DL_DONE) {
          ...
          while (remainlen) {
            ...
            if (retlen != readlen) {
              common->FatalError(
                  "short read %d of %d in idFileSystem::HandleDownload", retlen,
                  readlen);
            }
          ...
          if (!checksum || checksum != dlList[0].checksum) {
            ...
            session->MessageBox(
                MSG_OK,
                va(common->GetLanguageDict()->GetString("#str_07214"), checksum,
                   dlList[0].checksum),
          ...
          // "Download failed"
          session->MessageBox(
              MSG_OK, common->GetLanguageDict()->GetString("#str_07215"),
              common->GetLanguageDict()->GetString("#str_07216"), true);
          dlList.Clear();
          return;
        }
...
bool idAsyncClient::CheckTimeout(void) {
  if (lastPacketTime > 0 &&
      (lastPacketTime +
           idAsyncNetwork::clientServerTimeout.GetInteger() * 1000 <
       clientTime)) {
    session->StopBox();
    session->MessageBox(
        MSG_OK, common->GetLanguageDict()->GetString("#str_04328"),
        common->GetLanguageDict()->GetString("#str_04329"), true);
    cmdSystem->BufferCommandText(CMD_EXEC_NOW, "disconnect");
    return true;
  }
  return false;
...
void idAsyncClient::ProcessDownloadInfoMessage(const netadr_t from,
  ...
  if (infoType == SERVER_DL_REDIRECT) {
    ...
    if (session->MessageBox(
            MSG_YESNO,
            va(common->GetLanguageDict()->GetString("#str_07217"), buf),
    ...
    if (dlList.Num()) {
      common->Warning(
          "tried to process a download list while already busy downloading "
          "things");
      return;
    }
    ...
    do {
      ...
      if (pakDl == SERVER_PAK_YES) {
        ...
        common->Printf("download %s from %s ( 0x%x )\n", entry.filename.c_str(),
                       entry.url.c_str(), entry.checksum);
    ...
    if (totalDlSize == 0) {
      ...
      dlList.Clear();
      session->MessageBox(
          MSG_OK, common->GetLanguageDict()->GetString("#str_07219"),
          common->GetLanguageDict()->GetString("#str_07218"), true);
      return;
    }
    bool asked = false;
    if (gotGame) {
      ...
      if (!session->MessageBox(
              MSG_YESNO, common->GetLanguageDict()->GetString("#str_07220"),
              common->GetLanguageDict()->GetString("#str_07221"), true,
              "yes")[0]) {
        dlList.Clear();
        return;
      }
    ...
    if (!gotAllFiles) {
      ...
      if (!session->MessageBox(
              MSG_YESNO,
              va(common->GetLanguageDict()->GetString("#str_07222"),
                 sizeStr.c_str()),
              common->GetLanguageDict()->GetString("#str_07218"), true,
              "yes")[0]) {
        dlList.Clear();
    ...
    if (!asked && idAsyncNetwork::clientDownload.GetInteger() == 1) {
      ...
      if (!session->MessageBox(
              MSG_YESNO,
              va(common->GetLanguageDict()->GetString("#str_07224"),
                 sizeStr.c_str()),
              common->GetLanguageDict()->GetString("#str_07218"), true,
              "yes")[0]) {
        dlList.Clear();
    ...
    session->MessageBox(
        MSG_OK, common->GetLanguageDict()->GetString("#str_07223"),
        common->GetLanguageDict()->GetString("#str_07218"), true);
  }
}
...
int idAsyncClient::GetDownloadRequest(const int checksums[MAX_PURE_PAKS],
  ...
  if (memcmp(dlChecksums + 1, checksums, sizeof(int) * count) ||
      gamePakChecksum != dlChecksums[0]) {
    idRandom newreq;

...
const int ASYNC_PROTOCOL_VERSION =
    (ASYNC_PROTOCOL_MAJOR << 16) + ASYNC_PROTOCOL_MINOR;
...
class idAsyncNetwork {
 public:
  ...
  static void WriteUserCmdDelta(idBitMsg& msg,
                                const usercmd_t& cmd,
                                const usercmd_t* base);
  static void ReadUserCmdDelta(const idBitMsg& msg,
                               usercmd_t& cmd,
                               const usercmd_t* base);

  static bool DuplicateUsercmd(const usercmd_t& previousUserCmd,
                               usercmd_t& currentUserCmd,
                               int frame,
                               int time);
  static bool UsercmdInputChanged(const usercmd_t& previousUserCmd,
  ...
  static idCVar
      serverSnapshotDelay;  // number of milliseconds between snapshots
  ...
  static idCVar
      serverClientTimeout;  // time out in seconds for connected clients
  ...
  static idCVar
      serverAllowServerMod;  // let a pure server start with a different game

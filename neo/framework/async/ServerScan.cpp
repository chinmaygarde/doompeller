...
idCVar gui_filter_password("gui_filter_password",
                           "0",
                           CVAR_GUI | CVAR_INTEGER | CVAR_ARCHIVE,
                           "Password filter");
idCVar gui_filter_players("gui_filter_players",
                          "0",
                          CVAR_GUI | CVAR_INTEGER | CVAR_ARCHIVE,
                          "Players filter");
idCVar gui_filter_gameType("gui_filter_gameType",
                           "0",
                           CVAR_GUI | CVAR_INTEGER | CVAR_ARCHIVE,
                           "Gametype filter");
idCVar gui_filter_idle("gui_filter_idle",
                       "0",
                       CVAR_GUI | CVAR_INTEGER | CVAR_ARCHIVE,
                       "Idle servers filter");
idCVar gui_filter_game("gui_filter_game",
                       "0",
                       CVAR_GUI | CVAR_INTEGER | CVAR_ARCHIVE,
                       "Game filter");

const char* l_gameTypes[] = {"Deathmatch", "Tourney", "Team DM",
                             "Last Man",   "CTF",     NULL};
...
int idServerScan::InfoResponse(networkServer_t& server) {
  ...
  if (server.challenge != challenge) {
    common->DPrintf(
        "idServerScan::InfoResponse - ignoring response from %s, wrong "
        "challenge %d.",
        serv.c_str(), server.challenge);
    return false;
  }
  ...
  if (mapDef) {
    const char* mapName = common->GetLanguageDict()->GetString(
        mapDef->dict.GetString("name", si_map));
...
void idServerScan::RunFrame() {
  ...
  while (i < net_info.GetNumKeyVals()) {
    if (timeout_limit >
        net_servers[atoi(net_info.GetKeyVal(i)->GetValue().c_str())].time) {
  ...
  while (cur_info < net_servers.Num() &&
         net_info.GetNumKeyVals() < MAX_PINGREQUESTS) {
    netadr_t serv = net_servers[cur_info].adr;
    EmitGetInfo(serv);
    net_servers[cur_info].time = Sys_Milliseconds();
    net_info.SetInt(Sys_NetAdrToString(serv), cur_info);
  ...
  // update state
  if ((!incoming_net ||
       (incoming_useTimeout && Sys_Milliseconds() > incoming_lastTime)) &&
      net_info.GetNumKeyVals() == 0) {
    EndServers();
...
void idServerScan::GUIUpdateSelected(void) {
  ...
  if (i == -1 || i >= Num()) {
    ...
    m_pGUI->SetStateString("server_name",
                           (*this)[i].serverInfo.GetString("si_name"));
    for (int j = 0; j < 8; j++) {
      if ((*this)[i].clients > j) {
    ...
    m_pGUI->SetStateString("server_map",
                           (*this)[i].serverInfo.GetString("si_mapName"));
    fileSystem->FindMapScreenshot((*this)[i].serverInfo.GetString("si_map"),
                                  screenshot, MAX_STRING_CHARS);
...
void idServerScan::GUIAdd(int id, const networkServer_t server) {
  ...
  name += "\t";
  name +=
      va("%i/%i\t", server.clients, server.serverInfo.GetInt("si_maxPlayers"));
...
bool idServerScan::IsFiltered(const networkServer_t server) {
  ...
  if (keyval) {
    if (gui_filter_players.GetInteger() == 1 &&
        server.clients == atoi(keyval->GetValue())) {
      return true;
    } else if (gui_filter_players.GetInteger() == 2 &&
               (!server.clients ||
                server.clients == atoi(keyval->GetValue()))) {
      return true;
    }
  ...
  if (!fileSystem->HasD3XP() &&
      !idStr::Icmp(server.serverInfo.GetString("fs_game"), "d3xp")) {
    return true;
  }
...
int idServerScan::Cmp(const int* a, const int* b) {
  ...
  switch (l_serverScan->m_sort) {
    ...
    case SORT_PLAYERS:
      ret = serv1.clients < serv2.clients
                ? -1
                : (serv1.clients > serv2.clients ? 1 : 0);
      return ret;
    case SORT_GAMETYPE:

...
const char* authReplyStr[] = {"AUTH_NONE", "AUTH_OK", "AUTH_WAIT", "AUTH_DENY"};
...
void idAsyncServer::ExecuteMapChange(void) {
  ...
  game->GetBestGameType(cvarSystem->GetCVarString("si_map"),
                        cvarSystem->GetCVarString("si_gametype"), bestGameType);
  cvarSystem->SetCVarString("si_gametype", bestGameType);

  // initialize map settings
  cmdSystem->BufferCommandText(CMD_EXEC_NOW, "rescanSI");

  sprintf(mapName, "maps/%s",
          sessLocal.mapSpawnData.serverInfo.GetString("si_map"));
  ...
  switch (ff) {
    case FIND_NO:
      ...
      return;
    case FIND_ADDON:
      // NOTE: we have no problem with addon dependencies here because if the
      // map is in an addon pack that's already on search list, then all it's
      // deps are assumed to be on search as well
  ...
  if (!serverReloadingEngine &&
      (addonReload || idAsyncNetwork::serverReloadEngine.GetInteger() != 0)) {
    ...
    for (i = 0; i < MAX_ASYNC_CLIENTS; i++) {
      if (clients[i].clientState >= SCS_PUREWAIT && i != localClientNum) {
        ...
        clients[i].clientState =
            SCS_ZOMBIE;  // so we don't bother sending a disconnect
      }
    }
    ...
    // decrease feature
    if (idAsyncNetwork::serverReloadEngine.GetInteger() > 0) {
      idAsyncNetwork::serverReloadEngine.SetInteger(
          idAsyncNetwork::serverReloadEngine.GetInteger() - 1);
    }
    return;
  }
  serverReloadingEngine = false;
  ...
  // since this is not dependant on si_pure we catch anything bad before loading
  // map
  if (sessLocal.mapSpawnData.serverInfo.GetInt("si_pure")) {
    if (!fileSystem->UpdateGamePakChecksums()) {
      session->MessageBox(
          MSG_OK, common->GetLanguageDict()->GetString("#str_04337"),
          common->GetLanguageDict()->GetString("#str_04338"), true);
      cmdSystem->BufferCommandText(CMD_EXEC_APPEND, "disconnect\n");
      return;
    }
  }
...
void idAsyncServer::DuplicateUsercmds(int frame, int time) {
  ...
  for (i = 0; i < MAX_ASYNC_CLIENTS; i++) {
    ...
    if (idAsyncNetwork::DuplicateUsercmd(userCmds[previousIndex][i],
                                         userCmds[currentIndex][i], frame,
                                         time)) {
...
void idAsyncServer::InitClient(int clientNum, int clientId, int clientRate) {
  ...
  // clear the user info
  sessLocal.mapSpawnData.userInfo[clientNum]
      .Clear();  // always start with a clean base
  ...
  client.channel.SetMaxOutgoingRate(
      Min(idAsyncNetwork::serverMaxClientRate.GetInteger(), client.clientRate));
  client.clientPing = 0;
...
void idAsyncServer::InitLocalClient(int clientNum) {
  ...
  sessLocal.mapSpawnData.userInfo[clientNum] =
      *cvarSystem->MoveCVarsToDict(CVAR_USERINFO);
}
...
void idAsyncServer::BeginLocalClient(void) {
  ...
  game->SetUserInfo(localClientNum,
                    sessLocal.mapSpawnData.userInfo[localClientNum], false,
                    false);
...
void idAsyncServer::LocalClientInput(void) {
  ...
  if (idAsyncNetwork::UsercmdInputChanged(
          userCmds[(gameFrame - 1) & (MAX_USERCMD_BACKUP - 1)][localClientNum],
...
void idAsyncServer::DropClient(int clientNum, const char* reason) {
  ...
  cmdSystem->BufferCommandText(
      ...
      va("addChatLine \"%s^0 %s\"",
         sessLocal.mapSpawnData.userInfo[clientNum].GetString("ui_name"),
         reason));
...
void idAsyncServer::CheckClientTimeouts(void) {
  ...
  zombieTimeout =
      serverTime - idAsyncNetwork::serverZombieTimeout.GetInteger() * 1000;
  clientTimeout =
      serverTime - idAsyncNetwork::serverClientTimeout.GetInteger() * 1000;
  ...
  for (i = 0; i < MAX_ASYNC_CLIENTS; i++) {
    ...
    if (client.clientState == SCS_ZOMBIE &&
        client.lastPacketTime < zombieTimeout) {
      client.channel.Shutdown();
      client.clientState = SCS_FREE;
      continue;
    }
    ...
    if (client.clientState >= SCS_PUREWAIT &&
        client.lastPacketTime < clientTimeout) {
...
void idAsyncServer::SendUserInfoBroadcast(int userInfoNum,
  ...
  common->DPrintf("broadcast for client %d: 0x%x\n", userInfoNum,
                  sessLocal.mapSpawnData.userInfo[userInfoNum].Checksum());
  ...
  if (gameModifiedInfo || sendToAll) {
    ...
    msg.WriteDeltaDict(*gameInfo,
                       &sessLocal.mapSpawnData.userInfo[userInfoNum]);
  }

  for (int i = 0; i < MAX_ASYNC_CLIENTS; i++) {
    if (clients[i].clientState >= SCS_CONNECTED &&
        (sendToAll || i != userInfoNum || gameModifiedInfo)) {
      SendReliableMessage(i, msg);
    }
  }
...
bool idAsyncServer::SendEmptyToClient(int clientNum, bool force) {
  ...
  if (idAsyncNetwork::verbose.GetInteger()) {
    common->Printf(
        "sending empty to client %d: gameInitId = %d, gameFrame = %d, gameTime "
        "= %d\n",
...
void idAsyncServer::SendGameInitToClient(int clientNum) {
  ...
  if (idAsyncNetwork::verbose.GetInteger()) {
    common->Printf(
        "sending gameinit to client %d: gameInitId = %d, gameFrame = %d, "
        "gameTime = %d\n",
        clientNum, gameInitId, gameFrame, gameTime);
  }
...
bool idAsyncServer::SendSnapshotToClient(int clientNum) {
  ...
  if (serverTime - client.lastSnapshotTime <
      idAsyncNetwork::serverSnapshotDelay.GetInteger()) {
    return false;
  }

  if (idAsyncNetwork::verbose.GetInteger() == 2) {
    common->Printf(
        "sending snapshot to client %d: gameInitId = %d, gameFrame = %d, "
        "gameTime = %d\n",
        clientNum, gameInitId, gameFrame, gameTime);
  }
  ...
  game->ServerWriteSnapshot(clientNum, client.snapshotSequence, msg,
  ...
  for (last = NULL, i = 0; i < MAX_ASYNC_CLIENTS; i++) {
    ...
    int maxRelay =
        idMath::ClampInt(1, MAX_USERCMD_RELAY,
...
void idAsyncServer::ProcessUnreliableClientMessage(int clientNum,
                                                   const idBitMsg& msg) {
  int i, id, acknowledgeSequence, clientGameInitId, clientGameFrame,
      numUsercmds, index;
  usercmd_t* last;
  ...
  if (clientGameInitId != gameInitId) {
    if (acknowledgeSequence > client.gameInitSequence) {
      ...
      common->Printf(
          "ignore unreliable msg from client %d, wrong gameInit, old "
          "sequence\n",
          clientNum);
    }
    return;
  ...
  if (client.clientState == SCS_CONNECTED) {
    ...
    if (game->ServerApplySnapshot(clientNum,
                                  client.acknowledgeSnapshotSequence)) {
  ...
  switch (id) {
    ...
    case CLIENT_UNRELIABLE_MESSAGE_USERCMD: {
      ...
      for (last = NULL, i = clientGameFrame - numUsercmds + 1;
           i <= clientGameFrame; i++) {
        ...
        if (idAsyncNetwork::UsercmdInputChanged(
                userCmds[(i - 1) & (MAX_USERCMD_BACKUP - 1)][clientNum],
      ...
      if (idAsyncNetwork::verbose.GetInteger() == 2) {
        common->Printf(
            "received user command for client %d, gameInitId = %d, gameFrame, "
            "%d gameTime %d\n",
  ...
  while (client.channel.GetReliableMessage(msg)) {
    ...
    switch (id) {
      ...
      case CLIENT_RELIABLE_MESSAGE_PURE: {
        // we get this message once the client has successfully updated it's
        // pure list
...
void idAsyncServer::ProcessAuthMessage(const idBitMsg& msg) {
    ...
    if (!challenges[i].connected && challenges[i].clientId == clientId) {
      ...
      if (idStr::Cmp(challenges[i].guid, client_guid)) {
        common->DPrintf(
            ...
            Sys_NetAdrToString(challenges[i].address), challenges[i].guid,
            client_guid);
        return;
      }
      if (!Sys_CompareNetAdrBase(client_from, challenges[i].address)) {
        ...
        common->DPrintf("auth: matched guid '%s' for != IPs %s and %s\n",
                        client_guid, Sys_NetAdrToString(client_from),
                        Sys_NetAdrToString(challenges[i].address));
      }
      break;
    }
  ...
  if (challenges[i].authState != CDK_WAIT) {
    common->DWarning("auth: challenge 0x%x %s authState %d != CDK_WAIT",
...
void idAsyncServer::ProcessChallengeMessage(const netadr_t from,
  ...
  for (i = 0; i < MAX_CHALLENGES; i++) {
    if (!challenges[i].connected &&
        Sys_CompareNetAdrBase(from, challenges[i].address) &&
  ...
  if (Sys_IsLANAddress(from)) {
    ...
    if (idAsyncNetwork::LANServer.GetBool()) {
      common->Printf(
          "net_LANServer is enabled. Client %s is not a LAN address, will be "
          "rejected\n",
          Sys_NetAdrToString(from));
      ...
      outMsg.WriteLong(
          -1);  // this identifies "challenge" auth vs "connect" auth
      // protocol 1.37 addition
      outMsg.WriteByte(fileSystem->RunningD3XP());
      serverPort.SendPacket(idAsyncNetwork::GetMasterAddress(),
                            outMsg.GetData(), outMsg.GetSize());
    }
  }
}
...
bool idAsyncServer::SendReliablePureToClient(int clientNum) {
  ...
  fileSystem->GetPureServerChecksums(serverChecksums, clients[clientNum].OS,
                                     &gamePakChecksum);
  if (!serverChecksums[0]) {
    // happens if you run fully expanded assets with si_pure 1
    common->Warning("pure server has no pak files referenced");
...
int idAsyncServer::ValidateChallenge(const netadr_t from,
  ...
  for (i = 0; i < MAX_ASYNC_CLIENTS; i++) {
    ...
    if (Sys_CompareNetAdrBase(from, client.channel.GetRemoteAddress()) &&
        (clientId == client.clientId ||
         from.port == client.channel.GetRemoteAddress().port)) {
  ...
  for (i = 0; i < MAX_CHALLENGES; i++) {
    if (Sys_CompareNetAdrBase(from, challenges[i].address) &&
...
void idAsyncServer::ProcessConnectMessage(const netadr_t from,
                                          const idBitMsg& msg) {
  int clientNum, protocol, clientDataChecksum, challenge, clientId, ping,
      clientRate;
  idBitMsg outMsg;
  byte msgBuf[MAX_MESSAGE_SIZE];
  char guid[12];
  ...
  if (!sessLocal.mapSpawnData.serverInfo.GetInt("si_pure") &&
      clientDataChecksum != serverDataChecksum) {
  ...
  switch (challenges[ichallenge].authState) {
    ...
    case CDK_ONLYLAN:
      ...
      return;
    case CDK_WAIT:
      if (challenges[ichallenge].authReply == AUTH_NONE &&
          Min(serverTime - lastAuthTime,
              serverTime - challenges[ichallenge].time) > AUTHORIZE_TIMEOUT) {
      ...
      idStr::snPrintf(challenges[ichallenge].guid,
                      sizeof(challenges[ichallenge].guid), guid);
    ...
    default:
      assert(challenges[ichallenge].authState == CDK_OK ||
             challenges[ichallenge].authState == CDK_PUREOK);
  }
  ...
  allowReply_t reply = game->ServerAllowClient(
      numClients, Sys_NetAdrToString(from), guid, password, reason);
  if (reply != ALLOW_YES) {
...
bool idAsyncServer::VerifyChecksumMessage(int clientNum,
  ...
  fileSystem->GetPureServerChecksums(serverChecksums, OS,
                                     &serverGamePakChecksum);
  assert(serverChecksums[0]);

  // compare the lists
  if (serverGamePakChecksum != gamePakChecksum) {
    common->Printf("client %s: invalid game code pak ( 0x%x )\n",
                   from ? Sys_NetAdrToString(*from) : va("%d", clientNum),
                   gamePakChecksum);
...
void idAsyncServer::ProcessPureMessage(const netadr_t from,
  ...
  if (!VerifyChecksumMessage(iclient, &from, msg, reply,
                             challenges[iclient].OS)) {
    PrintOOB(from, SERVER_PRINT_MISC, reply);
    return;
  }

  common->DPrintf("client %s: passed pure checks\n", Sys_NetAdrToString(from));
  challenges[iclient].authState = CDK_PUREOK;  // next connect message will get
                                               // the client through completely
}
...
void idAsyncServer::ProcessReliablePure(int clientNum, const idBitMsg& msg) {
  ...
  if (clients[clientNum].clientState != SCS_PUREWAIT) {
    ...
    common->DPrintf(
        "client %d: got reliable pure while != SCS_PUREWAIT, sending a "
        "reload\n",
        clientNum);
    outMsg.Init(msgBuf, sizeof(msgBuf));
...
void idAsyncServer::PrintLocalServerInfo(void) {
  ...
  common->Printf("server '%s' IP = %s\nprotocol %d.%d OS mask 0x%x\n",
                 ...
                 Sys_NetAdrToString(serverPort.GetAdr()), ASYNC_PROTOCOL_MAJOR,
...
bool idAsyncServer::ConnectionlessMessage(const netadr_t from,
                                          const idBitMsg& msg) {
  char string[MAX_STRING_CHARS *
              2];  // M. Quinn - Even Balance - PB Packets need more than 1024
...
void idAsyncServer::RunFrame(void) {
  ...
  do {
    do {
      ...
      newPacket =
          serverPort.GetPacketBlocking(from, msgBuf, size, sizeof(msgBuf),
                                       USERCMD_MSEC - gameTimeResidual - 1);
      if (newPacket) {
        msg.Init(msgBuf, sizeof(msgBuf));
        msg.SetSize(size);
        msg.BeginReading();
  ...
  if (idAsyncNetwork::idleServer.GetBool() ==
      (!GetNumClients() || GetNumIdleClients() != GetNumClients())) {
    idAsyncNetwork::idleServer.SetBool(!idAsyncNetwork::idleServer.GetBool());
    // the need to propagate right away, only this
    sessLocal.mapSpawnData.serverInfo.Set(
        "si_idleServer", idAsyncNetwork::idleServer.GetString());
  ...
  while (gameTimeResidual >= USERCMD_MSEC) {
    ...
    // advance game
    gameReturn_t ret =
        game->RunFrame(userCmds[gameFrame & (MAX_USERCMD_BACKUP - 1)]);
  ...
  for (i = 0; i < MAX_ASYNC_CLIENTS; i++) {
    ...
    if (idAsyncNetwork::serverMaxClientRate.IsModified()) {
      client.channel.SetMaxOutgoingRate(Min(
          client.clientRate, idAsyncNetwork::serverMaxClientRate.GetInteger()));
    }
  ...
  if (com_showAsyncStats.GetBool()) {
    ...
    if (idAsyncNetwork::serverDedicated.GetBool() &&
        serverTime >= nextAsyncStatsTime) {
      common->Printf(
          "delay = %d msec, total outgoing rate = %d KB/s, total incoming rate "
          "= %d KB/s\n",
      ...
      for (i = 0; i < MAX_ASYNC_CLIENTS; i++) {
        ...
        if (outgoingRate != -1 && incomingRate != -1) {
          common->Printf(
              "client %d: out rate = %d B/s (% -2.1f%%), in rate = %d B/s (% "
              "-2.1f%%)\n",
              i, outgoingRate, outgoingCompression, incomingRate,
              incomingCompression);
        }
      }
...
void idAsyncServer::GetAsyncStatsAvgMsg(idStr& msg) {
  sprintf(msg, "avrg out: %d B/s - max %d B/s ( over %d ms )",
          stats_average_sum / stats_numsamples, stats_max,
...
void idAsyncServer::ProcessDownloadRequestMessage(const netadr_t from,
  ...
  // the first token of the pak names list passed to the game will be empty if
  // no game pak is requested
  dlGamePak = msg.ReadLong();
  if (dlGamePak) {
    if (!(dlSize[0] = fileSystem->ValidateDownloadPakForChecksum(
              dlGamePak, pakbuf, true))) {
  ...
  while (dlPakChecksum) {
    if (!(dlSize[numPaks] = fileSystem->ValidateDownloadPakForChecksum(
              dlPakChecksum, pakbuf, false))) {
  ...
  if (!game->DownloadRequest(Sys_NetAdrToString(from), challenges[iclient].guid,
                             paklist.c_str(), pakbuf)) {

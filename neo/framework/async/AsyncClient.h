...
typedef enum { AUTHKEY_BADKEY, AUTHKEY_GUID } authKeyMsg_t;
...
class idAsyncClient {
 ...
 private:
  ...
  int dlChecksums[MAX_PURE_PAKS];  // 0-terminated, first element is the game
                                   // pak checksum or 0
  ...
  int GetDownloadRequest(const int checksums[MAX_PURE_PAKS],
                         int count,
                         int gamePakChecksum);
};

...
idCVar net_channelShowPackets("net_channelShowPackets",
                              "0",
                              CVAR_SYSTEM | CVAR_BOOL,
                              "show all packets");
idCVar net_channelShowDrop("net_channelShowDrop",
                           "0",
                           CVAR_SYSTEM | CVAR_BOOL,
...
bool idMsgChannel::ReadMessageData(idBitMsg& out, const idBitMsg& msg) {
  ...
  while (reliableMessageSize != 0) {
    if (reliableMessageSize <= 0 ||
        reliableMessageSize > out.GetSize() - out.GetReadCount()) {
    ...
    if (reliableSequence == reliableReceive.GetLast() + 1) {
      reliableReceive.Add(out.GetData() + out.GetReadCount(),
                          reliableMessageSize);
    }
...
void idMsgChannel::SendNextFragment(idPort& port, const int time) {
  ...
  if (net_channelShowPackets.GetBool()) {
    common->Printf("%d send %4i : s = %i fragment = %i,%i\n", id, msg.GetSize(),
                   outgoingSequence - 1, unsentFragmentStart, fragLength);
  }
  ...
  if (unsentFragmentStart == unsentMsg.GetSize() &&
      fragLength != FRAGMENT_SIZE) {
    outgoingSequence++;
    unsentFragments = false;
  }
...
int idMsgChannel::SendMessage(idPort& port,
  ...
  if (net_channelShowPackets.GetBool()) {
    common->Printf("%d send %4i : s = %i ack = %i\n", id, unsentMsg.GetSize(),
                   outgoingSequence - 1, incomingSequence);
  }
...
bool idMsgChannel::Process(const netadr_t from,
  ...
  if (net_channelShowPackets.GetBool()) {
    if (fragmented) {
      common->Printf("%d recv %4i : s = %i fragment = %i,%i\n", id,
                     msg.GetSize(), sequence, fragStart, fragLength);
    } else {
  ...
  if (fragmented) {
    ...
    if (fragLength < 0 || fragLength > msg.GetRemaingData() ||
        fragmentLength + fragLength > sizeof(fragmentBuffer)) {
      if (net_channelShowDrop.GetBool() || net_channelShowPackets.GetBool()) {
    ...
    memcpy(fragmentBuffer + fragmentLength, msg.GetData() + msg.GetReadCount(),
           fragLength);
    ...
    memcpy(fragmentBuffer, msg.GetData() + msg.GetReadCount(),
           msg.GetRemaingData());
...
void idMsgChannel::UpdateOutgoingRate(const int time, const int size) {
  ...
  if (time - outgoingRateTime > 1000) {
    outgoingRateBytes -=
        outgoingRateBytes * (time - outgoingRateTime - 1000) / 1000;
...
void idMsgChannel::UpdateIncomingRate(const int time, const int size) {
  ...
  if (time - incomingRateTime > 1000) {
    incomingRateBytes -=
        incomingRateBytes * (time - incomingRateTime - 1000) / 1000;
...
float idMsgChannel::GetIncomingPacketLoss(void) const {
  ...
  return incomingDroppedPackets * 100.0f /
         (incomingReceivedPackets + incomingDroppedPackets);
}

...
const idDeclVideo* idDeclPDA::GetVideoByIndex(int index) const {
  if (index >= 0 && index < videos.Num()) {
    return static_cast<const idDeclVideo*>(
        declManager->FindType(DECL_VIDEO, videos[index], false));
  }
  return NULL;
}
...
const idDeclAudio* idDeclPDA::GetAudioByIndex(int index) const {
  if (index >= 0 && index < audios.Num()) {
    return static_cast<const idDeclAudio*>(
        declManager->FindType(DECL_AUDIO, audios[index], false));
  }
  return NULL;
}
...
const idDeclEmail* idDeclPDA::GetEmailByIndex(int index) const {
  if (index >= 0 && index < emails.Num()) {
    return static_cast<const idDeclEmail*>(
        declManager->FindType(DECL_EMAIL, emails[index], false));
  }
  return NULL;
}

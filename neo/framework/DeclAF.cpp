  ...
  switch (type) {
    ...
    case idAFVector::VEC_JOINT: {
      if (!GetJointTransform(model, frame, joint1, vec, axis)) {
        common->Warning("invalid joint %s in joint() in '%s'", joint1.c_str(),
                        fileName);
        vec.Zero();
      }
      break;
    ...
    case idAFVector::VEC_BONECENTER: {
      if (!GetJointTransform(model, frame, joint1, start, axis)) {
        common->Warning("invalid joint %s in bonecenter() in '%s'",
                        joint1.c_str(), fileName);
        start.Zero();
      }
      if (!GetJointTransform(model, frame, joint2, end, axis)) {
        common->Warning("invalid joint %s in bonecenter() in '%s'",
                        joint2.c_str(), fileName);
        end.Zero();
      }
      vec = (start + end) * 0.5f;
      break;
    ...
    case idAFVector::VEC_BONEDIR: {
      if (!GetJointTransform(model, frame, joint1, start, axis)) {
        common->Warning("invalid joint %s in bonedir() in '%s'", joint1.c_str(),
                        fileName);
        start.Zero();
      }
      if (!GetJointTransform(model, frame, joint2, end, axis)) {
        common->Warning("invalid joint %s in bonedir() in '%s'", joint2.c_str(),
                        fileName);
        end.Zero();
      }
      vec = (end - start);
...
bool idAFVector::Write(idFile* f) const {
  ...
  switch (type) {
    ...
    case idAFVector::VEC_BONECENTER: {
      f->WriteFloatString("bonecenter( \"%s\", \"%s\" )", joint1.c_str(),
                          joint2.c_str());
      break;
    }
    case idAFVector::VEC_BONEDIR: {
      f->WriteFloatString("bonedir( \"%s\", \"%s\" )", joint1.c_str(),
                          joint2.c_str());
      break;
    }
    default: {
...
const char* idAFVector::ToString(idStr& str, const int precision) {
  switch (type) {
    case idAFVector::VEC_COORDS: {
      char format[128];
      sprintf(format, "( %%.%df, %%.%df, %%.%df )", precision, precision,
              precision);
    ...
    case idAFVector::VEC_BONECENTER: {
      sprintf(str, "bonecenter( \"%s\", \"%s\" )", joint1.c_str(),
              joint2.c_str());
      break;
    }
    case idAFVector::VEC_BONEDIR: {
...
bool idDeclAF::WriteBody(idFile* f, const idDeclAF_Body& body) const {
  ...
  if (body.angles != ang_zero) {
    f->WriteFloatString("\tangles ( %f, %f, %f )\n", body.angles.pitch,
                        body.angles.yaw, body.angles.roll);
  }
  ...
  if (body.inertiaScale != mat3_identity) {
    ...
    f->WriteFloatString("\tinertiaScale (%f %f %f %f %f %f %f %f %f)\n",
                        ic[0][0], ic[0][1], ic[0][2], ic[1][0], ic[1][1],
                        ic[1][2], ic[2][0], ic[2][1], ic[2][2]);
  }
  if (body.linearFriction != -1) {
    f->WriteFloatString("\tfriction %f, %f, %f\n", body.linearFriction,
                        body.angularFriction, body.contactFriction);
  }
...
bool idDeclAF::WriteBallAndSocketJoint(idFile* f,
                                       const idDeclAF_Constraint& c) const {
  ...
  if (c.limit == idDeclAF_Constraint::LIMIT_CONE) {
    ...
    c.limitAxis.Write(f);
    f->WriteFloatString(", %f, %f, %f, ", c.limitAngles[0], c.limitAngles[1],
                        c.limitAngles[2]);
    c.shaft[0].Write(f);
    f->WriteFloatString("\n");
  }
...
bool idDeclAF::WriteUniversalJoint(idFile* f,
  ...
  if (c.limit == idDeclAF_Constraint::LIMIT_CONE) {
    ...
    f->WriteFloatString("\tpyramidLimit ");
    c.limitAxis.Write(f);
    f->WriteFloatString(", %f, %f, %f\n", c.limitAngles[0], c.limitAngles[1],
                        c.limitAngles[2]);
  }
  f->WriteFloatString("}\n");
  return true;
...
bool idDeclAF::WriteSettings(idFile* f) const {
  ...
  f->WriteFloatString("\tfriction %f, %f, %f, %f\n", defaultLinearFriction,
                      defaultAngularFriction, defaultContactFriction,
                      defaultConstraintFriction);
  f->WriteFloatString("\tsuspendSpeed %f, %f, %f, %f\n", suspendVelocity[0],
                      suspendVelocity[1], suspendAcceleration[0],
...
bool idDeclAF::RebuildTextSource(void) {
  ...
  f.WriteFloatString(
      ...
      "\tDo not edit directly but launch the game and type 'editAFs' on the "
      "console.\n"
      "*/\n");
  ...
  while (src.ReadToken(&token)) {
    if (!token.Icmp("model")) {
      ...
      if (!token.Icmp("box")) {
        ...
        if (!src.ExpectTokenString("(") || !body->v1.Parse(src) ||
            !src.ExpectTokenString(",") || !body->v2.Parse(src) ||
            !src.ExpectTokenString(")")) {
          return false;
        }
      ...
      body->angles =
          idAngles(angles.ToVec3().x, angles.ToVec3().y, angles.ToVec3().z);
...
bool idDeclAF::ParseUniversalJoint(idLexer& src) {
  ...
  while (src.ReadToken(&token)) {
    if (!token.Icmp("body1")) {
      ...
      if (!constraint->shaft[0].Parse(src) || !src.ExpectTokenString(",") ||
  ...
  for (i = 0; i < bodies.Num(); i++) {
    ...
    for (j = i + 1; j < bodies.Num(); j++) {
      if (bodies[i]->name == bodies[j]->name) {
        src.Error("two bodies with the same name \"%s\"",
                  bodies[i]->name.c_str());
      }
    }
  }
  ...
  for (i = 0; i < constraints.Num(); i++) {
    ...
    for (j = i + 1; j < constraints.Num(); j++) {
      if (constraints[i]->name == constraints[j]->name) {
        src.Error("two constraints with the same name \"%s\"",
                  constraints[i]->name.c_str());
      }
    }
    // check if there are two valid bodies set
    if (constraints[i]->body1 == "") {
      src.Error("no valid body1 specified for constraint '%s'",
                constraints[i]->name.c_str());
    }
    if (constraints[i]->body2 == "") {
      src.Error("no valid body2 specified for constraint '%s'",
                constraints[i]->name.c_str());
    }
  }

...
const ParticleParmDesc ParticleDistributionDesc[] = {{"rect", 3, ""},
                                                     {"cylinder", 4, ""},
                                                     {"sphere", 3, ""}};
...
const ParticleParmDesc ParticleOrientationDesc[] = {{"view", 0, ""},
                                                    {"aimed", 2, ""},
                                                    {"x", 0, ""},
                                                    {"y", 0, ""},
                                                    {"z", 0, ""}};
...
const int CustomParticleCount =
    sizeof(ParticleCustomDesc) / sizeof(const ParticleParmDesc);
  ...
  while (1) {
    ...
    if (!token.Icmp("distribution")) {
      ...
      ParseParms(src, stage->distributionParms,
                 sizeof(stage->distributionParms) /
                     sizeof(stage->distributionParms[0]));
      continue;
    }
    ...
    if (!token.Icmp("direction")) {
      ...
      ParseParms(
          src, stage->directionParms,
          sizeof(stage->directionParms) / sizeof(stage->directionParms[0]));
      continue;
    }
    ...
    if (!token.Icmp("orientation")) {
      ...
      ParseParms(
          src, stage->orientationParms,
          sizeof(stage->orientationParms) / sizeof(stage->orientationParms[0]));
      continue;
    }
    ...
    if (!token.Icmp("customPath")) {
      ...
      ParseParms(
          src, stage->customPathParms,
          sizeof(stage->customPathParms) / sizeof(stage->customPathParms[0]));
      continue;
    }
...
void idDeclParticle::WriteStage(idFile* f, idParticleStage* stage) {
  ...
  f->WriteFloatString("\t\tdistribution\t\t%s ",
                      ParticleDistributionDesc[stage->distributionType].name);
  for (i = 0; i < ParticleDistributionDesc[stage->distributionType].count;
       i++) {
    f->WriteFloatString("%.3f ", stage->distributionParms[i]);
  }
  f->WriteFloatString("\n");

  f->WriteFloatString("\t\tdirection\t\t\t%s ",
                      ParticleDirectionDesc[stage->directionType].name);
  for (i = 0; i < ParticleDirectionDesc[stage->directionType].count; i++) {
    f->WriteFloatString("\"%.3f\" ", stage->directionParms[i]);
  }
  f->WriteFloatString("\n");

  f->WriteFloatString("\t\torientation\t\t\t%s ",
                      ParticleOrientationDesc[stage->orientation].name);
  ...
  if (stage->customPathType != PPATH_STANDARD) {
    f->WriteFloatString("\t\tcustomPath %s ",
                        ParticleCustomDesc[stage->customPathType].name);
  ...
  f->WriteFloatString("\t\tcolor \t\t\t\t%.3f %.3f %.3f %.3f\n", stage->color.x,
                      stage->color.y, stage->color.z, stage->color.w);
  f->WriteFloatString("\t\tfadeColor \t\t\t%.3f %.3f %.3f %.3f\n",
                      stage->fadeColor.x, stage->fadeColor.y,
                      stage->fadeColor.z, stage->fadeColor.w);

  f->WriteFloatString("\t\toffset \t\t\t\t%.3f %.3f %.3f\n", stage->offset.x,
                      stage->offset.y, stage->offset.z);
...
bool idDeclParticle::RebuildTextSource(void) {
  ...
  f.WriteFloatString(
      ...
      "\tTo use the particle editor, launch the game and type 'editParticles' "
      "on the console.\n"
      "*/\n");
...
idParticleStage::idParticleStage(void) {
  ...
  distributionParms[0] = distributionParms[1] = distributionParms[2] =
      distributionParms[3] = 0.0f;
  directionType = PDIR_CONE;
  directionParms[0] = directionParms[1] = directionParms[2] =
      directionParms[3] = 0.0f;
  // idParticleParm		speed;
  gravity = 0.0f;
  worldGravity = false;
  customPathType = PPATH_STANDARD;
  customPathParms[0] = customPathParms[1] = customPathParms[2] =
      customPathParms[3] = 0.0f;
  customPathParms[4] = customPathParms[5] = customPathParms[6] =
  ...
  orientationParms[0] = orientationParms[1] = orientationParms[2] =
...
int idParticleStage::NumQuadsPerParticle() const {
  ...
  // if we are doing strip-animation, we need to double the number and cross
  // fade them
  ...
  if (customPathType == PPATH_STANDARD) {
    ...
    switch (distributionType) {
      case PDIST_RECT: {  // ( sizeX sizeY sizeZ )
        origin[0] = ((randomDistribution) ? g->random.CRandomFloat() : 1.0f) *
                    distributionParms[0];
        origin[1] = ((randomDistribution) ? g->random.CRandomFloat() : 1.0f) *
                    distributionParms[1];
      ...
      case PDIST_CYLINDER: {  // ( sizeX sizeY sizeZ ringFraction )
        angle1 = ((randomDistribution) ? g->random.CRandomFloat() : 1.0f) *
                 idMath::TWO_PI;
        ...
        if (distributionParms[3] > 0.0f) {
          ...
          if (radiusSqr < distributionParms[3] * distributionParms[3]) {
            // if we are inside the inner reject zone, rescale to put it out
            // into the good zone
            float f = sqrt(radiusSqr) / distributionParms[3];
            float invf = 1.0f / f;
            float newRadius =
                distributionParms[3] + f * (1.0f - distributionParms[3]);
      ...
      case PDIST_SPHERE: {  // ( sizeX sizeY sizeZ ringFraction )
        // iterating with rejection is the only way to get an even distribution
        // over a sphere
        ...
        if (distributionParms[3] > 0.0f) {
          // we could iterate until we got something that also satisfied
          // ringFraction, but for narrow rings that could be a lot of work, so
          // reproject inside points instead
          if (radiusSqr < distributionParms[3] * distributionParms[3]) {
            // if we are inside the inner reject zone, rescale to put it out
            // into the good zone
            float f = sqrt(radiusSqr) / distributionParms[3];
    ...
    switch (directionType) {
      case PDIR_CONE: {
        ...
        angle1 =
            g->random.CRandomFloat() * directionParms[0] * idMath::M_DEG2RAD;
    ...
    //
    // custom paths completely override both the origin and velocity
    // calculations, but still use the standard gravity
    //
    ...
    switch (customPathType) {
      case PPATH_HELIX: {  // ( sizeX sizeY sizeZ radialSpeed axialSpeed )
        ...
        angle1 = g->random.RandomFloat() * idMath::TWO_PI +
                 customPathParms[3] * speed1 * g->age;
        ...
        origin[2] = g->random.RandomFloat() * customPathParms[2] +
                    customPathParms[4] * speed2 * g->age;
        break;
      }
      case PPATH_FLIES: {  // ( radialSpeed axialSpeed size )
        ...
        angle1 = g->random.RandomFloat() * idMath::PI * 2 +
                 customPathParms[0] * speed1 * g->age;
        angle2 = g->random.RandomFloat() * idMath::PI * 2 +
                 customPathParms[1] * speed1 * g->age;
...
int idParticleStage::ParticleVerts(particleGen_t* g,
  ...
  if (orientation == POR_AIMED) {
    ...
    for (int i = 0; i <= numTrails; i++) {
      ...
      g->age = currentAge -
               (i + 1) * trailTime / (numTrails + 1);  // time to back up
...
void idParticleStage::ParticleColors(particleGen_t* g,
  ...
  // cycle goes on (note that totalParticles won't be correct for a
  // surface-particle deform)
  ...
  for (int i = 0; i < 4; i++) {
    float fcolor = ((entityColor) ? g->renderEnt->shaderParms[i] : color[i]) *
                       fadeFraction +
                   fadeColor[i] * (1.0f - fadeFraction);
    ...
    verts[0].color[i] = verts[1].color[i] = verts[2].color[i] =

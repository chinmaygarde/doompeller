...
void idMoveable::Spawn(void) {
  ...
  if (!collisionModelManager->TrmFromModel(clipModelName, trm)) {
    gameLocal.Error("idMoveable '%s': cannot load collision model %s",
  ...
  if (health) {
    if (brokenModel != "" && !renderModelManager->CheckModel(brokenModel)) {
      gameLocal.Error("idMoveable '%s' at (%s): cannot load broken model '%s'",
  ...
  physicsObj.SetClipMask(MASK_SOLID | CONTENTS_BODY | CONTENTS_CORPSE |
  ...
  if (!gameLocal.isClient && canDamage && gameLocal.time > nextDamageTime) {
    ...
    if (hasDamage || hasMonsterDamage) {
      ...
      if (ent && v > minDamageVelocity) {
        f = v > maxDamageVelocity
                ? 1.0f
                : idMath::Sqrt(v - minDamageVelocity) *
                      (1.0f /
                       idMath::Sqrt(maxDamageVelocity - minDamageVelocity));
        dir = velocity;
        dir.NormalizeFast();
        if (ent->IsType(idAI::Type) && hasMonsterDamage) {
          ...
          if (attacker) {
            ...
            ent->Damage(this, GetPhysics()->GetClipModel()->GetOwner(), dir,
                        monsterDamage, f, INVALID_JOINT);
          }
#else
          ent->Damage(this, GetPhysics()->GetClipModel()->GetOwner(), dir,
                      monsterDamage, f, INVALID_JOINT);
...
============
*/
void idMoveable::Killed(idEntity* inflictor,
                        idEntity* attacker,
                        int damage,
                        const idVec3& dir,
                        int location) {
...
bool idMoveable::FollowInitialSplinePath(void) {
  if (initialSpline != NULL) {
    if (gameLocal.time <
        initialSpline->GetTime(initialSpline->GetNumValues() - 1)) {
      ...
      idVec3 splineDir =
          initialSpline->GetCurrentFirstDerivative(gameLocal.time);
      idVec3 dir = initialSplineDir * physicsObj.GetAxis();
      idVec3 angularVelocity = dir.Cross(splineDir);
      angularVelocity.Normalize();
      angularVelocity *=
          idMath::ACos16(dir * splineDir / splineDir.Length()) * USERCMD_HZ;
  ...
  if (!wasAtRest) {
    ...
    if (onGround) {
      ...
      if (movedDistance > 0.0f &&
          idMath::Fabs(gravityNormal * curAxis[barrelAxis]) < 0.7f) {
        ...
        movedDistance =
            (1.0f - idMath::Fabs(dir * curAxis[barrelAxis])) * movedDistance;
        ...
        if (movedDistance > rotatedDistance) {
          ...
          angle = 180.0f * (movedDistance - rotatedDistance) /
                  (radius * idMath::PI);
          ...
          dir[barrelAxis] = 1.0f;
          additionalAxis =
              idRotation(vec3_origin, dir, additionalRotation).ToMat3();
        }
      }
    }
...
void idExplodingBarrel::Restore(idRestoreGame* savefile) {
  ...
  if (particleModelDefHandle != -1) {
    particleModelDefHandle =
        gameRenderWorld->AddEntityDef(&particleRenderEntity);
  }
#endif
}
...
void idExplodingBarrel::Think(void) {
  ...
  if (particleModelDefHandle >= 0) {
    ...
    gameRenderWorld->UpdateEntityDef(particleModelDefHandle,
...
void idExplodingBarrel::AddParticles(const char* name, bool burn) {
  if (name && *name) {
    ...
    const idDeclModelDef* modelDef = static_cast<const idDeclModelDef*>(
        declManager->FindType(DECL_MODELDEF, name));
    if (modelDef) {
      particleRenderEntity.origin = physicsObj.GetAbsBounds().GetCenter();
      particleRenderEntity.axis = mat3_identity;
      ...
      particleRenderEntity.shaderParms[SHADERPARM_TIMEOFFSET] =
          -MS2SEC(gameLocal.realClientTime);
      particleRenderEntity.shaderParms[SHADERPARM_DIVERSITY] =
          (burn) ? 1.0f : gameLocal.random.RandomInt(90);
#ifdef _D3XP
...
void idExplodingBarrel::ExplodingEffects(void) {
  ...
  if (*temp != '\0') {
    gameLocal.ProjectDecal(GetPhysics()->GetOrigin(),
                           GetPhysics()->GetGravity(), 128.0f, true, 96.0f,
                           temp);
  }
}
...
================
*/
void idExplodingBarrel::Killed(idEntity* inflictor,
                               idEntity* attacker,
                               int damage,
                               const idVec3& dir,
                               int location) {
  ...
  const char* splash =
      spawnArgs.GetString("def_splash_damage", "damage_explosion");
  if (splash && *splash) {
    gameLocal.RadiusDamage(GetPhysics()->GetOrigin(), this, attacker, this,
                           this, splash);
  }
  ...
  // bool first = true;
  while (kv) {
    const idDict* debris_args =
        gameLocal.FindEntityDefDict(kv->GetValue(), false);
    if (debris_args) {
      idEntity* ent;
      idVec3 dir;
      idDebris* debris;
      ...
      debris->Launch();
      debris->GetRenderEntity()->shaderParms[SHADERPARM_TIME_OF_DEATH] =
          (gameLocal.time + 1500) * 0.001f;
...
================
*/
void idExplodingBarrel::Damage(idEntity* inflictor,
                               idEntity* attacker,
                               const idVec3& dir,
  ...
  if (damageDef->FindKey("radius") && GetPhysics()->GetContents() != 0 &&
      GetBindMaster() == NULL) {
    PostEventMS(&EV_Explode, 400);
  } else {
    idEntity::Damage(inflictor, attacker, dir, damageDefName, damageScale,
                     location);
  }
}
...
void idExplodingBarrel::Event_Respawn() {
  ...
  if (minRespawnDist) {
    ...
    for (i = 0; i < gameLocal.numClients; i++) {
      ...
      idVec3 v = gameLocal.entities[i]->GetPhysics()->GetOrigin() -
...
================
*/
bool idExplodingBarrel::ClientReceiveEvent(int event,
                                           int time,
                                           const idBitMsg& msg) {
  switch (event) {
    case EVENT_EXPLODE: {
      if (gameLocal.realClientTime - msg.ReadLong() <
          spawnArgs.GetInt("explode_lapse", "1000")) {
        ExplodingEffects();
      }
      return true;
    }

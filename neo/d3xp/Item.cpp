...
================
*/
bool idItem::UpdateRenderEntity(renderEntity_s* renderEntity,
                                const renderView_t* renderView) const {
...
================
*/
bool idItem::ModelCallback(renderEntity_t* renderEntity,
...
void idItem::Spawn(void) {
  ...
  if (spawnArgs.GetFloat("triggersize", "0", tsize)) {
    GetPhysics()->GetClipModel()->LoadModel(
        idTraceModel(idBounds(vec3_origin).Expand(tsize)));
  ...
  if (spawnArgs.GetBool("spin") ||
      (gameLocal.isMultiplayer && !this->IsType(idItemTeam::Type))) {
    spin = true;
  ...
  canPickUp =
      !(spawnArgs.GetBool("triggerFirst") || spawnArgs.GetBool("no_touch"));
...
void idItem::GetAttributes(idDict& attributes) {
  ...
  for (i = 0; i < spawnArgs.GetNumKeyVals(); i++) {
    ...
    if (arg->GetKey().Left(4) == "inv_") {
      attributes.Set(arg->GetKey().Right(arg->GetKey().Length() - 4),
                     arg->GetValue());
    }
  }
}
...
void idItem::Event_DropToFloor(void) {
  ...
  gameLocal.clip.TraceBounds(
      trace, renderEntity.origin, renderEntity.origin - idVec3(0, 0, 64),
      renderEntity.bounds, MASK_SOLID | CONTENTS_CORPSE, this);
...
function_t* idItemTeam::LoadScript(char* script) {
  ...
  if (funcname.Length()) {
    ...
    if (function == NULL) {
#ifdef _DEBUG
      gameLocal.Warning("idItemTeam '%s' at (%s) calls unknown function '%s'",
...
void idItemTeam::Think(void) {
  ...
  if (gameLocal.isServer && nuggetName && carried &&
      (!lastNuggetDrop || (gameLocal.time - lastNuggetDrop) >
                              spawnArgs.GetInt("nugget_frequency"))) {
  ...
  if (dropped && !carried && lastDrop != 0 &&
      (gameLocal.time - lastDrop) >
          (si_flagDropTimeLimit.GetInteger() * 1000)) {
...
bool idItemTeam::Pickup(idPlayer* player) {
  ...
  if (lastDrop != 0 &&
      (gameLocal.time - lastDrop) < spawnArgs.GetInt("pickupDelay", "500"))
    return false;
  ...
  if (carried == false && player->team != this->team) {
    ...
    return true;
  } else if (carried == false && dropped == true &&
...
bool idItemTeam::ClientReceiveEvent(int event, int time, const idBitMsg& msg) {
  ...
  switch (event) {
    case EVENT_TAKEFLAG: {
      idPlayer* player = static_cast<idPlayer*>(
          gameLocal.entities[msg.ReadBits(GENTITYNUM_BITS)]);
      if (player == NULL) {
...
void idItemTeam::PrivateReturn(void) {
  ...
  trigger->Link(gameLocal.clip, this, 0, GetPhysics()->GetOrigin(),
                mat3_identity);
...
void idItemTeam::Event_TakeFlag(idPlayer* player) {
  ...
  if (gameLocal.isServer) {
    ...
    gameLocal.mpGame.PrintMessageEvent(-1, idMultiplayerGame::MSG_FLAGTAKEN,
                                       team, player->entityNumber);
  ...
  idVec3 origin(g_flagAttachOffsetX.GetFloat(), g_flagAttachOffsetY.GetFloat(),
                g_flagAttachOffsetZ.GetFloat());
  idAngles angle(g_flagAttachAngleX.GetFloat(), g_flagAttachAngleY.GetFloat(),
                 g_flagAttachAngleZ.GetFloat());
  SetAngles(angle);
  SetOrigin(origin);
...
void idItemTeam::Event_DropFlag(bool death) {
  ...
  if (gameLocal.isServer) {
    ...
    if (gameLocal.mpGame.IsFlagMsgOn()) {
      ...
      gameLocal.mpGame.PrintMessageEvent(-1, idMultiplayerGame::MSG_FLAGDROP,
                                         team);
    }
  }
  ...
  if (GetBindMaster()) {
    ...
    idVec3 origin = GetBindMaster()->GetPhysics()->GetOrigin() +
                    idVec3(0, 0, (bounds[1].z - bounds[0].z) * 0.6f);
...
void idItemTeam::Event_FlagReturn(idPlayer* player) {
  ...
  if (gameLocal.isServer) {
    ...
    if (gameLocal.mpGame.IsFlagMsgOn()) {
      ...
      gameLocal.mpGame.PrintMessageEvent(-1, idMultiplayerGame::MSG_FLAGRETURN,
                                         team, entitynum);
    }
  }
...
void idItemTeam::Event_FlagCapture(void) {
  ...
  if (gameLocal.isServer) {
    ...
    gameLocal.mpGame.PrintMessageEvent(-1, idMultiplayerGame::MSG_FLAGCAPTURE,
                                       team, playerIdx);
  }
...
void idItemTeam::SpawnNugget(idVec3 pos) {
  idAngles angle(
      gameLocal.random.RandomInt(spawnArgs.GetInt("nugget_pitch", "30")),
      gameLocal.random.RandomInt(spawnArgs.GetInt("nugget_yaw", "360")), 0);
  float velocity = float(gameLocal.random.RandomInt(40) + 15);

  velocity *= spawnArgs.GetFloat("nugget_velocity", "1");

  idEntity* ent = idMoveableItem::DropItem(
      nuggetName, pos, GetPhysics()->GetAxis(),
      angle.ToMat3() * idVec3(velocity, velocity, velocity), 0,
      spawnArgs.GetInt("nugget_removedelay"));
  idPhysics_RigidBody* physics =
      static_cast<idPhysics_RigidBody*>(ent->GetPhysics());
...
void idItemTeam::UpdateGuis(void) {
  ...
  for (int i = 0; i < gameLocal.numClients; i++) {
    ...
    player->hud->SetStateInt("red_flagstatus",
                             gameLocal.mpGame.GetFlagStatus(0));
    player->hud->SetStateInt("blue_flagstatus",
                             gameLocal.mpGame.GetFlagStatus(1));

    player->hud->SetStateInt("red_team_score",
                             gameLocal.mpGame.GetFlagPoints(0));
    player->hud->SetStateInt("blue_team_score",
                             gameLocal.mpGame.GetFlagPoints(1));
  }
}
  ...
  if (player) {
    ...
    if (spawnArgs.GetString("inv_objective", NULL)) {
      if (player && player->hud) {
        ...
        player->hud->SetStateString("objectivetext",
                                    spawnArgs.GetString("objectivetext"));
        player->hud->SetStateString("objectivetitle",
                                    spawnArgs.GetString("objectivetitle"));
        player->GiveObjective(spawnArgs.GetString("objectivetitle"),
                              spawnArgs.GetString("objectivetext"), shotName);

        // a tad slow but keeps from having to update all objectives in all maps
        // with a name ptr
        for (int i = 0; i < gameLocal.num_entities; i++) {
          if (gameLocal.entities[i] &&
              gameLocal.entities[i]->IsType(idObjectiveComplete::Type)) {
            if (idStr::Icmp(spawnArgs.GetString("objectivetitle"),
                            gameLocal.entities[i]->spawnArgs.GetString(
                                "objectivetitle")) == 0) {
...
void idMoveableItem::Spawn(void) {
  ...
  if (!collisionModelManager->TrmFromModel(clipModelName, trm)) {
    gameLocal.Error("idMoveableItem '%s': cannot load collision model %s",
                    name.c_str(), clipModelName.c_str());
    return;
  }
...
================
*/
idEntity* idMoveableItem::DropItem(const char* classname,
                                   const idVec3& origin,
                                   const idMat3& axis,
...
================
*/
void idMoveableItem::DropItems(idAnimatedEntity* ent,
                               const char* type,
  ...
  while (kv) {
    ...
    if (idStr::Icmp(c - 5, "Joint") != 0 &&
        idStr::Icmp(c - 8, "Rotation") != 0) {
      ...
      if (g_dropItemRotation.GetString()[0]) {
        angles.Zero();
        sscanf(g_dropItemRotation.GetString(), "%f %f %f", &angles.pitch,
...
void idMoveableItem::Gib(const idVec3& dir, const char* damageDefName) {
  ...
  if (*smokeName != '\0') {
    const idDeclParticle* smoke = static_cast<const idDeclParticle*>(
        declManager->FindType(DECL_PARTICLE, smokeName));
    gameLocal.smokeParticles->EmitSmoke(
        smoke, gameLocal.time, gameLocal.random.CRandomFloat(),
        renderEntity.origin, renderEntity.axis, timeGroup /*_D3XP*/);
  }
  // remove the entity
  PostEventMS(&EV_Remove, 0);
}
    ...
    if (spawnArgs.GetString("inv_objective", NULL)) {
      if (player->hud) {
        ...
        player->hud->SetStateString("objectivetext",
                                    spawnArgs.GetString("objectivetext"));
#ifdef _D3XP
        player->hud->SetStateString("objectivecompletetitle",
                                    spawnArgs.GetString("objectivetitle"));
#else
        player->hud->SetStateString("objectivetitle",
                                    spawnArgs.GetString("objectivetitle"));

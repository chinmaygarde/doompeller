...
idCVar g_spectatorChat("g_spectatorChat",
                       "0",
...
const char* idMultiplayerGame::GlobalSoundStrings[] = {
    "sound/feedback/voc_youwin.wav",    "sound/feedback/voc_youlose.wav",
    "sound/feedback/fight.wav",         "sound/feedback/vote_now.wav",
    "sound/feedback/vote_passed.wav",   "sound/feedback/vote_failed.wav",
    "sound/feedback/three.wav",         "sound/feedback/two.wav",
    "sound/feedback/one.wav",           "sound/feedback/sudden_death.wav",
#ifdef CTF
    "sound/ctf/flag_capped_yours.wav",  "sound/ctf/flag_capped_theirs.wav",
    "sound/ctf/flag_return.wav",        "sound/ctf/flag_taken_yours.wav",
    "sound/ctf/flag_taken_theirs.wav",  "sound/ctf/flag_dropped_yours.wav",
...
const char* idMultiplayerGame::GameStateStrings[] = {
    "INACTIVE",    "WARMUP",     "COUNTDOWN", "GAMEON",
    "SUDDENDEATH", "GAMEREVIEW", "NEXTGAME"};

const char* idMultiplayerGame::MPGuis[] = {"guis/mphud.gui", "guis/mpmain.gui",
                                           "guis/mpmsgmode.gui",
                                           "guis/netmenu.gui", NULL};

const char* idMultiplayerGame::ThrottleVars[] = {"ui_spectate", "ui_ready",
                                                 "ui_team", NULL};

const char* idMultiplayerGame::ThrottleVarsInEnglish[] = {
    "#str_06738", "#str_06737", "#str_01991", NULL};

const int idMultiplayerGame::ThrottleDelay[] = {8, 5, 5};
...
void idMultiplayerGame::Reset() {
  ...
  if (IsGametypeFlagBased())
    scoreBoard =
        uiManager->FindGui("guis/ctfscoreboard.gui", true, false, true);
  else
#endif
  ...
  mainGui->SetStateInt("com_machineSpec",
                       cvarSystem->GetCVarInteger("com_machineSpec"));
  SetMenuSkin();
    ...
    for (j = 0; j < numRankedPlayers; j++) {
      ...
      if (IsGametypeTeamBased()) { /* CTF */
        if (player->team != players[j]->team) {
          if (playerState[i].teamFragCount >
              playerState[players[j]->entityNumber].teamFragCount) {
            // team scores
            insert = true;
          } else if (playerState[i].teamFragCount ==
                         playerState[players[j]->entityNumber].teamFragCount &&
                     player->team < players[j]->team) {
            // at equal scores, sort by team number
            insert = true;
          }
        } else if (playerState[i].fragCount >
                   playerState[players[j]->entityNumber].fragCount) {
          // in the same team, sort by frag count
          insert = true;
        }
      } else {
        insert = (playerState[i].fragCount >
                  playerState[players[j]->entityNumber].fragCount);
      }
      if (insert) {
...
void idMultiplayerGame::UpdateScoreboard(idUserInterface* scoreBoard,
  ...
  scoreBoard->SetStateString(
      "scoretext", gameLocal.gameType == GAME_LASTMAN
    ...
    for (i = 0; i < numRankedPlayers; i++) {
      ...
      if (IsGametypeTeamBased()) { /* CTF */
        value = idMath::ClampInt(
            MP_PLAYER_MINFRAGS, MP_PLAYER_MAXFRAGS,
  ...
  for (k = 0; k < (gameState == WARMUP ? 2 : 1); k++) {
    for (i = 0; i < MAX_CLIENTS; i++) {
      ...
      if (!playerState[i].ingame) {
        scoreBoard->SetStateString(
            va("player%i", iline),
            common->GetLanguageDict()->GetString("#str_04244"));
        scoreBoard->SetStateString(
            va("player%i_score", iline),
            common->GetLanguageDict()->GetString("#str_04245"));
        // no color band
        ...
        if (gameState == WARMUP) {
          if (p->spectating) {
            scoreBoard->SetStateString(
                va("player%i_score", iline),
                common->GetLanguageDict()->GetString("#str_04246"));
            // no color band
            scoreBoard->SetStateInt(va("rank%i", iline), 0);
          } else {
            scoreBoard->SetStateString(
                va("player%i_score", iline),
                p->IsReady()
                    ? common->GetLanguageDict()->GetString("#str_04247")
                    : common->GetLanguageDict()->GetString("#str_04248"));
            // set the color band
            scoreBoard->SetStateInt(va("rank%i", iline), 1);
          ...
          if (gameLocal.gameType == GAME_LASTMAN &&
            ...
            scoreBoard->SetStateString(
                va("player%i_score", iline),
                common->GetLanguageDict()->GetString("#str_06736"));
            // set the color band
            scoreBoard->SetStateInt(va("rank%i", iline), 1);
            UpdateRankColor(scoreBoard, "rank%i_color%i", iline, p->colorBar);
          } else {
  ...
  gameinfo = va("%s: %s", common->GetLanguageDict()->GetString("#str_02376"),
                gameLocal.serverInfo.GetString("si_gameType"));
  if (gameLocal.gameType == GAME_LASTMAN) {
    if (gameState == GAMEON || gameState == SUDDENDEATH) {
      livesinfo =
          va("%s: %i", common->GetLanguageDict()->GetString("#str_04264"),
             startFragLimit);
    } else {
      livesinfo =
          va("%s: %i", common->GetLanguageDict()->GetString("#str_04264"),
             gameLocal.serverInfo.GetInt("si_fragLimit"));
    }
#ifdef CTF
...
#endif
    livesinfo = va("%s: %i", common->GetLanguageDict()->GetString("#str_01982"),
                   gameLocal.serverInfo.GetInt("si_fragLimit"));
  }
  if (gameLocal.serverInfo.GetInt("si_timeLimit") > 0) {
    timeinfo = va("%s: %i", common->GetLanguageDict()->GetString("#str_01983"),
                  gameLocal.serverInfo.GetInt("si_timeLimit"));
  } else {
...
================
*/
void idMultiplayerGame::UpdateCTFScoreboard(idUserInterface* scoreBoard,
                                            idPlayer* player) {
  int i, j;
  idStr gameinfo;
  idEntity* ent;
  ...
  if (gameState != WARMUP) {
    for (i = 0; i < numRankedPlayers; i++) {
      ...
      /* Player Name */
      scoreBoard->SetStateString(
          va("player%i_%s", ilines[player->team], curTeam),
      ...
      if (IsGametypeTeamBased()) {
        ...
        scoreBoard->SetStateInt(
            va("player%i_%s_score", ilines[player->team], curTeam), value);

        /* Team score and score, blanked */
        scoreBoard->SetStateString(
            va("player%i_%s_tscore", ilines[player->team], curTeam), "");
      ...
      value =
          idMath::ClampInt(0, MP_PLAYER_MAXWINS,
                           playerState[rankedPlayers[i]->entityNumber].wins);
      scoreBoard->SetStateInt(
          va("player%i_%s_wins", ilines[player->team], curTeam), value);

      /* Ping */
      scoreBoard->SetStateInt(
          va("player%i_%s_ping", ilines[player->team], curTeam),
          playerState[rankedPlayers[i]->entityNumber].ping);
    }
  }
  ...
  for (i = 0; i < MAX_CLIENTS; i++) {
    ...
    if (!playerState[i].ingame) {
      ...
      scoreBoard->SetStateString(
          va("player%i_%s_score", ilines[player->team], curTeam),
          common->GetLanguageDict()->GetString("#str_04245"));
  ...
  if (gameLocal.gameType == GAME_CTF) {
    ...
    if (captureLimit)
      scoreBoard->SetStateString(
          "gameinfo_red",
          va(common->GetLanguageDict()->GetString("#str_11108"), captureLimit));
    else
    ...
    if (timeLimit)
      scoreBoard->SetStateString(
          "gameinfo_blue",
          va(common->GetLanguageDict()->GetString("#str_11109"), timeLimit));
    else
...
bool idMultiplayerGame::AllPlayersReady() {
  ...
  for (i = 0; i < gameLocal.numClients; i++) {
    if (gameLocal.gameType == GAME_TOURNEY && i != currentTourneyPlayer[0] &&
        i != currentTourneyPlayer[1]) {
      continue;
    }
    ent = gameLocal.entities[i];
    if (!ent || !ent->IsType(idPlayer::Type)) {
...
idPlayer* idMultiplayerGame::FragLeader(void) {
  ...
  for (i = 0; i < gameLocal.numClients; i++) {
    ...
    if (gameLocal.gameType == GAME_TOURNEY &&
        ent->entityNumber != currentTourneyPlayer[0] &&
        ent->entityNumber != currentTourneyPlayer[1]) {
      continue;
    }
    ...
    int fragc = (IsGametypeTeamBased()) ? playerState[i].teamFragCount
                                        : playerState[i].fragCount; /* CTF */
    if (fragc > high) {
      high = fragc;
    }
...
void idMultiplayerGame::UpdateWinsLosses(idPlayer* winner) {
  if (winner) {
    ...
    for (int i = 0; i < gameLocal.numClients; i++) {
      ...
      if (IsGametypeTeamBased()) { /* CTF */
        if (player == winner ||
            (player != winner && player->team == winner->team)) {
          playerState[i].wins++;
        ...
        if (player == winner) {
          playerState[i].wins++;
          PlayGlobalSound(player->entityNumber, SND_YOUWIN);
        } else if (i == currentTourneyPlayer[0] ||
                   i == currentTourneyPlayer[1]) {
          PlayGlobalSound(player->entityNumber, SND_YOULOSE);
        }
      } else {
...
int idMultiplayerGame::GetFlagCarrier(int team) {
  ...
  for (int i = 0; i < gameLocal.numClients; i++) {
    ...
    if (player->carryingFlag) {
      if (iFlagCarrier != -1)
        gameLocal.Warning("BUG: more than one flag carrier on %s team",
                          team == 0 ? "red" : "blue");
      iFlagCarrier = i;
    }
  }
...
void idMultiplayerGame::PlayerDeath(idPlayer* dead,
  ...
  if (killer && killer == dead) {
    ...
    if (telefrag) {
      PrintMessageEvent(-1, MSG_TELEFRAGGED, dead->entityNumber,
                        killer->entityNumber);
    } else if (IsGametypeTeamBased() && dead->team == killer->team) { /* CTF */
      PrintMessageEvent(-1, MSG_KILLEDTEAM, dead->entityNumber,
                        killer->entityNumber);
    } else {
      PrintMessageEvent(-1, MSG_KILLED, dead->entityNumber,
                        killer->entityNumber);
    }
  } else {
...
void idMultiplayerGame::PlayerStats(int clientNum, char* data, const int len) {
  ...
  *data = 0;
  ...
  idStr::snPrintf(data, len, "team=%d score=%ld tks=%ld", team,
...
void idMultiplayerGame::DumpTourneyLine(void) {
  ...
  for (i = 0; i < gameLocal.numClients; i++) {
    if (gameLocal.entities[i] &&
        gameLocal.entities[i]->IsType(idPlayer::Type)) {
      common->Printf(
          "client %d: rank %d\n", i,
          static_cast<idPlayer*>(gameLocal.entities[i])->tourneyRank);
    }
  }
}
...
void idMultiplayerGame::NewState(gameState_t news, idPlayer* player) {
  ...
  assert(!gameLocal.isClient);
  gameLocal.DPrintf("%s -> %s\n", GameStateStrings[gameState],
                    GameStateStrings[news]);
  switch (news) {
    case GAMEON: {
      ...
      for (i = 0; i < gameLocal.numClients; i++) {
        ...
        if (gameLocal.gameType == GAME_TOURNEY &&
            currentTourneyPlayer[0] != i && currentTourneyPlayer[1] != i) {
          p->ServerSpectate(true);
          p->tourneyRank++;
        } else {
          int fragLimit = gameLocal.serverInfo.GetInt("si_fragLimit");
          int startingCount =
              (gameLocal.gameType == GAME_LASTMAN) ? fragLimit : 0;
    ...
    case GAMEREVIEW: {
      ...
      SetFlagMsg(false);
#endif
      nextState = INACTIVE;  // used to abort a game. cancel out any upcoming
                             // state change
    ...
    case COUNTDOWN: {
      ...
      warmupEndTime =
          gameLocal.time + 1000 * cvarSystem->GetCVarInteger("g_countDown");
    ...
    for (j = 0; j < gameLocal.numClients; j++) {
      ...
      if (p->tourneyRank >= rankmax) {
        ...
        if (p->tourneyRank == rankmax) {
          ...
          if (p->spawnedTime >
              static_cast<idPlayer*>(gameLocal.entities[rankmaxindex])
                  ->spawnedTime) {
            continue;
          }
        }
...
void idMultiplayerGame::UpdateTourneyLine(void) {
  ...
  for (j = 1; j <= gameLocal.numClients; j++) {
    ...
    for (i = 0; i < gameLocal.numClients; i++) {
      ...
      if (p->tourneyRank > max &&
          (globalmax == -1 || p->tourneyRank < globalmax)) {
        imax = i;
        max = p->tourneyRank;
      }
    }
...
void idMultiplayerGame::Run() {
  ...
  switch (gameState) {
    ...
    case GAMEON: {
      ...
      player = FragLimitHit();
      if (player) {
        // delay between detecting frag limit and ending game. let the death
        // anims play
        if (!fragLimitTimeout) {
        ...
        if (fragLimitTimeout) {
          // frag limit was hit and cancelled. means the two teams got even
          // during FRAGLIMIT_DELAY enter sudden death, the next frag leader
          // will win
          SuddenRespawn();
...
idUserInterface* idMultiplayerGame::StartMenu(void) {
  ...
  gameLocal.sessionCommand =
      "";  // in case we used "game_startMenu" to trigger the menu
  if (currentMenu == 1) {
    UpdateMainGui();
    ...
    if (vote == VOTE_NONE) {
      ...
      for (i = 0; i < VOTE_COUNT; i++) {
        ...
        // -1 disables
        mainGui->SetStateInt(va("vote%d", i),
                             g_voteFlags.GetInteger() & (1 << i) ? 0 : 1);
    ...
    for (i = 0; i < gameLocal.numClients; i++) {
      if (gameLocal.entities[i] &&
        ...
        kickList +=
            va("\"%d - %s\"", i, gameLocal.userInfo[i].GetString("ui_name"));
        kickVoteMap[j] = i;
        j++;
      }
    }
    ...
    const char* map = gameLocal.serverInfo.GetString(
        "si_map");  // what if server changes this strings while user in UI?
...
void idMultiplayerGame::SetMapShot(void) {
  ...
  fileSystem->FindMapScreenshot(dict ? dict->GetString("path") : "", screenshot,
                                MAX_STRING_CHARS);
  ...
  for (icmd = 0; icmd < args.Argc();) {
    ...
    if (!idStr::Icmp(cmd, ";")) {
      ...
      if (oldSpec != cvarSystem->GetCVarInteger("com_machineSpec")) {
        currentGui->SetStateInt("com_machineSpec",
                                cvarSystem->GetCVarInteger("com_machineSpec"));
      ...
      if (mode) {
        cmdSystem->BufferCommandText(
            CMD_EXEC_NOW,
            va("sayTeam \"%s\"", currentGui->State().GetString("chattext")));
      } else {
        cmdSystem->BufferCommandText(
            CMD_EXEC_NOW,
            va("say \"%s\"", currentGui->State().GetString("chattext")));
      }
      ...
      vote_flags_t voteIndex =
          (vote_flags_t)mainGui->State().GetInt("voteIndex");
      if (voteIndex == VOTE_MAP) {
        ...
        if (voteIndex == VOTE_KICK) {
          ...
          for (i = 0; si_gameTypeArgs[i]; i++) {
            if (!idStr::Icmp(gameTypeVoteMap[atoi(voteValue)],
                             si_gameTypeArgs[i])) {
      ...
      if (args.Argc() - icmd >= 2) {
        ...
        cmdSystem->BufferCommandText(
            CMD_EXEC_NOW,
            va("bindunbindtwo \"%s\" \"%s\"", key.c_str(), bind.c_str()));
        mainGui->SetKeyBindingNames();
      }
      continue;
      ...
      if (gametype == NULL || *gametype == 0 ||
          idStr::Icmp(gametype, "singleplayer") == 0) {
        gametype = "Deathmatch";
      }
...
bool idMultiplayerGame::Draw(int clientNum) {
  ...
  if (currentMenu) {
    ...
    if (player->wantSpectate) {
      mainGui->SetStateString(
          "spectext", common->GetLanguageDict()->GetString("#str_04249"));
    } else {
      mainGui->SetStateString(
          "spectext", common->GetLanguageDict()->GetString("#str_04250"));
    }
    DrawChat();
    if (currentMenu == 1) {
      UpdateMainGui();
    ...
    if (player->spectating) {
      ...
      if (gameLocal.gameType == GAME_TOURNEY) {
        if (!player->wantSpectate) {
          ...
          switch (player->tourneyLine) {
            case 0:
              spectatetext[0] +=
                  common->GetLanguageDict()->GetString("#str_07003");
              break;
            case 1:
              spectatetext[0] +=
                  common->GetLanguageDict()->GetString("#str_07004");
              break;
            case 2:
              spectatetext[0] +=
                  common->GetLanguageDict()->GetString("#str_07005");
              break;
            default:
              spectatetext[0] +=
                  va(common->GetLanguageDict()->GetString("#str_07006"),
                     player->tourneyLine);
              break;
          }
          ispecline++;
      ...
      if (player->spectator != player->entityNumber) {
        spectatetext[ispecline] =
            va(common->GetLanguageDict()->GetString("#str_07008"),
      ...
      if (vote != VOTE_NONE) {
        spectateGui->SetStateString(
            "vote", va("%s (y: %d n: %d)", voteString.c_str(), (int)yesVotes,
                       (int)noVotes));
      } else {
...
void idMultiplayerGame::UpdateHud(idPlayer* player, idUserInterface* hud) {
  ...
  if (gameState == WARMUP) {
    if (player->IsReady()) {
      hud->SetStateString("warmuptext",
                          common->GetLanguageDict()->GetString("#str_04251"));
    } else {
      hud->SetStateString("warmuptext",
                          common->GetLanguageDict()->GetString("#str_07002"));
    }
  }

  hud->SetStateString(
      "timer", (Warmup()) ? common->GetLanguageDict()->GetString("#str_04251")
               : (gameState == SUDDENDEATH)
                   ? common->GetLanguageDict()->GetString("#str_04252")
                   : GameTime());
  if (vote != VOTE_NONE) {
    hud->SetStateString("vote", va("%s (y: %d n: %d)", voteString.c_str(),
                                   (int)yesVotes, (int)noVotes));
  } else {
  ...
  if (gameState == GAMEON) {
    for (i = 0; i < numRankedPlayers; i++) {
      if (IsGametypeTeamBased()) { /* CTF */
        hud->SetStateInt(
            va("player%i_score", i + 1),
            playerState[rankedPlayers[i]->entityNumber].teamFragCount);
      } else {
        hud->SetStateInt(va("player%i_score", i + 1),
                         playerState[rankedPlayers[i]->entityNumber].fragCount);
      }
  ...
  if (guiChat) {
    if (gameLocal.time - lastChatLineTime > CHAT_FADE_TIME) {
      if (chatHistorySize > 0) {
        for (i = chatHistoryIndex - chatHistorySize; i < chatHistoryIndex;
             i++) {
    ...
    if (chatDataUpdated) {
      ...
      while (i < chatHistoryIndex) {
        guiChat->SetStateString(va("chat%i", j),
                                chatHistory[i % NUM_CHAT_NOTIFY].line);
        // don't set alpha above 4, the gui only knows that
        guiChat->SetStateInt(
            va("alpha%i", j),
            Min(4, (int)chatHistory[i % NUM_CHAT_NOTIFY].fade));
        j++;
        i++;
      }
...
const int ASYNC_PLAYER_FRAG_BITS =
    -(idMath::BitsForInteger(MP_PLAYER_MAXFRAGS - MP_PLAYER_MINFRAGS) +
      1);  // player can have negative frags
#else
const int ASYNC_PLAYER_FRAG_BITS = -idMath::BitsForInteger(
    MP_PLAYER_MAXFRAGS - MP_PLAYER_MINFRAGS);  // player can have negative frags
#endif
...
void idMultiplayerGame::WriteToSnapshot(idBitMsgDelta& msg) const {
  ...
  for (i = 0; i < MAX_CLIENTS; i++) {
    ...
    value = idMath::ClampInt(MP_PLAYER_MINFRAGS, MP_PLAYER_MAXFRAGS,
...
void idMultiplayerGame::PlayGlobalSound(int to,
  ...
  if (!gameLocal.isClient) {
    ...
    if (shader) {
      ...
      outMsg.WriteLong(
          gameLocal.ServerRemapDecl(to, DECL_SOUND, shaderDecl->Index()));
    } else {
...
================
*/
void idMultiplayerGame::PlayTeamSound(int toTeam,
                                      snd_evt_t evt,
                                      const char* shader) {
  for (int i = 0; i < gameLocal.numClients; i++) {
    idEntity* ent = gameLocal.entities[i];
...
void idMultiplayerGame::PrintMessageEvent(int to,
  ...
  switch (evt) {
    case MSG_SUICIDE:
      assert(parm1 >= 0);
      AddChatLine(common->GetLanguageDict()->GetString("#str_04293"),
                  gameLocal.userInfo[parm1].GetString("ui_name"));
      break;
    case MSG_KILLED:
      assert(parm1 >= 0 && parm2 >= 0);
      AddChatLine(common->GetLanguageDict()->GetString("#str_04292"),
                  gameLocal.userInfo[parm1].GetString("ui_name"),
                  gameLocal.userInfo[parm2].GetString("ui_name"));
      break;
    case MSG_KILLEDTEAM:
      assert(parm1 >= 0 && parm2 >= 0);
      AddChatLine(common->GetLanguageDict()->GetString("#str_04291"),
                  gameLocal.userInfo[parm1].GetString("ui_name"),
                  gameLocal.userInfo[parm2].GetString("ui_name"));
      break;
    case MSG_TELEFRAGGED:
      assert(parm1 >= 0 && parm2 >= 0);
      AddChatLine(common->GetLanguageDict()->GetString("#str_04290"),
                  gameLocal.userInfo[parm1].GetString("ui_name"),
                  gameLocal.userInfo[parm2].GetString("ui_name"));
      break;
    case MSG_DIED:
      assert(parm1 >= 0);
      AddChatLine(common->GetLanguageDict()->GetString("#str_04289"),
                  gameLocal.userInfo[parm1].GetString("ui_name"));
      break;
    case MSG_VOTE:
    ...
    case MSG_SUDDENDEATH:
      ...
      break;
    case MSG_FORCEREADY:
      AddChatLine(common->GetLanguageDict()->GetString("#str_04286"),
                  gameLocal.userInfo[parm1].GetString("ui_name"));
      if (gameLocal.entities[parm1] &&
          gameLocal.entities[parm1]->IsType(idPlayer::Type)) {
        static_cast<idPlayer*>(gameLocal.entities[parm1])->forcedReady = true;
      }
      break;
    case MSG_JOINEDSPEC:
      AddChatLine(common->GetLanguageDict()->GetString("#str_04285"),
                  gameLocal.userInfo[parm1].GetString("ui_name"));
      break;
    case MSG_TIMELIMIT:
    ...
    case MSG_FRAGLIMIT:
      if (gameLocal.gameType == GAME_LASTMAN) {
        AddChatLine(common->GetLanguageDict()->GetString("#str_04283"),
                    gameLocal.userInfo[parm1].GetString("ui_name"));
      } else if (IsGametypeTeamBased()) { /* CTF */
        AddChatLine(common->GetLanguageDict()->GetString("#str_04282"),
                    gameLocal.userInfo[parm1].GetString("ui_team"));
      } else {
        AddChatLine(common->GetLanguageDict()->GetString("#str_04281"),
                    gameLocal.userInfo[parm1].GetString("ui_name"));
      }
      break;
    case MSG_JOINTEAM:
      AddChatLine(common->GetLanguageDict()->GetString("#str_04280"),
                  gameLocal.userInfo[parm1].GetString("ui_name"),
                  parm2 ? common->GetLanguageDict()->GetString("#str_02500")
...
#ifdef CTF
    case MSG_POINTLIMIT:
      AddChatLine(common->GetLanguageDict()->GetString("#str_11100"),
                  parm1 ? common->GetLanguageDict()->GetString("#str_11110")
    ...
    case MSG_FLAGTAKEN:
      ...
      if (gameLocal.GetLocalPlayer()->team != parm1) {
        AddChatLine(
            common->GetLanguageDict()->GetString("#str_11101"),
            gameLocal.userInfo[parm2].GetString("ui_name"));  // your team
      } else {
        AddChatLine(common->GetLanguageDict()->GetString("#str_11102"),
    ...
    case MSG_FLAGDROP:
      ...
      if (gameLocal.GetLocalPlayer()->team != parm1) {
        AddChatLine(
            common->GetLanguageDict()->GetString("#str_11103"));  // your team
      } else {
        AddChatLine(
    ...
    case MSG_FLAGRETURN:
      ...
      if (parm2 >= 0 && parm2 < MAX_CLIENTS) {
        if (gameLocal.GetLocalPlayer()->team != parm1) {
          AddChatLine(
              common->GetLanguageDict()->GetString("#str_11120"),
              gameLocal.userInfo[parm2].GetString("ui_name"));  // your team
        } else {
          AddChatLine(common->GetLanguageDict()->GetString("#str_11121"),
                      gameLocal.userInfo[parm2].GetString("ui_name"));  // enemy
        }
      } else {
        AddChatLine(common->GetLanguageDict()->GetString("#str_11105"),
                    parm1 ? common->GetLanguageDict()->GetString("#str_11110")
                          : common->GetLanguageDict()->GetString("#str_11111"));
      }
      break;
    ...
    case MSG_FLAGCAPTURE:
      ...
      if (gameLocal.GetLocalPlayer()->team != parm1) {
        AddChatLine(
            common->GetLanguageDict()->GetString("#str_11122"),
            gameLocal.userInfo[parm2].GetString("ui_name"));  // your team
      } else {
        AddChatLine(common->GetLanguageDict()->GetString("#str_11123"),
                    gameLocal.userInfo[parm2].GetString("ui_name"));  // enemy
      }

      //			AddChatLine(
      //common->GetLanguageDict()->GetString( "#str_11106" ), parm1 ?
      //common->GetLanguageDict()->GetString( "#str_11110" ) :
      //common->GetLanguageDict()->GetString( "#str_11111" ) );
      break;

    case MSG_SCOREUPDATE:
      AddChatLine(common->GetLanguageDict()->GetString("#str_11107"), parm1,
                  parm2);
      break;
#endif
    default:
  ...
  for (int i = 0; i < gameLocal.numClients; i++) {
    ...
    if (WantRespawn(p) || p == spectator) {
      if (gameState == SUDDENDEATH && gameLocal.gameType != GAME_LASTMAN) {
...
#ifdef _DEBUG
            if (gameLocal.gameType == GAME_TOURNEY) {
              assert(p->entityNumber == currentTourneyPlayer[0] ||
                     p->entityNumber == currentTourneyPlayer[1]);
            }
#endif
        ...
        if (gameLocal.gameType == GAME_DM ||  // CTF : 3wave sboily, was DM
                                              // really included before?
            IsGametypeTeamBased()) {
          ...
          if (gameState == WARMUP || gameState == COUNTDOWN) {
            ...
            if (gameState == GAMEON && playerState[i].fragCount > 0 &&
                p->lastManPresent) {
              ...
              if (!fragLimitTimeout && !p->lastManOver) {
                ...
                // got stuck
                {
                  ...
                  for (j = 0; j < gameLocal.numClients; j++) {
                    ...
                    if (!CanPlay(
                            static_cast<idPlayer*>(gameLocal.entities[j]))) {
                      continue;
                    }
                    if (!static_cast<idPlayer*>(gameLocal.entities[j])
                             ->lastManOver) {
                      break;
                    }
                  }
      ...
      playerState[i].fragCount = 0;  // whenever you willingly go spectate
                                     // during game, your score resets
      p->ServerSpectate(true);
      UpdateTourneyLine();
      CheckAbortGame();
    }
...
void idMultiplayerGame::ServerStartVote(int clientNum,
  ...
  for (i = 0; i < gameLocal.numClients; i++) {
    if (gameLocal.entities[i] &&
        gameLocal.entities[i]->IsType(idPlayer::Type)) {
      playerState[i].vote =
          (i == clientNum) ? PLAYER_VOTE_YES : PLAYER_VOTE_WAIT;
    } else {
...
void idMultiplayerGame::ClientStartVote(int clientNum,
  ...
  AddChatLine(va(common->GetLanguageDict()->GetString("#str_04279"),
...
================
*/
void idMultiplayerGame::ClientUpdateVote(vote_result_t status,
                                         int yesCount,
                                         int noCount) {
  idBitMsg outMsg;
  byte msgBuf[MAX_GAME_MESSAGE_SIZE];
...
void idMultiplayerGame::CastVote(int clientNum, bool castVote) {
  ...
  // sanity
  if (vote == VOTE_NONE) {
    gameLocal.ServerSendChatMessage(
        clientNum, "server",
        common->GetLanguageDict()->GetString("#str_04275"));
  ...
  if (playerState[clientNum].vote != PLAYER_VOTE_WAIT) {
    gameLocal.ServerSendChatMessage(
        clientNum, "server",
        common->GetLanguageDict()->GetString("#str_04274"));
...
void idMultiplayerGame::ServerCallVote(int clientNum, const idBitMsg& msg) {
  ...
  if (vote != VOTE_NONE) {
    gameLocal.ServerSendChatMessage(
        clientNum, "server",
        common->GetLanguageDict()->GetString("#str_04273"));
    common->DPrintf(
  ...
  switch (voteIndex) {
    case VOTE_RESTART:
      ...
      ClientStartVote(clientNum,
                      common->GetLanguageDict()->GetString("#str_04271"));
      break;
    case VOTE_NEXTMAP:
      ServerStartVote(clientNum, voteIndex, "");
      ClientStartVote(clientNum,
                      common->GetLanguageDict()->GetString("#str_04272"));
      break;
    case VOTE_TIMELIMIT:
      ...
      if (vote_timeLimit == gameLocal.serverInfo.GetInt("si_timeLimit")) {
        gameLocal.ServerSendChatMessage(
            clientNum, "server",
            common->GetLanguageDict()->GetString("#str_04270"));
        common->DPrintf("client %d: already at the voted Time Limit\n",
                        clientNum);
        return;
      }
      if (vote_timeLimit < si_timeLimit.GetMinValue() ||
          vote_timeLimit > si_timeLimit.GetMaxValue()) {
        gameLocal.ServerSendChatMessage(
            clientNum, "server",
            common->GetLanguageDict()->GetString("#str_04269"));
        common->DPrintf(
      ...
      ClientStartVote(clientNum,
                      va(common->GetLanguageDict()->GetString("#str_04268"),
                         vote_timeLimit));
      break;
    case VOTE_FRAGLIMIT:
      ...
      if (vote_fragLimit == gameLocal.serverInfo.GetInt("si_fragLimit")) {
        gameLocal.ServerSendChatMessage(
            clientNum, "server",
            common->GetLanguageDict()->GetString("#str_04267"));
        common->DPrintf("client %d: already at the voted Frag Limit\n",
                        clientNum);
        return;
      }
      if (vote_fragLimit < si_fragLimit.GetMinValue() ||
          vote_fragLimit > si_fragLimit.GetMaxValue()) {
        gameLocal.ServerSendChatMessage(
            clientNum, "server",
            common->GetLanguageDict()->GetString("#str_04266"));
        common->DPrintf(
      ...
      ClientStartVote(
          clientNum,
          va(common->GetLanguageDict()->GetString("#str_04303"),
             gameLocal.gameType == GAME_LASTMAN
                 ? common->GetLanguageDict()->GetString("#str_04264")
                 : common->GetLanguageDict()->GetString("#str_04265"),
             vote_fragLimit));
      break;
    case VOTE_GAMETYPE:
      ...
      if (!idStr::Icmp(value, gameLocal.serverInfo.GetString("si_gameType"))) {
        gameLocal.ServerSendChatMessage(
            clientNum, "server",
            common->GetLanguageDict()->GetString("#str_04259"));
      ...
      ClientStartVote(
          clientNum,
          va(common->GetLanguageDict()->GetString("#str_04258"), value));
      break;
    case VOTE_KICK:
      ...
      if (vote_clientNum == gameLocal.localClientNum) {
        gameLocal.ServerSendChatMessage(
            clientNum, "server",
            common->GetLanguageDict()->GetString("#str_04257"));
      ...
      ClientStartVote(
          clientNum,
          va(common->GetLanguageDict()->GetString("#str_04302"), vote_clientNum,
             gameLocal.userInfo[vote_clientNum].GetString("ui_name")));
      break;
    case VOTE_MAP: {
      if (idStr::FindText(gameLocal.serverInfo.GetString("si_map"), value) !=
          -1) {
        gameLocal.ServerSendChatMessage(
            clientNum, "server",
            va(common->GetLanguageDict()->GetString("#str_04295"), value));
      ...
      if (!haveMap) {
        gameLocal.ServerSendChatMessage(
            clientNum, "server",
            va(common->GetLanguageDict()->GetString("#str_04296"), value));
        common->Printf("client %d: map not found: %s\n", clientNum, value);
        return;
      }
      ServerStartVote(clientNum, voteIndex, value);
      ClientStartVote(clientNum,
                      va(common->GetLanguageDict()->GetString("#str_04256"),
                         common->GetLanguageDict()->GetString(
                             dict ? dict->GetString("name") : value)));
      break;
    }
    case VOTE_SPECTATORS:
      if (gameLocal.serverInfo.GetBool("si_spectators")) {
        ServerStartVote(clientNum, voteIndex, "");
        ClientStartVote(clientNum,
                        common->GetLanguageDict()->GetString("#str_04255"));
      } else {
        ServerStartVote(clientNum, voteIndex, "");
        ClientStartVote(clientNum,
                        common->GetLanguageDict()->GetString("#str_04254"));
      }
      break;
    default:
      gameLocal.ServerSendChatMessage(
          clientNum, "server",
          va(common->GetLanguageDict()->GetString("#str_04297"),
             (int)voteIndex));
...
void idMultiplayerGame::CheckAbortGame(void) {
  ...
  if (gameLocal.gameType == GAME_TOURNEY && gameState == WARMUP) {
    ...
    for (i = 0; i < 2; i++) {
      if (!gameLocal.entities[currentTourneyPlayer[i]] ||
          static_cast<idPlayer*>(gameLocal.entities[currentTourneyPlayer[i]])
              ->spectating) {
  ...
  if (gameState != COUNTDOWN && gameState != GAMEON &&
      gameState != SUDDENDEATH) {
    return;
  }
...
void idMultiplayerGame::MapRestart(void) {
  ...
  if (g_balanceTDM.GetBool() && lastGameType != GAME_TDM &&
      lastGameType != GAME_CTF && gameLocal.mpGame.IsGametypeTeamBased()) {
#else
  if (g_balanceTDM.GetBool() && lastGameType != GAME_TDM &&
    ...
    for (clientNum = 0; clientNum < gameLocal.numClients; clientNum++) {
      if (gameLocal.entities[clientNum] &&
          gameLocal.entities[clientNum]->IsType(idPlayer::Type)) {
        if (static_cast<idPlayer*>(gameLocal.entities[clientNum])
                ->BalanceTDM()) {
...
void idMultiplayerGame::SwitchToTeam(int clientNum, int oldteam, int newteam) {
  ...
  for (i = 0; i < gameLocal.numClients; i++) {
    ...
    ent = gameLocal.entities[i];
    if (ent && ent->IsType(idPlayer::Type) &&
        static_cast<idPlayer*>(ent)->team == newteam) {
...
#ifdef CTF
  if ((gameState == GAMEON ||
       (IsGametypeFlagBased() && gameState == SUDDENDEATH)) &&
      oldteam != -1) {
#else
...
void idMultiplayerGame::ProcessChatMessage(int clientNum,
  ...
  if (clientNum >= 0) {
    ...
    if (p->spectating) {
      prefix = "spectating";
      if (team || (!g_spectatorChat.GetBool() &&
                   (gameState == GAMEON || gameState == SUDDENDEATH))) {
        // to specs
        send_to = 1;
      } else {
        // to all
...
void idMultiplayerGame::ToggleSpectate(void) {
  ...
  spectating =
      (idStr::Icmp(cvarSystem->GetCVarString("ui_spectate"), "Spectate") == 0);
  if (spectating) {
    ...
    if (gameLocal.serverInfo.GetBool("si_spectators")) {
      ...
      gameLocal.mpGame.AddChatLine(
          common->GetLanguageDict()->GetString("#str_06747"));
    }
  }
}
...
void idMultiplayerGame::ThrottleUserInfo(void) {
  ...
  while (ThrottleVars[i]) {
    if (idStr::Icmp(gameLocal.userInfo[gameLocal.localClientNum].GetString(
                        ThrottleVars[i]),
                    cvarSystem->GetCVarString(ThrottleVars[i]))) {
      if (gameLocal.realClientTime < switchThrottle[i]) {
        AddChatLine(
            common->GetLanguageDict()->GetString("#str_04299"),
            common->GetLanguageDict()->GetString(ThrottleVarsInEnglish[i]),
            (switchThrottle[i] - gameLocal.time) / 1000 + 1);
        cvarSystem->SetCVarString(
            ThrottleVars[i],
            gameLocal.userInfo[gameLocal.localClientNum].GetString(
                ThrottleVars[i]));
      } else {
...
void idMultiplayerGame::EnterGame(int clientNum) {
  ...
  if (!playerState[clientNum].ingame) {
    ...
    if (gameLocal.isMultiplayer) {
      ...
      gameLocal.ServerSendChatMessage(
          -1, common->GetLanguageDict()->GetString("#str_02047"),
          va(common->GetLanguageDict()->GetString("#str_07177"),
             gameLocal.userInfo[clientNum].GetString("ui_name")));
    }
  }
}
...
bool idMultiplayerGame::WantRespawn(idPlayer* p) {
  return p->forceRespawn && !p->wantSpectate &&
         playerState[p->entityNumber].ingame;
}
...
void idMultiplayerGame::ProcessVoiceChat(int clientNum, bool team, int index) {
  ...
  if (team || gameState == COUNTDOWN || gameState == GAMEREVIEW) {
    ProcessChatMessage(clientNum, team, name, spawnArgs->GetString(text_key),
                       spawnArgs->GetString(snd_key));
  } else {
...
void idMultiplayerGame::ClientReadStartState(const idBitMsg& msg) {
  ...
  while ((client = msg.ReadShort()) != MAX_CLIENTS) {
    assert(gameLocal.entities[client] &&
           gameLocal.entities[client]->IsType(idPlayer::Type));
...
void idMultiplayerGame::FindTeamFlags(void) {
  char* flagDefs[2] = {"team_CTF_redflag", "team_CTF_blueflag"};
...
bool idMultiplayerGame::IsFlagMsgOn(void) {
  return (GetGameState() == WARMUP || GetGameState() == GAMEON ||
          GetGameState() == SUDDENDEATH) &&
         flagMsgOn;
}
...
#ifdef _D3XP
idStr idMultiplayerGame::GetBestGametype(const char* map,
                                         const char* gametype) {
  int num = declManager->GetNumDecls(DECL_MAPDEF);
  int i, j;

...
void idGrabber::Restore(idRestoreGame* savefile) {
  ...
  if (dragEnt.IsValid()) {
    drag.SetPhysics(dragEnt.GetEntity()->GetPhysics(), id,
...
void idGrabber::StartDrag(idEntity* grabEnt, int id) {
  ...
  if (grabEnt->IsType(idProjectile::Type)) {
    ...
    // for helltime hunter)
    if (!idStr::Cmp(grabEnt->GetEntityDefName(),
                    "projectile_helltime_killer")) {
  ...
  localPlayerPoint = (thePlayer->firstPersonViewAxis[0] * HOLD_DISTANCE) *
                     thePlayer->firstPersonViewAxis.Transpose();
  ...
  drag.SetPhysics(phys, clipModelId,
                  thePlayer->firstPersonViewOrigin +
                      localPlayerPoint * thePlayer->firstPersonViewAxis);

  // start the screen warp
  warpId = thePlayer->playerView.AddWarp(phys->GetOrigin(), SCREEN_WIDTH / 2,
                                         SCREEN_HEIGHT / 2, 160, 2000);
}
  ...
  if (dragEnt.IsValid()) {
    ...
    if (holdingAF) {
      ...
      if (grabbableAI(ent->spawnArgs.GetString("classname"))) {
        ...
        aiEnt->Damage(thePlayer, thePlayer, vec3_origin, "damage_suicide", 1.0f,
                      INVALID_JOINT);
      }
      ...
      af_Phys->SetTimeScaleRamp(MS2SEC(gameLocal.slow.time) - 1.5f,
                                MS2SEC(gameLocal.slow.time) + 1.0f);
    }

    // If the object isn't near its goal, just drop it in place.
    if (!ent->IsType(idProjectile::Type) &&
        (dropOnly || drag.GetDistanceToGoal() > DRAG_FAIL_LEN)) {
      ent->GetPhysics()->SetLinearVelocity(vec3_origin);
      thePlayer->StartSoundShader(declManager->FindSound("grabber_maindrop"),
                                  SND_CHANNEL_WEAPON, 0, false, NULL);
      ...
      ent->ApplyImpulse(thePlayer, 0, ent->GetPhysics()->GetOrigin(),
                        thePlayer->firstPersonViewAxis[0] * THROW_SCALE *
                            ent->GetPhysics()->GetMass());
      thePlayer->StartSoundShader(declManager->FindSound("grabber_release"),
                                  SND_CHANNEL_WEAPON, 0, false, NULL);
...
int idGrabber::Update(idPlayer* player, bool hide) {
  ...
  if (endTime > gameLocal.time) {
    ...
    if (!gameLocal.isMultiplayer && !abort &&
        ((player->usercmd.flags & UCF_IMPULSE_SEQUENCE) !=
         (oldUcmdFlags & UCF_IMPULSE_SEQUENCE)) &&
        (player->usercmd.impulse == IMPULSE_13)) {
      abort = true;
    }
  ...
  if (!dragEnt.GetEntity()) {
    ...
    idVec3 end = player->firstPersonViewOrigin +
                 player->firstPersonViewAxis[0] * dragTraceDist;
    ...
    gameLocal.clip.TraceBounds(
        ...
        MASK_SHOT_RENDERMODEL | CONTENTS_PROJECTILE | CONTENTS_MOVEABLECLIP,
        player);
    ...
    if (trace.fraction < 1.0f) {
      ...
      if (newEnt &&
          ...
          newEnt->GetPhysics()->GetLinearVelocity().LengthSqr() <
              MAX_PICKUP_VELOCITY) {
        bool validAF = true;

        if (newEnt->IsType(idAFEntity_Gibbable::Type)) {
          idAFEntity_Gibbable* afEnt =
              static_cast<idAFEntity_Gibbable*>(newEnt);
  ...
  if (dragEnt.GetEntity() && allow) {
    ...
    goalPos = player->firstPersonViewOrigin +
              localPlayerPoint * player->firstPersonViewAxis;
...
void idGrabber::UpdateBeams(void) {
  ...
  if (dragEnt.IsValid()) {
    ...
    if (beamTarget) {
      beamTarget->SetOrigin(
          dragEnt.GetEntity()->GetPhysics()->GetAbsBounds().GetCenter());
    }

    muzzle_joint = thePlayer->weapon.GetEntity()->GetAnimator()->GetJointHandle(
        "particle_upper");
    if (muzzle_joint != INVALID_JOINT) {
      thePlayer->weapon.GetEntity()->GetJointWorldTransform(
          muzzle_joint, gameLocal.time, muzzle_origin, muzzle_axis);
    } else {
...
void idGrabber::ApplyShake(void) {
  float u = 1 - (float)(endTime - gameLocal.time) /
                    (g_grabberHoldSeconds.GetFloat() * 1000);

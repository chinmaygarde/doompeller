...
class idEntity : public idClass {
 public:
  ...
  struct entityFlags_s {
    ...
    bool bindOrientated : 1;  // if true both the master orientation is used for
                              // binding
...
ID_INLINE void SetTimeState::PushState(int timeGroup) {
  ...
  if (!gameLocal.isMultiplayer) {
    ...
    // set correct time
    if (fast) {
      gameLocal.fast.Get(gameLocal.time, gameLocal.previousTime, gameLocal.msec,
                         gameLocal.framenum, gameLocal.realClientTime);
    } else {
      gameLocal.slow.Get(gameLocal.time, gameLocal.previousTime, gameLocal.msec,
                         gameLocal.framenum, gameLocal.realClientTime);
    }
  }
}

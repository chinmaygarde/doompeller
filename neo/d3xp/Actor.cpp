  ...
  if (headEnt) {
    ...
    for (kv = spawnArgs.MatchPrefix("copy_joint", NULL); kv != NULL;
      ...
      if (copyJoint.to == INVALID_JOINT) {
        gameLocal.Warning("Unknown copy_joint '%s' on head of entity %s",
                          jointName.c_str(), name.c_str());
        continue;
      }
...
void idActor::CopyJointsFromBodyToHead(void) {
  ...
  for (i = 0; i < copyJoints.Num(); i++) {
    if (copyJoints[i].mod == JOINTMOD_WORLD_OVERRIDE) {
      ...
      animator.GetJointLocalTransform(copyJoints[i].from, gameLocal.time, pos,
                                      axis);
  ...
  if (headEnt) {
    ...
    if (!spawnArgs.GetFloat("eye_height", "0", eyeOffset.z)) {
      ...
      if (anim && (leftEyeJoint != INVALID_JOINT)) {
        idVec3 pos;
        idMat3 axis;
        headEnt->GetAnimator()->PlayAnim(ANIMCHANNEL_ALL, anim, gameLocal.time,
                                         0);
        headEnt->GetAnimator()->GetJointTransform(leftEyeJoint, gameLocal.time,
                                                  pos, axis);
...
idThread* idActor::ConstructScriptObject(void) {
  ...
  if (!scriptObject.HasObject()) {
    gameLocal.Error("No scriptobject set on '%s'.  Check the '%s' entityDef.",
...
bool idActor::StartRagdoll(void) {
  ...
  slomoStart =
      MS2SEC(gameLocal.time) + spawnArgs.GetFloat("ragdoll_slomoStart", "-1.6");
  slomoEnd =
  ...
  jointFrictionDentEnd = MS2SEC(gameLocal.time) +
    ...
    if (headEnt) {
      ...
      if (syncAnim) {
        ...
        if (anim) {
          ...
          headAnimator->PlayAnim(ANIMCHANNEL_ALL, anim, gameLocal.time,
                                 blendTime);
...
void idActor::Damage(idEntity* inflictor,
  ...
  if (finalBoss && idStr::Icmp(inflictor->GetEntityDefName(),
  ...
  if (!idStr::Icmp(GetEntityDefName(), "monster_boss_d3xp_maledict") &&
      (!idStr::Icmp(damageDefName, "damage_maledict_asteroid") ||
       !idStr::Icmp(damageDefName, "damage_maledict_asteroid_splash"))) {
    return;
  }
#else
  ...
  if (!anim) {
    if ((channel == ANIMCHANNEL_HEAD) && head.GetEntity()) {
      gameLocal.DPrintf("missing '%s' animation on '%s' (%s)\n", animname,
                        name.c_str(), spawnArgs.GetString("def_head", ""));
    } else {
      gameLocal.DPrintf("missing '%s' animation on '%s' (%s)\n", animname,
                        name.c_str(), GetEntityDefName());
    }
...
void idActor::Event_CheckAnim(int channel, const char* animname) {
  if (!GetAnim(channel, animname)) {
    if (animPrefix.Length()) {
      gameLocal.Error("Can't find anim '%s_%s' for '%s'", animPrefix.c_str(),
                      animname, name.c_str());
    } else {
...
void idActor::Event_AnimDistance(int channel, const char* animname) {
  ...
  if (anim) {
    if (channel == ANIMCHANNEL_HEAD) {
      if (head.GetEntity()) {
        idThread::ReturnFloat(
            head.GetEntity()->GetAnimator()->TotalMovementDelta(anim).Length());
        return;
      }
    } else {
...
void idActor::Event_NextEnemy(idEntity* ent) {
  ...
  if (!ent || (ent == this)) {
    ...
    if (actor->enemyNode.ListHead() != &enemyList) {
      gameLocal.Error("'%s' is not in '%s' enemy list", actor->name.c_str(),
                      name.c_str());
    }
  }

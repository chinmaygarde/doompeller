...
void idPhysics_StaticMulti::SetClipModel(idClipModel* model,
  ...
  clipModels[id] = model;
  if (clipModels[id]) {
    clipModels[id]->Link(gameLocal.clip, self, id, current[id].origin,
                         current[id].axis);
  }
...
bool idPhysics_StaticMulti::Evaluate(int timeStepMSec, int endTimeMSec) {
  ...
  if (hasMaster) {
    ...
    for (i = 0; i < clipModels.Num(); i++) {
      ...
      if (clipModels[i]) {
        clipModels[i]->Link(gameLocal.clip, self, i, current[i].origin,
                            current[i].axis);
      }
    }
...
void idPhysics_StaticMulti::Rotate(const idRotation& rotation, int id) {
  ...
  if (id >= 0 && id < clipModels.Num()) {
    ...
    if (hasMaster) {
      ...
      current[id].localOrigin =
          (current[id].origin - masterOrigin) * masterAxis.Transpose();
    } else {
    ...
    for (i = 0; i < clipModels.Num(); i++) {
      ...
      if (hasMaster) {
        ...
        current[i].localOrigin =
            (current[i].origin - masterOrigin) * masterAxis.Transpose();
      } else {
    ...
    if (clipModels[i]) {
      if (model) {
        contents |= gameLocal.clip.ContentsModel(
            clipModels[i]->GetOrigin(), clipModels[i], clipModels[i]->GetAxis(),
            -1, model->Handle(), model->GetOrigin(), model->GetAxis());
      } else {
        contents |=
            gameLocal.clip.Contents(clipModels[i]->GetOrigin(), clipModels[i],
                                    clipModels[i]->GetAxis(), -1, NULL);
      }
    }
  }
  return contents;
...
================
*/
const idVec3& idPhysics_StaticMulti::GetPushedAngularVelocity(
    const int id) const {
  return vec3_origin;
}

...
const int PMF_TIME_KNOCKBACK =
    64;  // movementTime is an air-accelerate only time
...
bool idPhysics_Player::SlideMove(bool gravity,
  ...
  for (bumpcount = 0; bumpcount < numbumps; bumpcount++) {
    ...
    if (stepUp) {
      ...
      if (!nearGround) {
        ...
        gameLocal.clip.Translation(downTrace, current.origin, stepEnd,
                                   clipModel, clipModel->GetAxis(), clipMask,
                                   self);
        nearGround = (downTrace.fraction < 1.0f &&
                      (downTrace.c.normal * -gravityNormal) > MIN_WALK_NORMAL);
      }
      ...
      if (nearGround) {
        ...
        gameLocal.clip.Translation(stepTrace, downTrace.endpos, stepEnd,
                                   clipModel, clipModel->GetAxis(), clipMask,
                                   self);

        // step down
        stepEnd = stepTrace.endpos + maxStepHeight * gravityNormal;
        gameLocal.clip.Translation(downTrace, stepTrace.endpos, stepEnd,
                                   clipModel, clipModel->GetAxis(), clipMask,
                                   self);
        ...
        if (downTrace.fraction >= 1.0f ||
          ...
          if (stepTrace.fraction >= 1.0f) {
            time_left = 0;
            current.stepUp -=
                (downTrace.endpos - current.origin) * gravityNormal;
    ...
    if (push && trace.c.entityNum != ENTITYNUM_WORLD) {
      ...
      // clip & push
      totalMass = gameLocal.push.ClipTranslationalPush(
  ...
  if (stepDown && groundPlane) {
    ...
    gameLocal.clip.Translation(downTrace, current.origin, stepEnd, clipModel,
                               clipModel->GetAxis(), clipMask, self);
...
void idPhysics_Player::WaterMove(void) {
  ...
  if (!scale) {
    ...
    wishvel = scale * (viewForward * command.forwardmove +
                       viewRight * command.rightmove);
...
void idPhysics_Player::WalkMove(void) {
  ...
  if (waterLevel > WATERLEVEL_WAIST &&
      (viewForward * groundTrace.c.normal) > 0.0f) {
    // begin swimming
    idPhysics_Player::WaterMove();
    return;
  }
...
void idPhysics_Player::NoclipMove(void) {
  ...
  wishdir = scale *
            (viewForward * command.forwardmove + viewRight * command.rightmove);
...
void idPhysics_Player::LadderMove(void) {
  ...
  current.velocity =
      (gravityNormal * current.velocity) * gravityNormal + wishvel;
  ...
  scale = idPhysics_Player::CmdScale(command);
  wishvel =
      -0.9f * gravityNormal * upscale * scale * (float)command.forwardmove;
...
void idPhysics_Player::CheckGround(void) {
  ...
  contents = gameLocal.clip.Contents(current.origin, clipModel,
                                     clipModel->GetAxis(), -1, self);
  ...
  if ((current.velocity * -gravityNormal) > 0.0f &&
      (current.velocity * groundTrace.c.normal) > 10.0f) {
    if (debugLevel) {
  ...
  if ((groundTrace.c.normal * -gravityNormal) < MIN_WALK_NORMAL) {
    ...
    if (current.velocity * gravityNormal > 150.0f) {
      current.velocity -=
          (current.velocity * gravityNormal - 150.0f) * gravityNormal;
    }
  ...
  if (groundEntityPtr.GetEntity()) {
    impactInfo_t info;
    groundEntityPtr.GetEntity()->GetImpactInfo(self, groundTrace.c.id,
                                               groundTrace.c.point, &info);
    if (info.invMass != 0.0f) {
      groundEntityPtr.GetEntity()->ApplyImpulse(
          self, groundTrace.c.id, groundTrace.c.point,
          current.velocity / (info.invMass * 10.0f));
    }
  }
}
    ...
    if (command.upmove < 0 && !ladder) {
      ...
      if (current.movementFlags & PMF_DUCKED) {
        // try to stand up
              ...
              (pm_normalheight.GetFloat() - pm_crouchheight.GetFloat()) *
                  gravityNormal;
        gameLocal.clip.Translation(trace, current.origin, end, clipModel,
                                   clipModel->GetAxis(), clipMask, self);
        if (trace.fraction >= 1.0f) {
...
void idPhysics_Player::CheckLadder(void) {
  ...
  if (trace.fraction < 1.0f) {
    ...
    if (trace.c.material &&
      ...
      gameLocal.clip.Translation(trace, start, end, clipModel,
                                 clipModel->GetAxis(), clipMask, self);
...
void idPhysics_Player::SetWaterLevel(void) {
  ...
  if (contents & MASK_WATER) {
    ...
    point =
        current.origin - (bounds[1][2] - bounds[0][2]) * 0.5f * gravityNormal;
    contents = gameLocal.clip.Contents(point, NULL, mat3_identity, -1, self);
    if (contents & MASK_WATER) {
...
bool idPhysics_Player::Evaluate(int timeStepMSec, int endTimeMSec) {
  ...
  if (IsOutsideWorld()) {
    gameLocal.Warning("clip model outside world bounds for entity '%s' at (%s)",
                      self->name.c_str(), current.origin.ToString(0));
  }
...
void idPhysics_Player::Rotate(const idRotation& rotation, int id) {
  ...
  clipModel->Link(gameLocal.clip, self, 0, current.origin,
                  clipModel->GetAxis() * rotation.ToMat3());
}
...
void idPhysics_Player::WriteToSnapshot(idBitMsgDelta& msg) const {
  ...
  msg.WriteFloat(current.velocity[0], PLAYER_VELOCITY_EXPONENT_BITS,
                 PLAYER_VELOCITY_MANTISSA_BITS);
  msg.WriteFloat(current.velocity[1], PLAYER_VELOCITY_EXPONENT_BITS,
                 PLAYER_VELOCITY_MANTISSA_BITS);
  msg.WriteFloat(current.velocity[2], PLAYER_VELOCITY_EXPONENT_BITS,
                 PLAYER_VELOCITY_MANTISSA_BITS);
...
void idPhysics_Player::ReadFromSnapshot(const idBitMsgDelta& msg) {
  ...
  current.velocity[0] = msg.ReadFloat(PLAYER_VELOCITY_EXPONENT_BITS,
                                      PLAYER_VELOCITY_MANTISSA_BITS);
  current.velocity[1] = msg.ReadFloat(PLAYER_VELOCITY_EXPONENT_BITS,
                                      PLAYER_VELOCITY_MANTISSA_BITS);
  current.velocity[2] = msg.ReadFloat(PLAYER_VELOCITY_EXPONENT_BITS,
                                      PLAYER_VELOCITY_MANTISSA_BITS);

...
void idPush::SaveEntityPosition(idEntity* ent) {
  ...
  if (ent->IsType(idActor::Type)) {
    ...
    pushed[numPushed].deltaViewAngles =
        static_cast<idActor*>(ent)->GetDeltaViewAngles();
  }
...
void idPush::RestorePushedEntityPositions(void) {
  ...
  for (i = 0; i < numPushed; i++) {
    ...
    if (pushed[i].ent->IsType(idActor::Type)) {
      ...
      static_cast<idActor*>(pushed[i].ent)
          ->SetDeltaViewAngles(pushed[i].deltaViewAngles);
    }
...
============
*/
bool idPush::CanPushEntity(idEntity* ent,
                           idEntity* pusher,
  ...
  if (!(ent->GetPhysics()->GetClipMask() &
        pusher->GetPhysics()->GetContents())) {
    return false;
  }
...
============
*/
void idPush::AddEntityToPushedGroup(idEntity* ent,
                                    float fraction,
                                    bool groundContact) {
  int i, j;
...
============
*/
bool idPush::ClipTranslationAgainstPusher(trace_t& results,
                                          idEntity* ent,
                                          idEntity* pusher,
  ...
  for (i = 0; i < n; i++) {
    ent->GetPhysics()->ClipTranslation(t, translation,
                                       pusher->GetPhysics()->GetClipModel(i));
...
============
*/
int idPush::GetPushableEntitiesForTranslation(idEntity* pusher,
                                              idEntity* initialPusher,
                                              const int flags,
                                              const idVec3& translation,
                                              idEntity* entityList[],
  ...
  pushBounds.FromBoundsTranslation(bounds, physics->GetOrigin(),
                                   physics->GetAxis(), translation);
...
float idPush::ClipTranslationalPush(trace_t& results,
  ...
  if (flags & PUSHFL_CLIP) {
    numListedEntities = GetPushableEntitiesForTranslation(
        pusher, pusher, flags, translation, entityList, MAX_GENTITIES);
  ...
  for (i = 0; i < pushedGroupSize;) {
    ...
    numListedEntities = GetPushableEntitiesForTranslation(
        curPusher, pusher, flags, realTranslation, entityList, MAX_GENTITIES);
    ...
    for (j = 0; j < numListedEntities; j++) {
      ...
      } else if (ClipTranslationAgainstPusher(trace, ent, curPusher,
                                              -fraction * realTranslation)) {
        AddEntityToPushedGroup(ent, (1.0f - trace.fraction) * fraction,
                               groundContact);
      }
    }
  }
  ...
  for (i = 1; i < pushedGroupSize; i++) {
    ...
    pushedGroup[i].ent->GetPhysics()->ClipTranslation(trace, partialTranslation,
                                                      NULL);
  ...
  if (blocked) {
    ...
    for (i = 0; i < pusher->GetPhysics()->GetNumClipModels(); i++) {
      pusher->GetPhysics()->GetClipModel(i)->Translate(results.fraction *
                                                       realTranslation);
...
============
*/
bool idPush::ClipRotationAgainstPusher(trace_t& results,
                                       idEntity* ent,
                                       idEntity* pusher,
  ...
  n = pusher->GetPhysics()->GetNumClipModels();
  for (i = 0; i < n; i++) {
    ent->GetPhysics()->ClipRotation(t, rotation,
                                    pusher->GetPhysics()->GetClipModel(i));
    if (t.fraction < results.fraction) {
      results = t;
    }
...
============
*/
int idPush::GetPushableEntitiesForRotation(idEntity* pusher,
                                           idEntity* initialPusher,
                                           const int flags,
                                           const idRotation& rotation,
                                           idEntity* entityList[],
  ...
  physics = pusher->GetPhysics();
  bounds = physics->GetBounds();
  pushBounds.FromBoundsRotation(bounds, physics->GetOrigin(),
...
float idPush::ClipRotationalPush(trace_t& results,
  ...
  // clip against all non-pushable physics objects
  if (flags & PUSHFL_CLIP) {
    numListedEntities = GetPushableEntitiesForRotation(
        pusher, pusher, flags, rotation, entityList, MAX_GENTITIES);
    // disable pushable entities for collision detection
    for (i = 0; i < numListedEntities; i++) {
      entityList[i]->GetPhysics()->DisableClip();
    }
  ...
  for (i = 0; i < pushedGroupSize;) {
    ...
    numListedEntities = GetPushableEntitiesForRotation(
        curPusher, pusher, flags, realRotation, entityList, MAX_GENTITIES);
    ...
    for (j = 0; j < numListedEntities; j++) {
      ...
      if (ent->GetPhysics()->IsGroundEntity(curPusher->entityNumber)) {
        AddEntityToPushedGroup(ent, 1.0f * fraction, false);
      } else if (ClipRotationAgainstPusher(trace, ent, curPusher,
                                           -fraction * realRotation)) {
        AddEntityToPushedGroup(ent, (1.0f - trace.fraction) * fraction,
                               groundContact);
      }
    }
  ...
  for (i = 1; i < pushedGroupSize; i++) {
    ...
    pushedGroup[i].ent->GetPhysics()->ClipRotation(trace, partialRotation,
                                                   NULL);
  ...
  if (blocked) {
    ...
    for (i = 1; i < pushedGroupSize; i++) {
      ...
      if (pushedGroup[i].ent->GetPhysics()->IsType(idPhysics_Actor::Type)) {
        ...
        if (!RotateEntityToAxial(
                pushedGroup[i].ent,
                pushedGroup[i].ent->GetPhysics()->GetOrigin())) {
...
int idPush::TryRotatePushEntity(trace_t& results,
  ...
  if (physics->IsGroundClipModel(clipModel->GetEntity()->entityNumber,
                                 clipModel->GetId())) {
    ...
    if (trace.fraction < 1.0f) {
      ...
      newRotation.Set(rotation.GetOrigin(), rotation.GetVec(),
                      -(rotation.GetAngle() - checkAngle));
    ...
    if (results.fraction >= 1.0f) {
      ...
      clipModel->Link(gameLocal.clip, clipModel->GetEntity(),
                      clipModel->GetId(), clipModel->GetOrigin(), newAxis);
      if (physics->ClipContents(clipModel)) {
        if (!startsolid) {
          int bah = 1;
        }
  ...
  newRotation.Set(rotation.GetOrigin(), rotation.GetVec(), checkAngle);
  // NOTE:	this code prevents msvc 6.0 & 7.0 from screwing up the above
  // code in
  ...
  // set pusher into final position
  clipModel->Link(gameLocal.clip, clipModel->GetEntity(), clipModel->GetId(),
                  clipModel->GetOrigin(), newAxis);
...
int idPush::TryTranslatePushEntity(trace_t& results,
  ...
  if (physics->IsGroundClipModel(clipModel->GetEntity()->entityNumber,
    ...
    if (trace.fraction < 1.0f) {
      ...
      // the entity in reverse only colliding with pusher
      ClipEntityTranslation(results, check, clipModel, NULL,
                            -(move - checkMove));
      // if there is a collision
      if (results.fraction < 1.0f) {
        // FIXME: try to push the blocking entity as well or try to slide along
...
int idPush::DiscardEntities(idEntity* entityList[],
  ...
  for (num = i = 0; i < numEntities; i++) {
    ...
    // if the entity doesn't clip with this pusher
    if (!(check->GetPhysics()->GetClipMask() &
          pusher->GetPhysics()->GetContents())) {
      continue;
    }
    ...
    if (check->IsType(idPlayer::Type) &&
        static_cast<idPlayer*>(check)->noclip) {
      continue;
    }
...
float idPush::ClipTranslationalPush(trace_t& results,
  ...
  pushBounds.FromBoundsTranslation(bounds, clipModel->GetOrigin(),
                                   clipModel->GetAxis(), translation);
  ...
  listedEntities = gameLocal.clip.EntitiesTouchingBounds(
      pushBounds, -1, entityList, MAX_GENTITIES);
  ...
  if (flags & PUSHFL_CLIP) {
    ...
    gameLocal.clip.Translation(results, clipModel->GetOrigin(),
                               clipModel->GetOrigin() + translation, clipModel,
                               clipModel->GetAxis(),
                               pusher->GetPhysics()->GetClipMask(), NULL);
  ...
  for (i = 0; i < listedEntities; i++) {
    ...
    res = TryTranslatePushEntity(pushResults, check, clipModel, flags,
                                 clipOrigin, clipMove);
    ...
    if (res == PUSH_OK) {
      ...
      check->ApplyImpulse(clipModel->GetEntity(), clipModel->GetId(),
                          clipModel->GetOrigin(), impulse);
    ...
    if (flags & PUSHFL_CRUSH) {
      check->Damage(clipModel->GetEntity(), clipModel->GetEntity(), vec3_origin,
                    "damage_crush", 1.0f,
                    CLIPMODEL_ID_TO_JOINT_HANDLE(pushResults.c.id));
      continue;
    }

    // if the entity is an active articulated figure and gibs
    if (check->IsType(idAFEntity_Base::Type) &&
        check->spawnArgs.GetBool("gib")) {
    ...
    if (check->IsType(idMoveableItem::Type) &&
        check->spawnArgs.GetBool("gib")) {
      check->ProcessEvent(&EV_Gib, "damage_Gib");
    }
...
float idPush::ClipRotationalPush(trace_t& results,
  ...
  pushBounds.FromBoundsRotation(bounds, clipModel->GetOrigin(),
                                clipModel->GetAxis(), rotation);
  ...
  for (i = 0; i < listedEntities; i++) {
    ...
    res = TryRotatePushEntity(pushResults, check, clipModel, flags, clipAxis,
                              clipRotation);
    ...
    if (res == PUSH_OK) {
      ...
      // put pusher back in old position
      clipModel->Link(gameLocal.clip, clipModel->GetEntity(),
                      clipModel->GetId(), clipModel->GetOrigin(), oldAxis);

      // wake up this object
      check->ApplyImpulse(clipModel->GetEntity(), clipModel->GetId(),
                          clipModel->GetOrigin(), vec3_origin);
...
float idPush::ClipPush(trace_t& results,
  ...
  if (translation != vec3_origin) {
    mass +=
        ClipTranslationalPush(results, pusher, flags, newOrigin, translation);
    if (results.fraction < 1.0f) {
      newOrigin = oldOrigin;
      newAxis = oldAxis;
      return mass;

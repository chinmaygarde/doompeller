...
class idAFConstraint {
 ...
 protected:
  ...
  struct constraintFlags_s {
    bool allowPrimary : 1;  // true if the constraint can be used as a primary
                            // constraint
    bool frameConstraint : 1;  // true if this constraint is added to the frame
                               // constraints
    bool noCollision : 1;  // true if body1 and body2 never collide with each
                           // other
...
class idAFConstraint_BallAndSocketJoint : public idAFConstraint {
 public:
  idAFConstraint_BallAndSocketJoint(const idStr& name,
                                    idAFBody* body1,
                                    idAFBody* body2);
  ...
  void SetConeLimit(const idVec3& coneAxis,
...
class idAFConstraint_UniversalJoint : public idAFConstraint {
 public:
  idAFConstraint_UniversalJoint(const idStr& name,
                                idAFBody* body1,
                                idAFBody* body2);
...
class idAFConstraint_CylindricalJoint : public idAFConstraint {
 public:
  idAFConstraint_CylindricalJoint(const idStr& name,
                                  idAFBody* body1,
                                  idAFBody* body2);
  virtual void DebugDraw(void);
  virtual void Translate(const idVec3& translation);
...
class idAFConstraint_Spring : public idAFConstraint {
 public:
  ...
  void SetSpring(const float stretch,
...
class idAFConstraint_Suspension : public idAFConstraint {
 public:
  ...
  void SetSuspension(const float up,
                     const float down,
                     const float k,
                     const float d,
                     const float f);
...
class idAFBody {
 ...
 public:
  ...
  idMat3 GetInverseWorldInertia(void) const {
    return current->worldAxis.Transpose() * inverseInertiaTensor *
           current->worldAxis;
  }
 ...
 private:
  ...
  idAFConstraint*
      primaryConstraint;  // primary constraint (this->constraint->body1 = this)
  ...
  struct bodyFlags_s {
    ...
    bool selfCollision : 1;  // true if this body can collide with other bodies
                             // of this AF
    bool spatialInertiaSparse : 1;  // true if the spatial inertia matrix is
                                    // sparse
    bool useFrictionDir : 1;  // true if a single friction direction should be
                              // used
...
class idAFTree {
 ...
 public:
  ...
  void Response(const idAFConstraint* constraint,
                int row,
...
class idPhysics_AF : public idPhysics_Base {
 public:
  ...
  int GetBodyContactConstraints(const int id,
  ...
  void SetSuspendTolerance(const float noMoveTime,
  ...
  void SetJointFrictionDent(const float dent,
                            const float start,
                            const float end);
  ...
  void SetContactFrictionDent(const float dent,
                              const float start,
                              const float end);
 ...
 public:  // common physics interface
  ...
  void GetImpactInfo(const int id,
                     const idVec3& point,
  ...
  void ClipTranslation(trace_t& results,
                       const idVec3& translation,
                       const idClipModel* model) const;
  void ClipRotation(trace_t& results,
 ...
 private:
  ...
  idList<idAFConstraint*>
      auxiliaryConstraints;  // list with auxiliary constraints
  idList<idAFConstraint*>
      frameConstraints;  // constraints that only live one frame
  ...
  float maxMoveTime;  // if > 0 the simulation is always suspeded after running
                      // this many seconds
  float impulseThreshold;  // threshold below which impulses are ignored to
                           // avoid continuous activation

  float
      timeScale;  // the time is scaled with this value for slow motion effects
  ...
  float jointFrictionDent;   // joint friction dives from 1 to this value and
                             // goes up again

...
int idClipModel::AllocTraceModel(const idTraceModel& trm) {
  ...
  entry->trm = trm;
  entry->trm.GetMassProperties(1.0f, entry->volume, entry->centerOfMass,
                               entry->inertiaTensor);
  entry->refCount = 1;
...
void idClipModel::FreeTraceModel(int traceModelIndex) {
  if (traceModelIndex < 0 || traceModelIndex >= traceModelCache.Num() ||
      traceModelCache[traceModelIndex]->refCount <= 0) {
    gameLocal.Warning(
...
int idClipModel::GetTraceModelHashKey(const idTraceModel& trm) {
  ...
  return (trm.type << 8) ^ (trm.numVerts << 4) ^ (trm.numEdges << 2) ^
         (trm.numPolys << 0) ^
         idMath::FloatHash(v.ToFloatPtr(), v.GetDimension());
}
...
void idClipModel::LoadModel(const int renderModelHandle) {
  ...
  if (renderModelHandle != -1) {
    const renderEntity_t* renderEntity =
        gameRenderWorld->GetRenderEntity(renderModelHandle);
    if (renderEntity) {
...
void idClipModel::Save(idSaveGame* savefile) const {
  ...
  if (collisionModelHandle >= 0) {
    savefile->WriteString(
        collisionModelManager->GetModelName(collisionModelHandle));
  } else {
    savefile->WriteString("");
  }
...
void idClipModel::Restore(idRestoreGame* savefile) {
  ...
  if (collisionModelName.Length()) {
    collisionModelHandle =
        collisionModelManager->LoadModel(collisionModelName, false);
  } else {
...
cmHandle_t idClipModel::Handle(void) const {
  ...
  if (collisionModelHandle) {
    ...
    return collisionModelManager->SetupTrmModel(
        *GetCachedTraceModel(traceModelIndex), material);
  } else {
    // this happens in multiplayer on the combat models
    gameLocal.Warning(
        "idClipModel::Handle: clip model %d on '%s' (%x) is not a collision or "
        "trace model",
...
void idClipModel::GetMassProperties(const float density,
                                    float& mass,
                                    idVec3& centerOfMass,
                                    idMat3& inertiaTensor) const {
  if (traceModelIndex == -1) {
    gameLocal.Error(
        "idClipModel::GetMassProperties: clip model %d on '%s' is not a trace "
        "model\n",
...
int idClip::EntitiesTouchingBounds(const idBounds& bounds,
  ...
  count = idClip::ClipModelsTouchingBounds(bounds, contentMask, clipModelList,
                                           MAX_GENTITIES);
  entCount = 0;
  for (i = 0; i < count; i++) {
...
int idClip::GetTraceClipModels(const idBounds& bounds,
                               ...
                               idClipModel** clipModelList) const {
  int i, num;
  idClipModel* cm;
  idEntity* passOwner;

  num = ClipModelsTouchingBounds(bounds, contentMask, clipModelList,
                                 MAX_GENTITIES);
  ...
  for (i = 0; i < num; i++) {
    ...
    if (cm->entity == passEntity) {
      ...
      if (cm->owner == passEntity) {
        ...
        clipModelList[i] =
            NULL;  // don't clip against other missiles from same owner
      }
    }
  }
...
void idClip::TraceRenderModel(trace_t& trace,
  ...
  if (touch->absBounds.Expand(radius).LineIntersection(start, end)) {
    ...
    if (gameRenderWorld->ModelTrace(modelTrace, touch->renderModelHandle, start,
                                    end, radius)) {
...
============
*/
const idTraceModel* idClip::TraceModelForClipModel(
  ...
  if (!mdl) {
    ...
    if (!mdl->IsTraceModel()) {
      if (mdl->GetEntity()) {
        gameLocal.Error(
            "TraceModelForClipModel: clip model %d on '%s' is not a trace "
            "model\n",
...
============
*/
ID_INLINE bool TestHugeTranslation(trace_t& results,
  ...
  if (mdl != NULL && (end - start).LengthSqr() > Square(CM_MAX_TRACE_DIST)) {
    ...
    // in the PVS of a player that has a flag that is spawning the
    // idMoveableItem "nuggets".  The error seems benign and the assert was
    // getting in the way of testing.
    assert(0);
#endif
    ...
    if (mdl->GetEntity()) {
      gameLocal.Printf("huge translation for clip model %d on entity %d '%s'\n",
                       mdl->GetId(), mdl->GetEntity()->entityNumber,
...
void idClip::TranslationEntities(trace_t& results,
  ...
  for (i = 0; i < num; i++) {
    ...
    if (touch->renderModelHandle != -1) {
      ...
      collisionModelManager->Translation(&trace, start, end, trm, trmAxis,
                                         contentMask, touch->Handle(),
                                         touch->origin, touch->axis);
    }
...
bool idClip::Translation(trace_t& results,
  ...
  if (!passEntity || passEntity->entityNumber != ENTITYNUM_WORLD) {
    // test world
    idClip::numTranslations++;
    collisionModelManager->Translation(&results, start, end, trm, trmAxis,
                                       contentMask, 0, vec3_origin,
                                       mat3_default);
    results.c.entityNum =
        results.fraction != 1.0f ? ENTITYNUM_WORLD : ENTITYNUM_NONE;
    if (results.fraction == 0.0f) {
  ...
  if (!trm) {
    ...
    traceBounds.FromBoundsTranslation(trm->bounds, start, trmAxis,
                                      results.endpos - start);
    radius = trm->bounds.GetRadius();
  }
...
bool idClip::Rotation(trace_t& results,
  ...
  if (!passEntity || passEntity->entityNumber != ENTITYNUM_WORLD) {
    // test world
    idClip::numRotations++;
    collisionModelManager->Rotation(&results, start, rotation, trm, trmAxis,
                                    contentMask, 0, vec3_origin, mat3_default);
    results.c.entityNum =
        results.fraction != 1.0f ? ENTITYNUM_WORLD : ENTITYNUM_NONE;
    if (results.fraction == 0.0f) {
      return true;  // blocked immediately by the world
  ...
  for (i = 0; i < num; i++) {
    ...
    collisionModelManager->Rotation(&trace, start, rotation, trm, trmAxis,
                                    contentMask, touch->Handle(), touch->origin,
                                    touch->axis);
...
bool idClip::Motion(trace_t& results,
  ...
  if (mdl != NULL && rotation.GetAngle() != 0.0f &&
      rotation.GetVec() != vec3_origin) {
    // if no translation
    if (start == end) {
      // pure rotation
      return Rotation(results, start, rotation, mdl, trmAxis, contentMask,
                      passEntity);
    }
  } else if (start != end) {
    // pure translation
    return Translation(results, start, end, mdl, trmAxis, contentMask,
                       passEntity);
  } else {
    // no motion
    results.fraction = 1.0f;
  ...
  if (!passEntity || passEntity->entityNumber != ENTITYNUM_WORLD) {
    ...
    idClip::numTranslations++;
    collisionModelManager->Translation(&translationalTrace, start, end, trm,
                                       trmAxis, contentMask, 0, vec3_origin,
                                       mat3_default);
    translationalTrace.c.entityNum =
        translationalTrace.fraction != 1.0f ? ENTITYNUM_WORLD : ENTITYNUM_NONE;
  } else {
  ...
  if (translationalTrace.fraction != 0.0f) {
    ...
    num =
        GetTraceClipModels(traceBounds, contentMask, passEntity, clipModelList);
  ...
  if (!passEntity || passEntity->entityNumber != ENTITYNUM_WORLD) {
    ...
    idClip::numRotations++;
    collisionModelManager->Rotation(&rotationalTrace, endPosition, endRotation,
                                    trm, trmAxis, contentMask, 0, vec3_origin,
                                    mat3_default);
    rotationalTrace.c.entityNum =
        rotationalTrace.fraction != 1.0f ? ENTITYNUM_WORLD : ENTITYNUM_NONE;
  } else {
  ...
  if (rotationalTrace.fraction != 0.0f) {
    if (num == -1) {
      traceBounds.FromBoundsRotation(trm->bounds, endPosition, trmAxis,
                                     endRotation);
      num = GetTraceClipModels(traceBounds, contentMask, passEntity,
                               clipModelList);
    }
    ...
    for (i = 0; i < num; i++) {
      ...
      idClip::numRotations++;
      collisionModelManager->Rotation(&trace, endPosition, endRotation, trm,
                                      trmAxis, contentMask, touch->Handle(),
                                      touch->origin, touch->axis);
  ...
  return (translationalTrace.fraction < 1.0f ||
          rotationalTrace.fraction < 1.0f);
}
...
int idClip::Contacts(contactInfo_t* contacts,
  ...
  if (!passEntity || passEntity->entityNumber != ENTITYNUM_WORLD) {
    // test world
    idClip::numContacts++;
    numContacts = collisionModelManager->Contacts(
        contacts, maxContacts, start, dir, depth, trm, trmAxis, contentMask, 0,
        vec3_origin, mat3_default);
  } else {
    numContacts = 0;
  }
  ...
  for (i = 0; i < num; i++) {
    ...
    n = collisionModelManager->Contacts(
        contacts + numContacts, maxContacts - numContacts, start, dir, depth,
        trm, trmAxis, contentMask, touch->Handle(), touch->origin, touch->axis);
...
int idClip::Contents(const idVec3& start,
  ...
  if (!passEntity || passEntity->entityNumber != ENTITYNUM_WORLD) {
    // test world
    idClip::numContents++;
    contents = collisionModelManager->Contents(start, trm, trmAxis, contentMask,
                                               0, vec3_origin, mat3_default);
  } else {
    contents = 0;
  }
  ...
  for (i = 0; i < num; i++) {
    ...
    idClip::numContents++;
    if (collisionModelManager->Contents(start, trm, trmAxis, contentMask,
...
void idClip::RotationModel(trace_t& results,
  ...
  idClip::numRotations++;
  collisionModelManager->Rotation(&results, start, rotation, trm, trmAxis,
                                  contentMask, model, modelOrigin, modelAxis);
}
...
int idClip::ContactsModel(contactInfo_t* contacts,
  ...
  const idTraceModel* trm = TraceModelForClipModel(mdl);
  idClip::numContacts++;
  return collisionModelManager->Contacts(contacts, maxContacts, start, dir,
                                         depth, trm, trmAxis, contentMask,
                                         model, modelOrigin, modelAxis);
}
...
int idClip::ContentsModel(const idVec3& start,
  ...
  const idTraceModel* trm = TraceModelForClipModel(mdl);
  idClip::numContents++;
  return collisionModelManager->Contents(start, trm, trmAxis, contentMask,
                                         model, modelOrigin, modelAxis);
}
...
bool idClip::GetModelContactFeature(const contactInfo_t& contact,
  ...
  if (clipModel == NULL) {
    ...
    if (clipModel->renderModelHandle != -1) {
      ...
      handle = collisionModelManager->SetupTrmModel(
  ...
  if (handle != -1) {
    switch (contact.type) {
      case CONTACT_EDGE: {
        ...
        collisionModelManager->GetModelEdge(handle, contact.modelFeature, start,
                                            end);
        winding += start;
        winding += end;
        break;
...
void idClip::PrintStatistics(void) {
  gameLocal.Printf(
      "t = %-3d, r = %-3d, m = %-3d, render = %-3d, contents = %-3d, contacts "
      "= %-3d\n",
...
bool idClip::DrawModelContactFeature(const contactInfo_t& contact,
  ...
  if (winding.GetNumPoints() == 1) {
    gameRenderWorld->DebugLine(colorCyan, winding[0].ToVec3(),
                               winding[0].ToVec3() + 2.0f * axis[0], lifetime);
    gameRenderWorld->DebugLine(colorWhite, winding[0].ToVec3() - 1.0f * axis[1],
                               winding[0].ToVec3() + 1.0f * axis[1], lifetime);
    gameRenderWorld->DebugLine(colorWhite, winding[0].ToVec3() - 1.0f * axis[2],
                               winding[0].ToVec3() + 1.0f * axis[2], lifetime);
  } else {
    for (i = 0; i < winding.GetNumPoints(); i++) {
      gameRenderWorld->DebugLine(
          colorCyan, winding[i].ToVec3(),
          winding[(i + 1) % winding.GetNumPoints()].ToVec3(), lifetime);
    }
  }

  axis[0] = -axis[0];
  axis[2] = -axis[2];
  gameRenderWorld->DrawText(contact.material->GetName(),
                            winding.GetCenter() - 4.0f * axis[2], 0.1f,
                            colorWhite, axis, 1, 5000);

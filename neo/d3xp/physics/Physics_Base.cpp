...
bool idPhysics_Base::IsGroundEntity(int entityNum) const {
  ...
  for (i = 0; i < contacts.Num(); i++) {
    if (contacts[i].entityNum == entityNum &&
        (contacts[i].normal * -gravityNormal > 0.0f)) {
      return true;
    }
  }
  return false;
...
bool idPhysics_Base::IsGroundClipModel(int entityNum, int id) const {
  ...
  for (i = 0; i < contacts.Num(); i++) {
    if (contacts[i].entityNum == entityNum && contacts[i].id == id &&
        (contacts[i].normal * -gravityNormal > 0.0f)) {
      return true;
    }
  }
...
bool idPhysics_Base::IsOutsideWorld(void) const {
  if (!gameLocal.clip.GetWorldBounds().Expand(128.0f).IntersectsBounds(
          GetAbsBounds())) {
    return true;
  }
  return false;

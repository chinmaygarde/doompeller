...
const idEventDef EV_Weapon_LaunchProjectilesEllipse("launchProjectilesEllipse",
                                                    "dffff");
...
EVENT(EV_Weapon_LaunchProjectilesEllipse,
...
void idWeapon::Restore(idRestoreGame* savefile) {
  ...
  for (int i = 0; i < particleCount; i++) {
    ...
    if (newParticle.smoke) {
      newParticle.particle = static_cast<const idDeclParticle*>(
          declManager->FindType(DECL_PARTICLE, particlename, false));
    } else {
...
void idWeapon::GetWeaponDef(const char* objectname, int ammoinclip) {
  ...
  if (!scriptObject.SetType(objectType)) {
    gameLocal.Error("Script object '%s' not found on weapon '%s'.", objectType,
                    objectname);
  }
  ...
  if (!gameLocal.isMultiplayer) {
    const idKeyValue* pkv =
        weaponDef->dict.MatchPrefix("weapon_particle", NULL);
    while (pkv) {
      WeaponParticle_t newParticle;
      memset(&newParticle, 0, sizeof(newParticle));
      ...
      idStr particle =
          weaponDef->dict.GetString(va("%s_particle", name.c_str()));
      strcpy(newParticle.particlename, particle.c_str());

      if (newParticle.smoke) {
        newParticle.particle = static_cast<const idDeclParticle*>(
            declManager->FindType(DECL_PARTICLE, particle, false));
      } else {
        idDict args;
    ...
    while (lkv) {
      ...
      newLight.light.lightRadius[0] = newLight.light.lightRadius[1] =
...
void idWeapon::UpdateGUI(void) {
  ...
  if (gameLocal.localClientNum != owner->entityNumber) {
    ...
    if (gameLocal.localClientNum >= 0 &&
      ...
      idPlayer* p =
          static_cast<idPlayer*>(gameLocal.entities[gameLocal.localClientNum]);
...
#endif
    renderEntity.gui[0]->SetStateString("player_ammo",
                                        ClipSize() ? va("%i", inclip) : "--");
    renderEntity.gui[0]->SetStateString(
        "player_clips", ClipSize() ? va("%i", ammoamount / ClipSize()) : "--");

#ifdef _D3XP
    renderEntity.gui[0]->SetStateString("player_allammo",
                                        va("%i/%i", inclip, ammoamount));
#else
    renderEntity.gui[0]->SetStateString(
  ...
  // value
  renderEntity.gui[0]->SetStateString("player_ammo_count",
                                      va("%i", AmmoCount()));
...
idEntity* idWeapon::DropItem(const idVec3& velocity,
  ...
  return idMoveableItem::DropItem(
      ...
      worldModel.GetEntity()->GetPhysics()->GetAxis(), velocity, activateDelay,
      removeDelay);
}
...
void idWeapon::AlertMonsters(void) {
  ...
  if (g_debugWeapon.GetBool()) {
    ...
    gameRenderWorld->DebugArrow(colorGreen, muzzleFlash.origin, tr.endpos, 2,
                                0);
  }
...
void idWeapon::PresentWeapon(bool showViewModel) {
  ...
  if (showViewModel && !disabled && weaponSmoke &&
    ...
    if (!gameLocal.smokeParticles->EmitSmoke(
            weaponSmoke, weaponSmokeStartTime, gameLocal.random.RandomFloat(),
            muzzleOrigin, muzzleAxis, timeGroup /*_D3XP*/)) {
      weaponSmokeStartTime = (continuousSmoke) ? gameLocal.time : 0;
    }
  }
  ...
  if (showViewModel && strikeSmoke && strikeSmokeStartTime != 0) {
    // spit out a particle
    if (!gameLocal.smokeParticles->EmitSmoke(
            strikeSmoke, strikeSmokeStartTime, gameLocal.random.RandomFloat(),
            strikePos, strikeAxis, timeGroup /*_D3XP*/)) {
      strikeSmokeStartTime = 0;
    }
  }
  ...
  if (showViewModel && !hide) {
      ...
      if (part->active) {
        if (part->smoke) {
          if (part->joint != INVALID_JOINT) {
            GetGlobalJointTransform(true, part->joint, muzzleOrigin,
                                    muzzleAxis);
          } else {
            // default to going straight out the view
            muzzleOrigin = playerViewOrigin;
          ...
          GetGlobalJointTransform(true, part->joint, rendEnt->origin,
                                  rendEnt->axis);

          if (part->emitter->GetModelDefHandle() != -1) {
            gameRenderWorld->UpdateEntityDef(part->emitter->GetModelDefHandle(),
                                             rendEnt);
          }
        }
      }
    ...
    for (int i = 0; i < weaponLights.Num(); i++) {
      ...
      if (light->active) {
        GetGlobalJointTransform(true, light->joint, light->light.origin,
                                light->light.axis);
        if ((light->lightHandle != -1)) {
...
ammo_t idWeapon::GetAmmoNumForName(const char* ammoname) {
  ...
  if (!ammoDict->GetInt(ammoname, "-1", num)) {
    ...
    for (i = 0; i < 2; i++) {
      ...
      if (gamedir.Length() > 0) {
        ammoDict = gameLocal.FindEntityDefDict(
            va("ammo_types_%s", gamedir.c_str()), false);
        if (ammoDict) {
          if (ammoDict->GetInt(ammoname, "-1", num)) {
            break;
          }
...
void idWeapon::Event_SetSkin(const char* skinname) {
  ...
  if (gameLocal.isServer) {
    ...
    msg.WriteLong((skinDecl != NULL) ? gameLocal.ServerRemapDecl(
...
void idWeapon::Event_LaunchProjectiles(int num_projectiles,
  ...
  if (worldModel.GetEntity()) {
    worldModel.GetEntity()->SetShaderParm(
        SHADERPARM_DIVERSITY, renderEntity.shaderParms[SHADERPARM_DIVERSITY]);
    worldModel.GetEntity()->SetShaderParm(
        SHADERPARM_TIMEOFFSET, renderEntity.shaderParms[SHADERPARM_TIMEOFFSET]);
  }
  ...
  if (gameLocal.isClient) {
    ...
    if (projectileDict.GetBool("net_instanthit")) {
      ...
      for (i = 0; i < num_projectiles; i++) {
        ...
        dir.Normalize();
        gameLocal.clip.Translation(tr, muzzle_pos, muzzle_pos + dir * 4096.0f,
                                   NULL, mat3_identity, MASK_SHOT_RENDERMODEL,
                                   owner);
        if (tr.fraction < 1.0f) {
          idProjectile::ClientPredictionCollide(this, projectileDict, tr,
                                                vec3_origin, true);
        }
      }
    }
...
================
*/
void idWeapon::Event_LaunchProjectilesEllipse(int num_projectiles,
                                              float spreada,
                                              float spreadb,
                                              float fuseOffset,
                                              float power) {
  ...
  if (!gameLocal.isClient) {
    ...
    for (i = 0; i < num_projectiles; i++) {
      ...
      dir = playerViewAxis[0] + playerViewAxis[2] * (angb * idMath::Sin(spin)) -
            playerViewAxis[1] * (anga * idMath::Cos(spin));
      dir.Normalize();
 ...
 */
void idWeapon::Event_LaunchPowerup(const char* powerup,
                                   float duration,
                                   int useAmmo) {
  if (IsHidden()) {
    return;
...
void idWeapon::Event_Melee(void) {
  ...
  if (!gameLocal.isClient) {
    ...
    idVec3 end =
        start + playerViewAxis[0] *
                    (meleeDistance * owner->PowerUpModifier(MELEE_DISTANCE));
    gameLocal.clip.TracePoint(tr, start, end, MASK_SHOT_RENDERMODEL, owner);
    if (tr.fraction < 1.0f) {
      ent = gameLocal.GetTraceEntity(tr);
    } else {
    ...
    if (g_debugWeapon.GetBool()) {
      ...
      if (ent) {
        gameRenderWorld->DebugBounds(colorRed, ent->GetPhysics()->GetBounds(),
    ...
    if (ent) {
      ...
      if (gameLocal.world->spawnArgs.GetBool("no_Weapons") &&
          (ent->IsType(idActor::Type) || ent->IsType(idAFAttachment::Type))) {
        idThread::ReturnInt(0);
        return;
      }
      ...
      if (gameLocal.isMultiplayer && weaponDef &&
          weaponDef->dict.GetBool("stealing") && ent->IsType(idPlayer::Type) &&
        ...
        if (gameLocal.mpGame.IsGametypeFlagBased()) { /* Do nothing ... */
        } else
#endif
      ...
      if (weaponDef->dict.GetBool("impact_damage_effect")) {
        if (ent->spawnArgs.GetBool("bleed")) {
          hitSound = meleeDef->dict.GetString(
              owner->PowerUpActive(BERSERK) ? "snd_hit_berserk" : "snd_hit");
          ...
          if (gameLocal.time > nextStrikeFx) {
            ...
            if (decal && *decal) {
              gameLocal.ProjectDecal(tr.c.point, -tr.c.normal, 8.0f, true, 6.0,
                                     decal);
            }

...
typedef struct {
  ...
  int stamina;
  int combat;
  bool syncNextGameFrame;  // used when cinematics are skipped to prevent
                           // session from simulating several game frames to
...
class idGame {
 public:
  ...
  virtual const idDict* SetUserInfo(int clientNum,
  ...
  virtual void SetPersistentPlayerInfo(int clientNum,
                                       const idDict& playerInfo) = 0;

  // Loads a map and spawns all the entities.
  virtual void InitFromNewMap(const char* mapName,
  ...
  virtual bool InitFromSaveGame(const char* mapName,
                                idRenderWorld* renderWorld,
                                idSoundWorld* soundWorld,
  ...
  virtual allowReply_t ServerAllowClient(int numClients,
                                         const char* IP,
                                         const char* guid,
                                         const char* password,
  ...
  virtual void ServerWriteSnapshot(int clientNum,
                                   int sequence,
                                   idBitMsg& msg,
                                   byte* clientInPVS,
                                   int numPVSClients) = 0;
  ...
  virtual void ServerProcessReliableMessage(int clientNum,
                                            const idBitMsg& msg) = 0;

  // Reads a snapshot and updates the client game state.
  virtual void ClientReadSnapshot(int clientNum,
                                  int sequence,
                                  const int gameFrame,
                                  const int gameTime,
  ...
  virtual void ClientProcessReliableMessage(int clientNum,
                                            const idBitMsg& msg) = 0;

  // Runs prediction on entities at the client.
  virtual gameReturn_t ClientPrediction(int clientNum,
                                        const usercmd_t* clientCmds,
                                        bool lastPredictFrame) = 0;
  ...
  virtual void GetBestGameType(const char* map,
                               const char* gametype,
  ...
  virtual bool DownloadRequest(const char* IP,
                               const char* guid,
                               const char* paks,
                               char urls[MAX_STRING_CHARS]) = 0;
...
typedef struct {
  idSoundEmitter*
      referenceSound;  // this is the interface to the sound system, created
...
class idGameEdit {
 public:
  ...
  // the game and tools.
  virtual void ParseSpawnArgsToRenderLight(const idDict* args,
                                           renderLight_t* renderLight);
  virtual void ParseSpawnArgsToRenderEntity(const idDict* args,
                                            renderEntity_t* renderEntity);
  virtual void ParseSpawnArgsToRefSound(const idDict* args,
                                        refSound_t* refSound);
  ...
  virtual const idMD5Anim* ANIM_GetAnimFromEntityDef(const char* classname,
                                                     const char* animname);
  virtual int ANIM_GetNumAnimsFromEntityDef(const idDict* args);
  virtual const char* ANIM_GetAnimNameFromEntityDef(const idDict* args,
                                                    int animNum);
  ...
  virtual void ANIM_CreateAnimFrame(const idRenderModel* model,
  ...
  virtual idRenderModel* ANIM_CreateMeshForAnim(idRenderModel* model,
  ...
  virtual idRenderModel* AF_CreateMesh(const idDict& args,
                                       idVec3& meshOrigin,
                                       idMat3& meshAxis,
                                       bool& poseIsSet);
  ...
  virtual const idDict* FindEntityDefDict(const char* name,
  ...
  virtual void EntityUpdateChangeableSpawnArgs(idEntity* ent,
                                               const idDict* dict);
  ...
  virtual void MapSetEntityKeyVal(const char* name,
                                  const char* key,
                                  const char* val) const;
  virtual void MapCopyDictToEntity(const char* name, const idDict* dict) const;
  virtual int MapGetUniqueMatchingKeyVals(const char* key,
                                          const char* list[],
                                          const int max) const;
  virtual void MapAddEntity(const idDict* dict) const;
  virtual int MapGetEntitiesMatchingClassWithString(const char* classname,
                                                    const char* match,
                                                    const char* list[],
                                                    const int max) const;

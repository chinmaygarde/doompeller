...
const char* idGameLocal::sufaceTypeNames[MAX_SURFACE_TYPES] = {
    "none",       "metal",      "stone",      "flesh",
    "wood",       "cardboard",  "liquid",     "glass",
    "plastic",    "ricochet",   "surftype10", "surftype11",
    "surftype12", "surftype13", "surftype14", "surftype15"};

#ifdef _D3XP
// List of all defs used by the player that will stay on the fast timeline
static char* fastEntityList[] = {"player_doommarine",
                                 "weapon_chainsaw",
                                 "weapon_fists",
                                 "weapon_flashlight",
...
void idGameLocal::Init(void) {
  ...
  declManager->RegisterDeclType("model", DECL_MODELDEF,
                                idDeclAllocator<idDeclModelDef>);
  declManager->RegisterDeclType("export", DECL_MODELEXPORT,
                                idDeclAllocator<idDecl>);
  ...
  cmdSystem->AddCommand("listModelDefs", idListDecls_f<DECL_MODELDEF>,
                        CMD_FL_SYSTEM | CMD_FL_GAME, "lists model defs");
  cmdSystem->AddCommand("printModelDefs", idPrintDecls_f<DECL_MODELDEF>,
                        CMD_FL_SYSTEM | CMD_FL_GAME, "prints a model def",
  ...
  if (clientNum >= 0 && clientNum < MAX_CLIENTS) {
    ...
    if (canModify) {
      ...
      if (idStr::IsNumeric(this->userInfo[clientNum].GetString("ui_name"))) {
        idGameLocal::userInfo[clientNum].Set(
            "ui_name",
            va("%s_", idGameLocal::userInfo[clientNum].GetString("ui_name")));
        modifiedInfo = true;
      }
      ...
      for (i = 0; i < numClients; i++) {
        ...
        if (entities[i] && entities[i]->IsType(idPlayer::Type)) {
          if (!idStr::Icmp(
                  idGameLocal::userInfo[clientNum].GetString("ui_name"),
                  idGameLocal::userInfo[i].GetString("ui_name"))) {
            idGameLocal::userInfo[clientNum].Set(
                "ui_name", va("%s_", idGameLocal::userInfo[clientNum].GetString(
                                         "ui_name")));
            modifiedInfo = true;
            i = -1;  // rescan
            continue;
    ...
    if (entities[clientNum] && entities[clientNum]->IsType(idPlayer::Type)) {
      modifiedInfo |= static_cast<idPlayer*>(entities[clientNum])
                          ->UserInfoChanged(canModify);
    }
...
void idGameLocal::LoadMap(const char* mapName, int randseed) {
  ...
  for (i = 0; i < aasNames.Num(); i++) {
    aasList[i]->Init(idStr(mapFileName).SetFileExtension(aasNames[i]).c_str(),
                     mapFile->GetGeometryCRC());
  }
  ...
  if (isClient) {
    ...
    for (i = 0; i < newInfo.GetNumKeyVals(); i++) {
      ...
      if (keyval->GetValue().Cmp(keyval2->GetValue()) &&
          (!keyval->GetKey().Cmp("si_pure") ||
           !keyval->GetKey().Cmp("si_map"))) {
        break;
      }
    }
...
bool idGameLocal::NextMap(void) {
  ...
  if (fileSystem->ReadFile(g_mapCycle.GetString(), NULL, NULL) < 0) {
    if (fileSystem->ReadFile(va("%s.scriptcfg", g_mapCycle.GetString()), NULL,
                             NULL) < 0) {
...
bool idGameLocal::InitFromSaveGame(const char* mapName,
  ...
  if (num) {
    if (num != gameRenderWorld->NumAreas()) {
      savegame.Error(
          "idGameLocal::InitFromSaveGame: number of areas in map differs from "
          "save game.");
    }
...
void idGameLocal::MapClear(bool clearClients) {
  ...
  if (!clearClients) {
    ...
    for (i = 0; i < MAX_CLIENTS; i++) {
      ...
      entityHash.Add(entityHash.GenerateKey(entities[i]->name.c_str(), true),
                     i);
    }
  }
...
void idGameLocal::DumpOggSounds(void) {
  ...
  for (i = 0; i < declManager->GetNumDecls(DECL_SOUND); i++) {
    soundShader = static_cast<const idSoundShader*>(
        declManager->DeclByIndex(DECL_SOUND, i, false));
    parms = soundShader->GetParms();

    if (soundShader->EverReferenced() &&
        soundShader->GetState() != DS_DEFAULTED) {
...
void idGameLocal::CacheDictionaryMedia(const idDict* dict) {
  ...
  if (kv && kv->GetValue().Length()) {
    ...
    const char* p = (teleportType) ? va("fx/teleporter%i.fx", teleportType)
                                   : "fx/teleporter.fx";
...
void idGameLocal::SpawnPlayer(int clientNum) {
...
#else
  args.Set("classname",
           isMultiplayer ? "player_doommarine_mp" : "player_doommarine");
#endif
  ...
  if (!ent->IsType(idPlayer::Type)) {
    Error(
        "'%s' spawned the player as a '%s'.  Player spawnclass must be a "
...
idPlayer* idGameLocal::GetLocalPlayer() const {
  ...
  if (!entities[localClientNum] ||
      !entities[localClientNum]->IsType(idPlayer::Type)) {
...
void idGameLocal::SetupPlayerPVS(void) {
  ...
  for (i = 0; i < numClients; i++) {
    ...
    if (portalSkyEnt.GetEntity()) {
      ...
      otherPVS =
          pvs.SetupCurrentPVS(skyEnt->GetPVSAreas(), skyEnt->GetNumPVSAreas());
      newPVS = pvs.MergeCurrentPVS(playerPVS, otherPVS);
      pvs.FreeCurrentPVS(playerPVS);
      pvs.FreeCurrentPVS(otherPVS);
      playerPVS = newPVS;
...
bool idGameLocal::InPlayerConnectedArea(idEntity* ent) const {
  ...
  return pvs.InCurrentPVS(playerConnectedAreas, ent->GetPVSAreas(),
                          ent->GetNumPVSAreas());
}
...
void idGameLocal::UpdateGravity(void) {
  ...
  if (g_gravity.IsModified()) {
    ...
    for (ent = spawnedEntities.Next(); ent != NULL;
  ...
  if (!isMultiplayer && g_stopTime.GetBool()) {
    ...
    do {
      ...
      if (g_timeentities.GetFloat()) {
        num = 0;
        for (ent = activeEntities.Next(); ent != NULL;
      ...
      if (skipCinematic && (time > cinematicMaxSkipTime)) {
        Warning(
            "Exceeded maximum cinematic skip length.  Cinematic may be looping "
            "infinitely.");
        skipCinematic = false;
        break;
      }
...
void idGameLocal::CallObjectFrameCommand(idEntity* ent,
  ...
  if (!func) {
    if (!ent->IsType(idTestModel::Type)) {
      Error("Unknown function '%s' called for frame command on entity '%s'",
            frameCommand, ent->name.c_str());
    }
  } else {
...
void idGameLocal::ShowTargets(void) {
  ...
  for (ent = spawnedEntities.Next(); ent != NULL; ent = ent->spawnNode.Next()) {
    ...
    gameRenderWorld->DebugBounds(
        (ent->IsHidden() ? colorLtGrey : colorOrange) * frac,
        ent->GetPhysics()->GetAbsBounds());
    if (viewTextBounds.IntersectsBounds(ent->GetPhysics()->GetAbsBounds())) {
      idVec3 center = ent->GetPhysics()->GetAbsBounds().GetCenter();
      gameRenderWorld->DrawText(ent->name.c_str(), center - up, 0.1f,
                                colorWhite * frac, axis, 1);
      gameRenderWorld->DrawText(ent->GetEntityDefName(), center, 0.1f,
                                colorWhite * frac, axis, 1);
      gameRenderWorld->DrawText(va("#%d", ent->entityNumber), center + up, 0.1f,
                                colorWhite * frac, axis, 1);
    }
    ...
    for (i = 0; i < ent->targets.Num(); i++) {
      ...
      if (target) {
        gameRenderWorld->DebugArrow(
            colorYellow * frac, ent->GetPhysics()->GetAbsBounds().GetCenter(),
            target->GetPhysics()->GetOrigin(), 10, 0);
        gameRenderWorld->DebugBounds(colorGreen * frac, box,
...
void idGameLocal::RunDebugInfo(void) {
    ...
    for (ent = spawnedEntities.Next(); ent != NULL;
      ...
      if (viewTextBounds.IntersectsBounds(entBounds)) {
        gameRenderWorld->DrawText(ent->name.c_str(), entBounds.GetCenter(),
                                  0.1f, colorWhite, axis, 1);
        gameRenderWorld->DrawText(va("#%d", ent->entityNumber),
                                  entBounds.GetCenter() + up, 0.1f, colorWhite,
                                  axis, 1);
      }
    }
  }
  ...
  if (g_showActiveEntities.GetBool()) {
    for (ent = activeEntities.Next(); ent != NULL;
      ...
      if (ent->fl.isDormant) {
        ...
        gameRenderWorld->DebugBounds(colorGreen, b,
                                     ent->GetPhysics()->GetOrigin());
      }
    }
  }
  ...
  if (g_showCollisionWorld.GetBool()) {
    collisionModelManager->DrawModel(0, vec3_origin, mat3_identity, origin,
                                     128.0f);
  }

  if (g_showCollisionModels.GetBool()) {
    clip.DrawClipModels(player->GetEyePosition(), g_maxShowDistance.GetFloat(),
                        pm_thirdPerson.GetBool() ? NULL : player);
  }
  ...
  if (g_showPVS.GetInteger()) {
    pvs.DrawPVS(origin, (g_showPVS.GetInteger() == 2) ? PVS_ALL_PORTALS_OPEN
                                                      : PVS_NORMAL);
  }
  ...
  if (aas_test.GetInteger() >= 0) {
    ...
    if (aas) {
      ...
      if (ai_testPredictPath.GetBool()) {
        ...
        velocity.z = 0.0f;
        idAI::PredictPath(player, aas, origin, velocity, 1000, 100,
  ...
  if (ai_showObstacleAvoidance.GetInteger() == 2) {
    ...
    if (aas) {
      ...
      seekPos =
          player->GetPhysics()->GetOrigin() + player->viewAxis[0] * 200.0f;
      idAI::FindPathAroundObstacles(player->GetPhysics(), aas, NULL,
                                    player->GetPhysics()->GetOrigin(), seekPos,
                                    path);
    }
  }
...
void idGameLocal::UnregisterEntity(idEntity* ent) {
  ...
  if ((ent->entityNumber != ENTITYNUM_NONE) &&
    ...
    if (ent->entityNumber >= MAX_CLIENTS &&
        ent->entityNumber < firstFreeIndex) {
...
bool idGameLocal::SpawnEntityDef(const idDict& args,
  ...
  if (spawn) {
    ...
    if (!cls) {
      Warning("Could not spawn '%s'.  Class '%s' not found %s.", classname,
              spawn, error.c_str());
      return false;
    }
    ...
    if (!obj) {
      Warning("Could not spawn '%s'. Instance could not be created %s.",
  ...
  if (spawn) {
    ...
    if (!func) {
      Warning("Could not spawn '%s'.  Script function '%s' not found%s.",
  ...
  Warning("%s doesn't include a spawnfunc or spawnclass%s.", classname,
          error.c_str());
  return false;
}
...
bool idGameLocal::InhibitEntitySpawn(idDict& spawnArgs) {
  ...
  if (g_skill.GetInteger() == 3) {
    ...
    if (idStr::Icmp(name, "item_medkit") == 0 ||
        idStr::Icmp(name, "item_medkit_small") == 0 ||
        idStr::Icmp(name, "moveable_item_medkit") == 0 ||
        idStr::Icmp(name, "moveable_item_medkit_small") == 0) {
      result = true;
    }
  }
#endif
  ...
  if (gameLocal.isMultiplayer) {
    name = spawnArgs.GetString("classname");
    if (idStr::Icmp(name, "weapon_bfg") == 0 ||
        idStr::Icmp(name, "weapon_soulcube") == 0) {
      result = true;
    }
  }
...
void idGameLocal::SpawnMapEntities(void) {
  ...
  if (!SpawnEntityDef(args) || !entities[ENTITYNUM_WORLD] ||
      !entities[ENTITYNUM_WORLD]->IsType(idWorldspawn::Type)) {
...
bool idGameLocal::RemoveEntityFromHash(const char* name, idEntity* ent) {
  ...
  for (i = entityHash.First(hash); i != -1; i = entityHash.Next(i)) {
    if (entities[i] && entities[i] == ent &&
        entities[i]->name.Icmp(name) == 0) {
...
void idGameLocal::KillBox(idEntity* ent, bool catch_teleport) {
  ...
  num = clip.ClipModelsTouchingBounds(phys->GetAbsBounds(), phys->GetClipMask(),
                                      clipModels, MAX_GENTITIES);
  for (i = 0; i < num; i++) {
    cm = clipModels[i];
    ...
    if (hit->IsType(idPlayer::Type) &&
        static_cast<idPlayer*>(hit)->IsInTeleport()) {
      static_cast<idPlayer*>(hit)->TeleportDeath(ent->entityNumber);
    } else if (!catch_teleport) {
      hit->Damage(ent, ent, vec3_origin, "damage_telefrag", 1.0f,
                  INVALID_JOINT);
    }
...
void idGameLocal::RadiusDamage(const idVec3& origin,
  ...
  numListedEntities =
      clip.EntitiesTouchingBounds(bounds, -1, entityList, MAX_GENTITIES);
  ...
  for (e = 0; e < numListedEntities; e++) {
    ...
    if (ent == inflictor ||
        (ent->IsType(idAFAttachment::Type) &&
         static_cast<idAFAttachment*>(ent)->GetBody() == inflictor)) {
      continue;
    }

    if (ent == ignoreDamage ||
        (ent->IsType(idAFAttachment::Type) &&
         static_cast<idAFAttachment*>(ent)->GetBody() == ignoreDamage)) {
      continue;
    }

    // don't damage a dead player
    if (isMultiplayer && ent->entityNumber < MAX_CLIENTS &&
        ent->IsType(idPlayer::Type) &&
        static_cast<idPlayer*>(ent)->health < 0) {
      continue;
    }
    ...
    if (ent->CanDamage(origin, damagePoint)) {
      ...
      if (ent == attacker ||
          (ent->IsType(idAFAttachment::Type) &&
           static_cast<idAFAttachment*>(ent)->GetBody() == attacker)) {
        damageScale *= attackerDamageScale;
      }

      ent->Damage(inflictor, attacker, dir, damageDefName, damageScale,
                  INVALID_JOINT);
    }
  }
  ...
  if (push) {
    RadiusPush(origin, radius, push * dmgPower, attacker, ignorePush,
               attackerPushScale, false);
  }
}
...
void idGameLocal::RadiusPush(const idVec3& origin,
  ...
  numListedClipModels =
      clip.ClipModelsTouchingBounds(bounds, -1, clipModelList, MAX_GENTITIES);
  ...
  for (i = 0; i < numListedClipModels; i++) {
    ...
    if (ent == ignore ||
        (ent->IsType(idAFAttachment::Type) &&
         static_cast<idAFAttachment*>(ent)->GetBody() == ignore)) {
      continue;
    }

    if (gameRenderWorld->FastWorldTrace(result, origin,
                                        clipModel->GetOrigin())) {
      continue;
    }
    ...
    if (quake) {
      clipModel->GetEntity()->ApplyImpulse(world, clipModel->GetId(),
                                           clipModel->GetOrigin(),
                                           scale * push * dir);
    } else {
...
void idGameLocal::RadiusPushClipModel(const idVec3& origin,
  ...
  if (!trm || 1) {
    ...
    impulse.Normalize();
    impulse.z += 1.0f;
    clipModel->GetEntity()->ApplyImpulse(
        world, clipModel->GetId(), clipModel->GetOrigin(), push * impulse);
    return;
  }

  localOrigin =
      (origin - clipModel->GetOrigin()) * clipModel->GetAxis().Transpose();
  for (i = 0; i < trm->numPolys; i++) {
    poly = &trm->polys[i];

    center.Zero();
    for (j = 0; j < poly->numEdges; j++) {
      v = trm->verts[trm->edges[abs(poly->edges[j])]
                         .v[INTSIGNBITSET(poly->edges[j])]];
      center += v;
      v -= localOrigin;
    ...
    clipModel->GetEntity()->ApplyImpulse(world, clipModel->GetId(), center,
                                         impulse);
  }
}
...
void idGameLocal::ProjectDecal(const idVec3& origin,
  ...
  static idVec3 decalWinding[4] = {
      idVec3(1.0f, 1.0f, 0.0f), idVec3(-1.0f, 1.0f, 0.0f),
      idVec3(-1.0f, -1.0f, 0.0f), idVec3(1.0f, -1.0f, 0.0f)};
  ...
  idMath::SinCos16((angle) ? angle : random.RandomFloat() * idMath::TWO_PI, s,
                   c);
  ...
  winding.Clear();
  winding +=
      idVec5(windingOrigin + (axis * decalWinding[0]) * size, idVec2(1, 1));
  winding +=
      idVec5(windingOrigin + (axis * decalWinding[1]) * size, idVec2(0, 1));
  winding +=
      idVec5(windingOrigin + (axis * decalWinding[2]) * size, idVec2(0, 0));
  winding +=
      idVec5(windingOrigin + (axis * decalWinding[3]) * size, idVec2(1, 0));
  gameRenderWorld->ProjectDecalOntoWorld(
      winding, projectionOrigin, parallel, depth * 0.5f,
      declManager->FindMaterial(material), gameLocal.slow.time /* _D3XP */);
}
...
void idGameLocal::BloodSplat(const idVec3& origin,
  ...
  idVec3 verts[] = {
      idVec3(0.0f, +halfSize, +halfSize), idVec3(0.0f, +halfSize, -halfSize),
      idVec3(0.0f, -halfSize, -halfSize), idVec3(0.0f, -halfSize, +halfSize)};
  idTraceModel trm;
  idClipModel mdl;
  trace_t results;
  ...
  mdl.LoadModel(trm);
  clip.Translation(results, origin, origin + dir * 64.0f, &mdl, mat3_identity,
                   CONTENTS_SOLID, NULL);
...
bool idGameLocal::SkipCinematic(void) {
  ...
  if (!skipCinematic) {
    ...
    cinematicMaxSkipTime =
        gameLocal.time + SEC2MS(g_cinematicMaxSkipTime.GetFloat());
  }
...
void idGameLocal::SpreadLocations() {
  ...
  for (ent = spawnedEntities.Next(); ent != NULL; ent = ent->spawnNode.Next()) {
    ...
    if (areaNum >= numAreas) {
      Error(
          "idGameLocal::SpreadLocations: areaNum >= "
          "gameRenderWorld->NumAreas()");
    }
    if (locationEntities[areaNum]) {
      Warning("location entity '%s' overlaps '%s'",
              ent->spawnArgs.GetString("name"),
...
idLocationEntity* idGameLocal::LocationForPoint(const idVec3& point) {
  ...
  if (areaNum >= gameRenderWorld->NumAreas()) {
    Error(
        "idGameLocal::LocationForPoint: areaNum >= "
        "gameRenderWorld->NumAreas()");
  }
...
void idGameLocal::RandomizeInitialSpawns(void) {
  ...
  while (spot.ent) {
    ...
    if (mpGame.IsGametypeFlagBased()) /* CTF */
    {
      if (spot.team == 0 || spot.team == 1)
        ...
        common->Warning(
            "info_player_deathmatch : invalid or no team attached to spawn "
            "point\n");
    }
#endif
  ...
  if (mpGame.IsGametypeFlagBased()) /* CTF */
  {
    ...
    if (!teamInitialSpots[0].Num()) {
      common->Warning(
          "red team : no info_player_deathmatch entities marked initial in "
          "map");
    ...
    if (!teamInitialSpots[1].Num()) {
      common->Warning(
          "blue team : no info_player_deathmatch entities marked initial in "
          "map");
...
idEntity* idGameLocal::SelectInitialSpawnPoint(idPlayer* player) {
  ...
  if (!isMultiplayer || !spawnSpots.Num() ||
      (mpGame.IsGametypeFlagBased() &&
       (!teamSpawnSpots[0].Num() || !teamSpawnSpots[1].Num()))) { /* CTF */
#else
  if (!isMultiplayer || !spawnSpots.Num()) {
#endif
  ...
  if (mpGame.IsGametypeFlagBased()) { /* CTF */
    ...
    useInitialSpots =
        player->useInitialSpawns && teamCurrentInitialSpot[player->team] <
                                        teamInitialSpots[player->team].Num();
  } else {
    useInitialSpots =
        player->useInitialSpawns && currentInitialSpot < initialSpots.Num();
  }
#endif
  ...
  if (player->spectating) {
    ...
    if (mpGame.IsGametypeFlagBased()) { /* CTF */
      ...
      return teamInitialSpots[player->team]
                             [teamCurrentInitialSpot[player->team]++];
    }
    return initialSpots[currentInitialSpot++];
#else
  } else if (player->useInitialSpawns &&
             currentInitialSpot < initialSpots.Num()) {
    return initialSpots[currentInitialSpot++];
#endif
  } else {
    ...
    if (alone) {
      ...
      {
        assert(player->team == 0 || player->team == 1);
        return teamSpawnSpots[player->team]
                             [random.RandomInt(
                                  teamSpawnSpots[player->team].Num())]
                                 .ent;
      }
#endif
    ...
    if (mpGame.IsGametypeFlagBased()) /* CTF */
    {
      ...
      // sort the list
      qsort((void*)teamSpawnSpots[team].Ptr(), teamSpawnSpots[team].Num(),
            sizeof(spawnSpot_t),
            (int (*)(const void*, const void*))sortSpawnPoints);
    ...
    // sort the list
    qsort((void*)spawnSpots.Ptr(), spawnSpots.Num(), sizeof(spawnSpot_t),
          (int (*)(const void*, const void*))sortSpawnPoints);
...
void idGameLocal::UpdateServerInfoFlags() {
  ...
  if ((idStr::Icmp(serverInfo.GetString("si_gameType"), "deathmatch") == 0)) {
    gameType = GAME_DM;
  } else if ((idStr::Icmp(serverInfo.GetString("si_gameType"), "Tourney") ==
              0)) {
    gameType = GAME_TOURNEY;
  } else if ((idStr::Icmp(serverInfo.GetString("si_gameType"), "Team DM") ==
              0)) {
    gameType = GAME_TDM;
  } else if ((idStr::Icmp(serverInfo.GetString("si_gameType"), "Last Man") ==
              0)) {
...
int idGameLocal::GetSpawnId(const idEntity* ent) const {
  return (gameLocal.spawnIds[ent->entityNumber] << GENTITYNUM_BITS) |
         ent->entityNumber;
}
...
===============
*/
void idGameLocal::GetBestGameType(const char* map,
                                  const char* gametype,
                                  char buf[MAX_STRING_CHARS]) {

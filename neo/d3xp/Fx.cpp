...
void idEntityFx::Restore(idRestoreGame* savefile) {
  ...
  for (i = 0; i < num; i++) {
    ...
    if (hasObject) {
      ...
      actions[i].lightDefHandle =
          gameRenderWorld->AddLightDef(&actions[i].renderLight);
    } else {
    ...
    if (hasObject) {
      ...
      actions[i].modelDefHandle =
          gameRenderWorld->AddEntityDef(&actions[i].renderEntity);
    } else {
...
void idEntityFx::Setup(const char* fx) {
  ...
  // early during MP Spawn() with no information. wait till we ReadFromSnapshot
  // for more
  ...
  fxEffect = static_cast<const idDeclFX*>(
      declManager->FindType(DECL_FX, systemName.c_str()));
  ...
  if (fxEffect) {
    ...
    for (int i = 0; i < fxEffect->events.Num(); i++) {
      ...
      if (fxaction.random1 || fxaction.random2) {
        laction.delay =
            fxaction.random1 + gameLocal.random.RandomFloat() *
                                   (fxaction.random2 - fxaction.random1);
      } else {
...
================
*/
void idEntityFx::CleanUpSingleAction(const idFXSingleAction& fxaction,
                                     idFXLocalAction& laction) {
  if (laction.lightDefHandle != -1 && fxaction.sibling == -1 &&
      fxaction.type != FX_ATTACHLIGHT) {
    gameRenderWorld->FreeLightDef(laction.lightDefHandle);
    laction.lightDefHandle = -1;
  }
  if (laction.modelDefHandle != -1 && fxaction.sibling == -1 &&
...
================
*/
void idEntityFx::ApplyFade(const idFXSingleAction& fxaction,
                           idFXLocalAction& laction,
                           const int time,
                           const int actualStart) {
  if (fxaction.fadeInTime || fxaction.fadeOutTime) {
    float fadePct =
        (float)(time - actualStart) /
        (1000.0f * ((fxaction.fadeInTime != 0) ? fxaction.fadeInTime
                                               : fxaction.fadeOutTime));
    if (fadePct > 1.0) {
      fadePct = 1.0;
    }
    if (laction.modelDefHandle != -1) {
      laction.renderEntity.shaderParms[SHADERPARM_RED] =
          (fxaction.fadeInTime) ? fadePct : 1.0f - fadePct;
      laction.renderEntity.shaderParms[SHADERPARM_GREEN] =
          (fxaction.fadeInTime) ? fadePct : 1.0f - fadePct;
      laction.renderEntity.shaderParms[SHADERPARM_BLUE] =
          (fxaction.fadeInTime) ? fadePct : 1.0f - fadePct;

      gameRenderWorld->UpdateEntityDef(laction.modelDefHandle,
                                       &laction.renderEntity);
    }
    if (laction.lightDefHandle != -1) {
      laction.renderLight.shaderParms[SHADERPARM_RED] =
          fxaction.lightColor.x *
          ((fxaction.fadeInTime) ? fadePct : 1.0f - fadePct);
      laction.renderLight.shaderParms[SHADERPARM_GREEN] =
          fxaction.lightColor.y *
          ((fxaction.fadeInTime) ? fadePct : 1.0f - fadePct);
      laction.renderLight.shaderParms[SHADERPARM_BLUE] =
          fxaction.lightColor.z *
          ((fxaction.fadeInTime) ? fadePct : 1.0f - fadePct);

      gameRenderWorld->UpdateLightDef(laction.lightDefHandle,
                                      &laction.renderLight);
    }
  }
}
...
void idEntityFx::Run(int time) {
  ...
  for (ieff = 0; ieff < fxEffect->events.Num(); ieff++) {
    ...
    //
    if (laction.delay) {
      if (laction.start + (time - laction.start) <
          laction.start + (laction.delay * 1000)) {
        continue;
      }
    }
    ...
    //
    int actualStart = laction.delay
                          ? laction.start + (int)(laction.delay * 1000)
                          : laction.start;
    ...
    switch (fxaction.type) {
        ...
        if (useAction->lightDefHandle == -1) {
          if (fxaction.type == FX_LIGHT) {
            ...
            useAction->renderLight.origin =
                GetPhysics()->GetOrigin() + fxaction.offset;
            ...
            useAction->renderLight.shader =
                declManager->FindMaterial(fxaction.data, false);
            useAction->renderLight.shaderParms[SHADERPARM_RED] =
                fxaction.lightColor.x;
            useAction->renderLight.shaderParms[SHADERPARM_GREEN] =
                fxaction.lightColor.y;
            useAction->renderLight.shaderParms[SHADERPARM_BLUE] =
                fxaction.lightColor.z;
            useAction->renderLight.shaderParms[SHADERPARM_TIMESCALE] = 1.0f;
            useAction->renderLight.shaderParms[SHADERPARM_TIMEOFFSET] =
                -MS2SEC(time);
            ...
            useAction->lightDefHandle =
                gameRenderWorld->AddLightDef(&useAction->renderLight);
          }
          if (fxaction.noshadows) {
            for (j = 0; j < fxEffect->events.Num(); j++) {
        ...
        if (!useAction->soundStarted) {
          ...
          for (j = 0; j < fxEffect->events.Num(); j++) {
            ...
            if (laction2.lightDefHandle != -1) {
              ...
              gameRenderWorld->UpdateLightDef(laction2.lightDefHandle,
      ...
      case FX_DECAL: {
        if (!useAction->decalDropped) {
          ...
          gameLocal.ProjectDecal(GetPhysics()->GetOrigin(),
                                 GetPhysics()->GetGravity(), 8.0f, true,
                                 fxaction.size, fxaction.data);
        }
        break;
      }
      case FX_SHAKE: {
        if (!useAction->shakeStarted) {
          ...
          for (j = 0; j < gameLocal.numClients; j++) {
            ...
            if (player &&
                (player->GetPhysics()->GetOrigin() - GetPhysics()->GetOrigin())
                        .LengthSqr() < Square(fxaction.shakeDistance)) {
              if (!gameLocal.isMultiplayer || !fxaction.shakeIgnoreMaster ||
                  GetBindMaster() != player) {
          ...
          if (fxaction.shakeImpulse != 0.0f && fxaction.shakeDistance != 0.0f) {
            ...
            gameLocal.RadiusPush(GetPhysics()->GetOrigin(),
                                 fxaction.shakeDistance, fxaction.shakeImpulse,
                                 this, ignore_ent, 1.0f, true);
          }
      ...
      case FX_MODEL: {
        if (useAction->modelDefHandle == -1) {
          ...
          useAction->renderEntity.origin =
              GetPhysics()->GetOrigin() + fxaction.offset;
          useAction->renderEntity.axis =
              (fxaction.explicitAxis) ? fxaction.axis : GetPhysics()->GetAxis();
          ...
          useAction->renderEntity.shaderParms[SHADERPARM_TIMEOFFSET] =
              -MS2SEC(time);
          useAction->renderEntity.shaderParms[3] = 1.0f;
          useAction->renderEntity.shaderParms[5] = 0.0f;
          if (useAction->renderEntity.hModel) {
            useAction->renderEntity.bounds =
                useAction->renderEntity.hModel->Bounds(
                    &useAction->renderEntity);
          }
          useAction->modelDefHandle =
              gameRenderWorld->AddEntityDef(&useAction->renderEntity);
          ...
          useAction->renderEntity.axis =
              fxaction.explicitAxis ? fxaction.axis : GetPhysics()->GetAxis();
#ifdef _D3XP
          gameRenderWorld->UpdateEntityDef(useAction->modelDefHandle,
                                           &useAction->renderEntity);
#endif
        }
        ApplyFade(fxaction, *useAction, time, actualStart);
      ...
      case FX_LAUNCH: {
        ...
        if (!useAction->launched) {
          ...
          if (!projectileDef) {
            gameLocal.Warning("projectile \'%s\' not found",
                              fxaction.data.c_str());
          } else {
            gameLocal.SpawnEntityDef(*projectileDef, &ent, false);
            if (ent && ent->IsType(idProjectile::Type)) {
              projectile = (idProjectile*)ent;
              projectile->Create(this, GetPhysics()->GetOrigin(),
                                 GetPhysics()->GetAxis()[0]);
              projectile->Launch(GetPhysics()->GetOrigin(),
                                 GetPhysics()->GetAxis()[0], vec3_origin);
            }
          }
        }
        break;
      ...
      case FX_SHOCKWAVE: {
        ...
        if (!useAction->shakeStarted) {
          ...
          if (!projectileDef) {
            gameLocal.Warning("shockwave \'%s\' not found",
                              shockDefName.c_str());
          } else {
...
================
*/
idEntityFx* idEntityFx::StartFx(const char* fx,
                                const idVec3* useOrigin,
                                const idMat3* useAxis,
                                idEntity* ent,
                                bool bind) {
  ...
  args.Set("fx", fx);
  idEntityFx* nfx = static_cast<idEntityFx*>(
      gameLocal.SpawnEntityType(idEntityFx::Type, &args));
...
void idEntityFx::WriteToSnapshot(idBitMsgDelta& msg) const {
  ...
  msg.WriteLong((fxEffect != NULL)
                    ? gameLocal.ServerRemapDecl(-1, DECL_FX, fxEffect->Index())
                    : -1);
...
void idEntityFx::ReadFromSnapshot(const idBitMsgDelta& msg) {
  ...
  if (fx_index != -1 && start_time > 0 && !fxEffect && started < 0) {
    ...
    const idDeclFX* fx = static_cast<const idDeclFX*>(
        declManager->DeclByIndex(DECL_FX, fx_index));
    if (!fx) {

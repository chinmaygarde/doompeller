...
bool idModelExport::CheckMayaInstall(void) {
  ...
  lres = RegOpenKey(HKEY_LOCAL_MACHINE,
                    "SOFTWARE\\Alias|Wavefront\\Maya\\4.5\\Setup\\InstallPath",
                    &hKey);
  ...
  lres = RegQueryValueEx(hKey, "MAYA_INSTALL_LOCATION", NULL,
                         (unsigned long*)&lType, (unsigned char*)NULL,
                         (unsigned long*)NULL);
  ...
  lres =
      RegOpenKey(HKEY_LOCAL_MACHINE, "SOFTWARE\\Alias|Wavefront\\Maya", &hKey);
  RegCloseKey(hKey);
...
void idModelExport::LoadMayaDll(void) {
  ...
  Maya_ConvertModel = (exporterInterface_t)sys->DLL_GetProcAddress(
      importDLL, "Maya_ConvertModel");
  Maya_Shutdown =
      (exporterShutdown_t)sys->DLL_GetProcAddress(importDLL, "Maya_Shutdown");
  ...
  if (!force && (fileSystem->ReadFile(dest, NULL, &destTime) >= 0)) {
    ...
    if (parser.CheckTokenString(MD5_VERSION_STRING)) {
      ...
      if (parser.CheckTokenString("commandline")) {
        ...
        if ((destTime >= sourceTime) && (version == MD5_VERSION) &&
            (cmdLine == commandLine)) {
          // don't convert it
          return true;
        }
...
bool idModelExport::ExportModel(const char* model) {
  ...
  sprintf(commandLine, "mesh %s -dest %s -game %s", src.c_str(), dest.c_str(),
          game);
  if (!ConvertMayaToMD5()) {
    gameLocal.Printf("Failed to export '%s' : %s", src.c_str(),
                     Maya_Error.c_str());
    return false;
  }
...
bool idModelExport::ExportAnim(const char* anim) {
  ...
  sprintf(commandLine, "anim %s -dest %s -game %s", src.c_str(), dest.c_str(),
          game);
  if (!ConvertMayaToMD5()) {
    gameLocal.Printf("Failed to export '%s' : %s", src.c_str(),
                     Maya_Error.c_str());
...
int idModelExport::ParseExportSection(idParser& parser) {
  ...
  lex.SetFlags(LEXFL_NOSTRINGCONCAT | LEXFL_ALLOWPATHNAMES |
  ...
  while (1) {
    ...
    if (command == "options") {
    ...
    } else if ((command == "mesh") || (command == "anim") ||
               (command == "camera")) {
      ...
      if (ParseOptions(lex)) {
        ...
        sprintf(commandLine, "%s %s -dest %s -game %s%s", command.c_str(),
                src.c_str(), dest.c_str(), game, commandLine.c_str());
        if (ConvertMayaToMD5()) {
          count++;
        } else {
          parser.Warning("Failed to export '%s' : %s", src.c_str(),
                         Maya_Error.c_str());
        }
      }
      lex.FreeSource();
...
int idModelExport::ExportDefFile(const char* filename) {
  idParser parser(LEXFL_NOSTRINGCONCAT | LEXFL_ALLOWPATHNAMES |

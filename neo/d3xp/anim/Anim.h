...
typedef enum {
  ...
  JOINTMOD_LOCAL,  // modifies the joint's position or orientation in joint
                   // local space
...
class idMD5Anim {
 ...
 public:
  ...
  void GetSingleFrame(int framenum,
                      idJointQuat* joints,
                      const int* index,
                      int numIndexes) const;
...
class idAnim {
 ...
 public:
  ...
  void SetAnim(const idDeclModelDef* modelDef,
  ...
  bool GetOriginRotation(idQuat& rotation,
                         int animNum,
                         int currentTime,
                         int cyclecount) const;
  bool GetBounds(idBounds& bounds, int animNum, int time, int cyclecount) const;
  const char* AddFrameCommand(const class idDeclModelDef* modelDef,
                              int framenum,
                              idLexer& src,
                              const idDict* def);
  ...
  // found.
  int FindFrameForFrameCommand(frameCommandType_t framecommand,
...
class idDeclModelDef : public idDecl {
 public:
  ...
  void SetupJoints(int* numJoints,
...
class idAnimBlend {
 private:
  ...
  void SetFrame(const idDeclModelDef* modelDef,
                int animnum,
                int frame,
                int currenttime,
                int blendtime);
  void CycleAnim(const idDeclModelDef* modelDef,
                 int animnum,
                 int currenttime,
                 int blendtime);
  void PlayAnim(const idDeclModelDef* modelDef,
                int animnum,
                int currenttime,
                int blendtime);
  bool BlendAnim(int currentTime,
                 int channel,
                 int numJoints,
  ...
  void BlendOrigin(int currentTime,
                   idVec3& blendPos,
                   float& blendWeight,
                   bool removeOriginOffset) const;
  void BlendDelta(int fromtime,
                  int totime,
                  idVec3& blendDelta,
                  float& blendWeight) const;
  void BlendDeltaRotation(int fromtime,
                          int totime,
                          idQuat& blendDelta,
                          float& blendWeight) const;
  bool AddBounds(int currentTime,
                 idBounds& bounds,
                 bool removeOriginOffset) const;
...
class idAnimator {
 public:
  ...
  // twice.
  void SyncAnimChannels(int channelNum,
                        int fromChannelNum,
                        int currenttime,
                        int blendTime);

  void SetJointPos(jointHandle_t jointnum,
                   jointModTransform_t transform_type,
                   const idVec3& pos);
  void SetJointAxis(jointHandle_t jointnum,
                    jointModTransform_t transform_type,
                    const idMat3& mat);
  ...
  void SetAFPoseJointMod(const jointHandle_t jointNum,
  ...
  bool GetJointTransform(jointHandle_t jointHandle,
                         int currenttime,
                         idVec3& offset,
                         idMat3& axis);
  bool GetJointLocalTransform(jointHandle_t jointHandle,
                              int currentTime,
                              idVec3& offset,
                              idMat3& axis);
 ...
 private:
  ...
  mutable int
      lastTransformTime;  // mutable because the value is updated in CreateFrame

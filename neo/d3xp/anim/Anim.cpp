...
size_t idMD5Anim::Allocated(void) const {
  size_t size = bounds.Allocated() + jointInfo.Allocated() +
                componentFrames.Allocated() + name.Allocated();
  return size;
}
...
bool idMD5Anim::LoadAnim(const char* filename) {
  ...
  if ((numAnimatedComponents < 0) || (numAnimatedComponents > numJoints * 6)) {
    parser.Error("Invalid number of animated components: %d",
...
void idMD5Anim::GetOrigin(idVec3& offset, int time, int cyclecount) const {
  ...
  const float* componentPtr1 =
      &componentFrames[numAnimatedComponents * frame.frame1 +
                       jointInfo[0].firstComponent];
  const float* componentPtr2 =
      &componentFrames[numAnimatedComponents * frame.frame2 +
                       jointInfo[0].firstComponent];

  if (jointInfo[0].animBits & ANIM_TX) {
    offset.x =
        *componentPtr1 * frame.frontlerp + *componentPtr2 * frame.backlerp;
    componentPtr1++;
    componentPtr2++;
  }
...
void idMD5Anim::GetInterpolatedFrame(frameBlend_t& frame,
  ...
  // copy the baseframe
  SIMDProcessor->Memcpy(joints, baseFrame.Ptr(),
  ...
  SIMDProcessor->BlendJoints(joints, blendJoints, frame.backlerp, lerpIndex,
                             numLerpJoints);
...
void idMD5Anim::CheckModelHierarchy(const idRenderModel* model) const {
  ...
  if (jointInfo.Num() != model->NumJoints()) {
    gameLocal.Error("Model '%s' has different # of joints than anim '%s'",
  ...
  for (i = 0; i < jointInfo.Num(); i++) {
    ...
    if (modelJoints[i].name != animationLib.JointName(jointNum)) {
      gameLocal.Error("Model '%s''s joint names don't match anim '%s''s",
                      model->Name(), name.c_str());
    }
    ...
    if (parent != jointInfo[i].parentNum) {
      gameLocal.Error("Model '%s' has different joint hierarchy than anim '%s'",
                      model->Name(), name.c_str());
    }
  }
}

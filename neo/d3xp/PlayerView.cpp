...
void idPlayerView::DamageImpulse(idVec3 localKickDir, const idDict* damageDef) {
  ...
  if (lastDamageTime > 0.0f &&
      SEC2MS(lastDamageTime) + IMPULSE_DELAY > gameLocal.slow.time) {
  ...
  if (blobTime) {
    ...
    blob->finishTime =
        gameLocal.slow.time + blobTime * g_blobTime.GetFloat() *
                                  ((float)gameLocal.msec / USERCMD_MSEC);
    ...
    blob->h =
        damageDef->GetFloat("blob_height") * g_blobSize.GetFloat() * scale;
    blob->s1 = 0;
    blob->t1 = 0;
    blob->s2 = 1;
    blob->t2 = 1;
...
void idPlayerView::CalculateShake() {
  ...
  float shakeVolume = gameSoundWorld->CurrentShakeAmplitudeForPosition(
      gameLocal.slow.time, player->firstPersonViewOrigin);
  //
...
void idPlayerView::SingleView(idUserInterface* hud, const renderView_t* view) {
  ...
  gameSoundWorld->PlaceListener(
      view->vieworg, view->viewaxis, player->entityNumber + 1,
      gameLocal.slow.time,
...
#ifdef _D3XP
  if (gameLocal.portalSkyEnt.GetEntity() && gameLocal.IsPortalSkyAcive() &&
      g_enablePortalSky.GetBool()) {
    ...
    hackedView.forceUpdate =
        true;  // FIX: for smoke particles not drawing when portalSky present
  }
  ...
  if (!pm_thirdPerson.GetBool() && !g_skipViewEffects.GetBool()) {
    for (int i = 0; i < MAX_SCREEN_BLOBS; i++) {
      ...
      float fade = (float)(blob->finishTime - gameLocal.slow.time) /
      ...
      if (fade) {
        ...
        renderSystem->DrawStretchPic(blob->x, blob->y, blob->w, blob->h,
                                     blob->s1, blob->t1, blob->s2, blob->t2,
                                     blob->material);
      }
    }
    player->DrawHUD(hud);
    ...
    if (alpha < 1.0f) {
      renderSystem->SetColor4(
          (player->health <= 0.0f) ? MS2SEC(gameLocal.slow.time)
                                   : lastDamageTime,
          1.0f, 1.0f, (player->health <= 0.0f) ? 0.0f : alpha);
      renderSystem->DrawStretchPic(0.0f, 0.0f, 640.0f, 480.0f, 0.0f, 0.0f, 1.0f,
                                   1.0f, tunnelMaterial);
    }
...
void idPlayerView::RenderPlayerView(idUserInterface* hud) {
  ...
  if (net_clientLagOMeter.GetBool() && lagoMaterial && gameLocal.isClient) {
    renderSystem->SetColor4(1.0f, 1.0f, 1.0f, 1.0f);
    renderSystem->DrawStretchPic(10.0f, 380.0f, 64.0f, 64.0f, 0.0f, 0.0f, 1.0f,
                                 1.0f, lagoMaterial);
  }
}
...
===================
*/
int idPlayerView::AddWarp(idVec3 worldOrigin,
                          float centerx,
                          float centery,
                          float initialRadius,
                          float durationMsec) {
...
void FullscreenFX_Helltime::Initialize() {
  acInitMaterials[0] =
      declManager->FindMaterial("textures/smf/bloodorb1/ac_init");
  acInitMaterials[1] =
      declManager->FindMaterial("textures/smf/bloodorb2/ac_init");
  acInitMaterials[2] =
      declManager->FindMaterial("textures/smf/bloodorb3/ac_init");

  acCaptureMaterials[0] =
      declManager->FindMaterial("textures/smf/bloodorb1/ac_capture");
  acCaptureMaterials[1] =
      declManager->FindMaterial("textures/smf/bloodorb2/ac_capture");
  acCaptureMaterials[2] =
      declManager->FindMaterial("textures/smf/bloodorb3/ac_capture");

  acDrawMaterials[0] =
      declManager->FindMaterial("textures/smf/bloodorb1/ac_draw");
  acDrawMaterials[1] =
      declManager->FindMaterial("textures/smf/bloodorb2/ac_draw");
  acDrawMaterials[2] =
      declManager->FindMaterial("textures/smf/bloodorb3/ac_draw");

  crCaptureMaterials[0] =
      declManager->FindMaterial("textures/smf/bloodorb1/cr_capture");
  crCaptureMaterials[1] =
      declManager->FindMaterial("textures/smf/bloodorb2/cr_capture");
  crCaptureMaterials[2] =
      declManager->FindMaterial("textures/smf/bloodorb3/cr_capture");

  crDrawMaterials[0] =
      declManager->FindMaterial("textures/smf/bloodorb1/cr_draw");
  crDrawMaterials[1] =
      declManager->FindMaterial("textures/smf/bloodorb2/cr_draw");
  crDrawMaterials[2] =
      declManager->FindMaterial("textures/smf/bloodorb3/cr_draw");
...
void FullscreenFX_Multiplayer::Initialize() {
  acInitMaterials =
      declManager->FindMaterial("textures/smf/multiplayer1/ac_init");
  acCaptureMaterials =
      declManager->FindMaterial("textures/smf/multiplayer1/ac_capture");
  acDrawMaterials =
      declManager->FindMaterial("textures/smf/multiplayer1/ac_draw");
  crCaptureMaterials =
      declManager->FindMaterial("textures/smf/multiplayer1/cr_capture");
  crDrawMaterials =
      declManager->FindMaterial("textures/smf/multiplayer1/cr_draw");
  clearAccumBuffer = true;
}
...
void FullscreenFX_Multiplayer::HighQuality() {
  ...
  renderSystem->DrawStretchPic(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, 0, 1, 1, 0,
                               acDrawMaterials);
  renderSystem->DrawStretchPic(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, 0,
                               shiftScale.y, shiftScale.x, 0, crDrawMaterials);
}
...
void FullscreenFX_Warp::DrawWarp(WarpPolygon_t wp, float interp) {
  ...
  renderSystem->DrawStretchTri(drawPts[0], drawPts[1], drawPts[2], drawPts[3],
                               drawPts[4], drawPts[5], material);
...
void FullscreenFX_Warp::HighQuality() {
  ...
  interp =
      (idMath::Sin((float)(gameLocal.slow.time - startWarpTime) / 1000) + 1) /
      2.f;
...
void FullscreenFX_DoubleVision::HighQuality() {
  ...
  if (!gameLocal.isMultiplayer &&
          gameLocal.fast.time < player->inventory.powerupEndTime[HELLTIME] ||
      gameLocal.fast.time < player->inventory.powerupEndTime[INVULNERABILITY]) {
    color.y = 0;
    color.z = 0;
  }

  renderSystem->SetColor4(color.x, color.y, color.z, 1.0f);
  renderSystem->DrawStretchPic(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, shift,
                               shiftScale.y, shiftScale.x, 0, material);
  renderSystem->SetColor4(color.x, color.y, color.z, 0.5f);
  renderSystem->DrawStretchPic(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, 0,
                               shiftScale.y, (1 - shift) * shiftScale.x, 0,
                               material);
}
...
void FullscreenFX_InfluenceVision::HighQuality() {
  ...
  if (player->GetInfluenceMaterial()) {
    ...
    renderSystem->DrawStretchPic(0.0f, 0.0f, 640.0f, 480.0f, 0.0f, 0.0f, 1.0f,
                                 1.0f, player->GetInfluenceMaterial());
...
void FullscreenFX_Bloom::Initialize() {
  ...
  currentMaterial =
      declManager->FindMaterial("textures/smf/bloom2/currentMaterial");
...
void FullscreenFX_Bloom::HighQuality() {
  ...
  for (int i = 0; i < num; i++) {
    ...
    renderSystem->DrawStretchPic(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, s1, t2, s2,
                                 t1, drawMaterial);
...
void FullscreenFXManager::Blendback(float alpha) {
  ...
  if (alpha < 1.f) {
    ...
    renderSystem->DrawStretchPic(0.0f, 0.0f, 640.0f, 480.0f, 0.0f, shiftScale.y,
                                 shiftScale.x, 0.f, blendBackMaterial);
  }
}
...
void FullscreenFXManager::Process(const renderView_t* view) {
  ...
  if (!highQualityMode) {
    ...
    renderSystem->SetColor4(1, 1, 1, 1);
    renderSystem->DrawStretchPic(0.0f, 0.0f, 640.0f, 480.0f, 0.0f, 1, 1, 0.f,
                                 blendBackMaterial);
  }
}

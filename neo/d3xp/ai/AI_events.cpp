...
const idEventDef
    AI_ClosestReachableEnemyOfEntity("closestReachableEnemyOfEntity", "E", 'e');
...
const idEventDef AI_TestAnimMoveTowardEnemy("testAnimMoveTowardEnemy",
                                            "s",
                                            'd');
...
const idEventDef AI_TravelDistanceBetweenPoints("travelDistanceBetweenPoints",
                                                "vv",
                                                'f');
const idEventDef AI_TravelDistanceBetweenEntities(
...
const idEventDef AI_GetReachableEntityPosition("getReachableEntityPosition",
                                               "e",
                                               'v');
#ifdef _D3XP
...
EVENT(AI_ClosestReachableEnemyOfEntity,
...
EVENT(AI_TravelDistanceBetweenEntities,
...
void idAI::Event_Touch(idEntity* other, trace_t* trace) {
  if (!enemy.GetEntity() && !other->fl.notarget &&
      (ReactionTo(other) & ATTACK_ON_ACTIVATE)) {
    Activate(other);
  }
  AI_PUSHED = true;
}
...
void idAI::Event_FindEnemyAI(int useFOV) {
  ...
  bestEnemy = NULL;
  for (ent = gameLocal.activeEntities.Next(); ent != NULL;
    ...
    if (!gameLocal.pvs.InCurrentPVS(pvs, actor->GetPVSAreas(),
...
void idAI::Event_FindEnemyInCombatNodes(void) {
  ...
  for (i = 0; i < gameLocal.numClients; i++) {
    ...
    for (j = 0; j < targets.Num(); j++) {
      ...
      if (!node->IsDisabled() &&
          node->EntityInView(actor, actor->GetPhysics()->GetOrigin())) {
        idThread::ReturnEntity(actor);
        return;
      }
    }
...
void idAI::Event_ClosestReachableEnemyOfEntity(idEntity* team_mate) {
  ...
  if (!team_mate->IsType(idActor::Type)) {
    gameLocal.Error("Entity '%s' is not an AI character or player",
                    team_mate->GetName());
  }
  ...
  bestEnt = NULL;
  for (ent = actor->enemyList.Next(); ent != NULL;
...
void idAI::Event_HeardSound(int ignore_team) {
  ...
  if (actor && (!ignore_team || (ReactionTo(actor) & ATTACK_ON_SIGHT)) &&
...
void idAI::Event_SetEnemy(idEntity* ent) {
  if (!ent) {
    ...
    gameLocal.Error(
        "'%s' is not an idActor (player or ai controlled character)",
        ent->name.c_str());
  } else {
...
void idAI::Event_FireMissileAtTarget(const char* jointname,
  ...
  if (!aent) {
    gameLocal.Warning("Entity '%s' not found for 'fireMissileAtTarget'",
                      targetname);
  }
...
void idAI::Event_LaunchMissile(const idVec3& org, const idAngles& ang) {
  ...
  if (((ownerBounds[1][0] - ownerBounds[0][0]) >
    ...
    if ((ownerBounds - projBounds)
            .RayIntersection(org, viewAxis[0], distance)) {
  ...
  gameLocal.clip.Translation(tr, start, org, projClip, projClip->GetAxis(),
...
void idAI::Event_LaunchProjectile(const char* entityDefName) {
  ...
  gameLocal.clip.Translation(tr, start, muzzle, projClip, projClip->GetAxis(),
                             MASK_SHOT_RENDERMODEL, this);
  muzzle = tr.endpos;
...
void idAI::Event_MeleeAttackToJoint(const char* jointname,
  ...
  end = physicsObj.GetOrigin() +
        (end + modelOffset) * viewAxis * physicsObj.GetGravityAxis();
  ...
  gameLocal.clip.TranslationEntities(trace, start, end, NULL, mat3_identity,
                                     MASK_SHOT_BOUNDINGBOX, this);
...
void idAI::Event_CanBecomeSolid(void) {
  ...
  num = gameLocal.clip.ClipModelsTouchingBounds(
      physicsObj.GetAbsBounds(), MASK_MONSTERSOLID, clipModels, MAX_GENTITIES);
  for (i = 0; i < num; i++) {
    cm = clipModels[i];
...
#ifdef _D3XP
    if (spawnClearMoveables && hit->IsType(idMoveable::Type) ||
        hit->IsType(idBarrel::Type) || hit->IsType(idExplodingBarrel::Type)) {
      idVec3 push;
...
void idAI::Event_GetCombatNode(void) {
  ...
  if (!enemyEnt || !EnemyPositionValid()) {
    ...
    if (team == 0) {
      ...
      const idVec3& playerPos =
          gameLocal.GetLocalPlayer()->GetPhysics()->GetOrigin();
...
void idAI::Event_GetJumpVelocity(const idVec3& pos,
  ...
  result = PredictTrajectory(start, end, speed, physicsObj.GetGravity(),
                             physicsObj.GetClipModel(), MASK_MONSTERSOLID,
                             max_height, this, enemyEnt,
                             ai_debugMove.GetBool() ? 4000 : 0, dir);
  if (result) {
...
void idAI::Event_EnemyRange(void) {
  ...
  if (enemyEnt) {
    dist = (enemyEnt->GetPhysics()->GetOrigin() - GetPhysics()->GetOrigin())
               .Length();
  } else {
...
void idAI::Event_EnemyRange2D(void) {
  ...
  if (enemyEnt) {
    dist = (enemyEnt->GetPhysics()->GetOrigin().ToVec2() -
            GetPhysics()->GetOrigin().ToVec2())
               .Length();
  } else {
    // Just some really high number
    dist = idMath::INFINITY;
...
void idAI::Event_PredictEnemyPos(float time) {
  ...
  idAI::PredictPath(
      enemyEnt, aas, lastVisibleEnemyPos,
      enemyEnt->GetPhysics()->GetLinearVelocity(), SEC2MS(time), SEC2MS(time),
      (move.moveType == MOVETYPE_FLY) ? SE_BLOCKED
                                      : (SE_BLOCKED | SE_ENTER_LEDGE_AREA),
      path);
...
void idAI::Event_CanHitEnemyFromAnim(const char* animname) {
  ...
  if (enemyEnt->GetPhysics()->GetAbsBounds().IntersectsBounds(
          physicsObj.GetAbsBounds().Expand(16.0f))) {
    Event_CanHitEnemy();
    return;
  }
  ...
  gameLocal.clip.Translation(tr, start, fromPos, projectileClipModel,
...
void idAI::Event_CanHitEnemyFromJoint(const char* jointname) {
  ...
  muzzle =
      org + (muzzle + modelOffset) * viewAxis * physicsObj.GetGravityAxis();
  ...
  gameLocal.clip.Translation(tr, start, muzzle, projectileClipModel,
                             mat3_identity, MASK_SHOT_BOUNDINGBOX, this);
  muzzle = tr.endpos;

  gameLocal.clip.Translation(tr, muzzle, toPos, projectileClipModel,
                             mat3_identity, MASK_SHOT_BOUNDINGBOX, this);
...
void idAI::Event_TestChargeAttack(void) {
  ...
  idAI::PredictPath(
      this, aas, physicsObj.GetOrigin(), end - physicsObj.GetOrigin(), 1000,
      1000,
  ...
  if (ai_debugMove.GetBool()) {
    gameRenderWorld->DebugLine(colorGreen, physicsObj.GetOrigin(), end,
                               gameLocal.msec);
    gameRenderWorld->DebugBounds(path.endEvent == 0 ? colorYellow : colorRed,
                                 physicsObj.GetBounds(), end, gameLocal.msec);
  }
...
void idAI::Event_TestAnimMoveTowardEnemy(const char* animname) {
  ...
  anim = GetAnim(ANIMCHANNEL_LEGS, animname);
  if (!anim) {
    gameLocal.DWarning("missing '%s' animation on '%s' (%s)", animname,
                       name.c_str(), GetEntityDefName());
    idThread::ReturnInt(false);
    return;
  }
  ...
  moveVec = animator.TotalMovementDelta(anim) *
            idAngles(0.0f, yaw, 0.0f).ToMat3() * physicsObj.GetGravityAxis();
  idAI::PredictPath(
      this, aas, physicsObj.GetOrigin(), moveVec, 1000, 1000,
      (move.moveType == MOVETYPE_FLY)
          ? SE_BLOCKED
          : (SE_ENTER_OBSTACLE | SE_BLOCKED | SE_ENTER_LEDGE_AREA),
      path);
  ...
  if (ai_debugMove.GetBool()) {
    gameRenderWorld->DebugLine(colorGreen, physicsObj.GetOrigin(),
                               physicsObj.GetOrigin() + moveVec,
                               gameLocal.msec);
    gameRenderWorld->DebugBounds(
        path.endEvent == 0 ? colorYellow : colorRed, physicsObj.GetBounds(),
        physicsObj.GetOrigin() + moveVec, gameLocal.msec);
  }
...
void idAI::Event_TestAnimMove(const char* animname) {
  ...
  moveVec = animator.TotalMovementDelta(anim) *
            idAngles(0.0f, ideal_yaw, 0.0f).ToMat3() *
...
void idAI::Event_TestMoveToPosition(const idVec3& position) {
  ...
  idAI::PredictPath(
      this, aas, physicsObj.GetOrigin(), position - physicsObj.GetOrigin(),
      1000, 1000,
      (move.moveType == MOVETYPE_FLY)
          ? SE_BLOCKED
          : (SE_ENTER_OBSTACLE | SE_BLOCKED | SE_ENTER_LEDGE_AREA),
  ...
  if (ai_debugMove.GetBool()) {
    gameRenderWorld->DebugLine(colorGreen, physicsObj.GetOrigin(), position,
                               gameLocal.msec);
    gameRenderWorld->DebugBounds(colorYellow, physicsObj.GetBounds(), position,
                                 gameLocal.msec);
    if (path.endEvent) {
      gameRenderWorld->DebugBounds(colorRed, physicsObj.GetBounds(),
                                   path.endPos, gameLocal.msec);
    }
  }
...
void idAI::Event_TestAnimAttack(const char* animname) {
  ...
  idAI::PredictPath(
      this, aas, physicsObj.GetOrigin(), animator.TotalMovementDelta(anim),
      1000, 1000,
      (move.moveType == MOVETYPE_FLY)
          ? SE_BLOCKED
          : (SE_ENTER_OBSTACLE | SE_BLOCKED | SE_ENTER_LEDGE_AREA),
  ...
  idThread::ReturnInt(path.blockingEntity &&
                      (path.blockingEntity == enemy.GetEntity()));
}
...
void idAI::Event_SetSmokeVisibility(int num, int on) {
  ...
  if (num >= particles.Num()) {
    gameLocal.Warning("Particle #%d out of range (%d particles) on entity '%s'",
                      num, particles.Num(), name.c_str());
    return;
  }
...
void idAI::Event_GetClosestHiddenTarget(const char* type) {
  ...
  if (targets.Num() == 1) {
    ...
    if (ent && idStr::Cmp(ent->GetEntityDefName(), type) == 0) {
      if (!EntityCanSeePos(enemyEnt, lastVisibleEnemyPos,
...
void idAI::Event_TravelDistanceBetweenEntities(idEntity* source,
  ...
  assert(source);
  assert(dest);
  time = TravelDistance(source->GetPhysics()->GetOrigin(),
                        dest->GetPhysics()->GetOrigin());
  idThread::ReturnFloat(time);
}
...
void idAI::Event_AnimTurn(float angles) {
  ...
  if (angles) {
    ...
    anim_turn_amount =
        idMath::Fabs(idMath::AngleNormalize180(current_yaw - ideal_yaw));
...
void idAI::Event_FindActorsInBounds(const idVec3& mins, const idVec3& maxs) {
  ...
  numListedEntities = gameLocal.clip.EntitiesTouchingBounds(
      idBounds(mins, maxs), CONTENTS_BODY, entityList, MAX_GENTITIES);
...
void idAI::Event_CanReachPosition(const idVec3& pos) {
  ...
  if (!toAreaNum ||
      !PathToGoal(path, areaNum, physicsObj.GetOrigin(), toAreaNum, pos)) {
    idThread::ReturnInt(false);
  } else {
    idThread::ReturnInt(true);
  }
...
void idAI::Event_StartEmitter(const char* name,
                              const char* joint,
                              const char* particle) {

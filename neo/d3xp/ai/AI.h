...
typedef struct obstaclePath_s {
  ...
  idEntity* startPosObstacle;  // if != NULL the obstacle containing the start
                               // position
  idVec3 seekPosOutsideObstacles;  // seek position outside obstacles
  idEntity*
      seekPosObstacle;  // if != NULL the obstacle containing the seek position
} obstaclePath_t;
...
class idAASFindAttackPosition : public idAASCallback {
 public:
  idAASFindAttackPosition(const idAI* self,
...
class idAI : public idActor {
 public:
  ...
  bool GetAimDir(const idVec3& firePos,
                 idEntity* aimAtEnt,
  ...
  static bool FindPathAroundObstacles(const idPhysics* physics,
                                      const idAAS* aas,
                                      const idEntity* ignore,
  ...
  static bool PredictPath(const idEntity* ent,
                          const idAAS* aas,
                          const idVec3& start,
 ...
 protected:
  ...
  bool allowHiddenMovement;  // allows character to still move around while
                             // hidden
  ...
  idVec3 FirstVisiblePointOnPath(const idVec3 origin,
                                 const idVec3& target,
  ...
  int PointReachableAreaNum(const idVec3& pos,
                            const float boundsScale = 2.0f) const;
  bool PathToGoal(aasPath_t& path,
                  int areaNum,
                  const idVec3& origin,
                  int goalAreaNum,
  ...
  bool EntityCanSeePos(idActor* actor,
  ...
  // effects
  const idDeclParticle* SpawnParticlesOnJoint(particleEmitter_t& pe,
  ...
  idProjectile* LaunchProjectile(const char* jointname,
                                 idEntity* target,
  ...
  idEntity* StartEmitter(const char* name,
                         const char* joint,
                         const char* particle);
  ...
  void Event_RadiusDamageFromJoint(const char* jointname,
                                   const char* damageDefName);
  void Event_BeginAttack(const char* name);
  void Event_EndAttack(void);
  void Event_MeleeAttackToJoint(const char* jointname,
  ...
  void Event_TravelDistanceBetweenPoints(const idVec3& source,
                                         const idVec3& dest);
  ...
  void Event_StartEmitter(const char* name,
                          const char* joint,
                          const char* particle);

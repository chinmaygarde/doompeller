...
bool idAASFindAreaOutOfRange::TestArea(const idAAS* aas, int areaNum) {
  ...
  gameLocal.clip.TracePoint(trace, targetPos,
                            areaCenter + idVec3(0.0f, 0.0f, 1.0f), MASK_OPAQUE,
                            NULL);
  ...
  if (head.GetEntity()) {
    ...
    if (*jointname) {
      ...
      if (focusJoint == INVALID_JOINT) {
        gameLocal.Warning("Joint '%s' not found on head on '%s'", jointname,
                          name.c_str());
      }
    }
  } else {
...
void idAI::List_f(const idCmdArgs& args) {
  ...
  for (e = 0; e < MAX_GENTITIES; e++) {
    ...
    gameLocal.Printf("%4i: %-20s %-20s %s  move: %d\n", e,
                     check->GetEntityDefName(), check->name.c_str(), statename,
                     check->allowMove);
    count++;
  }
...
void idAI::DrawRoute(void) const {
  if (aas && move.toAreaNum && move.moveCommand != MOVE_NONE &&
      move.moveCommand != MOVE_WANDER && move.moveCommand != MOVE_FACE_ENEMY &&
...
bool idAI::MoveOutOfRange(idEntity* ent, float range) {
  ...
  if (!aas->FindNearestGoal(goal, areaNum, org, pos, travelFlags, &obstacle, 1,
                            findGoal)) {
    StopMove(MOVE_STATUS_DEST_UNREACHABLE);
    AI_DEST_UNREACHABLE = true;
    return false;
...
bool idAI::MoveToAttackPosition(idEntity* ent, int attack_anim) {
  ...
  idAASFindAttackPosition findGoal(this, physicsObj.GetGravityAxis(), ent, pos,
...
bool idAI::StepDirection(float dir) {
  ...
  if ((move.moveType == MOVETYPE_FLY) && (path.endEvent == SE_BLOCKED)) {
    ...
    idAI::PredictPath(this, aas, org, idVec3(0.0f, 0.0f, -1024.0f), 1000, 1000,
                      SE_BLOCKED, path);
    ...
    idAI::PredictPath(this, aas, org, idVec3(0.0f, 0.0f, 256.0f), 1000, 1000,
                      SE_BLOCKED, path);
    ...
    for (z = org.z; z <= ceilingPos.z + 64.0f; z += 64.0f) {
      ...
      idAI::PredictPath(this, aas, start, move.moveDir * 48.0f, 1000, 1000,
                        SE_BLOCKED, path);
      if (!path.endEvent) {
        move.moveDir.z = 1.0f;
        return true;
...
bool idAI::GetMovePos(idVec3& seekPos) {
  ...
  if (!result) {
    ...
    if (ai_debugMove.GetBool()) {
      gameRenderWorld->DebugLine(colorYellow, org, seekPos, gameLocal.msec,
                                 true);
    }
  } else {
    AI_DEST_UNREACHABLE = false;
...
void idAI::Turn(void) {
  ...
  if (ai_debugMove.GetBool()) {
    ...
    gameRenderWorld->DebugLine(colorRed, org,
                               org + idAngles(0, ideal_yaw, 0).ToForward() * 64,
                               gameLocal.msec);
...
void idAI::CheckObstacleAvoidance(const idVec3& goalPos, idVec3& newPos) {
  ...
  if (ai_showObstacleAvoidance.GetBool()) {
    gameRenderWorld->DebugLine(colorBlue, goalPos + idVec3(1.0f, 1.0f, 0.0f),
                               goalPos + idVec3(1.0f, 1.0f, 64.0f),
                               gameLocal.msec);
    gameRenderWorld->DebugLine(foundPath ? colorYellow : colorRed, path.seekPos,
                               path.seekPos + idVec3(0.0f, 0.0f, 64.0f),
                               gameLocal.msec);
  }
...
void idAI::AnimMove(void) {
  ...
  if (move.moveCommand == MOVE_SLIDE_TO_POSITION) {
    if (gameLocal.time < move.startTime + move.duration) {
      goalPos = move.moveDest -
                move.moveDir *
                    MS2SEC(move.startTime + move.duration - gameLocal.time);
  ...
  if (ai_debugMove.GetBool()) {
    ...
    gameRenderWorld->DebugLine(
        ...
        org + EyeOffset() + viewAxis[0] * physicsObj.GetGravityAxis() * 16.0f,
        gameLocal.msec, true);
    DrawRoute();
  }
}
...
void idAI::AdjustFlyHeight(idVec3& vel, const idVec3& goalPos) {
  ...
  if (origin.z > goalPos.z) {
    ...
    idAI::PredictPath(this, aas, goalPos, dest - origin, 1000, 1000, SE_BLOCKED,
                      path);
  ...
  if (!goLower) {
    ...
    gameLocal.clip.Translation(trace, origin, end, physicsObj.GetClipModel(),
                               mat3_identity, MASK_MONSTERSOLID, this);
...
void idAI::StaticMove(void) {
  ...
  if (ai_debugMove.GetBool()) {
    ...
    gameRenderWorld->DebugBounds(colorMagenta, physicsObj.GetBounds(), org,
                                 gameLocal.msec);
    gameRenderWorld->DebugLine(colorBlue, org, move.moveDest, gameLocal.msec,
                               true);
    gameRenderWorld->DebugLine(
        colorYellow, org + EyeOffset(),
        org + EyeOffset() + viewAxis[0] * physicsObj.GetGravityAxis() * 16.0f,
...
const idDeclParticle* idAI::SpawnParticlesOnJoint(particleEmitter_t& pe,
  ...
  if (pe.joint == INVALID_JOINT) {
    ...
    gameLocal.smokeParticles->EmitSmoke(pe.particle, pe.time,
                                        gameLocal.random.CRandomFloat(), origin,
                                        axis, timeGroup /*_D3XP*/);
  }
...
void idAI::SetEnemyPosition(void) {
  ...
  if (move.moveCommand == MOVE_TO_ENEMY) {
    ...
    if (move.moveType == MOVETYPE_FLY) {
      ...
      idAI::PredictPath(this, aas, move.moveDest, end - move.moveDest, 1000,
...
void idAI::UpdateEnemyPosition(void) {
  ...
  if (ai_debugMove.GetBool()) {
    ...
    gameRenderWorld->DebugBounds(colorWhite,
                                 enemyEnt->GetPhysics()->GetBounds(),
...
idVec3 idAI::FirstVisiblePointOnPath(const idVec3 origin,
  ...
  for (i = 0; i < 10; i++) {
    if (!aas->RouteToGoalArea(curAreaNum, curOrigin, targetAreaNum, travelFlags,
                              travelTime, &reach)) {
      break;
    }
  ...
  for (i = 1; i <= num; i++) {
    ...
    if (anim) {
      ...
      if (frame >= 0) {
        ...
        if (joint == INVALID_JOINT) {
          gameLocal.Error(
              "Invalid joint '%s' on 'launch_missile' frame command on frame "
              "%d of model '%s'",
...
bool idAI::GetAimDir(const idVec3& firePos,
  ...
  if (aimAtEnt == enemy.GetEntity()) {
    static_cast<idActor*>(aimAtEnt)->GetAIAimTargets(lastVisibleEnemyPos,
...
#ifdef _D3XP
  if (this->team == 0 &&
      !idStr::Cmp(aimAtEnt->GetEntityDefName(), "monster_demon_vulgar")) {
    targetPos1.z -= 28.f;
    targetPos2.z -= 12.f;
  }
#endif
  ...
  result = PredictTrajectory(
      ...
      projectileClipModel, MASK_SHOT_RENDERMODEL, max_height, ignore, aimAtEnt,
      ai_debugTrajectory.GetBool() ? 1000 : 0, aimDir);
...
bool idAI::AttackMelee(const char* meleeDefName) {
  ...
  if (enemyEnt->IsType(idPlayer::Type) && g_skill.GetInteger() < 2) {
    ...
    player->CalcDamagePoints(this, this, meleeDef, 1.0f, INVALID_JOINT, &damage,
                             &armor);
...
void idAI::GetMuzzle(const char* jointname, idVec3& muzzle, idMat3& axis) {
  ...
  if (!jointname || !jointname[0]) {
    muzzle =
        physicsObj.GetOrigin() + viewAxis[0] * physicsObj.GetGravityAxis() * 14;
    muzzle -=
  ...
  if (worldMuzzleFlashHandle != -1) {
    if (gameLocal.time >= muzzleFlashEnd) {
      ...
      muzzle = physicsObj.GetOrigin() +
               (muzzle + modelOffset) * viewAxis * physicsObj.GetGravityAxis();
    ...
    for (int i = 0; i < particles.Num(); i++) {
      ...
      if (particles[i].particle && particles[i].time) {
        ...
        if (af.IsActive()) {
          ...
          animator.GetJointTransform(particles[i].joint, gameLocal.time,
                                     realVector, realAxis);
        ...
        if (!gameLocal.smokeParticles->EmitSmoke(
                ...
                gameLocal.random.CRandomFloat(), realVector, realAxis,
                timeGroup /*_D3XP*/)) {
          if (restartParticles) {
            particles[i].time = gameLocal.time;
          } else {
...
void idAI::TriggerFX(const char* joint, const char* fx) {
  if (!strcmp(joint, "origin")) {
    ...
    if (jointNum == INVALID_JOINT) {
      gameLocal.Warning("Unknown fx joint '%s' on entity %s", joint,
                        name.c_str());
      return;
    }
...
idEntity* idAI::StartEmitter(const char* name,
                             const char* joint,
                             const char* particle) {
  ...
  const idDeclEntityDef* emitterDef =
      gameLocal.FindEntityDef("func_emitter", false);
...
bool idAI::UpdateAnimationControllers(void) {
  ...
  if (!allowJointMod || !allowEyeFocus || (gameLocal.time >= focusTime)) {
    ...
    focusPos =
        lastVisibleEnemyPos + lastVisibleEnemyEyeOffset -
        eyeVerticalOffset * enemy.GetEntity()->GetPhysics()->GetGravityNormal();
  } else if (focusEnt->IsType(idActor::Type)) {
    focusPos = static_cast<idActor*>(focusEnt)->GetEyePosition() -
  ...
  for (ent = gameLocal.spawnedEntities.Next(); ent != NULL;
    ...
    if (node->disabled) {
      color = colorMdGrey;
               ...
               node->EntityInView(player, player->GetPhysics()->GetOrigin())) {
      color = colorYellow;
    } else {
      color = colorRed;
    }

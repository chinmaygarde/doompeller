...
bool idProjectile::Collide(const trace_t& collision, const idVec3& velocity) {
  ...
  if (ent->fl.takedamage) {
    ...
    if (damageDefName[0] != '\0') {
      ent->Damage(this, owner.GetEntity(), dir, damageDefName, damageScale,
...
void idProjectile::DefaultDamageEffect(idEntity* soundEnt,
  ...
  if (*sound != '\0') {
    soundEnt->StartSoundShader(declManager->FindSound(sound), SND_CHANNEL_BODY,
                               0, false, NULL);
  }
  ...
  if (*decal != '\0') {
    gameLocal.ProjectDecal(collision.c.point, -collision.c.normal, 8.0f, true,
...
void idProjectile::AddDefaultDamageEffect(const trace_t& collision,
  ...
  if (gameLocal.isServer && fl.networkSync) {
    ...
    msg.WriteLong((collision.c.material != NULL)
                      ? gameLocal.ServerRemapDecl(-1, DECL_MATERIAL,
  ...
  switch (event) {
    case EVENT_DAMAGE_EFFECT: {
      ...
      collision.c.material =
          (index != -1) ? static_cast<const idMaterial*>(
  ...
  if (owner.GetEntity()) {
    if (owner.GetEntity()->IsType(idAI::Type)) {
      ...
      if (enemy.GetEntity() == NULL ||
          ...
          (static_cast<idActor*>(enemy.GetEntity())->team == player->team)) {
  ...
  if (state == LAUNCHED) {
    if (killPhase) {
      ...
      if (gameLocal.time < orbitTime + 1500) {
        if (!gameLocal.smokeParticles->EmitSmoke(
                smokeKill, smokeKillTime, gameLocal.random.CRandomFloat(),
                orbitOrg, mat3_identity, timeGroup /*_D3XP*/)) {
          smokeKillTime = gameLocal.time;
        }
      }
    } else {
      if (accelTime && gameLocal.time < launchTime + accelTime * 1000) {
        ...
        speed = (startingVelocity + (startingVelocity + endingVelocity) * pct)
                    .Length();
      }
    }
    idGuidedProjectile::Think();
...
void idBFGProjectile::Think(void) {
  if (state == LAUNCHED) {
    ...
    for (int i = 0; i < beamTargets.Num(); i++) {
      ...
      beamEnt =
          static_cast<idAnimatedEntity*>(beamTargets[i].target.GetEntity());
      ...
      beamTargets[i].renderEntity.shaderParms[SHADERPARM_RED] =
          beamTargets[i].renderEntity.shaderParms[SHADERPARM_GREEN] =
              beamTargets[i].renderEntity.shaderParms[SHADERPARM_BLUE] =
                  beamTargets[i].renderEntity.shaderParms[SHADERPARM_ALPHA] =
                      1.0f;
      if (gameLocal.time > nextDamageTime) {
        ...
        if (damageFreq && *(const char*)damageFreq &&
            ...
            (forceDamage || beamTargets[i].target.GetEntity()->CanDamage(
        ...
        if (damageFreq && *(const char*)damageFreq &&
            ...
            beamTargets[i].target.GetEntity()->CanDamage(
                GetPhysics()->GetOrigin(), org)) {
#endif
          org = beamTargets[i].target.GetEntity()->GetPhysics()->GetOrigin() -
          ...
          beamTargets[i].renderEntity.shaderParms[SHADERPARM_RED] =
              beamTargets[i].renderEntity.shaderParms[SHADERPARM_GREEN] =
                  beamTargets[i].renderEntity.shaderParms[SHADERPARM_BLUE] =
                      beamTargets[i]
                          .renderEntity.shaderParms[SHADERPARM_ALPHA] = 0.0f;
          bfgVision = false;
        }
        if (player) {
...
void idBFGProjectile::Explode(const trace_t& collision, idEntity* ignore) {
  ...
  for (i = 0; i < beamTargets.Num(); i++) {
    ...
    if (!beamTargets[i].target.GetEntity()->CanDamage(GetPhysics()->GetOrigin(),
                                                      dmgPoint)) {
      continue;
    }
    ...
    if (damage[0] && (beamTargets[i].target.GetEntity()->entityNumber >
      ...
      beamTargets[i].target.GetEntity()->Damage(
          ...
          (collision.c.id < 0) ? CLIPMODEL_ID_TO_JOINT_HANDLE(collision.c.id)
                               : INVALID_JOINT);
    }
  }

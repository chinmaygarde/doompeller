...
static const char* iconKeys[ICON_NONE] = {"mtr_icon_lag", "mtr_icon_chat"
#ifdef CTF
                                          ,
                                          "mtr_icon_redteam",
                                          "mtr_icon_blueteam"
...
#ifdef CTF
  } else if (g_CTFArrows.GetBool() && gameLocal.mpGame.IsGametypeFlagBased() &&
             gameLocal.GetLocalPlayer() &&
             player->team == gameLocal.GetLocalPlayer()->team &&
             !player->IsHidden() && !player->AI_DEAD) {

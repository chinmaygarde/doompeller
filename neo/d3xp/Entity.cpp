...
void idEntity::SetName(const char* newname) {
  ...
  if (name.Length()) {
    if ((name == "NULL") || (name == "null_entity")) {
      gameLocal.Error("Cannot name entity '%s'.  '%s' is reserved for script.",
...
void idEntity::UpdatePVSAreas(void) {
  ...
  modelAbsBounds.FromTransformedBounds(renderEntity.bounds, renderEntity.origin,
                                       renderEntity.axis);
  ...
  if (localNumPVSAreas > MAX_PVS_AREAS) {
    localNumPVSAreas = gameLocal.pvs.GetPVSAreas(
        idBounds(renderEntity.origin).Expand(64.0f), localPVSAreas,
        sizeof(localPVSAreas) / sizeof(localPVSAreas[0]));
  }
...
bool idEntity::StartSoundShader(const idSoundShader* shader,
  ...
  len = refSound.referenceSound->StartSound(
      shader, channel, diversity, soundShaderFlags, !timeGroup /*_D3XP*/);
  if (length) {
    *length = len;
  }
...
void idEntity::UpdateSound(void) {
  if (refSound.referenceSound) {
    ...
    refSound.referenceSound->UpdateEmitter(refSound.origin, refSound.listenerId,
                                           &refSound.parms);
  }
}
...
void idEntity::GetWorldVelocities(idVec3& linearVelocity,
  ...
  if (bindMaster) {
    ...
    // velocity
    linearVelocity =
        linearVelocity * masterAxis + masterLinearVelocity +
...
bool idEntity::CanDamage(const idVec3& origin, idVec3& damagePoint) const {
  ...
  midpoint =
      (GetPhysics()->GetAbsBounds()[0] + GetPhysics()->GetAbsBounds()[1]) * 0.5;
...
void idEntity::AddDamageEffect(const trace_t& collision,
  ...
  if (*sound != '\0') {
    StartSoundShader(declManager->FindSound(sound), SND_CHANNEL_BODY, 0, false,
                     NULL);
  }
        ...
        if (src.ReadToken(&token2)) {
          while (src.CheckTokenString("::")) {
            ...
            if (!src.ReadToken(&token3)) {
              gameLocal.Error(
                  "Expecting function name following '::' in gui for entity "
                  "'%s'",
          ...
          if (!func) {
            gameLocal.Error("Can't find function '%s' for gui in entity '%s'",
  ...
  if (spawnArgs.GetString("bind", "", &bind)) {
    ...
    if (parent) {
      ...
      if (spawnArgs.GetString("bindToJoint", "", &joint) && *joint) {
        ...
        if (!parentAnimator) {
          gameLocal.Error(
              "Cannot bind to joint '%s' on '%s'.  Entity does not support "
              "skeletal models.",
              joint, name.c_str());
        }
        bindJoint = parentAnimator->GetJointHandle(joint);
        if (bindJoint == INVALID_JOINT) {
          gameLocal.Error("Joint '%s' not found for bind on '%s'", joint,
                          name.c_str());
        }
        ...
        if ((parent->spawnArgs.GetString("bindanim", "", &bindanim) ||
          ...
          if (!animNum) {
            gameLocal.Error("Anim '%s' not found for bind on '%s'", bindanim,
                            name.c_str());
          }
...
void idEntity::Event_StartSoundShader(const char* soundName, int channel) {
  ...
  StartSoundShader(declManager->FindSound(soundName), (s_channelType)channel, 0,
                   false, &length);
...
void idEntity::Event_DistanceTo(idEntity* ent) {
  if (!ent) {
    ...
    float dist = (GetPhysics()->GetOrigin() - ent->GetPhysics()->GetOrigin())
                     .LengthFast();
...
void idAnimatedEntity::AddDamageEffect(const trace_t& collision,
  ...
  if (gameLocal.isServer) {
    ...
    msg.WriteLong(gameLocal.ServerRemapDecl(-1, DECL_MATERIAL,
...
void idAnimatedEntity::UpdateDamageEffects(void) {
  ...
  for (de = this->damageEffects; de; de = de->next) {
    ...
    if (!gameLocal.smokeParticles->EmitSmoke(
            de->type, de->time, gameLocal.random.CRandomFloat(), start, axis,
            timeGroup /*_D3XP*/)) {
      de->time = 0;
    }
  }
...
bool idAnimatedEntity::ClientReceiveEvent(int event,
  ...
  switch (event) {
    case EVENT_ADD_DAMAGE_EFFECT: {
      ...
      damageDefIndex =
          gameLocal.ClientRemapDecl(DECL_ENTITYDEF, msg.ReadLong());
      materialIndex = gameLocal.ClientRemapDecl(DECL_MATERIAL, msg.ReadLong());
      const idDeclEntityDef* damageDef = static_cast<const idDeclEntityDef*>(
          declManager->DeclByIndex(DECL_ENTITYDEF, damageDefIndex));
      const idMaterial* collisionMaterial = static_cast<const idMaterial*>(

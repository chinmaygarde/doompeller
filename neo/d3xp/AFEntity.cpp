...
================
*/
void idChain::BuildChain(const idStr& name,
                         const idVec3& origin,
                         float linkLength,
                         float linkWidth,
                         float density,
  ...
  for (i = 0; i < numLinks; i++) {
    ...
    if (bindToWorld) {
      ...
      if (lastBody) {
        bsj = new idAFConstraint_BallAndSocketJoint("joint" + idStr(i),
                                                    lastBody, body);
...
void idAFAttachment::Damage(idEntity* inflictor,
                            ...
                            const int location) {
  if (body) {
    body->Damage(inflictor, attacker, dir, damageDefName, damageScale,
                 attachJoint);
  }
}
...
void idAFAttachment::GetImpactInfo(idEntity* ent,
                                   ...
                                   impactInfo_t* info) {
  if (body) {
    body->GetImpactInfo(ent, JOINT_HANDLE_TO_CLIPMODEL_ID(attachJoint), point,
                        info);
  } else {
...
void idAFAttachment::ApplyImpulse(idEntity* ent,
  ...
  if (body) {
    body->ApplyImpulse(ent, JOINT_HANDLE_TO_CLIPMODEL_ID(attachJoint), point,
                       impulse);
  } else {
...
void idAFAttachment::AddForce(idEntity* ent,
                              ...
                              const idVec3& force) {
  if (body) {
    body->AddForce(ent, JOINT_HANDLE_TO_CLIPMODEL_ID(attachJoint), point,
                   force);
  } else {
...
void idAFAttachment::PlayIdleAnim(int blendTime) {
  if (idleAnim &&
      (idleAnim != animator.CurrentAnim(ANIMCHANNEL_ALL)->AnimNum())) {
...
bool idAFEntity_Base::Collide(const trace_t& collision,
  ...
  if (af.IsActive()) {
    ...
    if (v > BOUNCE_SOUND_MIN_VELOCITY && gameLocal.time > nextSoundTime) {
      f = v > BOUNCE_SOUND_MAX_VELOCITY
              ? 1.0f
              : idMath::Sqrt(v - BOUNCE_SOUND_MIN_VELOCITY) *
                    (1.0f / idMath::Sqrt(BOUNCE_SOUND_MAX_VELOCITY -
...
================
*/
void idAFEntity_Gibbable::Damage(idEntity* inflictor,
                                 idEntity* attacker,
                                 const idVec3& dir,
                                 const char* damageDefName,
                                 const float damageScale,
  ...
  idAFEntity_Base::Damage(inflictor, attacker, dir, damageDefName, damageScale,
                          location);
...
=====================
*/
bool idAFEntity_Gibbable::Collide(const trace_t& collision,
                                  const idVec3& velocity) {
  if (!gibbed && wasThrown) {
    ...
    if (spawnArgs.GetBool("gib")) {
      ...
      if (ent->fl.takedamage) {
        ent->Damage(this, gameLocal.GetLocalPlayer(), collision.c.normal,
                    "damage_thrown_ragdoll", 1.f,
...
=====================
*/
void idAFEntity_Gibbable::SpawnGibs(const idVec3& dir,
                                    const char* damageDefName) {
  int i;
  bool gibNonSolid;
  ...
  for (i = 0; i < list.Num(); i++) {
    if (gibNonSolid) {
      ...
      velocity =
          list[i]->GetPhysics()->GetAbsBounds().GetCenter() - entityCenter;
    ...
    list[i]->GetRenderEntity()->shaderParms[SHADERPARM_TIME_OF_DEATH] =
...
void idAFEntity_WithAttachedHead::Spawn(void) {
  ...
  if (head.GetEntity()) {
    ...
    if (anim) {
      head.GetEntity()->GetAnimator()->SetFrame(ANIMCHANNEL_ALL, anim, 0,
                                                gameLocal.time, 0);
    }
  }
}
...
void idAFEntity_WithAttachedHead::SetupHead(void) {
  ...
  if (headModel[0]) {
    ...
    headEnt = static_cast<idAFAttachment*>(
        gameLocal.SpawnEntityType(idAFAttachment::Type, NULL));
...
================
*/
void idAFEntity_WithAttachedHead::ProjectOverlay(const idVec3& origin,
                                                 const idVec3& dir,
                                                 float size,
                                                 const char* material) {
...
============
*/
void idAFEntity_WithAttachedHead::Gib(const idVec3& dir,
                                      const char* damageDefName) {
  // only gib once
  if (gibbed) {
    return;
...
void idAFEntity_Vehicle::Spawn(void) {
  ...
  const char* steeringWheelJointName =
      spawnArgs.GetString("steeringWheelJoint", "steeringWheel");
  ...
  if (!eyesJointName[0]) {
    gameLocal.Error("idAFEntity_Vehicle '%s' no eyes joint specified",
                    name.c_str());
  }
  eyesJoint = animator.GetJointHandle(eyesJointName);
  if (!steeringWheelJointName[0]) {
    gameLocal.Error("idAFEntity_Vehicle '%s' no steering wheel joint specified",
                    name.c_str());
  }
  ...
  const char* smokeName =
      spawnArgs.GetString("smoke_vehicle_dust", "muzzlesmoke");
  if (*smokeName != '\0') {
    dustSmoke = static_cast<const idDeclParticle*>(
...
void idAFEntity_VehicleSimple::Spawn(void) {
  static const char* wheelJointKeys[] = {
      "wheelJointFrontLeft", "wheelJointFrontRight", "wheelJointRearLeft",
      "wheelJointRearRight"};
  static idVec3 wheelPoly[4] = {idVec3(2, 2, 0), idVec3(2, -2, 0),
                                idVec3(-2, -2, 0), idVec3(-2, 2, 0)};
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (!wheelJointName[0]) {
      gameLocal.Error("idAFEntity_VehicleSimple '%s' no '%s' specified",
                      name.c_str(), wheelJointKeys[i]);
    }
    ...
    suspension[i]->Setup(va("suspension%d", i), af.GetPhysics()->GetBody(0),
                         origin, af.GetPhysics()->GetAxis(0), wheelModel);
...
void idAFEntity_VehicleSimple::Think(void) {
  ...
  if (thinkFlags & TH_THINK) {
    if (player) {
      ...
      force = idMath::Fabs(player->usercmd.forwardmove *
                           g_vehicleForce.GetFloat()) *
              (1.0f / 128.0f);
    ...
    for (i = 0; i < 4; i++) {
      ...
      if (i < 2) {
        ...
        // set wheel rotation
        animator.SetJointAxis(wheelJoints[i], JOINTMOD_WORLD,
                              wheelRotation.ToMat3() * steerRotation.ToMat3());
      } else {
        // set wheel rotation
        animator.SetJointAxis(wheelJoints[i], JOINTMOD_WORLD,
      ...
      GetAnimator()->SetJointPos(wheelJoints[i], JOINTMOD_WORLD_OVERRIDE,
                                 origin);
    }
    /*
...
void idAFEntity_VehicleFourWheels::Spawn(void) {
  ...
  static const char* wheelBodyKeys[] = {
      "wheelBodyFrontLeft", "wheelBodyFrontRight", "wheelBodyRearLeft",
      "wheelBodyRearRight"};
  static const char* wheelJointKeys[] = {
      "wheelJointFrontLeft", "wheelJointFrontRight", "wheelJointRearLeft",
      "wheelJointRearRight"};
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (!wheelBodyName[0]) {
      gameLocal.Error("idAFEntity_VehicleFourWheels '%s' no '%s' specified",
                      name.c_str(), wheelBodyKeys[i]);
    }
  ...
  for (i = 0; i < 2; i++) {
    ...
    if (!steeringHingeName[0]) {
      gameLocal.Error("idAFEntity_VehicleFourWheels '%s' no '%s' specified",
                      name.c_str(), steeringHingeKeys[i]);
    }
    steering[i] = static_cast<idAFConstraint_Hinge*>(
        af.GetPhysics()->GetConstraint(steeringHingeName));
    if (!steering[i]) {
      gameLocal.Error(
...
void idAFEntity_VehicleFourWheels::Think(void) {
  ...
  if (thinkFlags & TH_THINK) {
    ...
    animator.GetJointTransform(steeringWheelJoint, gameLocal.time, origin,
                               axis);
    rotation.SetVec(axis[2]);
    rotation.SetAngle(-steerAngle);
    animator.SetJointAxis(steeringWheelJoint, JOINTMOD_WORLD,
                          rotation.ToMat3());
    ...
    for (i = 0; i < 4; i++) {
      if (force == 0.0f) {
        velocity =
            wheels[i]->GetLinearVelocity() * wheels[i]->GetWorldAxis()[0];
      }
      wheelAngles[i] += velocity * MS2SEC(gameLocal.msec) / wheelRadius;
    ...
    if (force != 0.0f && !(gameLocal.framenum & 7)) {
      ...
      idAFConstraint_Contact* contacts[2];
      for (i = 0; i < 4; i++) {
        numContacts = af.GetPhysics()->GetBodyContactConstraints(
            wheels[i]->GetClipModel()->GetId(), contacts, 2);
        for (int j = 0; j < numContacts; j++) {
          gameLocal.smokeParticles->EmitSmoke(
              dustSmoke, gameLocal.time, gameLocal.random.RandomFloat(),
              contacts[j]->GetContact().point,
              contacts[j]->GetContact().normal.ToMat3(), timeGroup /* D3XP */);
        }
      }
    }
  }
...
void idAFEntity_VehicleSixWheels::Spawn(void) {
  int i;
  static const char* wheelBodyKeys[] = {
      "wheelBodyFrontLeft",   "wheelBodyFrontRight", "wheelBodyMiddleLeft",
      "wheelBodyMiddleRight", "wheelBodyRearLeft",   "wheelBodyRearRight"};
  static const char* wheelJointKeys[] = {
      "wheelJointFrontLeft",   "wheelJointFrontRight", "wheelJointMiddleLeft",
      "wheelJointMiddleRight", "wheelJointRearLeft",   "wheelJointRearRight"};
  static const char* steeringHingeKeys[] = {
      "steeringHingeFrontLeft", "steeringHingeFrontRight",
      "steeringHingeRearLeft", "steeringHingeRearRight"};
  ...
  for (i = 0; i < 6; i++) {
    wheelBodyName = spawnArgs.GetString(wheelBodyKeys[i], "");
    if (!wheelBodyName[0]) {
      gameLocal.Error("idAFEntity_VehicleSixWheels '%s' no '%s' specified",
                      name.c_str(), wheelBodyKeys[i]);
    }
    wheels[i] = af.GetPhysics()->GetBody(wheelBodyName);
    if (!wheels[i]) {
...
EVENT(EV_Vehicle_setSteeringSpeed,
...
void idAFEntity_VehicleAutomated::Think(void) {
  ...
  if (g_vehicleDebug.GetBool()) {
    gameRenderWorld->DebugBounds(colorRed,
                                 idBounds(idVec3(-4, -4, -4), idVec3(4, 4, 4)),
                                 vehicle_origin);
    gameRenderWorld->DebugBounds(colorRed,
                                 idBounds(idVec3(-4, -4, -4), idVec3(4, 4, 4)),
                                 waypoint_origin);
    gameRenderWorld->DrawText(
        waypoint->name.c_str(), waypoint_origin + idVec3(0, 0, 16), 0.25f,
        colorYellow, gameLocal.GetLocalPlayer()->viewAxis);
    gameRenderWorld->DebugArrow(colorWhite, vehicle_origin, waypoint_origin,
                                12.f);
  }
...
void idAFEntity_SteamPipe::Spawn(void) {
  ...
  force.SetPosition(af.GetPhysics(), steamBody,
                    af.GetPhysics()->GetOrigin(steamBody));
...
void idAFEntity_SteamPipe::InitSteamRenderEntity(void) {
  ...
  if (*temp != '\0') {
    ...
    if (steamRenderEntity.hModel) {
      steamRenderEntity.bounds =
          steamRenderEntity.hModel->Bounds(&steamRenderEntity);
    } else {
...
static const char* clawConstraintNames[] = {"claw1", "claw2", "claw3", "claw4"};
...
void idAFEntity_ClawFourFingers::Restore(idRestoreGame* savefile) {
  ...
  for (i = 0; i < 4; i++) {
    fingers[i] = static_cast<idAFConstraint_Hinge*>(
        af.GetPhysics()->GetConstraint(clawConstraintNames[i]));
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (!fingers[i]) {
      gameLocal.Error(
          ...
          name.c_str(), clawConstraintNames[i]);
    }
  }
}
...
bool idGameEdit::AF_SpawnEntity(const char* fileName) {
  ...
  args.Set("nodrop", "1");
  ent = static_cast<idAFEntity_Generic*>(
      gameLocal.SpawnEntityType(idAFEntity_Generic::Type, &args));
...
void idGameEdit::AF_UndoChanges(void) {
  ...
  for (i = 0; i < c; i++) {
    decl = static_cast<idDeclAF*>(
        const_cast<idDecl*>(declManager->DeclByIndex(DECL_AF, i, false)));
    if (!decl->modified) {
      continue;
    }
...
static bool GetJointTransform(void* model,
...
================
*/
static const char* GetArgString(const idDict& args,
...
idRenderModel* idGameEdit::AF_CreateMesh(const idDict& args,
  ...
  ANIM_CreateAnimFrame(md5, MD5anim, ent.numJoints, ent.joints, 1,
                       modelDef->GetVisualOffset(), false);
  ...
  for (arg = args.MatchPrefix("body ", NULL); arg;
    ...
    sscanf(arg->GetValue(), "%f %f %f %f %f %f", &origin.x, &origin.y,
           &origin.z, &angles.pitch, &angles.yaw, &angles.roll);
  ...
  originalJoints =
      (idJointMat*)_alloca16(numMD5joints * sizeof(originalJoints[0]));
  memcpy(originalJoints, ent.joints, numMD5joints * sizeof(originalJoints[0]));

  // buffer to store the joint mods
  jointMod =
      (declAFJointMod_t*)_alloca16(numMD5joints * sizeof(declAFJointMod_t));
  ...
  for (i = 0; i < af->bodies.Num(); i++) {
    ...
    if (jointNum >= 0 && jointNum < ent.numJoints) {
      ...
      modifiedAxis[jointNum] =
          (bodyAxis[i] * originalJoints[jointNum].ToMat3().Transpose())
              .Transpose() *
          (newBodyAxis[i] * meshAxis.Transpose());
  ...
  for (i = 1; i < numMD5joints; i++, MD5joint++) {
    ...
    idVec3 localt =
        (originalJoints[i].ToVec3() - originalJoints[parentNum].ToVec3()) *
        parentAxis.Transpose();
    ...
    switch (jointMod[i]) {
      ...
      case DECLAF_JOINTMOD_AXIS: {
        ...
        ent.joints[i].SetTranslation(ent.joints[parentNum].ToVec3() +
                                     localt * ent.joints[parentNum].ToMat3());
        break;
      }
...
void idHarvestable::Init(idEntity* parent) {
  ...
  if (parent->IsType(idAFEntity_WithAttachedHead::Type)) {
    idAFEntity_WithAttachedHead* withHead =
        (idAFEntity_WithAttachedHead*)parent;
...
void idHarvestable::Think() {
  ...
  if (trigger && parent && !parent->GetPhysics()->IsAtRest()) {
    trigger->Link(gameLocal.clip, this, 0, parent->GetPhysics()->GetOrigin(),
  ...
  if (startTime && gameLocal.slow.time - startTime > giveDelay && !given) {
    ...
    thePlayer->Give(spawnArgs.GetString("give_item"),
                    spawnArgs.GetString("give_value"));
  ...
  if (fxFollowPlayer) {
    ...
    if (fxEnt) {
      ...
      if (GetFxOrientationAxis(orientAxisLocal)) {
        // gameRenderWorld->DebugAxis(fxEnt->GetPhysics()->GetOrigin(),
        // orientAxisLocal);
...
void idHarvestable::BeginFX() {
  ...
  fx = idEntityFx::StartFx(spawnArgs.GetString("fx"), NULL, orientAxis, this,
                           spawnArgs.GetBool("fx_bind"));
}
...
bool idHarvestable::GetFxOrientationAxis(idMat3& mat) {
  ...
  if (!fxOrient.Icmp("up")) {
    ...
    idMat3 temp(left.x, left.y, left.z, up.x, up.y, up.z, grav.x, grav.y,
                grav.z);
    mat = temp;
    ...
    joint = thePlayer->weapon.GetEntity()->GetAnimator()->GetJointHandle(
        spawnArgs.GetString("fx_weapon_joint"));
    if (joint != INVALID_JOINT) {
      thePlayer->weapon.GetEntity()->GetJointWorldTransform(
          joint, gameLocal.slow.time, joint_origin, joint_axis);
    } else {
    ...
    idMat3 temp(left.x, left.y, left.z, up.x, up.y, up.z, toPlayer.x,
    ...
    // toPlayer.x, toPlayer.y, toPlayer.z, left.x, left.y, left.z, up.x, up.y,
    // up.z );
...
void idHarvestable::Event_SpawnHarvestTrigger(void) {
  ...
  trigger->Link(gameLocal.clip, this, 255, parent->GetPhysics()->GetOrigin(),
                mat3_identity);
...
void idHarvestable::Event_Touch(idEntity* other, trace_t* trace) {
  ...
  if (!startTime && other && other->IsType(idPlayer::Type)) {
    ...
    if (requiredWeapons.Length() > 0) {
      ...
      if (playerWeap.Length() == 0 ||
          requiredWeapons.Find(playerWeap, false) == -1) {
        okToGive = false;
      }
    }

    if (okToGive) {
      if (thePlayer->CanGive(spawnArgs.GetString("give_item"),
                             spawnArgs.GetString("give_value"))) {
        startTime = gameLocal.slow.time;

        // Lock the player from harvesting to prevent multiple harvests when
        // only one is needed
...
EVENT(EV_Harvest_SpawnHarvestEntity,
...
void idAFEntity_Harvest::Event_SpawnHarvestEntity(void) {
  const idDict* harvestDef = gameLocal.FindEntityDefDict(
      spawnArgs.GetString("def_harvest_type"), false);
  if (harvestDef) {
    idEntity* temp;

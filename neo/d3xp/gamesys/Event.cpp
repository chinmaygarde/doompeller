...
idEventDef::idEventDef(const char* command,
  ...
  if (numargs > D_EVENT_MAXARGS) {
    ...
    sprintf(eventErrorMsg,
            "idEventDef::idEventDef : Too many args for '%s' event.", name);
    return;
  }
...
idEvent* idEvent::Alloc(const idEventDef* evdef, int numargs, va_list args) {
  ...
  if (numargs != evdef->GetNumArgs()) {
    gameLocal.Error("idEvent::Alloc : Wrong number of args for '%s' event.",
                    evdef->GetName());
  }
  ...
  for (i = 0; i < numargs; i++) {
    ...
    if (format[i] != arg->type) {
      ...
      if (!(((format[i] == D_EVENT_TRACE) || (format[i] == D_EVENT_ENTITY)) &&
            (arg->type == 'd') && (arg->value == 0))) {
        gameLocal.Error(
            "idEvent::Alloc : Wrong type passed in for arg # %d on '%s' event.",
            i, evdef->GetName());
      }
    }
    ...
    switch (format[i]) {
      ...
      case D_EVENT_VECTOR:
        if (arg->value) {
          *reinterpret_cast<idVec3*>(dataPtr) =
              *reinterpret_cast<const idVec3*>(arg->value);
        }
        break;

      case D_EVENT_STRING:
        if (arg->value) {
          idStr::Copynz(reinterpret_cast<char*>(dataPtr),
                        reinterpret_cast<const char*>(arg->value),
                        MAX_STRING_LEN);
        }
        break;
      ...
      case D_EVENT_ENTITY:
      case D_EVENT_ENTITY_NULL:
        *reinterpret_cast<idEntityPtr<idEntity>*>(dataPtr) =
            reinterpret_cast<idEntity*>(arg->value);
        break;

      case D_EVENT_TRACE:
        if (arg->value) {
          *reinterpret_cast<bool*>(dataPtr) = true;
          *reinterpret_cast<trace_t*>(dataPtr + sizeof(bool)) =
              *reinterpret_cast<const trace_t*>(arg->value);

          // save off the material as a string since the pointer won't be valid
          // in save games. since we save off the entire trace_t structure, if
          // the material is NULL here, it will be NULL when we process it, so
          // we don't need to save off anything in that case.
          if (reinterpret_cast<const trace_t*>(arg->value)->c.material) {
            materialName = reinterpret_cast<const trace_t*>(arg->value)
                               ->c.material->GetName();
            idStr::Copynz(reinterpret_cast<char*>(dataPtr + sizeof(bool) +
                                                  sizeof(trace_t)),
                          materialName, MAX_STRING_LEN);
          }
        } else {
      ...
      default:
        gameLocal.Error(
            "idEvent::Alloc : Invalid arg format '%s' string for '%s' event.",
...
void idEvent::CopyArgs(const idEventDef* evdef,
  ...
  format = evdef->GetArgFormat();
  if (numargs != evdef->GetNumArgs()) {
    gameLocal.Error("idEvent::CopyArgs : Wrong number of args for '%s' event.",
                    evdef->GetName());
  }
  ...
  for (i = 0; i < numargs; i++) {
    ...
    if (format[i] != arg->type) {
      ...
      if (!(((format[i] == D_EVENT_TRACE) || (format[i] == D_EVENT_ENTITY)) &&
            (arg->type == 'd') && (arg->value == 0))) {
        gameLocal.Error(
            "idEvent::CopyArgs : Wrong type passed in for arg # %d on '%s' "
            "event.",
            i, evdef->GetName());
      }
    }
  ...
  while (!EventQueue.IsListEmpty()) {
    ...
    for (i = 0; i < numargs; i++) {
      ...
      switch (formatspec[i]) {
        ...
        case D_EVENT_VECTOR:
          *reinterpret_cast<idVec3**>(&args[i]) =
              reinterpret_cast<idVec3*>(&data[offset]);
          break;

        case D_EVENT_STRING:
          *reinterpret_cast<const char**>(&args[i]) =
              reinterpret_cast<const char*>(&data[offset]);
          break;

        case D_EVENT_ENTITY:
        case D_EVENT_ENTITY_NULL:
          *reinterpret_cast<idEntity**>(&args[i]) =
              reinterpret_cast<idEntityPtr<idEntity>*>(&data[offset])
                  ->GetEntity();
          break;
        ...
        default:
          gameLocal.Error(
              "idEvent::ServiceEvents : Invalid arg format '%s' string for "
              "'%s' event.",
  ...
  while (!FastEventQueue.IsListEmpty()) {
    ...
    for (i = 0; i < numargs; i++) {
      ...
      switch (formatspec[i]) {
        ...
        default:
          gameLocal.Error(
              "idEvent::ServiceFastEvents : Invalid arg format '%s' string for "
              "'%s' event.",
              formatspec, ev->GetName());
      }
    }
      ...
      switch (format[i]) {
        ...
        case D_EVENT_TRACE:
          ...
          if (validTrace) {
            ...
            if (t.c.material) {
              size += MAX_STRING_LEN;
              str = reinterpret_cast<char*>(dataPtr + sizeof(bool) +
                                            sizeof(trace_t));
...
void idEvent::Restore(idRestoreGame* savefile) {
  ...
  for (i = 0; i < num; i++) {
    ...
    if (!event->typeinfo) {
      savefile->Error("idEvent::Restore: unknown class '%s' on event '%s'",
                      name.c_str(), event->eventdef->GetName());
    }
    ...
    if (argsize != event->eventdef->GetArgSize()) {
      savefile->Error(
          "idEvent::Restore: arg size (%d) doesn't match saved arg size(%d) on "
          "event '%s'",

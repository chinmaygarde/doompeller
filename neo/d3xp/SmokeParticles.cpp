...
void idSmokeParticles::FreeSmokes(void) {
  for (int activeStageNum = 0; activeStageNum < activeStages.Num();
       activeStageNum++) {
    ...
    for (last = NULL, smoke = active->smokes; smoke; smoke = next) {
      ...
      if (smoke->timeGroup) {
        frac = (float)(gameLocal.fast.time - smoke->privateStartTime) /
               (stage->particleLife * 1000);
      } else {
        frac = (float)(gameLocal.slow.time - smoke->privateStartTime) /
               (stage->particleLife * 1000);
      }
#else
      float frac = (float)(gameLocal.time - smoke->privateStartTime) /
                   (stage->particleLife * 1000);
#endif
      if (frac >= 1.0f) {
...
bool idSmokeParticles::EmitSmoke(const idDeclParticle* smoke,
  ...
  // for each stage in the smoke that is still emitting particles, emit a new
  // singleSmoke_t
  for (int stageNum = 0; stageNum < smoke->stages.Num(); stageNum++) {
    ...
    if (finalParticleTime == 0) {
      ...
      nowCount =
          floor(((float)deltaMsec / finalParticleTime) * stage->totalParticles);
      if (nowCount >= stage->totalParticles) {
        nowCount = stage->totalParticles - 1;
      }
      prevCount =
          floor(((float)(deltaMsec -
                         gameLocal.msec /*_D3XP - FIX - was USERCMD_MSEC*/) /
                 finalParticleTime) *
    ...
    for (prevCount++; prevCount <= nowCount; prevCount++) {
      if (!freeSmokes) {
        gameLocal.Printf(
            "idSmokeParticles::EmitSmoke: no free smokes with %d active "
            "stages\n",
            activeStages.Num());
        return true;
      }
      ...
      newSmoke->privateStartTime = systemStartTime + prevCount *
                                                         finalParticleTime /
                                                         stage->totalParticles;
...
bool idSmokeParticles::UpdateRenderEntity(renderEntity_s* renderEntity,
  ...
  for (int activeStageNum = 0; activeStageNum < activeStages.Num();
    ...
    srfTriangles_t* tri =
        renderEntity->hModel->AllocSurfaceTriangles(quads * 4, quads * 6);
...
#ifdef _D3XP
      if (smoke->timeGroup) {
        g.frac = (float)(gameLocal.fast.time - smoke->privateStartTime) /
                 (stage->particleLife * 1000);
      } else {
        g.frac = (float)(gameLocal.time - smoke->privateStartTime) /
                 (stage->particleLife * 1000);
      }
#else
      g.frac = (float)(gameLocal.time - smoke->privateStartTime) /
...
bool idSmokeParticles::ModelCallback(renderEntity_s* renderEntity,
  ...
  if (gameLocal.smokeParticles) {
    return gameLocal.smokeParticles->UpdateRenderEntity(renderEntity,
                                                        renderView);
  }

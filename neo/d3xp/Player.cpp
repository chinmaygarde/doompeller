...
===============
*/
bool idInventory::CanGive(idPlayer* owner,
                          const idDict& spawnArgs,
                          const char* statname,
                          const char* value,
                          int* idealWeapon) {
...
void idPlayer::SetupWeaponEntity(void) {
  ...
  if (weapon.GetEntity()) {
    ...
    weapon =
        static_cast<idWeapon*>(gameLocal.SpawnEntityType(idWeapon::Type, NULL));
...
void idPlayer::Spawn(void) {
  ...
  if (entityNumber >= MAX_CLIENTS) {
    gameLocal.Error(
        "entityNum > MAX_CLIENTS for player.  Player may only be spawned with "
        "a client.");
  }
  ...
  if (hud) {
    ...
    if (weapon_bloodstone > 0 &&
        (inventory.weapons & (1 << weapon_bloodstone))) {
...
void idPlayer::ServerSpectate(bool spectate) {
  ...
  if (spectate && gameLocal.isMultiplayer &&
...
void idPlayer::SpawnToPoint(const idVec3& spawn_origin,
  ...
  if (!spectating) {
    physicsObj.SetClipMask(
        MASK_PLAYERSOLID);  // the clip mask is usually maintained in Move(),
                            // but KillBox requires it
...
bool idPlayer::UserInfoChanged(bool canModify) {
  ...
  if (canModify && gameLocal.mpGame.IsGametypeTeamBased() &&
      !gameLocal.mpGame.IsInGame(entityNumber) &&
      g_balanceTDM.GetBool()) { /* CTF */
...
void idPlayer::UpdateHudAmmo(idUserInterface* _hud) {
  ...
  if (ammoamount < 0 || !weapon.GetEntity()->IsReady() ||
      currentWeapon == weapon_bloodstone) {
#else
    if (ammoamount < 0 || !weapon.GetEntity()->IsReady()) {
#endif
    // show infinite ammo
  ...
  _hud->SetStateBool(
      "player_clip_low",
      (weapon.GetEntity()->ClipSize() ? inclip <= weapon.GetEntity()->LowAmmo()
                                      : false));
  ...
  // value
  _hud->SetStateString("player_ammo_count",
                       va("%i", weapon.GetEntity()->AmmoCount()));
#endif

#ifdef _D3XP
  // Make sure the hud always knows how many bloodstone charges there are
  int ammoRequired;
  ammo_t ammo_i = inventory.AmmoIndexForWeaponClass("weapon_bloodstone_passive",
                                                    &ammoRequired);
...
bool idPlayer::Give(const char* statname, const char* value) {
  ...
  if (!idStr::Icmp(statname, "health")) {
    ...
    bool ret =
        inventory.Give(this, spawnArgs, statname, value, &idealWeapon, true);
    if (!idStr::Icmp(statname, "ammo_bloodstone")) {
      ...
      if (hud) {
        ...
        int ammoRequired;
        ammo_t ammo_i = inventory.AmmoIndexForWeaponClass(
            "weapon_bloodstone_passive", &ammoRequired);
        int bloodstoneAmmo = inventory.HasAmmo(ammo_i, ammoRequired);
  ...
  if (powerup >= 0 && powerup < MAX_POWERUPS) {
    ...
    switch (powerup) {
      ...
      /*case HASTE: {
              ...
              if ( baseSkinName.Length() ) {
                      powerUpSkin = declManager->FindSkin( baseSkinName +
      "_haste" );
              }
              break;
      }*/
...
void idPlayer::UpdateObjectiveInfo(void) {
  ...
  for (int i = 0; i < inventory.objectiveNames.Num(); i++) {
    ...
    objectiveSystem->SetStateString(va("objectivetitle%i", i + 1),
                                    inventory.objectiveNames[i].title.c_str());
    objectiveSystem->SetStateString(va("objectivetext%i", i + 1),
...
void idPlayer::SelectWeapon(int num, bool force) {
  ...
  if (weaponToggles.Get(va("weapontoggle%d", num), &weaponToggle)) {
    ...
    for (int i = 0; i < weaponToggle->toggleList.Num(); i++) {
      // Is it available
      if (inventory.weapons &
          (1 << weaponToggle->toggleList[weaponToggleIndex])) {
        break;
      }
  ...
  if (hud) {
    ...
    if (locationEntity) {
      ...
      hud->SetStateString("location",
                          common->GetLanguageDict()->GetString("#str_02911"));
    }
  }
}
  ...
  if (oldChar != focusCharacter && hud) {
    if (focusCharacter) {
      ...
      hud->SetStateString("npc_action",
                          common->GetLanguageDict()->GetString("#str_02036"));
#endif
...
void idPlayer::UpdatePowerupHud() {
  ...
  if (hudPowerup != -1) {
    if (PowerUpActive(hudPowerup)) {
      ...
      if (hud) {
        hud->SetStateInt("player_powerup",
                         100 * filledbar / hudPowerupDuration);
  ...
  for (j = 0; j < inventory.pdas.Num(); j++) {
    ...
    if (j == currentPDA) {
      ...
      if (j == 0) {
        ...
        objectiveSystem->SetStateString("pda_location",
                                        hud->State().GetString("location"));
        objectiveSystem->SetStateString("pda_name",
        ...
        if (sel >= 0 && sel < inventory.videos.Num()) {
          vid = static_cast<const idDeclVideo*>(
              declManager->FindType(DECL_VIDEO, inventory.videos[sel], false));
        }
        if (vid) {
          pdaVideo = vid->GetRoq();
        ...
        int audioCount =
            AddGuiPDAData(DECL_AUDIO, "listPDAAudio", pda, objectiveSystem);
...
void idPlayer::PerformImpulse(int impulse) {
  ...
  switch (impulse) {
    ...
    case IMPULSE_25: {
      if (gameLocal.isServer && gameLocal.mpGame.IsGametypeFlagBased() &&
        ...
        if (enviroSuitLight.IsValid()) {
          ...
          if (lightDef) {
            idEntity* temp =
                static_cast<idEntity*>(enviroSuitLight.GetEntity());
            idAngles lightAng = firstPersonViewAxis.ToAngles();
            idVec3 lightOrg = firstPersonViewOrigin;
...
void idPlayer::Damage(idEntity* inflictor,
  ...
  CalcDamagePoints(inflictor, attacker, &damageDef->dict, damageScale, location,
                   &damage, &armorSave);
...
void idPlayer::SetLastHitTime(int time) {
  ...
  if (hud) {
    if (MPAim != -1) {
      ...
      hud->SetStateString("aim_text",
                          gameLocal.userInfo[lastMPAim].GetString("ui_name"));
      if (aimed) {
        hud->SetStateFloat("aim_color", aimed->colorBarIndex);
      }
      hud->HandleNamedEvent("aim_flash");
...
void idPlayer::Event_GetPreviousWeapon(void) {
  ...
  if (previousWeapon >= 0) {
    int pw =
        (gameLocal.world->spawnArgs.GetBool("no_Weapons")) ? 0 : previousWeapon;
...
void idPlayer::ClientPredictionThink(void) {
  ...
  pvsHandle_t clientPVS =
      gameLocal.pvs.SetupCurrentPVS(GetPVSAreas(), GetNumPVSAreas());
  gameLocal.portalSkyActive = gameLocal.pvs.CheckAreasForPortalSky(
  ...
  switch (event) {
    ...
    case EVENT_ADD_DAMAGE_EFFECT: {
      if (spectating) {
        ...
        // happens if the event and the spectate change are written on the
        // server during the same frame (fraglimit)
        return true;
      }

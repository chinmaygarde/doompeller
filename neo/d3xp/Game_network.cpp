...
idCVar net_clientShowSnapshot("net_clientShowSnapshot",
                              "0",
...
idCVar net_clientShowSnapshotRadius("net_clientShowSnapshotRadius",
                                    "128",
                                    CVAR_GAME | CVAR_FLOAT,
                                    "");
idCVar net_clientSmoothing("net_clientSmoothing",
                           "0.8",
                           CVAR_GAME | CVAR_FLOAT,
...
idCVar net_clientSelfSmoothing(
    ...
    0.0f,
    0.95f);
idCVar net_clientMaxPrediction(
...
idCVar net_clientLagOMeter("net_clientLagOMeter",
                           "1",
                           CVAR_GAME | CVAR_BOOL | CVAR_NOCHEAT | CVAR_ARCHIVE,
...
void idGameLocal::InitAsyncNetwork(void) {
  ...
  entityDefBits =
      -(idMath::BitsForInteger(declManager->GetNumDecls(DECL_ENTITYDEF)) + 1);
...
void idGameLocal::ServerSendDeclRemapToClient(int clientNum,
  ...
  if (decl == NULL) {
    gameLocal.Error("server tried to remap bad %s decl index %d",
                    declManager->GetDeclNameFromType(type), index);
    return;
  }
...
int idGameLocal::ClientRemapDecl(declType_t type, int index) {
  ...
  if (clientDeclRemap[localClientNum][(int)type].Num() == 0) {
    gameLocal.Error(
        ...
        index, declManager->GetDeclNameFromType(type));
    return -1;
  }
  if (index >= clientDeclRemap[localClientNum][(int)type].Num()) {
    gameLocal.Error("client received unmapped %s decl index %d from server",
                    declManager->GetDeclNameFromType(type), index);
    return -1;
  }
...
allowReply_t idGameLocal::ServerAllowClient(int numClients,
  ...
  if (pass[0] == '\0') {
    ...
    cmdSystem->BufferCommandText(
        CMD_EXEC_NOW, "say si_usepass is set but g_password is empty");
...
void idGameLocal::ServerWriteInitialReliableMessages(int clientNum) {
  ...
  for (event = savedEventQueue.Start(); event; event = event->next) {
    ...
    outMsg.WriteBits(event->paramsSize,
  ...
  for (i = 0; i < numPortals; i++) {
    outMsg.WriteBits(gameRenderWorld->GetPortalState((qhandle_t)(i + 1)),
...
void idGameLocal::FreeSnapshotsOlderThanSequence(int clientNum, int sequence) {
  ...
  for (lastSnapshot = NULL, snapshot = clientSnapshots[clientNum]; snapshot;
       snapshot = nextSnapshot) {
    nextSnapshot = snapshot->next;
    if (snapshot->sequence < sequence) {
      for (state = snapshot->firstEntityState; state;
    ...
    if (snapshot->sequence == sequence) {
      for (state = snapshot->firstEntityState; state; state = state->next) {
        if (clientEntityStates[clientNum][state->entityNumber]) {
          entityStateAllocator.Free(
              clientEntityStates[clientNum][state->entityNumber]);
        }
...
void idGameLocal::ServerWriteSnapshot(int clientNum,
  ...
  numSourceAreas = gameRenderWorld->BoundsInAreas(
      spectated->GetPlayerPhysics()->GetAbsBounds(), sourceAreas,
      idEntity::MAX_PVS_AREAS);
  pvsHandle =
      gameLocal.pvs.SetupCurrentPVS(sourceAreas, numSourceAreas, PVS_NORMAL);
  ...
  if (portalSkyEnt.GetEntity()) {
    ...
    otherPVS = gameLocal.pvs.SetupCurrentPVS(skyEnt->GetPVSAreas(),
                                             skyEnt->GetNumPVSAreas());
  ...
  for (ent = spawnedEntities.Next(); ent != NULL; ent = ent->spawnNode.Next()) {
    ...
    deltaMsg.WriteBits(
        ServerRemapDecl(-1, DECL_ENTITYDEF, ent->entityDefNumber),
        entityDefBits);
  ...
  base = clientEntityStates[clientNum]
                           [ENTITYNUM_NONE];  // ENTITYNUM_NONE is used for the
  ...
  if (player->spectating && player->spectator != player->entityNumber &&
      gameLocal.entities[player->spectator] &&
      gameLocal.entities[player->spectator]->IsType(idPlayer::Type)) {
    static_cast<idPlayer*>(gameLocal.entities[player->spectator])
        ->WritePlayerStateToSnapshot(deltaMsg);
  } else {
...
void idGameLocal::NetworkEventWarning(const entityNetEvent_t* event,
  ...
  length += idStr::snPrintf(buf + length, sizeof(buf) - 1 - length,
                            "event %d for entity %d %d: ", event->event,
                            entityNum, id);
  va_start(argptr, fmt);
  length =
      idStr::vsnPrintf(buf + length, sizeof(buf) - 1 - length, fmt, argptr);
  va_end(argptr);
...
void idGameLocal::ServerProcessReliableMessage(int clientNum,
  ...
  switch (id) {
    ...
    case GAME_RELIABLE_MESSAGE_TCHAT: {
      ...
      mpGame.ProcessChatMessage(clientNum, id == GAME_RELIABLE_MESSAGE_TCHAT,
                                name, text, NULL);
    ...
    case GAME_RELIABLE_MESSAGE_EVENT: {
      ...
      event->paramsSize =
          msg.ReadBits(idMath::BitsForInteger(MAX_EVENT_PARAM_SIZE));
      if (event->paramsSize) {
...
void idGameLocal::ClientShowSnapshot(int clientNum) const {
  ...
  viewBounds = player->GetPhysics()->GetAbsBounds().Expand(
      net_clientShowSnapshotRadius.GetFloat());

  for (ent = snapshotEntities.Next(); ent != NULL;
    ...
    gameRenderWorld->DrawText(
        va("%d: %s (%d,%d bytes of %d,%d)\n", ent->entityNumber,
           ent->name.c_str(), ent->snapshotBits >> 3, ent->snapshotBits & 7,
           baseBits >> 3, baseBits & 7),
...
void idGameLocal::UpdateLagometer(int aheadOfServer, int dupeUsercmds) {
  ...
  for (i = 0; i < LAGO_HEIGHT; i++) {
    memmove((byte*)lagometer + LAGO_WIDTH * 4 * i,
            (byte*)lagometer + LAGO_WIDTH * 4 * i + 4, (LAGO_WIDTH - 1) * 4);
  }
  j = LAGO_WIDTH - 1;
  for (i = 0; i < LAGO_HEIGHT; i++) {
    lagometer[i][j][0] = lagometer[i][j][1] = lagometer[i][j][2] =
        lagometer[i][j][3] = 0;
  }
...
void idGameLocal::ClientReadSnapshot(int clientNum,
  ...
  if (net_clientLagOMeter.GetBool() && renderSystem) {
    ...
    if (!renderSystem->UploadImage(LAGO_IMAGE, (byte*)lagometer, LAGO_IMG_WIDTH,
                                   LAGO_IMG_HEIGHT)) {
      common->Printf(
  ...
  for (i = msg.ReadBits(GENTITYNUM_BITS); i != ENTITYNUM_NONE;
    ...
    entityDefNumber =
        ClientRemapDecl(DECL_ENTITYDEF, deltaMsg.ReadBits(entityDefBits));

    typeInfo = idClass::GetType(typeNum);
    if (!typeInfo) {
      Error("Unknown type number %d for entity %d with class number %d",
            typeNum, i, entityDefNumber);
    }
    ...
    if (!ent || ent->GetType()->typeNum != typeNum ||
        ent->entityDefNumber != entityDefNumber || spawnId != spawnIds[i]) {
      ...
      if (entityDefNumber >= 0) {
        if (entityDefNumber >= declManager->GetNumDecls(DECL_ENTITYDEF)) {
          Error("server has %d entityDefs instead of %d", entityDefNumber,
                declManager->GetNumDecls(DECL_ENTITYDEF));
        }
        classname =
            declManager->DeclByIndex(DECL_ENTITYDEF, entityDefNumber, false)
                ->GetName();
        args.Set("classname", classname);
        if (!SpawnEntityDef(args, &ent) || !entities[i] ||
            entities[i]->GetType()->typeNum != typeNum) {
      ...
      if (entityDefNumber >= 0 &&
        ...
        classname =
            declManager->DeclByIndex(DECL_ENTITYDEF, entityDefNumber, false)
                ->GetName();
        Error(
            "write to and read from snapshot out of sync for classname '%s' of "
            "type '%s'",
            classname, typeInfo->classname);
      } else {
        Error("write to and read from snapshot out of sync for type '%s'",
              typeInfo->classname);
      }
    }
#endif
  ...
  if (memcmp(sourceAreas, serverPVS, idEntity::MAX_PVS_AREAS * sizeof(int))) {
    common->Warning("client PVS areas != server PVS areas, sequence 0x%x",
                    sequence);
    for (i = 0; i < idEntity::MAX_PVS_AREAS; i++) {
  ...
  for (ent = spawnedEntities.Next(); ent != NULL; ent = ent->spawnNode.Next()) {
    ...
    if (!(snapshot->pvs[ent->entityNumber >> 5] &
          (1 << (ent->entityNumber & 31)))) {
      if (ent->PhysicsTeamInPVS(pvsHandle)) {
        if (ent->entityNumber >= MAX_CLIENTS &&
            ent->entityNumber < mapSpawnCount &&
            !ent->spawnArgs.GetBool("net_dynamic", "0")) {  //_D3XP
          ...
          common->DWarning(
              ...
              ent->entityNumber, ent->name.c_str(), sequence);
        } else {
          ent->FreeModelDef();
#ifdef CTF
    ...
    if (!typeInfo || ent->GetType()->typeNum != typeNum ||
        ent->entityDefNumber != entityDefNumber) {
      // should never happen - it does though. with != entityDefNumber only?
      common->DWarning(
          "entity '%s' is not the right type %p 0x%d 0x%x 0x%x 0x%x",
          ent->GetName(), typeInfo, ent->GetType()->typeNum, typeNum,
          ent->entityDefNumber, entityDefNumber);
      continue;
    }
  ...
  if (player->spectating && player->spectator != player->entityNumber &&
      ...
      gameLocal.entities[player->spectator]->IsType(idPlayer::Type)) {
    static_cast<idPlayer*>(gameLocal.entities[player->spectator])
        ->ReadPlayerStateFromSnapshot(deltaMsg);
    weap = static_cast<idPlayer*>(gameLocal.entities[player->spectator])
               ->weapon.GetEntity();
    if (weap && (weap->GetRenderEntity()->bounds[0] ==
                 weap->GetRenderEntity()->bounds[1])) {
      // update the weapon's viewmodel bounds so that the model doesn't flicker
      // in the spectator's view
      weap->GetAnimator()->GetBounds(gameLocal.time,
                                     weap->GetRenderEntity()->bounds);
      weap->UpdateVisuals();
    }
  } else {
...
void idGameLocal::ClientProcessReliableMessage(int clientNum,
  ...
  switch (id) {
    ...
    case GAME_RELIABLE_MESSAGE_SPAWN_PLAYER: {
      ...
      // otherwise there is going to be a bogus delete/new of the client entity
      // in the first ClientReadFromSnapshot
    ...
    case GAME_RELIABLE_MESSAGE_DB: {
      idMultiplayerGame::msg_evt_t msg_evt =
          (idMultiplayerGame::msg_evt_t)msg.ReadByte();
      int parm1, parm2;
      parm1 = msg.ReadByte();
      parm2 = msg.ReadByte();
    ...
    case GAME_RELIABLE_MESSAGE_UPDATEVOTE: {
      ...
      mpGame.ClientUpdateVote((idMultiplayerGame::vote_result_t)result,
                              yesCount, noCount);
      break;
    }
...
gameReturn_t idGameLocal::ClientPrediction(int clientNum,
  ...
  if (networkSystem->ClientGetTimeSinceLastPacket() >=
      net_clientMaxPrediction.GetInteger()) {
...
bool idGameLocal::DownloadRequest(const char* IP,
  ...
  if (cvarSystem->GetCVarInteger("net_serverDownload") == 1) {
    ...
    idStr::snPrintf(urls, MAX_STRING_CHARS, "1;%s",
                    cvarSystem->GetCVarString("si_serverURL"));
    return true;
  } else {
    // 2: table of pak URLs
...
void idEventQueue::Enqueue(entityNetEvent_t* event,
                           outOfOrderBehaviour_t behaviour) {
  if (behaviour == OUTOFORDER_DROP) {
    ...
    while (end && end->time > event->time) {
      ...
      common->DPrintf(
          "WARNING: new event with id %d ( time %d ) caused removal of event "
          "with id %d ( time %d ), game time = %d.\n",
          event->event, event->time, outOfOrder->event, outOfOrder->time,
          gameLocal.time);
      Free(outOfOrder);
    }

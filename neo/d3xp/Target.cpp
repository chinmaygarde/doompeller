...
void idTarget_LightFadeIn::Event_Activate(idEntity* activator) {
  ...
  for (i = 0; i < targets.Num(); i++) {
    ...
    if (ent->IsType(idLight::Type)) {
      ...
      gameLocal.Printf("'%s' targets non-light '%s'", name.c_str(),
                       ent->GetName());
    }
  }
}
  ...
  if (player) {
    ...
    while (kv) {
      ...
      if (dict) {
        ...
        idEntity* ent = NULL;
        if (gameLocal.SpawnEntityDef(d2, &ent) && ent &&
...
void idTarget_GiveEmail::Event_Activate(idEntity* activator) {
  ...
  if (pda) {
    ...
    player->ShowTip(spawnArgs.GetString("text_infoTitle"),
                    spawnArgs.GetString("text_PDANeeded"), true);
  }
}
...
void idTarget_SetInfluence::Event_Flash(float flash, int out) {
  ...
  if (!out && flashInSound.Length()) {
    ...
    shader = declManager->FindSound(flashOutSound.Length() ? flashOutSound
                                                           : flashInSound);
    player->StartSoundShader(shader, SND_CHANNEL_VOICE, 0, false, NULL);
  }
...
void idTarget_SetInfluence::Event_GatherEntities() {
  ...
  if (targetsOnly) {
    ...
    listedEntities = gameLocal.EntitiesWithinRadius(
        GetPhysics()->GetOrigin(), radius, entityList, MAX_GENTITIES);
  }

  for (i = 0; i < listedEntities; i++) {
    idEntity* ent = entityList[i];
    if (ent) {
      if (lights && ent->IsType(idLight::Type) &&
          ent->spawnArgs.FindKey("color_demonic")) {
        lightList.Append(ent->entityNumber);
        continue;
      }
      if (sounds && ent->IsType(idSound::Type) &&
          ent->spawnArgs.FindKey("snd_demonic")) {
...
void idTarget_SetInfluence::Event_Activate(idEntity* activator) {
  ...
  if (fov) {
    fovSetting.Init(gameLocal.time, SEC2MS(spawnArgs.GetFloat("fovTime")),
  ...
  for (i = 0; i < guiList.Num(); i++) {
    ...
    for (j = 0; j < MAX_RENDERENTITY_GUI; j++) {
      if (ent->GetRenderEntity()->gui[j] &&
          ent->spawnArgs.FindKey(j == 0 ? "gui_demonic"
                                        : va("gui_demonic%d", j + 1))) {
#ifdef _D3XP
        // Backup the old one
        savedGuiList[i].gui[j] = ent->GetRenderEntity()->gui[j];
#endif
        ent->GetRenderEntity()->gui[j] = uiManager->FindGui(
            ent->spawnArgs.GetString(j == 0 ? "gui_demonic"
                                            : va("gui_demonic%d", j + 1)),
            true);
        update = true;
      }
  ...
  if (spawnArgs.GetBool("effect_vision")) {
    ...
    player->SetInfluenceView(parm, skin, spawnArgs.GetInt("visionRadius"),
                             this);
  }
...
void idTarget_SetInfluence::Event_RestoreInfluence() {
  ...
  for (i = 0; i < lightList.Num(); i++) {
    ...
    if (!light->spawnArgs.GetBool("leave_demonic_mat")) {
      const char* texture =
          light->spawnArgs.GetString("texture", "lights/squarelight1");
...
#else
        ent->GetRenderEntity()->gui[j] = uiManager->FindGui(
            ent->spawnArgs.GetString(j == 0 ? "gui" : va("gui%d", j + 1)));
#endif
        update = true;
      }
    }
...
void idTarget_SetFov::Event_Activate(idEntity* activator) {
  ...
  fovSetting.Init(gameLocal.time, SEC2MS(spawnArgs.GetFloat("time")),
                  player ? player->DefaultFov() : g_fov.GetFloat(),
                  spawnArgs.GetFloat("fov"));
  BecomeActive(TH_THINK);
}
...
void idTarget_SetPrimaryObjective::Event_Activate(idEntity* activator) {
  ...
  if (player && player->objectiveSystem) {
    player->objectiveSystem->SetStateString(
        "missionobjective",
        spawnArgs.GetString(
            "text", common->GetLanguageDict()->GetString("#str_04253")));
  }
}
...
void idTarget_CallObjectFunction::Event_Activate(idEntity* activator) {
  ...
  for (i = 0; i < targets.Num(); i++) {
    ...
    if (ent && ent->scriptObject.HasObject()) {
      ...
      if (!func) {
        gameLocal.Error(
            "Function '%s' not found on entity '%s' for function call from "
            "'%s'",
            funcName, ent->name.c_str(), name.c_str());
      }
      if (func->type->NumParameters() != 1) {
        gameLocal.Error(
            "Function '%s' on entity '%s' has the wrong number of parameters "
            "for function call from '%s'",
            funcName, ent->name.c_str(), name.c_str());
      }
      if (!ent->scriptObject.GetTypeDef()->Inherits(
              func->type->GetParmType(0))) {
        gameLocal.Error(
            "Function '%s' on entity '%s' is the wrong type for function call "
            "from '%s'",
            funcName, ent->name.c_str(), name.c_str());
      }
...
void idTarget_EnableLevelWeapons::Event_Activate(idEntity* activator) {
  ...
  gameLocal.world->spawnArgs.SetBool("no_Weapons",
                                     spawnArgs.GetBool("disable"));
  ...
  if (spawnArgs.GetBool("disable")) {
    ...
    for (i = 0; i < gameLocal.numClients; i++) {
      if (gameLocal.entities[i]) {
        ...
        if (weap && weap[0]) {
          gameLocal.entities[i]->PostEventSec(&EV_Player_SelectWeapon, 0.5f,
                                              weap);
        }
      }
    }
...
void idTarget_Tip::Event_Activate(idEntity* activator) {
  ...
  if (player) {
    ...
    player->ShowTip(spawnArgs.GetString("text_title"),
                    spawnArgs.GetString("text_tip"), false);
...
void idTarget_LevelTrigger::Event_Activate(idEntity* activator) {
  for (int i = 0; i < gameLocal.numClients; i++) {
    if (gameLocal.entities[i]) {
      idPlayer* player = static_cast<idPlayer*>(gameLocal.entities[i]);
      player->SetLevelTrigger(spawnArgs.GetString("levelName"),
                              spawnArgs.GetString("triggerName"));
    }
  }
}
...
void idTarget_FadeSoundClass::Event_Activate(idEntity* activator) {
  ...
  // start any sound fading now
  if (fadeTime) {
    gameSoundWorld->FadeSoundClasses(
        fadeClass, spawnArgs.GetBool("fadeIn") ? fadeDB : 0.0f - fadeDB,
        fadeTime);
    if (fadeDuration) {

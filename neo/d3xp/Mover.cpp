...
void idMover::Save(idSaveGame* savefile) const {
  ...
  if (splineEnt.GetEntity() && splineEnt.GetEntity()->GetSpline()) {
    ...
    savefile->WriteInt(spline->GetTime(spline->GetNumValues() - 1) -
                       spline->GetTime(0));
...
void idMover::Restore(idRestoreGame* savefile) {
  ...
  if (hasSpline) {
    ...
    PostEventMS(&EV_PostRestore, 0, starttime, totaltime, accel, decel,
                useAngles);
  }
}
...
void idMover::Event_PostRestore(int start,
  ...
  physicsObj.SetLinearExtrapolation(EXTRAPOLATION_NONE, 0, 0, dest_position,
...
void idMover::Spawn(void) {
  ...
  move_time =
      1000.0f * spawnArgs.GetFloat("move_time", "1");  // safe default value
  ...
  physicsObj.SetLinearExtrapolation(EXTRAPOLATION_NONE, 0, 0, dest_position,
                                    vec3_origin, vec3_origin);
  physicsObj.SetAngularExtrapolation(EXTRAPOLATION_NONE, 0, 0, dest_angles,
                                     ang_zero, ang_zero);
...
void idMover::Event_SetCallback(void) {
  ...
  } else if ((lastCommand == MOVER_MOVING || lastCommand == MOVER_SPLINE) &&
             !move_thread) {
    lastCommand = MOVER_NONE;
    ...
    if (ent) {
      for (int j = 0; j < MAX_RENDERENTITY_GUI; j++) {
        if (ent->GetRenderEntity() && ent->GetRenderEntity()->gui[j]) {
          ...
          ent->GetRenderEntity()->gui[j]->StateChanged(gameLocal.slow.time,
                                                       true);
        }
      }
      ent->UpdateVisuals();
...
void idMover::Event_UpdateMove(void) {
  ...
  switch (move.stage) {
    case ACCELERATION_STAGE: {
      physicsObj.SetLinearExtrapolation(EXTRAPOLATION_ACCELLINEAR,
                                        gameLocal.slow.time, move.acceleration,
                                        org, move.dir, vec3_origin);
      if (move.movetime > 0) {
    ...
    case LINEAR_STAGE: {
      physicsObj.SetLinearExtrapolation(EXTRAPOLATION_LINEAR,
                                        gameLocal.slow.time, move.movetime, org,
                                        move.dir, vec3_origin);
    ...
    case DECELERATION_STAGE: {
      physicsObj.SetLinearExtrapolation(EXTRAPOLATION_DECELLINEAR,
                                        gameLocal.slow.time, move.deceleration,
                                        org, move.dir, vec3_origin);
      move.stage = FINISHED_STAGE;
      break;
    }
...
void idMover::Event_UpdateRotation(void) {
  ...
  switch (rot.stage) {
    case ACCELERATION_STAGE: {
      physicsObj.SetAngularExtrapolation(EXTRAPOLATION_ACCELLINEAR,
                                         gameLocal.slow.time, rot.acceleration,
                                         ang, rot.rot, ang_zero);
      if (rot.movetime > 0) {
    ...
    case LINEAR_STAGE: {
      if (!stopRotation && !rot.deceleration) {
        physicsObj.SetAngularExtrapolation(
            ...
            gameLocal.slow.time, rot.movetime, ang, rot.rot, ang_zero);
      } else {
        physicsObj.SetAngularExtrapolation(EXTRAPOLATION_LINEAR,
                                           gameLocal.slow.time, rot.movetime,
                                           ang, rot.rot, ang_zero);
      }
    ...
    case DECELERATION_STAGE: {
      physicsObj.SetAngularExtrapolation(EXTRAPOLATION_DECELLINEAR,
                                         gameLocal.slow.time, rot.deceleration,
                                         ang, rot.rot, ang_zero);
      rot.stage = FINISHED_STAGE;
      break;
    }
    case FINISHED_STAGE: {
      ...
      if (stopRotation) {
        ...
        physicsObj.SetAngularExtrapolation(EXTRAPOLATION_NONE, 0, 0,
                                           dest_angles, ang_zero, ang_zero);
        stopRotation = false;
      } else if (physicsObj.GetAngularExtrapolationType() ==
                 EXTRAPOLATION_ACCELLINEAR) {
        // keep our angular velocity constant
        physicsObj.SetAngularExtrapolation(
            extrapolation_t(EXTRAPOLATION_LINEAR | EXTRAPOLATION_NOSTOP),
            gameLocal.slow.time, 0, ang, rot.rot, ang_zero);
      }
...
===============
*/
void idMover::Event_TeamBlocked(idEntity* blockedEntity,
                                idEntity* blockingEntity) {
  if (g_debugMover.GetBool()) {
    gameLocal.Printf(
        "%d: '%s' stopped due to team member '%s' blocked by '%s'\n",
        gameLocal.slow.time, name.c_str(), blockedEntity->name.c_str(),
        blockingEntity->name.c_str());
  }
}
...
void idMover::Event_PartBlocked(idEntity* blockingEntity) {
  if (damage > 0.0f) {
    blockingEntity->Damage(this, this, vec3_origin, "damage_moverCrush", damage,
                           INVALID_JOINT);
  }
  if (g_debugMover.GetBool()) {
    gameLocal.Printf("%d: '%s' blocked by '%s'\n", gameLocal.slow.time,
                     name.c_str(), blockingEntity->name.c_str());
  }
}
...
void idMover::Event_SetMoveSpeed(float speed) {
  ...
  move_speed = speed;
  move_time =
      0;  // move_time is calculated for each move when move_speed is non-0
}
...
void idMover::Event_MoveAccelerateTo(float speed, float time) {
  ...
  if (time < 0) {
    gameLocal.Error(
        "idMover::Event_MoveAccelerateTo: cannot set acceleration time less "
        "than 0.");
  }
  ...
  StartSound("snd_move", SND_CHANNEL_BODY, 0, false, NULL);
  physicsObj.SetLinearExtrapolation(EXTRAPOLATION_ACCELLINEAR,
                                    gameLocal.slow.time, move.acceleration, org,
                                    dir * (speed - v), dir * v);
}
...
void idMover::Event_MoveDecelerateTo(float speed, float time) {
  ...
  if (time < 0) {
    gameLocal.Error(
        "idMover::Event_MoveDecelerateTo: cannot set deceleration time less "
        "than 0.");
  }
  ...
  StartSound("snd_move", SND_CHANNEL_BODY, 0, false, NULL);
  physicsObj.SetLinearExtrapolation(EXTRAPOLATION_DECELLINEAR,
                                    gameLocal.slow.time, move.deceleration, org,
                                    dir * (v - speed), dir * speed);
}
...
void idMover::Event_Rotate(idAngles& angles) {
  ...
  physicsObj.GetLocalAngles(ang);
  dest_angles =
      ang + angles * (move_time - (acceltime + deceltime) / 2) * 0.001f;
...
void idMover::Event_Bob(float speed, float phase, idVec3& depth) {
  ...
  physicsObj.SetLinearExtrapolation(
      extrapolation_t(EXTRAPOLATION_DECELSINE | EXTRAPOLATION_NOSTOP),
      speed * 1000 * phase, speed * 500, org, depth * 2.0f, vec3_origin);
}
...
void idMover::Event_Sway(float speed, float phase, idAngles& depth) {
  ...
  physicsObj.GetLocalAngles(ang);
  assert(speed > 0.0f);
  duration = idMath::Sqrt(depth[0] * depth[0] + depth[1] * depth[1] +
                          depth[2] * depth[2]) /
             speed;
  angSpeed = depth / (duration * idMath::SQRT_1OVER2);
  physicsObj.SetAngularExtrapolation(
      extrapolation_t(EXTRAPOLATION_DECELSINE | EXTRAPOLATION_NOSTOP),
      duration * 1000.0f * phase, duration * 1000.0f, ang, angSpeed, ang_zero);
}
...
void idMover::Event_RemoveInitialSplineAngles(void) {
  ...
  physicsObj.SetAngularExtrapolation(EXTRAPOLATION_NONE, 0, 0, -ang, ang_zero,
                                     ang_zero);
}
...
void idMover::Event_StartSpline(idEntity* splineEntity) {
  ...
  physicsObj.SetSpline(spline, move.acceleration, move.deceleration,
                       useSplineAngles);
  physicsObj.SetLinearExtrapolation(EXTRAPOLATION_NONE, 0, 0, dest_position,
                                    vec3_origin, vec3_origin);
}
...
void idElevator::Spawn(void) {
  ...
  SetGuiStates((pendingFloor == 1) ? guiBinaryMoverStates[0]
                                   : guiBinaryMoverStates[1]);
...
void idElevator::BeginMove(idThread* thread) {
  ...
  SetGuiStates((pendingFloor == 1) ? guiBinaryMoverStates[3]
                                   : guiBinaryMoverStates[2]);
...
void idElevator::Event_PostFloorArrival() {
  OpenFloorDoor(currentFloor);
  OpenInnerDoor();
  SetGuiStates((currentFloor == 1) ? guiBinaryMoverStates[0]
                                   : guiBinaryMoverStates[1]);
  controlsDisabled = false;
...
void idElevator::DoneMoving(void) {
  ...
  while (kv) {
    ...
    if (ent) {
      for (int j = 0; j < MAX_RENDERENTITY_GUI; j++) {
        if (ent->GetRenderEntity() && ent->GetRenderEntity()->gui[j]) {
          ent->GetRenderEntity()->gui[j]->SetStateString(
              "floor", va("%i", currentFloor));
          ent->GetRenderEntity()->gui[j]->StateChanged(gameLocal.slow.time,
                                                       true);
        }
      }
...
void idMover_Binary::Spawn(void) {
  ...
  if (!team.Length()) {
    ...
    for (ent = gameLocal.spawnedEntities.Next(); ent != NULL;
      ...
      if (ent->IsType(idMover_Binary::Type) &&
          !idStr::Icmp(static_cast<idMover_Binary*>(ent)->team.c_str(), temp)) {
        break;
      }
    }
    if (!ent) {
  ...
  physicsObj.SetLinearExtrapolation(EXTRAPOLATION_NONE, 0, 0,
                                    GetPhysics()->GetOrigin(), vec3_origin,
                                    vec3_origin);
  physicsObj.SetAngularExtrapolation(EXTRAPOLATION_NONE, 0, 0,
...
void idMover_Binary::SetMoverState(moverState_t newstate, int time) {
  ...
  switch (moverState) {
    case MOVER_POS1: {
      ...
      physicsObj.SetLinearExtrapolation(EXTRAPOLATION_NONE, time, 0, pos1,
                                        vec3_origin, vec3_origin);
      break;
    }
    case MOVER_POS2: {
      Signal(SIG_MOVER_POS2);
      physicsObj.SetLinearExtrapolation(EXTRAPOLATION_NONE, time, 0, pos2,
                                        vec3_origin, vec3_origin);
      break;
    }
    case MOVER_1TO2: {
      ...
      physicsObj.SetLinearExtrapolation(
          EXTRAPOLATION_LINEAR, time, duration, pos1,
          (pos2 - pos1) * 1000.0f / duration, vec3_origin);
      if (accelTime != 0 || decelTime != 0) {
        physicsObj.SetLinearInterpolation(time, accelTime, decelTime, duration,
                                          pos1, pos2);
      } else {
    ...
    case MOVER_2TO1: {
      ...
      physicsObj.SetLinearExtrapolation(
          EXTRAPOLATION_LINEAR, time, duration, pos2,
          (pos1 - pos2) * 1000.0f / duration, vec3_origin);
      if (accelTime != 0 || decelTime != 0) {
        physicsObj.SetLinearInterpolation(time, accelTime, decelTime, duration,
                                          pos2, pos1);
      } else {
...
#ifdef _D3XP
    if (slave->playerOnly) {
      gameLocal.SetAASAreaState(slave->GetPhysics()->GetAbsBounds(),
                                AREACONTENTS_CLUSTERPORTAL, false);
    }
#endif
  }
}
...
void idMover_Binary::Event_ClosePortal(void) {
  ...
  for (slave = moveMaster; slave != NULL; slave = slave->activateChain) {
    if (!slave->IsHidden()) {
      ...
      if (slave->playerOnly) {
        gameLocal.SetAASAreaState(slave->GetPhysics()->GetAbsBounds(),
                                  AREACONTENTS_CLUSTERPORTAL, true);
      }
#endif
    }
  }
...
void idMover_Binary::Use_BinaryMover(idEntity* activator) {
  ...
  if (moverState == MOVER_POS2) {
    ...
    for (slave = this; slave != NULL; slave = slave->activateChain) {
      slave->CancelEvents(&EV_Mover_ReturnToPos1);
      slave->PostEventSec(&EV_Mover_ReturnToPos1,
                          spawnArgs.GetBool("toggle") ? 0 : wait);
    }
    return;
  }
...
void idMover_Binary::InitSpeed(idVec3& mpos1,
  ...
  physicsObj.SetLinearExtrapolation(EXTRAPOLATION_NONE, 0, 0, pos1, vec3_origin,
                                    vec3_origin);
...
void idMover_Binary::SetBlocked(bool b) {
  for (idMover_Binary* slave = moveMaster; slave != NULL;
...
void idDoor::Think(void) {
  ...
  if (thinkFlags & TH_PHYSICS) {
    ...
    if (GetMasterPosition(masterOrigin, masterAxis)) {
      if (trigger) {
        trigger->Link(gameLocal.clip, this, 0,
                      masterOrigin + localTriggerOrigin * masterAxis,
                      localTriggerAxis * masterAxis);
      }
      if (sndTrigger) {
        sndTrigger->Link(gameLocal.clip, this, 0,
                         masterOrigin + localTriggerOrigin * masterAxis,
                         localTriggerAxis * masterAxis);
      }
    }
...
void idDoor::SetAASAreaState(bool closed) {
  ...
  gameLocal.SetAASAreaState(physicsObj.GetAbsBounds(),
                            AREACONTENTS_CLUSTERPORTAL | AREACONTENTS_OBSTACLE,
                            closed);
}
  ...
  if (this != master) {
    ...
    for (slave = this; slave != NULL; slave = slave->GetActivateChain()) {
      if (slave->IsType(idDoor::Type)) {
        ...
        if (companion && (companion != master) &&
            (companion->GetMoveMaster() != master)) {
          companion->Hide();
        }
...
void idDoor::CalcTriggerBounds(float size, idBounds& bounds) {
  ...
  fl.takedamage = true;
  for (other = activateChain; other != NULL;
...
void idDoor::Event_SpawnDoorTrigger(void) {
  ...
  trigger->Link(gameLocal.clip, this, 255, GetPhysics()->GetOrigin(),
                mat3_identity);
...
void idDoor::Event_SpawnSoundTrigger(void) {
  ...
  sndTrigger->Link(gameLocal.clip, this, 254, GetPhysics()->GetOrigin(),
                   mat3_identity);
...
================
*/
void idDoor::Event_TeamBlocked(idEntity* blockedEntity,
                               idEntity* blockingEntity) {
  SetBlocked(true);
...
void idDoor::Event_Touch(idEntity* other, trace_t* trace) {
  ...
  if (trigger && trace->c.id == trigger->GetId()) {
    ...
    if (other && other->IsType(idPlayer::Type) && IsLocked() &&
        gameLocal.slow.time > nextSndTriggerTime) {
...
void idDoor::Event_SpectatorTouch(idEntity* other, trace_t* trace) {
  ...
  assert(other && other->IsType(idPlayer::Type) &&
         static_cast<idPlayer*>(other)->spectating);
...
void idPlat::Spawn(void) {
  ...
  if (!spawnArgs.GetFloat("height", "0", height)) {
    height =
        (GetPhysics()->GetBounds()[1][2] - GetPhysics()->GetBounds()[0][2]) -
        lip;
  }
...
void idPlat::Event_Touch(idEntity* other, trace_t* trace) {
  ...
  if ((GetMoverState() == MOVER_POS1) && trigger &&
      (trace->c.id == trigger->GetId()) && (other->health > 0)) {
...
================
*/
void idPlat::Event_TeamBlocked(idEntity* blockedEntity,
                               idEntity* blockingEntity) {
  // reverse direction
...
===============
*/
void idMover_Periodic::Event_TeamBlocked(idEntity* blockedEntity,
                                         idEntity* blockingEntity) {}
...
void idRotater::Spawn(void) {
  ...
  physicsObj.SetLinearExtrapolation(EXTRAPOLATION_NONE, gameLocal.slow.time, 0,
                                    GetPhysics()->GetOrigin(), vec3_origin,
                                    vec3_origin);
  physicsObj.SetAngularExtrapolation(
      extrapolation_t(EXTRAPOLATION_LINEAR | EXTRAPOLATION_NOSTOP),
      gameLocal.slow.time, 0, GetPhysics()->GetAxis().ToAngles(), ang_zero,
      ang_zero);
  SetPhysics(&physicsObj);
...
void idRotater::Event_Activate(idEntity* activator) {
  ...
  physicsObj.SetAngularExtrapolation(
      extrapolation_t(EXTRAPOLATION_LINEAR | EXTRAPOLATION_NOSTOP),
      gameLocal.slow.time, 0, physicsObj.GetAxis().ToAngles(), delta, ang_zero);
}
...
void idBobber::Spawn(void) {
  ...
  physicsObj.SetLinearExtrapolation(
      extrapolation_t(EXTRAPOLATION_DECELSINE | EXTRAPOLATION_NOSTOP),
      phase * 1000, speed * 500, GetPhysics()->GetOrigin(), delta * 2.0f,
      vec3_origin);
  SetPhysics(&physicsObj);
}
...
void idPendulum::Spawn(void) {
  ...
  if (spawnArgs.GetFloat("freq", "", freq)) {
    ...
    freq =
        1 / (idMath::TWO_PI)*idMath::Sqrt(g_gravity.GetFloat() / (3 * length));
  }
  ...
  physicsObj.SetAngularExtrapolation(
      extrapolation_t(EXTRAPOLATION_DECELSINE | EXTRAPOLATION_NOSTOP),
      phase * 1000, 500 / freq, GetPhysics()->GetAxis().ToAngles(),
      idAngles(0, 0, speed * 2.0f), ang_zero);
  SetPhysics(&physicsObj);
}
...
void idRiser::Event_Activate(idEntity* activator) {
  if (!IsHidden() && spawnArgs.GetBool("hide")) {
    ...
    physicsObj.SetLinearExtrapolation(EXTRAPOLATION_LINEAR, gameLocal.slow.time,
                                      time * 1000, physicsObj.GetOrigin(),
                                      delta, vec3_origin);
  }
}

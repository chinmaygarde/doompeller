...
typedef struct pvsPassage_s {
  byte*
      canSee;  // bit set for all portals that can be seen through this passage
} pvsPassage_t;

typedef struct pvsPortal_s {
  int areaNum;      // area this portal leads to
  idWinding* w;     // winding goes counter clockwise seen from the area this
                    // portal is part of
  idBounds bounds;  // winding bounds
  idPlane plane;    // winding plane, normal points towards the area this portal
                    // leads to
  pvsPassage_t*
      passages;    // passages to portals in the area this portal leads to
...
void idPVS::CreatePVSData(void) {
  ...
  for (i = 0; i < numAreas; i++) {
    ...
    for (j = 0; j < n; j++) {
      ...
      p->w = portal.w->Copy();
      p->areaNum =
          portal.areas[1];  // area[1] is always the area the portal leads to
      ...
      for (p = 0; p < area->numPortals; p++) {
        ...
        if (side1 != PLANESIDE_FRONT) {
          ...
          for (k = 0; k < p2->w->GetNumPoints(); k++) {
            ...
            if (p1->plane.Side((*p2->w)[k].ToVec3(), ON_EPSILON) ==
                PLANESIDE_FRONT) {
              break;
            }
          }
        ...
        if (side2 != PLANESIDE_BACK) {
          ...
          for (k = 0; k < p1->w->GetNumPoints(); k++) {
            ...
            if (p2->plane.Side((*p1->w)[k].ToVec3(), ON_EPSILON) ==
                PLANESIDE_BACK) {
              break;
            }
          }
...
===============
*/
pvsStack_t* idPVS::FloodPassagePVS_r(pvsPortal_t* source,
                                     const pvsPortal_t* portal,
                                     pvsStack_t* prevStack) const {
  int i, j, n, m;
  pvsPortal_t* p;
  pvsArea_t* area;
  pvsStack_t* stack;
  ...
  if (!stack) {
    stack = reinterpret_cast<pvsStack_t*>(
        new byte[sizeof(pvsStack_t) + portalVisBytes]);
...
void idPVS::CreatePassages(void) const {
  ...
  for (i = 0; i < numPortals; i++) {
    ...
    for (j = 0; j < area->numPortals; j++) {
      ...
      numBounds = 0;
      AddPassageBoundaries(*(source->w), *(target->w), false, passageBounds,
                           numBounds, MAX_PASSAGE_BOUNDS);
      AddPassageBoundaries(*(target->w), *(source->w), true, passageBounds,
                           numBounds, MAX_PASSAGE_BOUNDS);
...
void idPVS::GetConnectedAreas(int srcArea, bool* areas) const {
  ...
  for (curArea = srcArea; queueStart < queueEnd;
...
pvsHandle_t idPVS::SetupCurrentPVS(const idBounds& source,
  ...
  numSourceAreas =
      gameRenderWorld->BoundsInAreas(source, sourceAreas, MAX_BOUNDS_AREAS);
...
pvsHandle_t idPVS::SetupCurrentPVS(const int sourceArea,
  ...
  if (type != PVS_CONNECTED_AREAS) {
    memcpy(currentPVS[handle.i].pvs, areaPVS + sourceArea * areaVisBytes,
           areaVisBytes);
  } else {
...
pvsHandle_t idPVS::SetupCurrentPVS(const int* sourceAreas,
  ...
  if (type != PVS_CONNECTED_AREAS) {
    ...
    memcpy(currentPVS[handle.i].pvs, areaPVS + sourceAreas[0] * areaVisBytes,
           areaVisBytes);
...
pvsHandle_t idPVS::MergeCurrentPVS(pvsHandle_t pvs1, pvsHandle_t pvs2) const {
  ...
  if (pvs1.i < 0 || pvs1.i >= MAX_CURRENT_PVS ||
      pvs1.h != currentPVS[pvs1.i].handle.h || pvs2.i < 0 ||
      pvs2.i >= MAX_CURRENT_PVS || pvs2.h != currentPVS[pvs2.i].handle.h) {
...
bool idPVS::InCurrentPVS(const pvsHandle_t handle, const idVec3& target) const {
  ...
  return ((currentPVS[handle.i].pvs[targetArea >> 3] &
           (1 << (targetArea & 7))) != 0);
}
...
bool idPVS::InCurrentPVS(const pvsHandle_t handle,
  ...
  numTargetAreas =
      gameRenderWorld->BoundsInAreas(target, targetAreas, MAX_BOUNDS_AREAS);

  for (i = 0; i < numTargetAreas; i++) {
    if (currentPVS[handle.i].pvs[targetAreas[i] >> 3] &
        (1 << (targetAreas[i] & 7))) {
      return true;
    }
  }
    ...
    for (i = 0; i < n; i++) {
      ...
      for (k = 0; k < numPoints; k++) {
        gameRenderWorld->DebugLine(
            *color, (*portal.w)[k].ToVec3() + offset,
            (*portal.w)[(k + 1) % numPoints].ToVec3() + offset);
      }
    }
  }
...
void idPVS::ReadPVS(const pvsHandle_t handle, const idBitMsg& msg) {
  ...
  if (memcmp(l_pvs, currentPVS[handle.i].pvs, areaVisBytes)) {
    common->Printf(
        "PVS not matching ( %d areaVisBytes ) - server then client:\n",
        areaVisBytes);
...
================
*/
bool idPVS::CheckAreasForPortalSky(const pvsHandle_t handle,
                                   const idVec3& origin) {
  int j, sourceArea;

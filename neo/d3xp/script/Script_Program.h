...
ID_INLINE void idScriptVariable<type, etype, returnType>::LinkTo(
  ...
  if (!data) {
    gameError("Missing '%s' field in script object '%s'", name,
              obj.GetTypeName());
  }
}
...
extern idTypeDef
    type_argsize;  // only used for function call and thread opcodes
...
class idProgram {
 ...
 public:
  ...
  idTypeDef* AllocType(etype_t etype,
                       idVarDef* edef,
                       const char* ename,
                       int esize,
                       idTypeDef* aux);
  ...
  idVarDef* AllocDef(idTypeDef* type,
                     const char* name,
                     idVarDef* scope,
                     bool constant);
  idVarDef* GetDef(const idTypeDef* type,
                   const char* name,
                   const idVarDef* scope) const;
  void FreeDef(idVarDef* d, const idVarDef* scope);
  idVarDef* FindFreeResultDef(idTypeDef* type,
                              const char* name,
                              idVarDef* scope,
                              const idVarDef* a,
                              const idVarDef* b);
  ...
  function_t* FindFunction(
      const char* name) const;  // returns NULL if function not found
  function_t* FindFunction(const char* name, const idTypeDef* type)
      const;  // returns NULL if function not found

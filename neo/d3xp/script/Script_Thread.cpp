...
#ifdef _D3XP
const idEventDef EV_Thread_VecToOrthoBasisAngles("VecToOrthoBasisAngles",
                                                 "v",
                                                 'v');
...
void idThread::DisplayInfo(void) {
  gameLocal.Printf(
      ...
      "     Created: %d (%d ms ago)\n"
      "      Status: ",
      threadNum, threadName.c_str(), interpreter.CurrentFile(),
      interpreter.CurrentLine(), creationTime, gameLocal.time - creationTime);
  ...
  if (interpreter.threadDying) {
    ...
    } else if ((waitingFor != ENTITYNUM_NONE) &&
               (gameLocal.entities[waitingFor])) {
      gameLocal.Printf("Waiting for entity #%3i '%s'\n", waitingFor,
                       gameLocal.entities[waitingFor]->name.c_str());
...
NOTE: If this is called from within a event called by this thread, the function
arguments will be invalid after calling this function.
================
*/
...
================
*/
void idThread::CallFunction(idEntity* self,
                            const function_t* func,
                            bool clearStack) {
  assert(self);
  ClearWaitFor();
...
void idThread::Event_SpawnVector(const char* key, idVec3& defaultvalue) {
  ...
  spawnArgs.GetVector(
      key, va("%f %f %f", defaultvalue.x, defaultvalue.y, defaultvalue.z),
      result);
  ReturnVector(result);
}
...
void idThread::Event_Trace(const idVec3& start,
  ...
  if (mins == vec3_origin && maxs == vec3_origin) {
    ...
    gameLocal.clip.TraceBounds(trace, start, end, idBounds(mins, maxs),
                               contents_mask, passEntity);
  }
...
void idThread::Event_GetTraceJoint(void) {
  if (trace.fraction < 1.0f && trace.c.id < 0) {
    idAFEntity_Base* af =
        static_cast<idAFEntity_Base*>(gameLocal.entities[trace.c.entityNum]);
    if (af && af->IsType(idAFEntity_Base::Type) && af->IsActiveAF()) {
      ReturnString(af->GetAnimator()->GetJointName(
          CLIPMODEL_ID_TO_JOINT_HANDLE(trace.c.id)));
      return;
    }
  }
  ReturnString("");
...
void idThread::Event_RadiusDamage(const idVec3& origin,
                                  ...
                                  float dmgPower) {
  gameLocal.RadiusDamage(origin, inflictor, attacker, ignore, ignore,
                         damageDefName, dmgPower);
}
...
void idThread::Event_DebugLine(const idVec3& color,
  ...
  gameRenderWorld->DebugLine(idVec4(color.x, color.y, color.z, 0.0f), start,
                             end, SEC2MS(lifetime));
}
...
void idThread::Event_DebugArrow(const idVec3& color,
                                ...
                                const int size,
                                const float lifetime) {
  gameRenderWorld->DebugArrow(idVec4(color.x, color.y, color.z, 0.0f), start,
                              end, size, SEC2MS(lifetime));
}
...
void idThread::Event_DebugCircle(const idVec3& color,
                                 ...
                                 const int numSteps,
                                 const float lifetime) {
  gameRenderWorld->DebugCircle(idVec4(color.x, color.y, color.z, 0.0f), origin,
                               dir, radius, numSteps, SEC2MS(lifetime));
}
...
void idThread::Event_DebugBounds(const idVec3& color,
                                 ...
                                 const idVec3& maxs,
                                 const float lifetime) {
  gameRenderWorld->DebugBounds(idVec4(color.x, color.y, color.z, 0.0f),
                               idBounds(mins, maxs), vec3_origin,
                               SEC2MS(lifetime));
}
...
void idThread::Event_DrawText(const char* text,
  ...
  gameRenderWorld->DrawText(
      text, origin, scale, idVec4(color.x, color.y, color.z, 0.0f),

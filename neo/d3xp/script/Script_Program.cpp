...
idTypeDef type_argsize(ev_argsize,
                       ...
                       "<argsize>",
                       sizeof(int),
                       NULL);  // only used for function call and thread opcodes
...
size_t idTypeDef::Allocated(void) const {
  ...
  memsize = name.Allocated() + parmTypes.Allocated() + parmNames.Allocated() +
            functions.Allocated();
...
void idTypeDef::AddFunctionParm(idTypeDef* parmtype, const char* name) {
  if (type != ev_function) {
    throw idCompileError(
        "idTypeDef::AddFunctionParm : tried to add parameter on non-function "
        "type");
  }
...
void idTypeDef::SetReturnType(idTypeDef* returntype) {
  if (type != ev_function) {
    throw idCompileError(
        "idTypeDef::SetReturnType: tried to set return type on non-function "
        "type");
  }
...
void idTypeDef::SetFieldType(idTypeDef* fieldtype) {
  if (type != ev_field) {
    throw idCompileError(
        "idTypeDef::SetFieldType: tried to set return type on non-function "
        "type");
  }
...
void idTypeDef::AddFunction(const function_t* func) {
  ...
  for (i = 0; i < functions.Num(); i++) {
    if (!strcmp(functions[i]->def->Name(), func->def->Name())) {
      if (func->def->TypeDef()->MatchesVirtualFunction(
              *functions[i]->def->TypeDef())) {
        functions[i] = func;
        return;
      }
    }
...
void idVarDef::PrintInfo(idFile* file, int instructionPointer) const {
  ...
  switch (etype) {
    case ev_jumpoffset:
      ...
      file->Printf("address %d [%s(%d)]", jumpto,
                   gameLocal.program.GetFilename(jumpst->file),
                   jumpst->linenumber);
      break;
...
void idScriptObject::Restore(idRestoreGame* savefile) {
  ...
  if (!SetType(typeName)) {
    savefile->Error(
        "idScriptObject::Restore: failed to restore object of type '%s'.",
        typeName.c_str());
  }
  ...
  if (size != type->Size()) {
    savefile->Error(
        "idScriptObject::Restore: size of object '%s' doesn't match size in "
        "save game.",
        typeName.c_str());
  }
...
bool idScriptObject::SetType(const char* typeName) {
  ...
  if (newtype != type) {
    ...
    if (!newtype->Inherits(&type_object)) {
      gameLocal.Warning(
          "idScriptObject::SetType: Can't create object of type '%s'.  Must be "
          "an object type.",
          newtype->Name());
      return false;
    }
...
idVarDef* idProgram::AllocDef(idTypeDef* type,
  ...
  if ((type->Type() == ev_vector) ||
    ...
    if (numVariables > sizeof(variables)) {
      throw idCompileError(
          va("Exceeded global memory size (%d bytes)", sizeof(variables)));
    }
...
function_t* idProgram::FindFunction(const char* name) const {
  ...
  if ((def->Type() == ev_function) &&
      (def->value.functionPtr->eventdef == NULL)) {
...
function_t* idProgram::FindFunction(const char* name,
                                    const idTypeDef* type) const {
  const idVarDef* tdef;
  const idVarDef* def;

  // look for the function
  def = NULL;
  for (tdef = type->def; tdef != &def_object;
...
function_t& idProgram::AllocFunction(idVarDef* def) {
  if (functions.Num() >= functions.Max()) {
    throw idCompileError(va("Exceeded maximum allowed number of functions (%d)",
                            functions.Max()));
  }
...
statement_t* idProgram::AllocStatement(void) {
  if (statements.Num() >= statements.Max()) {
    throw idCompileError(
        va("Exceeded maximum allowed number of statements (%d)",
           statements.Max()));
  }
...
void idProgram::DisassembleStatement(idFile* file,
  ...
  file->Printf("%20s(%d):\t%6d: %15s\t", fileList[statement->file].c_str(),
               statement->linenumber, instructionPointer, op->opname);
...
void idProgram::Disassemble(void) const {
  ...
  for (i = 0; i < functions.Num(); i++) {
    ...
    file->Printf("\nfunction %s() %d stack used, %d parms, %d locals {\n",
                 func->Name(), func->locals, func->parmTotal,
                 func->locals - func->parmTotal);

    for (instructionPointer = 0; instructionPointer < func->numStatements;
         instructionPointer++) {
...
bool idProgram::CompileText(const char* source,
  ...
  try {
    ...
    for (i = 0; i < varDefs.Num(); i++) {
      def = varDefs[i];
      if ((def->Type() == ev_function) &&
          ((def->scope->Type() == ev_namespace) ||
           def->scope->TypeDef()->Inherits(&type_object))) {
        if (!def->value.functionPtr->eventdef &&
            !def->value.functionPtr->firstStatement) {
          throw idCompileError(
              va("function %s was not defined\n", def->GlobalName()));
        }
      }
    }
  }

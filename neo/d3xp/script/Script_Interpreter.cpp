...
bool idInterpreter::GetRegisterValue(const char* name,
  ...
  switch (d->Type()) {
    ...
    case ev_vector:
      if (reg.vectorPtr) {
        out = va("%g,%g,%g", reg.vectorPtr->x, reg.vectorPtr->y,
                 reg.vectorPtr->z);
      } else {
        out = "0,0,0";
      }
    ...
    case ev_field:
      ...
      obj = *reinterpret_cast<const idScriptObject**>(
      ...
      switch (field->Type()) {
        ...
        case ev_float:
          out =
              va("%g", *(reinterpret_cast<float*>(&obj->data[reg.ptrOffset])));
          return true;
...
void idInterpreter::StackTrace(void) const {
  ...
  if (!currentFunction) {
    ...
    gameLocal.Printf("%12s : %s\n",
                     gameLocal.program.GetFilename(currentFunction->filenum),
  ...
  for (i = top; i >= 0; i--) {
    ...
    if (!f) {
      ...
      gameLocal.Printf("%12s : %s\n", gameLocal.program.GetFilename(f->filenum),
                       f->Name());
    }
  }
}
...
void idInterpreter::Error(char* fmt, ...) const {
  ...
  if ((instructionPointer >= 0) &&
      (instructionPointer < gameLocal.program.NumStatements())) {
    statement_t& line = gameLocal.program.GetStatement(instructionPointer);
    common->Error("%s(%d): Thread '%s': %s\n",
                  gameLocal.program.GetFilename(line.file), line.linenumber,
...
void idInterpreter::EnterFunction(const function_t* func, bool clearStack) {
  ...
  stack->s =
      instructionPointer + 1;  // point to the next instruction to execute
...
void idInterpreter::LeaveFunction(idVarDef* returnDef) {
  ...
  if (debug) {
    statement_t& line = gameLocal.program.GetStatement(instructionPointer);
    gameLocal.Printf("%d: %s(%d): exit %s", gameLocal.time,
                     gameLocal.program.GetFilename(line.file), line.linenumber,
                     currentFunction->Name());
    if (callStackDepth > 1) {
      gameLocal.Printf(
          " return to %s(line %d)\n", callStack[callStackDepth - 1].f->Name(),
          gameLocal.program.GetStatement(callStack[callStackDepth - 1].s)
              .linenumber);
    } else {
...
void idInterpreter::CallEvent(const function_t* func, int argsize) {
  ...
  if (!eventEntity || !eventEntity->RespondsTo(*evdef)) {
    if (eventEntity && developer.GetBool()) {
      ...
      Warning("Function '%s' not supported on entity '%s'", evdef->GetName(),
              eventEntity->name.c_str());
    }
  ...
  for (j = 0, i = 0, pos = type_object.Size();
       (pos < argsize) || (format[i] != 0); i++) {
    switch (format[i]) {
      case D_EVENT_INTEGER:
      ...
      case D_EVENT_ENTITY:
        ...
        if (!(*(idEntity**)&data[i])) {
          Warning("Entity not found for event '%s'. Terminating thread.",
                  evdef->GetName());
          threadDying = true;
          PopParms(argsize);
          return;
      ...
      case D_EVENT_TRACE:
        Error("trace type not supported from script for '%s' event.",
              evdef->GetName());
        break;
...
bool idInterpreter::Execute(void) {
  ...
  while (!doneProcessing && !threadDying) {
    ...
    switch (st->op) {
      ...
      case OP_THREAD:
        newThread =
            new idThread(this, st->a->value.functionPtr, st->b->value.argSize);
        newThread->Start();
      ...
      case OP_OBJTHREAD:
        ...
        if (obj) {
          ...
          newThread = new idThread(this, GetEntity(*var_a.entityNumberPtr),
                                   func, func->parmTotal);
          newThread->Start();
      ...
      case OP_MOD_F:
        ...
        if (*var_b.floatPtr == 0.0f) {
          ...
          *var_c.floatPtr = static_cast<int>(*var_a.floatPtr) %
                            static_cast<int>(*var_b.floatPtr);
        }
        break;
      ...
      case OP_AND:
        ...
        var_c = GetVariable(st->c);
        *var_c.floatPtr =
            (*var_a.floatPtr != 0.0f) && (*var_b.floatPtr != 0.0f);
        break;
      ...
      case OP_UDIV_V:
        ...
        if (*var_a.floatPtr == 0.0f) {
          Warning("Divide by zero");
          var_b.vectorPtr->Set(idMath::INFINITY, idMath::INFINITY,
                               idMath::INFINITY);
        } else {
      ...
      case OP_UMOD_F:
        ...
        if (*var_a.floatPtr == 0.0f) {
          ...
          *var_b.floatPtr = static_cast<int>(*var_b.floatPtr) %
                            static_cast<int>(*var_a.floatPtr);
        }
        break;
      ...
      case OP_STOREP_S:
        ...
        if (var_b.evalPtr && var_b.evalPtr->stringPtr) {
          idStr::Copynz(var_b.evalPtr->stringPtr, GetString(st->a),
                        MAX_STRING_LEN);
        }
        break;
      ...
      case OP_STOREP_FTOS:
        ...
        if (var_b.evalPtr && var_b.evalPtr->stringPtr) {
          var_a = GetVariable(st->a);
          idStr::Copynz(var_b.evalPtr->stringPtr,
                        FloatToString(*var_a.floatPtr), MAX_STRING_LEN);
        }
        break;
      ...
      case OP_STOREP_VTOS:
        ...
        if (var_b.evalPtr && var_b.evalPtr->stringPtr) {
          var_a = GetVariable(st->a);
          idStr::Copynz(var_b.evalPtr->stringPtr, var_a.vectorPtr->ToString(),
                        MAX_STRING_LEN);
        }
        break;
        ...
        if (var_b.evalPtr && var_b.evalPtr->entityNumberPtr) {
          ...
          if (!obj) {
            *var_b.evalPtr->entityNumberPtr = 0;
            ...
            // its type reassigned, so we store the real type in st->c so that
            // we can do a type check during run time since we don't know what
            // type the script object is at compile time because it comes from
            // an entity

    ...
    if (ent->GetPhysics()->GetContents() &
      ...
      if (!show) {
        for (i = 0; i < ent->targets.Num(); i++) {
          ...
          if (target && viewBounds.IntersectsBounds(
      ...
      for (i = 0; i < ent->targets.Num(); i++) {
        ...
        if (target) {
          gameRenderWorld->DebugArrow(
              colorYellow, ent->GetPhysics()->GetAbsBounds().GetCenter(),
              target->GetPhysics()->GetOrigin(), 10, 0);
          gameRenderWorld->DebugBounds(colorGreen, box,
                                       target->GetPhysics()->GetOrigin());
          if (viewTextBounds.IntersectsBounds(
                  target->GetPhysics()->GetAbsBounds())) {
            gameRenderWorld->DrawText(
                target->name.c_str(),
                target->GetPhysics()->GetAbsBounds().GetCenter(), 0.1f,
                colorWhite, axis, 1);
          }
        }
      }
...
void idTrigger::Spawn(void) {
  ...
  if (funcname.Length()) {
    scriptFunction = gameLocal.program.FindFunction(funcname);
    if (scriptFunction == NULL) {
      gameLocal.Warning("trigger '%s' at (%s) calls unknown function '%s'",
                        name.c_str(), GetPhysics()->GetOrigin().ToString(0),
                        funcname.c_str());
    }
  } else {
...
void idTrigger_Multi::Spawn(void) {
  ...
  if (random && (random >= wait) && (wait >= 0)) {
    random = wait - 1;
    gameLocal.Warning("idTrigger_Multi '%s' at (%s) has random >= wait",
                      name.c_str(), GetPhysics()->GetOrigin().ToString(0));
  }

  if (random_delay && (random_delay >= delay) && (delay >= 0)) {
    random_delay = delay - 1;
    gameLocal.Warning("idTrigger_Multi '%s' at (%s) has random_delay >= delay",
                      name.c_str(), GetPhysics()->GetOrigin().ToString(0));
  }
...
#ifdef _D3XP
    // If the player spawned inside the trigger, the player Spawn function
    // called Think directly, allowing for multiple triggers on a trigger_once.
    // Increasing the nextTriggerTime prevents it.
...
void idTrigger_Multi::Event_Trigger(idEntity* activator) {
  ...
  if (delay > 0) {
    ...
    nextTriggerTime +=
        SEC2MS(delay + random_delay * gameLocal.random.CRandomFloat());
...
void idTrigger_EntityName::Spawn(void) {
  ...
  if (random && (random >= wait) && (wait >= 0)) {
    random = wait - 1;
    gameLocal.Warning("idTrigger_EntityName '%s' at (%s) has random >= wait",
                      name.c_str(), GetPhysics()->GetOrigin().ToString(0));
  }
  ...
  if (!entityName.Length()) {
    gameLocal.Error(
        "idTrigger_EntityName '%s' at (%s) doesn't have 'entityname' key "
        "specified",
        name.c_str(), GetPhysics()->GetOrigin().ToString(0));
  }
...
void idTrigger_Timer::Spawn(void) {
  ...
  if (random >= wait && wait >= 0) {
    random = wait - 0.001;
    gameLocal.Warning("idTrigger_Timer '%s' at (%s) has random >= wait",
                      name.c_str(), GetPhysics()->GetOrigin().ToString(0));
  }
...
void idTrigger_Touch::TouchEntities(void) {
  ...
  bounds.FromTransformedBounds(clipModel->GetBounds(), clipModel->GetOrigin(),
                               clipModel->GetAxis());
  ...
  for (i = 0; i < numClipModels; i++) {
    ...
    if (!gameLocal.clip.ContentsModel(
            cm->GetOrigin(), cm, cm->GetAxis(), -1, clipModel->Handle(),
            clipModel->GetOrigin(), clipModel->GetAxis())) {
      continue;
    }
...
void idTrigger_Flag::Spawn(void) {
  ...
  if (funcname.Length()) {
    eventFlag =
        idEventDef::FindEvent(funcname);  // gameLocal.program.FindFunction(
                                          // funcname );//, &idItemTeam::Type );
    if (eventFlag == NULL) {
      gameLocal.Warning("trigger '%s' at (%s) event unknown '%s'", name.c_str(),
...
void idTrigger_Flag::Event_Touch(idEntity* other, trace_t* trace) {
  ...
  if (player) {
    ...
    if (team != -1 &&
        (player->team != team || (player->team != 0 && player->team != 1)))
      return;
    ...
    if (flags[iFriend]->carried &&
        !flags[iFriend]->dropped &&  // flags[iFriend]->IsBoundTo( player ) &&

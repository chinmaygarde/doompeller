...
/*
 * EAX Unified Interface (using Direct X 7)
...
/*
 * EAX Unified Interface (using Direct X 8)
...
__declspec(dllimport) HRESULT WINAPI
    EAXDirectSoundCreate8(GUID*, LPDIRECTSOUND8*, IUnknown FAR*);
...
__declspec(dllimport) HRESULT WINAPI
    EAXDirectSoundCreate(GUID*, LPDIRECTSOUND*, IUnknown FAR*);
...
typedef struct _EAXSOURCEPROPERTIES {
  ...
  long lObstruction;  // main obstruction control (attenuation at high
                      // frequencies)
  float
      flObstructionLFRatio;  // obstruction low-frequency level re. main control
...
// Use this structure for EAXREVERB_REFLECTIONSPAN and EAXREVERB_REVERBPAN
// properties.
...
#define EAXCONTEXT_PARAMETER_IMMEDIATE \
...
typedef enum {
  EAXFXSLOT_PARAMETER =
      0,  // range 0-0x40 reserved for loaded effect parameters
...
// OR these flags with property id
#define EAXSOURCE_PARAMETER_IMMEDIATE \
  0x00000000  // changes take effect immediately
...
#define EAXSOURCE_PARAMETER_COMMITDEFERREDSETTINGS \
  (EAXSOURCE_NONE | EAXSOURCE_PARAMETER_IMMEDIATE)
...
// A 3D Source's default ACTIVEFXSLOTID is { EAX_NULL_GUID, EAX_PrimaryFXSlotID,
// EAX_NULL_GUID, EAX_NULL_GUID }
#define EAXSOURCE_3DDEFAULTACTIVEFXSLOTID                                        \
  {                                                                              \
    ...
    {                                                                            \
      EAX_NULL_GUID.Data1, EAX_NULL_GUID.Data2, EAX_NULL_GUID.Data3,             \
          EAX_NULL_GUID.Data4[0], EAX_NULL_GUID.Data4[1],                        \
          EAX_NULL_GUID.Data4[2], EAX_NULL_GUID.Data4[3],                        \
          EAX_NULL_GUID.Data4[4], EAX_NULL_GUID.Data4[5],                        \
          EAX_NULL_GUID.Data4[6], EAX_NULL_GUID.Data4[7]                         \
    }                                                                            \
  }

// A 2D Source's default ACTIVEFXSLOTID is { EAX_NULL_GUID, EAX_NULL_GUID,
// EAX_NULL_GUID, EAX_NULL_GUID }
...
//              myFlags = EAXREVERBFLAGS_DECAYTIMESCALE |
...
#define EAXREVERBFLAGS_REFLECTIONSDELAYSCALE \
  0x00000004                                   // initial reflection delay time
...
#define EAXREVERB_DEFAULTREFLECTIONSPAN \
  { 0.0f, 0.0f, 0.0f }
...
#define EAXREVERB_DEFAULTREVERBPAN \
  { 0.0f, 0.0f, 0.0f }
...
#define EAXAUTOWAH_COMMITDEFERREDSETTINGS \
  (EAXAUTOWAH_NONE | EAXAUTOWAH_IMMEDIATE)
...
#define EAXDISTORTION_COMMITDEFERREDSETTINGS \
  (EAXDISTORTION_NONE | EAXDISTORTION_IMMEDIATE)
...
#define EAXECHO_COMMITDEFERREDSETTINGS (EAXECHO_NONE | EAXECHO_IMMEDIATE)
...
typedef struct _EAXECHOPROPERTIES {
  ...
  float flLRDelay;  // Controls the delay time between the first and second taps
                    // (seconds)
  float
      flDamping;  // Controls a low-pass filter that dampens the echoes (0 to 1)
...
#define EAXFLANGER_COMMITDEFERREDSETTINGS \
  (EAXFLANGER_NONE | EAXFLANGER_IMMEDIATE)
...
// OR these flags with property id
#define EAXFREQUENCYSHIFTER_IMMEDIATE \
  0x00000000  // changes take effect immediately
...
#define EAXFREQUENCYSHIFTER_COMMITDEFERREDSETTINGS \
  (EAXFREQUENCYSHIFTER_NONE | EAXFREQUENCYSHIFTER_IMMEDIATE)

// used by EAXFREQUENCYSHIFTER_LEFTDIRECTION and
// EAXFREQUENCYSHIFTER_RIGHTDIRECTION
enum {
...
#define EAXVOCALMORPHER_COMMITDEFERREDSETTINGS \
  (EAXVOCALMORPHER_NONE | EAXVOCALMORPHER_IMMEDIATE)
...
#define EAXPITCHSHIFTER_COMMITDEFERREDSETTINGS \
  (EAXPITCHSHIFTER_NONE | EAXPITCHSHIFTER_IMMEDIATE)

...
/
/ Description : The EAX-AC3 extension to Open AL provides a way to playback
Dolby Digital AC3 /				files on systems equipped with a
SB Live! card.  The packaged AC3 data is output /
via the MMSYSTEM Wave device. /				If a SB Live! 5.1 card
is installed then the AC3 data will be decoded by the /
audio card. /				If a legacy SB Live! card is installed
then the AC3 data will be sent directly /				to the
S/PDIF Out. /				The API supports multiple EAX-AC3
devices, and multiple AC3 streams. However /				the
current implementation provides one EAX-AC3 device capable of playing /
...
// Do not define the symbol EAXAC3_EXPORTS in any projects that use the EAX-AC3
// Open AL Extension
...
typedef HRESULT (*LPEAXAC3QUERYDEVICECAPS)(EAXAC3HANDLE,
                                           LPEAXAC3DEVICEINFO,
                                           int);
...
typedef HRESULT (*LPEAXAC3OPENSTREAM)(EAXAC3HANDLE,
                                      AC3STREAM*,
                                      LPAC3CALLBACK,
                                      char*,
                                      enum SOURCE);
...
typedef HRESULT (*LPEAXAC3LOCKBUFFER)(AC3STREAM,
                                      unsigned long,
                                      void**,
                                      unsigned long*,
                                      void**,
                                      unsigned long*,
                                      unsigned long);
typedef HRESULT (*LPEAXAC3UNLOCKBUFFER)(AC3STREAM,
                                        void*,
                                        unsigned long,
                                        void*,
                                        unsigned long,

      ...
      case '\\':
        ...
        switch (*fmt) {
          ...
          case 'n':
            index += fprintf(file, "\n");
          default:
            MayaError("WriteFloatString: unknown escape character \'%c\'",
                      *fmt);
            break;
        }
        fmt++;
...
idExportOptions::idExportOptions(const char* commandline, const char* ospath) {
  ...
  for (token = tokens.NextToken(); token != ""; token = tokens.NextToken()) {
    if (token == "-force") {
      ...
      joints.from = tokens.NextToken(
          "Missing joint name for -rename.  Usage: -rename [joint name] [new "
          "name]");
      joints.to = tokens.NextToken(
          "Missing new name for -rename.  Usage: -rename [joint name] [new "
          "name]");
      renamejoints.Append(joints);

    } else if (token == "-prefix") {
      prefix = tokens.NextToken(
          "Missing name for -prefix.  Usage: -prefix [joint prefix]");

    } else if (token == "-parent") {
      // parse joint to reparent
      joints.from = tokens.NextToken(
          "Missing joint name for -parent.  Usage: -parent [joint name] [new "
          "parent]");
      joints.to = tokens.NextToken(
          "Missing new parent for -parent.  Usage: -parent [joint name] [new "
          "parent]");
      remapjoints.Append(joints);

    } else if (!token.Icmp("-sourcedir")) {
      // parse source directory
      sourceDir = tokens.NextToken(
          "Missing filename for -sourcedir.  Usage: -sourcedir [directory]");

    } else if (!token.Icmp("-destdir")) {
      // parse destination directory
      destDir = tokens.NextToken(
          "Missing filename for -destdir.  Usage: -destdir [directory]");

    } else if (token == "-dest") {
      // parse destination filename
      dest = tokens.NextToken(
          "Missing filename for -dest.  Usage: -dest [filename]");

    } else if (token == "-range") {
      // parse frame range to export
      token = tokens.NextToken(
          "Missing start frame for -range.  Usage: -range [start frame] [end "
          "frame]");
      startframe = atoi(token);
      token = tokens.NextToken(
          "Missing end frame for -range.  Usage: -range [start frame] [end "
          "frame]");
      endframe = atoi(token);
      ...
      token = tokens.NextToken(
          "Missing cycle start frame for -cycleStart.  Usage: -cycleStart "
      ...
      // parse align joint
      align = tokens.NextToken(
          "Missing joint name for -align.  Usage: -align [joint name]");

    } else if (token == "-rotate") {
      // parse angle rotation
      token =
          tokens.NextToken("Missing value for -rotate.  Usage: -rotate [yaw]");
      rotate = -atof(token);
      ...
      // parse quaternion precision
      token = tokens.NextToken(
          "Missing value for -quatprecision.  Usage: -quatprecision "
          "[precision]");
      ...
      token = tokens.NextToken(
          "Missing weight for -jointthreshold.  Usage: -jointthreshold "
          "[minimum joint weight]");
      jointThreshold = atof(token);

    } else if (token == "-skipmesh") {
      token = tokens.NextToken(
          "Missing name for -skipmesh.  Usage: -skipmesh [name of mesh to "
          "skip]");
      skipmeshes.AddUnique(token);

    } else if (token == "-keepmesh") {
      token = tokens.NextToken(
          "Missing name for -keepmesh.  Usage: -keepmesh [name of mesh to "
          "keep]");
      keepmeshes.AddUnique(token);

    } else if (token == "-jointgroup") {
      token = tokens.NextToken(
          "Missing name for -jointgroup.  Usage: -jointgroup [group name] "
...
void idExportMesh::GetBounds(idBounds& bounds) const {
  ...
  for (i = 0; i < verts.Num(); i++, vert++) {
    ...
    for (j = 0; j < vert->numWeights; j++, weight++) {
      pos += weight->jointWeight *
             (weight->joint->idwm * weight->offset + weight->joint->idt);
    }
    bounds.AddPoint(pos);
  }
}
...
bool idExportModel::WriteMesh(const char* filename, idExportOptions& options) {
  ...
  for (joint = exportHead.GetNext(); joint != NULL;
       joint = joint->exportNode.GetNext()) {
  ...
  for (i = 0; i < jointList.Num(); i++) {
    ...
    WriteFloatString(file, "\t\"%s\"\t%d ( %f %f %f ) ( %f %f %f )\t\t// %s\n",
                     joint->name.c_str(), parentNum, joint->bindpos.x,
                     joint->bindpos.y, joint->bindpos.z, bindQuat[0],
                     bindQuat[1], bindQuat[2], parentName);
  }
  ...
  for (i = 0; i < meshes.Num(); i++) {
    ...
    for (j = 0; j < mesh->verts.Num(); j++) {
      WriteFloatString(file, "\tvert %d ( %f %f ) %d %d\n", j,
                       mesh->verts[j].texCoords[0], mesh->verts[j].texCoords[1],
                       mesh->verts[j].startweight, mesh->verts[j].numWeights);
    }

    WriteFloatString(file, "\n\tnumtris %d\n", mesh->tris.Num());
    for (j = 0; j < mesh->tris.Num(); j++) {
      WriteFloatString(file, "\ttri %d %d %d %d\n", j, mesh->tris[j].indexes[2],
                       mesh->tris[j].indexes[1], mesh->tris[j].indexes[0]);
    }
    ...
    for (j = 0; j < mesh->weights.Num(); j++) {
      ...
      WriteFloatString(file, "\tweight %d %d %f ( %f %f %f )\n", j,
                       weight->joint->exportNum, weight->jointWeight,
                       weight->offset.x, weight->offset.y, weight->offset.z);
    }
...
bool idExportModel::WriteAnim(const char* filename, idExportOptions& options) {
  ...
  for (i = 0; i < jointList.Num(); i++) {
    ...
    parent = joint->exportNode.GetParent();
    if (parent) {
      WriteFloatString(file, "\t\"%s\"\t%d %d %d\t// %s", joint->name.c_str(),
                       parent->exportNum, joint->animBits,
                       joint->firstComponent, parent->name.c_str());
    } else {
      WriteFloatString(file, "\t\"%s\"\t-1 %d %d\t//", joint->name.c_str(),
                       joint->animBits, joint->firstComponent);
    }
  ...
  for (i = 0; i < numFrames; i++) {
    WriteFloatString(file, "\t( %f %f %f ) ( %f %f %f )\n", bounds[i][0].x,
                     bounds[i][0].y, bounds[i][0].z, bounds[i][1].x,
                     bounds[i][1].y, bounds[i][1].z);
  }
  WriteFloatString(file, "}\n");
  ...
  for (i = 0; i < jointList.Num(); i++) {
    joint = jointList[i];
    WriteFloatString(file, "\t( %f %f %f ) ( %f %f %f )\n",
                     joint->baseFrame.t[0], joint->baseFrame.t[1],
                     joint->baseFrame.t[2], joint->baseFrame.q[0],
                     joint->baseFrame.q[1], joint->baseFrame.q[2]);
  }
  WriteFloatString(file, "}\n");
...
bool idExportModel::WriteCamera(const char* filename,
  ...
  for (i = 0; i < camera.Num(); i++, frame++) {
    WriteFloatString(file, "\t( %f %f %f ) ( %f %f %f ) %f\n", frame->t.x,
                     frame->t.y, frame->t.z, frame->q[0], frame->q[1],
                     frame->q[2], frame->fov);
  }
  WriteFloatString(file, "}\n");
...
void idMayaExport::PruneJoints(idStrList& keepjoints, idStr& prefix) {
    ...
    for (i = 0; i < model.meshes.Num(); i++) {
      ...
      for (j = 0; j < mesh->weights.Num(); j++) {
        if (mesh->weights[j].joint->keep) {
          num_weights++;
        } else if (prefix.Length() && !mesh->weights[j].joint->realname.Cmpn(
  ...
  for (i = 0; i < model.joints.Num(); i++, joint++) {
    if (!joint->keep) {
      ...
      for (parent = joint->exportNode.GetParent(); parent != NULL;
           parent = parent->exportNode.GetParent()) {
        if (parent->keep) {
          break;
        }
      }
  ...
  for (joint = model.exportHead.GetNext(); joint != NULL;
       joint = joint->exportNode.GetNext()) {
    if (!joint->keep) {
      MayaError("Non-kept joint in export tree ('%s')", joint->name.c_str());
    }

    for (joint2 = model.exportHead.GetNext(); joint2 != NULL;
         joint2 = joint2->exportNode.GetNext()) {
      if ((joint2 != joint) && (joint2->name == joint->name)) {
        MayaError("Two joints found with the same name ('%s')",
                  joint->name.c_str());
      }
    }
  }
...
void idMayaExport::CreateJoints(float scale) {
  ...
  for (; !dagIterator.isDone(); dagIterator.next()) {
    ...
    if (!status) {
      MayaError("CreateJoints: MItDag::getPath failed (%s)",
    ...
    if (!status) {
      MayaError("CreateJoints: MFnDagNode constructor failed (%s)",
                status.errorString().asChar());
      continue;
    }
...
bool idMayaExport::RemapParents(idList<idNamePair>& remapjoints) {
  ...
  for (i = 0; i < remapjoints.Num(); i++) {
    ...
    if (parent->exportNode.ParentedBy(joint->exportNode)) {
      MayaError(
          "Joint '%s' is a child of joint '%s' and can't become the parent.",
          joint->name.c_str(), parent->name.c_str());
    }
    joint->exportNode.ParentTo(parent->exportNode);
  }
...
MObject idMayaExport::FindShader(MObject& setNode) {
  ...
  if (!shaderPlug.isNull()) {
    ...
    if (connectedPlugs.length() != 1) {
      MayaError("FindShader: Error getting shader (%s)",
                status.errorString().asChar());
    } else {
...
void idMayaExport::GetTextureForMesh(idExportMesh* mesh, MFnDagNode& dagNode) {
  ...
  if (!status) {
    MayaError(
        "GetTextureForMesh: MFnMesh::getConnectedSetsAndMembers failed (%s)",
        status.errorString().asChar());
  }
  ...
  for (i = 0; i < (int)sets.length(); i++) {
    ...
    if (status == MS::kFailure) {
      MayaError("GetTextureForMesh: MFnSet constructor failed (%s)",
                status.errorString().asChar());
      continue;
    }
    ...
    MPlug filenamePlug =
        MFnDependencyNode(textureNode).findPlug("fileTextureName");
    MString textureName;
...
idExportMesh* idMayaExport::CopyMesh(MFnSkinCluster& skinCluster, float scale) {
  ...
  if (!status) {
    MayaError("CopyMesh: Error getting input geometry (%s)",
              status.errorString().asChar());
    return NULL;
  }
  ...
  for (i = 0; i < nGeom; i++) {
    ...
    if (!status) {
      common->Printf("CopyMesh: MFnDagNode Constructor failed (%s)",
                     status.errorString().asChar());
      continue;
    }
    ...
    if (!status) {
      common->Printf("CopyMesh: Error getting output geometry (%s)",
                     status.errorString().asChar());
      return NULL;
    }
    ...
    if (!status) {
      MayaError("CopyMesh: MFnMesh::getCurrentUVSetName failed (%s)",
                status.errorString().asChar());
    }
    ...
    for (j = 0; j < p; j++) {
      ...
      for (k = 0; k < 3; k++) {
        ...
        if (!status) {
          MayaError("CopyMesh: MFnMesh::getPolygonUV failed (%s)",
                    status.errorString().asChar());
        }
...
void idMayaExport::CreateMesh(float scale) {
  ...
  for (; !iter.isDone(); iter.next()) {
    ...
    if (!status) {
      MayaError("%s: Error getting skin cluster (%s)", object.apiTypeStr(),
                status.errorString().asChar());
    }
    ...
    if (!status) {
      MayaError("Mesh '%s': Error getting influence objects (%s)",
                mesh->name.c_str(), status.errorString().asChar());
    }
    ...
    for (size_t ii = 0; ii < nGeoms; ++ii) {
      ...
      if (!status) {
        MayaError("Mesh '%s': Error getting geometry index (%s)",
                  mesh->name.c_str(), status.errorString().asChar());
      }
      ...
      if (!status) {
        MayaError("Mesh '%s': Error getting geometry path (%s)",
                  mesh->name.c_str(), status.errorString().asChar());
      }
      ...
      for (/* nothing */; !gIter.isDone(); gIter.next()) {
        ...
        if (!status) {
          MayaError("Mesh '%s': Error getting component (%s)",
                    mesh->name.c_str(), status.errorString().asChar());
        }
        ...
        if (!status) {
          MayaError("Mesh '%s': Error getting weights (%s)", mesh->name.c_str(),
                    status.errorString().asChar());
        }
        if (0 == infCount) {
          MayaError("Mesh '%s': Error: 0 influence objects.",
                    mesh->name.c_str());
        }
        ...
        for (jj = 0; jj < (int)infCount; ++jj) {
          ...
          if (w > options.jointThreshold) {
            ...
            if (!options.ignoreScale) {
              weight.joint->bindmat.ProjectVector(
                  vert->pos - (weight.joint->bindpos * weight.joint->invscale),
                  weight.offset);
              weight.offset *= weight.joint->scale;
            } else {
              weight.joint->bindmat.ProjectVector(
                  vert->pos - weight.joint->bindpos, weight.offset);
            }
        ...
        if (!vert->numWeights) {
          if (numNonZeroWeights) {
            MayaError(
                "Error on mesh '%s': Vertex %d doesn't have any joint weights "
                "exceeding jointThreshold (%f).",
          ...
          MayaError("Error on mesh '%s': Combined weight of 0 on vertex %d.",
                    mesh->name.c_str(), num);
        }
        // if ( numNonZeroWeights ) {
...
void idMayaExport::CreateCameraAnim(idMat3& align) {
  ...
  for (frameNum = 0; frameNum < model.numFrames; frameNum++) {
    common->Printf("\rFrame %d/%d...", options.startframe + frameNum,
                   options.endframe);
    ...
    if (refCam != NULL) {
      ...
      if (MS::kSuccess != status) {
        MayaError("Error getting camera name on frame %d",
                  GetMayaFrameNum(frameNum));
      }
...
void idMayaExport::CreateAnimation(idMat3& align) {
  ...
  if (options.cycleStart > options.startframe) {
    ...
    for (i = 0; i < model.numFrames; i++) {
      ...
      memcpy(&model.jointFrames[model.joints.Num() * i],
             &copyFrames[model.joints.Num() * frameNum],
             model.joints.Num() * sizeof(copyFrames[0]));
...
void idMayaExport::ConvertModel(void) {
  ...
  if (options.cycleStart < 0) {
    options.cycleStart = options.startframe;
  } else if ((options.cycleStart < options.startframe) ||
             (options.cycleStart > options.endframe)) {
...
bool dllEntry(int version, idCommon* common, idSys* sys) {
  ...
  if (version != MD5_VERSION) {
    common->Printf(
        "Error initializing Maya exporter: DLL version %d different from .exe "
        "version %d\n",
        MD5_VERSION, version);
    return false;
  }

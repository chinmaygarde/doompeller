...
typedef enum { WRITE_MESH, WRITE_ANIM, WRITE_CAMERA } exportType_t;
...
ID_INLINE int operator==(exportVertex_t a, exportVertex_t b) {
  ...
  if ((a.texCoords[0] != b.texCoords[0]) ||
...
class idMayaExport {
 private:
  ...
  void GetWorldTransform(idExportJoint* joint,
                         idVec3& pos,
                         idMat3& mat,
                         float scale);
  ...
  void GetAlignment(idStr& alignName,
                    idMat3& align,
                    float rotate,
                    int startframe);

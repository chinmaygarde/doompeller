  ...
  switch (move.stage) {
    case ACCELERATION_STAGE: {
      physicsObj.SetLinearExtrapolation(EXTRAPOLATION_ACCELLINEAR,
                                        gameLocal.time, move.acceleration, org,
    ...
    case LINEAR_STAGE: {
      physicsObj.SetLinearExtrapolation(EXTRAPOLATION_LINEAR, gameLocal.time,
                                        move.movetime, org, move.dir,
                                        vec3_origin);
    ...
    case DECELERATION_STAGE: {
      physicsObj.SetLinearExtrapolation(EXTRAPOLATION_DECELLINEAR,
                                        gameLocal.time, move.deceleration, org,
                                        move.dir, vec3_origin);
      move.stage = FINISHED_STAGE;
      break;
    }
  ...
  switch (rot.stage) {
    case ACCELERATION_STAGE: {
      physicsObj.SetAngularExtrapolation(EXTRAPOLATION_ACCELLINEAR,
                                         gameLocal.time, rot.acceleration, ang,
                                         rot.rot, ang_zero);
      if (rot.movetime > 0) {
    ...
    case LINEAR_STAGE: {
      if (!stopRotation && !rot.deceleration) {
        physicsObj.SetAngularExtrapolation(
            ...
            gameLocal.time, rot.movetime, ang, rot.rot, ang_zero);
      } else {
        physicsObj.SetAngularExtrapolation(EXTRAPOLATION_LINEAR, gameLocal.time,
                                           rot.movetime, ang, rot.rot,
                                           ang_zero);
      }
    ...
    case DECELERATION_STAGE: {
      physicsObj.SetAngularExtrapolation(EXTRAPOLATION_DECELLINEAR,
                                         gameLocal.time, rot.deceleration, ang,
                                         rot.rot, ang_zero);
      rot.stage = FINISHED_STAGE;
      break;
    }
    case FINISHED_STAGE: {
      ...
      if (stopRotation) {
        ...
        physicsObj.SetAngularExtrapolation(
            extrapolation_t(EXTRAPOLATION_LINEAR | EXTRAPOLATION_NOSTOP),
            gameLocal.time, 0, ang, rot.rot, ang_zero);
      }
...
void idMover::Event_TeamBlocked(idEntity* blockedEntity,
  ...
  if (g_debugMover.GetBool()) {
    gameLocal.Printf(
        ...
        gameLocal.time, name.c_str(), blockedEntity->name.c_str(),
...
void idMover::Event_MoveAccelerateTo(float speed, float time) {
  ...
  StartSound("snd_accel", SND_CHANNEL_BODY2, 0, false, NULL);
  StartSound("snd_move", SND_CHANNEL_BODY, 0, false, NULL);
  physicsObj.SetLinearExtrapolation(EXTRAPOLATION_ACCELLINEAR, gameLocal.time,
                                    move.acceleration, org, dir * (speed - v),
                                    dir * v);
}
...
void idMover::Event_MoveDecelerateTo(float speed, float time) {
  ...
  StartSound("snd_decel", SND_CHANNEL_BODY2, 0, false, NULL);
  StartSound("snd_move", SND_CHANNEL_BODY, 0, false, NULL);
  physicsObj.SetLinearExtrapolation(EXTRAPOLATION_DECELLINEAR, gameLocal.time,
                                    move.deceleration, org, dir * (v - speed),
                                    dir * speed);
}
...
void idDoor::Event_Touch(idEntity* other, trace_t* trace) {
  ...
  if (trigger && trace->c.id == trigger->GetId()) {
    ...
    if (other && other->IsType(idPlayer::Type) && IsLocked() &&
        gameLocal.time > nextSndTriggerTime) {
...
void idRotater::Spawn(void) {
  ...
  physicsObj.SetLinearExtrapolation(EXTRAPOLATION_NONE, gameLocal.time, 0,
                                    GetPhysics()->GetOrigin(), vec3_origin,
                                    vec3_origin);
  physicsObj.SetAngularExtrapolation(
      extrapolation_t(EXTRAPOLATION_LINEAR | EXTRAPOLATION_NOSTOP),
      gameLocal.time, 0, GetPhysics()->GetAxis().ToAngles(), ang_zero,
      ang_zero);
  SetPhysics(&physicsObj);

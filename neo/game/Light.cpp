...
================
*/
void idGameEdit::ParseSpawnArgsToRenderLight(const idDict* args,
  ...
  if ((gotTarget || gotUp || gotRight) != (gotTarget && gotUp && gotRight)) {
    gameLocal.Printf("Light at (%f,%f,%f) has bad target info\n",
                     renderLight->origin[0], renderLight->origin[1],
                     renderLight->origin[2]);
    return;
  }
  ...
  if (!gotTarget) {
    ...
    if (!args->GetVector("light_radius", "300 300 300",
                         renderLight->lightRadius)) {
      float radius;
  ...
  args->GetFloat("shaderParm3", "1",
                 renderLight->shaderParms[SHADERPARM_TIMESCALE]);
  if (!args->GetFloat("shaderParm4", "0",
...
void idLight::UpdateChangeableSpawnArgs(const idDict* source) {
  ...
  gameEdit->ParseSpawnArgsToRenderLight(source ? source : &spawnArgs,
                                        &renderLight);
...
void idLight::Restore(idRestoreGame* savefile) {
  ...
  renderLight.prelightModel =
      renderModelManager->CheckModel(va("_prelight_%s", name.c_str()));
...
void idLight::Spawn(void) {
  ...
  localLightOrigin = (renderLight.origin - GetPhysics()->GetOrigin()) *
  ...
  baseColor.Set(renderLight.shaderParms[SHADERPARM_RED],
  ...
  currentLevel = levels;
  if (levels <= 0) {
    gameLocal.Error("Invalid light level set on entity #%d(%s)", entityNumber,
                    name.c_str());
  }
  ...
  if (health) {
    ...
    if (!model.Length()) {
      gameLocal.Error("Breakable light without a model set on entity #%d(%s)",
    ...
    if (!renderModelManager->CheckModel(brokenModel)) {
      if (needBroken) {
        gameLocal.Error("Model '%s' not found for entity %d(%s)",
                        brokenModel.c_str(), entityNumber, name.c_str());
      } else {
        brokenModel = "";
      }
    }

    GetPhysics()->SetContents(spawnArgs.GetBool("nonsolid") ? 0
                                                            : CONTENTS_SOLID);
...
void idLight::BecomeBroken(idEntity* activator) {
  ...
  if (gameLocal.isServer) {
    ...
    if (spawnArgs.GetString("def_damage", "", &damageDefName)) {
      idVec3 origin = renderEntity.origin +
                      renderEntity.bounds.GetCenter() * renderEntity.axis;
      gameLocal.RadiusDamage(origin, activator, activator, this, this,
                             damageDefName);
    }
  }
  ...
  if (refSound.shader || (parm && *parm)) {
    StopSound(SND_CHANNEL_ANY, false);
    const idSoundShader* alternate = refSound.shader
                                         ? refSound.shader->GetAltSound()
                                         : declManager->FindSound(parm);
    if (alternate) {
...
void idLight::Present(void) {
  ...
  renderLight.origin =
      GetPhysics()->GetOrigin() + GetPhysics()->GetAxis() * localLightOrigin;
...
void idLight::SaveState(idDict* args) {
  ...
  for (i = 0; i < c; i++) {
    ...
    if (pv->GetKey().Find("editor_", false) >= 0 ||
        pv->GetKey().Find("parse_", false) >= 0) {
      continue;
    }

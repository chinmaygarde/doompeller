...
void idIK::Save(idSaveGame* savefile) const {
  ...
  savefile->WriteString(animator != NULL && animator->GetAnim(modifiedAnim)
                            ? animator->GetAnim(modifiedAnim)->Name()
                            : "");
...
void idIK::Restore(idRestoreGame* savefile) {
  ...
  if (self) {
    ...
    if (modifiedAnim == 0) {
      gameLocal.Warning(
          "idIK::Restore: IK for entity '%s' at (%s) has no modified "
          "animation.",
          self->name.c_str(), self->GetPhysics()->GetOrigin().ToString(0));
    }
  } else {
...
bool idIK_Walk::Init(idEntity* self,
  ...
  gameEdit->ANIM_CreateAnimFrame(
      animator->ModelHandle(), animator->GetAnim(modifiedAnim)->MD5Anim(0),
      numJoints, joints, 1,
...
void idIK_Walk::Evaluate(void) {
  ...
  float shift, smallestShift, newHeight, step, newPivotYaw, height,
      largestAnkleHeight;
  idVec3 modelOrigin, normal, hipDir, kneeDir, start, end,
      jointOrigins[MAX_LEGS];
  ...
  for (i = 0; i < numLegs; i++) {
    animator->GetJointTransform(footJoints[i], gameLocal.time, footOrigin,
                                axis);
  ...
  if (usePivot) {
    ...
    // change pivot foot
    if (newPivotFoot != pivotFoot || idMath::Fabs(idMath::AngleNormalize180(
                                         newPivotYaw - pivotYaw)) > 30.0f) {
      pivotFoot = newPivotFoot;
      pivotYaw = newPivotYaw;
      animator->GetJointTransform(footJoints[pivotFoot], gameLocal.time,
                                  footOrigin, axis);
  ...
  for (i = 0; i < numLegs; i++) {
    ...
    gameLocal.clip.Translation(results, start, end, footModel, mat3_identity,
                               CONTENTS_SOLID | CONTENTS_IKCLIP, self);
    ...
    if (ik_debug.GetBool() && footModel) {
      ...
      gameRenderWorld->DebugWinding(colorRed, w, results.endpos,
                                    results.endAxis);
    }
  }
  ...
  for (i = 0; i < numLegs; i++) {
    ...
    animator->GetJointTransform(ankleJoints[i], gameLocal.time, ankleOrigin,
                                ankleAxis[i]);
  ...
  if (minWaistFloorDist > 0.0f && waistOffset * normal < 0.0f) {
    ...
    gameLocal.clip.Translation(results, start, end, footModel, modelAxis,
                               CONTENTS_SOLID | CONTENTS_IKCLIP, self);
  ...
  if (minWaistAnkleDist > 0.0f) {
    ...
    if (height - largestAnkleHeight < minWaistAnkleDist) {
      waistOffset +=
          (minWaistAnkleDist - (height - largestAnkleHeight)) * normal;
    }
  }
  ...
  for (i = 0; i < numLegs; i++) {
    ...
    animator->GetJointTransform(kneeJoints[i], gameLocal.time, kneeOrigin,
                                axis);
    kneeDir = kneeForward[i] * axis * modelAxis;

    // solve IK and calculate knee position
    SolveTwoBones(hipOrigin, jointOrigins[i], kneeDir, upperLegLength[i],
                  lowerLegLength[i], kneeOrigin);
  ...
  animator->SetJointPos(
      waistJoint, JOINTMOD_WORLD_OVERRIDE,
  ...
  for (i = 0; i < numLegs; i++) {
    ...
    animator->SetJointAxis(ankleJoints[i], JOINTMOD_WORLD_OVERRIDE,
                           ankleAxis[i]);
  }
...
bool idIK_Reach::Init(idEntity* self,
  ...
  int numJoints = animator->NumJoints();
  idJointMat* joints = (idJointMat*)__builtin_alloca_with_align(
      numJoints * sizeof(joints[0]), 16);
...
void idIK_Reach::Evaluate(void) {
  int i;
  idVec3 modelOrigin, shoulderOrigin, elbowOrigin, handOrigin, shoulderDir,
      elbowDir;
  ...
  for (i = 0; i < numArms; i++) {
    ...
    animator->GetJointTransform(shoulderJoints[i], gameLocal.time,
                                shoulderOrigin, axis);
    ...
    animator->GetJointTransform(handJoints[i], gameLocal.time, handOrigin,
                                axis);
    handOrigin = modelOrigin + handOrigin * modelAxis;

    // get first collision going from shoulder to hand
    gameLocal.clip.TracePoint(trace, shoulderOrigin, handOrigin, CONTENTS_SOLID,
                              self);
    handOrigin = trace.endpos;

    // get the IK bend direction
    animator->GetJointTransform(elbowJoints[i], gameLocal.time, elbowOrigin,
                                axis);
    elbowDir = elbowForward[i] * axis * modelAxis;

    // solve IK and calculate elbow position
    SolveTwoBones(shoulderOrigin, handOrigin, elbowDir, upperArmLength[i],
                  lowerArmLength[i], elbowOrigin);

    if (ik_debug.GetBool()) {
      gameRenderWorld->DebugLine(colorCyan, shoulderOrigin, elbowOrigin);
      gameRenderWorld->DebugLine(colorRed, elbowOrigin, handOrigin);
      gameRenderWorld->DebugLine(colorYellow, elbowOrigin,
                                 elbowOrigin + elbowDir);
      gameRenderWorld->DebugLine(colorGreen, elbowOrigin,
                                 elbowOrigin + shoulderDir);
    }

    // get the axis for the shoulder joint
    GetBoneAxis(shoulderOrigin, elbowOrigin, shoulderDir, axis);
    shoulderAxis[i] =
        upperArmToShoulderJoint[i] * (axis * modelAxis.Transpose());
  ...
  for (i = 0; i < numArms; i++) {
    animator->SetJointAxis(shoulderJoints[i], JOINTMOD_WORLD_OVERRIDE,
                           shoulderAxis[i]);
    animator->SetJointAxis(elbowJoints[i], JOINTMOD_WORLD_OVERRIDE,
                           elbowAxis[i]);
  }

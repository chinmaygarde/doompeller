...
Base class for all game objects.  Provides fast run-time type checking and
...
class idEventArg {
 public:
  ...
  idEventArg(const idStr& data) {
    type = D_EVENT_STRING;
    value = reinterpret_cast<int64_t>(data.c_str());
  };
...
This macro must be included in the definition of any abstract subclass of
idClass. It prototypes variables used in class instanciation and type checking.
...
#define new ID_DEBUG_NEW
  ...
  bool PostEventMS(const idEventDef* ev,
                   int time,
                   idEventArg arg1,
                   idEventArg arg2);
  bool PostEventMS(const idEventDef* ev,
  ...
  bool PostEventSec(const idEventDef* ev,
                    float time,
                    idEventArg arg1,
                    idEventArg arg2);
  bool PostEventSec(const idEventDef* ev,

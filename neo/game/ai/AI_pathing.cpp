...
============
*/
bool LineIntersectsPath(const idVec2& start,
                        const idVec2& end,
  ...
  while (node->parent) {
    d1 = plane1.x * node->parent->pos.x + plane1.y * node->parent->pos.y +
         plane1.z;
...
int PointInsideObstacle(const obstacle_t* obstacles,
  ...
  for (i = 0; i < numObstacles; i++) {
    ...
    if (point.x < bounds[0].x || point.y < bounds[0].y ||
        point.x > bounds[1].x || point.y > bounds[1].y) {
      continue;
    }
...
void GetPointOutsideObstacles(const obstacle_t* obstacles,
  ...
  for (i = 0; i < w.GetNumPoints(); i++) {
    plane = idWinding2D::Plane2DFromPoints(w[(i + 1) % w.GetNumPoints()], w[i],
                                           true);
  ...
  for (i = queue[0]; queueStart < queueEnd; i = queue[++queueStart]) {
    ...
    for (j = 0; j < numObstacles; j++) {
      ...
      if (obstacles[j].bounds[0].x > obstacles[i].bounds[1].x ||
          obstacles[j].bounds[0].y > obstacles[i].bounds[1].y ||
          obstacles[j].bounds[1].x < obstacles[i].bounds[0].x ||
...
bool GetFirstBlockingObstacle(const obstacle_t* obstacles,
  ...
  for (i = 0; i < numObstacles; i++) {
    ...
    if (bounds[0].x > obstacles[i].bounds[1].x ||
        bounds[0].y > obstacles[i].bounds[1].y ||
        bounds[1].x < obstacles[i].bounds[0].x ||
        bounds[1].y < obstacles[i].bounds[0].y) {
      continue;
    }
    if (obstacles[i].winding.RayIntersection(startPos, delta, scale1, scale2,
                                             edgeNums)) {
      if (scale1 < blockingScale && scale1 * dist > -0.01f &&
          scale2 * dist > 0.01f) {
...
int GetObstacles(const idPhysics* physics,
  ...
  int i, j, numListedClipModels, numObstacles, numVerts, clipMask,
      blockingObstacle, blockingEdgeNum;
  int wallEdges[MAX_AAS_WALL_EDGES], numWallEdges, verts[2], lastVerts[2],
      nextVerts[2];
  float stepHeight, headHeight, blockingScale, min, max;
  idVec3 seekDelta, silVerts[32], start, end, nextStart, nextEnd;
  idVec2 expBounds[2], edgeDir, edgeNormal, nextEdgeDir, nextEdgeNormal,
      lastEdgeNormal;
  idVec2 obDelta;
  idPhysics* obPhys;
  idBox box;
  ...
  expBounds[0] =
      physics->GetBounds()[0].ToVec2() - idVec2(CM_BOX_EPSILON, CM_BOX_EPSILON);
  expBounds[1] =
      physics->GetBounds()[1].ToVec2() + idVec2(CM_BOX_EPSILON, CM_BOX_EPSILON);

  physics->GetAbsBounds().AxisProjection(-physics->GetGravityNormal(),
                                         stepHeight, headHeight);
  ...
  for (i = 0; i < numListedClipModels && numObstacles < MAX_OBSTACLES; i++) {
    ...
    clipModel->GetAbsBounds().AxisProjection(-physics->GetGravityNormal(), min,
                                             max);
    ...
    box = idBox(clipModel->GetBounds(), clipModel->GetOrigin(),
                clipModel->GetAxis());
    numVerts = box.GetParallelProjectionSilhouetteVerts(
        physics->GetGravityNormal(), silVerts);
    ...
    if (ai_showObstacleAvoidance.GetBool()) {
      ...
      for (j = 0; j < numVerts; j++) {
        gameRenderWorld->DebugArrow(colorWhite, silVerts[j],
                                    silVerts[(j + 1) % numVerts], 4);
      }
    }
  ...
  if (PointInsideObstacle(obstacles, numObstacles, startPos.ToVec2()) == -1) {
    if (!GetFirstBlockingObstacle(
            obstacles, numObstacles, -1, startPos.ToVec2(), seekDelta.ToVec2(),
  ...
  if (aas) {
    ...
    numWallEdges = aas->GetWallEdges(areaNum, clipBounds, TFL_WALK, wallEdges,
                                     MAX_AAS_WALL_EDGES);
    ...
    for (i = 0; i < numWallEdges && numObstacles < MAX_OBSTACLES; i++) {
      ...
      obstacle.winding.AddPoint(start.ToVec2() - edgeDir -
                                edgeNormal * halfBoundsSize);
      obstacle.winding.AddPoint(end.ToVec2() + edgeDir -
                                edgeNormal * halfBoundsSize);
    ...
    for (i = 0; i < numObstacles; i++) {
      ...
      for (j = 0; j < obstacle.winding.GetNumPoints(); j++) {
        gameRenderWorld->DebugArrow(
            colorGreen, silVerts[j],
            silVerts[(j + 1) % obstacle.winding.GetNumPoints()], 4);
      }
    }
  }
  ...
  for (node = root; node; node = node->next) {
    for (i = 0; i < 2; i++) {
      if (node->children[i]) {
        ...
        gameRenderWorld->DebugArrow(node->edgeNum == -1 ? colorYellow
                                    : i                 ? colorBlue
                                                        : colorRed,
                                    start, end, 1);
        break;
...
bool GetPathNodeDelta(pathNode_t* node,
  ...
  while (1) {
    ...
    node->edgeNum =
        (node->edgeNum + numPoints + (2 * node->dir - 1)) % numPoints;
  }
  ...
  if (!blocked) {
    ...
    facing = ((2 * node->dir - 1) * (node->delta.x * seekDelta.y -
                                     node->delta.y * seekDelta.x)) >= 0.0f;
...
pathNode_t* BuildPathTree(const obstacle_t* obstacles,
                          ...
                          const idVec2& seekPos,
                          obstaclePath_t& path) {
  int blockingEdgeNum, blockingObstacle, obstaclePoints,
      bestNumNodes = MAX_OBSTACLE_PATH;
  float blockingScale;
  pathNode_t *root, *node, *child;
  // gcc 4.0
  idQueueTemplate<pathNode_t, offsetof(pathNode_t, next)> pathNodeQueue,
      treeQueue;
  ...
  for (node = pathNodeQueue.Get();
       node && pathNodeAllocator.GetAllocCount() < MAX_PATH_NODES;
    ...
    if (endPos.x - CLIP_BOUNDS_EPSILON < clipBounds[0].x ||
        endPos.x + CLIP_BOUNDS_EPSILON > clipBounds[1].x ||
        endPos.y - CLIP_BOUNDS_EPSILON < clipBounds[0].y ||
        endPos.y + CLIP_BOUNDS_EPSILON > clipBounds[1].y) {
      continue;
    }

    // if an obstacle is blocking the path
    if (GetFirstBlockingObstacle(obstacles, numObstacles, node->obstacle,
                                 node->pos, node->delta, blockingScale,
                                 blockingObstacle, blockingEdgeNum)) {
      ...
      if (node->edgeNum == -1) {
        ...
        node->children[0]->pos = node->children[1]->pos =
            node->pos + node->delta;
      ...
      child->edgeNum = (node->edgeNum + obstaclePoints + (2 * node->dir - 1)) %
                       obstaclePoints;
...
void PrunePathTree(pathNode_t* root, const idVec2& seekPos) {
  ...
  while (node) {
    ...
    if (node->children[0]) {
      ...
      for (lastNode = bestNode, node = bestNode->parent; node;
           lastNode = node, node = node->parent) {
...
int OptimizePath(const pathNode_t* root,
  ...
  for (nextNode = curNode = root; curNode != leafNode; curNode = nextNode) {
    for (nextNode = leafNode; nextNode->parent != curNode;
      ...
      for (i = 0; i < numObstacles; i++) {
        ...
        if (obstacles[i].winding.RayIntersection(curPos, curDelta, scale1,
                                                 scale2, edgeNums)) {
          if (scale1 >= 0.0f && scale1 <= 1.0f &&
              (i != nextNode->obstacle ||
               scale1 * curLength < curLength - 0.5f)) {
            break;
          }
          if (scale2 >= 0.0f && scale2 <= 1.0f &&
              (i != nextNode->obstacle ||
               scale2 * curLength < curLength - 0.5f)) {
            break;
          }
        }
      }
...
bool FindOptimalPath(const pathNode_t* root,
  ...
  while (node) {
    ...
    if (node->dist <= bestNode->dist) {
      if (idMath::Fabs(node->dist - bestNode->dist) < 0.1f) {
        if (!optimizedPathCalculated) {
          bestNumPathPoints = OptimizePath(root, bestNode, obstacles,
                                           numObstacles, optimizedPath);
          bestPathLength =
              PathLength(optimizedPath, bestNumPathPoints, curDir.ToVec2());
    ...
    if (node->children[0]) {
      ...
      for (lastNode = node, node = node->parent; node;
           lastNode = node, node = node->parent) {
        if (node->children[1] && node->children[1] != lastNode) {
          node = node->children[1];
          break;
...
bool idAI::FindPathAroundObstacles(const idPhysics* physics,
  ...
  areaNum =
      aas->PointReachableAreaNum(path.startPosOutsideObstacles, bounds,
                                 (AREA_REACHABLE_WALK | AREA_REACHABLE_FLY));
  aas->PushPointIntoAreaNum(areaNum, path.startPosOutsideObstacles);

  // get all the nearby obstacles
  numObstacles = GetObstacles(
      physics, aas, ignore, areaNum, path.startPosOutsideObstacles,
      path.seekPosOutsideObstacles, obstacles, MAX_OBSTACLES, clipBounds);

  // get a source position outside the obstacles
  GetPointOutsideObstacles(obstacles, numObstacles,
  ...
  // around the obstacle
  if ((path.seekPosOutsideObstacles.ToVec2() -
       path.startPosOutsideObstacles.ToVec2())
  ...
  // build a path tree
  root = BuildPathTree(obstacles, numObstacles, clipBounds,
                       path.startPosOutsideObstacles.ToVec2(),
                       path.seekPosOutsideObstacles.ToVec2(), path);
  ...
  pathToGoalExists =
      FindOptimalPath(root, obstacles, numObstacles, physics->GetOrigin().z,
                      physics->GetLinearVelocity(), path.seekPos);
...
bool PathTrace(const idEntity* ent,
  ...
  if (!aas || !aas->GetSettings()) {
    ...
    gameLocal.clip.TranslationEntities(
    ...
    for (j = 0; j < MAX_FRAME_SLIDE; j++) {
      ...
      for (step = 0; step < 3; step++) {
        ...
        if (step) {
          ...
          // floor surface
          if ((lastEnd - start).LengthSqr() >
                  (trace.endPos - start).LengthSqr() - 0.1f ||
        ...
        if (PathTrace(ent, aas, curStart, curStart + stepUp, stopEvent, trace,
                      path)) {
          return true;
        }
...
static int Ballistics(const idVec3& start,
                      const idVec3& end,
                      float speed,
                      float gravity,
                      ballistics_t bal[2]) {
...
=====================
*/
static float HeightForTrajectory(const idVec3& start,
                                 float zVel,
                                 float gravity) {
  float maxHeight, t;
...
bool idAI::TestTrajectory(const idVec3& start,
  ...
  if (t < time) {
    ...
    t2 = (time - t) * 0.5f;
    points[1].ToVec2() =
        start.ToVec2() + (end.ToVec2() - start.ToVec2()) * (t2 / time);
    ...
    t2 = time * 0.5f;
    points[1].ToVec2() =
        start.ToVec2() + (end.ToVec2() - start.ToVec2()) * 0.5f;
    points[1].z = start.z + t2 * zVel + 0.5f * gravity * t2 * t2;
  }
  ...
  result = true;
  for (i = 0; i < numSegments; i++) {
    gameLocal.clip.Translation(trace, points[i], points[i + 1], clip,
                               mat3_identity, clipmask, ignore);
    if (trace.fraction < 1.0f) {
  ...
  if (drawtime) {
    if (clip) {
      gameRenderWorld->DebugBounds(result ? colorGreen : colorYellow,
                                   clip->GetBounds().Expand(1.0f), trace.endpos,
                                   drawtime);
    } else {
      idBounds bnds(trace.endpos);
      bnds.ExpandSelf(1.0f);
      gameRenderWorld->DebugBounds(result ? colorGreen : colorYellow, bnds,
                                   vec3_zero, drawtime);
    }
  }
...
bool idAI::PredictTrajectory(const idVec3& firePos,
  ...
  if (targetEntity->GetPhysics()->GetAbsBounds().IntersectsBounds(
          clip->GetBounds().Translate(firePos))) {
  ...
  if (projectileSpeed <= 0.0f || projGravity == vec3_origin) {
    ...
    gameLocal.clip.Translation(trace, firePos, target, clip, mat3_identity,
                               clipmask, ignore);
    ...
    if (drawtime) {
      ...
      gameRenderWorld->DebugBounds(
          (trace.fraction >= 1.0f ||
           (gameLocal.GetTraceEntity(trace) == targetEntity))
              ? colorGreen
              : colorYellow,
          bnds, vec3_zero, drawtime);
    }
  ...
  for (i = 0; i < n; i++) {
    ...
    if (TestTrajectory(firePos, target, zVel, projGravity[2],
                       ballistics[i].time, firePos.z + max_height, clip,
                       clipmask, ignore, targetEntity, drawtime)) {
      aimDir = dir[i];
      return true;
    }
  }

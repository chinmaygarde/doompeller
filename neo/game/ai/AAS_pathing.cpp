...
============
*/
bool idAASLocal::FloorEdgeSplitPoint(idVec3& bestSplit,
                                     int areaNum,
...
bool idAASLocal::WalkPathValid(int areaNum,
  ...
  pathPlane.SetNormal(
      (goalOrigin - origin).Cross(file->GetSettings().gravityDir));
  pathPlane.Normalize();
  ...
  while (1) {
    ...
    if (!FloorEdgeSplitPoint(endPos, curAreaNum, pathPlane, frontPlane,
                             false)) {
      endPos = origin;
    }
    ...
    for (reach = area->reach; reach; reach = reach->next) {
      ...
      if (reach->toAreaNum == lastAreas[0] ||
          reach->toAreaNum == lastAreas[1] ||
          reach->toAreaNum == lastAreas[2] ||
          reach->toAreaNum == lastAreas[3]) {
        continue;
      }
      ...
      if (!FloorEdgeSplitPoint(p, reach->toAreaNum, pathPlane, frontPlane,
                               true)) {
        continue;
      }
      ...
      dir =
          (file->GetSettings().gravityDir * endPos *
           file->GetSettings().gravityDir) -
...
idVec3 idAASLocal::SubSampleWalkPath(int areaNum,
  ...
  for (i = 1; i < numSamples; i++) {
    ...
    if (!idAASLocal::WalkPathValid(areaNum, origin, 0, nextPoint, travelFlags,
                                   endPos, curAreaNum)) {
      return point;
    }
    point = nextPoint;
  ...
  for (i = 0; i < maxWalkPathIterations; i++) {
    ...
    if (areaNum != curAreaNum) {
      ...
      if ((reach->start - origin).LengthSqr() > Square(maxWalkPathDistance)) {
        ...
        path.moveGoal =
            SubSampleWalkPath(areaNum, origin, path.moveGoal, reach->start,
                              travelFlags, path.moveAreaNum);
#endif
        return true;
      }

      if (!idAASLocal::WalkPathValid(areaNum, origin, 0, reach->start,
                                     travelFlags, endPos, endAreaNum)) {
#if SUBSAMPLE_WALK_PATH
        path.moveGoal =
            SubSampleWalkPath(areaNum, origin, path.moveGoal, reach->start,
                              travelFlags, path.moveAreaNum);
    ...
    if (!idAASLocal::WalkPathValid(areaNum, origin, 0, reach->end, travelFlags,
                                   endPos, endAreaNum)) {
      return true;
    }
    ...
    if (reach->toAreaNum == goalAreaNum) {
      if (!idAASLocal::WalkPathValid(areaNum, origin, 0, goalOrigin,
                                     travelFlags, endPos, endAreaNum)) {
#if SUBSAMPLE_WALK_PATH
        path.moveGoal =
            SubSampleWalkPath(areaNum, origin, path.moveGoal, goalOrigin,
                              travelFlags, path.moveAreaNum);
#endif
        return true;
      }
    ...
    if (curAreaNum == lastAreas[0] || curAreaNum == lastAreas[1] ||
      ...
      common->Warning(
          "idAASLocal::WalkPathToGoal: local routing minimum going from area "
          "%d to area %d",
          areaNum, goalAreaNum);
      break;
    }
...
idVec3 idAASLocal::SubSampleFlyPath(int areaNum,
  ...
  for (i = 1; i < numSamples; i++) {
    ...
    if (!idAASLocal::FlyPathValid(areaNum, origin, 0, nextPoint, travelFlags,
                                  endPos, curAreaNum)) {
      return point;
    }
    point = nextPoint;
  ...
  for (i = 0; i < maxFlyPathIterations; i++) {
    ...
    if (areaNum != curAreaNum) {
      if ((reach->start - origin).LengthSqr() > Square(maxFlyPathDistance)) {
        ...
        path.moveGoal =
            SubSampleFlyPath(areaNum, origin, path.moveGoal, reach->start,
                             travelFlags, path.moveAreaNum);
#endif
        return true;
      }

      if (!idAASLocal::FlyPathValid(areaNum, origin, 0, reach->start,
                                    travelFlags, endPos, endAreaNum)) {
#if SUBSAMPLE_FLY_PATH
        path.moveGoal =
            SubSampleFlyPath(areaNum, origin, path.moveGoal, reach->start,
    ...
    if (!idAASLocal::FlyPathValid(areaNum, origin, 0, reach->end, travelFlags,
                                  endPos, endAreaNum)) {
      return true;
    }
    ...
    if (reach->toAreaNum == goalAreaNum) {
      if (!idAASLocal::FlyPathValid(areaNum, origin, 0, goalOrigin, travelFlags,
                                    endPos, endAreaNum)) {
#if SUBSAMPLE_FLY_PATH
        path.moveGoal =
            SubSampleFlyPath(areaNum, origin, path.moveGoal, goalOrigin,
                             travelFlags, path.moveAreaNum);
#endif
        return true;
      }
    ...
    if (curAreaNum == lastAreas[0] || curAreaNum == lastAreas[1] ||
        curAreaNum == lastAreas[2] || curAreaNum == lastAreas[3]) {
      common->Warning(
          "idAASLocal::FlyPathToGoal: local routing minimum going from area %d "
          "to area %d",
          areaNum, goalAreaNum);
      break;
    }
...
int idAASLocal::GetWallEdges(int areaNum,
  ...
  for (curArea = areaNum; queueStart < queueEnd;
    ...
    for (reach = area->reach; reach; reach = reach->next) {
      if (reach->travelType & travelFlags) {
        ...
        if (!areasVisited[reach->toAreaNum] &&
            bounds.IntersectsBounds(file->GetArea(reach->toAreaNum).bounds)) {

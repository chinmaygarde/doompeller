...
int idRoutingCache::Size(void) const {
  return sizeof(idRoutingCache) + size * sizeof(reachabilities[0]) +
         size * sizeof(travelTimes[0]);
}
...
void idAASLocal::CalculateAreaTravelTimes(void) {
  ...
  for (n = 0; n < file->GetNumAreas(); n++) {
    ...
    numRevReach = 0;
    for (rev_reach = file->GetArea(n).rev_reach; rev_reach;
  ...
  areaTravelTimes =
      (unsigned short*)Mem_Alloc(numAreaTravelTimes * sizeof(unsigned short));
  ...
  for (n = 0; n < file->GetNumAreas(); n++) {
    ...
    for (maxt = i = 0, reach = file->GetArea(n).reach; reach;
         reach = reach->next, i++) {
      ...
      for (j = 0, rev_reach = file->GetArea(n).rev_reach; rev_reach;
           rev_reach = rev_reach->rev_next, j++) {
  ...
  assert(((unsigned int)bytePtr - (unsigned int)areaTravelTimes) <=
         numAreaTravelTimes * sizeof(unsigned short));
}
...
void idAASLocal::SetupRoutingCache(void) {
  ...
  areaCacheIndex = (idRoutingCache***)Mem_ClearedAlloc(
      file->GetNumClusters() * sizeof(idRoutingCache**) +
  ...
  portalCacheIndex = (idRoutingCache**)Mem_ClearedAlloc(
      portalCacheIndexSize * sizeof(idRoutingCache*));

  areaUpdate = (idRoutingUpdate*)Mem_ClearedAlloc(file->GetNumAreas() *
                                                  sizeof(idRoutingUpdate));
  portalUpdate = (idRoutingUpdate*)Mem_ClearedAlloc(
      (file->GetNumPortals() + 1) * sizeof(idRoutingUpdate));

  goalAreaTravelTimes = (unsigned short*)Mem_ClearedAlloc(
      file->GetNumAreas() * sizeof(unsigned short));
...
void idAASLocal::DeleteClusterCache(int clusterNum) {
  ...
  for (i = 0; i < file->GetCluster(clusterNum).numReachableAreas; i++) {
    for (cache = areaCacheIndex[clusterNum][i]; cache;
...
void idAASLocal::RoutingStats(void) const {
  ...
  for (cache = cacheListStart; cache; cache = cache->time_next) {
    if (cache->type == CACHETYPE_AREA) {
      numAreaCache++;
      totalAreaCacheMemory +=
          sizeof(idRoutingCache) +
          cache->size * (sizeof(unsigned short) + sizeof(byte));
    } else {
      numPortalCache++;
      totalPortalCacheMemory +=
          sizeof(idRoutingCache) +
          cache->size * (sizeof(unsigned short) + sizeof(byte));
    }
  }
...
bool idAASLocal::SetAreaState_r(int nodeNum,
  ...
  while (nodeNum != 0) {
    ...
    if (res == PLANESIDE_BACK) {
      ...
      foundClusterPortal |=
          SetAreaState_r(node->children[1], bounds, areaContents, disabled);
      nodeNum = node->children[0];
    }
  }
...
void idAASLocal::SetObstacleState(const idRoutingObstacle* obstacle,
  ...
  for (i = 0; i < obstacle->areas.Num(); i++) {
    ...
    for (rev_reach = area->rev_reach; rev_reach;
         rev_reach = rev_reach->rev_next) {
...
void idAASLocal::DeleteOldestCache(void) const {
  ...
  if (cache->prev) {
    ...
    areaCacheIndex[cache->cluster][ClusterAreaNum(
        cache->cluster, cache->areaNum)] = cache->next;
...
void idAASLocal::UpdateAreaRoutingCache(idRoutingCache* areaCache) const {
  int i, nextAreaNum, cluster, badTravelFlags, clusterAreaNum,
      numReachableAreas;
  ...
  while (updateListStart) {
    ...
    for (i = 0, reach = file->GetArea(curUpdate->areaNum).rev_reach; reach;
         reach = reach->rev_next, i++) {
      ...
      t = curUpdate->tmpTravelTime + curUpdate->areaTravelTimes[i] +
          reach->travelTime;

      if (!areaCache->travelTimes[clusterAreaNum] ||
          t < areaCache->travelTimes[clusterAreaNum]) {
...
void idAASLocal::UpdatePortalRoutingCache(idRoutingCache* portalCache) const {
  ...
  while (updateListStart) {
    ...
    cache = GetAreaRoutingCache(curUpdate->cluster, curUpdate->areaNum,
    ...
    for (i = 0; i < cluster->numPortals; i++) {
      ...
      if (!portalCache->travelTimes[portalNum] ||
        ...
        portalCache->reachabilities[portalNum] =
...
bool idAASLocal::RouteToGoalArea(int areaNum,
  ...
  *reach = NULL;
  ...
  if (clusterNum < 0) {
    ...
    portalCache =
        GetPortalRoutingCache(goalClusterNum, goalAreaNum, travelFlags);
    *reach =
        GetAreaReachability(areaNum, portalCache->reachabilities[-clusterNum]);
    travelTime = portalCache->travelTimes[-clusterNum] +
                 AreaTravelTime(areaNum, origin, (*reach)->start);
    return true;
  }
  ...
  if (clusterNum > 0 && goalClusterNum > 0 && clusterNum == goalClusterNum) {
    ...
    if (clusterCache->travelTimes[clusterAreaNum]) {
      bestReach = GetAreaReachability(
          areaNum, clusterCache->reachabilities[clusterAreaNum]);
      bestTime = clusterCache->travelTimes[clusterAreaNum] +
                 AreaTravelTime(areaNum, origin, bestReach->start);
    } else {
      clusterCache = NULL;
    }
  } else {
  ...
  for (i = 0; i < cluster->numPortals; i++) {
    ...
    if (clusterCache) {
      ...
      nextr = GetAreaReachability(portal->areaNum,
                                  portalCache->reachabilities[portalNum]);
      if (file->GetArea(nextr->toAreaNum).cluster < 0 ||
          file->GetArea(nextr->toAreaNum).cluster == clusterNum) {
        continue;
      }
    }

    // the total travel time is the travel time from the portal area to the goal
    // area plus the travel time from the source area towards the portal area
    t = portalCache->travelTimes[portalNum] +
    ...
    //portal area. 			We cannot directly calculate the exact travel time through
    //the portal area 			because the reachability used to travel into the portal
    //area is not known.
...
int idAASLocal::TravelTimeToGoalArea(int areaNum,
  ...
  if (!RouteToGoalArea(areaNum, origin, goalAreaNum, travelFlags, travelTime,
                       &reach)) {
    return 0;
  }
  return travelTime;
...
bool idAASLocal::FindNearestGoal(aasGoal_t& goal,
  ...
  for (k = 0; k < numObstacles; k++) {
    obstacles[k].expAbsBounds[0] =
        obstacles[k].absBounds[0] - file->GetSettings().boundingBoxes[0][1];
    obstacles[k].expAbsBounds[1] =
        obstacles[k].absBounds[1] - file->GetSettings().boundingBoxes[0][0];
  }

  badTravelFlags = ~travelFlags;
  SIMDProcessor->Memset(goalAreaTravelTimes, 0,
                        file->GetNumAreas() * sizeof(unsigned short));
  ...
  while (updateListStart) {
    ...
    for (i = 0, reach = file->GetArea(curUpdate->areaNum).reach; reach;
         reach = reach->next, i++) {
      // if the reachability uses an undesired travel type
      if (reach->travelType & badTravelFlags) {
        continue;
      ...
      for (j = 0; j < 3; j++) {
        if ((p[j] > curUpdate->start[j] + 0.1f &&
             p[j] > reach->end[j] + 0.1f) ||
            (p[j] < curUpdate->start[j] - 0.1f &&
             p[j] < reach->end[j] - 0.1f)) {
          break;
        }
      }
      ...
      if (goalAreaTravelTimes[nextAreaNum] &&
          t >= goalAreaTravelTimes[nextAreaNum]) {
        continue;
      }
      ...
      for (k = 0; k < numObstacles; k++) {
        ...
        if (obstacles[k].expAbsBounds.LineIntersection(curUpdate->start,
                                                       reach->end)) {
          break;
        }
      }

...
// if set to 1 the server sends the client PVS with snapshots and the client
// compares against what it sees
...
class idGameLocal : public idGame {
 public:
  ...
  idDict
      persistentLevelInfo;  // contains args that are kept around between levels
  ...
  static const int msec =
      USERCMD_MSEC;  // time since last update in milliseconds
  ...
  static const char*
      sufaceTypeNames[MAX_SURFACE_TYPES];  // text names for surface types

  idEntityPtr<idEntity>
      lastGUIEnt;  // last entity with a GUI, used by Cmd_NextGUI_f
  ...
  void SetAASAreaState(const idBounds& bounds,
  ...
  idEntity* SpawnEntityType(const idTypeInfo& classdef,
  ...
  const idDict* FindEntityDefDict(const char* name,
                                  bool makeDefault = true) const;

  void RegisterEntity(idEntity* ent);
  void UnregisterEntity(idEntity* ent);

  bool RequirementMet(idEntity* activator,
  ...
  int GetTargets(const idDict& args,
  ...
  static void ArgCompletion_EntityName(const idCmdArgs& args,
                                       void (*callback)(const char* s));
  idEntity* FindTraceEntity(idVec3 start,
                            idVec3 end,
                            const idTypeInfo& c,
                            const idEntity* skip) const;
  idEntity* FindEntity(const char* name) const;
  idEntity* FindEntityUsingDef(idEntity* from, const char* match) const;
  int EntitiesWithinRadius(const idVec3 org,
                           float radius,
                           idEntity** entityList,
                           int maxCount) const;

  void KillBox(idEntity* ent, bool catch_teleport = false);
  void RadiusDamage(const idVec3& origin,
                    idEntity* inflictor,
                    idEntity* attacker,
                    ...
                    float dmgPower = 1.0f);
  void RadiusPush(const idVec3& origin,
                  const float radius,
                  const float push,
                  ...
                  const bool quake);
  void RadiusPushClipModel(const idVec3& origin,
                           const float push,
                           const idClipModel* clipModel);

  void ProjectDecal(const idVec3& origin,
                    const idVec3& dir,
                    float depth,
                    bool parallel,
                    float size,
                    const char* material,
                    float angle = 0);
  void BloodSplat(const idVec3& origin,
                  const idVec3& dir,
                  float size,
                  const char* material);
  ...
  void SaveEntityNetworkEvent(const idEntity* ent,
                              int event,
                              const idBitMsg* msg);
 ...
 private:
  ...
  void GetBestGameType(const char* map,
                       const char* gametype,

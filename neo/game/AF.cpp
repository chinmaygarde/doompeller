...
bool idAF::UpdateAnimation(void) {
  ...
  for (i = 0; i < jointMods.Num(); i++) {
    ...
    axis = jointMods[i].jointBodyAxis.Transpose() *
           (bodyAxis * renderAxis.Transpose());
    origin = (bodyOrigin - jointMods[i].jointBodyOrigin * axis - renderOrigin) *
             renderAxis.Transpose();
    animator->SetAFPoseJointMod(jointMods[i].jointHandle, jointMods[i].jointMod,
                                axis, origin);
  }
  animator->FinishAFPose(
      modifiedAnim, GetBounds().Expand(POSE_BOUNDS_EXPANSION), gameLocal.time);
...
void idAF::SetupPose(idEntity* ent, int time) {
  ...
  for (i = 0; i < jointMods.Num(); i++) {
    body = physicsObj.GetBody(jointMods[i].bodyId);
    animatorPtr->GetJointTransform(jointMods[i].jointHandle, time, origin,
                                   axis);
    body->SetWorldOrigin(renderEntity->origin +
                         (origin + jointMods[i].jointBodyOrigin * axis) *
                             renderEntity->axis);
...
int idAF::EntitiesTouchingAF(afTouch_t touchList[MAX_GENTITIES]) const {
  ...
  numClipModels = gameLocal.clip.ClipModelsTouchingBounds(
      physicsObj.GetAbsBounds(), -1, clipModels, MAX_GENTITIES);
  ...
  for (i = 0; i < jointMods.Num(); i++) {
    ...
    for (j = 0; j < numClipModels; j++) {
      ...
      if (!body->GetClipModel()->GetAbsBounds().IntersectsBounds(
              cm->GetAbsBounds())) {
        continue;
      }

      if (gameLocal.clip.ContentsModel(body->GetWorldOrigin(),
                                       body->GetClipModel(),
                                       body->GetWorldAxis(), -1, cm->Handle(),
                                       cm->GetOrigin(), cm->GetAxis())) {
...
void idAF::AddBody(idAFBody* body,
  ...
  if (handle == INVALID_JOINT) {
    gameLocal.Error("idAF for entity '%s' at (%s) modifies unknown joint '%s'",
                    self->name.c_str(),
  ...
  jointMods[index].jointBodyOrigin =
      (body->GetWorldOrigin() - origin) * axis.Transpose();
...
bool idAF::LoadBody(const idDeclAF_Body* fb, const idJointMat* joints) {
  ...
  if (fb->linearFriction != -1.0f) {
    body->SetFriction(fb->linearFriction, fb->angularFriction,
                      fb->contactFriction);
  }
  ...
  for (i = 0; i < jointList.Num(); i++) {
    if (jointBody[jointList[i]] != -1) {
      gameLocal.Warning(
          "%s: joint '%s' is already contained by body '%s'", name.c_str(),
...
bool idAF::LoadConstraint(const idDeclAF_Constraint* fc) {
  ...
  switch (fc->type) {
    case DECLAF_CONSTRAINT_FIXED: {
      ...
      c = static_cast<idAFConstraint_Fixed*>(
          physicsObj.GetConstraint(fc->name));
      if (c) {
        c->SetBody1(body1);
        c->SetBody2(body2);
      } else {
    ...
    case DECLAF_CONSTRAINT_BALLANDSOCKETJOINT: {
      ...
      c = static_cast<idAFConstraint_BallAndSocketJoint*>(
          physicsObj.GetConstraint(fc->name));
      if (c) {
        c->SetBody1(body1);
        c->SetBody2(body2);
      ...
      switch (fc->limit) {
        ...
        case idDeclAF_Constraint::LIMIT_PYRAMID: {
          ...
          axis = angles.ToMat3();
          c->SetPyramidLimit(axis[0], axis[1], fc->limitAngles[0],
                             fc->limitAngles[1], fc->shaft[0].ToVec3());
          break;
        }
        default: {
          c->SetNoLimit();
    ...
    case DECLAF_CONSTRAINT_UNIVERSALJOINT: {
      ...
      c = static_cast<idAFConstraint_UniversalJoint*>(
          physicsObj.GetConstraint(fc->name));
      if (c) {
        c->SetBody1(body1);
        c->SetBody2(body2);
    ...
    case DECLAF_CONSTRAINT_HINGE: {
      ...
      c = static_cast<idAFConstraint_Hinge*>(
          physicsObj.GetConstraint(fc->name));
      if (c) {
        c->SetBody1(body1);
        c->SetBody2(body2);
      ...
      switch (fc->limit) {
        case idDeclAF_Constraint::LIMIT_CONE: {
          ...
          axis = left *
                 idRotation(vec3_origin, fc->axis.ToVec3(), fc->limitAngles[0]);
          shaft = left * idRotation(vec3_origin, fc->axis.ToVec3(),
                                    fc->limitAngles[2]);
    ...
    case DECLAF_CONSTRAINT_SLIDER: {
      ...
      c = static_cast<idAFConstraint_Slider*>(
          physicsObj.GetConstraint(fc->name));
      if (c) {
        c->SetBody1(body1);
        c->SetBody2(body2);
    ...
    case DECLAF_CONSTRAINT_SPRING: {
      ...
      c = static_cast<idAFConstraint_Spring*>(
          physicsObj.GetConstraint(fc->name));
      if (c) {
        c->SetBody1(body1);
        c->SetBody2(body2);
...
static bool GetJointTransform(void* model,
  ...
  if ((joint >= 0) &&
      (joint < reinterpret_cast<idAnimator*>(model)->NumJoints())) {
...
bool idAF::Load(idEntity* ent, const char* fileName) {
  ...
  if (file->bodies.Num() == 0 || file->bodies[0]->jointName != "origin") {
    gameLocal.Warning(
        "idAF::Load: articulated figure '%s' for entity '%s' at (%s) has no "
        "body which modifies the origin joint.",
        name.c_str(), ent->name.c_str(),
        ent->GetPhysics()->GetOrigin().ToString(0));
    return false;
  }
  ...
  if (modelDef == NULL || modelDef->GetState() == DS_DEFAULTED) {
    gameLocal.Warning(
        "idAF::Load: articulated figure '%s' for entity '%s' at (%s) has no or "
        "defaulted modelDef '%s'",
        name.c_str(), ent->name.c_str(),
        ent->GetPhysics()->GetOrigin().ToString(0),
  ...
  if (model == NULL || model->IsDefaultModel()) {
    gameLocal.Warning(
        ...
        "defaulted model '%s'",
        name.c_str(), ent->name.c_str(),
        ent->GetPhysics()->GetOrigin().ToString(0), model ? model->Name() : "");
    return false;
  }
  ...
  if (!modifiedAnim) {
    gameLocal.Warning(
        "idAF::Load: articulated figure '%s' for entity '%s' at (%s) has no "
        "modified animation '%s'",
        name.c_str(), ent->name.c_str(),
        ent->GetPhysics()->GetOrigin().ToString(0), ARTICULATED_FIGURE_ANIM);
    return false;
  }
  ...
  gameEdit->ANIM_CreateAnimFrame(
      model, animator->GetAnim(modifiedAnim)->MD5Anim(0), numJoints, joints, 1,
  ...
  physicsObj.SetDefaultFriction(file->defaultLinearFriction,
  ...
  physicsObj.SetSuspendTolerance(file->noMoveTime, file->noMoveTranslation,
                                 file->noMoveRotation);
  ...
  for (i = 0; i < animator->NumJoints(); i++) {
    if (jointBody[i] == -1) {
      gameLocal.Warning(
          "idAF::Load: articulated figure '%s' for entity '%s' at (%s) joint "
          "'%s' is not contained by a body",
          name.c_str(), self->name.c_str(),
...
bool idAF::TestSolid(void) const {
  ...
  for (i = 0; i < physicsObj.GetNumBodies(); i++) {
    ...
    if (gameLocal.clip.Translation(trace, body->GetWorldOrigin(),
                                   body->GetWorldOrigin(), body->GetClipModel(),
                                   body->GetWorldAxis(), body->GetClipMask(),
                                   self)) {
      float depth = idMath::Fabs(trace.c.point * trace.c.normal - trace.c.dist);

      body->SetWorldOrigin(body->GetWorldOrigin() +
                           trace.c.normal * (depth + 8.0f));

      gameLocal.DWarning(
          "%s: body '%s' stuck in %d (normal = %.2f %.2f %.2f, depth = %.2f)",
          self->name.c_str(), body->GetName().c_str(), trace.c.contents,
          trace.c.normal.x, trace.c.normal.y, trace.c.normal.z, depth);
      solid = true;
    }
  }
  return solid;
...
void idAF::SetConstraintPosition(const char* name, const idVec3& pos) {
  ...
  switch (constraint->GetType()) {
    case CONSTRAINT_BALLANDSOCKETJOINT: {
      idAFConstraint_BallAndSocketJoint* bs =
          static_cast<idAFConstraint_BallAndSocketJoint*>(constraint);
      bs->Translate(pos - bs->GetAnchor());
      break;
    }
    case CONSTRAINT_UNIVERSALJOINT: {
      idAFConstraint_UniversalJoint* uj =
          static_cast<idAFConstraint_UniversalJoint*>(constraint);
      uj->Translate(pos - uj->GetAnchor());
      break;
    }
    case CONSTRAINT_HINGE: {
      idAFConstraint_Hinge* hinge =
          static_cast<idAFConstraint_Hinge*>(constraint);
...
void idAF::LoadState(const idDict& args) {
  ...
  while (kv) {
    ...
    if (body) {
      sscanf(kv->GetValue(), "%f %f %f %f %f %f", &origin.x, &origin.y,
             &origin.z, &angles.pitch, &angles.yaw, &angles.roll);
      body->SetWorldOrigin(origin);
      body->SetWorldAxis(angles.ToMat3());
    } else {
      gameLocal.Warning("Unknown body part %s in articulated figure %s",
...
void idAF::AddBindConstraints(void) {
  ...
  for (kv = args.MatchPrefix("bindConstraint ", NULL); kv;
    ...
    if (!body) {
      gameLocal.Warning(
          ...
          bodyName.c_str(), self->name.c_str());
      lexer.FreeSource();
      continue;
    }
    ...
    if (type.Icmp("fixed") == 0) {
      ...
      gameLocal.Warning(
          "idAF::AddBindConstraints: unknown constraint type '%s' on entity "
          "'%s'",

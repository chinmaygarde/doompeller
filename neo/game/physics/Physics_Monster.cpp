...
void idPhysics_Monster::CheckGround(monsterPState_t& state) {
  ...
  gameLocal.clip.Translation(groundTrace, state.origin, down, clipModel,
  ...
  if (groundTrace.c.entityNum != ENTITYNUM_WORLD &&
    ...
    if (info.invMass != 0.0f) {
      groundEntityPtr.GetEntity()->ApplyImpulse(
          self, 0, groundTrace.c.point,
          state.velocity / (info.invMass * 10.0f));
    }
  }
}
...
monsterMoveResult_t idPhysics_Monster::SlideMove(idVec3& start,
  ...
  move = delta;
  for (i = 0; i < 3; i++) {
    gameLocal.clip.Translation(tr, start, start + move, clipModel,
                               clipModel->GetAxis(), clipMask, self);
...
monsterMoveResult_t idPhysics_Monster::StepMove(idVec3& start,
  ...
  if (result1 == MM_OK) {
    ...
    gameLocal.clip.Translation(tr, noStepPos, down, clipModel,
                               clipModel->GetAxis(), clipMask, self);
    if (tr.fraction < 1.0f) {
      start = tr.endpos;
      return MM_STEPPED;
  ...
  gameLocal.clip.Translation(tr, start, up, clipModel, clipModel->GetAxis(),
                             clipMask, self);
  if (tr.fraction == 0.0f) {
    start = noStepPos;
    velocity = noStepVel;
  ...
  gameLocal.clip.Translation(tr, stepPos, down, clipModel, clipModel->GetAxis(),
                             clipMask, self);
  stepPos = tr.endpos;
  ...
  if ((nostepdist >= stepdist) ||
      ((tr.c.normal * -gravityNormal) < minFloorCosine)) {
    start = noStepPos;
    velocity = noStepVel;
    return MM_SLIDING;
  }
...
bool idPhysics_Monster::Evaluate(int timeStepMSec, int endTimeMSec) {
  ...
  if (fly || (!forceDeltaMove && (!current.onGround || upspeed > 1.0f))) {
    ...
    if (delta != vec3_origin) {
      moveResult =
          idPhysics_Monster::SlideMove(current.origin, current.velocity, delta);
      delta.Zero();
    }
    ...
    if (delta == vec3_origin) {
      ...
      moveResult =
          idPhysics_Monster::StepMove(current.origin, current.velocity, delta);
      delta.Zero();
    }
  }
...
void idPhysics_Monster::SetPushed(int deltaTime) {
  ...
  current.pushVelocity +=
      (current.origin - saved.origin) / (deltaTime * idMath::M_MS2SEC);
}
...
void idPhysics_Monster::WriteToSnapshot(idBitMsgDelta& msg) const {
  ...
  msg.WriteFloat(current.velocity[0], MONSTER_VELOCITY_EXPONENT_BITS,
                 MONSTER_VELOCITY_MANTISSA_BITS);
  msg.WriteFloat(current.velocity[1], MONSTER_VELOCITY_EXPONENT_BITS,
                 MONSTER_VELOCITY_MANTISSA_BITS);
  msg.WriteFloat(current.velocity[2], MONSTER_VELOCITY_EXPONENT_BITS,
                 MONSTER_VELOCITY_MANTISSA_BITS);
...
void idPhysics_Monster::ReadFromSnapshot(const idBitMsgDelta& msg) {
  ...
  current.velocity[0] = msg.ReadFloat(MONSTER_VELOCITY_EXPONENT_BITS,
                                      MONSTER_VELOCITY_MANTISSA_BITS);
  current.velocity[1] = msg.ReadFloat(MONSTER_VELOCITY_EXPONENT_BITS,
                                      MONSTER_VELOCITY_MANTISSA_BITS);
  current.velocity[2] = msg.ReadFloat(MONSTER_VELOCITY_EXPONENT_BITS,
                                      MONSTER_VELOCITY_MANTISSA_BITS);
  current.localOrigin[0] = msg.ReadDeltaFloat(current.origin[0]);
  current.localOrigin[1] = msg.ReadDeltaFloat(current.origin[1]);
  current.localOrigin[2] = msg.ReadDeltaFloat(current.origin[2]);
  current.pushVelocity[0] = msg.ReadDeltaFloat(
      0.0f, MONSTER_VELOCITY_EXPONENT_BITS, MONSTER_VELOCITY_MANTISSA_BITS);

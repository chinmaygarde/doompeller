...
const idVec6 vec6_lcp_epsilon = idVec6(LCP_EPSILON,
                                       LCP_EPSILON,
                                       LCP_EPSILON,
                                       LCP_EPSILON,
                                       LCP_EPSILON,
...
void idAFConstraint_Fixed::InitOffset(void) {
  if (body2) {
    offset = (body1->GetWorldOrigin() - body2->GetWorldOrigin()) *
             body2->GetWorldAxis().Transpose();
...
void idAFConstraint_Fixed::Evaluate(float invTimeStep) {
  ...
  c1.SubVec3(0) =
      -(invTimeStep * ERROR_REDUCTION) * (ofs - body1->GetWorldOrigin());
  r = (body1->GetWorldAxis().Transpose() * ax).ToRotation();
  c1.SubVec3(1) = -(invTimeStep * ERROR_REDUCTION) *
                  (r.GetVec() * -(float)DEG2RAD(r.GetAngle()));
...
void idAFConstraint_Fixed::DebugDraw(void) {
  ...
  master = body2 ? body2 : physics->GetMasterBody();
  if (master) {
    gameRenderWorld->DebugLine(colorRed, body1->GetWorldOrigin(),
...
void idAFConstraint_BallAndSocketJoint::SetAnchor(const idVec3& worldPosition) {
  ...
  anchor1 = (worldPosition - body1->GetWorldOrigin()) *
            body1->GetWorldAxis().Transpose();
  if (body2) {
    // get anchor relative to center of mass of body2
    anchor2 = (worldPosition - body2->GetWorldOrigin()) *
              body2->GetWorldAxis().Transpose();
  } else {
...
void idAFConstraint_BallAndSocketJoint::SetConeLimit(const idVec3& coneAxis,
  ...
  if (body2) {
    coneLimit->Setup(body1, body2, anchor2,
                     coneAxis * body2->GetWorldAxis().Transpose(), coneAngle,
                     body1Axis * body1->GetWorldAxis().Transpose());
  } else {
    coneLimit->Setup(body1, body2, anchor2, coneAxis, coneAngle,
                     body1Axis * body1->GetWorldAxis().Transpose());
  }
}
...
void idAFConstraint_BallAndSocketJoint::SetPyramidLimit(
  ...
  if (body2) {
    pyramidLimit->Setup(body1, body2, anchor2,
                        pyramidAxis * body2->GetWorldAxis().Transpose(),
                        baseAxis * body2->GetWorldAxis().Transpose(), angle1,
                        angle2, body1Axis * body1->GetWorldAxis().Transpose());
  } else {
    pyramidLimit->Setup(body1, body2, anchor2, pyramidAxis, baseAxis, angle1,
                        angle2, body1Axis * body1->GetWorldAxis().Transpose());
  }
}
...
void idAFConstraint_BallAndSocketJoint::Evaluate(float invTimeStep) {
  ...
  if (master) {
    ...
    c1.SubVec3(0) =
        -(invTimeStep * ERROR_REDUCTION) *
        (a2 + master->GetWorldOrigin() - (a1 + body1->GetWorldOrigin()));
  } else {
    c1.SubVec3(0) = -(invTimeStep * ERROR_REDUCTION) *
                    (anchor2 - (a1 + body1->GetWorldOrigin()));
  }
...
void idAFConstraint_BallAndSocketJoint::ApplyFriction(float invTimeStep) {
  ...
  if (af_useImpulseFriction.GetBool() || af_useJointImpulseFriction.GetBool()) {
    ...
    body1->SetAngularVelocity(body1->GetAngularVelocity() -
                              angular * body1->GetInverseMass());
    if (body2) {
      body2->SetAngularVelocity(body2->GetAngularVelocity() +
                                angular * body2->GetInverseMass());
    }
  } else {
    if (!fc) {
...
================
*/
void idAFConstraint_BallAndSocketJoint::GetForce(idAFBody* body,
                                                 idVec6& force) {
...
void idAFConstraint_BallAndSocketJoint::DebugDraw(void) {
  ...
  gameRenderWorld->DebugLine(colorBlue, a1 - idVec3(5, 0, 0),
                             a1 + idVec3(5, 0, 0));
  gameRenderWorld->DebugLine(colorBlue, a1 - idVec3(0, 5, 0),
                             a1 + idVec3(0, 5, 0));
  gameRenderWorld->DebugLine(colorBlue, a1 - idVec3(0, 0, 5),
                             a1 + idVec3(0, 0, 5));
...
================
*/
void idAFConstraint_BallAndSocketJointFriction::Setup(
...
================
*/
void idAFConstraint_BallAndSocketJointFriction::ApplyFriction(
    float invTimeStep) {
  // do nothing
}
...
================
*/
void idAFConstraint_BallAndSocketJointFriction::Translate(
...
================
*/
void idAFConstraint_BallAndSocketJointFriction::Rotate(
...
void idAFConstraint_UniversalJoint::SetPyramidLimit(const idVec3& pyramidAxis,
  ...
  if (body2) {
    pyramidLimit->Setup(
        body1, body2, anchor2, pyramidAxis * body2->GetWorldAxis().Transpose(),
...
================
*/
void idAFConstraint_UniversalJointFriction::Setup(
...
================
*/
bool idAFConstraint_UniversalJointFriction::Add(idPhysics_AF* phys,
                                                float invTimeStep) {
...
================
*/
void idAFConstraint_UniversalJointFriction::Translate(
    const idVec3& translation) {}
...
void idAFConstraint_Hinge::SetLimit(const idVec3& axis,
  ...
  if (body2) {
    coneLimit->Setup(body1, body2, anchor2,
                     axis * body2->GetWorldAxis().Transpose(), angle,
                     body1Axis * body1->GetWorldAxis().Transpose());
  } else {
    coneLimit->Setup(body1, body2, anchor2, axis, angle,
                     body1Axis * body1->GetWorldAxis().Transpose());
  }
}
...
float idAFConstraint_Hinge::GetAngle(void) const {
  ...
  axis = body1->GetWorldAxis() * body2->GetWorldAxis().Transpose() *
         initialAxis.Transpose();
...
void idAFConstraint_Hinge::Evaluate(float invTimeStep) {
  ...
  J1.Set(mat3_identity, -SkewSymmetric(a1), mat3_zero,
         idMat3(vecX[0], vecX[1], vecX[2], vecY[0], vecY[1], vecY[2], 0.0f,
                0.0f, 0.0f));
  J1.SetSize(5, 6);

  if (body2) {
    J2.Set(-mat3_identity, SkewSymmetric(a2), mat3_zero,
           idMat3(-vecX[0], -vecX[1], -vecX[2], -vecY[0], -vecY[1], -vecY[2],
                  0.0f, 0.0f, 0.0f));
    J2.SetSize(5, 6);
  } else {
    J2.Zero(5, 6);
...
================
*/
idAFConstraint_Slider::idAFConstraint_Slider(const idStr& name,
                                             idAFBody* body1,
                                             idAFBody* body2) {
  assert(body1);
...
void idAFConstraint_Slider::Evaluate(float invTimeStep) {
  ...
  if (master) {
    ...
    ofs = master->GetWorldOrigin() + master->GetWorldAxis() * offset -
          body1->GetWorldOrigin();
    r = (body1->GetWorldAxis().Transpose() * (relAxis * master->GetWorldAxis()))
            .ToRotation();
  } else {
  ...
  if (body2) {
    J2.Set(mat3_zero, -mat3_identity, idMat3(-vecX, -vecY, vec3_origin),
           mat3_zero);
    J2.SetSize(5, 6);
  } else {
    J2.Zero(5, 6);
...
void idAFConstraint_Slider::GetCenter(idVec3& center) {
  ...
  master = body2 ? body2 : physics->GetMasterBody();
  if (master) {
    center = master->GetWorldOrigin() + master->GetWorldAxis() * offset -
             body1->GetWorldOrigin();
  } else {
...
void idAFConstraint_Slider::DebugDraw(void) {
  ...
  gameRenderWorld->DebugLine(colorGreen, ofs,
                             ofs + axis * body1->GetWorldAxis());
}
...
================
*/
idAFConstraint_Line::idAFConstraint_Line(const idStr& name,
                                         idAFBody* body1,
                                         idAFBody* body2) {
  assert(0);  // FIXME: implement
}
...
================
*/
void idAFConstraint_Plane::SetPlane(const idVec3& normal,
                                    const idVec3& anchor) {
  // get anchor relative to center of mass of body1
  anchor1 =
      (anchor - body1->GetWorldOrigin()) * body1->GetWorldAxis().Transpose();
  if (body2) {
    // get anchor relative to center of mass of body2
    anchor2 =
        (anchor - body2->GetWorldOrigin()) * body2->GetWorldAxis().Transpose();
...
void idAFConstraint_Spring::SetAnchor(const idVec3& worldAnchor1,
  ...
  // get anchor relative to center of mass of body1
  anchor1 = (worldAnchor1 - body1->GetWorldOrigin()) *
            body1->GetWorldAxis().Transpose();
  if (body2) {
    // get anchor relative to center of mass of body2
    anchor2 = (worldAnchor2 - body2->GetWorldOrigin()) *
              body2->GetWorldAxis().Transpose();
  } else {
...
void idAFConstraint_Spring::Evaluate(float invTimeStep) {
  ...
  if (length > restLength) {
    if (kstretch > 0.0f) {
      idVec3 springForce =
          force * (Square(length - restLength) * kstretch - dampingForce);
    ...
    if (kcompress > 0.0f) {
      idVec3 springForce =
          force * -(Square(restLength - length) * kcompress - dampingForce);
      body1->AddForce(a1, springForce);
      if (master) {
        master->AddForce(a2, -springForce);
      }
...
void idAFConstraint_Spring::DebugDraw(void) {
  ...
  if (minLength > 0.0f) {
    // draw min length
    gameRenderWorld->DebugCircle(colorBlue, mid + minLength * 0.5f * dir, dir,
                                 2.0f, 10);
    gameRenderWorld->DebugCircle(colorBlue, mid - minLength * 0.5f * dir, dir,
                                 2.0f, 10);
  }
  ...
  if (maxLength > 0.0f) {
    // draw max length
    gameRenderWorld->DebugCircle(colorRed, mid + maxLength * 0.5f * dir, dir,
                                 2.0f, 10);
    gameRenderWorld->DebugCircle(colorRed, mid - maxLength * 0.5f * dir, dir,
                                 2.0f, 10);
  }
...
void idAFConstraint_Contact::Setup(idAFBody* b1,
  ...
  J1.Set(1, 6, v.ToFloatPtr());
  vel = v.SubVec3(0) * body1->GetLinearVelocity() +
        v.SubVec3(1) * body1->GetAngularVelocity();
  ...
  if (body2) {
    ...
    v.SubVec3(1) = p.Cross(-c.normal);
    J2.Set(1, 6, v.ToFloatPtr());
    vel += v.SubVec3(0) * body2->GetLinearVelocity() +
           v.SubVec3(1) * body2->GetAngularVelocity();
    c2[0] = 0.0f;
  }
...
void idAFConstraint_Contact::ApplyFriction(float invTimeStep) {
  ...
  // seperate friction per contact is silly but it's fast and often looks close
  // enough
  if (af_useImpulseFriction.GetBool()) {
    ...
    velocity =
        body1->GetLinearVelocity() + body1->GetAngularVelocity().Cross(r);
    ...
    forceDenominator =
        body1->GetInverseMass() +
        ((body1->GetInverseWorldInertia() * r.Cross(normal)).Cross(r) * normal);
...
void idAFConstraint_Contact::DebugDraw(void) {
  ...
  gameRenderWorld->DebugLine(colorWhite, contact.point,
                             contact.point + 6.0f * contact.normal);
  gameRenderWorld->DebugLine(colorWhite, contact.point - 2.0f * x,
                             contact.point + 2.0f * x);
  gameRenderWorld->DebugLine(colorWhite, contact.point - 2.0f * y,
                             contact.point + 2.0f * y);
}
...
bool idAFConstraint_ContactFriction::Add(idPhysics_AF* phys,
  ...
  if (body1->GetContactMotorDirection(dir1) &&
      body1->GetContactMotorForce() > 0.0f) {
...
void idAFConstraint_ConeLimit::DebugDraw(void) {
  ...
  gameRenderWorld->DebugLine(
      colorGreen, anchor, anchor + size * (body1Axis * body1->GetWorldAxis()));
  ...
  for (a = 0.0f; a < 360.0f; a += 45.0f) {
    end = x * (float)cos(DEG2RAD(a + 45.0f)) +
          y * (float)sin(DEG2RAD(a + 45.0f)) + z;
...
void idAFConstraint_Suspension::Setup(const char* name,
  ...
  body2 = NULL;
  localOrigin =
      (origin - body->GetWorldOrigin()) * body->GetWorldAxis().Transpose();
...
void idAFConstraint_Suspension::Evaluate(float invTimeStep) {
  float velocity, suspensionLength, springLength, compression, dampingForce,
      springForce;
  ...
  gameLocal.clip.Translation(trace, start, end, wheelModel, axis, MASK_SOLID,
                             NULL);

  wheelOffset = (trace.endpos - body1->GetWorldOrigin()) *
                body1->GetWorldAxis().Transpose();
  ...
  dampingForce = suspensionDamping * idMath::Fabs((vel2 - vel1) * springDir) /
  ...
  c1.SetSize(2);
  c1[0] = 0.0f;
  velocity = J1.SubVec6(0).SubVec3(0) * body1->GetLinearVelocity() +
             J1.SubVec6(0).SubVec3(1) * body1->GetAngularVelocity();
  ...
  if (body2) {
    ...
    c2.SetSize(2);
    c2[0] = 0.0f;
    velocity += J2.SubVec6(0).SubVec3(0) * body2->GetLinearVelocity() +
                J2.SubVec6(0).SubVec3(1) * body2->GetAngularVelocity();
  }
...
void idAFConstraint_Suspension::DebugDraw(void) {
  ...
  if (trace.fraction < 1.0f) {
    ...
    gameRenderWorld->DebugLine(colorWhite, origin - 4.0f * axis[0],
                               origin + 4.0f * axis[0]);
    gameRenderWorld->DebugLine(colorWhite, origin - 2.0f * axis[1],
                               origin + 2.0f * axis[1]);
  }
}
...
void idAFBody::SetFriction(float linear, float angular, float contact) {
  if (linear < 0.0f || linear > 1.0f || angular < 0.0f || angular > 1.0f ||
      contact < 0.0f) {
    gameLocal.Warning(
        "idAFBody::SetFriction: friction out of range, linear = %.1f, angular "
        "= %.1f, contact = %.1f",
...
void idAFBody::SetBouncyness(float bounce) {
  if (bounce < 0.0f || bounce > 1.0f) {
    gameLocal.Warning(
        "idAFBody::SetBouncyness: bouncyness out of range, bounce = %.1f",
        bounce);
    return;
  }
  bouncyness = bounce;
...
void idAFBody::SetDensity(float density, const idMat3& inertiaScale) {
  ...
  if (mass <= 0.0f || FLOAT_IS_NAN(mass)) {
    gameLocal.Warning("idAFBody::SetDensity: invalid mass for body '%s'",
                      name.c_str());
    mass = 1.0f;
    centerOfMass.Zero();
...
idVec3 idAFBody::GetPointVelocity(const idVec3& point) const {
  ...
  return current->spatialVelocity.SubVec3(0) +
         current->spatialVelocity.SubVec3(1).Cross(r);
}
...
void idAFBody::AddForce(const idVec3& point, const idVec3& force) {
  ...
  current->externalForce.SubVec3(1) +=
      (point - current->worldOrigin).Cross(force);
}
...
================
*/
ID_INLINE void idAFBody::InverseWorldSpatialInertiaMultiply(
    idVecX& dst,
    const float* v) const {
  ...
  if (fl.spatialInertiaSparse) {
    ...
    dstPtr[3] = mPtr[3 * 6 + 3] * vPtr[3] + mPtr[3 * 6 + 4] * vPtr[4] +
                mPtr[3 * 6 + 5] * vPtr[5];
    dstPtr[4] = mPtr[4 * 6 + 3] * vPtr[3] + mPtr[4 * 6 + 4] * vPtr[4] +
                mPtr[4 * 6 + 5] * vPtr[5];
    dstPtr[5] = mPtr[5 * 6 + 3] * vPtr[3] + mPtr[5 * 6 + 4] * vPtr[4] +
                mPtr[5 * 6 + 5] * vPtr[5];
  } else {
    gameLocal.Warning("spatial inertia is not sparse for body %s",
                      name.c_str());
  }
}
  ...
  for (i = sortedBodies.Num() - 1; i >= 0; i--) {
    ...
    if (body->children.Num()) {
      for (j = 0; j < body->children.Num(); j++) {
        ...
        child->body1->J.TransposeMultiply(child->body1->I)
            .Multiply(childI, child->body1->J);
        childI.Negate();

        child->invI = childI;
        if (!child->invI.InverseFastSelf()) {
          gameLocal.Warning(
              "idAFTree::Factor: couldn't invert %dx%d matrix for constraint "
              "'%s'",
              child->invI.GetNumRows(), child->invI.GetNumColumns(),
              child->GetName().c_str());
        }
...
void idAFTree::Solve(int auxiliaryIndex) const {
  ...
  for (i = sortedBodies.Num() - 1; i >= 0; i--) {
    ...
    for (j = 0; j < body->children.Num(); j++) {
      ...
      if (!primaryConstraint->fl.isZero) {
        primaryConstraint->J.TransposeMultiplySub(body->s,
                                                  primaryConstraint->s);
  ...
  for (i = 0; i < sortedBodies.Num(); i++) {
    ...
    if (primaryConstraint) {
      if (useSymmetry &&
          body->parent->maxSubTreeAuxiliaryIndex < auxiliaryIndex) {
        continue;
      }
      ...
      primaryConstraint->J.MultiplySub(primaryConstraint->s,
                                       primaryConstraint->body2->s);
...
void idAFTree::CalculateForces(float timeStep) const {
  ...
  for (i = 0; i < sortedBodies.Num(); i++) {
    ...
    body->totalForce.SubVec6(0) =
        body->current->externalForce + body->auxForce.SubVec6(0);
  }
  ...
  for (i = 0; i < sortedBodies.Num(); i++) {
    ...
    body->InverseWorldSpatialInertiaMultiply(body->acceleration,
                                             body->totalForce.ToFloatPtr());
    body->acceleration.SubVec6(0) += body->current->spatialVelocity * invStep;
    primaryConstraint = body->primaryConstraint;
    if (primaryConstraint) {
      // b = ( J * acc + c )
      c = primaryConstraint;
      c->s = c->J1 * c->body1->acceleration + c->J2 * c->body2->acceleration +
             invStep * (c->c1 + c->c2);
      c->fl.isZero = false;
    }
    body->s.Zero();
  ...
  for (i = 0; i < sortedBodies.Num(); i++) {
    ...
    primaryConstraint = body->primaryConstraint;
    if (primaryConstraint) {
      primaryConstraint->J1.TransposeMultiplyAdd(body->totalForce,
                                                 primaryConstraint->lm);
    }
    for (j = 0; j < body->children.Num(); j++) {
      child = body->children[j]->primaryConstraint;
...
void idAFTree::DebugDraw(const idVec4& color) const {
  ...
  for (i = 1; i < sortedBodies.Num(); i++) {
    body = sortedBodies[i];
    gameRenderWorld->DebugArrow(color, body->parent->current->worldOrigin,
                                body->current->worldOrigin, 1);
  }
}
...
void idPhysics_AF::EvaluateBodies(float timeStep) {
  ...
  for (i = 0; i < bodies.Num(); i++) {
    ...
    if (body->centerOfMass.Compare(vec3_origin, CENTER_OF_MASS_EPSILON)) {
      ...
      body->I.Set(body->mass * mat3_identity, mat3_zero, mat3_zero,
                  axis * body->inertiaTensor * axis.Transpose());

      // inverse spatial inertia in world space
      body->inverseWorldSpatialInertia.Set(
          body->invMass * mat3_identity, mat3_zero, mat3_zero,
...
void idPhysics_AF::RemoveFrameConstraints(void) {
  ...
  auxiliaryConstraints.SetNum(
      auxiliaryConstraints.Num() - frameConstraints.Num(), false);
...
void idPhysics_AF::ApplyFriction(float timeStep, float endTimeMSec) {
  ...
  if (jointFrictionDentStart < MS2SEC(endTimeMSec) &&
      jointFrictionDentEnd > MS2SEC(endTimeMSec)) {
    ...
    if (jointFrictionDentStart + halfTime > MS2SEC(endTimeMSec)) {
      jointFrictionDentScale =
          1.0f - (1.0f - jointFrictionDent) *
                     (MS2SEC(endTimeMSec) - jointFrictionDentStart) / halfTime;
    } else {
      jointFrictionDentScale =
          jointFrictionDent +
          (1.0f - jointFrictionDent) *
              (MS2SEC(endTimeMSec) - jointFrictionDentStart - halfTime) /
              halfTime;
    }
  } else {
  ...
  if (contactFrictionDentStart < MS2SEC(endTimeMSec) &&
      contactFrictionDentEnd > MS2SEC(endTimeMSec)) {
    ...
    if (contactFrictionDentStart + halfTime > MS2SEC(endTimeMSec)) {
      contactFrictionDentScale =
          1.0f - (1.0f - contactFrictionDent) *
                     (MS2SEC(endTimeMSec) - contactFrictionDentStart) /
                     halfTime;
    } else {
      contactFrictionDentScale =
          contactFrictionDent +
          (1.0f - contactFrictionDent) *
              (MS2SEC(endTimeMSec) - contactFrictionDentStart - halfTime) /
              halfTime;
    }
  } else {
...
void idPhysics_AF::AuxiliaryForces(float timeStep) {
  ...
  forcePtr = (float*)__builtin_alloca_with_align(
      bodies.Num() * numAuxConstraints * 8 * sizeof(float), 16);
  index = (int*)__builtin_alloca_with_align(
      bodies.Num() * numAuxConstraints * sizeof(int), 16);
  for (i = 0; i < bodies.Num(); i++) {
    body = bodies[i];
  ...
  for (k = 0, i = 0; i < auxiliaryConstraints.Num(); i++) {
    ...
    for (j = 0; j < constraint->J1.GetNumRows(); j++, k++) {
      ...
      if (constraint->body2 &&
          constraint->body2->tree != constraint->body1->tree) {
  ...
  jmk.SetData(numAuxConstraints, ((numAuxConstraints + 3) & ~3),
  ...
  for (k = 0, i = 0; i < auxiliaryConstraints.Num(); i++) {
    ...
    for (j = 0; j < constraint->J1.GetNumRows(); j++, k++) {
      constraint->body1->InverseWorldSpatialInertiaMultiply(tmp,
                                                            constraint->J1[j]);
      j1 = tmp.ToFloatPtr();
      ...
      for (l = n = 0, m = index[n];
           n < constraint->body1->numResponses && m < s; n++, m = index[n]) {
        while (l < m) {
          dstPtr[l++] = 0.0f;
        }
      ...
      if (constraint->body2) {
        constraint->body2->InverseWorldSpatialInertiaMultiply(
            tmp, constraint->J2[j]);
        j2 = tmp.ToFloatPtr();
        ptr = constraint->body2->response;
        index = constraint->body2->responseIndex;
        for (n = 0, m = index[n]; n < constraint->body2->numResponses && m < s;
             n++, m = index[n]) {
  ...
  for (k = 0, i = 0; i < auxiliaryConstraints.Num(); i++) {
    ...
    for (j = 0; j < constraint->J1.GetNumRows(); j++, k++) {
      ...
      rhs[k] = j1[0] * ptr[0] + j1[1] * ptr[1] + j1[2] * ptr[2] +
               j1[3] * ptr[3] + j1[4] * ptr[4] + j1[5] * ptr[5];
      ...
      if (constraint->body2) {
        ...
        rhs[k] += j2[0] * ptr[0] + j2[1] * ptr[1] + j2[2] * ptr[2] +
                  j2[3] * ptr[3] + j2[4] * ptr[4] + j2[5] * ptr[5];
      ...
      if (constraint->boxIndex[j] >= 0) {
        ...
        boxIndex[k] =
            constraint->boxConstraint->firstIndex + constraint->boxIndex[j];
      } else {
        boxIndex[k] = -1;
      }
...
void idPhysics_AF::Evolve(float timeStep) {
  ...
  for (i = 0; i < bodies.Num(); i++) {
    ...
    body->next->spatialVelocity = body->current->spatialVelocity +
                                  timeStep * body->acceleration.SubVec6(0);
    ...
    if (maxLinearVelocity > 0.0f) {
      ...
      if (vSqr > Square(maxLinearVelocity)) {
        body->next->spatialVelocity.SubVec3(0) *=
            idMath::InvSqrt(vSqr) * maxLinearVelocity;
      }
    }
    ...
    if (maxAngularVelocity > 0.0f) {
      ...
      if (vSqr > Square(maxAngularVelocity)) {
        body->next->spatialVelocity.SubVec3(1) *=
            idMath::InvSqrt(vSqr) * maxAngularVelocity;
      }
    }
  }
  ...
  for (i = 0; i < bodies.Num(); i++) {
    ...
    body->next->worldOrigin = body->current->worldOrigin +
                              timeStep * body->next->spatialVelocity.SubVec3(0);
    ...
    body->next->spatialVelocity.SubVec3(0) -=
        body->linearFriction * body->next->spatialVelocity.SubVec3(0);
    body->next->spatialVelocity.SubVec3(1) -=
        body->angularFriction * body->next->spatialVelocity.SubVec3(1);
  }
}
...
bool idPhysics_AF::CollisionImpulse(float timeStep,
  ...
  r = collision.c.point - (body->current->worldOrigin +
                           body->centerOfMass * body->current->worldAxis);
  // the velocity at the collision point
  velocity = body->current->spatialVelocity.SubVec3(0) +
             body->current->spatialVelocity.SubVec3(1).Cross(r);
  ...
  inverseWorldInertiaTensor = body->current->worldAxis.Transpose() *
                              body->inverseInertiaTensor *
                              body->current->worldAxis;
  impulseNumerator =
      -(1.0f + body->bouncyness) * (velocity * collision.c.normal);
  impulseDenominator =
      body->invMass +
  ...
  for (i = 0; i < bodies.Num(); i++) {
    ...
    if (body->clipMask != 0) {
      ...
      if (gameLocal.clip.Motion(collision, body->current->worldOrigin,
                                body->next->worldOrigin, rotation,
      ...
      if (gameLocal.clip.Contents(body->next->worldOrigin, body->clipModel,
                                  body->next->worldAxis, body->clipMask,
                                  passEntity)) {
        if (!startsolid) {
          int bah = 1;
        }
    ...
    body->clipModel->Link(gameLocal.clip, self, body->clipModel->GetId(),
                          body->next->worldOrigin, body->next->worldAxis);
  }
}
...
bool idPhysics_AF::EvaluateContacts(void) {
  ...
  for (i = 0; i < bodies.Num(); i++) {
    ...
    body->InverseWorldSpatialInertiaMultiply(
        dir, body->current->externalForce.ToFloatPtr());
    dir.SubVec6(0) =
    ...
    numContacts = gameLocal.clip.Contacts(
        contactInfo, 10, body->current->worldOrigin, dir.SubVec6(0),
        2.0f,  // CONTACT_EPSILON,
      ...
      for (k = 0; k < contacts.Num(); k++) {
        if (contacts[k].entityNum == contactInfo[j].entityNum) {
          if ((contacts[k].id == i && contactInfo[j].id == contactBodies[k]) ||
            ...
            if ((contacts[k].point - contactInfo[j].point).LengthSqr() <
                Square(2.0f)) {
              break;
            }
            if (idMath::Fabs(contacts[k].normal * contactInfo[j].normal) >
                0.9f) {
              numBodyContacts++;
            }
          }
...
void idPhysics_AF::SetupContactConstraints(void) {
  ...
  contactConstraints.AssureSizeAlloc(contacts.Num(),
                                     idListNewElement<idAFConstraint_Contact>);
  ...
  for (i = 0; i < contacts.Num(); i++) {
    ...
    if (contacts[i].entityNum == self->entityNumber) {
      contactConstraints[i]->Setup(bodies[contactBodies[i]],
                                   bodies[contacts[i].id], contacts[i]);
    } else {
...
void idPhysics_AF::SetSuspendTolerance(const float noMoveTime,
                                       const float noMoveTranslation,
                                       const float noMoveRotation) {
...
bool idPhysics_AF::TestIfAtRest(float timeStep) {
  ...
  if (current.noMoveTime == 0.0f) {
    ...
    for (i = 0; i < bodies.Num(); i++) {
      ...
      translationSqr =
          (body->current->worldOrigin - body->atRestOrigin).LengthSqr();
      if (translationSqr > maxTranslationSqr) {
        maxTranslationSqr = translationSqr;
      }
      rotation = (body->atRestAxis.Transpose() * body->current->worldAxis)
                     .ToRotation()
                     .GetAngle();
    ...
    if (maxTranslationSqr < Square(noMoveTranslation) &&
        maxRotation < noMoveRotation) {
  ...
  for (i = 0; i < bodies.Num(); i++) {
    ...
    if (body->current->spatialVelocity.SubVec3(0).LengthSqr() >
        Square(suspendVelocity[0])) {
      return false;
    }
    if (body->current->spatialVelocity.SubVec3(1).LengthSqr() >
        Square(suspendVelocity[1])) {
      return false;
    }
    if (body->acceleration.SubVec3(0).LengthSqr() >
        Square(suspendAcceleration[0])) {
      return false;
    }
    if (body->acceleration.SubVec3(1).LengthSqr() >
        Square(suspendAcceleration[1])) {
      return false;
    }
  }
...
const idBounds& idPhysics_AF::GetBounds(int id) const {
  ...
  if (id >= 0 && id < bodies.Num()) {
    ...
    for (i = 1; i < bodies.Num(); i++) {
      idBounds bounds;
      idVec3 origin =
          (bodies[i]->GetWorldOrigin() - bodies[0]->GetWorldOrigin()) *
          bodies[0]->GetWorldAxis().Transpose();
      idMat3 axis =
          bodies[i]->GetWorldAxis() * bodies[0]->GetWorldAxis().Transpose();
...
bool idPhysics_AF::Evaluate(int timeStepMSec, int endTimeMSec) {
  ...
  if (timeScaleRampStart < MS2SEC(endTimeMSec) &&
      timeScaleRampEnd > MS2SEC(endTimeMSec)) {
    timeStep = MS2SEC(timeStepMSec) *
               (MS2SEC(endTimeMSec) - timeScaleRampStart) /
  ...
  if (af_showTimings.GetInteger() == 1) {
    gameLocal.Printf(
        ...
        self->name.c_str(), timer_total.Milliseconds(), numPrimary,
        timer_pc.Milliseconds(), numAuxiliary,
        timer_ac.Milliseconds() - timer_lcp.Milliseconds(),
    ...
    if (endTimeMSec > lastTimerReset) {
      gameLocal.Printf(
          ...
          numArticulatedFigures, timer_total.Milliseconds(), numPrimary,
          timer_pc.Milliseconds(), numAuxiliary,
          timer_ac.Milliseconds() - timer_lcp.Milliseconds(),
          timer_lcp.Milliseconds(), timer_collision.Milliseconds());
    }
...
void DrawTraceModelSilhouette(const idVec3& projectionOrigin,
  ...
  numSilEdges = trm->GetProjectionSilhouetteEdges(
      (projectionOrigin - origin) * axis.Transpose(), silEdges);
  for (i = 0; i < numSilEdges; i++) {
    v1 = trm->verts[trm->edges[abs(silEdges[i])].v[INTSIGNBITSET(silEdges[i])]];
    v2 = trm->verts[trm->edges[abs(silEdges[i])]
                        .v[INTSIGNBITNOTSET(silEdges[i])]];
    gameRenderWorld->DebugArrow(colorRed, origin + v1 * axis,
                                origin + v2 * axis, 1);
  }
}
...
void idPhysics_AF::DebugDraw(void) {
  int i;
  idAFBody *body, *highlightBody = NULL, *constrainedBody1 = NULL,
                  *constrainedBody2 = NULL;
  idAFConstraint* constraint;
  idVec3 center;
  idMat3 axis;
  ...
  if (af_highlightConstraint.GetString()[0]) {
    ...
    if (constraint) {
      ...
      gameRenderWorld->DebugCone(colorYellow, center,
                                 (axis[2] - axis[1]) * 4.0f, 0.0f, 1.0f, 0);
      ...
      if (af_showConstrainedBodies.GetBool()) {
        ...
        if (constrainedBody1) {
          collisionModelManager->DrawModel(
              constrainedBody1->clipModel->Handle(),
              constrainedBody1->clipModel->GetOrigin(),
        ...
        if (constrainedBody2) {
          collisionModelManager->DrawModel(
              constrainedBody2->clipModel->Handle(),
              constrainedBody2->clipModel->GetOrigin(),
  ...
  if (af_highlightBody.GetString()[0]) {
    ...
    if (highlightBody) {
      ...
      collisionModelManager->DrawModel(highlightBody->clipModel->Handle(),
                                       highlightBody->clipModel->GetOrigin(),
                                       highlightBody->clipModel->GetAxis(),
                                       vec3_origin, 0.0f);
      cvarSystem->SetCVarString("cm_drawColor", colorRed.ToString(0));
    }
  }
  ...
  if (af_showBodyNames.GetBool()) {
    for (i = 0; i < bodies.Num(); i++) {
      ...
      gameRenderWorld->DrawText(
          body->GetName().c_str(), body->GetWorldOrigin(), 0.08f, colorCyan,
  ...
  if (af_showMass.GetBool()) {
    for (i = 0; i < bodies.Num(); i++) {
      body = bodies[i];
      gameRenderWorld->DrawText(
          va("\n%1.2f", 1.0f / body->GetInverseMass()), body->GetWorldOrigin(),
          0.08f, colorCyan, gameLocal.GetLocalPlayer()->viewAngles.ToMat3(), 1);
    }
  }
  ...
  if (af_showTotalMass.GetBool()) {
    ...
    gameRenderWorld->DrawText(va("\n%1.2f", totalMass),
                              bodies[0]->GetWorldOrigin() + axis[2] * 8.0f,
                              0.15f, colorCyan, axis, 1);
  }
    ...
    for (i = 0; i < bodies.Num(); i++) {
      ...
      gameRenderWorld->DrawText(
          va("\n\n\n( %.1f %.1f %.1f )\n( %.1f %.1f %.1f )\n( %.1f %.1f %.1f )",
             ...
             I[2].z),
          body->GetWorldOrigin(), 0.05f, colorCyan,
          gameLocal.GetLocalPlayer()->viewAngles.ToMat3(), 1);
    }
  }
  ...
  if (af_showConstraintNames.GetBool()) {
    for (i = 0; i < primaryConstraints.Num(); i++) {
      ...
      constraint->GetCenter(center);
      gameRenderWorld->DrawText(
          constraint->GetName().c_str(), center, 0.08f, colorCyan,
          gameLocal.GetLocalPlayer()->viewAngles.ToMat3(), 1);
    }
    if (!af_showPrimaryOnly.GetBool()) {
      for (i = 0; i < auxiliaryConstraints.Num(); i++) {
  ...
  if (af_showTrees.GetBool() ||
      (af_showActive.GetBool() && current.atRest < 0)) {
    for (i = 0; i < trees.Num(); i++) {
...
idPhysics_AF::idPhysics_AF(void) {
  ...
  suspendAcceleration.Set(SUSPEND_LINEAR_ACCELERATION,
                          SUSPEND_LINEAR_ACCELERATION);
...
void idPhysics_AF::BuildTrees(void) {
  ...
  if (af_useLinearTime.GetBool()) {
    for (i = 0; i < constraints.Num(); i++) {
      ...
      if (!c->body1->primaryConstraint && c->fl.allowPrimary &&
          c->body2 != NULL && !IsClosedLoop(c->body1, c->body2)) {
    ...
    if (trees.Num() > 1) {
      gameLocal.Warning(
          "Articulated figure has multiple seperate tree structures for entity "
          "'%s' type '%s'.",
          self->name.c_str(), self->GetType()->classname);
    }
...
void idPhysics_AF::AddConstraint(idAFConstraint* constraint) {
  if (constraints.Find(constraint)) {
    gameLocal.Error("idPhysics_AF::AddConstraint: constraint '%s' added twice.",
                    constraint->name.c_str());
  }
  if (GetConstraint(constraint->name)) {
    gameLocal.Error(
        "idPhysics_AF::AddConstraint: a constraint with the name '%s' already "
        "exists.",
        constraint->name.c_str());
  }
  if (!constraint->body1) {
  ...
  if (!bodies.Find(constraint->body1)) {
    gameLocal.Error(
        "idPhysics_AF::AddConstraint: body1 of constraint '%s' is not part of "
        "the articulated figure.",
        constraint->name.c_str());
  }
  if (constraint->body2 && !bodies.Find(constraint->body2)) {
    gameLocal.Error(
        "idPhysics_AF::AddConstraint: body2 of constraint '%s' is not part of "
        "the articulated figure.",
        constraint->name.c_str());
  }
  if (constraint->body1 == constraint->body2) {
    gameLocal.Error(
        "idPhysics_AF::AddConstraint: body1 and body2 of constraint '%s' are "
        "the same.",
        constraint->name.c_str());
  }
...
int idPhysics_AF::GetBodyId(const char* bodyName) const {
  ...
  gameLocal.Error(
      "GetBodyId: no body with the name '%s' is not part of the articulated "
      "figure.\n",
      bodyName);
  return 0;
}
...
int idPhysics_AF::GetConstraintId(idAFConstraint* constraint) const {
  ...
  if (id == -1 && constraint) {
    gameLocal.Error(
        "GetConstraintId: constraint '%s' is not part of the articulated "
        "figure.\n",
        constraint->name.c_str());
  }
  return id;
...
int idPhysics_AF::GetConstraintId(const char* constraintName) const {
  ...
  gameLocal.Error(
      "GetConstraintId: no constraint with the name '%s' is not part of the "
      "articulated figure.\n",
      constraintName);
  return 0;
}
...
void idPhysics_AF::DeleteBody(const char* bodyName) {
  ...
  if (i >= bodies.Num()) {
    gameLocal.Warning(
        "DeleteBody: no body found in the articulated figure with the name "
        "'%s' for entity '%s' type '%s'.",
...
void idPhysics_AF::DeleteBody(const int id) {
  ...
  for (j = 0; j < constraints.Num(); j++) {
    if (constraints[j]->body1 == bodies[id] ||
        constraints[j]->body2 == bodies[id]) {
...
void idPhysics_AF::DeleteConstraint(const char* constraintName) {
  ...
  if (i >= constraints.Num()) {
    gameLocal.Warning(
        "DeleteConstraint: no constriant found in the articulated figure with "
...
void idPhysics_AF::ApplyImpulse(const int id,
  ...
  idMat3 invWorldInertiaTensor = bodies[id]->current->worldAxis.Transpose() *
                                 bodies[id]->inverseInertiaTensor *
                                 bodies[id]->current->worldAxis;
  bodies[id]->current->spatialVelocity.SubVec3(0) +=
      bodies[id]->invMass * impulse;
  bodies[id]->current->spatialVelocity.SubVec3(1) +=
      invWorldInertiaTensor *
      (point - bodies[id]->current->worldOrigin).Cross(impulse);
  Activate();
}
...
void idPhysics_AF::AddForce(const int id,
  ...
  bodies[id]->current->externalForce.SubVec3(1) +=
      (point - bodies[id]->current->worldOrigin).Cross(force);
  Activate();
}
...
void idPhysics_AF::SetOrigin(const idVec3& newOrigin, int id) {
  if (masterBody) {
    Translate(masterBody->current->worldOrigin +
              masterBody->current->worldAxis * newOrigin -
...
void idPhysics_AF::SetAxis(const idMat3& newAxis, int id) {
  ...
  if (masterBody) {
    axis = bodies[0]->current->worldAxis.Transpose() *
           (newAxis * masterBody->current->worldAxis);
  } else {
...
void idPhysics_AF::ClipTranslation(trace_t& results,
    ...
    if (body->clipModel->IsTraceModel()) {
      if (model) {
        gameLocal.clip.TranslationModel(
            bodyResults, body->current->worldOrigin,
            body->current->worldOrigin + translation, body->clipModel,
            body->current->worldAxis, body->clipMask, model->Handle(),
            model->GetOrigin(), model->GetAxis());
      } else {
        gameLocal.clip.Translation(bodyResults, body->current->worldOrigin,
                                   body->current->worldOrigin + translation,
                                   body->clipModel, body->current->worldAxis,
                                   body->clipMask, self);
      }
  ...
  results.endpos =
      bodies[0]->current->worldOrigin + results.fraction * translation;
...
void idPhysics_AF::ClipRotation(trace_t& results,
  ...
  for (i = 0; i < bodies.Num(); i++) {
    ...
    if (body->clipModel->IsTraceModel()) {
      if (model) {
        gameLocal.clip.RotationModel(
            bodyResults, body->current->worldOrigin, rotation, body->clipModel,
            body->current->worldAxis, body->clipMask, model->Handle(),
            model->GetOrigin(), model->GetAxis());
      } else {
        gameLocal.clip.Rotation(bodyResults, body->current->worldOrigin,
                                rotation, body->clipModel,
                                body->current->worldAxis, body->clipMask, self);
      }
      if (bodyResults.fraction < results.fraction) {
        results = bodyResults;
      }
...
int idPhysics_AF::ClipContents(const idClipModel* model) const {
  ...
  for (i = 0; i < bodies.Num(); i++) {
    ...
    if (body->clipModel->IsTraceModel()) {
      if (model) {
        contents |= gameLocal.clip.ContentsModel(
            body->current->worldOrigin, body->clipModel,
            body->current->worldAxis, -1, model->Handle(), model->GetOrigin(),
            model->GetAxis());
      } else {
        contents |=
            gameLocal.clip.Contents(body->current->worldOrigin, body->clipModel,
                                    body->current->worldAxis, -1, NULL);
      }
    }
  }
...
void idPhysics_AF::SetPushed(int deltaTime) {
  ...
  if (bodies.Num()) {
    body = bodies[0];
    rotation = (body->saved.worldAxis.Transpose() * body->current->worldAxis)
                   .ToRotation();

    // velocity with which the af is pushed
    current.pushVelocity.SubVec3(0) +=
        (body->current->worldOrigin - body->saved.worldOrigin) /
...
const int AF_VELOCITY_EXPONENT_BITS =
    idMath::BitsForInteger(idMath::BitsForFloat(AF_VELOCITY_MAX)) + 1;
const int AF_VELOCITY_MANTISSA_BITS =
    AF_VELOCITY_TOTAL_BITS - 1 - AF_VELOCITY_EXPONENT_BITS;
const float AF_FORCE_MAX = 1e20f;
const int AF_FORCE_TOTAL_BITS = 16;
const int AF_FORCE_EXPONENT_BITS =
    idMath::BitsForInteger(idMath::BitsForFloat(AF_FORCE_MAX)) + 1;
const int AF_FORCE_MANTISSA_BITS =
    AF_FORCE_TOTAL_BITS - 1 - AF_FORCE_EXPONENT_BITS;
...
void idPhysics_AF::WriteToSnapshot(idBitMsgDelta& msg) const {
  ...
  msg.WriteDeltaFloat(0.0f, current.pushVelocity[0], AF_VELOCITY_EXPONENT_BITS,
                      AF_VELOCITY_MANTISSA_BITS);
  msg.WriteDeltaFloat(0.0f, current.pushVelocity[1], AF_VELOCITY_EXPONENT_BITS,
                      AF_VELOCITY_MANTISSA_BITS);
  msg.WriteDeltaFloat(0.0f, current.pushVelocity[2], AF_VELOCITY_EXPONENT_BITS,
                      AF_VELOCITY_MANTISSA_BITS);
  msg.WriteDeltaFloat(0.0f, current.pushVelocity[3], AF_VELOCITY_EXPONENT_BITS,
                      AF_VELOCITY_MANTISSA_BITS);
  msg.WriteDeltaFloat(0.0f, current.pushVelocity[4], AF_VELOCITY_EXPONENT_BITS,
                      AF_VELOCITY_MANTISSA_BITS);
  msg.WriteDeltaFloat(0.0f, current.pushVelocity[5], AF_VELOCITY_EXPONENT_BITS,
                      AF_VELOCITY_MANTISSA_BITS);
  ...
  for (i = 0; i < bodies.Num(); i++) {
    ...
    msg.WriteDeltaFloat(0.0f, state->spatialVelocity[0],
                        AF_VELOCITY_EXPONENT_BITS, AF_VELOCITY_MANTISSA_BITS);
    msg.WriteDeltaFloat(0.0f, state->spatialVelocity[1],
                        AF_VELOCITY_EXPONENT_BITS, AF_VELOCITY_MANTISSA_BITS);
    msg.WriteDeltaFloat(0.0f, state->spatialVelocity[2],
                        AF_VELOCITY_EXPONENT_BITS, AF_VELOCITY_MANTISSA_BITS);
    msg.WriteDeltaFloat(0.0f, state->spatialVelocity[3],
                        AF_VELOCITY_EXPONENT_BITS, AF_VELOCITY_MANTISSA_BITS);
    msg.WriteDeltaFloat(0.0f, state->spatialVelocity[4],
                        AF_VELOCITY_EXPONENT_BITS, AF_VELOCITY_MANTISSA_BITS);
    msg.WriteDeltaFloat(0.0f, state->spatialVelocity[5],
                        AF_VELOCITY_EXPONENT_BITS, AF_VELOCITY_MANTISSA_BITS);
...
void idPhysics_AF::ReadFromSnapshot(const idBitMsgDelta& msg) {
  ...
  current.pushVelocity[0] = msg.ReadDeltaFloat(0.0f, AF_VELOCITY_EXPONENT_BITS,
                                               AF_VELOCITY_MANTISSA_BITS);
  current.pushVelocity[1] = msg.ReadDeltaFloat(0.0f, AF_VELOCITY_EXPONENT_BITS,
                                               AF_VELOCITY_MANTISSA_BITS);
  current.pushVelocity[2] = msg.ReadDeltaFloat(0.0f, AF_VELOCITY_EXPONENT_BITS,
                                               AF_VELOCITY_MANTISSA_BITS);
  current.pushVelocity[3] = msg.ReadDeltaFloat(0.0f, AF_VELOCITY_EXPONENT_BITS,
                                               AF_VELOCITY_MANTISSA_BITS);
  current.pushVelocity[4] = msg.ReadDeltaFloat(0.0f, AF_VELOCITY_EXPONENT_BITS,
                                               AF_VELOCITY_MANTISSA_BITS);
  current.pushVelocity[5] = msg.ReadDeltaFloat(0.0f, AF_VELOCITY_EXPONENT_BITS,
                                               AF_VELOCITY_MANTISSA_BITS);
  ...
  for (i = 0; i < bodies.Num(); i++) {
    ...
    state->spatialVelocity[0] = msg.ReadDeltaFloat(
        0.0f, AF_VELOCITY_EXPONENT_BITS, AF_VELOCITY_MANTISSA_BITS);
    state->spatialVelocity[1] = msg.ReadDeltaFloat(
        0.0f, AF_VELOCITY_EXPONENT_BITS, AF_VELOCITY_MANTISSA_BITS);
    state->spatialVelocity[2] = msg.ReadDeltaFloat(
        0.0f, AF_VELOCITY_EXPONENT_BITS, AF_VELOCITY_MANTISSA_BITS);
    state->spatialVelocity[3] = msg.ReadDeltaFloat(
        0.0f, AF_VELOCITY_EXPONENT_BITS, AF_VELOCITY_MANTISSA_BITS);
    state->spatialVelocity[4] = msg.ReadDeltaFloat(
        0.0f, AF_VELOCITY_EXPONENT_BITS, AF_VELOCITY_MANTISSA_BITS);
    state->spatialVelocity[5] = msg.ReadDeltaFloat(
        0.0f, AF_VELOCITY_EXPONENT_BITS, AF_VELOCITY_MANTISSA_BITS);

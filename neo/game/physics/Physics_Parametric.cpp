...
bool idPhysics_Parametric::TestIfAtRest(void) const {
  if ((current.linearExtrapolation.GetExtrapolationType() &
       ~EXTRAPOLATION_NOSTOP) == EXTRAPOLATION_NONE &&
      (current.angularExtrapolation.GetExtrapolationType() &
       ~EXTRAPOLATION_NOSTOP) == EXTRAPOLATION_NONE &&
...
idPhysics_Parametric::idPhysics_Parametric(void) {
  ...
  current.linearExtrapolation.Init(0, 0, vec3_zero, vec3_zero, vec3_zero,
                                   EXTRAPOLATION_NONE);
  current.angularExtrapolation.Init(0, 0, ang_zero, ang_zero, ang_zero,
                                    EXTRAPOLATION_NONE);
...
================
*/
void idPhysics_Parametric_SavePState(idSaveGame* savefile,
                                     const parametricPState_t& state) {
...
================
*/
void idPhysics_Parametric_RestorePState(idRestoreGame* savefile,
                                        parametricPState_t& state) {
  ...
  state.linearExtrapolation.Init(startTime, duration, linearStartValue,
                                 linearBaseSpeed, linearSpeed, etype);
  ...
  state.angularExtrapolation.Init(startTime, duration, angularStartValue,
                                  angularBaseSpeed, angularSpeed, etype);
  ...
  state.linearInterpolation.Init(startTime, accelTime, decelTime, duration,
                                 startPos, endPos);
  ...
  state.angularInterpolation.Init(startTime, accelTime, decelTime, duration,
                                  startAng, endAng);
  ...
  state.splineInterpolate.Init(startTime, accelTime, decelTime, duration,
                               startValue, endValue);
}
...
void idPhysics_Parametric::SetLinearExtrapolation(extrapolation_t type,
  ...
  current.time = gameLocal.time;
  current.linearExtrapolation.Init(time, duration, base, baseSpeed, speed,
                                   type);
...
void idPhysics_Parametric::SetAngularExtrapolation(extrapolation_t type,
  ...
  current.time = gameLocal.time;
  current.angularExtrapolation.Init(time, duration, base, baseSpeed, speed,
                                    type);
...
void idPhysics_Parametric::SetLinearInterpolation(int time,
                                                  ...
                                                  const idVec3& endPos) {
  current.time = gameLocal.time;
  current.linearInterpolation.Init(time, accelTime, decelTime, duration,
                                   startPos, endPos);
...
void idPhysics_Parametric::SetAngularInterpolation(int time,
  ...
  current.time = gameLocal.time;
  current.angularInterpolation.Init(time, accelTime, decelTime, duration,
                                    startAng, endAng);
...
void idPhysics_Parametric::SetSpline(idCurve_Spline<idVec3>* spline,
  ...
  if (current.spline != NULL) {
    ...
    current.splineInterpolate.Init(startTime, accelTime, decelTime,
                                   endTime - startTime, 0.0f, length);
  }
...
bool idPhysics_Parametric::Evaluate(int timeStepMSec, int endTimeMSec) {
  ...
  if (current.spline != NULL) {
    ...
    if (current.useSplineAngles) {
      current.localAngles =
          current.spline->GetCurrentFirstDerivative(t).ToAngles();
    }
  } else if (current.linearInterpolation.GetDuration() != 0) {
    current.localOrigin +=
        current.linearInterpolation.GetCurrentValue(endTimeMSec);
  } else {
    current.localOrigin +=
        current.linearExtrapolation.GetCurrentValue(endTimeMSec);
  }

  if (current.angularInterpolation.GetDuration() != 0) {
    current.localAngles +=
        current.angularInterpolation.GetCurrentValue(endTimeMSec);
  } else {
    current.localAngles +=
        current.angularExtrapolation.GetCurrentValue(endTimeMSec);
  }
  ...
  if (isPusher) {
    gameLocal.push.ClipPush(pushResults, self, pushFlags, oldOrigin, oldAxis,
                            current.origin, current.axis);
...
void idPhysics_Parametric::UpdateTime(int endTimeMSec) {
  ...
  current.time = endTimeMSec;
  // move the trajectory start times to sync the trajectory with the current
  // endTime
  current.linearExtrapolation.SetStartTime(
      current.linearExtrapolation.GetStartTime() + timeLeap);
  current.angularExtrapolation.SetStartTime(
      current.angularExtrapolation.GetStartTime() + timeLeap);
  current.linearInterpolation.SetStartTime(
      current.linearInterpolation.GetStartTime() + timeLeap);
  current.angularInterpolation.SetStartTime(
      current.angularInterpolation.GetStartTime() + timeLeap);
  if (current.spline != NULL) {
    current.spline->ShiftTime(timeLeap);
    current.splineInterpolate.SetStartTime(
        current.splineInterpolate.GetStartTime() + timeLeap);
  }
}
...
void idPhysics_Parametric::SetOrigin(const idVec3& newOrigin, int id) {
  ...
  current.localOrigin =
      current.linearExtrapolation.GetCurrentValue(current.time);
  if (hasMaster) {
    self->GetMasterPosition(masterOrigin, masterAxis);
...
void idPhysics_Parametric::SetAxis(const idMat3& newAxis, int id) {
  ...
  current.localAngles =
      current.angularExtrapolation.GetCurrentValue(current.time);
...
void idPhysics_Parametric::SetLinearVelocity(const idVec3& newLinearVelocity,
  ...
  SetLinearExtrapolation(
      ...
      gameLocal.time, 0, current.origin, newLinearVelocity, vec3_origin);
  current.linearInterpolation.Init(0, 0, 0, 0, vec3_zero, vec3_zero);
  Activate();
}
...
void idPhysics_Parametric::SetAngularVelocity(const idVec3& newAngularVelocity,
  ...
  SetAngularExtrapolation(
      extrapolation_t(EXTRAPOLATION_LINEAR | EXTRAPOLATION_NOSTOP),
      gameLocal.time, 0, current.angles, rotation.ToAngles(), ang_zero);
  current.angularInterpolation.Init(0, 0, 0, 0, ang_zero, ang_zero);
  Activate();
}
...
const idVec3& idPhysics_Parametric::GetLinearVelocity(int id) const {
  ...
  curLinearVelocity =
      current.linearExtrapolation.GetCurrentSpeed(gameLocal.time);
...
void idPhysics_Parametric::SetMaster(idEntity* master, const bool orientated) {
  ...
  if (master) {
    if (!hasMaster) {
      ...
      self->GetMasterPosition(masterOrigin, masterAxis);
      current.localOrigin =
          (current.origin - masterOrigin) * masterAxis.Transpose();
      if (orientated) {
        current.localAngles =
            (current.axis * masterAxis.Transpose()).ToAngles();
      } else {
    ...
    if (hasMaster) {
      ...
      SetLinearExtrapolation(EXTRAPOLATION_NONE, 0, 0, current.origin,
                             vec3_origin, vec3_origin);
      SetAngularExtrapolation(EXTRAPOLATION_NONE, 0, 0, current.angles,
                              ang_zero, ang_zero);
      hasMaster = false;
    }
  }
...
int idPhysics_Parametric::GetLinearEndTime(void) const {
  if (current.spline != NULL) {
    if (current.spline->GetBoundaryType() !=
...
void idPhysics_Parametric::ReadFromSnapshot(const idBitMsgDelta& msg) {
  ...
  current.linearExtrapolation.Init(startTime, duration, linearStartValue,
                                   linearBaseSpeed, linearSpeed, linearType);
  ...
  current.angularExtrapolation.Init(startTime, duration, angularStartValue,
                                    angularBaseSpeed, angularSpeed,
                                    angularType);
  ...
  current.linearInterpolation.Init(startTime, accelTime, decelTime, duration,
                                   startPos, endPos);
  ...
  current.angularInterpolation.Init(startTime, accelTime, decelTime, duration,
                                    startAng, endAng);

...
bool idPhysics_RigidBody::TestIfAtRest(void) const {
  ...
  for (i = 0; i < contacts.Num(); i++) {
    ...
    point =
        contacts[i].point - (contacts[i].point * gravityNormal) * gravityNormal;
...
void idPhysics_RigidBody::DebugDraw(void) {
  if (rb_showBodies.GetBool() ||
    ...
    collisionModelManager->DrawModel(clipModel->Handle(),
                                     clipModel->GetOrigin(),
                                     clipModel->GetAxis(), vec3_origin, 0.0f);
  }
...
void idPhysics_RigidBody::SetClipModel(idClipModel* model,
  ...
  if (mass <= 0.0f || FLOAT_IS_NAN(mass)) {
    gameLocal.Warning(
        "idPhysics_RigidBody::SetClipModel: invalid mass for entity '%s' type "
        "'%s'",
        self->name.c_str(), self->GetType()->classname);
    mass = 1.0f;
    centerOfMass.Zero();
  ...
  minIndex =
      Min3Index(inertiaTensor[0][0], inertiaTensor[1][1], inertiaTensor[2][2]);
...
void idPhysics_RigidBody::ClipTranslation(trace_t& results,
  ...
  if (model) {
    gameLocal.clip.TranslationModel(
        ...
        clipModel, clipModel->GetAxis(), clipMask, model->Handle(),
...
void idPhysics_RigidBody::SetPushed(int deltaTime) {
  ...
  current.pushVelocity.SubVec3(0) +=
      (current.i.position - saved.i.position) / (deltaTime * idMath::M_MS2SEC);
...
void idPhysics_RigidBody::WriteToSnapshot(idBitMsgDelta& msg) const {
  ...
  msg.WriteDeltaFloat(0.0f, current.pushVelocity[0], RB_VELOCITY_EXPONENT_BITS,
                      RB_VELOCITY_MANTISSA_BITS);
  msg.WriteDeltaFloat(0.0f, current.pushVelocity[1], RB_VELOCITY_EXPONENT_BITS,
                      RB_VELOCITY_MANTISSA_BITS);
  msg.WriteDeltaFloat(0.0f, current.pushVelocity[2], RB_VELOCITY_EXPONENT_BITS,
                      RB_VELOCITY_MANTISSA_BITS);
...
void idPhysics_RigidBody::ReadFromSnapshot(const idBitMsgDelta& msg) {
  ...
  current.pushVelocity[0] = msg.ReadDeltaFloat(0.0f, RB_VELOCITY_EXPONENT_BITS,
                                               RB_VELOCITY_MANTISSA_BITS);
  current.pushVelocity[1] = msg.ReadDeltaFloat(0.0f, RB_VELOCITY_EXPONENT_BITS,
                                               RB_VELOCITY_MANTISSA_BITS);
  current.pushVelocity[2] = msg.ReadDeltaFloat(0.0f, RB_VELOCITY_EXPONENT_BITS,
                                               RB_VELOCITY_MANTISSA_BITS);
  current.externalForce[0] =
      msg.ReadDeltaFloat(0.0f, RB_FORCE_EXPONENT_BITS, RB_FORCE_MANTISSA_BITS);

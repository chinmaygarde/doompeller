...
opcode_t idCompiler::opcodes[] = {
    ...
    {"<THREAD>", "OBJTHREAD", -1, false, &def_function, &def_argsize,
     &def_void},
...
ID_INLINE idVarDef* idCompiler::VirtualFunctionConstant(idVarDef* func) {
  ...
  eval._int =
      func->scope->TypeDef()->GetFunctionNumber(func->value.functionPtr);
  if (eval._int < 0) {
...
idVarDef* idCompiler::FindImmediate(const idTypeDef* type,
  ...
  for (def = gameLocal.program.GetDefList("<IMMEDIATE>"); def != NULL;
       def = def->Next()) {
    if (def->TypeDef() != type) {
      continue;
    }
...
idVarDef* idCompiler::OptimizeOpcode(const opcode_t* op,
  ...
  switch (op - opcodes) {
    ...
    case OP_NOT_F:
      ...
      break;
    case OP_NOT_V:
      c._float = !var_a->value.vectorPtr->x && !var_a->value.vectorPtr->y &&
                 !var_a->value.vectorPtr->z;
      type = &type_float;
      break;
    case OP_NEG_F:
      c._float = -*var_a->value.floatPtr;
...
idVarDef* idCompiler::EmitOpcode(const opcode_t* op,
  ...
  if ((op->type_c == &def_void) || op->rightAssociative) {
    ...
    var_c = gameLocal.program.FindFreeResultDef(
...
bool idCompiler::EmitPush(idVarDef* expression, const idTypeDef* funcArg) {
  ...
  out = NULL;
  for (op = &opcodes[OP_PUSH_F]; op->name && !strcmp(op->name, "<PUSH>");
       op++) {
    if ((funcArg->Type() == op->type_a->Type()) &&
        (expression->Type() == op->type_b->Type())) {
      out = op;
      break;
    }
  }
  ...
  if (!out) {
    if ((expression->TypeDef() != funcArg) &&
        !expression->TypeDef()->Inherits(funcArg)) {
      return false;
    }
...
void idCompiler::NextToken(void) {
  ...
  if (currentFileNumber !=
      gameLocal.program.GetFilenum(parserPtr->GetFileName())) {
    if ((braceDepth > 0) && (token != "}")) {
      // missing a closing brace.  try to give as much info as possible.
      if (scope->Type() == ev_function) {
        Error(
            "Unexpected end of file inside function '%s'.  Missing closing "
            "braces.",
            scope->Name());
      } else if (scope->Type() == ev_object) {
        Error(
            "Unexpected end of file inside object '%s'.  Missing closing "
            "braces.",
            scope->Name());
      } else if (scope->Type() == ev_namespace) {
        Error(
            "Unexpected end of file inside namespace '%s'.  Missing closing "
            "braces.",
            scope->Name());
      } else {
  ...
  switch (token.type) {
    ...
    case TT_LITERAL: {
      ...
      idLexer lex(token, token.Length(), parserPtr->GetFileName(),
                  LEXFL_NOERRORS);
      idToken token2;
      for (i = 0; i < 3; i++) {
        if (!lex.ReadToken(&token2)) {
          Error("Couldn't read vector. '%s' is not in the form of 'x y z'",
                token.c_str());
        }
        if (token2.type == TT_PUNCTUATION && token2 == "-") {
          if (!lex.CheckTokenType(TT_NUMBER, 0, &token2)) {
            Error(
                "expected a number following '-' but found '%s' in vector '%s'",
                token2.c_str(), token.c_str());
          }
          ...
          Error(
              "vector '%s' is not in the form of 'x y z'.  expected float "
              "value, found '%s'",
...
idTypeDef* idCompiler::ParseType(void) {
  ...
  if ((type == &type_namespace) && (scope->Type() != ev_namespace)) {
    Error(
        "A namespace may only be defined globally, or within another "
        "namespace");
  }
...
idVarDef* idCompiler::EmitFunctionParms(int op,
  ...
  if (!CheckToken(")")) {
    do {
      ...
      if (!EmitPush(e, funcArg)) {
        Error("type mismatch on parm %i of call to '%s'", arg + 1,
              func->Name());
      }
  ...
  if (op == OP_CALL) {
    ...
    statement_t& statement =
        gameLocal.program.GetStatement(gameLocal.program.NumStatements() - 1);
  ...
  statement_t& statement =
      gameLocal.program.GetStatement(gameLocal.program.NumStatements() - 1);
  idVarDef* resultDef = gameLocal.program.FindFreeResultDef(
...
idVarDef* idCompiler::ParseFunctionCall(idVarDef* funcDef) {
  ...
  if (callthread) {
    if ((funcDef->initialized != idVarDef::uninitialized) &&
        funcDef->value.functionPtr->eventdef) {
    ...
    if ((funcDef->initialized != idVarDef::uninitialized) &&
      ...
      if ((scope->Type() != ev_namespace) &&
          (scope->scope->Type() == ev_object)) {
        // get the local object pointer
        idVarDef* thisdef =
            gameLocal.program.GetDef(scope->scope->TypeDef(), "self", scope);
        if (!thisdef) {
...
idVarDef* idCompiler::LookupDef(const char* name, const idVarDef* baseobj) {
  ...
  if (baseobj && (baseobj->Type() == ev_object)) {
    ...
    def = NULL;
    for (tdef = baseobj; tdef != &def_object;
    ...
    if (!def) {
      ...
      if ((scope->Type() != ev_namespace) &&
        ...
        if (field->Type() == ev_function) {
          ...
          type_c = field->TypeDef()
                       ->FieldType()
                       ->Type();  // field access gets type from field
          if (CheckToken("++")) {
            if (type_c != ev_float) {
        ...
        while ((op->type_a->Type() != ev_object) ||
          ...
          if ((op->priority == FUNCTION_PRIORITY) &&
              (op->type_a->Type() == ev_object) &&
              (op->type_c->Type() == ev_void) &&
              (type_c != op->type_c->Type())) {
          ...
          if (!op->name || strcmp(op->name, ".")) {
            Error("no valid opcode to access type '%s'",
                  field->TypeDef()->SuperClass()->Name());
          }
        }
...
idVarDef* idCompiler::ParseValue(void) {
  ...
  if (immediateType == &type_entity) {
    ...
    if (!def) {
      def = gameLocal.program.AllocDef(&type_entity, "$" + token,
                                       &def_namespace, true);
    }
    NextToken();
    return def;
  ...
  if (!def) {
    if (basetype) {
      Error("%s is not a member of %s", name.c_str(),
    ...
    while (def->Type() == ev_namespace) {
      ...
      if (!def) {
        Error("Unknown value \"%s::%s\"", namespaceDef->GlobalName(),
              name.c_str());
      }
    }
    // def = LookupDef( name, basetype );
  ...
  while (1) {
    ...
    if (op->rightAssociative) {
      ...
      if (gameLocal.program.NumStatements() > 0) {
        statement_t& statement = gameLocal.program.GetStatement(
            gameLocal.program.NumStatements() - 1);
    ...
    switch (op - opcodes) {
      ...
      case OP_OBJECTCALL:
        ExpectToken("(");
        if ((e2->initialized != idVarDef::uninitialized) &&
            e2->value.functionPtr->eventdef) {
      ...
      default:
        ...
        if ((type_a == ev_pointer) &&
            (type_b != e->TypeDef()->PointerType()->Type())) {
          // FIXME: need to make a general case for this
          if ((op - opcodes == OP_STOREP_F) &&
              (e->TypeDef()->PointerType()->Type() == ev_boolean)) {
            // copy from float to boolean pointer
            op = &opcodes[OP_STOREP_FTOBOOL];
          } else if ((op - opcodes == OP_STOREP_BOOL) &&
                     (e->TypeDef()->PointerType()->Type() == ev_float)) {
            // copy from boolean to float pointer
            op = &opcodes[OP_STOREP_BOOLTOF];
          } else if ((op - opcodes == OP_STOREP_F) &&
                     (e->TypeDef()->PointerType()->Type() == ev_string)) {
          ...
          } else if ((op - opcodes == OP_STOREP_V) &&
                     (e->TypeDef()->PointerType()->Type() == ev_string)) {
            // copy from vector to string pointer
            op = &opcodes[OP_STOREP_VTOS];
          } else if ((op - opcodes == OP_STOREP_ENT) &&
                     (e->TypeDef()->PointerType()->Type() == ev_object)) {
        ...
        if (op - opcodes == OP_STOREP_OBJENT) {
          ...
          // gets its type reassigned, so we store the real type in statement.c
          // so that we can do a type check during run time since we don't know
          // what type the script object is at compile time because it comes
          // from an entity
          statement_t& statement = gameLocal.program.GetStatement(
              gameLocal.program.NumStatements() - 1);
...
void idCompiler::ParseObjectDef(const char* objname) {
  ...
  objtype = gameLocal.program.AllocType(
      ev_object, NULL, objname,
      parentType == &type_object ? 0 : parentType->Size(), parentType);
...
void idCompiler::ParseFunctionDef(idTypeDef* returnType, const char* name) {
  ...
  if ((scope->Type() != ev_namespace) &&
      !scope->TypeDef()->Inherits(&type_object)) {
  ...
  // define the parms
  for (i = 0; i < numParms; i++) {
    if (gameLocal.program.GetDef(type->GetParmType(i), type->GetParmName(i),
                                 def)) {
      Error("'%s' defined more than once in function parameters",
            type->GetParmName(i));
    }
    parm = gameLocal.program.AllocDef(type->GetParmType(i),
                                      type->GetParmName(i), def, false);
  }
  ...
  if (oldscope->TypeDef()->Inherits(&type_object) &&
      !idStr::Icmp(name, "init")) {
    idTypeDef* superClass;
    function_t* constructorFunc = NULL;

    // find the superclass constructor
    for (superClass = oldscope->TypeDef()->SuperClass();
         superClass != &type_object; superClass = superClass->SuperClass()) {
      constructorFunc =
          gameLocal.program.FindFunction(va("%s::init", superClass->Name()));
      if (constructorFunc) {
        break;
      }
    }
    ...
    if (constructorFunc) {
      idVarDef* selfDef = gameLocal.program.GetDef(type->GetParmType(0),
                                                   type->GetParmName(0), def);
      assert(selfDef);
  ...
  if (oldscope->TypeDef()->Inherits(&type_object) &&
    ...
    for (superClass = oldscope->TypeDef()->SuperClass();
         superClass != &type_object; superClass = superClass->SuperClass()) {
      destructorFunc =
          gameLocal.program.FindFunction(va("%s::destroy", superClass->Name()));
      if (destructorFunc) {
        break;
      }
    }
    ...
    if (destructorFunc) {
      if (func->firstStatement < gameLocal.program.NumStatements()) {
        ...
        for (i = func->firstStatement; i < gameLocal.program.NumStatements();
             i++, pos++) {
          if (pos->op == OP_RETURN) {
            pos->op = OP_GOTO;
...
// when last statement is in the form "if ( x ) { return; }"
  ...
  func->numStatements =
      gameLocal.program.NumStatements() - func->firstStatement;
...
void idCompiler::ParseEventDef(idTypeDef* returnType, const char* name) {
  ...
  if (!expectedType) {
    Error("Invalid return type '%c' in definition of '%s' event.",
          ev->GetReturnType(), name);
  }
  if (returnType != expectedType) {
    Error("Return type doesn't match internal return type '%s'",
          expectedType->Name());
  }
  ...
  for (i = 0; i < num; i++) {
    ...
    if (!expectedType || (expectedType == &type_void)) {
      Error("Invalid parameter '%c' in definition of '%s' event.", format[i],
            name);
    }
    ...
    if (argType != expectedType) {
      Error(
          "The type of parm %d ('%s') does not match the internal type '%s' in "
          "definition of '%s' event.",
    ...
    if (i < num - 1) {
      if (CheckToken(")")) {
        Error(
            "Too few parameters for event definition.  Internal definition has "
            "%d parameters.",
            num);
      }
      ExpectToken(",");
  ...
  if (!CheckToken(")")) {
    Error(
        "Too many parameters for event definition.  Internal definition has %d "
        "parameters.",
        num);
  }
  ExpectToken(";");
  ...
  if (type) {
    if (!newtype.MatchesType(*type) ||
        (type->def->value.functionPtr->eventdef != ev)) {
  ...
  try {
    ...
    if (console) {
      ...
      sprintf(error, "Error: file %s, line %d: %s\n",
              gameLocal.program.GetFilename(currentFileNumber),

...
void idTestModel::Spawn(void) {
  ...
  if (renderEntity.hModel && renderEntity.hModel->IsDefaultModel() &&
      !animator.ModelDef()) {
    gameLocal.Warning("Unable to create testmodel for '%s' : model defaulted",
  ...
  if (headModel[0]) {
    ...
    if (joint == INVALID_JOINT) {
      gameLocal.Warning("Joint '%s' not found for 'head_joint'",
                        jointName.c_str());
    } else {
      ...
      origin = GetPhysics()->GetOrigin() +
               (origin + modelOffset) * GetPhysics()->GetAxis();
      ...
      for (kv = spawnArgs.MatchPrefix("copy_joint", NULL); kv != NULL;
        ...
        if (copyJoint.to == INVALID_JOINT) {
          gameLocal.Warning("Unknown copy_joint '%s' on head",
                            jointName.c_str());
          continue;
        }
...
void idTestModel::Think(void) {
  ...
  if (thinkFlags & TH_THINK) {
    if (anim && (gameLocal.testmodel == this) &&
        (mode != g_testModelAnimate.GetInteger())) {
      StopSound(SND_CHANNEL_ANY, false);
      if (head.GetEntity()) {
        head.GetEntity()->StopSound(SND_CHANNEL_ANY, false);
      ...
      switch (g_testModelAnimate.GetInteger()) {
        ...
        case 0:
          ...
          if (animator.NumFrames(anim) <= 1) {
            ...
            animator.CycleAnim(ANIMCHANNEL_ALL, anim, gameLocal.time,
                               FRAME2MS(g_testModelBlend.GetInteger()));
            if (headAnim) {
              headAnimator->CycleAnim(ANIMCHANNEL_ALL, headAnim, gameLocal.time,
                                      FRAME2MS(g_testModelBlend.GetInteger()));
            }
          } else {
            animator.PlayAnim(ANIMCHANNEL_ALL, anim, gameLocal.time,
                              FRAME2MS(g_testModelBlend.GetInteger()));
            if (headAnim) {
              headAnimator->PlayAnim(ANIMCHANNEL_ALL, headAnim, gameLocal.time,
                                     FRAME2MS(g_testModelBlend.GetInteger()));
              if (headAnimator->AnimLength(headAnim) >
                  animator.AnimLength(anim)) {
        ...
        case 3:
          ...
          animator.SetFrame(ANIMCHANNEL_ALL, anim, frame, gameLocal.time,
                            FRAME2MS(g_testModelBlend.GetInteger()));
          animator.RemoveOriginOffset(false);
          if (headAnim) {
            headAnimator->SetFrame(ANIMCHANNEL_ALL, headAnim, frame,
                                   gameLocal.time,
                                   FRAME2MS(g_testModelBlend.GetInteger()));
          }
          break;
    ...
    physicsObj.SetAngularExtrapolation(
        ...
        gameLocal.time, 0, ang,
        idAngles(0, g_testModelRotate.GetFloat() * 360.0f / 60.0f, 0),
        ang_zero);
    ...
    if (clip && animator.ModelDef()) {
      ...
      neworigin = ((neworigin - animator.ModelDef()->GetVisualOffset()) *
                   physicsObj.GetAxis()) +
  ...
  if ((gameLocal.testmodel == this) && g_showTestModelFrame.GetInteger() &&
      anim) {
    gameLocal.Printf(
        "^5 Anim: ^7%s  ^5Frame: ^7%d/%d  Time: %.3f\n",
        animator.AnimFullName(anim),
        animator.CurrentAnim(ANIMCHANNEL_ALL)->GetFrameNumber(gameLocal.time),
        animator.CurrentAnim(ANIMCHANNEL_ALL)->NumFrames(),
        MS2SEC(gameLocal.time -
               animator.CurrentAnim(ANIMCHANNEL_ALL)->GetStartTime()));
    if (headAnim) {
      gameLocal.Printf(
          "^5 Head: ^7%s  ^5Frame: ^7%d/%d  Time: %.3f\n\n",
          headAnimator->AnimFullName(headAnim),
          headAnimator->CurrentAnim(ANIMCHANNEL_ALL)
              ->GetFrameNumber(gameLocal.time),
          headAnimator->CurrentAnim(ANIMCHANNEL_ALL)->NumFrames(),
          MS2SEC(gameLocal.time -
                 headAnimator->CurrentAnim(ANIMCHANNEL_ALL)->GetStartTime()));
    } else {
      gameLocal.Printf("\n\n");
    }
  }
...
void idTestModel::NextAnim(const idCmdArgs& args) {
  ...
  gameLocal.Printf("anim '%s', %d.%03d seconds, %d frames\n", animname.c_str(),
                   ...
                   animator.AnimLength(anim) % 1000, animator.NumFrames(anim));
  if (headAnim) {
...
void idTestModel::NextFrame(const idCmdArgs& args) {
  if (!anim || ((g_testModelAnimate.GetInteger() != 3) &&
                (g_testModelAnimate.GetInteger() != 5))) {
    return;
  }
  ...
  gameLocal.Printf("^5 Anim: ^7%s\n^5Frame: ^7%d/%d\n\n",
                   animator.AnimFullName(anim), frame,
...
void idTestModel::BlendAnim(const idCmdArgs& args) {
  ...
  animator.CycleAnim(ANIMCHANNEL_ALL, anim2, gameLocal.time,
                     FRAME2MS(atoi(args.Argv(3))));
...
void idTestModel::TestModel_f(const idCmdArgs& args) {
  ...
  gameLocal.testmodel =
      (idTestModel*)gameLocal.SpawnEntityType(idTestModel::Type, &dict);
  gameLocal.testmodel->renderEntity.shaderParms[SHADERPARM_TIMEOFFSET] =
      -MS2SEC(gameLocal.time);
}
...
void idTestModel::ArgCompletion_TestModel(const idCmdArgs& args,
  ...
  for (i = 0; i < num; i++) {
    callback(idStr(args.Argv(0)) + " " +
             declManager->DeclByIndex(DECL_ENTITYDEF, i, false)->GetName());
  }
  num = declManager->GetNumDecls(DECL_MODELDEF);
  for (i = 0; i < num; i++) {
    callback(idStr(args.Argv(0)) + " " +
             declManager->DeclByIndex(DECL_MODELDEF, i, false)->GetName());
  }
...
void idTestModel::TestParticleStopTime_f(const idCmdArgs& args) {
  ...
  gameLocal.testmodel->renderEntity.shaderParms[SHADERPARM_PARTICLE_STOPTIME] =
      MS2SEC(gameLocal.time);

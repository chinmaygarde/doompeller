...
================
*/
bool idBrittleFracture::UpdateRenderEntity(
  ...
  tris = renderEntity->hModel->AllocSurfaceTriangles(
      numTris * 3,
      material->ShouldCreateBackSides() ? numTris * 6 : numTris * 3);
  decalTris = renderEntity->hModel->AllocSurfaceTriangles(
      numDecalTris * 3, decalMaterial->ShouldCreateBackSides()
                            ? numDecalTris * 6
                            : numDecalTris * 3);
  ...
  for (i = 0; i < shards.Num(); i++) {
    ...
    packedColor = PackColor(
        idVec4(renderEntity->shaderParms[SHADERPARM_RED] * fade,
               renderEntity->shaderParms[SHADERPARM_GREEN] * fade,
               renderEntity->shaderParms[SHADERPARM_BLUE] * fade, fade));
  ...
  SIMDProcessor->MinMax(tris->bounds[0], tris->bounds[1], tris->verts,
                        tris->numVerts);
  SIMDProcessor->MinMax(decalTris->bounds[0], decalTris->bounds[1],
                        decalTris->verts, decalTris->numVerts);
...
================
*/
bool idBrittleFracture::ModelCallback(renderEntity_s* renderEntity,
                                      const renderView_t* renderView) {
  const idBrittleFracture* ent;

  ent = static_cast<idBrittleFracture*>(
      gameLocal.entities[renderEntity->entityNum]);
  if (!ent) {
    gameLocal.Error(
...
void idBrittleFracture::Present() {
  ...
  // force an update because the bounds/origin/axis may stay the same while the
  // model changes
...
================
*/
void idBrittleFracture::ApplyImpulse(idEntity* ent,
                                     int id,
                                     const idVec3& point,
...
================
*/
void idBrittleFracture::AddForce(idEntity* ent,
                                 int id,
                                 const idVec3& point,
                                 const idVec3& force) {
  if (id < 0 || id >= shards.Num()) {
...
================
*/
void idBrittleFracture::ProjectDecal(const idVec3& point,
                                     const idVec3& dir,
                                     const int time,
  ...
  if (time >= gameLocal.time) {
    ...
    if (damageDefName) {
      ...
      if (damageDef) {
        sndShader = declManager->FindSound(
            damageDef->dict.GetString("snd_shatter", ""));
      }
    }
...
void idBrittleFracture::DropShard(shard_t* shard,
  ...
  dir2 = origin - point;
  dist = dir2.Normalize();
  f = dist > maxShatterRadius
          ? 1.0f
          : idMath::Sqrt(dist - minShatterRadius) *
                (1.0f / idMath::Sqrt(maxShatterRadius - minShatterRadius));
  ...
  shard->physicsObj.ApplyImpulse(0, origin,
                                 impulse * linearVelocityScale * dir);
  shard->physicsObj.SetAngularVelocity(dir.Cross(dir2) *
                                       (f * angularVelocityScale));
...
================
*/
void idBrittleFracture::Shatter(const idVec3& point,
                                const idVec3& impulse,
                                const int time) {
  int i;
  idVec3 dir;
  ...
  if (fxFracture.Length()) {
    idEntityFx::StartFx(fxFracture, &point, &GetPhysics()->GetAxis(), this,
                        true);
  }
  ...
  for (i = 0; i < shards.Num(); i++) {
    ...
    if ((shard->clipModel->GetOrigin() - point).LengthSqr() >
        Square(maxShatterRadius)) {
      continue;
    }
...
================
*/
void idBrittleFracture::DropFloatingIslands(const idVec3& point,
                                            const idVec3& impulse,
                                            const int time) {
  int i, j, numIslands;
  ...
  for (i = 0; i < shards.Num(); i++) {
    ...
    for (curShard = queue[queueStart]; queueStart < queueEnd;
...
================
*/
void idBrittleFracture::Killed(idEntity* inflictor,
                               idEntity* attacker,
                               int damage,
                               const idVec3& dir,
                               int location) {
...
================
*/
void idBrittleFracture::AddDamageEffect(const trace_t& collision,
                                        const idVec3& velocity,
                                        const char* damageDefName) {
  if (!disableFracture) {
    ProjectDecal(collision.c.point, collision.c.normal, gameLocal.time,
                 damageDefName);
  }
}
    ...
    for (k = 0; k < w1.GetNumPoints(); k++) {
      ...
      for (j = 0; j < shards.Num(); j++) {
        ...
        for (l = w2.GetNumPoints() - 1; l >= 0; l--) {
          ...
          p2 = origin2 +
               w2[(l - 1 + w2.GetNumPoints()) % w2.GetNumPoints()].ToVec3() *
                   axis2;
          if (plane[0].Side(p2, 0.1f) == SIDE_FRONT &&
              plane[1].Side(p1, 0.1f) == SIDE_FRONT) {
            if (plane[2].Side(p1, 0.1f) == SIDE_ON &&
                plane[3].Side(p1, 0.1f) == SIDE_ON) {
              if (plane[2].Side(p2, 0.1f) == SIDE_ON &&
                  plane[3].Side(p2, 0.1f) == SIDE_ON) {
                shard1->neighbours.Append(shard2);
                shard1->edgeHasNeighbour[k] = true;
                shard2->neighbours.Append(shard1);
                shard2->edgeHasNeighbour[(l - 1 + w2.GetNumPoints()) %
                                         w2.GetNumPoints()] = true;
                break;
              }
            }
          }
...
void idBrittleFracture::Event_Touch(idEntity* other, trace_t* trace) {
  ...
  impulse =
      other->GetPhysics()->GetLinearVelocity() * other->GetPhysics()->GetMass();
...
================
*/
bool idBrittleFracture::ClientReceiveEvent(int event,
                                           int time,
                                           const idBitMsg& msg) {
  idVec3 point, dir;

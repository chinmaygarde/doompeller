...
bool idBitMsg::CheckOverflow(int numBits) {
  ...
  if (numBits > GetRemainingWriteBits()) {
    ...
    if (numBits > (maxSize << 3)) {
      idLib::common->FatalError("idBitMsg: %i bits is > full message size",
                                numBits);
    }
...
byte* idBitMsg::GetByteSpace(int length) {
  ...
  if (!writeData) {
    idLib::common->FatalError(
        "idBitMsg::GetByteSpace: cannot write to message");
  }
...
void idBitMsg::WriteBits(int value, int numBits) {
  ...
  // this should be an error really, as it can go unnoticed and cause either
  // bandwidth or corrupted data transmitted
  if (numBits != 32) {
    if (numBits > 0) {
      if (value > (1 << numBits) - 1) {
        idLib::common->Warning("idBitMsg::WriteBits: value overflow %d %d",
                               value, numBits);
      } else if (value < 0) {
        idLib::common->Warning("idBitMsg::WriteBits: value overflow %d %d",
                               value, numBits);
...
idVec3 idBitMsg::BitsToDir(int bits, int numBits) {
  ...
  dir.x = sign[(bits >> (numBits * 3 - 1)) & 1] *
          ((bits >> (numBits * 2)) & max) * invMax;
  dir.y = sign[(bits >> (numBits * 2 - 1)) & 1] *
          ((bits >> (numBits * 1)) & max) * invMax;
  dir.z = sign[(bits >> (numBits * 1 - 1)) & 1] *
          ((bits >> (numBits * 0)) & max) * invMax;
  dir.NormalizeFast();
  return dir;
}

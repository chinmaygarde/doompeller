...
void idCmdArgs::TokenizeString(const char* text, bool keepAsStrings) {
  ...
  lex.SetFlags(LEXFL_NOERRORS | LEXFL_NOWARNINGS | LEXFL_NOSTRINGCONCAT |
               LEXFL_ALLOWPATHNAMES | LEXFL_NOSTRINGESCAPECHARS |
               LEXFL_ALLOWIPADDRESSES |
               (keepAsStrings ? LEXFL_ONLYSTRINGS : 0));
  ...
  while (1) {
    ...
    idStr::Copynz(tokenized + totalLen, token.c_str(),
                  sizeof(tokenized) - totalLen);
...
void idCmdArgs::AppendArg(const char* text) {
  if (!argc) {
    ...
    idStr::Copynz(argv[argc], text,
                  sizeof(tokenized) - (argv[argc] - tokenized));
    argc++;
  }
}

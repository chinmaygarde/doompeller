...
int default_nextpunctuation[sizeof(default_punctuations) /
                            sizeof(punctuation_t)];
int default_setup;
...
void idLexer::CreatePunctuationTable(const punctuation_t* punctuations) {
  ...
  if (punctuations == default_punctuations) {
    ...
    if (!idLexer::punctuationtable ||
        idLexer::punctuationtable == default_punctuationtable) {
      idLexer::punctuationtable = (int*)Mem_Alloc(256 * sizeof(int));
    }
    if (idLexer::nextpunctuation &&
        idLexer::nextpunctuation != default_nextpunctuation) {
  ...
  for (i = 0; punctuations[i].p; i++) {
    ...
    // first)
    for (n = idLexer::punctuationtable[(unsigned int)newp->p[0]]; n >= 0;
...
void idLexer::Error(const char* str, ...) {
  ...
  if (idLexer::flags & LEXFL_NOFATALERRORS) {
    idLib::common->Warning("file %s, line %d: %s", idLexer::filename.c_str(),
                           idLexer::line, text);
  } else {
    idLib::common->Error("file %s, line %d: %s", idLexer::filename.c_str(),
                         idLexer::line, text);
  }
}
...
int idLexer::ReadString(idToken* token, int quote) {
  ...
  while (1) {
    ...
    if (*idLexer::script_p == '\\' &&
        !(idLexer::flags & LEXFL_NOSTRINGESCAPECHARS)) {
    ...
    // if a trailing quote
      ...
      if ((idLexer::flags & LEXFL_NOSTRINGCONCAT) &&
          (!(idLexer::flags & LEXFL_ALLOWBACKSLASHSTRINGCONCAT) ||
           (quote != '\"'))) {
        break;
      }
...
int idLexer::ReadName(idToken* token) {
  ...
  do {
    ...
    c = *idLexer::script_p;
      ...
      ((idLexer::flags & LEXFL_ALLOWPATHNAMES) &&
       (c == '/' || c == '\\' || c == ':' || c == '.')));
  token->data[token->len] = '\0';
...
#ifdef PUNCTABLE
  for (n = idLexer::punctuationtable[(unsigned int)*(idLexer::script_p)];
       n >= 0; n = idLexer::nextpunctuation[n]) {
...
int idLexer::ReadToken(idToken* token) {
  ...
  // if there is a number
           ...
           (c == '.' && (*(idLexer::script_p + 1) >= '0' &&
                         *(idLexer::script_p + 1) <= '9'))) {
...
int idLexer::ExpectTokenType(int type, int subtype, idToken* token) {
  ...
  if (token->type == TT_NUMBER) {
    ...
    if (token->subtype != subtype) {
      idLexer::Error("expected '%s' but found '%s'",

  ...
  while (!idParser::tokens) {
    ...
    if (idParser::scriptstack->EndOfFile()) {
      ...
      while (idParser::indentstack &&
             idParser::indentstack->script == idParser::scriptstack) {
...
int idParser::MergeTokens(idToken* t1, idToken* t2) {
  ...
  if (t1->type == TT_NAME &&
      (t2->type == TT_NAME ||
       (t2->type == TT_NUMBER && !(t2->subtype & TT_FLOAT)))) {
    t1->Append(t2->c_str());
    return true;
  }
  ...
  if (t1->type == TT_NUMBER && t2->type == TT_NUMBER &&
      !(t1->subtype & (TT_HEX | TT_BINARY)) &&
      !(t2->subtype & (TT_HEX | TT_BINARY)) &&
...
void idParser::AddBuiltinDefines(void) {
  ...
  struct builtin {
    char* string;
    int id;
  } builtin[] = {{"__LINE__", BUILTIN_LINE}, {"__FILE__", BUILTIN_FILE},
                 {"__DATE__", BUILTIN_DATE}, {"__TIME__", BUILTIN_TIME},
                 {"__STDC__", BUILTIN_STDC}, {NULL, 0}};

  for (i = 0; builtin[i].string; i++) {
    define =
        (define_t*)Mem_Alloc(sizeof(define_t) + strlen(builtin[i].string) + 1);
...
int idParser::ExpandDefine(idToken* deftoken,
  ...
  if (define->builtin) {
    return idParser::ExpandBuiltinDefine(deftoken, define, firsttoken,
                                         lasttoken);
  }
      ...
      if ((*t->next) == "##") {
        ...
        if (t2) {
          if (!idParser::MergeTokens(t1, t2)) {
            idParser::Error("can't merge '%s' with '%s'", t1->c_str(),
                            t2->c_str());
            return false;
          }
          delete t1->next;
...
int idParser::Directive_undef(void) {
  ...
  for (lastdefine = NULL, define = idParser::definehash[hash]; define;
    ...
    switch (t->type) {
      ...
      case TT_PUNCTUATION: {
        ...
        switch (t->subtype) {
          ...
          case P_QUESTIONMARK: {
            if (!lastwasvalue) {
              idParser::Error("operator '%s' after operator in #if/#elif",
                              t->c_str());
              error = 1;
              break;
            }
  ...
  while (!error && firstoperator) {
    ...
    if (integer) {
      Log_Write("operator %s, value1 = %d",
                idParser::scriptstack->getPunctuationFromId(o->op),
                v1->intvalue);
      if (v2)
...
int idParser::ReadToken(idToken* token) {
  ...
  while (1) {
    ...
    if (token->type == TT_PUNCTUATION && (*token)[0] == '#' &&
        (*token)[1] == '\0') {
    ...
    // resolving defines
    if (token->type == TT_STRING &&
        !(idParser::scriptstack->GetFlags() & LEXFL_NOSTRINGCONCAT)) {
      idToken newtoken;
...
int idParser::ExpectTokenType(int type, int subtype, idToken* token) {
  ...
  if (token->type != type) {
    ...
    idParser::Error("expected a %s but found '%s'", str.c_str(),
                    token->c_str());
    return 0;
  }
  if (token->type == TT_NUMBER) {
    if ((token->subtype & subtype) != subtype) {
      ...
      if (subtype & TT_INTEGER)
        str += "integer ";
      str.StripTrailing(' ');
      idParser::Error("expected %s but found '%s'", str.c_str(),
                      token->c_str());
      return 0;
    }
    ...
    if (token->subtype != subtype) {
      idParser::Error("expected '%s' but found '%s'",
...
int idParser::LoadFile(const char* filename, bool OSPath) {
  ...
  if (idParser::loaded) {
    idLib::common->FatalError(
        "idParser::loadFile: another source already loaded");
    return false;
  }
  ...
  if (!idParser::definehash) {
    ...
    idParser::definehash =
        (define_t**)Mem_ClearedAlloc(DEFINEHASHSIZE * sizeof(define_t*));
    idParser::AddGlobalDefinesToSource();
  }
  return true;
}
...
int idParser::LoadMemory(const char* ptr, int length, const char* name) {
  ...
  if (idParser::loaded) {
    idLib::common->FatalError(
        "idParser::loadMemory: another source already loaded");
    return false;
  }

...
#define SMALL_ALIGN(bytes) \
  (ALIGN_SIZE((bytes) + SMALL_HEADER_SIZE) - SMALL_HEADER_SIZE)
...
class idHeap {
 ...
 private:
  ...
  void* SmallAllocate(
      dword bytes);  // allocate memory (1-255 bytes) from small heap manager
...
void idHeap::Free(void* p) {
  ...
  switch (((byte*)(p))[-1]) {
    ...
    default: {
      idLib::common->FatalError("idHeap::Free: invalid memory block (%s)",
                                idLib::sys->GetCallStackCurStr(4));
      break;
    }
  }
#endif
...
dword idHeap::Msize(void* p) {
  ...
  switch (((byte*)(p))[-1]) {
    ...
    case LARGE_ALLOC: {
      return ((idHeap::page_s*)(*(
                  (dword*)(((byte*)p) - ALIGN_SIZE(LARGE_HEADER_SIZE)))))
                 ->dataSize -
             ALIGN_SIZE(LARGE_HEADER_SIZE);
    }
    default: {
      idLib::common->FatalError("idHeap::Msize: invalid memory block (%s)",
                                idLib::sys->GetCallStackCurStr(4));
      return 0;
    }
  }
...
idHeap::page_s* idHeap::AllocatePage(dword bytes) {
  ...
  if (swapPage &&
      swapPage->dataSize == bytes) {  // if we've got a swap page somewhere
    p = swapPage;
    swapPage = NULL;
  } else {
    dword size;
...
void* idHeap::SmallAllocate(dword bytes) {
  ...
  smallBlock[1] = SMALL_ALLOC;            // allocation identifier
  smallCurPageOffset +=
      bytes + SMALL_HEADER_SIZE;  // increase the offset on the current page
...
void idHeap::MediumFree(void* ptr) {
  ...
  mediumHeapEntry_s* e =
      (mediumHeapEntry_s*)((byte*)ptr - ALIGN_SIZE(MEDIUM_HEADER_SIZE));
...
void idHeap::LargeFree(void* ptr) {
  ...
  // get page pointer
  pg = (idHeap::page_s*)(*(
      (dword*)(((byte*)ptr) - ALIGN_SIZE(LARGE_HEADER_SIZE))));
...
const char* Mem_CleanupFileName(const char* fileName) {
  ...
  while (1) {
    ...
    newFileName = newFileName.Left(i2 - 1) +
                  newFileName.Right(newFileName.Length() - (i1 + 4));
  }
  index = (index + 1) & 3;
  strncpy(newFileNames[index], newFileName.c_str(),
          sizeof(newFileNames[index]));
  ...
  for (numBlocks = 0, b = mem_debugMemory; b; b = b->next, numBlocks++) {
    ...
    if ((b->size >> 10) != 0) {
      fprintf(f, "size: %6d KB: %s, line: %d [%s], call stack: %s\r\n",
              (b->size >> 10), Mem_CleanupFileName(b->fileName), b->lineNumber,
              dump,
              idLib::sys->GetCallStackStr(b->callStack, MAX_CALLSTACK_DEPTH));
    } else {
      fprintf(f, "size: %7d B: %s, line: %d [%s], call stack: %s\r\n", b->size,
              Mem_CleanupFileName(b->fileName), b->lineNumber, dump,
              idLib::sys->GetCallStackStr(b->callStack, MAX_CALLSTACK_DEPTH));
    }
  }
...
void Mem_DumpCompressed(const char* fileName,
                        memorySortType_t memSort,
                        int sortCallStack,
                        int numFrames) {
  int numBlocks, totalSize, r, j;
  debugMemory_t* b;
  allocInfo_t *a, *nexta, *allocInfo = NULL, *sortedAllocInfo = NULL,
  ...
  for (a = allocInfo; a; a = nexta) {
    ...
    switch (memSort) {
      // sort on size
      case MEMSORT_SIZE: {
        for (nextSorted = sortedAllocInfo; nextSorted;
      ...
      case MEMSORT_LOCATION: {
        for (nextSorted = sortedAllocInfo; nextSorted;
             nextSorted = nextSorted->next) {
          r = idStr::Cmp(Mem_CleanupFileName(a->fileName),
                         Mem_CleanupFileName(nextSorted->fileName));
      ...
      case MEMSORT_CALLSTACK: {
        for (nextSorted = sortedAllocInfo; nextSorted;
             nextSorted = nextSorted->next) {
          if (a->callStack[sortCallStack] <
              nextSorted->callStack[sortCallStack]) {
            break;
          }
          prevSorted = nextSorted;
        }
  ...
  for (a = sortedAllocInfo; a; a = nexta) {
    ...
    fprintf(f, "size: %6d KB, allocs: %5d: %s, line: %d, call stack: %s\r\n",
            ...
            a->lineNumber,
            idLib::sys->GetCallStackStr(a->callStack, MAX_CALLSTACK_DEPTH));
    ::free(a);
  }
...
void Mem_DumpCompressed_f(const idCmdArgs& args) {
  ...
  while (arg[0] == '-') {
    ...
    if (idStr::Icmp(arg, "s") == 0) {
      ...
      idLib::common->Printf(
          "memoryDumpCompressed [options] [filename]\n"
          "options:\n"
          "  -s     sort on size\n"
          "  -l     sort on location\n"
...
==================
*/
void* Mem_AllocDebugMemory(const int size,
                           const char* fileName,
...
==================
*/
void Mem_FreeDebugMemory(void* p,
                         const char* fileName,
                         const int lineNumber,
                         const bool align16) {
  debugMemory_t* m;
  ...
  if (m->size < 0) {
    idLib::common->FatalError(
        "memory freed twice, first from %s, now from %s",
        idLib::sys->GetCallStackStr(m->callStack, MAX_CALLSTACK_DEPTH),
        idLib::sys->GetCallStackCurStr(MAX_CALLSTACK_DEPTH));
  }
...
void Mem_Shutdown(void) {
  if (mem_leakName[0] != '\0') {
    Mem_DumpCompressed(va("%s_leak_size.txt", mem_leakName), MEMSORT_SIZE, 0,
                       0);
    Mem_DumpCompressed(va("%s_leak_location.txt", mem_leakName),
                       MEMSORT_LOCATION, 0, 0);
    Mem_DumpCompressed(va("%s_leak_cs1.txt", mem_leakName), MEMSORT_CALLSTACK,
                       2, 0);
  }

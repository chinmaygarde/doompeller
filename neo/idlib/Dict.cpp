...
void idDict::TransferKeyValues(idDict& other) {
  ...
  if (other.args.Num() && other.args[0].key->GetPool() != &globalKeys) {
    common->FatalError(
        "idDict::TransferKeyValues: can't transfer values across a DLL "
        "boundary");
    return;
  }
...
void idDict::SetDefaults(const idDict* dict) {
  ...
  for (i = 0; i < n; i++) {
    ...
    if (!kv) {
      ...
      argHash.Add(argHash.GenerateKey(newkv.GetKey(), false),
                  args.Append(newkv));
    }
  }
}
...
bool idDict::GetMatrix(const char* key,
  ...
  sscanf(s, "%f %f %f %f %f %f %f %f %f", &out[0].x, &out[0].y, &out[0].z,
         &out[1].x, &out[1].y, &out[1].z, &out[2].x, &out[2].y, &out[2].z);
  return found;
}
...
const idKeyValue* idDict::MatchPrefix(const char* prefix,
                                      const idKeyValue* lastMatch) const {
  int i;
  int len;
  int start;
...
const char* idDict::RandomPrefix(const char* prefix, idRandom& random) const {
  ...
  list[0] = "";
  for (count = 0, kv = MatchPrefix(prefix); kv && count < MAX_RANDOM_KEYS;
...
void idDict::WriteToFileHandle(idFile* f) const {
  ...
  f->Write(&c, sizeof(c));
  for (int i = 0; i < args.Num();
       i++) {  // don't loop on the swapped count use the original

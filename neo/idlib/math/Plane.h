...
class idPlane {
 public:
  ...
  idPlane& operator=(
      const idVec3& v);  // sets normal and sets idPlane::d to zero
  ...
  bool Compare(const idPlane& p,
               const float epsilon) const;  // compare with epsilon
  bool Compare(const idPlane& p,
               const float normalEps,
               const float distEps) const;  // compare with epsilon
  ...
  float Normalize(bool fixDegenerate = true);  // only normalizes the plane
                                               // normal, does not adjust d
  ...
  bool FromPoints(const idVec3& p1,
                  const idVec3& p2,
                  const idVec3& p3,
                  bool fixDegenerate = true);
  bool FromVecs(const idVec3& dir1,
                const idVec3& dir2,
                const idVec3& p,
                bool fixDegenerate = true);
  ...
  bool RayIntersection(const idVec3& start,
                       const idVec3& dir,
                       float& scale) const;
  bool PlaneIntersection(const idPlane& plane,
                         idVec3& start,
                         idVec3& dir) const;

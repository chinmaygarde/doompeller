...
static idCVar lcp_showFailures("lcp_showFailures",
                               "0",
...
class idLCP_Square : public idLCP {
 public:
  virtual bool Solve(const idMatX& o_m,
                     idVecX& o_x,
                     const idVecX& o_b,
                     const idVecX& o_lo,
                     const idVecX& o_hi,
...
void idLCP_Square::RemoveClamped(int r) {
  ...
  for (i = r; i < numClamped; i++) {
    ...
    if (diag == 0.0f) {
      idLib::common->Printf(
          "idLCP_Square::RemoveClamped: updating factorization failed\n");
      return;
    }
...
ID_INLINE void idLCP_Square::ChangeAccel(int d, float step) {
  ...
  SIMDProcessor->MulAdd(a.ToFloatPtr() + numClamped, step,
                        delta_a.ToFloatPtr() + numClamped, d - numClamped + 1);
}
...
bool idLCP_Square::Solve(const idMatX& o_m,
  ...
  if (boxIndex) {
    for (i = m.GetNumRows() - 1; i >= numUnbounded; i--) {
      if (boxIndex[i] >= 0 &&
          (lo[i] != -idMath::INFINITY || hi[i] != idMath::INFINITY)) {
        boxStartIndex--;
        if (boxStartIndex != i) {
  ...
  clamped.SetData(m.GetNumRows(), m.GetNumColumns(),
                  MATX_ALLOCA(m.GetNumRows() * m.GetNumColumns()));
  ...
  if (numUnbounded) {
    ...
    if (!FactorClamped()) {
      idLib::common->Printf(
          "idLCP_Square::Solve: unbounded factorization failed\n");
      return false;
    }
...
void idLCP_Symmetric::SolveClamped(idVecX& x, const float* b) {
  // solve L
  SIMDProcessor->MatX_LowerTriangularSolve(clamped, solveCache1.ToFloatPtr(), b,
                                           numClamped, clampedChangeStart);

  // solve D
  SIMDProcessor->Mul(solveCache2.ToFloatPtr(), solveCache1.ToFloatPtr(),
                     diagonal.ToFloatPtr(), numClamped);
...
void idLCP_Symmetric::AddClamped(int r, bool useSolveCache) {
  ...
  if (useSolveCache) {
    ...
    // CalcForceDelta
    memcpy(clamped[numClamped], solveCache2.ToFloatPtr(),
           numClamped * sizeof(float));
    // calculate row dot product
    SIMDProcessor->Dot(dot, solveCache2.ToFloatPtr(), solveCache1.ToFloatPtr(),
                       numClamped);

  } else {
    float* v = (float*)_alloca16(numClamped * sizeof(float));

    SIMDProcessor->MatX_LowerTriangularSolve(clamped, v, rowPtrs[numClamped],
                                             numClamped);
    // add bottom row to L
    SIMDProcessor->Mul(clamped[numClamped], v, diagonal.ToFloatPtr(),
                       numClamped);
    // calculate row dot product
  ...
  if (d == 0.0f) {
    idLib::common->Printf(
        "idLCP_Symmetric::AddClamped: updating factorization failed\n");
    numClamped++;
    return;
  }
...
bool idLCP_Symmetric::Solve(const idMatX& o_m,
  ...
  if (numUnbounded) {
    ...
    if (!FactorClamped()) {
      idLib::common->Printf(
          "idLCP_Symmetric::Solve: unbounded factorization failed\n");
      return false;
    }
    SolveClamped(f, b.ToFloatPtr());

...
class idPolynomial {
 public:
  ...
  bool Compare(const idPolynomial& p,
  ...
  float GetValue(const float x)
      const;  // evaluate the polynomial with the given real value
  idComplex GetValue(const idComplex& x)
      const;  // evaluate the polynomial with the given complex value
  idPolynomial GetDerivative(
      void) const;  // get the first derivative of the polynomial
  idPolynomial GetAntiDerivative(
      void) const;  // get the anti derivative of the polynomial
...
ID_INLINE int idPolynomial::GetRoots4(float a,
  ...
  GetRoots3(1.0f, -c, b * d - 4.0f * e, -b * b * e + 4.0f * c * e - d * d,
            roots3);
  y = roots3[0];
  ds = 0.25f * b * b - c + y;

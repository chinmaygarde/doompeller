...
#define ADDRESS_ADDRC(reg0, reg1, constant) \
  0x44 _asm _emit((reg1 & 7) << 3) | (reg0 & 7) _asm _emit constant

#define ADDRESS_SCALEADDR(reg0, reg1, scale) \
  0x04 _asm _emit((reg1 & 7) << 3) | (reg0 & 7) | RSCALE(scale)

#define ADDRESS_SCALEADDRC(reg0, reg1, scale, constant) \
  0x44 _asm _emit((reg1 & 7) << 3) | (reg0 & 7) |       \
      RSCALE(scale) _asm _emit constant
...
#define addsubps(dst, src)                                                     \
  _asm _emit 0xF2 _asm _emit 0x0F _asm _emit 0xD0 _asm _emit((dst & 7) << 3) | \
      src

// Packed Double-FP Add/Subtract ( dst[0]=dst[0]+src[0], dst[1]=dst[1]-src[1] )
#define addsubpd(dst, src)                                                     \
  _asm _emit 0x66 _asm _emit 0x0F _asm _emit 0xD0 _asm _emit((dst & 7) << 3) | \
      src
...
#define haddps(dst, src)                                                       \
  _asm _emit 0xF2 _asm _emit 0x0F _asm _emit 0x7C _asm _emit((dst & 7) << 3) | \
      src

// Packed Double-FP Horizontal Add ( dst[0]=dst[0]+dst[1], dst[1]=src[0]+src[1]
// )
#define haddpd(dst, src)                                                       \
  _asm _emit 0x66 _asm _emit 0x0F _asm _emit 0x7C _asm _emit((dst & 7) << 3) | \
      src
...
============
*/
void VPCALL idSIMD_SSE3::TransformVerts(idDrawVert* verts,
                                        const int numVerts,

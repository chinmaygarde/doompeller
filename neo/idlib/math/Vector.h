...
class idVec3 {
 public:
  ...
  bool ProjectAlongPlane(const idVec3& normal,
                         const float epsilon,
...
class idVec6 {
 public:
  ...
  explicit idVec6(const float a1,
                  const float a2,
                  const float a3,
                  const float a4,
                  const float a5,
...
ID_INLINE float idVec6::operator*(const idVec6& a) const {
  return p[0] * a[0] + p[1] * a[1] + p[2] * a[2] + p[3] * a[3] + p[4] * a[4] +
         p[5] * a[5];
}
...
ID_INLINE idVec6 idVec6::operator/(const float a) const {
  ...
  assert(a != 0.0f);
  inva = 1.0f / a;
  return idVec6(p[0] * inva, p[1] * inva, p[2] * inva, p[3] * inva, p[4] * inva,
                p[5] * inva);
}

ID_INLINE idVec6 idVec6::operator+(const idVec6& a) const {
  return idVec6(p[0] + a[0], p[1] + a[1], p[2] + a[2], p[3] + a[3], p[4] + a[4],
                p[5] + a[5]);
}
...
ID_INLINE float idVec6::Length(void) const {
  return (float)idMath::Sqrt(p[0] * p[0] + p[1] * p[1] + p[2] * p[2] +
                             p[3] * p[3] + p[4] * p[4] + p[5] * p[5]);
}

ID_INLINE float idVec6::LengthSqr(void) const {
  return (p[0] * p[0] + p[1] * p[1] + p[2] * p[2] + p[3] * p[3] + p[4] * p[4] +
          p[5] * p[5]);
}
...
ID_INLINE float idVec6::Normalize(void) {
  ...
  sqrLength = p[0] * p[0] + p[1] * p[1] + p[2] * p[2] + p[3] * p[3] +
              p[4] * p[4] + p[5] * p[5];
  invLength = idMath::InvSqrt(sqrLength);
  p[0] *= invLength;
  p[1] *= invLength;

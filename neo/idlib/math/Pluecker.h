...
class idPluecker {
 public:
  ...
  explicit idPluecker(const float a1,
                      const float a2,
                      const float a3,
                      const float a4,
                      const float a5,
  ...
  float PermutedInnerProduct(
      const idPluecker& a) const;  // pluecker permuted inner product
...
ID_INLINE float idPluecker::operator*(const idPluecker& a) const {
  return p[0] * a.p[4] + p[1] * a.p[5] + p[2] * a.p[3] + p[4] * a.p[0] +
         p[5] * a.p[1] + p[3] * a.p[2];
}
...
ID_INLINE idPluecker idPluecker::operator/(const float a) const {
  ...
  assert(a != 0.0f);
  inva = 1.0f / a;
  return idPluecker(p[0] * inva, p[1] * inva, p[2] * inva, p[3] * inva,
                    p[4] * inva, p[5] * inva);
}

ID_INLINE idPluecker idPluecker::operator+(const idPluecker& a) const {
  return idPluecker(p[0] + a[0], p[1] + a[1], p[2] + a[2], p[3] + a[3],
                    p[4] + a[4], p[5] + a[5]);
}

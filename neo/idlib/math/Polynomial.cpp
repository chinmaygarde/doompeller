...
int idPolynomial::Laguer(const idComplex* coef,
  ...
  static const float frac[] = {0.0f,  0.5f,  0.25f, 0.75f, 0.13f,
                               0.38f, 0.62f, 0.88f, 1.0f};
  int i, j;
  ...
  for (i = 1; i <= MAX_ITERATIONS; i++) {
    ...
    if (Max(abp, abm) > 0.0f) {
      ...
      dx = idMath::Exp(idMath::Log(1.0f + abx)) *
           idComplex(idMath::Cos(i), idMath::Sin(i));
    }
    cx = x - dx;
    if (x == cx) {
      return i;
...
int idPolynomial::GetRoots(idComplex* roots) const {
  ...
  coef = (idComplex*)__builtin_alloca_with_align(
      (degree + 1) * sizeof(idComplex), 16);
  for (i = 0; i <= degree; i++) {
...
int idPolynomial::GetRoots(float* roots) const {
  ...
  switch (degree) {
    ...
    case 3:
      return GetRoots3(coefficient[3], coefficient[2], coefficient[1],
                       coefficient[0], roots);
    case 4:
      return GetRoots4(coefficient[4], coefficient[3], coefficient[2],
                       coefficient[1], coefficient[0], roots);
  }
  ...
  complexRoots =
      (idComplex*)__builtin_alloca_with_align(degree * sizeof(idComplex), 16);
...
void idPolynomial::Test(void) {
  ...
  for (i = 0; i < num; i++) {
    ...
    assert(idMath::Fabs(complexValue.r) < 1e-4f &&
           idMath::Fabs(complexValue.i) < 1e-4f);
  }

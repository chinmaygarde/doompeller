...
typedef enum {
  EXTRAPOLATION_NONE = 0x01,  // no extrapolation, covered distance = duration *
                              // 0.001 * ( baseSpeed )
  EXTRAPOLATION_LINEAR = 0x02,  // linear extrapolation, covered distance =
                                // duration * 0.001 * ( baseSpeed + speed )
  EXTRAPOLATION_ACCELLINEAR =
      0x04,  // linear acceleration, covered distance = duration * 0.001 * (
             // baseSpeed + 0.5 * speed )
  EXTRAPOLATION_DECELLINEAR =
      0x08,  // linear deceleration, covered distance = duration * 0.001 * (
             // baseSpeed + 0.5 * speed )
  EXTRAPOLATION_ACCELSINE =
      0x10,  // sinusoidal acceleration, covered distance = duration * 0.001 * (
             // baseSpeed + sqrt( 0.5 ) * speed )
  EXTRAPOLATION_DECELSINE =
      0x20,  // sinusoidal deceleration, covered distance = duration * 0.001 * (
             // baseSpeed + sqrt( 0.5 ) * speed )
...
class idExtrapolate {
 public:
  ...
  bool IsDone(float time) const {
    return (!(extrapolationType & EXTRAPOLATION_NOSTOP) &&
  ...
  float GetEndTime(void) const {
    return (!(extrapolationType & EXTRAPOLATION_NOSTOP) && duration > 0)
               ? startTime + duration
               : 0;
  }
...
*/
template <class type>
ID_INLINE void idExtrapolate<type>::Init(
...
ID_INLINE type idExtrapolate<type>::GetCurrentValue(float time) const {
  ...
  if (!(extrapolationType & EXTRAPOLATION_NOSTOP) &&
      (time > startTime + duration)) {
    time = startTime + duration;
  }
  ...
  switch (extrapolationType & ~EXTRAPOLATION_NOSTOP) {
    ...
    case EXTRAPOLATION_ACCELSINE: {
      if (!duration) {
        ...
        deltaTime = (time - startTime) / duration;
        s = (1.0f - idMath::Cos(deltaTime * idMath::HALF_PI)) * duration *
            0.001f * idMath::SQRT_1OVER2;
        currentValue = startValue + deltaTime * baseSpeed + s * speed;
      }
      break;
    }
    case EXTRAPOLATION_DECELSINE: {
      if (!duration) {
        ...
        deltaTime = (time - startTime) / duration;
        s = idMath::Sin(deltaTime * idMath::HALF_PI) * duration * 0.001f *
            idMath::SQRT_1OVER2;
        currentValue = startValue + deltaTime * baseSpeed + s * speed;
      }
      break;

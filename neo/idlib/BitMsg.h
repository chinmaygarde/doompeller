...
class idBitMsg {
 public:
  ...
  void SetAllowOverflow(
      bool set);  // generate error if not set and message is overflowed
  ...
  void WriteDeltaFloat(float oldValue,
                       float newValue,
                       int exponentBits,
                       int mantissaBits);
 ...
 private:
  ...
  bool allowOverflow;     // if false, generate an error when the message is
                          // overflowed
...
ID_INLINE void idBitMsg::WriteDeltaFloat(float oldValue, float newValue) {
  WriteDelta(*reinterpret_cast<int*>(&oldValue),
             *reinterpret_cast<int*>(&newValue), 32);
}

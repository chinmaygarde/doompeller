...
ID_INLINE int idVectorSet<type, dimension>::FindVector(const type& v,
  ...
  for (i = 0; i < dimension; i++) {
    ...
    partialHashKey[i] =
        (int)((v[i] - mins[i] - boxHalfSize[i]) * boxInvSize[i]);
  }
...
class idVectorSubset {
 public:
  idVectorSubset(void);
  idVectorSubset(const type& mins,
                 const type& maxs,
                 const int boxHashSize,
                 const int initialSize);
  ...
  int FindVector(const type* vectorList,
                 const int vectorNum,
                 const float epsilon);
...
template <class type, int dimension>
ID_INLINE int idVectorSubset<type, dimension>::FindVector(

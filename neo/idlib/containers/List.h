...
class idList {
 public:
  ...
  size_t Size(void) const;  // returns total size of allocated memory including
                            // size of list type
  size_t MemoryUsed(
      void) const;  // returns size of the used elements in the list
  ...
  void Condense(
      void);  // resizes list to exactly the number of elements it contains
  ...
  void AssureSize(int newSize);     // assure list has given number of elements,
                                    // but leave them uninitialized
  void AssureSize(
      int newSize,
      const type& initValue);  // assure list has given number of elements and
  ...
  type& Alloc(
      void);  // returns reference to a new data element at the end of the list
  ...
  int FindNull(
      void) const;  // find the index for the first NULL pointer in the list
  int IndexOf(const type* obj)
      const;  // returns the index for the pointer to an element in the list
  bool RemoveIndex(int index);   // remove the element at the given index
  bool Remove(const type& obj);  // remove the element
  void Sort(cmp_t* compare = (cmp_t*)&idListSortCompare<type>);
  void SortSubSection(int startIndex,
                      int endIndex,
...
ID_INLINE void idList<type>::SortSubSection(int startIndex,
  ...
  cmp_c* vCompare = (cmp_c*)compare;
  qsort((void*)(&list[startIndex]), (size_t)(endIndex - startIndex + 1),
        sizeof(type), vCompare);
}

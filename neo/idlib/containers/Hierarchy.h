...
class idHierarchy {
 public:
  ...
  type* GetNextLeaf(
      void) const;  // goes through all leaf nodes of the hierarchy
 ...
 private:
  ...
  idHierarchy<type>* GetPriorSiblingNode(
      void) const;  // previous node with the same parent
};
...
idHierarchy<type>* idHierarchy<type>::GetPriorSiblingNode(void) const {
  ...
  if (node != this) {
    idLib::Error(
        "idHierarchy::GetPriorSibling: could not find node in parent's list of "
        "children");
  }

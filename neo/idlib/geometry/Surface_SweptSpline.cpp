...
====================
*/
void idSurface_SweptSpline::SetSweptSpline(
...
void idSurface_SweptSpline::Tessellate(const int splineSubdivisions,
  ...
  totalTime = sweptSpline->GetTime(sweptSpline->GetNumValues() - 1) -
              sweptSpline->GetTime(0) + sweptSpline->GetCloseTime();
  sweptSplineDiv =
      sweptSpline->GetBoundaryType() == idCurve_Spline<idVec3>::BT_CLOSED
  ...
  // sweep the spline
  totalTime = spline->GetTime(spline->GetNumValues() - 1) - spline->GetTime(0) +
              spline->GetCloseTime();
  splineDiv = spline->GetBoundaryType() == idCurve_Spline<idVec3>::BT_CLOSED
                  ? splineSubdivisions

...
class idSurface {
 public:
  ...
  explicit idSurface(const idDrawVert* verts,
                     const int numVerts,
                     const int* indexes,
  ...
  // splits the surface into a front and back surface, the surface itself stays
  // unchanged frontOnPlaneEdges and backOnPlaneEdges optionally store the
  // indexes to the edges that lay on the split plane returns a SIDE_?
  int Split(const idPlane& plane,
            const float epsilon,
            idSurface** front,
            idSurface** back,
            int* frontOnPlaneEdges = NULL,
            int* backOnPlaneEdges = NULL) const;
  ...
  // set to zero
  bool ClipInPlace(const idPlane& plane,
                   const float epsilon = ON_EPSILON,
                   const bool keepOn = false);

  // returns true if each triangle can be reached from any other triangle by a
  // traversal
  ...
  bool LineIntersection(const idVec3& start,
                        const idVec3& end,
 ...
 protected:
  ...
  idList<int> edgeIndexes;  // 3 references to edges for each triangle, may be
                            // negative for reversed edge

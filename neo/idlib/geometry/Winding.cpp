...
idWinding* idWinding::Clip(const idPlane& plane,
  ...
  newPoints =
      (idVec5*)__builtin_alloca_with_align((maxpts * sizeof(idVec5)), 16);
  newNumPoints = 0;
...
void idWinding::RemoveEqualPoints(const float epsilon) {
  ...
  for (i = 0; i < numPoints; i++) {
    if ((p[i].ToVec3() - p[(i + numPoints - 1) % numPoints].ToVec3())
            .LengthSqr() >= Square(epsilon)) {
      continue;
    }
    numPoints--;
...
void idWinding::RemoveColinearPoints(const idVec3& normal,
  ...
  for (i = 0; i < numPoints; i++) {
    ...
    edgeNormal = (p[i].ToVec3() - p[(i + numPoints - 1) % numPoints].ToVec3())
                     .Cross(normal);
    edgeNormal.Normalize();
    dist = edgeNormal * p[i].ToVec3();

    if (idMath::Fabs(edgeNormal * p[(i + 1) % numPoints].ToVec3() - dist) >
        epsilon) {
      continue;
    }
...
void idWinding::AddToConvexHull(const idWinding* winding,
  ...
  for (i = 0; i < winding->numPoints; i++) {
    ...
    for (k = 0; k < this->numPoints; k++) {
      if (hullSide[(j + k) % this->numPoints] &&
          hullSide[(j + k + 1) % this->numPoints]) {
        continue;
      }
...
void idWinding::AddToConvexHull(const idVec3& point,
  ...
  switch (numPoints) {
    ...
    case 2: {
      ...
      // if only two points make sure we have the right ordering according to
      // the normal
...
bool idWinding::InsertPointIfOnEdge(const idVec3& point,
  ...
  for (i = 0; i < numPoints; i++) {
    // create plane through edge orthogonal to winding plane
    normal =
        (p[(i + 1) % numPoints].ToVec3() - p[i].ToVec3()).Cross(plane.Normal());
    normal.Normalize();
...
bool idFixedWinding::ReAllocate(int n, bool keep) {
  ...
  if (n > MAX_POINTS_ON_WINDING) {
    idLib::common->Printf(
        "WARNING: idFixedWinding -> MAX_POINTS_ON_WINDING overflowed\n");
    return false;
  }
  return true;
}

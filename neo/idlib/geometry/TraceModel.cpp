...
void idTraceModel::SetupDodecahedron(const idBounds& dodBounds) {
  ...
  b[0] = b[1] = b[2] =
      0.3568220897730899f;  // ( ( 3.0f - ( 5.0f ) ^ 0.5f ) / 6.0f ) ^ 0.5f;
  c[0] = c[1] = c[2] =
      0.9341723589627156f;  // ( ( 3.0f + ( 5.0f ) ^ 0.5f ) / 6.0f ) ^ 0.5f;
  d = 0.5f / c[0];
...
void idTraceModel::SetupCylinder(const idBounds& cylBounds,
  ...
  if (n * 2 > MAX_TRACEMODEL_VERTS) {
    idLib::common->Printf(
        "WARNING: idTraceModel::SetupCylinder: too many vertices\n");
    n = MAX_TRACEMODEL_VERTS / 2;
  }
  if (n * 3 > MAX_TRACEMODEL_EDGES) {
    idLib::common->Printf(
        "WARNING: idTraceModel::SetupCylinder: too many sides\n");
    n = MAX_TRACEMODEL_EDGES / 3;
  }
  if (n + 2 > MAX_TRACEMODEL_POLYS) {
    idLib::common->Printf(
        "WARNING: idTraceModel::SetupCylinder: too many polygons\n");
  ...
  for (i = 0; i < n; i++) {
    ...
    polys[i].normal =
        (verts[(i + 1) % n] - verts[i]).Cross(verts[n + i] - verts[i]);
    polys[i].normal.Normalize();
...
void idTraceModel::SetupCone(const idBounds& coneBounds, const int numSides) {
  ...
  if (n + 1 > MAX_TRACEMODEL_VERTS) {
    idLib::common->Printf(
        "WARNING: idTraceModel::SetupCone: too many vertices\n");
  ...
  if (n + 1 > MAX_TRACEMODEL_POLYS) {
    idLib::common->Printf(
        "WARNING: idTraceModel::SetupCone: too many polygons\n");
  ...
  for (i = 0; i < n; i++) {
    // polygon plane
    polys[i].normal =
        (verts[(i + 1) % n] - verts[i]).Cross(verts[n] - verts[i]);
    polys[i].normal.Normalize();
    polys[i].dist = polys[i].normal * verts[i];
    // polygon bounds
    polys[i].bounds.Clear();
...
void idTraceModel::SetupBone(const float length, const float width) {
  ...
  polys[2].normal.Set(-polys[0].normal[0], polys[0].normal[1],
                      polys[0].normal[2]);
  polys[3].normal.Set(polys[0].normal[0], polys[0].normal[1],
                      -polys[0].normal[2]);
  polys[5].normal.Set(-polys[0].normal[0], polys[0].normal[1],
                      -polys[0].normal[2]);
  polys[1].normal = (verts[3] - verts[0]).Cross(verts[2] - verts[0]);
  polys[1].normal.Normalize();
  polys[4].normal.Set(polys[1].normal[0], polys[1].normal[1],
                      -polys[1].normal[2]);
...
void idTraceModel::SetupPolygon(const idVec3* v, const int count) {
  ...
  if (numVerts * 3 > MAX_TRACEMODEL_EDGES) {
    idLib::common->Printf(
        "WARNING: idTraceModel::SetupPolygon: too many vertices\n");
...
void idTraceModel::SetupPolygon(const idWinding& w) {
  ...
  verts = (idVec3*)__builtin_alloca_with_align(
      (w.GetNumPoints() * sizeof(idVec3)), 16);
...
void idTraceModel::VolumeFromPolygon(idTraceModel& trm, float thickness) const {
  ...
  for (i = 0; i < numEdges; i++) {
    ...
    trm.polys[2 + i].normal =
        (verts[(i + 1) % numVerts] - verts[i]).Cross(polys[0].normal);
    ...
    for (j = 0; j < poly->numEdges; j++) {
      ...
      if (edge->normal[0] == 0.0f && edge->normal[1] == 0.0f &&
        ...
        if (dot < SHARP_EDGE_DOT) {
          ...
          edge->normal =
              (0.5f / (0.5f + 0.5f * dot)) * (edge->normal + poly->normal);
        }
      }
    }
  }
...
void idTraceModel::Rotate(const idMat3& rotation) {
  ...
  for (i = 0; i < numPolys; i++) {
    ...
    for (j = 0; j < polys[i].numEdges; j++) {
      edgeNum = polys[i].edges[j];
      polys[i].bounds.AddPoint(
          verts[edges[abs(edgeNum)].v[INTSIGNBITSET(edgeNum)]]);
    }
    polys[i].dist =
        polys[i].normal * verts[edges[abs(edgeNum)].v[INTSIGNBITSET(edgeNum)]];
...
void idTraceModel::Shrink(const float m) {
  ...
  if (type == TRM_POLYGON) {
    for (i = 0; i < numEdges; i++) {
      ...
      edge = &edges[abs(edgeNum)];
      dir = verts[edge->v[INTSIGNBITSET(edgeNum)]] -
            verts[edge->v[INTSIGNBITNOTSET(edgeNum)]];
...
float idTraceModel::GetPolygonArea(int polyNum) const {
  ...
  for (i = 0; i < poly->numEdges; i++) {
    v1 = verts[edges[abs(poly->edges[i])].v[INTSIGNBITSET(poly->edges[i])]] -
         base;
    v2 = verts[edges[abs(poly->edges[i])].v[INTSIGNBITNOTSET(poly->edges[i])]] -
         base;
    cross = v1.Cross(v2);
    total += cross.Length();
  }
...
============
*/
int idTraceModel::GetOrderedSilhouetteEdges(
    const int edgeIsSilEdge[MAX_TRACEMODEL_EDGES + 1],
...
============
*/
int idTraceModel::GetProjectionSilhouetteEdges(
  ...
  for (i = 0; i < numPolys; i++) {
    ...
    dir =
        verts[edges[abs(edgeNum)].v[INTSIGNBITSET(edgeNum)]] - projectionOrigin;
...
============
*/
int idTraceModel::GetParallelProjectionSilhouetteEdges(
...
/*
  ...
  credits to Brian Mirtich for his paper "Fast and Accurate Computation of
...
void idTraceModel::ProjectionIntegrals(
    ...
    int a,
    int b,
    struct projectionIntegrals_s& integrals) const {
  const traceModelPoly_t* poly;
  int i, edgeNum;
  idVec3 v1, v2;
  float a0, a1, da;
...
============
*/
void idTraceModel::PolygonIntegrals(
    int polyNum,
    int a,
    int b,
    int c,
    struct polygonIntegrals_s& integrals) const {
  ...
  integrals.Fcc = k3 * (Square(n[a]) * pi.Paa + 2 * n[a] * n[b] * pi.Pab +
                        Square(n[b]) * pi.Pbb +
...
void idTraceModel::VolumeIntegrals(struct volumeIntegrals_s& integrals) const {
  ...
  for (i = 0; i < numPolys; i++) {
    ...
    integrals.T0 +=
        poly->normal[0] * ((a == 0) ? pi.Fa : ((b == 0) ? pi.Fb : pi.Fc));
...
void idTraceModel::GetMassProperties(const float density,
  ...
  inertiaTensor[0][0] -= mass * (centerOfMass[1] * centerOfMass[1] +
                                 centerOfMass[2] * centerOfMass[2]);
  inertiaTensor[1][1] -= mass * (centerOfMass[2] * centerOfMass[2] +
                                 centerOfMass[0] * centerOfMass[0]);
  inertiaTensor[2][2] -= mass * (centerOfMass[0] * centerOfMass[0] +
                                 centerOfMass[1] * centerOfMass[1]);
  inertiaTensor[0][1] = inertiaTensor[1][0] +=
      mass * centerOfMass[0] * centerOfMass[1];
  inertiaTensor[1][2] = inertiaTensor[2][1] +=
      mass * centerOfMass[1] * centerOfMass[2];
  inertiaTensor[2][0] = inertiaTensor[0][2] +=
      mass * centerOfMass[2] * centerOfMass[0];
}

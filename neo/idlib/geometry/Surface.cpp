...
int idSurface::Split(const idPlane& plane,
                     ...
                     idSurface** front,
                     idSurface** back,
                     int* frontOnPlaneEdges,
                     int* backOnPlaneEdges) const {
  float* dists;
  float f;
  byte* sides;
  int counts[3];
  ...
  *front = *back = NULL;
  ...
  if (!counts[SIDE_FRONT] && !counts[SIDE_BACK]) {
    f = (verts[indexes[1]].xyz - verts[indexes[0]].xyz)
            .Cross(verts[indexes[0]].xyz - verts[indexes[2]].xyz) *
        plane.Normal();
  ...
  *back = surface[1] = new idSurface();
  ...
  surface[0]->indexes.Resize(((counts[SIDE_FRONT] + counts[SIDE_ON]) * 2) +
                             (numEdgeSplitVertexes * 4));
  surface[1]->indexes.Resize(((counts[SIDE_BACK] + counts[SIDE_ON]) * 2) +
                             (numEdgeSplitVertexes * 4));
  ...
  vertexCopyIndex[0] =
      (int*)_alloca((numEdgeSplitVertexes + verts.Num()) * sizeof(int));
  vertexCopyIndex[1] =
      (int*)_alloca((numEdgeSplitVertexes + verts.Num()) * sizeof(int));
  ...
  for (i = 0; i < edgeIndexes.Num(); i += 3) {
    ...
    switch ((INTSIGNBITSET(edgeSplitVertex[e0]) |
             (INTSIGNBITSET(edgeSplitVertex[e1]) << 1) |
             (INTSIGNBITSET(edgeSplitVertex[e2]) << 2)) ^
            7) {
      case 0: {  // no edges split
        if ((sides[v0] & sides[v1] & sides[v2]) & SIDE_ON) {
          // coplanar
          f = (verts[v1].xyz - verts[v0].xyz)
                  .Cross(verts[v0].xyz - verts[v2].xyz) *
              plane.Normal();
          s = FLOATSIGNBITSET(f);
        } else {
        ...
        index = indexPtr[s];
        index[n++] = UpdateVertexIndex(vertexIndexNum[s], vertexRemap[s],
                                       vertexCopyIndex[s], v0);
        index[n++] = UpdateVertexIndex(vertexIndexNum[s], vertexRemap[s],
                                       vertexCopyIndex[s], v1);
        index[n++] = UpdateVertexIndex(vertexIndexNum[s], vertexRemap[s],
                                       vertexCopyIndex[s], v2);
        indexNum[s] = n;
        break;
      }
  ...
  if (frontOnPlaneEdges) {
    memcpy(frontOnPlaneEdges, onPlaneEdges[0],
           numOnPlaneEdges[0] * sizeof(int));
    ...
    switch ((INTSIGNBITSET(edgeSplitVertex[e0]) |
      ...
      case 1: {  // first edge split
        if (!(sides[v0] & SIDE_BACK)) {
          indexPtr[indexNum++] = UpdateVertexIndex(vertexIndexNum, vertexRemap,
                                                   vertexCopyIndex, v0);
...
bool idSurface::IsConnected(void) const {
  ...
  for (i = 0; i < numTris; i++) {
    ...
    for (curTri = queue[queueStart]; queueStart < queueEnd;
...
bool idSurface::IsPolytope(const float epsilon) const {
  ...
  for (i = 0; i < indexes.Num(); i += 3) {
    plane.FromPoints(verts[indexes[i + 0]].xyz, verts[indexes[i + 1]].xyz,
...
void idSurface::GenerateEdgeIndexes(void) {
  ...
  vertexEdges =
      (int*)__builtin_alloca_with_align((verts.Num() * sizeof(int)), 16);
  memset(vertexEdges, -1, verts.Num() * sizeof(int));
  edgeChain =
      (int*)__builtin_alloca_with_align((indexes.Num() * sizeof(int)), 16);
  ...
  for (i = 0; i < indexes.Num(); i += 3) {
    ...
    for (j = 0; j < 3; j++) {
      v0 = e[j].verts[0];
      v1 = e[j].verts[1];
      for (edgeNum = vertexEdges[v0]; edgeNum >= 0;
      ...
      if (index[j] == v0) {
        assert(edges[edgeNum].tris[0] ==
               -1);  // edge may not be shared by more than two triangles
        edges[edgeNum].tris[0] = i;
        edgeIndexes[i + j] = edgeNum;
      } else {
        assert(edges[edgeNum].tris[1] ==
               -1);  // edge may not be shared by more than two triangles

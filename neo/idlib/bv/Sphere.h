...
class idSphere {
 public:
  ...
  bool AddPoint(
      const idVec3& p);  // add the point, returns true if the sphere expanded
  bool AddSphere(const idSphere&
                     s);  // add the sphere, returns true if the sphere expanded
  idSphere Expand(const float d)
  ...
  void FromSphereTranslation(const idSphere& sphere,
                             const idVec3& start,
                             const idVec3& translation);
  // Most tight sphere for a rotation.
  void FromPointRotation(const idVec3& point, const idRotation& rotation);
  void FromSphereRotation(const idSphere& sphere,
                          const idVec3& start,
...
ID_INLINE bool idSphere::Compare(const idSphere& a, const float epsilon) const {
  return (origin.Compare(a.origin, epsilon) &&
          idMath::Fabs(radius - a.radius) <= epsilon);
}
...
ID_INLINE void idSphere::FromSphereRotation(const idSphere& sphere,
  ...
  radius =
      idMath::Sqrt(0.5f * (end - sphere.origin).LengthSqr()) + sphere.radius;
}

...
idBounds::LineIntersection
  ...
  Returns true if the line intersects the bounds between the start and end
point.
============
*/
...
idBounds BoundsForPointRotation(const idVec3& start,
  ...
  origin =
      rotation.GetOrigin() + axis * (axis * (start - rotation.GetOrigin()));
...
void idBounds::FromBoundsRotation(const idBounds& bounds,
  ...
  if (idMath::Fabs(rotation.GetAngle()) < 180.0f) {
    ...
    radius =
        (bounds[1] - point).Length() + (point - rotation.GetOrigin()).Length();

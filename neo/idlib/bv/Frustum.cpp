...
static int boxVertPlanes[8] = {
    ((1 << 0) | (1 << 2) | (1 << 4)), ((1 << 1) | (1 << 2) | (1 << 4)),
    ((1 << 1) | (1 << 3) | (1 << 4)), ((1 << 0) | (1 << 3) | (1 << 4)),
    ((1 << 0) | (1 << 2) | (1 << 5)), ((1 << 1) | (1 << 2) | (1 << 5)),
    ((1 << 1) | (1 << 3) | (1 << 5)), ((1 << 0) | (1 << 3) | (1 << 5)),
};
...
bool idFrustum::CullLocalBox(const idVec3& localOrigin,
  ...
  d2 = idMath::Fabs(extents[0] *
                    (dFar * testAxis[0][1] - dLeft * testAxis[0][0])) +
       idMath::Fabs(extents[1] *
                    (dFar * testAxis[1][1] - dLeft * testAxis[1][0])) +
       idMath::Fabs(extents[2] *
                    (dFar * testAxis[2][1] - dLeft * testAxis[2][0]));
  if (d1 - d2 > 0.0f) {
    return true;
  }
  ...
  d2 =
      idMath::Fabs(extents[0] *
                   (dFar * testAxis[0][2] - dUp * testAxis[0][0])) +
      idMath::Fabs(extents[1] *
                   (dFar * testAxis[1][2] - dUp * testAxis[1][0])) +
...
bool idFrustum::CullLocalFrustum(const idFrustum& localFrustum,
                                 ...
                                 const idVec3 cornerVecs[4]) const {
  int index;
...
bool idFrustum::LocalFrustumIntersectsFrustum(const idVec3 points[8],
                                              const bool testFirstSide) const {
  int i;
...
bool idFrustum::IntersectsBox(const idBox& box) const {
  ...
  if (BoundsCullLocalFrustum(idBounds(-box.GetExtents(), box.GetExtents()),
                             localFrustum, indexPoints, cornerVecs)) {
    return false;
  }
  ...
  if (LocalFrustumIntersectsBounds(
          indexPoints, idBounds(-box.GetExtents(), box.GetExtents()))) {
    return true;
  }
...
bool idFrustum::IntersectsFrustum(const idFrustum& frustum) const {
  ...
  if (LocalFrustumIntersectsFrustum(indexPoints2,
                                    (localFrustum2.dNear > 0.0f))) {
    return true;
  }
  ...
  if (frustum.LocalFrustumIntersectsFrustum(indexPoints1,
                                            (localFrustum1.dNear > 0.0f))) {
    return true;
  }
...
bool idFrustum::IntersectsWinding(const idWinding& winding) const {
  ...
  if (dNear > 0.0f) {
    for (i = 0; i < 4; i++) {
      if (winding.LineIntersection(plane, indexPoints[i],
                                   indexPoints[(i + 1) & 3])) {
        return true;
      }
    }
  ...
  for (i = 0; i < 4; i++) {
    if (winding.LineIntersection(plane, indexPoints[4 + i],
                                 indexPoints[4 + ((i + 1) & 3)])) {
      return true;
    }
  }
...
bool idFrustum::LineIntersection(const idVec3& start, const idVec3& end) const {
  return LocalLineIntersection((start - origin) * axis.Transpose(),
...
bool idFrustum::RayIntersection(const idVec3& start,
                                ...
                                float& scale1,
                                float& scale2) const {
  if (LocalRayIntersection((start - origin) * axis.Transpose(),
                           dir * axis.Transpose(), scale1, scale2)) {
    return true;
  }
  if (scale1 <= scale2) {
    return true;
...
bool idFrustum::FromProjection(const idBounds& bounds,
                               ...
                               const float dFar) {
  return FromProjection(idBox(bounds, vec3_origin, mat3_identity),
                        projectionOrigin, dFar);
}
...
bool idFrustum::FromProjection(const idBox& box,
  ...
  for (i = 1; i < 3; i++) {
    ...
    if (value * box.GetExtents()[bestAxis] * box.GetExtents()[bestAxis] <
        bestValue * box.GetExtents()[i] * box.GetExtents()[i]) {
      bestValue = value;
      bestAxis = i;
    }
  }
  ...
  for (j = 0; j < 2; j++) {
    axis[0] = dir;
    axis[1] =
        box.GetAxis()[bestAxis] - (box.GetAxis()[bestAxis] * axis[0]) * axis[0];
    axis[1].Normalize();
    axis[2].Cross(axis[0], axis[1]);

    BoxToPoints((box.GetCenter() - projectionOrigin) * axis.Transpose(),
                box.GetExtents(), box.GetAxis() * axis.Transpose(), points);
    ...
    if (j == 0) {
      dir += idMath::Tan16(0.5f *
                           (idMath::ATan16(points[minY].y, points[minY].x) +
                            idMath::ATan16(points[maxY].y, points[maxY].x))) *
             axis[1];
      dir += idMath::Tan16(0.5f *
                           (idMath::ATan16(points[minZ].z, points[minZ].x) +
                            idMath::ATan16(points[maxZ].z, points[maxZ].x))) *
             axis[2];
      dir.Normalize();
    }
  }
  ...
  this->dFar = dFar;
  this->dLeft = Max(idMath::Fabs(points[minY].y / points[minY].x),
                    idMath::Fabs(points[maxY].y / points[maxY].x)) *
                dFar;
  this->dUp = Max(idMath::Fabs(points[minZ].z / points[minZ].x),
                  idMath::Fabs(points[maxZ].z / points[maxZ].x)) *
              dFar;
  ...
  for (j = 0; j < 2; j++) {
    ...
    if (j == 0) {
      dir += idMath::Tan16(
                 0.5f * (idMath::ATan16(b[1][1]) + idMath::ATan16(b[0][1]))) *
             axis[1];
      dir += idMath::Tan16(
                 0.5f * (idMath::ATan16(b[1][2]) + idMath::ATan16(b[0][2]))) *
             axis[2];
      dir.Normalize();
    }
  }
...
void idFrustum::AxisProjection(const idVec3 indexPoints[8],
  ...
  index = (FLOATSIGNBITSET(dy) << 1) | FLOATSIGNBITSET(dz);
  dx = dir.x * cornerVecs[index].x + dir.y * cornerVecs[index].y +
       dir.z * cornerVecs[index].z;
  index |= (FLOATSIGNBITSET(dx) << 2);
...
#ifdef FRUSTUM_DEBUG
  static idCVar r_showInteractionScissors(
      "r_showInteractionScissors", "0", CVAR_RENDERER | CVAR_INTEGER, "", 0, 2,
      idCmdSystem::ArgCompletion_Integer<0, 2>);
  if (r_showInteractionScissors.GetInteger() > 1) {
    session->rw->DebugLine(colorGreen, origin + start * axis,
                           origin + end * axis);
  }
#endif
...
bool idFrustum::ProjectionBounds(const idBounds& bounds,
                                 idBounds& projectionBounds) const {
  return ProjectionBounds(idBox(bounds, vec3_origin, mat3_identity),
                          projectionBounds);
}
...
bool idFrustum::ProjectionBounds(const idBox& box,
  ...
  if (bounds.ContainsPoint((origin - box.GetCenter()) *
                           box.GetAxis().Transpose())) {
  ...
  for (i = 0; i < 4; i++) {
    p1 = i;
    p2 = 4 + i;
    AddLocalLineToProjectionBoundsSetCull(points[p1], points[p2], pointCull[p1],
                                          pointCull[p2], projectionBounds);
  ...
  for (i = 0; i < 4; i++) {
    p1 = i;
    p2 = (i + 1) & 3;
    AddLocalLineToProjectionBoundsUseCull(points[p1], points[p2], pointCull[p1],
                                          pointCull[p2], projectionBounds);
  }
  ...
  if (outside != 1 && outside != 2 && outside != 4 && outside != 8) {
    ...
    if ((outside & 2) && (outside & 8)) {
      BoundsRayIntersection(bounds, localOrigin,
...
bool idFrustum::ProjectionBounds(const idFrustum& frustum,
  ...
  if (localFrustum.dNear > 0.0f) {
    for (i = 0; i < 4; i++) {
      p1 = i;
      p2 = (i + 1) & 3;
      AddLocalLineToProjectionBoundsUseCull(points[p1], points[p2],
                                            pointCull[p1], pointCull[p2],
                                            projectionBounds);
    }
  }
  ...
  if (outside != 1 && outside != 2 && outside != 4 && outside != 8) {
    ...
    if ((outside & 2) && (outside & 8)) {
      frustum.LocalRayIntersection(
          localOrigin, localScaled[0] - localScaled[1] - localScaled[2], scale1,
          scale2);
      if (scale1 <= scale2 && scale1 >= 0.0f) {
        projectionBounds.AddPoint(idVec3(scale1 * dFar, -1.0f, -1.0f));
        projectionBounds.AddPoint(idVec3(scale2 * dFar, -1.0f, -1.0f));
...
bool idFrustum::ProjectionBounds(const idWinding& winding,
  ...
  for (i = 0; i < winding.GetNumPoints(); i += 2) {
    ...
    AddLocalLineToProjectionBoundsSetCull(localPoints[p1], localPoints[p2],
                                          pointCull[p1], pointCull[p2],
                                          projectionBounds);
    culled &= pointCull[p1] & pointCull[p2];
    outside |= pointCull[p1] | pointCull[p2];
  ...
  for (i = 1; i < winding.GetNumPoints(); i += 2) {
    p1 = i;
    p2 = (i + 1) % winding.GetNumPoints();
    AddLocalLineToProjectionBoundsUseCull(localPoints[p1], localPoints[p2],
                                          pointCull[p1], pointCull[p2],
                                          projectionBounds);
  }
  ...
  if (outside != 1 && outside != 2 && outside != 4 && outside != 8) {
    ...
    if ((outside & 2) && (outside & 8)) {
      if (winding.RayIntersection(plane, origin,
                                  scaled[0] - scaled[1] - scaled[2], scale)) {
...
bool idFrustum::ClipLine(const idVec3 localPoints[8],
  ...
  if (!(startCull | endCull)) {
    ...
    if (!startCull) {
      ...
      start =
          points[startIndex] + scale1 * (points[endIndex] - points[startIndex]);
    }
    if (!endCull) {
      end = points[endIndex];
      endClip = -1;
          ...
          points[startIndex] + scale2 * (points[endIndex] - points[startIndex]);
    }
    return true;
  }
  return false;
...
============
*/
static int capPointIndex[4][2] = {{0, 3}, {1, 2}, {0, 1}, {2, 3}};

ID_INLINE bool idFrustum::AddLocalCapsToProjectionBounds(
    const idVec3 endPoints[4],
  ...
  AddLocalLineToProjectionBoundsUseCull(
      endPoints[p[0]], point, endPointCull[p[0]], pointCull, projectionBounds);
  AddLocalLineToProjectionBoundsUseCull(
      endPoints[p[1]], point, endPointCull[p[1]], pointCull, projectionBounds);
  return true;
}
...
bool idFrustum::ClippedProjectionBounds(const idFrustum& frustum,
                                        const idBox& clipBox,
                                        idBounds& projectionBounds) const {
  int i, p1, p2, clipPointCull[8], clipPlanes[4], usedClipPlanes, nearCull,
      farCull, outside;
  int pointCull[2], startClip, endClip, boxPointCull[8];
  float clipFractions[4], s1, s2, t1, t2, leftScale, upScale;
  idFrustum localFrustum;
  idVec3 clipPoints[8], localPoints1[8], localPoints2[8], localOrigin1,
      localOrigin2, start, end;
  ...
  usedClipPlanes =
      clipPlanes[0] | clipPlanes[1] | clipPlanes[2] | clipPlanes[3];
  ...
  for (i = 0; i < 4; i++) {
    p1 = i;
    p2 = 4 + i;
    AddLocalLineToProjectionBoundsSetCull(clipPoints[p1], clipPoints[p2],
                                          clipPointCull[p1], clipPointCull[p2],
                                          projectionBounds);
  }

  // get cull bits for the clipped frustum
  outside = clipPointCull[0] | clipPointCull[1] | clipPointCull[2] |
            clipPointCull[3] | clipPointCull[4] | clipPointCull[5] |
            clipPointCull[6] | clipPointCull[7];
  nearCull =
      clipPointCull[0] & clipPointCull[1] & clipPointCull[2] & clipPointCull[3];
  farCull =
  ...
  if (outside) {
    ...
    if (!nearCull && localFrustum.dNear > 0.0f) {
      for (i = 0; i < 4; i++) {
        ...
        p2 = (i + 1) & 3;
        AddLocalLineToProjectionBoundsUseCull(
            clipPoints[p1], clipPoints[p2], clipPointCull[p1],
            clipPointCull[p2], projectionBounds);
      }
    }
  ...
  if (!(farCull && !(nearCull & farCull)) &&
      ...
      // bounds
      (clipPlanes[0] != clipPlanes[1] || clipPlanes[1] != clipPlanes[2] ||
       clipPlanes[2] != clipPlanes[3])) {
    ...
    for (i = 0; i < 4; i++) {
      ...
      if (!(boxPointCull[p1] & boxPointCull[p2])) {
        if (frustum.ClipLine(localPoints1, localPoints2, p1, p2, start, end,
                             startClip, endClip)) {
          AddLocalLineToProjectionBoundsSetCull(start, end, pointCull[0],
                                                pointCull[1], projectionBounds);
          AddLocalCapsToProjectionBounds(clipPoints + 4, clipPointCull + 4,
                                         start, pointCull[0], startClip,
                                         projectionBounds);
          AddLocalCapsToProjectionBounds(clipPoints + 4, clipPointCull + 4, end,
                                         pointCull[1], endClip,
                                         projectionBounds);
  ...
  if (outside != 1 && outside != 2 && outside != 4 && outside != 8) {
    ...
    if ((outside & 2) && (outside & 8)) {
      frustum.LocalRayIntersection(
          localOrigin1, localAxis1[0] - localAxis1[1] - localAxis1[2], s1, s2);
      if (s1 <= s2 && s1 >= 0.0f) {
        BoundsRayIntersection(clipBounds, localOrigin2,
                              localAxis2[0] - localAxis2[1] - localAxis2[2], t1,
                              t2);

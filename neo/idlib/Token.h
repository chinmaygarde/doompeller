...
class idToken : public idStr {
 ...
 public:
  ...
  int WhiteSpaceBeforeToken(
      void) const;                  // returns length of whitespace before token
 ...
 private:
  ...
  const char* whiteSpaceStart_p;  // start of white space before token, only
                                  // used by idLexer
  const char*
      whiteSpaceEnd_p;  // end of white space before token, only used by idLexer

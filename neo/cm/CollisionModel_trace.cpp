...
void idCollisionModelManagerLocal::TraceTrmThroughNode(cm_traceWork_t* tw,
  ...
  if (tw->positionTest) {
    ...
    for (pref = node->polygons; pref; pref = pref->next) {
      if (idCollisionModelManagerLocal::TranslateTrmThroughPolygon(tw,
                                                                   pref->p)) {
        return;
      }
    }
...
void idCollisionModelManagerLocal::TraceThroughAxialBSPTree_r(
    cm_traceWork_t* tw,
    cm_node_t* node,
    float p1f,
    float p2f,
  ...
  idCollisionModelManagerLocal::TraceThroughAxialBSPTree_r(
      tw, node->children[0], p1f, p2f, p1, p2);
  idCollisionModelManagerLocal::TraceThroughAxialBSPTree_r(
      tw, node->children[1], p1f, p2f, p1, p2);
  return;
#endif
  ...
  idCollisionModelManagerLocal::TraceThroughAxialBSPTree_r(
      tw, node->children[side], p1f, midf, p1, mid);
  ...
  idCollisionModelManagerLocal::TraceThroughAxialBSPTree_r(
      tw, node->children[side ^ 1], midf, p2f, mid, p2);
}
...
void idCollisionModelManagerLocal::TraceThroughModel(cm_traceWork_t* tw) {
  ...
  if (!tw->rotation) {
    ...
    idCollisionModelManagerLocal::TraceThroughAxialBSPTree_r(
        tw, tw->model->node, 0, 1, tw->start, tw->end);
  } else {
    ...
    if (d > CIRCLE_APPROXIMATION_LENGTH) {
      ...
      for (i = 0; i < numSteps; i++) {
        ...
        // trace through spatial subdivision and then through leafs
        idCollisionModelManagerLocal::TraceThroughAxialBSPTree_r(
            tw, tw->model->node, 0, 1, start, end);
    ...
    idCollisionModelManagerLocal::TraceThroughAxialBSPTree_r(
        tw, tw->model->node, 0, 1, start, tw->end);
  }
}

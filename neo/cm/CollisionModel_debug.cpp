...
idCVar cm_drawMask("cm_drawMask",
                   "none",
                   CVAR_GAME,
                   "collision mask",
                   cm_contentsNameByIndex,
                   idCmdSystem::ArgCompletion_String<cm_contentsNameByIndex>);
idCVar cm_drawColor("cm_drawColor",
                    "1 0 0 .5",
                    CVAR_GAME,
                    "color used to draw the collision models");
idCVar cm_drawFilled("cm_drawFilled",
                     "0",
                     CVAR_GAME | CVAR_BOOL,
                     "draw filled polygons");
idCVar cm_drawInternal("cm_drawInternal",
                       "1",
                       CVAR_GAME | CVAR_BOOL,
                       "draw internal edges green");
idCVar cm_drawNormals("cm_drawNormals",
                      "0",
                      CVAR_GAME | CVAR_BOOL,
                      "draw polygon and edge normals");
idCVar cm_backFaceCull("cm_backFaceCull",
                       "0",
                       CVAR_GAME | CVAR_BOOL,
                       "cull back facing polygons");
idCVar cm_debugCollision("cm_debugCollision",
                         "0",
                         CVAR_GAME | CVAR_BOOL,
...
================
*/
const char* idCollisionModelManagerLocal::StringFromContents(
  ...
  for (i = 1; cm_contentsFlagByIndex[i] != 0; i++) {
    if (contents & cm_contentsFlagByIndex[i]) {
      if (length != 0) {
        length += idStr::snPrintf(contentsString + length,
...
void idCollisionModelManagerLocal::DrawPolygon(cm_model_t* model,
  ...
  if (cm_drawFilled.GetBool()) {
    ...
    for (i = p->numEdges - 1; i >= 0; i--) {
      ...
      winding +=
          origin +
          model->vertices[edge->vertexNum[INTSIGNBITSET(edgeNum)]].p * axis;
    }
  ...
  while (1) {
    ...
    if (radius && viewOrigin[node->planeType] > node->planeDist + radius) {
      node = node->children[0];
               ...
               viewOrigin[node->planeType] < node->planeDist - radius) {
      node = node->children[1];
    } else {
      DrawNodePolygons(model, node->children[1], origin, axis, viewOrigin,
                       radius);
      node = node->children[0];
    }
  }
...
================
*/
void idCollisionModelManagerLocal::DrawModel(cmHandle_t handle,
  ...
  if (cm_drawColor.IsModified()) {
    sscanf(cm_drawColor.GetString(), "%f %f %f %f", &cm_color.x, &cm_color.y,
           &cm_color.z, &cm_color.w);
...
static idCVar cm_testCollision("cm_testCollision",
                               "0",
                               CVAR_GAME | CVAR_BOOL,
                               "");
static idCVar cm_testRotation("cm_testRotation",
                              "1",
                              CVAR_GAME | CVAR_BOOL,
                              "");
static idCVar cm_testModel("cm_testModel", "0", CVAR_GAME | CVAR_INTEGER, "");
static idCVar cm_testTimes("cm_testTimes",
                           "1000",
                           CVAR_GAME | CVAR_INTEGER,
                           "");
static idCVar cm_testRandomMany("cm_testRandomMany",
                                "0",
                                CVAR_GAME | CVAR_BOOL,
                                "");
...
static idCVar cm_testLength("cm_testLength",
                            "1024",
...
void idCollisionModelManagerLocal::DebugOutput(const idVec3& origin) {
  ...
  if (cm_testReset.GetBool() ||
      (cm_testWalk.GetBool() && !start.Compare(start))) {
  ...
  if (cm_testWalk.GetBool()) {
    ...
    cm_testOrigin.SetString(
        va("%1.2f %1.2f %1.2f", start[0], start[1], start[2]));
  } else {
    sscanf(cm_testOrigin.GetString(), "%f %f %f", &start[0], &start[1],
           &start[2]);
  }

  sscanf(cm_testBox.GetString(), "%f %f %f %f %f %f", &bounds[0][0],
         &bounds[0][1], &bounds[0][2], &bounds[1][0], &bounds[1][1],
         &bounds[1][2]);
  sscanf(cm_testBoxRotation.GetString(), "%f %f %f", &boxAngles[0],
         &boxAngles[1], &boxAngles[2]);
  ...
  if (cm_testRandomMany.GetBool()) {
    ...
    for (i = 0; i < 3; i++) {
      testend[0][i] =
          start[i] + random.CRandomFloat() * cm_testLength.GetFloat();
    }
  ...
  for (i = 0; i < cm_testTimes.GetInteger(); i++) {
    Translation(&trace, start, testend[i], &itm, boxAxis,
                CONTENTS_SOLID | CONTENTS_PLAYERCLIP, cm_testModel.GetInteger(),
                vec3_origin, modelAxis);
  }
  timer.Stop();
  ...
  if (cm_testRandomMany.GetBool()) {
    ...
    for (i = 0; i < 3; i++) {
      testend[0][i] =
          start[i] + random.CRandomFloat() * cm_testRadius.GetFloat();
    }
    for (k = 1; k < cm_testTimes.GetInteger(); k++) {
      testend[k] = testend[0];
    }
  ...
  if (cm_testRotation.GetBool()) {
    ...
    idVec3 vec(random.CRandomFloat(), random.CRandomFloat(),
               random.RandomFloat());
    vec.Normalize();
    ...
    for (i = 0; i < cm_testTimes.GetInteger(); i++) {
      ...
      Rotation(&trace, start, rotation, &itm, boxAxis,
               CONTENTS_SOLID | CONTENTS_PLAYERCLIP, cm_testModel.GetInteger(),
               vec3_origin, modelAxis);
    }
    timer.Stop();

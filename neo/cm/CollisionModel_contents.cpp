...
bool idCollisionModelManagerLocal::TestTrmInPolygon(cm_traceWork_t* tw,
  ...
  for (i = 1; i <= tw->numEdges; i++) {
    ...
    for (j = 0; j < p->numEdges; j++) {
      ...
      edge = tw->model->edges + abs(edgeNum);
#if 1
      CM_SetTrmEdgeSidedness(edge, tw->edges[i].pl,
                             tw->polygonEdgePlueckerCache[j], i);
  ...
  for (i = 0; i < p->numEdges; i++) {
    ...
    for (j = 0; j < tw->numPolys; j++) {
      ...
      for (k = 0; k < tw->polys[j].numEdges; k++) {
        ...
        CM_SetTrmEdgeSidedness(edge, trmEdge->pl,
                               tw->polygonEdgePlueckerCache[i], bitNum);
...
int idCollisionModelManagerLocal::PointContents(const idVec3 p,
  ...
  node = idCollisionModelManagerLocal::PointNode(
      p, idCollisionModelManagerLocal::models[model]);
...
==================
*/
int idCollisionModelManagerLocal::TransformedPointContents(
    const idVec3& p,
    cmHandle_t model,
    const idVec3& origin,
...
int idCollisionModelManagerLocal::ContentsTrm(trace_t* results,
  ...
  if (!trm || (trm->bounds[1][0] - trm->bounds[0][0] <= 0.0f &&
    ...
    results->c.contents =
        idCollisionModelManagerLocal::TransformedPointContents(
            start, model, modelOrigin, modelAxis);
  ...
  for (i = 0; i < tw.numPolys; i++) {
    tw.polys[i].plane.FitThroughPoint(
        tw.edges[abs(tw.polys[i].edges[0])].start);
  }
...
int idCollisionModelManagerLocal::Contents(const idVec3& start,
  ...
  if (model < 0 || model > idCollisionModelManagerLocal::maxModels ||
      model > MAX_SUBMODELS) {
    common->Printf(
        "idCollisionModelManagerLocal::Contents: invalid model handle\n");
    return 0;
  }
  if (!idCollisionModelManagerLocal::models ||
      !idCollisionModelManagerLocal::models[model]) {
    common->Printf("idCollisionModelManagerLocal::Contents: invalid model\n");
    return 0;
  }

  return ContentsTrm(&results, start, trm, trmAxis, contentMask, model,

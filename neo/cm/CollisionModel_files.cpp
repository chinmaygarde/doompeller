...
void idCollisionModelManagerLocal::WritePolygons(idFile* fp, cm_node_t* node) {
  ...
  for (pref = node->polygons; pref; pref = pref->next) {
    ...
    fp->WriteFloatString(" ) ( %f %f %f ) %f", p->plane.Normal()[0],
                         p->plane.Normal()[1], p->plane.Normal()[2],
                         p->plane.Dist());
    fp->WriteFloatString(" ( %f %f %f )", p->bounds[0][0], p->bounds[0][1],
                         p->bounds[0][2]);
    fp->WriteFloatString(" ( %f %f %f )", p->bounds[1][0], p->bounds[1][1],
                         p->bounds[1][2]);
  ...
  for (bref = node->brushes; bref; bref = bref->next) {
    ...
    for (i = 0; i < b->numPlanes; i++) {
      fp->WriteFloatString("\t\t( %f %f %f ) %f\n", b->planes[i].Normal()[0],
                           b->planes[i].Normal()[1], b->planes[i].Normal()[2],
                           b->planes[i].Dist());
    }
    fp->WriteFloatString("\t} ( %f %f %f )", b->bounds[0][0], b->bounds[0][1],
                         b->bounds[0][2]);
    fp->WriteFloatString(" ( %f %f %f ) \"%s\"\n", b->bounds[1][0],
                         b->bounds[1][1], b->bounds[1][2],
...
void idCollisionModelManagerLocal::WriteCollisionModel(idFile* fp,
  ...
  for (i = 0; i < model->numVertices; i++) {
    fp->WriteFloatString("\t/* %d */ ( %f %f %f )\n", i,
                         model->vertices[i].p[0], model->vertices[i].p[1],
  ...
  for (i = 0; i < model->numEdges; i++) {
    fp->WriteFloatString("\t/* %d */ ( %d %d ) %d %d\n", i,
                         ...
                         model->edges[i].vertexNum[1], model->edges[i].internal,
...
================
*/
void idCollisionModelManagerLocal::WriteCollisionModelsToFile(
    const char* filename,
    int firstModel,
    int lastModel,
  ...
  if (!fp) {
    common->Warning(
        "idCollisionModelManagerLocal::WriteCollisionModelsToFile: Error "
        "opening file %s\n",
        name.c_str());
    return;
  }
...
================
*/
bool idCollisionModelManagerLocal::WriteCollisionModelForMapEntity(
    const idMapEntity* mapEnt,
    const char* filename,
    const bool testTraceModel) {
  idFile* fp;
  idStr name;
  cm_model_t* model;
  ...
  if (!fp) {
    common->Printf(
        "idCollisionModelManagerLocal::WriteCollisionModelForMapEntity: Error "
        "opening file %s\n",
        name.c_str());
    FreeModel(model);
    return false;
...
void idCollisionModelManagerLocal::ParseVertices(idLexer* src,
  ...
  model->vertices =
      (cm_vertex_t*)Mem_Alloc(model->maxVertices * sizeof(cm_vertex_t));
  for (i = 0; i < model->numVertices; i++) {
...
cm_node_t* idCollisionModelManagerLocal::ParseNodes(idLexer* src,
  ...
  model->numNodes++;
  node = AllocNode(model, model->numNodes < NODE_BLOCK_SIZE_SMALL
                              ? NODE_BLOCK_SIZE_SMALL
...
void idCollisionModelManagerLocal::ParsePolygons(idLexer* src,
  ...
  if (src->CheckTokenType(TT_NUMBER, 0, &token)) {
    model->polygonBlock = (cm_polygonBlock_t*)Mem_Alloc(
        sizeof(cm_polygonBlock_t) + token.GetIntValue());
    model->polygonBlock->bytesRemaining = token.GetIntValue();
    model->polygonBlock->next =
        ((byte*)model->polygonBlock) + sizeof(cm_polygonBlock_t);
  }
...
void idCollisionModelManagerLocal::ParseBrushes(idLexer* src,
  ...
  if (src->CheckTokenType(TT_NUMBER, 0, &token)) {
    model->brushBlock = (cm_brushBlock_t*)Mem_Alloc(sizeof(cm_brushBlock_t) +
                                                    token.GetIntValue());
    model->brushBlock->bytesRemaining = token.GetIntValue();
    model->brushBlock->next =
        ((byte*)model->brushBlock) + sizeof(cm_brushBlock_t);
  }
...
================
*/
bool idCollisionModelManagerLocal::LoadCollisionModelFile(
    const char* name,
    unsigned int mapFileCRC) {
  idStr fileName;
  idToken token;
  ...
  if (!src->ReadToken(&token) || token != CM_FILEVERSION) {
    common->Warning("%s has version %s instead of %s", fileName.c_str(),
                    token.c_str(), CM_FILEVERSION);
    delete src;
    return false;
  }
  ...
  while (1) {
    ...
    src->Error(
        "idCollisionModelManagerLocal::LoadCollisionModelFile: bad token "
        "\"%s\"",
        token.c_str());
  }

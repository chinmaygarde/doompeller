...
void idCollisionModelManagerLocal::ParseProcNodes(idLexer* src) {
  ...
  procNodes =
      (cm_procNode_t*)Mem_ClearedAlloc(numProcNodes * sizeof(cm_procNode_t));
...
void idCollisionModelManagerLocal::LoadProcBSP(const char* name) {
  ...
  if (!src->ReadToken(&token) || token.Icmp(PROC_FILE_ID)) {
    common->Warning(
        "idCollisionModelManagerLocal::LoadProcBSP: bad id '%s' instead of "
        "'%s'",
  ...
  while (1) {
    ...
    src->Error("idCollisionModelManagerLocal::LoadProcBSP: bad token \"%s\"",
               token.c_str());
  }
...
void idCollisionModelManagerLocal::FreePolygon(cm_model_t* model,
  ...
  model->numPolygons--;
  model->polygonMemory -=
      sizeof(cm_polygon_t) + (poly->numEdges - 1) * sizeof(poly->edges[0]);
...
void idCollisionModelManagerLocal::FreeBrush(cm_model_t* model,
  ...
  model->numBrushes--;
  model->brushMemory -=
      sizeof(cm_brush_t) + (brush->numPlanes - 1) * sizeof(brush->planes[0]);
...
void idCollisionModelManagerLocal::FreeModel(cm_model_t* model) {
  ...
  for (polygonRefBlock = model->polygonRefBlocks; polygonRefBlock;
  ...
  for (brushRefBlock = model->brushRefBlocks; brushRefBlock;
  ...
  while (1) {
    for (pref = node->polygons; pref; pref = pref->next) {
      ...
      for (i = 0; i < p->numEdges; i++) {
        ...
        if (edge->normal[0] == 0.0f && edge->normal[1] == 0.0f &&
            edge->normal[2] == 0.0f) {
          // if the edge is only used by this polygon
          if (edge->numUsers == 1) {
            dir = model->vertices[edge->vertexNum[edgeNum < 0]].p -
          ...
          if (dot < SHARP_EDGE_DOT) {
            ...
            dir = model->vertices[edge->vertexNum[edgeNum > 0]].p -
                  model->vertices[edge->vertexNum[edgeNum < 0]].p;
            edge->normal =
                edge->normal.Cross(dir) + p->plane.Normal().Cross(-dir);
            edge->normal *=
                (0.5f / (0.5f + 0.5f * SHARP_EDGE_DOT)) / edge->normal.Length();
            model->numSharpEdges++;
          } else {
...
cm_model_t* idCollisionModelManagerLocal::AllocModel(void) {
  ...
  model->numPolygons = model->polygonMemory = model->numBrushes =
      model->brushMemory = model->numNodes = model->numBrushRefs =
...
cm_node_t* idCollisionModelManagerLocal::AllocNode(cm_model_t* model,
  ...
  if (!model->nodeBlocks || !model->nodeBlocks->nextNode) {
    nodeBlock = (cm_nodeBlock_t*)Mem_ClearedAlloc(
        sizeof(cm_nodeBlock_t) + blockSize * sizeof(cm_node_t));
    nodeBlock->nextNode =
        (cm_node_t*)(((byte*)nodeBlock) + sizeof(cm_nodeBlock_t));
...
================
*/
cm_polygonRef_t* idCollisionModelManagerLocal::AllocPolygonReference(
    cm_model_t* model,
    int blockSize) {
  int i;
  cm_polygonRef_t* pref;
  ...
  if (!model->polygonRefBlocks || !model->polygonRefBlocks->nextRef) {
    prefBlock = (cm_polygonRefBlock_t*)Mem_Alloc(
        sizeof(cm_polygonRefBlock_t) + blockSize * sizeof(cm_polygonRef_t));
    prefBlock->nextRef =
        (cm_polygonRef_t*)(((byte*)prefBlock) + sizeof(cm_polygonRefBlock_t));
...
================
*/
cm_brushRef_t* idCollisionModelManagerLocal::AllocBrushReference(
    cm_model_t* model,
    int blockSize) {
  int i;
  cm_brushRef_t* bref;
  ...
  if (!model->brushRefBlocks || !model->brushRefBlocks->nextRef) {
    brefBlock = (cm_brushRefBlock_t*)Mem_Alloc(
        sizeof(cm_brushRefBlock_t) + blockSize * sizeof(cm_brushRef_t));
    brefBlock->nextRef =
        (cm_brushRef_t*)(((byte*)brefBlock) + sizeof(cm_brushRefBlock_t));
...
void idCollisionModelManagerLocal::AddPolygonToNode(cm_model_t* model,
  ...
  pref = AllocPolygonReference(
      model, model->numPolygonRefs < REFERENCE_BLOCK_SIZE_SMALL
                 ? REFERENCE_BLOCK_SIZE_SMALL
                 : REFERENCE_BLOCK_SIZE_LARGE);
  pref->p = p;
...
void idCollisionModelManagerLocal::AddBrushToNode(cm_model_t* model,
  ...
  bref = AllocBrushReference(model,
                             model->numBrushRefs < REFERENCE_BLOCK_SIZE_SMALL
                                 ? REFERENCE_BLOCK_SIZE_SMALL
                                 : REFERENCE_BLOCK_SIZE_LARGE);
  bref->b = b;
...
void idCollisionModelManagerLocal::SetupTrmModelStructure(void) {
  ...
  model->vertices =
      (cm_vertex_t*)Mem_ClearedAlloc(model->maxVertices * sizeof(cm_vertex_t));
  model->numEdges = 0;
  model->maxEdges = MAX_TRACEMODEL_EDGES + 1;
  model->edges =
      (cm_edge_t*)Mem_ClearedAlloc(model->maxEdges * sizeof(cm_edge_t));
...
================
*/
cmHandle_t idCollisionModelManagerLocal::SetupTrmModel(
  ...
  do {
    ...
    if (res == SIDE_FRONT) {
      ...
      if (node->children[1] > 0) {
        if (!R_ChoppedAwayByProcBSP(node->children[1], &back, normal, origin,
                                    radius)) {
          return false;
        }
      }
...
=============
*/
void idCollisionModelManagerLocal::ChopWindingListWithBrush(
  ...
  for (k = 0; k < list->numWindings; k++) {
    ...
    do {
      ...
      for (planeNum = startPlane, i = 0; i < b->numPlanes; i++, planeNum++) {
        ...
        //a decal brush side 			with exactly the same size as another brush side ?
        ...
        if (res == SIDE_ON && list->primitiveNum >= 0 &&
            (list->normal * b->planes[planeNum].Normal()) > 0) {
        ...
        if (res == SIDE_BACK) {
          if (cm_outList->numWindings >= MAX_WINDING_LIST) {
            common->Warning(
                "idCollisionModelManagerLocal::ChopWindingWithBrush: primitive "
...
============
*/
void idCollisionModelManagerLocal::R_ChopWindingListWithTreeBrushes(
    cm_windingList_t* list,
    cm_node_t* node) {
  int i;
  cm_brushRef_t* bref;
...
============
*/
idFixedWinding* idCollisionModelManagerLocal::WindingOutsideBrushes(
    idFixedWinding* w,
    const idPlane& plane,
    int contents,
    int primitiveNum,
  ...
  cm_windingList->origin =
      (cm_windingList->bounds[1] - cm_windingList->bounds[0]) * 0.5;
...
void idCollisionModelManagerLocal::ReplacePolygons(cm_model_t* model,
  ...
  while (1) {
    ...
    if (p1->bounds[0][node->planeType] > node->planeDist &&
        p2->bounds[0][node->planeType] > node->planeDist) {
      node = node->children[0];
    } else if (p1->bounds[1][node->planeType] < node->planeDist &&
               p2->bounds[1][node->planeType] < node->planeDist) {
      node = node->children[1];
    } else {
...
cm_polygon_t* idCollisionModelManagerLocal::TryMergePolygons(cm_model_t* model,
  ...
  for (i = 0; i < 3; i++) {
    if (idMath::Fabs(p1->plane.Normal()[i] - p2->plane.Normal()[i]) >
        NORMAL_EPSILON) {
      return NULL;
    }
  ...
  if (p1BeforeShare < 0 || p1AfterShare < 0 || p2BeforeShare < 0 ||
      p2AfterShare < 0) {
    return NULL;
  }
  ...
  if (!keep1) {
    ...
    GetEdge(model,
            model
                ->vertices[model->edges[abs(edgeNum1)]
                               .vertexNum[INTSIGNBITSET(edgeNum1)]]
                .p,
            model
                ->vertices[model->edges[abs(edgeNum2)]
                               .vertexNum[INTSIGNBITNOTSET(edgeNum2)]]
                .p,
            &newEdgeNum1, -1);
    if (newEdgeNum1 == 0) {
      keep1 = true;
...
=============
*/
bool idCollisionModelManagerLocal::MergePolygonWithTreePolygons(
    cm_model_t* model,
    cm_node_t* node,
...
=============
*/
void idCollisionModelManagerLocal::FindInternalEdgesOnPolygon(
    cm_model_t* model,
    cm_polygon_t* p1,
    cm_polygon_t* p2) {
  int i, j, k, edgeNum;
...
=============
*/
void idCollisionModelManagerLocal::FindInternalPolygonEdges(
    cm_model_t* model,
    cm_node_t* node,
    cm_polygon_t* polygon) {
  cm_polygonRef_t* pref;
  ...
  if (polygon->material->GetCullType() == CT_TWO_SIDED ||
      polygon->material->ShouldCreateBackSides()) {
    return;
  }
...
================
*/
static int CM_FindSplitter(const cm_node_t* node,
...
================
*/
void idCollisionModelManagerLocal::R_FilterPolygonIntoTree(
    cm_model_t* model,
    cm_node_t* node,
    cm_polygonRef_t* pref,
    cm_polygon_t* p) {
...
================
*/
cm_node_t* idCollisionModelManagerLocal::R_CreateAxialBSPTree(
    cm_model_t* model,
    cm_node_t* node,
    const idBounds& bounds) {
  int planeType;
...
int idCollisionModelManagerLocal::GetVertex(cm_model_t* model,
  ...
  for (vn = cm_vertexHash->First(hashKey); vn >= 0;
  ...
  if (model->numVertices >= model->maxVertices) {
    ...
    model->vertices = (cm_vertex_t*)Mem_ClearedAlloc(model->maxVertices *
                                                     sizeof(cm_vertex_t));
    memcpy(model->vertices, oldVertices,
...
void idCollisionModelManagerLocal::CreatePolygon(cm_model_t* model,
  ...
  for (i = 0, j = 1; i < w->GetNumPoints(); i++, j++) {
    ...
    GetEdge(model, (*w)[i].ToVec3(), (*w)[j].ToVec3(), &polyEdges[numPolyEdges],
            v1num);
    if (polyEdges[numPolyEdges]) {
      // last vertex of this edge is the first vertex of the next edge
      v1num = model->edges[abs(polyEdges[numPolyEdges])]
                  .vertexNum[INTSIGNBITNOTSET(polyEdges[numPolyEdges])];
  ...
  if (numPolyEdges > CM_MAX_POLYGON_EDGES) {
    common->Warning(
        "idCollisionModelManagerLocal::CreatePolygon: polygon has more than %d "
        "edges",
        numPolyEdges);
...
================
*/
void idCollisionModelManagerLocal::PolygonFromWinding(
    cm_model_t* model,
    idFixedWinding* w,
    const idPlane& plane,
    const idMaterial* material,
  ...
  if (w->IsHuge()) {
    common->Warning(
        "idCollisionModelManagerLocal::PolygonFromWinding: model %s primitive "
        "%d is degenerate",
  ...
  if (material->GetCullType() == CT_TWO_SIDED ||
      material->ShouldCreateBackSides()) {
    w->ReverseSelf();
    CreatePolygon(model, w, -plane, material, primitiveNum);
  }
}
...
=================
*/
void idCollisionModelManagerLocal::CreatePatchPolygons(
    cm_model_t* model,
...
=================
*/
static void CM_EstimateVertsAndEdges(const idMapEntity* mapEnt,
                                     int* numVerts,
                                     int* numEdges) {
  int j, width, height;
  ...
  *numVerts = *numEdges = 0;
  for (j = 0; j < mapEnt->GetNumPrimitives(); j++) {
    ...
    if (mapPrim->GetType() == idMapPrimitive::TYPE_PATCH) {
      ...
      *numEdges += (width - 1) * height + width * (height - 1) +
                   (width - 1) * (height - 1);
      continue;
    }
    if (mapPrim->GetType() == idMapPrimitive::TYPE_BRUSH) {
      ...
      // the bottom
      *numVerts +=
          (static_cast<const idMapBrush*>(mapPrim)->GetNumSides() - 2) * 2;
      *numEdges +=
          (static_cast<const idMapBrush*>(mapPrim)->GetNumSides() - 2) * 3;
      continue;
    }
  }
}
...
void idCollisionModelManagerLocal::ConvertPatch(cm_model_t* model,
  ...
  if (patch->GetExplicitlySubdivided()) {
    cp->SubdivideExplicit(patch->GetHorzSubdivisions(),
                          patch->GetVertSubdivisions(), false, true);
  } else {
    cp->Subdivide(DEFAULT_CURVE_MAX_ERROR_CD, DEFAULT_CURVE_MAX_ERROR_CD,
...
void idCollisionModelManagerLocal::OptimizeArrays(cm_model_t* model) {
  ...
  remap = (int*)Mem_ClearedAlloc(Max(model->numVertices, model->numEdges) *
                                 sizeof(int));
  ...
  oldVertices = model->vertices;
  if (oldVertices) {
    model->vertices = (cm_vertex_t*)Mem_ClearedAlloc(model->numVertices *
                                                     sizeof(cm_vertex_t));
    memcpy(model->vertices, oldVertices,
           model->numVertices * sizeof(cm_vertex_t));
    Mem_Free(oldVertices);
  ...
  if (oldEdges) {
    model->edges =
        (cm_edge_t*)Mem_ClearedAlloc(model->numEdges * sizeof(cm_edge_t));
    memcpy(model->edges, oldEdges, model->numEdges * sizeof(cm_edge_t));
    Mem_Free(oldEdges);
  }
}
...
void idCollisionModelManagerLocal::FinishModel(cm_model_t* model) {
  ...
  model->usedMemory = model->numVertices * sizeof(cm_vertex_t) +
                      ...
                      model->polygonMemory + model->brushMemory +
...
================
*/
cm_model_t* idCollisionModelManagerLocal::LoadRenderModel(
    const char* fileName) {
  int i, j;
  ...
  for (i = 0; i < renderModel->NumSurfaces(); i++) {
    ...
    // surface
    if (collisionSurface &&
        !(surf->shader->GetSurfaceFlags() & SURF_COLLISION)) {
      continue;
    }
...
================
*/
cm_model_t* idCollisionModelManagerLocal::CollisionModelForMapEntity(
  ...
  if (!LoadCollisionModelFile(mapFile->GetName(), mapFile->GetGeometryCRC())) {
    ...
    for (i = 0; i < mapFile->GetNumEntities(); i++) {
      ...
      if (numModels >= MAX_SUBMODELS) {
        common->Error(
            "idCollisionModelManagerLocal::BuildModels: more than %d collision "
            "models",
            MAX_SUBMODELS);
        break;
      }
    ...
    WriteCollisionModelsToFile(mapFile->GetName(), 0, numModels,
...
void idCollisionModelManagerLocal::LoadMap(const idMapFile* mapFile) {
  ...
  numModels = 0;
  models =
      (cm_model_t**)Mem_ClearedAlloc((maxModels + 1) * sizeof(cm_model_t*));
...
const char* idCollisionModelManagerLocal::GetModelName(cmHandle_t model) const {
  if (model < 0 || model > MAX_SUBMODELS || model >= numModels ||
      !models[model]) {
    common->Printf(
...
bool idCollisionModelManagerLocal::GetModelContents(cmHandle_t model,
  ...
  if (model < 0 || model > MAX_SUBMODELS || model >= numModels ||
      !models[model]) {
    common->Printf(
        "idCollisionModelManagerLocal::GetModelContents: invalid model "
        "handle\n");
    return false;
  }
...
bool idCollisionModelManagerLocal::GetModelVertex(cmHandle_t model,
  ...
  if (vertexNum < 0 || vertexNum >= models[model]->numVertices) {
    common->Printf(
        "idCollisionModelManagerLocal::GetModelVertex: invalid vertex "
        "number\n");
    return false;
  }
...
===================
*/
bool idCollisionModelManagerLocal::GetModelPolygon(
    cmHandle_t model,
    int polygonNum,
  ...
  if (model < 0 || model > MAX_SUBMODELS || model >= numModels ||
      !models[model]) {
    common->Printf(
        "idCollisionModelManagerLocal::GetModelPolygon: invalid model "
        "handle\n");
    return false;
  }
  ...
  for (i = 0; i < poly->numEdges; i++) {
    ...
    winding += models[model]
                   ->vertices[models[model]
                                  ->edges[abs(edgeNum)]
                                  .vertexNum[INTSIGNBITSET(edgeNum)]]
                   .p;
  }
...
cmHandle_t idCollisionModelManagerLocal::LoadModel(const char* modelName,
  ...
  if (LoadCollisionModelFile(modelName, 0)) {
    ...
    if (handle >= 0) {
      ...
      common->Warning(
          "idCollisionModelManagerLocal::LoadModel: collision file for '%s' "
...
bool idCollisionModelManagerLocal::TrmFromModel(const cm_model_t* model,
  ...
  if (model->numVertices > MAX_TRACEMODEL_VERTS) {
    common->Printf(
        "idCollisionModelManagerLocal::TrmFromModel: model %s has too many "
        "vertices.\n",
        model->name.c_str());
    PrintModelInfo(model);
    return false;
  ...
  for (i = 1; i <= trm.numEdges; i++) {
    if (numEdgeUsers[i] != 2) {
      common->Printf(
          "idCollisionModelManagerLocal::TrmFromModel: model %s has dangling "
          "edges, the model has to be an enclosed hull.\n",
          model->name.c_str());
      PrintModelInfo(model);
      return false;
    }

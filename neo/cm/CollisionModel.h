...
class idCollisionModelManager {
 public:
  ...
  virtual cmHandle_t SetupTrmModel(const idTraceModel& trm,
  ...
  virtual bool GetModelVertex(cmHandle_t model,
                              int vertexNum,
                              idVec3& vertex) const = 0;
  // Gets an edge of a model.
  virtual bool GetModelEdge(cmHandle_t model,
                            int edgeNum,
                            idVec3& start,
                            idVec3& end) const = 0;
  // Gets a polygon of a model.
  virtual bool GetModelPolygon(cmHandle_t model,
                               int polygonNum,
  ...
  // Draws a model.
  virtual void DrawModel(cmHandle_t model,
                         const idVec3& modelOrigin,
  ...
  virtual bool WriteCollisionModelForMapEntity(
      const idMapEntity* mapEnt,
      const char* filename,
      const bool testTraceModel = true) = 0;
};

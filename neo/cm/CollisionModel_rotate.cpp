...
// if the collision point is this close to the rotation axis it is not
...
CM_RotatePoint
  ...
  rotates a point about an arbitrary axis using the tangent of half the rotation
angle
================
*/
void CM_RotatePoint(idVec3& point,
                    const idVec3& origin,
                    const idVec3& axis,
...
CM_RotateEdge
  ...
  rotates an edge about an arbitrary axis using the tangent of half the rotation
angle
================
*/
...
idCollisionModelManagerLocal::CollisionBetweenEdgeBounds
  ...
  also calculates the collision point and collision plane normal if the
collision occurs between the bounds
================
*/
...
int idCollisionModelManagerLocal::RotateEdgeThroughEdge(cm_traceWork_t* tw,
  ...
  v[0] = (a[0] * cos(t) + a[1] * sin(t)) * (b[0] * -sin(t) + b[1] * cos(t)) -
  (b[0] * cos(t) + b[1] * sin(t)) * (a[0] * -sin(t) + a[1] * cos(t)); v[1] =
  (a[0] * cos(t) + a[1] * sin(t)) * b[2] - (b[0] * cos(t) + b[1] * sin(t)) *
  a[2]; v[2] = (a[0] * cos(t) + a[1] * sin(t)) - (b[0] * cos(t) + b[1] *
  sin(t)); v[3] = (a[0] * -sin(t) + a[1] * cos(t)) * b[2] - (b[0] * -sin(t) +
  b[1] * cos(t)) * a[2]; v[4] = a[2] - b[2]; v[5] = (b[0] * -sin(t) + b[1] *
  cos(t)) - (a[0] * -sin(t) + a[1] * cos(t));

  pl2[0] * v[4] + pl2[1] * v[5] + pl2[2] * v[3] + pl2[4] * v[0] + pl2[5] * v[1]
  + pl2[3] * v[2] = 0;

  v[0] = (a[0] * cos(t) + a[1] * sin(t)) * (b[0] * -sin(t) + b[1] * cos(t)) -
  (b[0] * cos(t) + b[1] * sin(t)) * (a[0] * -sin(t) + a[1] * cos(t)); v[0] =
  (a[1] * b[1] - a[0] * b[0]) * cos(t) * sin(t) + (a[0] * b[1] + a[1] * b[0] *
  cos(t)^2) - (a[1] * b[0]) - ((b[1] * a[1] - b[0] * a[0]) * cos(t) * sin(t) +
  (b[0] * a[1] + b[1] * a[0]) * cos(t)^2 - (b[1] * a[0])) v[0] = - (a[1] * b[0])
  - ( - (b[1] * a[0])) v[0] = (b[1] * a[0]) - (a[1] * b[0])
...
idCollisionModelManagerLocal::EdgeFurthestFromEdge
  ...
  calculates the direction of motion at the initial position, where dir < 0
means the edges move towards each other if the edges move away from each other
the tangent of half the rotation angle at which the edges are furthest apart is
also calculated
================
*/
int idCollisionModelManagerLocal::EdgeFurthestFromEdge(cm_traceWork_t* tw,
  ...
  */
  ...
  // negative direction means the edges move towards each other at the initial
  // position
  if (dir <= 0.0f) {
    return true;
  }
...
================
*/
void idCollisionModelManagerLocal::RotateTrmEdgeThroughPolygon(
    cm_traceWork_t* tw,
    cm_polygon_t* poly,
  ...
  if (tw->isConvex && tw->axisIntersectsTrm) {
    // if both points are behind the polygon the edge cannot collide within a
    // 180 degrees rotation
    if (tw->vertices[trmEdge->vertexNum[0]].polygonSide &
        tw->vertices[trmEdge->vertexNum[1]].polygonSide) {
      return;
    }
  }
  ...
  for (i = 0; i < poly->numEdges; i++) {
    ...
    // if the trace model edge rotation bounds do not intersect the polygon edge
    // bounds
    ...
    // if the rotating edge is inbetween the polygon edge and the epsilon
    // expanded edge
    if ((f1 < 0.0f && f2 > 0.0f) || (f1 > 0.0f && f2 < 0.0f)) {
      if (!EdgeFurthestFromEdge(tw, trmEdge->plzaxis, v1->p, v2->p, startTan,
                                dir)) {
        continue;
      }
      ...
      if (dir <= 0.0f) {
        ...
        // never going to get beyond the start tangent during the current
        // rotation
        continue;
      } else {
        // collide with the epsilon expanded edge
        if (!RotateEdgeThroughEdge(tw, trmEdge->plzaxis, v1->p + epsDir,
                                   v2->p + epsDir, idMath::Fabs(startTan),
                                   tanHalfAngle)) {
    ...
    if (!CollisionBetweenEdgeBounds(tw, trmEdge->start, trmEdge->end, v1->p,
                                    v2->p, tanHalfAngle, collisionPoint,
                                    collisionNormal)) {
      continue;
    }
    ...
    if ((collisionPoint - origin).LengthSqr() <
        ROTATION_AXIS_EPSILON * ROTATION_AXIS_EPSILON) {
      continue;
    }
...
idCollisionModelManagerLocal::RotatePointThroughPlane
  ...
  calculates the tangent of half the rotation angle at which the point collides
with the plane
================
*/
int idCollisionModelManagerLocal::RotatePointThroughPlane(
...
idCollisionModelManagerLocal::PointFurthestFromPlane
  ...
  calculates the direction of motion at the initial position, where dir < 0
means the point moves towards the plane if the point moves away from the plane
the tangent of half the rotation angle at which the point is furthest away from
the plane is also calculated
================
*/
int idCollisionModelManagerLocal::PointFurthestFromPlane(
    const cm_traceWork_t* tw,
    const idVec3& point,
    const idPlane& plane,
    const float angle,
  ...
  */
  ...
  // the point will always start at the front of the plane, therefore v0 + v2 >
  // 0 is always true
  if (angle < 0.0f) {
    dir = -v1;
  } else {
  ...
  // negative direction means the point moves towards the plane at the initial
  // position
  if (dir <= 0.0f) {
    return true;
  }
...
int idCollisionModelManagerLocal::RotatePointThroughEpsilonPlane(
  ...
  // if the rotation sphere at the rotation origin is too far away from the
  // polygon plane
  ...
  if (d <= CM_PL_RANGE_EPSILON) {
    // calculate tangent of half the rotation for which the vertex is furthest
    // away from the plane
    ...
    if (dir <= 0.0f) {
      ...
      if (!RotatePointThroughPlane(tw, point, epsPlane, angle,
                                   idMath::Fabs(startTan), tanHalfAngle)) {
        tanHalfAngle = startTan;
      }
    }
  } else {
    // calculate collision with epsilon expanded plane
    if (!RotatePointThroughPlane(tw, point, epsPlane, angle, 0.0f,
                                 tanHalfAngle)) {
      return false;
    }
  }
...
================
*/
void idCollisionModelManagerLocal::RotateTrmVertexThroughPolygon(
    cm_traceWork_t* tw,
    cm_polygon_t* poly,
    cm_trmVertex_t* v,
    int vertexNum) {
  ...
  // if the trm vertex is behind the polygon plane it cannot collide with the
  // polygon within a 180 degrees rotation
  if (tw->isConvex && tw->axisIntersectsTrm && v->polygonSide) {
    return;
  }

  // if the trace model vertex rotation bounds do not intersect the polygon
  // bounds
  ...
  if (!RotatePointThroughEpsilonPlane(tw, v->p, v->endp, poly->plane, tw->angle,
                                      v->rotationOrigin, tanHalfAngle,
                                      collisionPoint, endDir)) {
    return;
  }

  if (idMath::Fabs(tanHalfAngle) < tw->maxTan) {
    // verify if 'collisionPoint' moving along 'endDir' moves between polygon
    // edges
...
================
*/
void idCollisionModelManagerLocal::RotateVertexThroughTrmPolygon(
    cm_traceWork_t* tw,
  ...
  // if the polygon vertex is behind the trm plane it cannot collide with the
  // trm polygon within a 180 degrees rotation
  if (tw->isConvex && tw->axisIntersectsTrm &&
      trmpoly->plane.Distance(v->p) < 0.0f) {
    return;
  }
  ...
  // rotate the vertex through the epsilon plane
  if (!RotatePointThroughEpsilonPlane(tw, v->p, endp, trmpoly->plane,
                                      -tw->angle, rotationOrigin, tanHalfAngle,
                                      collisionPoint, endDir)) {
    return;
  }
...
bool idCollisionModelManagerLocal::RotateTrmThroughPolygon(cm_traceWork_t* tw,
  ...
  if (tw->isConvex) {
    ...
    if (p->plane.Distance(tw->start) < 0.0f) {
      ...
      if (tw->axisIntersectsTrm) {
        ...
        // or if the intersections of the rotation axis with the expanded heart
        // planes are both in front of the polygon plane
      }
    }
  }
  ...
  for (i = 0; i < p->numEdges; i++) {
    ...
    // calculate rotation origin projected into rotation plane through the
    // vertex
    tw->polygonRotationOriginCache[i] =
        tw->origin + tw->axis * (tw->axis * (v->p - tw->origin));
  }
  // copy first to last so we can easily cycle through
  tw->polygonRotationOriginCache[p->numEdges] =
      tw->polygonRotationOriginCache[0];
...
void idCollisionModelManagerLocal::BoundsForRotation(const idVec3& origin,
  ...
  for (i = 0; i < 3; i++) {
    // if the derivative changes sign along this axis during the rotation from
    // start to end
    if ((v1[i] > 0.0f && v2[i] < 0.0f) || (v1[i] < 0.0f && v2[i] > 0.0f)) {
      if ((0.5f * (start[i] + end[i]) - origin[i]) > 0.0f) {
        bounds[0][i] = Min(start[i], end[i]);
        bounds[1][i] =
            origin[i] + idMath::Sqrt(radiusSqr * (1.0f - axis[i] * axis[i]));
      } else {
        bounds[0][i] =
            origin[i] - idMath::Sqrt(radiusSqr * (1.0f - axis[i] * axis[i]));
...
void idCollisionModelManagerLocal::Rotation180(trace_t* results,
  ...
  if (model < 0 || model > MAX_SUBMODELS ||
      model > idCollisionModelManagerLocal::maxModels) {
    common->Printf(
  ...
  tw.axis = axis;
  assert(tw.axis[0] * tw.axis[0] + tw.axis[1] * tw.axis[1] +
             tw.axis[2] * tw.axis[2] >
         0.99f);
  ...
  d = tw.radius * tw.radius -
      (CIRCLE_APPROXIMATION_LENGTH * CIRCLE_APPROXIMATION_LENGTH * 0.25f);
  if (d > 0.0f) {
  ...
  if (!trm || (trm->bounds[1][0] - trm->bounds[0][0] <= 0.0f &&
    ...
    BoundsForRotation(tw.vertices[0].rotationOrigin, tw.axis, tw.start, tw.end,
    ...
    // extents is set to maximum error of the circle approximation traced
    // through the axial BSP tree
    ...
    *results = tw.trace;
    ...
    if (tw.maxTan == initialTan) {
      ...
      results->fraction = idMath::Fabs(atan(tw.maxTan) *
                                       (2.0f * 180.0f / idMath::PI) / tw.angle);
    }
    assert(results->fraction <= 1.0f);
    endRotation.Set(rorg, axis,
                    startAngle + (endAngle - startAngle) * results->fraction);
  ...
  for (vert = tw.vertices, i = 0; i < tw.numVerts; i++, vert++) {
    ...
    // vertex
    vert->rotationOrigin =
        tw.origin + tw.axis * (tw.axis * (vert->p - tw.origin));
    // calculate rotation bounds for this vertex
    BoundsForRotation(vert->rotationOrigin, tw.axis, vert->p, vert->endp,
                      vert->rotationBounds);
  ...
  for (edge = tw.edges + 1, i = 1; i <= tw.numEdges; i++, edge++) {
    // if the rotation axis goes through both the edge vertices then the edge is
    // not used
    if (tw.vertices[edge->vertexNum[0]].used |
        tw.vertices[edge->vertexNum[1]].used) {
      edge->used = true;
    }
    ...
    edge->rotationBounds.AddBounds(
        tw.vertices[edge->vertexNum[1]].rotationBounds);
  ...
  // extents including the maximum error of the circle approximation traced
  // through the axial BSP tree
  for (i = 0; i < 3; i++) {
  ...
  *results = tw.trace;
  ...
  if (tw.maxTan == initialTan) {
    ...
    results->fraction =
        idMath::Fabs(atan(tw.maxTan) * (2.0f * 180.0f / idMath::PI) / tw.angle);
  }
  assert(results->fraction <= 1.0f);
  endRotation.Set(rorg, axis,
                  startAngle + (endAngle - startAngle) * results->fraction);
...
void idCollisionModelManagerLocal::Rotation(trace_t* results,
  ...
  assert(((byte*)&start) < ((byte*)results) ||
         ((byte*)&start) > (((byte*)results) + sizeof(trace_t)));
  assert(((byte*)&trmAxis) < ((byte*)results) ||
         ((byte*)&trmAxis) > (((byte*)results) + sizeof(trace_t)));

  memset(results, 0, sizeof(*results));

  // if special position test
  if (rotation.GetAngle() == 0.0f) {
    idCollisionModelManagerLocal::ContentsTrm(results, start, trm, trmAxis,
  ...
  if (cm_debugCollision.GetBool()) {
    if (!entered) {
      ...
      if (idCollisionModelManagerLocal::Contents(start, trm, trmAxis, -1, model,
  ...
  if (rotation.GetAngle() >= 180.0f || rotation.GetAngle() <= -180.0f) {
    ...
    for (lasta = 0.0f, a = stepa; fabs(a) < fabs(maxa) + 1.0f;
         lasta = a, a += stepa) {
      // partial rotation
      idCollisionModelManagerLocal::Rotation180(
          results, rotation.GetOrigin(), rotation.GetVec(), lasta, a, start,
          trm, trmAxis, contentMask, model, modelOrigin, modelAxis);
      // if there is a collision
      if (results->fraction < 1.0f) {
        // fraction of total rotation
        results->fraction =
            (lasta + stepa * results->fraction) / rotation.GetAngle();
        return;
      }
    }
    results->fraction = 1.0f;
  ...
  idCollisionModelManagerLocal::Rotation180(
      results, rotation.GetOrigin(), rotation.GetVec(), 0.0f,
      rotation.GetAngle(), start, trm, trmAxis, contentMask, model, modelOrigin,
      modelAxis);
  ...
  if (cm_debugCollision.GetBool()) {
    if (!entered) {
      ...
      if (idCollisionModelManagerLocal::Contents(results->endpos, trm,
                                                 results->endAxis, -1, model,
                                                 modelOrigin, modelAxis) &
          contentMask) {
        trace_t tr;

        // test where the trm is stuck in the model
        idCollisionModelManagerLocal::Contents(results->endpos, trm,
                                               results->endAxis, -1, model,
                                               modelOrigin, modelAxis);
        // re-run collision detection to find out where it failed
        idCollisionModelManagerLocal::Rotation(&tr, start, rotation, trm,
                                               trmAxis, contentMask, model,
                                               modelOrigin, modelAxis);
      }
      entered = 0;
    }

...
ID_INLINE void CM_SetEdgeSidedness(cm_edge_t* edge,
  ...
  if (!(edge->sideSet & (1 << bitNum))) {
    ...
    fl = vpl.PermutedInnerProduct(epl);
    edge->side =
        (edge->side & ~(1 << bitNum)) | (FLOATSIGNBITSET(fl) << bitNum);
...
================
*/
void idCollisionModelManagerLocal::TranslateTrmEdgeThroughPolygon(
    cm_traceWork_t* tw,
    cm_polygon_t* poly,
  ...
  for (i = 0; i < poly->numEdges; i++) {
    ...
    CM_SetEdgeSidedness(edge, *pl, tw->vertices[trmEdge->vertexNum[0]].pl,
                        trmEdge->vertexNum[0]);
    CM_SetEdgeSidedness(edge, *pl, tw->vertices[trmEdge->vertexNum[1]].pl,
                        trmEdge->vertexNum[1]);
    // if the trm edge start and end vertex do not pass the polygon edge at
    // different sides
    if (!(((edge->side >> trmEdge->vertexNum[0]) ^
           (edge->side >> trmEdge->vertexNum[1])) &
          1)) {
      continue;
    }
    ...
    CM_SetVertexSidedness(v1, tw->polygonVertexPlueckerCache[i], trmEdge->pl,
                          trmEdge->bitNum);
    v2 = tw->model->vertices + edge->vertexNum[INTSIGNBITNOTSET(edgeNum)];
    CM_SetVertexSidedness(v2, tw->polygonVertexPlueckerCache[i + 1],
                          trmEdge->pl, trmEdge->bitNum);
    ...
    // edge
    if (!idCollisionModelManagerLocal::TranslateEdgeThroughEdge(
            trmEdge->cross, trmEdge->pl, *pl, &f1)) {
      continue;
    }
    ...
    epsPl.FromLine(tw->model->vertices[edge->vertexNum[0]].p +
                       edge->normal * CM_CLIP_EPSILON,
                   tw->model->vertices[edge->vertexNum[1]].p +
                       edge->normal * CM_CLIP_EPSILON);
    // calculate collision fraction with epsilon expanded edge
    if (!idCollisionModelManagerLocal::TranslateEdgeThroughEdge(
            trmEdge->cross, trmEdge->pl, epsPl, &f2)) {
      continue;
    }
...
================
*/
void idCollisionModelManagerLocal::TranslateTrmVertexThroughPolygon(
    cm_traceWork_t* tw,
    cm_polygon_t* poly,
    cm_trmVertex_t* v,
    int bitNum) {
...
================
*/
void idCollisionModelManagerLocal::TranslatePointThroughPolygon(
    cm_traceWork_t* tw,
    cm_polygon_t* poly,
    cm_trmVertex_t* v) {
  int i, edgeNum;
  ...
  if (f < tw->trace.fraction) {
    for (i = 0; i < poly->numEdges; i++) {
      ...
      if (edge->checkcount != idCollisionModelManagerLocal::checkCount) {
        ...
        pl.FromLine(tw->model->vertices[edge->vertexNum[0]].p,
                    tw->model->vertices[edge->vertexNum[1]].p);
...
================
*/
void idCollisionModelManagerLocal::TranslateVertexThroughTrmPolygon(
    cm_traceWork_t* tw,
...
================
*/
bool idCollisionModelManagerLocal::TranslateTrmThroughPolygon(
    cm_traceWork_t* tw,
    cm_polygon_t* p) {
  int i, j, k, edgeNum;
  float fraction, d;
  ...
  if (tw->pointTrace) {
    ...
    tw->polygonVertexPlueckerCache[p->numEdges] =
        tw->polygonVertexPlueckerCache[0];

    // trace trm vertices through polygon
    for (i = 0; i < tw->numVerts; i++) {
      bv = tw->vertices + i;
      if (bv->used) {
        idCollisionModelManagerLocal::TranslateTrmVertexThroughPolygon(tw, p,
                                                                       bv, i);
      }
    }
...
================
*/
void idCollisionModelManagerLocal::SetupTranslationHeartPlanes(
    cm_traceWork_t* tw) {
...
void idCollisionModelManagerLocal::Translation(trace_t* results,
  ...
  assert(((byte*)&end) < ((byte*)results) ||
         ((byte*)&end) >= (((byte*)results) + sizeof(trace_t)));
  ...
  if (cm_debugCollision.GetBool()) {
    if (!entered && !idCollisionModelManagerLocal::getContacts) {
      ...
      if (idCollisionModelManagerLocal::Contents(results->endpos, trm, trmAxis,
                                                 -1, model, modelOrigin,
                                                 modelAxis) &
          contentMask) {
        trace_t tr;
        ...
        idCollisionModelManagerLocal::Translation(&tr, start, end, trm, trmAxis,
                                                  contentMask, model,
                                                  modelOrigin, modelAxis);
      }
      entered = 0;

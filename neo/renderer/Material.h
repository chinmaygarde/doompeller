...
class idMaterial : public idDecl {
 public:
  ...
  bool LightCastsShadows() const {
    return TestMaterialFlag(MF_FORCESHADOWS) ||
           (!fogLight && !ambientLight && !blendLight &&
            !TestMaterialFlag(MF_NOSHADOWS));
  }
  ...
  // floats
  void EvaluateRegisters(float* regs,
 ...
 private:
  ...
  void MultiplyTextureMatrix(
      textureStage_t* ts,
      int registers[2][3]);  // FIXME: for some reason the const is bad for gcc
                             // and Mac
 ...
 private:
  ...
  mutable idUserInterface*
      gui;  // non-custom guis are shared by all users of a material

...
idScreenRect idRenderWorldLocal::ScreenRectFromWinding(const idWinding* w,
  ...
  for (i = 0; i < w->GetNumPoints(); i++) {
    ...
    windowX = 0.5f * (1.0f + ndc[0]) *
              (tr.viewDef->viewport.x2 - tr.viewDef->viewport.x1);
    windowY = 0.5f * (1.0f + ndc[1]) *
...
bool idRenderWorldLocal::PortalIsFoggedOut(const portal_t* p) {
  ...
  lightShader->EvaluateRegisters(regs, ldef->parms.shaderParms, tr.viewDef,
...
===================
*/
void idRenderWorldLocal::FloodLightThroughArea_r(
  ...
  for (p = area->portals; p; p = p->next) {
    ...
    for (i = 0; i < addPlanes; i++) {
      ...
      newStack.portalPlanes[newStack.numPortalPlanes].FitThroughPoint(
...
void idRenderWorldLocal::AddAreaEntityRefs(int areaNum,
  ...
  for (ref = area->entityRefs.areaNext; ref != &area->entityRefs;
    ...
    if (r_singleEntity.GetInteger() >= 0 &&
        r_singleEntity.GetInteger() != entity->index) {
      continue;
    }
...
void idRenderWorldLocal::AddAreaLightRefs(int areaNum,
  ...
  for (lref = area->lightRefs.areaNext; lref != &area->lightRefs;
       lref = lref->areaNext) {
    light = lref->light;

    // debug tool to allow viewing of only one light at a time
    if (r_singleLight.GetInteger() >= 0 &&
        r_singleLight.GetInteger() != light->index) {
      continue;
    }
    ...
    // door
    if (r_useLightCulling.GetInteger() >= 3 && !light->parms.noShadows &&
        light->lightShader->LightCastsShadows() && light->areaNum != -1 &&
        !tr.viewDef->connectedAreas[light->areaNum]) {
      continue;
    }
...
void idRenderWorldLocal::BuildConnectedAreas(void) {
  ...
  tr.viewDef->connectedAreas = (bool*)R_FrameAlloc(
      numPortalAreas * sizeof(tr.viewDef->connectedAreas[0]));
  ...
  memset(tr.viewDef->connectedAreas, 0,
         numPortalAreas * sizeof(tr.viewDef->connectedAreas[0]));
...
void idRenderWorldLocal::FindViewLightsAndEntities(void) {
  ...
  if (r_singleArea.GetBool()) {
    ...
    FlowViewThroughPortals(tr.viewDef->renderView.vieworg, 5,
                           tr.viewDef->frustum);
  }
}
...
bool idRenderWorldLocal::AreasAreConnected(int areaNum1,
  ...
  if (areaNum1 > numPortalAreas || areaNum2 > numPortalAreas || areaNum1 < 0 ||
      areaNum2 < 0) {
    common->Error("idRenderWorldLocal::AreAreasConnected: bad parms: %i, %i",
                  areaNum1, areaNum2);
  }
  ...
  if (attribute >= NUM_PORTAL_ATTRIBUTES ||
      (1 << attribute) != (int)connection) {
    common->Error(
        "idRenderWorldLocal::AreasAreConnected: bad connection number: %i\n",
        (int)connection);
  }

  return portalAreas[areaNum1].connectedAreaNum[attribute] ==
         portalAreas[areaNum2].connectedAreaNum[attribute];
}
...
void idRenderWorldLocal::SetPortalState(qhandle_t portal, int blockTypes) {
  ...
  for (int i = 0; i < NUM_PORTAL_ATTRIBUTES; i++) {
    if ((old ^ blockTypes) & (1 << i)) {
      connectedAreaNum++;
      FloodConnectedAreas(
          &portalAreas[doublePortals[portal - 1].portals[1]->intoArea], i);
    }
  }

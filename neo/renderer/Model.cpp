...
void idRenderModelStatic::Print() const {
  ...
  common->Printf("bounds: (%f %f %f) to (%f %f %f)\n", bounds[0][0],
                 bounds[0][1], bounds[0][2], bounds[1][0], bounds[1][1],
                 bounds[1][2]);
  ...
  for (int i = 0; i < NumSurfaces(); i++) {
    ...
    common->Printf("%2i: %5i %5i %s", i, tri->numVerts, tri->numIndexes / 3,
                   material->GetName());
...
void idRenderModelStatic::List() const {
  ...
  common->Printf("%c%4ik %3i %4i %4i %s", closed, totalBytes / 1024,
                 NumSurfaces(), totalVerts, totalTris, Name());
...
================
*/
static void AddCubeFace(srfTriangles_t* tri,
                        idVec3 v1,
                        idVec3 v2,
                        idVec3 v3,
                        idVec3 v4) {
...
void idRenderModelStatic::MakeDefaultModel() {
  ...
  AddCubeFace(tri, idVec3(-1, 1, 1), idVec3(1, 1, 1), idVec3(1, -1, 1),
              idVec3(-1, -1, 1));
  AddCubeFace(tri, idVec3(-1, 1, -1), idVec3(-1, -1, -1), idVec3(1, -1, -1),
              idVec3(1, 1, -1));
...
================
*/
srfTriangles_t* idRenderModelStatic::AllocSurfaceTriangles(
    int numVerts,
...
void idRenderModelStatic::FinishSurfaces() {
  ...
  for (i = 0; i < surfaces.Num(); i++) {
    ...
    R_CleanupTriangles(surf->geometry, surf->geometry->generateNormals, true,
...
bool idRenderModelStatic::ConvertASEToModelSurfaces(
    const struct aseModel_s* ase) {
  aseObject_t* object;
  aseMesh_t* mesh;
  ...
  for (objectNum = 0; objectNum < ase->objects.Num(); objectNum++) {
    ...
    if (fastLoad) {
      ...
      for (j = 0; j < mesh->numTVertexes; j++) {
        tvRemap[j] =
            texCoordSubset.FindVector(mesh->tvertexes, j, texCoordEpsilon);
      }
    }
    ...
    mvTable = (matchVert_t*)R_ClearedStaticAlloc(mesh->numFaces * 3 *
                                                 sizeof(mvTable[0]));

    // we will have a hash chain based on the xyz values
    mvHash = (matchVert_t**)R_ClearedStaticAlloc(mesh->numVertexes *
                                                 sizeof(mvHash[0]));
    ...
    for (j = 0; j < mesh->numFaces; j++) {
      for (k = 0; k < 3; k++) {
        ...
        for (lastmv = NULL, mv = mvHash[v]; mv != NULL;
             lastmv = mv, mv = mv->next) {
          if (mv->tv != tv) {
            continue;
          }
...
=================
*/
bool idRenderModelStatic::ConvertLWOToModelSurfaces(
    const struct st_lwObject* lwo) {
  const idMaterial *im1, *im2;
  srfTriangles_t* tri;
  lwSurface* lwoSurf;
  int numTVertexes;
  ...
  if (layer->point.count <= 0) {
    common->Warning(
        "ConvertLWOToModelSurfaces: model \'%s\' has bad or missing vertex "
        "data",
        name.c_str());
    return false;
  }
  ...
  for (lwoSurf = lwo->surf, i = 0; lwoSurf; lwoSurf = lwoSurf->next, i++) {
    ...
    // the maximum possible number of combined vertexes is the number of indexes
    mvTable = (matchVert_t*)R_ClearedStaticAlloc(layer->polygon.count * 3 *
                                                 sizeof(mvTable[0]));

    // we will have a hash chain based on the xyz values
    mvHash = (matchVert_t**)R_ClearedStaticAlloc(layer->point.count *
                                                 sizeof(mvHash[0]));
    ...
    for (j = 0; j < layer->polygon.count; j++) {
      ...
      if (poly->nverts != 3) {
        common->Warning(
            "ConvertLWOToModelSurfaces: model %s has too many verts for a "
...
=================
*/
struct aseModel_s* idRenderModelStatic::ConvertLWOToASE(
    const struct st_lwObject* obj,
    const char* fileName) {
  int j, k;
  aseModel_t* ase;
  ...
  for (lwSurface* surf = obj->surf; surf; surf = surf->next) {
    ...
    mesh->faces =
        (aseFace_t*)Mem_Alloc(mesh->numFaces * sizeof(mesh->faces[0]));

    mesh->numVertexes = layer->point.count;
    mesh->vertexes =
        (idVec3*)Mem_Alloc(mesh->numVertexes * sizeof(mesh->vertexes[0]));
    ...
    if (mesh->numTVertexes) {
      mesh->tvertexes =
          (idVec2*)Mem_Alloc(mesh->numTVertexes * sizeof(mesh->tvertexes[0]));
      int offset = 0;
      for (lwVMap* vm = layer->vmap; vm; vm = vm->next) {
        if (vm->type == LWID_('T', 'X', 'U', 'V')) {
          vm->offset = offset;
          for (k = 0; k < vm->nverts; k++) {
            ...
            mesh->tvertexes[k + offset].y =
                1.0f - vm->val[k][1];  // invert the t
          }
          offset += vm->nverts;
        }
      }
      ...
      mesh->numTVertexes = 1;
      mesh->tvertexes = (idVec2*)Mem_ClearedAlloc(mesh->numTVertexes *
                                                  sizeof(mesh->tvertexes[0]));
    }

    mesh->normalsParsed = true;
    mesh->colorsParsed =
        true;  // because we are falling back to the surface color
    ...
    for (j = 0; j < layer->polygon.count; j++) {
      ...
      if (poly->nverts != 3) {
        common->Warning(
            "ConvertLWOToASE: model %s has too many verts for a poly! Make "
  ...
  for (objectNum = 0; objectNum < ma->objects.Num(); objectNum++) {
    ...
    for (j = 0; j < mesh->numFaces; j++) {
      for (k = 0; k < 3; k++) {
        ...
        //  we may or may not have colors to compare
        if (mesh->faces[j].vertexColors[k] != -1 &&
            mesh->faces[j].vertexColors[k] != -999) {
...
bool idRenderModelStatic::LoadFLT(const char* fileName) {
  ...
  for (int i = 0; i < len / 4; i++) {
    ...
    image_p[0] = image_p[1] = image_p[2] = v * 255;
    image_p[3] = 255;
    image_p += 4;
  }
...
// return false;
  ...
  for (int i = 0; i < height; i++) {
    for (int j = 0; j < width; j++) {
      ...
      tri->verts[v].xyz[2] =
          data[(minY + i) * size + minX + j];  // height is in meters

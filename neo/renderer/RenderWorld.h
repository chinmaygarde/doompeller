...
const int SHADERPARM_DIVERSITY =
    5;  // random between 0.0 and 1.0 for some effects (muzzle flashes, etc)
const int SHADERPARM_MODE = 7;  // for selecting which shader passes to enable
const int SHADERPARM_TIME_OF_DEATH =
    7;  // for the monster skin-burn-away effect enable and time offset

// model parms
const int SHADERPARM_MD5_SKINSCALE =
    8;  // for scaling vertex offsets on md5 models (jack skellington effect)
...
const int SHADERPARM_PARTICLE_STOPTIME =
    8;  // don't spawn any more particles after this time
...
typedef struct renderEntity_s {
  ...
  const idMaterial*
      referenceShader;  // used so flares can reference the proper light shader
  ...
  struct renderView_s*
      remoteRenderView;  // any remote camera surfaces will use this
...
typedef struct renderLight_s {
  ...
  float
      shaderParms[MAX_ENTITY_SHADER_PARMS];  // can be used in any way by shader
...
typedef struct renderView_s {
  ...
  idVec3 vieworg;
  idMat3
      viewaxis;  // transformation matrix, view looks down the positive X axis
  ...
  int time;
  float
      shaderParms[MAX_GLOBAL_SHADER_PARMS];  // can be used in any way by shader
...
typedef struct {
  ...
  const idWinding*
      w;  // winding points have counter clockwise ordering seen from areas[0]
...
class idRenderWorld {
 public:
  ...
  virtual void UpdateEntityDef(qhandle_t entityHandle,
                               const renderEntity_t* re) = 0;
  virtual void FreeEntityDef(qhandle_t entityHandle) = 0;
  virtual const renderEntity_t* GetRenderEntity(
      qhandle_t entityHandle) const = 0;

  virtual qhandle_t AddLightDef(const renderLight_t* rlight) = 0;
  virtual void UpdateLightDef(qhandle_t lightHandle,
                              const renderLight_t* rlight) = 0;
  ...
  virtual void ProjectDecalOntoWorld(const idFixedWinding& winding,
  ...
  virtual void ProjectDecal(qhandle_t entityHandle,
  ...
  virtual void ProjectOverlay(qhandle_t entityHandle,
  ...
  virtual bool AreasAreConnected(int areaNum1,
                                 int areaNum2,
                                 portalConnection_t connection) = 0;
  ...
  virtual int BoundsInAreas(const idBounds& bounds,
                            int* areas,
                            int maxAreas) const = 0;
  ...
  // flags.
  virtual bool Trace(modelTrace_t& trace,
                     const idVec3& start,
                     const idVec3& end,
                     const float radius,
                     bool skipDynamic = true,
                     bool skipPlayer = false) const = 0;

  // Traces vs the world model bsp tree.
  virtual bool FastWorldTrace(modelTrace_t& trace,
                              const idVec3& start,
                              const idVec3& end) const = 0;
  ...
  virtual void DebugLine(const idVec4& color,
                         const idVec3& start,
                         const idVec3& end,
                         const int lifetime = 0,
                         const bool depthTest = false) = 0;
  virtual void DebugArrow(const idVec4& color,
                          const idVec3& start,
                          const idVec3& end,
                          int size,
                          const int lifetime = 0) = 0;
  virtual void DebugWinding(const idVec4& color,
                            const idWinding& w,
                            const idVec3& origin,
                            const idMat3& axis,
                            const int lifetime = 0,
                            const bool depthTest = false) = 0;
  virtual void DebugCircle(const idVec4& color,
                           const idVec3& origin,
                           const idVec3& dir,
                           const float radius,
                           const int numSteps,
                           const int lifetime = 0,
                           const bool depthTest = false) = 0;
  virtual void DebugSphere(const idVec4& color,
                           const idSphere& sphere,
                           const int lifetime = 0,
                           bool depthTest = false) = 0;
  virtual void DebugBounds(const idVec4& color,
                           const idBounds& bounds,
                           const idVec3& org = vec3_origin,
                           const int lifetime = 0) = 0;
  virtual void DebugBox(const idVec4& color,
                        const idBox& box,
                        const int lifetime = 0) = 0;
  virtual void DebugFrustum(const idVec4& color,
                            const idFrustum& frustum,
                            const bool showFromOrigin = false,
                            const int lifetime = 0) = 0;
  virtual void DebugCone(const idVec4& color,
                         const idVec3& apex,
                         const idVec3& dir,
                         float radius1,
                         float radius2,
  ...
  virtual void DebugClearPolygons(
      int time) = 0;  // a time of 0 will clear all polygons
  virtual void DebugPolygon(const idVec4& color,

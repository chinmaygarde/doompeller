...
bool MA_ReadVec3(idParser& parser, idVec3& vec) {
  ...
  if (!parser.SkipUntilString("double3")) {
    throw idException(
        va("Maya Loader '%s': Invalid Vec3", parser.GetFileName()));
    return false;
  }
...
bool IsNodeComplete(idToken& token) {
  if (!token.Icmp("createNode") || !token.Icmp("connectAttr") ||
      !token.Icmp("select")) {
    return true;
  }
  return false;
...
bool MA_ParseVertexTransforms(idParser& parser, maAttribHeader_t* header) {
  ...
  if (!pMesh->vertTransforms) {
    ...
    pMesh->vertTransforms =
        (idVec4*)Mem_Alloc(sizeof(idVec4) * pMesh->numVertTransforms);
    pMesh->nextVertTransformIndex = 0;
  }

  // Get the start and end index for this attribute
  int minIndex, maxIndex;
  if (!MA_ParseHeaderIndex(header, minIndex, maxIndex, "VertexTransformHeader",
                           NULL)) {
    // This was just a header
    return true;
  }
  ...
  // Read each vert
  for (int i = minIndex; i <= maxIndex; i++) {
    pMesh->vertTransforms[pMesh->nextVertTransformIndex].x =
        parser.ParseFloat();
    pMesh->vertTransforms[pMesh->nextVertTransformIndex].z =
        parser.ParseFloat();
    pMesh->vertTransforms[pMesh->nextVertTransformIndex].y =
        -parser.ParseFloat();
...
bool MA_ParseFace(idParser& parser, maAttribHeader_t* header) {
  ...
  while (parser.ReadToken(&token)) {
    ...
    if (!token.Icmp("f")) {
      ...
      if (count != 3) {
        throw idException(va("Maya Loader '%s': Face is not a triangle.",
                             parser.GetFileName()));
        return false;
      }
      ...
      int count = parser.ParseInt();
      if (count != 3) {
        throw idException(va("Maya Loader '%s': Invalid texture coordinates.",
                             parser.GetFileName()));
        return false;
      }
      ...
      int count = parser.ParseInt();
      if (count != 3) {
        throw idException(va("Maya Loader '%s': Invalid vertex color.",
                             parser.GetFileName()));
        return false;
      }
...
bool MA_ParseTVert(idParser& parser, maAttribHeader_t* header) {
  ...
  // Get the start and end index for this attribute
  int minIndex, maxIndex;
  if (!MA_ParseHeaderIndex(header, minIndex, maxIndex, "TextureCoordHeader",
                           "uvsp")) {
    // This was just a header
    return true;
  }
...
bool MA_QuickIsVertShared(int faceIndex, int vertIndex) {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (pMesh->edges[edge].z == 1 &&
        (pMesh->edges[edge].x == vertNum || pMesh->edges[edge].y == vertNum)) {
      return true;
    }
  }
  return false;
...
void MA_ParseMesh(idParser& parser) {
  ...
  for (int i = 0; i < pMesh->numVertTransforms; i++) {
    pMesh->vertexes[(int)pMesh->vertTransforms[i].w] +=
        pMesh->vertTransforms[i].ToVec3();
  }
...
void MA_ParseCreateNode(idParser& parser) {
  ...
  } else if (!token.Icmp("shadingEngine") || !token.Icmp("lambert") ||
             !token.Icmp("phong") || !token.Icmp("blinn")) {
...
bool MA_ParseConnectAttr(idParser& parser) {
  ...
  int dot = temp.Find(".");
  if (dot == -1) {
    throw idException(va("Maya Loader '%s': Invalid Connect Attribute.",
                         parser.GetFileName()));
    return false;
  }
...
void MA_ApplyTransformation(maModel_t* model) {
  for (int i = 0; i < model->objects.Num(); i++) {
    ...
    while (transform) {
      ...
      MA_BuildScale(scale, transform->scale.x, transform->scale.y,
                    transform->scale.z);

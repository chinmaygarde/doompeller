...
void R_SetLightProject(idPlane lightProject[4],
  ...
  targetGlobal[3] = 1;
  ofs = 0.5f - (targetGlobal * lightProject[0].ToVec4()) /
                   (targetGlobal * lightProject[2].ToVec4());
  lightProject[0].ToVec4() += ofs * lightProject[2].ToVec4();
  ofs = 0.5f - (targetGlobal * lightProject[1].ToVec4()) /
                   (targetGlobal * lightProject[2].ToVec4());
...
void R_DeriveLightData(idRenderLightLocal* light) {
  ...
  if (!light->parms.pointLight) {
    ...
    R_SetLightProject(light->lightProject,
                      vec3_origin /* light->parms.origin */,
                      light->parms.target, light->parms.right, light->parms.up,
                      light->parms.start, light->parms.end);
  } else {
    // point light
  ...
  if (light->parms.parallel) {
    ...
    light->globalLightOrigin =
        light->parms.origin + light->parms.axis * light->parms.lightCenter;
  }

  R_FreeLightDefFrustum(light);

  light->frustumTris =
      R_PolytopeSurface(6, light->frustum, light->frustumWindings);
...
void R_CreateLightRefs(idRenderLightLocal* light) {
  ...
  if (tri->numVerts > MAX_LIGHT_VERTS) {
    common->Error("R_CreateLightRefs: %i points in frustumTris!",
                  tri->numVerts);
  }
  ...
  if (r_showUpdates.GetBool() &&
      (tri->bounds[1][0] - tri->bounds[0][0] > 1024 ||
  ...
  if (light->parms.prelightModel && r_useLightPortalFlow.GetBool() &&
...
void R_FreeLightDefDerivedData(idRenderLightLocal* ldef) {
  ...
  for (doublePortal_t* dp = ldef->foggedPortals; dp;
...
void R_ClearEntityDefDynamicModel(idRenderEntityLocal* def) {
  ...
  for (idInteraction* inter = def->firstInteraction;
       inter != NULL && !inter->IsEmpty(); inter = inter->entityNext) {
    inter->FreeSurfaces();
  }

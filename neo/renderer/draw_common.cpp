...
void RB_PrepareStageTexturing(const shaderStage_t* pStage,
  ...
  if (pStage->texture.texgen == TG_SKYBOX_CUBE ||
      pStage->texture.texgen == TG_WOBBLESKY_CUBE) {
    qglTexCoordPointer(3, GL_FLOAT, 0,
  ...
  if (pStage->texture.texgen == TG_SCREEN) {
    ...
    myGlMultMatrix(surf->space->modelViewMatrix,
...
void RB_FinishStageTexturing(const shaderStage_t* pStage,
  ...
  if (pStage->texture.texgen == TG_REFLECT_CUBE) {
    if (tr.backEndRenderer == BE_ARB2) {
      ...
      // remove as soon as it gets fixed.
...
void RB_T_FillDepthBuffer(const drawSurf_t* surf) {
  ...
  if (backEnd.viewDef->numClipPlanes && surf->space != backEnd.currentSpace) {
    ...
    R_GlobalPlaneToLocal(surf->space->modelMatrix,
                         backEnd.viewDef->clipPlanes[0], plane);
...
void RB_STD_FillDepthBuffer(drawSurf_t** drawSurfs, int numDrawSurfs) {
  ...
  RB_RenderDrawSurfListWithFunction(drawSurfs, numDrawSurfs,
                                    RB_T_FillDepthBuffer);
...
Sets variables related to the current space that can be used by all vertex
programs
==================
*/
void RB_SetProgramEnvironmentSpace(void) {
  ...
  R_GlobalPointToLocal(space->modelMatrix, backEnd.viewDef->renderView.vieworg,
                       *(idVec3*)parm);
  parm[3] = 1.0;
...
void RB_STD_T_RenderShaderPasses(const drawSurf_t* surf) {
  ...
  for (stage = 0; stage < shader->GetNumStages(); stage++) {
    ...
    // skip if the stage is ( GL_ZERO, GL_ONE ), which is used for some alpha
    // masks
    ...
    if (newStage) {
      ...
      qglColorPointer(4, GL_UNSIGNED_BYTE, sizeof(idDrawVert),
                      (void*)&ac->color);
      qglVertexAttribPointerARB(9, 3, GL_FLOAT, false, sizeof(idDrawVert),
                                ac->tangents[0].ToFloatPtr());
      qglVertexAttribPointerARB(10, 3, GL_FLOAT, false, sizeof(idDrawVert),
      ...
      if (newStage->megaTexture) {
        ...
        R_GlobalPointToLocal(surf->space->modelMatrix,
                             backEnd.viewDef->renderView.vieworg, localViewer);
...
int RB_STD_DrawShaderPasses(drawSurf_t** drawSurfs, int numDrawSurfs) {
  ...
  for (i = 0; i < numDrawSurfs; i++) {
    ...
    // we need to draw the post process shaders after we have drawn the fog
    // lights
...
static void RB_T_Shadow(const drawSurf_t* surf) {
  ...
  // set the light position if we are using a vertex program to project the rear
  // surfaces
  ...
  qglVertexPointer(4, GL_FLOAT, sizeof(shadowCache_t),
                   vertexCache.Position(tri->shadowCache));

  // we always draw the sil planes, but we may not need to draw the front or
  // rear caps
  int numIndexes;
  bool external = false;
  ...
  if (!r_useExternalShadows.GetInteger()) {
    numIndexes = tri->numIndexes;
             ...
             2) {  // force to no caps for testing
    ...
    // if we are inside the shadow projection, but outside the light, and
    // drawing a non-infinite shadow, we can skip some caps
    if (backEnd.vLight->viewSeesShadowPlaneBits &
        surf->geo->shadowCapPlaneBits) {
      // we can see through a rear cap, so we need to draw it, but we can skip
      // the caps on the actual surface
  ...
  if (r_showShadows.GetInteger()) {
    if (r_showShadows.GetInteger() == 3) {
      if (external) {
        qglColor3f(0.1 / backEnd.overBright, 1 / backEnd.overBright,
                   0.1 / backEnd.overBright);
      } else {
        // these are the surfaces that require the reverse
        qglColor3f(1 / backEnd.overBright, 0.1 / backEnd.overBright,
                   0.1 / backEnd.overBright);
      }
    } else {
      ...
      if (surf->geo->shadowCapPlaneBits & SHADOW_CAP_INFINITE) {
        if (numIndexes == tri->numIndexes) {
          ...
          qglColor3f(1 / backEnd.overBright, 0.4 / backEnd.overBright,
                     0.1 / backEnd.overBright);
        }
      } else {
        if (numIndexes == tri->numIndexes) {
          ...
          qglColor3f(0.1 / backEnd.overBright, 1 / backEnd.overBright,
                     0.6 / backEnd.overBright);
        } else {
          qglColor3f(0.6 / backEnd.overBright, 1 / backEnd.overBright,
                     0.1 / backEnd.overBright);
        }
      }
    }
...
void RB_StencilShadowPass(const drawSurf_t* drawSurfs) {
  ...
  if (r_showShadows.GetInteger()) {
    if (r_showShadows.GetInteger() == 2) {
      // draw filled in
      GL_State(GLS_DEPTHMASK | GLS_SRCBLEND_ONE | GLS_DSTBLEND_ONE |
               GLS_DEPTHFUNC_LESS);
    } else {
      // draw as lines, filling the depth buffer
      GL_State(GLS_SRCBLEND_ONE | GLS_DSTBLEND_ZERO | GLS_POLYMODE_LINE |
  ...
  if (r_shadowPolygonFactor.GetFloat() || r_shadowPolygonOffset.GetFloat()) {
    qglPolygonOffset(r_shadowPolygonFactor.GetFloat(),
...
static void RB_T_BlendLight(const drawSurf_t* surf) {
  ...
  if (backEnd.currentSpace != surf->space) {
    ...
    for (i = 0; i < 4; i++) {
      R_GlobalPlaneToLocal(surf->space->modelMatrix,
                           backEnd.vLight->lightProject[i], lightProject[i]);
    }
...
=====================
*/
static void RB_BlendLight(const drawSurf_t* drawSurfs,
  ...
  for (i = 0; i < lightShader->GetNumStages(); i++) {
    ...
    // get the modulate values from the light, including alpha, unlike normal
    // lights
...
static void RB_T_BasicFog(const drawSurf_t* surf) {
  if (backEnd.currentSpace != surf->space) {
    ...
    //		R_GlobalPlaneToLocal( surf->space->modelMatrix, fogPlanes[1],
    //local ); 		local[3] += 0.5;
...
==================
*/
static void RB_FogPass(const drawSurf_t* drawSurfs,
                       const drawSurf_t* drawSurfs2) {
  ...
  // create a surface for the light frustom triangles, which are oriented drawn
  // side out
  ...
  GL_State(GLS_DEPTHMASK | GLS_SRCBLEND_SRC_ALPHA |
  ...
  // T will get a texgen for the fade plane, which is always the "top" plane on
  // unrotated lights
  ...
  float s = backEnd.viewDef->renderView.vieworg * fogPlanes[2].Normal() +
            fogPlanes[2][3];
  ...
  // depthfunc_less instead of depthfunc_equal
  GL_State(GLS_DEPTHMASK | GLS_SRCBLEND_SRC_ALPHA |
           GLS_DSTBLEND_ONE_MINUS_SRC_ALPHA | GLS_DEPTHFUNC_LESS);
  GL_Cull(CT_BACK_SIDED);

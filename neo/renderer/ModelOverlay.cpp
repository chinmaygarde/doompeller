...
void idRenderModelOverlay::CreateOverlay(const idRenderModel* model,
  ...
  overlayVertex_t* overlayVerts =
      (overlayVertex_t*)_alloca(maxVerts * sizeof(*overlayVerts));
  glIndex_t* overlayIndexes =
      (glIndex_t*)_alloca16(maxIndexes * sizeof(*overlayIndexes));
  ...
  for (surfNum = 0; surfNum < model->NumBaseSurfaces(); surfNum++) {
    ...
    idVec2* texCoords =
        (idVec2*)_alloca16(stri->numVerts * sizeof(texCoords[0]));

    SIMDProcessor->OverlayPointCull(cullBits, texCoords, localTextureAxis,
                                    stri->verts, stri->numVerts);

    glIndex_t* vertexRemap =
        (glIndex_t*)_alloca16(sizeof(vertexRemap[0]) * stri->numVerts);
    SIMDProcessor->Memset(vertexRemap, -1,
                          sizeof(vertexRemap[0]) * stri->numVerts);
    ...
    overlaySurface_t* s =
        (overlaySurface_t*)Mem_Alloc(sizeof(overlaySurface_t));
...
void idRenderModelOverlay::AddOverlaySurfacesToModel(idRenderModel* baseModel) {
  ...
  if (baseModel->IsDynamicModel() != DM_STATIC) {
    common->Error(
        "idRenderModelOverlay::AddOverlaySurfacesToModel: baseModel is not a "
        "static model");
  }
  ...
  for (k = 0; k < materials.Num(); k++) {
    ...
    if (newSurf->geometry == NULL || newSurf->geometry->numVerts < numVerts ||
      ...
      SIMDProcessor->Memset(newSurf->geometry->verts, 0,
                            numVerts * sizeof(newTri->verts[0]));
    } else {
    ...
    for (i = 0; i < materials[k]->surfaces.Num(); i++) {
      ...
      for (j = 0; j < surf->numVerts; j++) {
        ...
        if (overlayVert->vertexNum >= baseSurf->geometry->numVerts) {
          ...
          common->Warning(
              "idRenderModelOverlay::AddOverlaySurfacesToModel: overlay vertex "
              "out of range.  Model has probably changed since generating the "
              "overlay.");
          FreeSurface(surf);
          materials[k]->surfaces.RemoveIndex(i);
        ...
        newTri->verts[numVerts].xyz =
            baseSurf->geometry->verts[overlayVert->vertexNum].xyz;
        numVerts++;
      }
    }
    ...
    staticModel->overlaysAdded++;  // so we don't create an overlay on an
                                   // overlay surface
  }
}
...
====================
*/
void idRenderModelOverlay::RemoveOverlaySurfacesFromModel(
    idRenderModel* baseModel) {
  idRenderModelStatic* staticModel;

...
void RB_DrawElementsWithCounters(const srfTriangles_t* tri) {
  ...
  if (tri->indexCache && r_useIndexBuffers.GetBool()) {
    qglDrawElements(GL_TRIANGLES,
                    ...
                    GL_INDEX_TYPE, (int*)vertexCache.Position(tri->indexCache));
...
void RB_DrawShadowElementsWithCounters(const srfTriangles_t* tri,
  ...
  if (tri->indexCache && r_useIndexBuffers.GetBool()) {
    qglDrawElements(GL_TRIANGLES, r_singleTriangle.GetBool() ? 3 : numIndexes,
                    GL_INDEX_TYPE, (int*)vertexCache.Position(tri->indexCache));
...
void RB_BindVariableStageImage(const textureStage_t* texture,
  ...
  if (texture->cinematic) {
    ...
    cin = texture->cinematic->ImageForTime(
        (int)(1000 * (backEnd.viewDef->floatTime +
                      backEnd.viewDef->renderView.shaderParms[11])));

    if (cin.image) {
      globalImages->cinematicImage->UploadScratch(cin.image, cin.imageWidth,
                                                  cin.imageHeight);
    } else {
...
void RB_BindStageTexture(const float* shaderRegisters,
  ...
  if (texture->texgen == TG_DIFFUSE_CUBE) {
    qglTexCoordPointer(
        3, GL_FLOAT, sizeof(idDrawVert),
        ((idDrawVert*)vertexCache.Position(surf->geo->ambientCache))
            ->normal.ToFloatPtr());
  }
  if (texture->texgen == TG_SKYBOX_CUBE ||
...
void RB_FinishStageTexture(const textureStage_t* texture,
  ...
  if (texture->texgen == TG_DIFFUSE_CUBE || texture->texgen == TG_SKYBOX_CUBE ||
      texture->texgen == TG_WOBBLESKY_CUBE) {
    qglTexCoordPointer(
        2, GL_FLOAT, sizeof(idDrawVert),
        (void*)&(
            ((idDrawVert*)vertexCache.Position(surf->geo->ambientCache))->st));
  }
...
void RB_DetermineLightScale(void) {
  ...
  for (vLight = backEnd.viewDef->viewLights; vLight; vLight = vLight->next) {
    ...
    for (i = 0; i < numStages; i++) {
      ...
      for (j = 0; j < 3; j++) {
        float v = r_lightScale.GetFloat() *
                  vLight->shaderRegisters[stage->color.registers[j]];
        if (v > max) {
          max = v;
        }
      }
...
void RB_BeginDrawingView(void) {
  ...
  qglScissor(tr.viewportOffset[0] + backEnd.viewDef->viewport.x1 +
                 backEnd.viewDef->scissor.x1,
             tr.viewportOffset[1] + backEnd.viewDef->viewport.y1 +
                 backEnd.viewDef->scissor.y1,
...
=================
*/
static void RB_SubmittInteraction(
  ...
  if (((din->diffuseColor[0] > 0 || din->diffuseColor[1] > 0 ||
        din->diffuseColor[2] > 0) &&
       din->diffuseImage != globalImages->blackImage) ||
      ((din->specularColor[0] > 0 || din->specularColor[1] > 0 ||
...
void RB_CreateSingleDrawInteractions(
  ...
  R_GlobalPointToLocal(surf->space->modelMatrix, vLight->globalLightOrigin,
                       inter.localLightOrigin.ToVec3());
  R_GlobalPointToLocal(surf->space->modelMatrix,
                       backEnd.viewDef->renderView.vieworg,
                       inter.localViewOrigin.ToVec3());
  ...
  for (int lightStageNum = 0; lightStageNum < lightShader->GetNumStages();
       lightStageNum++) {
    ...
    if (lightStage->texture.hasMatrix) {
      RB_GetShaderTextureMatrix(lightRegs, &lightStage->texture,
                                backEnd.lightTextureMatrix);
      RB_BakeTextureMatrixIntoTexgen(
          reinterpret_cast<class idPlane*>(inter.lightProjection),
          backEnd.lightTextureMatrix);
    }
    ...
    inter.diffuseColor[0] = inter.diffuseColor[1] = inter.diffuseColor[2] =
        inter.diffuseColor[3] = 0;
    inter.specularColor[0] = inter.specularColor[1] = inter.specularColor[2] =
        inter.specularColor[3] = 0;
    ...
    for (int surfaceStageNum = 0;
         surfaceStageNum < surfaceShader->GetNumStages(); surfaceStageNum++) {
      const shaderStage_t* surfaceStage =
          surfaceShader->GetStage(surfaceStageNum);
      ...
      switch (surfaceStage->lighting) {
        ...
        case SL_BUMP: {
          ...
          inter.diffuseImage = NULL;
          inter.specularImage = NULL;
          R_SetDrawInteraction(surfaceStage, surfaceRegs, &inter.bumpImage,
        ...
        case SL_SPECULAR: {
          ...
          R_SetDrawInteraction(surfaceStage, surfaceRegs, &inter.specularImage,
                               ...
                               inter.specularColor.ToFloatPtr());

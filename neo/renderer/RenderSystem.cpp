...
static void R_PerformanceCounters(void) {
  if (r_showPrimitives.GetInteger() != 0) {
    ...
    if (r_showPrimitives.GetInteger() > 1) {
      common->Printf(
          ...
          backEnd.pc.c_shadowIndexes / 3, backEnd.pc.c_shadowVertexes,
          megaBytes);
    } else {
      common->Printf(
          ...
          backEnd.pc.c_shadowIndexes / 3, backEnd.pc.c_vboIndexes / 3,
          megaBytes);
    }
  }
  ...
  if (r_showDynamic.GetBool()) {
    common->Printf(
        ...
        tr.pc.c_deformedIndexes / 3, tr.pc.c_tangentIndexes / 3,
        tr.pc.c_guiSurfs);
  }

  if (r_showCull.GetBool()) {
    common->Printf("%i sin %i sclip  %i sout %i bin %i bout\n",
                   tr.pc.c_sphere_cull_in, tr.pc.c_sphere_cull_clip,
                   tr.pc.c_sphere_cull_out, tr.pc.c_box_cull_in,
  ...
  if (r_showDefs.GetBool()) {
    common->Printf("viewEntities:%i  shadowEntities:%i  viewLights:%i\n",
                   tr.pc.c_visibleViewEntities, tr.pc.c_shadowViewEntities,
                   tr.pc.c_viewLights);
  }
  if (r_showUpdates.GetBool()) {
    common->Printf(
        "entityUpdates:%i  entityRefs:%i  lightUpdates:%i  lightRefs:%i\n",
        tr.pc.c_entityUpdates, tr.pc.c_entityReferences, tr.pc.c_lightUpdates,
...
void R_ClearCommandChain(void) {
  ...
  frameData->cmdHead = frameData->cmdTail =
      (emptyCommand_t*)R_FrameAlloc(sizeof(*frameData->cmdHead));
...
void R_LockSurfaceScene(viewDef_t* parms) {
  ...
  for (vModel = tr.lockSurfacesCmd.viewDef->viewEntitys; vModel;
       vModel = vModel->next) {
...
=============
*/
void idRenderSystemLocal::GlobalToNormalizedDeviceCoordinates(
    const idVec3& global,
    idVec3& ndc) {
...
void idRenderSystemLocal::DrawBigChar(int x,
  ...
  DrawStretchPic(x, y, BIGCHAR_WIDTH, BIGCHAR_HEIGHT, fcol, frow, fcol + size,
                 frow + size, material);
}
...
void idRenderSystemLocal::RenderViewToViewport(const renderView_t* renderView,
  ...
  viewport->y1 =
      idMath::Ftoi((rc->y + rc->height) -
                   floor((renderView->y + renderView->height) * hRatio + 0.5f));
  viewport->y2 = idMath::Ftoi((rc->y + rc->height) -
                              floor(renderView->y * hRatio + 0.5f) - 1);
}
...
void idRenderSystemLocal::CropRenderSize(int width,
  ...
  if (currentRenderCrop == MAX_RENDER_CROPS) {
    common->Error(
        "idRenderSystemLocal::CropRenderSize: currentRenderCrop == "
        "MAX_RENDER_CROPS");
  }
...
void idRenderSystemLocal::CaptureRenderToImage(const char* imageName) {
  ...
  idImage* image = globalImages->ImageFromFile(imageName, TF_DEFAULT, true,
                                               TR_REPEAT, TD_DEFAULT);

  renderCrop_t* rc = &renderCrops[currentRenderCrop];

  copyRenderCommand_t* cmd =
      (copyRenderCommand_t*)R_GetCommandBuffer(sizeof(*cmd));
...
void idRenderSystemLocal::CaptureRenderToFile(const char* fileName,
  ...
  qglReadPixels(rc->x, rc->y, rc->width, rc->height, GL_RGB, GL_UNSIGNED_BYTE,
                data);

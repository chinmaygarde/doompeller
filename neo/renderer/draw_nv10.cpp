...
static void RB_RenderInteraction(const drawSurf_t* surf) {
  ...
  for (int i = 0; i < surfaceShader->GetNumStages(); i++) {
    ...
    if (surfaceStage->lighting == SL_BUMP) {
      ...
      if (lightShader->IsAmbientLight()) {
        ...
        qglCombinerParameterfvNV(GL_CONSTANT_COLOR0_NV,
                                 tr.ambientLightVector.ToFloatPtr());
        ...
        qglCombinerOutputNV(GL_COMBINER0_NV, GL_RGB, GL_PRIMARY_COLOR_NV,
        ...
        qglCombinerOutputNV(GL_COMBINER1_NV, GL_ALPHA, GL_PRIMARY_COLOR_NV,
                            GL_DISCARD_NV, GL_DISCARD_NV, GL_NONE, GL_NONE,
      ...
      qglCombinerOutputNV(GL_COMBINER0_NV, GL_ALPHA, GL_DISCARD_NV,
                          GL_DISCARD_NV, GL_DISCARD_NV, GL_NONE, GL_NONE,
                          GL_FALSE, GL_FALSE, GL_FALSE);
      ...
      //
      GL_State(GLS_SRCBLEND_DST_ALPHA | GLS_DSTBLEND_ZERO | GLS_COLORMASK |
               GLS_DEPTHMASK | backEnd.depthFunc);
      ...
      qglEnableClientState(GL_TEXTURE_COORD_ARRAY);
      qglTexCoordPointer(
          3, GL_FLOAT, sizeof(lightingCache_t),
          ((lightingCache_t*)vertexCache.Position(tri->lightingCache))
              ->localLightVector.ToFloatPtr());
    ...
    if (surfaceStage->lighting == SL_SPECULAR) {
      ...
      GL_State(GLS_SRCBLEND_DST_ALPHA | GLS_DSTBLEND_SRC_ALPHA | GLS_COLORMASK |
               GLS_DEPTHMASK | backEnd.depthFunc);
      ...
      globalImages->normalCubeMapImage->Bind();
      qglEnableClientState(GL_TEXTURE_COORD_ARRAY);
      qglTexCoordPointer(4, GL_FLOAT, 0,
      ...
      qglCombinerOutputNV(GL_COMBINER1_NV, GL_RGB, GL_PRIMARY_COLOR_NV,
                          GL_DISCARD_NV, GL_DISCARD_NV, GL_NONE, GL_NONE,
                          GL_FALSE, GL_FALSE, GL_FALSE);
      ...
      // fall through to the common handling of diffuse and specular projected
      // lighting
    }
    ...
    //-----------------------------------------------------
    if (surfaceStage->lighting == SL_DIFFUSE ||
        surfaceStage->lighting == SL_SPECULAR) {
      // don't trash alpha
      GL_State(GLS_SRCBLEND_DST_ALPHA | GLS_DSTBLEND_ONE | GLS_ALPHAMASK |
               GLS_DEPTHMASK | backEnd.depthFunc);
      ...
      if (surfaceStage->vertexColor == SVC_MODULATE) {
        ...
        qglCombinerInputNV(GL_COMBINER0_NV, GL_RGB, GL_VARIABLE_B_NV, GL_ZERO,
      ...
      qglCombinerInputNV(GL_COMBINER0_NV, GL_RGB, GL_VARIABLE_D_NV,
                         GL_CONSTANT_COLOR1_NV, GL_UNSIGNED_IDENTITY_NV,
                         GL_RGB);
      qglCombinerOutputNV(GL_COMBINER0_NV, GL_RGB, GL_SPARE0_NV, GL_SPARE1_NV,

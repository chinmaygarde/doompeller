  ...
  while (1) {
    ...
    switch (id) {
      ...
      case ID_KEY:
        ...
        key->time = getF4(fp);
        key->value = getF4(fp);
        lwListInsert((void**)&env->key, key,
                     (int(__cdecl*)(void*, void*))compare_keys);
        env->nkeys++;
        break;
  ...
  while (1) {
    ...
    switch (id) {
      ...
      case ID_TVEL:
        for (i = 0; i < 3; i++)
          v[i] = getF4(fp);
        tex->tmap.center.eindex =
            add_tvel(tex->tmap.center.val, v, &obj->env, &obj->nenvs);
        break;
...
void lwGetPolyNormals(lwPointList* point, lwPolygonList* polygon) {
  ...
  for (i = 0; i < polygon->count; i++) {
    ...
    for (j = 0; j < 3; j++) {
      ...
      pn[j] =
          point->pt[polygon->pol[i].v[polygon->pol[i].nverts - 1].index].pos[j];
    }
...
void lwFreeSurface(lwSurface* surf) {
  if (surf) {
    ...
    lwListFree(surf->transparency.val.tex,
               (void(__cdecl*)(void*))lwFreeTexture);
  ...
  while (1) {
    ...
    switch (id) {
      ...
      case ID_FKEY:
        ...
        tex->param.grad.key =
            (lwGradKey*)Mem_ClearedAlloc(nkeys * sizeof(lwGradKey));
...
static int add_texture(lwSurface* surf, lwTexture* tex) {
  ...
  lwListInsert((void**)list, tex,
               (int(__cdecl*)(void*, void*))compare_textures);
  return 1;
}
      ...
      case ID_BLOK:
        ...
        switch (type) {
          ...
          case ID_SHDR:
            ...
            if (!shdr)
              goto Fail;
            lwListInsert((void**)&surf->shader, shdr,
                         (int(__cdecl*)(void*, void*))compare_shaders);
            ++surf->nshaders;
            set_flen(4 + get_flen());
            break;
        }
...
int lwGetPointVMaps(lwPointList* point, lwVMap* vmap) {
  ...
  for (i = 0; i < point->count; i++) {
    if (point->pt[i].nvmaps) {
      point->pt[i].vm =
          (lwVMapPt*)Mem_ClearedAlloc(point->pt[i].nvmaps * sizeof(lwVMapPt));
      if (!point->pt[i].vm)
        return 0;
      point->pt[i].nvmaps = 0;
    }

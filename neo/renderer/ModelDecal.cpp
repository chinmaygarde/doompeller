...
bool idRenderModelDecal::CreateProjectionInfo(decalProjectionInfo_t& info,
  ...
  if (winding.GetNumPoints() != NUM_DECAL_BOUNDING_PLANES - 2) {
    common->Printf(
        "idRenderModelDecal::CreateProjectionInfo: winding must have %d "
        "points\n",
  ...
  // calculate the world space projection volume bounding planes, positive sides
  // face outside the decal
  if (parallel) {
    for (int i = 0; i < winding.GetNumPoints(); i++) {
      idVec3 edge = winding[(i + 1) % winding.GetNumPoints()].ToVec3() -
                    winding[i].ToVec3();
    ...
    for (int i = 0; i < winding.GetNumPoints(); i++) {
      info.boundingPlanes[i].FromPoints(
          projectionOrigin, winding[i].ToVec3(),
  ...
  info.textureAxis[0][3] =
      winding[0].s - (winding[0].ToVec3() * info.textureAxis[0].Normal());
  ...
  info.textureAxis[1][3] =
      winding[0].t - (winding[0].ToVec3() * info.textureAxis[1].Normal());
...
=================
*/
void idRenderModelDecal::GlobalProjectionInfoToLocal(
  ...
  for (int j = 0; j < NUM_DECAL_BOUNDING_PLANES; j++) {
    R_GlobalPlaneToLocal(modelMatrix, info.boundingPlanes[j],
                         localInfo.boundingPlanes[j]);
  }
  R_GlobalPlaneToLocal(modelMatrix, info.fadePlanes[0],
                       localInfo.fadePlanes[0]);
  R_GlobalPlaneToLocal(modelMatrix, info.fadePlanes[1],
                       localInfo.fadePlanes[1]);
  R_GlobalPlaneToLocal(modelMatrix, info.textureAxis[0],
                       localInfo.textureAxis[0]);
  R_GlobalPlaneToLocal(modelMatrix, info.textureAxis[1],
                       localInfo.textureAxis[1]);
  R_GlobalPointToLocal(modelMatrix, info.projectionOrigin,
                       localInfo.projectionOrigin);
...
void idRenderModelDecal::CreateDecal(const idRenderModel* model,
  ...
  for (int surfNum = 0; surfNum < model->NumSurfaces(); surfNum++) {
    ...
    SIMDProcessor->DecalPointCull(cullBits, localInfo.boundingPlanes,
                                  stri->verts, stri->numVerts);

    // find triangles inside the projection volume
    for (int triNum = 0, index = 0; index < stri->numIndexes;
         index += 3, triNum++) {
      ...
      if (stri->facePlanes && stri->facePlanesCalculated &&
          stri->facePlanes[triNum].Normal() *
                  localInfo.boundingPlanes[NUM_DECAL_BOUNDING_PLANES - 2]
                      .Normal() <
              -0.1f) {
        continue;
      }
      ...
      AddDepthFadedWinding(fw, localInfo.material, localInfo.fadePlanes,
                           localInfo.fadeDepth, localInfo.startTime);
    }
  }
}
...
=====================
*/
idRenderModelDecal* idRenderModelDecal::RemoveFadedDecals(
    ...
    if (decals->indexStartTime[i] > minTime) {
      ...
      if (newNumIndexes != i) {
        for (j = 0; j < 3; j++) {
          ...
          decals->indexStartTime[newNumIndexes + j] =
              decals->indexStartTime[i + j];
        }
      }
      newNumIndexes += 3;
    }
...
void idRenderModelDecal::AddDecalDrawSurf(viewEntity_t* space) {
  ...
  for (i = 0; i < tri.numIndexes; i += 3) {
    ...
    for (j = 0; j < 3; j++) {
      ...
      for (int k = 0; k < 4; k++) {
        float fcolor =
            decalInfo.start[k] + (decalInfo.end[k] - decalInfo.start[k]) * f;
  ...
  *newTri = tri;
  ...
  newTri->ambientCache =
      vertexCache.AllocFrameTemp(tri.verts, tri.numVerts * sizeof(idDrawVert));

  // create the drawsurf
  R_AddDrawSurf(newTri, space, &space->entityDef->parms, material,
                space->scissorRect);
}

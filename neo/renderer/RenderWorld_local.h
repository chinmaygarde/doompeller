...
typedef struct portal_s {
  ...
  idWinding*
      w;  // winding points have counter clockwise ordering seen this area
...
typedef struct portalArea_s {
  ...
  int connectedAreaNum
      [NUM_PORTAL_ATTRIBUTES];  // if two areas have matching connectedAreaNum,
...
class idRenderWorldLocal : public idRenderWorld {
 public:
  ...
  virtual void DebugCone(const idVec4& color,
                         ...
                         float radius2,
                         const int lifetime = 0);
  virtual void DebugScreenRect(const idVec4& color,
  ...
  void FloodViewThroughArea_r(const idVec3 origin,
                              int areaNum,
                              const struct portalStack_s* ps);
  void FlowViewThroughPortals(const idVec3 origin,
                              int numPlanes,
                              const idPlane* planes);
  void FloodLightThroughArea_r(idRenderLightLocal* light,
                               int areaNum,
                               const struct portalStack_s* ps);
  void FlowLightThroughPortals(idRenderLightLocal* light);
  areaNumRef_t* FloodFrustumAreas_r(const idFrustum& frustum,
                                    const int areaNum,
                                    const idBounds& bounds,
                                    areaNumRef_t* areas);
  areaNumRef_t* FloodFrustumAreas(const idFrustum& frustum,
                                  areaNumRef_t* areas);
  bool CullEntityByPortals(const idRenderEntityLocal* entity,
                           const struct portalStack_s* ps);
  void AddAreaEntityRefs(int areaNum, const struct portalStack_s* ps);
  bool CullLightByPortals(const idRenderLightLocal* light,
                          const struct portalStack_s* ps);
  ...
  bool AreasAreConnected(int areaNum1,
                         int areaNum2,
  ...
  bool ProcessDemoCommand(idDemoFile* readDemo,
  ...
  void RecurseProcBSP_r(modelTrace_t* results,
                        int parentNodeNum,
                        int nodeNum,
                        float p1f,
                        float p2f,
  ...
  void BoundsInAreas_r(int nodeNum,
                       const idBounds& bounds,
                       int* areas,
                       int* numAreas,
                       int maxAreas) const;
  ...
  void PushVolumeIntoTree_r(idRenderEntityLocal* def,
  ...
  void PushVolumeIntoTree(idRenderEntityLocal* def,
                          idRenderLightLocal* light,
                          int numPoints,
                          const idVec3(*points));

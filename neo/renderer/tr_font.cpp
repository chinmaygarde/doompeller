...
============
*/
void R_GetGlyphInfo(FT_GlyphSlot glyph,
                    int* left,
                    int* right,
                    int* width,
                    int* top,
...
============
*/
glyphInfo_t* RE_ConstructGlyphInfo(unsigned char* imageOut,
                                   int* xOut,
                                   int* yOut,
                                   int* maxHeight,
                                   FT_Face face,
...
bool idRenderSystemLocal::RegisterFont(const char* fontName,
  ...
  */
  ...
  for (fontCount = 0; fontCount < 3; fontCount++) {
    ...
    float glyphScale = 1.0f;  // change the scale to be relative to 1 based on
    ...
    for (i = GLYPH_START; i < GLYPH_END; i++) {
      idStr::snPrintf(name, sizeof(name), "%s/%s", fontName,
  ...
  while (i <= GLYPH_END) {
    ...
    if (xOut == -1 || yOut == -1 || i == GLYPH_END) {
      ...
      idStr::snprintf(name, sizeof(name), "fonts/fontImage_%i_%i.tga",
      ...
      // imageNumber++, pointSize );
      image =
          R_CreateImage(name, imageBuff, 256, 256, qfalse, qfalse, GL_CLAMP);
      h = RE_RegisterShaderFromImage(name, LIGHTMAP_2D, image, qfalse);
      for (j = lastStart; j < i; j++) {
        font.glyphs[j].glyph = h;
        idStr::Copynz(font.glyphs[j].shaderName, name,

...
void idRenderModelManagerLocal::Init() {
  cmdSystem->AddCommand("listModels", ListModels_f, CMD_FL_RENDERER,
                        "lists all models");
  cmdSystem->AddCommand("printModel", PrintModel_f, CMD_FL_RENDERER,
                        "prints model info",
                        idCmdSystem::ArgCompletion_ModelName);
  cmdSystem->AddCommand("reloadModels", ReloadModels_f,
                        CMD_FL_RENDERER | CMD_FL_CHEAT, "reloads models");
  cmdSystem->AddCommand("touchModel", TouchModel_f, CMD_FL_RENDERER,
                        "touches a model",
                        idCmdSystem::ArgCompletion_ModelName);
...
void idRenderModelManagerLocal::ReloadModels(bool forceAll) {
  ...
  for (int i = 1; i < models.Num(); i++) {
    ...
    // we may want to allow world model reloading in the future, but we don't
    // now
    if (!model->IsReloadable()) {
      continue;
    }
...
void idRenderModelManagerLocal::EndLevelLoad() {
  ...
  for (int i = 0; i < models.Num(); i++) {
    ...
    if (!model->IsLevelLoadReferenced() && model->IsLoaded() &&
        model->IsReloadable()) {
...
void idRenderModelManagerLocal::PrintMemInfo(MemInfo_t* mi) {
  ...
  for (i = 0; i < localModelManager.models.Num() - 1; i++) {
    for (j = i + 1; j < localModelManager.models.Num(); j++) {
      if (localModelManager.models[sortIndex[i]]->Memory() <
          localModelManager.models[sortIndex[j]]->Memory()) {

...
typedef struct lightingCache_s {
  idVec3
      localLightVector;  // this is the statically computed vector to the light
...
typedef struct srfTriangles_s {
  ...
  dominantTri_t*
      dominantTris;  // [numVerts] for deformed surface fast tangent calculation
  ...
  int shadowCapPlaneBits;  // bits 0-5 are set when that plane of the
                           // interacting light has triangles projected on it,
                           // which means that if the view is on the outside of
                           // that plane, we need to draw the rear caps of the
                           // shadow volume turboShadows will have
                           // SHADOW_CAP_INFINITE
  ...
  struct srfTriangles_s*
      ambientSurface;  // for light interactions, point back at the original
...
typedef enum { INVALID_JOINT = -1 } jointHandle_t;
...
class idRenderModel {
 public:
  ...
  virtual srfTriangles_t* AllocSurfaceTriangles(int numVerts,
                                                int numIndexes) const = 0;

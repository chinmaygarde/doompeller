...
static const char* parametricParticle_SnapshotName =
...
void idRenderModelPrt::InitFromFile(const char* fileName) {
  name = fileName;
  particleSystem = static_cast<const idDeclParticle*>(
      declManager->FindType(DECL_PARTICLE, fileName));
}
...
void idRenderModelPrt::TouchData(void) {
  ...
  particleSystem = static_cast<const idDeclParticle*>(
      declManager->FindType(DECL_PARTICLE, name));
}
...
idRenderModel* idRenderModelPrt::InstantiateDynamicModel(
    const struct renderEntity_s* renderEntity,
  ...
  */
  ...
  if (cachedModel != NULL) {
    ...
    assert(idStr::Icmp(cachedModel->Name(), parametricParticle_SnapshotName) ==
           0);
    ...
    for (int index = 0; index < stage->totalParticles; index++) {
      ...
      if (renderEntity->shaderParms[SHADERPARM_PARTICLE_STOPTIME] &&
          g.renderView->time - inCycleTime >=
              renderEntity->shaderParms[SHADERPARM_PARTICLE_STOPTIME] * 1000) {

...
=================
*/
static void R_FinishDeform(drawSurf_t* drawSurf,
  ...
  newTri->ambientCache =
      vertexCache.AllocFrameTemp(ac, newTri->numVerts * sizeof(idDrawVert));
...
static void R_AutospriteDeform(drawSurf_t* surf) {
  ...
  R_GlobalVectorToLocal(surf->space->modelMatrix,
                        tr.viewDef->renderView.viewaxis[1], leftDir);
  R_GlobalVectorToLocal(surf->space->modelMatrix,
                        tr.viewDef->renderView.viewaxis[2], upDir);
  ...
  newTri->indexes =
      (glIndex_t*)R_FrameAlloc(newTri->numIndexes * sizeof(newTri->indexes[0]));

  idDrawVert* ac =
      (idDrawVert*)_alloca16(newTri->numVerts * sizeof(idDrawVert));
  ...
  for (i = 0; i < tri->numVerts; i += 4) {
    ...
    mid[0] = 0.25 *
             (v->xyz[0] + (v + 1)->xyz[0] + (v + 2)->xyz[0] + (v + 3)->xyz[0]);
    mid[1] = 0.25 *
             (v->xyz[1] + (v + 1)->xyz[1] + (v + 2)->xyz[1] + (v + 3)->xyz[1]);
    mid[2] = 0.25 *
             (v->xyz[2] + (v + 1)->xyz[2] + (v + 2)->xyz[2] + (v + 3)->xyz[2]);
...
static void R_TubeDeform(drawSurf_t* surf) {
  ...
  // as the view changes
  idVec3 localView;
  R_GlobalPointToLocal(surf->space->modelMatrix, tr.viewDef->renderView.vieworg,
                       localView);

  // this srfTriangles_t and all its indexes and caches are in frame
  // memory, and will be automatically disposed of
  srfTriangles_t* newTri =
      (srfTriangles_t*)R_ClearedFrameAlloc(sizeof(*newTri));
  newTri->numVerts = tri->numVerts;
  newTri->numIndexes = tri->numIndexes;
  newTri->indexes =
      (glIndex_t*)R_FrameAlloc(newTri->numIndexes * sizeof(newTri->indexes[0]));
...
static void R_FlareDeform(drawSurf_t* surf) {
  ...
  float spread = surf->shaderRegisters[surf->material->GetDeformRegister(0)] *
                 r_flareSize.GetFloat();
  idVec3 edgeDir[4][3];
  glIndex_t indexes[MAX_TRI_WINDING_INDEXES];
  ...
  static glIndex_t triIndexes[4 * 3] = {0, 1, 2, 0, 2, 3, 0, 4, 5, 0, 5, 6};
#endif
...
static void R_TurbulentDeform(drawSurf_t* surf) {
  ...
  for (i = 0; i < tri->numVerts; i++) {
    float f = tri->verts[i].xyz[0] * 0.003 + tri->verts[i].xyz[1] * 0.007 +
              tri->verts[i].xyz[2] * 0.011;
...
static void AddTriangleToIsland_r(const srfTriangles_t* tri,
                                  int triangleNum,
                                  bool* usedList,
                                  eyeIsland_t* island) {
  int a, b, c;
  ...
  for (int i = 0; i < numTri; i++) {
    ...
    if (tri->indexes[i * 3 + 0] == a || tri->indexes[i * 3 + 1] == a ||
        tri->indexes[i * 3 + 2] == a || tri->indexes[i * 3 + 0] == b ||
        tri->indexes[i * 3 + 1] == b || tri->indexes[i * 3 + 2] == b ||
        tri->indexes[i * 3 + 0] == c || tri->indexes[i * 3 + 1] == c ||
        tri->indexes[i * 3 + 2] == c) {
...
static void R_EyeballDeform(drawSurf_t* surf) {
  ...
  newTri->numIndexes = tri->numIndexes;
  newTri->indexes =
      (glIndex_t*)R_FrameAlloc(tri->numIndexes * sizeof(newTri->indexes[0]));
  ...
  for (i = 0; i < numIslands; i++) {
    ...
    const idVec3& p1 =
        tri->verts[tri->indexes[islands[originIsland].tris[0] + 0]].xyz;
    const idVec3& p2 =
        tri->verts[tri->indexes[islands[originIsland].tris[0] + 1]].xyz;
    const idVec3& p3 =
        tri->verts[tri->indexes[islands[originIsland].tris[0] + 2]].xyz;
...
static void R_ParticleDeform(drawSurf_t* surf, bool useArea) {
  ...
  const idDeclParticle* particleSystem =
      (idDeclParticle*)surf->material->GetDeformDecl();
  ...
  if (useArea) {
    ...
    for (int i = 0; i < srcTri->numIndexes; i += 3, triNum++) {
      float area;
      area = idWinding::TriangleArea(srcTri->verts[srcTri->indexes[i]].xyz,
  ...
  for (int currentTri = 0; currentTri < ((useArea) ? 1 : numSourceTris);
       currentTri++) {
    for (int stageNum = 0; stageNum < particleSystem->stages.Num();
         stageNum++) {
      ...
      int totalParticles = (useArea)
                               ? stage->totalParticles * totalArea / 4096.0
      ...
      tri->verts =
          (idDrawVert*)R_FrameAlloc(tri->numVerts * sizeof(tri->verts[0]));
      tri->indexes =
          (glIndex_t*)R_FrameAlloc(tri->numIndexes * sizeof(tri->indexes[0]));
      ...
      int stageAge = g.renderView->time +
                     renderEntity->shaderParms[SHADERPARM_TIMEOFFSET] * 1000 -
                     stage->timeOffset * 1000;
      ...
      steppingRandom.SetSeed(
          ((stageCycle << 10) & idRandom::MAX_RAND) ^
          (int)(renderEntity->shaderParms[SHADERPARM_DIVERSITY] *
                idRandom::MAX_RAND));
      steppingRandom2.SetSeed(
          (((stageCycle - 1) << 10) & idRandom::MAX_RAND) ^
          (int)(renderEntity->shaderParms[SHADERPARM_DIVERSITY] *
                idRandom::MAX_RAND));
      ...
      for (int index = 0; index < totalParticles; index++) {
        ...
        int bunchOffset = stage->particleLife * 1000 * stage->spawnBunching *
        ...
        if (useArea) {
          ...
          pointTri =
              idBinSearch_LessEqual<float>(sourceTriAreas, numSourceTris,
                                           g.random.RandomFloat() * totalArea);
        }

        // now pick a random point inside pointTri
        const idDrawVert* v1 =
            &srcTri->verts[srcTri->indexes[pointTri * 3 + 0]];
        const idDrawVert* v2 =
            &srcTri->verts[srcTri->indexes[pointTri * 3 + 1]];
        const idDrawVert* v3 =
            &srcTri->verts[srcTri->indexes[pointTri * 3 + 2]];
        ...
        g.axis[0] =
            v1->tangents[0] * f1 + v2->tangents[0] * f2 + v3->tangents[0] * f3;
        g.axis[1] =
            v1->tangents[1] * f1 + v2->tangents[1] * f2 + v3->tangents[1] * f3;
      ...
      if (tri->numVerts > 0) {
        ...
        tri->ambientCache = vertexCache.AllocFrameTemp(
            tri->verts, tri->numVerts * sizeof(idDrawVert));
        if (tri->ambientCache) {
          // add the drawsurf
          R_AddDrawSurf(tri, surf->space, renderEntity, stage->material,
                        surf->scissorRect);
        }
      }
    }

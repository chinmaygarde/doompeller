    ...
    for (int x = 0; x < width; x++) {
      ...
      for (int yy = -1; yy <= 1; yy++) {
        for (int xx = -1; xx <= 1; xx++) {
          ...
          byte* corner_p =
              in +
              (((y + yy) & (height - 1)) * width + ((x + xx) & width - 1)) * 4;
          idVec3 corner;
...
byte* R_MipMapWithAlphaSpecularity(const byte* in, int width, int height) {
  ...
  if (width < 1 || height < 1 || (width + height == 2)) {
    common->FatalError("R_MipMapWithAlphaMin called with size %i,%i", width,
                       height);
  }
...
byte* R_MipMap3D(const byte* in,
  ...
  if (width < 2 || height < 2 || depth < 2) {
    common->FatalError("R_MipMap3D called with size %i,%i,%i", width, height,
                       depth);
  }
    ...
    for (i = 0; i < height; i++, in_p += row) {
      for (j = 0; j < width; j++, out_p += 4, in_p += 8) {
        out_p[0] = (in_p[0] + in_p[4] + in_p[row + 0] + in_p[row + 4] +
                    in_p[plane + 0] + in_p[plane + 4] + in_p[plane + row + 0] +
                    in_p[plane + row + 4]) >>
                   3;
        out_p[1] = (in_p[1] + in_p[5] + in_p[row + 1] + in_p[row + 5] +
                    in_p[plane + 1] + in_p[plane + 5] + in_p[plane + row + 1] +
                    in_p[plane + row + 5]) >>
                   3;
        out_p[2] = (in_p[2] + in_p[6] + in_p[row + 2] + in_p[row + 6] +
                    in_p[plane + 2] + in_p[plane + 6] + in_p[plane + row + 2] +
                    in_p[plane + row + 6]) >>
                   3;
        out_p[3] = (in_p[3] + in_p[7] + in_p[row + 3] + in_p[row + 7] +
                    in_p[plane + 3] + in_p[plane + 6] + in_p[plane + row + 3] +
                    in_p[plane + row + 6]) >>
                   3;
      }
    }

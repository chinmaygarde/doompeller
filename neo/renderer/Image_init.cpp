...
const char* imageFilter[] = {"GL_LINEAR_MIPMAP_NEAREST",
...
idCVar idImageManager::image_filter(
    ...
    imageFilter,
    idCmdSystem::ArgCompletion_String<imageFilter>);
idCVar idImageManager::image_anisotropy(
    "image_anisotropy",
    "1",
    CVAR_RENDERER | CVAR_ARCHIVE,
    "set the maximum texture anisotropy if available");
idCVar idImageManager::image_lodbias("image_lodbias",
                                     "0",
                                     CVAR_RENDERER | CVAR_ARCHIVE,
                                     "change lod bias on mipmapped images");
idCVar idImageManager::image_downSize("image_downSize",
                                      "0",
                                      CVAR_RENDERER | CVAR_ARCHIVE,
                                      "controls texture downsampling");
idCVar idImageManager::image_forceDownSize("image_forceDownSize",
                                           "0",
                                           CVAR_RENDERER | CVAR_ARCHIVE |
                                               CVAR_BOOL,
                                           "");
...
idCVar idImageManager::image_preload("image_preload",
                                     "1",
...
idCVar idImageManager::image_useNormalCompression(
    ...
    "2 = use rxgb compression for normal maps, 1 = use 256 color compression "
    "for normal maps if available");
...
idCVar idImageManager::image_cacheMegs(
    ...
    "20",
    CVAR_RENDERER | CVAR_ARCHIVE,
    "maximum MB set aside for temporary loading of full-sized precompressed "
    "images");
idCVar idImageManager::image_useCache("image_useCache",
                                      "0",
                                      CVAR_RENDERER | CVAR_ARCHIVE | CVAR_BOOL,
...
idCVar idImageManager::image_downSizeSpecular("image_downSizeSpecular",
                                              "0",
                                              CVAR_RENDERER | CVAR_ARCHIVE,
                                              "controls specular downsampling");
idCVar idImageManager::image_downSizeBump("image_downSizeBump",
                                          "0",
                                          CVAR_RENDERER | CVAR_ARCHIVE,
...
static void R_RampImage(idImage* image) {
  ...
  for (x = 0; x < 256; x++) {
    data[x][0] = data[x][1] = data[x][2] = data[x][3] = x;
  }

  image->GenerateImage((byte*)data, 256, 1, TF_NEAREST, false, TR_CLAMP,
                       TD_HIGH_QUALITY);
}
...
static void R_SpecularTableImage(idImage* image) {
  ...
  image->GenerateImage((byte*)data, 256, 1, TF_LINEAR, false, TR_CLAMP,
                       TD_HIGH_QUALITY);
}
...
static void R_Specular2DTableImage(idImage* image) {
  ...
  for (x = 0; x < 256; x++) {
    ...
    for (y = 0; y < 256; y++) {
      ...
      if (b == 0) {
        // as soon as b equals zero all remaining values in this column are
        // going to be zero we early out to avoid pow() underflows
        break;
      }
  ...
  image->GenerateImage((byte*)data, 256, 256, TF_LINEAR, false, TR_CLAMP,
                       TD_HIGH_QUALITY);
}
...
static void R_AlphaRampImage(idImage* image) {
  ...
  for (x = 0; x < 256; x++) {
    data[x][0] = data[x][1] = data[x][2] = 255;
    data[x][3] = x;
  }
...
void idImage::MakeDefault() {
  ...
  GenerateImage((byte*)data, DEFAULT_SIZE, DEFAULT_SIZE, TF_DEFAULT, true,
...
static void R_WhiteImage(idImage* image) {
  ...
  image->GenerateImage((byte*)data, DEFAULT_SIZE, DEFAULT_SIZE, TF_DEFAULT,
                       false, TR_REPEAT, TD_DEFAULT);
}
...
static void R_BorderClampImage(idImage* image) {
        ...
        data[i][BORDER_CLAMP_SIZE - 1][0] = data[i][BORDER_CLAMP_SIZE - 1][1] =
            data[i][BORDER_CLAMP_SIZE - 1][2] =
                data[i][BORDER_CLAMP_SIZE - 1][3] =
                    ...
                    data[0][i][0] = data[0][i][1] = data[0][i][2] =
                        data[0][i][3] =
  ...
  image->GenerateImage((byte*)data, BORDER_CLAMP_SIZE, BORDER_CLAMP_SIZE,
                       TF_LINEAR /* TF_NEAREST */, false, TR_CLAMP_TO_BORDER,
                       TD_DEFAULT);
...
static void R_AlphaNotchImage(idImage* image) {
  ...
  image->GenerateImage((byte*)data, 2, 1, TF_NEAREST, false, TR_CLAMP,
                       TD_HIGH_QUALITY);
}
...
static void R_FlatNormalImage(idImage* image) {
  ...
  int red =
      (globalImages->image_useNormalCompression.GetInteger() == 1) ? 0 : 3;
  int alpha = (red == 0) ? 3 : 0;
...
static void R_AmbientNormalImage(idImage* image) {
  ...
  // this must be a cube map for fragment programs to simply substitute for the
...
static void CreateSquareLight(void) {
  ...
  for (x = 0; x < 128; x++) {
    ...
    for (y = 0; y < 128; y++) {
      ...
      buffer[(y * 128 + x) * 4 + 0] = buffer[(y * 128 + x) * 4 + 1] =
...
static void CreateFlashOff(void) {
  ...
  for (x = 0; x < width; x++) {
    for (y = 0; y < height; y++) {
      d = 255 - (x * 256 / width);
      buffer[(y * width + x) * 4 + 0] = buffer[(y * width + x) * 4 + 1] =
...
static void getCubeVector(int i, int cubesize, int x, int y, float* vector) {
  ...
  mag = idMath::InvSqrt(vector[0] * vector[0] + vector[1] * vector[1] +
                        vector[2] * vector[2]);
  vector[0] *= mag;
  vector[1] *= mag;
  vector[2] *= mag;
...
static void makeNormalizeVectorCubeMap(idImage* image) {
  ...
  image->GenerateCubeImage((const byte**)pixels, size, TF_LINEAR, false,
                           TD_HIGH_QUALITY);
...
static void R_CreateNoFalloffImage(idImage* image) {
  ...
  image->GenerateImage((byte*)data, FALLOFF_TEXTURE_SIZE, 16, TF_DEFAULT, false,
                       TR_CLAMP_TO_ZERO, TD_HIGH_QUALITY);
}
...
void R_FogImage(idImage* image) {
  ...
  image->GenerateImage((byte*)data, FOG_SIZE, FOG_SIZE, TF_LINEAR, false,
...
static float FogFraction(float viewHeight, float targetHeight) {
  ...
  float ramp = (1.0 - (rampTop * rampSlope + rampBottom * rampSlope) * -0.5) *
               (rampTop - rampBottom);
...
void R_FogEnterImage(idImage* image) {
  ...
  image->GenerateImage((byte*)data, FOG_ENTER_SIZE, FOG_ENTER_SIZE, TF_LINEAR,
...
void idImageManager::ChangeTextureFilter(void) {
  ...
  for (i = 0; i < images.Num(); i++) {
    ...
    if (glt->filter == TF_DEFAULT) {
      qglTexParameterf(texEnum, GL_TEXTURE_MIN_FILTER,
                       globalImages->textureMinFilter);
      qglTexParameterf(texEnum, GL_TEXTURE_MAG_FILTER,
                       globalImages->textureMaxFilter);
    }
    if (glConfig.anisotropicAvailable) {
      qglTexParameterf(texEnum, GL_TEXTURE_MAX_ANISOTROPY_EXT,
                       globalImages->textureAnisotropy);
    }
    if (glConfig.textureLODBiasAvailable) {
      qglTexParameterf(texEnum, GL_TEXTURE_LOD_BIAS_EXT,
                       globalImages->textureLODBias);
    }
  }
}
...
void R_ReloadImages_f(const idCmdArgs& args) {
  ...
  all = false;
  checkPrecompressed = false;  // if we are doing this as a vid_restart, look
                               // for precompressed like normal
...
void R_ListImages_f(const idCmdArgs& args) {
  ...
  if (failed) {
    common->Printf(
        "usage: listImages [ sorted | partial | unloaded | cached | uncached | "
        "tagged | duplicated | touched | classify | showOverSized ]\n");
    return;
  }
  ...
  sortedImage_t* sortedArray = (sortedImage_t*)alloca(
      sizeof(sortedImage_t) * globalImages->images.Num());
  ...
  for (i = 0; i < globalImages->images.Num(); i++) {
    ...
    if (cached && (!image->partialImage ||
                   image->texnum == idImage::TEXTURE_NOT_LOADED)) {
      continue;
    }
    if (uncached && (!image->partialImage ||
                     image->texnum != idImage::TEXTURE_NOT_LOADED)) {
      continue;
    }
    ...
    if (duplicated) {
      ...
      for (j = i + 1; j < globalImages->images.Num(); j++) {
        if (idStr::Icmp(image->imgName, globalImages->images[j]->imgName) ==
            0) {
          break;
        }
      }
    ...
    // "listimages touched" will list only images bound since the last
    // "listimages touched" call
    if (touched) {
  ...
  if (byClassification) {
    ...
    for (i = 0; i < IC_COUNT; i++) {
      ...
      for (j = 0; j < classifications[i].Num(); j++) {
        ...
        if (overSized) {
          if (sortedArray[classifications[i][j]].image->uploadWidth >
                  IC_Info[i].maxWidth &&
              sortedArray[classifications[i][j]].image->uploadHeight >
                  IC_Info[i].maxHeight) {
      ...
      common->Printf(
          ...
          IC_Info[i].desc, classifications[i].Num(),
          partialSize / (1024 * 1024.0));
...
void idImageManager::SetNormalPalette(void) {
  ...
  temptable[255 * 3 + 0] = temptable[255 * 3 + 1] = temptable[255 * 3 + 2] =
      128;
...
==================
*/
idImage* idImageManager::ImageFromFunction(
    const char* _name,
...
==============
*/
idImage* idImageManager::ImageFromFile(const char* _name,
  ...
  if (!_name || !_name[0] || idStr::Icmp(_name, "default") == 0 ||
      idStr::Icmp(_name, "_default") == 0) {
    declManager->MediaPrint("DEFAULTED\n");
    return globalImages->defaultImage;
  }

  // strip any .tga file extensions from anywhere in the _name, including image
  // program parameters
  name = _name;
  name.Replace(".tga", "");
  name.BackSlashesToSlashes();
  ...
  for (image = imageHashTable[hash]; image; image = image->hashNext) {
    if (name.Icmp(image->imgName) == 0) {
      ...
      // the same image is being requested, but with a different allowDownSize
      // or depth so pick the highest of the two and reload the old image with
      // those parameters
      ...
      if (image_preload.GetBool() && !insideLevelLoad) {
        ...
        image->ActuallyLoadImage(
            true, false);  // check for precompressed, load is from front end
  ...
  // HACK: to allow keep fonts from being mip'd, as new ones will be introduced
  // with localization this keeps us from having to make a material for each
  // font tga
  ...
  // also create a shrunken version if we are going to dynamically cache the
  // full size image
  if (image->ShouldImageBePartialCached()) {
    // if we only loaded part of the file, create a new idImage for the shrunken
    // version
    ...
    // we don't bother hooking this into the hash table for lookup, but we do
    // add it to the manager list for listImages
  ...
  if (image_preload.GetBool() && !insideLevelLoad) {
    ...
    image->ActuallyLoadImage(
        true, false);  // check for precompressed, load is from front end
    declManager->MediaPrint("%ix%i %s\n", image->uploadWidth,
                            image->uploadHeight, image->imgName.c_str());
  } else {
    declManager->MediaPrint("%s\n", image->imgName.c_str());
  }
...
void R_CombineCubeImages_f(const idCmdArgs& args) {
  if (args.Argc() != 2) {
    ...
    common->Printf(
        " combines basename[1-6][0001-9999].tga to "
        "basenameCM[0001-9999].tga\n");
  ...
  for (int frameNum = 1; frameNum < 10000; frameNum++) {
    ...
    for (side = 0; side < 6; side++) {
      sprintf(filename, "%s%i%04i.tga", baseName.c_str(), orderRemap[side],
              frameNum);
    ...
    for (side = 0; side < 6; side++) {
      memcpy(combined + width * height * 4 * side, pics[side],
             width * height * 4);
      Mem_Free(pics[side]);
    }
...
void idImage::StartBackgroundImageLoad() {
  if (imageManager.numActiveBackgroundImageLoads >=
      idImageManager::MAX_BACKGROUND_IMAGE_LOADS) {
    return;
  }
  ...
  if (!precompressedFile) {
    common->Warning(
        "idImageManager::StartBackgroundImageLoad: %s wasn't a precompressed "
        "file",
        imgName.c_str());
    return;
  }
  ...
  if (!bgl.f) {
    common->Warning(
        "idImageManager::StartBackgroundImageLoad: Couldn't load %s",
        imgName.c_str());
    return;
  }
  bgl.file.position = 0;
  ...
  int totalSize = 0;
  for (idImage* check = globalImages->cacheLRU.cacheUsageNext;
       check != &globalImages->cacheLRU; check = check->cacheUsageNext) {
    totalSize += check->StorageSize();
  }
  int needed = this->StorageSize();

  while ((totalSize + needed) >
         globalImages->image_cacheMegs.GetFloat() * 1024 * 1024) {
...
void idImageManager::CompleteBackgroundImageLoads() {
  ...
  for (idImage* image = backgroundImageLoads; image; image = next) {
    ...
    if (image->bgl.completed) {
      ...
      // upload the image
      image->UploadPrecompressedImage((byte*)image->bgl.file.buffer,
                                      image->bgl.file.length);
...
void idImageManager::Init() {
  ...
  specularTableImage =
      ImageFromFunction("_specularTable", R_SpecularTableImage);
  specular2DTableImage =
      ImageFromFunction("_specular2DTable", R_Specular2DTableImage);
  ...
  normalCubeMapImage =
      ImageFromFunction("_normalCubeMap", makeNormalizeVectorCubeMap);
  ...
  scratchCubeMapImage =
      ImageFromFunction("_scratchCubeMap", makeNormalizeVectorCubeMap);
  currentRenderImage = ImageFromFunction("_currentRender", R_RGBA8Image);

  cmdSystem->AddCommand("reloadImages", R_ReloadImages_f, CMD_FL_RENDERER,
                        "reloads images");
  cmdSystem->AddCommand("listImages", R_ListImages_f, CMD_FL_RENDERER,
                        "lists images");
  cmdSystem->AddCommand("combineCubeImages", R_CombineCubeImages_f,
                        CMD_FL_RENDERER,
...
void idImageManager::FinishBuild(bool removeDups) {
  ...
  batchFile =
      fileSystem->OpenFileWrite((removeDups) ? "makedds2.bat" : "makedds.bat");
  if (batchFile) {
    int i;
    ...
    for (i = 0; i < ddsNum; i++) {
      ...
      batchFile->Printf(
          ...
          ddsNum, ((float)(i + 1) / (float)ddsNum) * 100.f);
    }
...
void idImageManager::PrintMemInfo(MemInfo_t* mi) {
  ...
  for (i = 0; i < images.Num() - 1; i++) {
    for (j = i + 1; j < images.Num(); j++) {
      if (images[sortIndex[i]]->StorageSize() <
          images[sortIndex[j]]->StorageSize()) {
  ...
  for (i = 0; i < images.Num(); i++) {
    ...
    f->Printf("%s %3i %s\n", idStr::FormatNumber(size).c_str(), im->refCount,
              im->imgName.c_str());
  }

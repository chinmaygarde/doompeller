...
void idImage::UploadCompressedNormalMap(int width,
  ...
  for (i = 0; i < height; i++, out += row, in += width * 4) {
    for (j = 0; j < width; j++) {
      ...
      if (x == 128 && y == 128 && z == 128) {
        ...
        c = (globalImages->originalToCompressed[x] << 4) |
            globalImages->originalToCompressed[y];
        if (c == 255) {
  ...
  if (mipLevel == 0) {
    ...
    if (globalImages->image_writeNormalTGAPalletized.GetBool()) {
      ...
      if (ext) {
        strcpy(ext, "_pal.tga");
        R_WritePalTGA(filename, normals, globalImages->compressedPalette, width,
                      height);
      }
    }
  }
  ...
  if (glConfig.sharedTexturePaletteAvailable) {
    qglTexImage2D(GL_TEXTURE_2D, mipLevel, GL_COLOR_INDEX8_EXT, width, height,
                  0, GL_COLOR_INDEX, GL_UNSIGNED_BYTE, normals);
  }
}
  ...
  if (minimumDepth == TD_BUMP) {
    if (globalImages->image_useCompression.GetBool() &&
      ...
      return GL_COLOR_INDEX8_EXT;
               ...
               globalImages->image_useNormalCompression.GetInteger() &&
               glConfig.textureCompressionAvailable) {
      // image_useNormalCompression == 2 uses rxgb format which produces really
      // good quality for medium settings
...
void idImage::SetImageFilterAndRepeat() const {
  ...
  if (glConfig.textureLODBiasAvailable) {
    qglTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_LOD_BIAS_EXT,
...
void idImage::GetDownsize(int& scaled_width, int& scaled_height) const {
  ...
  if (depth == TD_SPECULAR &&
      globalImages->image_downSizeSpecular.GetInteger()) {
    size = globalImages->image_downSizeSpecularLimit.GetInteger();
    if (size == 0) {
      size = 64;
    }
  } else if (depth == TD_BUMP &&
             globalImages->image_downSizeBump.GetInteger()) {
    size = globalImages->image_downSizeBumpLimit.GetInteger();
    if (size == 0) {
      size = 64;
    }
  } else if ((allowDownSize || globalImages->image_forceDownSize.GetBool()) &&
             globalImages->image_downSize.GetInteger()) {
...
void idImage::GenerateImage(const byte* pic,
  ...
  internalFormat =
      SelectInternalFormat(&pic, 1, width, height, depth, &isMonochrome);
  ...
  if ((scaled_width == width) && (scaled_height == height)) {
    ...
    scaledBuffer =
        (byte*)R_StaticAlloc(sizeof(unsigned) * scaled_width * scaled_height);
  ...
  if (generatorFunction == NULL &&
      (depth == TD_BUMP && globalImages->image_writeNormalTGA.GetBool() ||
  ...
  while (scaled_width > 1 || scaled_height > 1) {
    ...
    // upload the mip map
    if (internalFormat == GL_COLOR_INDEX8_EXT) {
      UploadCompressedNormalMap(scaled_width, scaled_height, scaledBuffer,
                                miplevel);
    } else {
      qglTexImage2D(GL_TEXTURE_2D, miplevel, internalFormat, scaled_width,
                    scaled_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, scaledBuffer);
    }
  }
...
void idImage::Generate3DImage(const byte* pic,
  ...
  internalFormat = SelectInternalFormat(&pic, 1, width, height * picDepth,
                                        minDepthParm, &isMonochrome);
  ...
  qglTexImage3D(GL_TEXTURE_3D, 0, internalFormat, scaled_width, scaled_height,
                scaled_depth, 0, GL_RGBA, GL_UNSIGNED_BYTE, pic);
  ...
  scaledBuffer =
      (byte*)R_StaticAlloc(scaled_width * scaled_height * scaled_depth * 4);
  ...
  while (scaled_width > 1 || scaled_height > 1 || scaled_depth > 1) {
    ...
    // upload the mip map
    qglTexImage3D(GL_TEXTURE_3D, miplevel, internalFormat, scaled_width,
                  scaled_height, scaled_depth, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                  scaledBuffer);
  }
  R_StaticFree(scaledBuffer);
  ...
  switch (filter) {
    case TF_DEFAULT:
      qglTexParameterf(GL_TEXTURE_3D, GL_TEXTURE_MIN_FILTER,
                       globalImages->textureMinFilter);
      qglTexParameterf(GL_TEXTURE_3D, GL_TEXTURE_MAG_FILTER,
                       globalImages->textureMaxFilter);
      break;
    case TF_LINEAR:
...
void idImage::GenerateCubeImage(const byte* pic[6],
  ...
  // select proper internal format before we resample
  internalFormat =
      SelectInternalFormat(pic, 6, width, height, depth, &isMonochrome);
  ...
  qglTexParameteri(GL_TEXTURE_CUBE_MAP_EXT, GL_TEXTURE_WRAP_S,
                   GL_CLAMP_TO_EDGE);
  qglTexParameteri(GL_TEXTURE_CUBE_MAP_EXT, GL_TEXTURE_WRAP_T,
                   GL_CLAMP_TO_EDGE);

  // set the minimize / maximize filtering
  switch (filter) {
    case TF_DEFAULT:
      qglTexParameterf(GL_TEXTURE_CUBE_MAP_EXT, GL_TEXTURE_MIN_FILTER,
                       globalImages->textureMinFilter);
      qglTexParameterf(GL_TEXTURE_CUBE_MAP_EXT, GL_TEXTURE_MAG_FILTER,
                       globalImages->textureMaxFilter);
      break;
    case TF_LINEAR:
      qglTexParameterf(GL_TEXTURE_CUBE_MAP_EXT, GL_TEXTURE_MIN_FILTER,
  ...
  for (i = 0; i < 6; i++) {
    qglTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X_EXT + i, 0, internalFormat,
                  scaled_width, scaled_height, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                  pic[i]);
  }
  ...
  while (scaled_width > 1) {
    for (i = 0; i < 6; i++) {
      ...
      qglTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X_EXT + i, miplevel,
                    internalFormat, scaled_width / 2, scaled_height / 2, 0,
    ...
    if (*s == '/' || *s == '\\' || *s == '(') {
      ...
      *f = '_';
      f++;
               ...
               *(f - 1) == '/') {         // ignore a space right after a slash
...
void idImage::WritePrecompressedImage() {
  ...
  if (!com_makingBuild.GetBool()) {
    if (!globalImages->image_writePrecompressedTextures.GetBool() ||
        !globalImages->image_usePrecompressedTextures.GetBool()) {
      return;
    }
  }
  ...
  switch (internalFormat) {
    case GL_COLOR_INDEX8_EXT:
    case GL_COLOR_INDEX:
      // this will not work with dds viewers but we need it in this format to
      // save disk load speed ( i.e. size )
  ...
  if (globalImages->image_useOffLineCompression.GetBool() &&
      FormatIsDXT(altInternalFormat)) {
    ...
    globalImages->AddDDSCommand(
        va("z:/d3xp/compressonator/thecompressonator -convert \"%s\" \"%s\" %s "
           "-mipmaps\n",
  ...
  if (FormatIsDXT(altInternalFormat)) {
    ...
    header.dwPitchOrLinearSize =
        ((uploadWidth + 3) / 4) * ((uploadHeight + 3) / 4) *
  ...
  if (FormatIsDXT(altInternalFormat)) {
    ...
    header.ddspf.dwFlags = (internalFormat == GL_COLOR_INDEX8_EXT)
                               ? DDSF_RGB | DDSF_ID_INDEXCOLOR
                               : DDSF_RGB;
  ...
  if (f == NULL) {
    common->Warning("Could not open %s trying to write precompressed image",
                    filename);
    return;
  }
  ...
  for (int level = 0; level < numLevels; level++) {
    ...
    if (FormatIsDXT(altInternalFormat)) {
      ...
      qglGetTexImage(GL_TEXTURE_2D, level, altInternalFormat, GL_UNSIGNED_BYTE,
                     data);
    }
...
bool idImage::CheckPrecompressedImage(bool fullLoad) {
...
#if 1  // ( _D3XP had disabled ) - Allow grabbing of DDS's from original Doom
       // pak files if we are doing a copyFiles, make sure the original images
       // are referenced
  if (fileSystem->PerformingCopyFiles()) {
    return false;
  }
  ...
  if (depth == TD_BUMP &&
      globalImages->image_useNormalCompression.GetInteger() != 2) {
    return false;
  }

  // god i love last minute hacks :-)
  if (com_machineSpec.GetInteger() >= 1 && com_videoRam.GetInteger() >= 128 &&
      imgName.Icmpn("lights/", 7) == 0) {
    return false;
  }
...
void idImage::UploadPrecompressedImage(byte* data, int len) {
  ...
  } else if ((header->ddspf.dwFlags & DDSF_RGBA) &&
             header->ddspf.dwRGBBitCount == 32) {
    externalFormat = GL_BGRA_EXT;
    internalFormat = GL_RGBA8;
  } else if ((header->ddspf.dwFlags & DDSF_RGB) &&
             header->ddspf.dwRGBBitCount == 32) {
    externalFormat = GL_BGRA_EXT;
    internalFormat = GL_RGBA8;
  } else if ((header->ddspf.dwFlags & DDSF_RGB) &&
  ...
  type = TT_2D;  // FIXME: we may want to support pre-compressed cube maps in
                 // the future
  ...
  for (int i = 0; i < numMipmaps; i++) {
    ...
    if (uw > uploadWidth || uh > uploadHeight) {
      ...
      if (FormatIsDXT(internalFormat)) {
        qglCompressedTexImage2DARB(GL_TEXTURE_2D, i - skipMip, internalFormat,
                                   uw, uh, 0, size, imagedata);
      } else {
        qglTexImage2D(GL_TEXTURE_2D, i - skipMip, internalFormat, uw, uh, 0,
                      externalFormat, GL_UNSIGNED_BYTE, imagedata);
      }
    }
...
void idImage::PurgeImage() {
  if (texnum != TEXTURE_NOT_LOADED) {
    ...
    if (qglDeleteTextures) {
      qglDeleteTextures(
          1, &texnum);  // this should be the ONLY place it is ever called!
    }
...
void idImage::Bind() {
  ...
  if (tmu->textureType != type &&
      (backEnd.glState.currenttmu < glConfig.maxTextureUnits)) {
...
void idImage::CopyFramebuffer(int x,
  ...
  if ((useOversizedBuffer &&
    ...
    if (potWidth == imageWidth && potHeight == imageHeight) {
      qglCopyTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, x, y, imageWidth,
                        imageHeight, 0);
    } else {
      byte* junk;
...
#if 0  // Disabling because it's unnecessary and introduces a green strip on
       // edge of _currentRender
			for ( int i = 0 ; i < potWidth * potHeight * 4 ; i+=4 ) {
				junk[i+1] = 255;
			}
#endif
      qglTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, potWidth, potHeight, 0, GL_RGBA,
                    GL_UNSIGNED_BYTE, junk);
      Mem_Free(junk);

      qglCopyTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, x, y, imageWidth,
                           imageHeight);
    }
  } else {
  ...
  if (imageWidth != potWidth) {
    qglCopyTexSubImage2D(GL_TEXTURE_2D, 0, imageWidth, 0, x + imageWidth - 1, y,
                         1, imageHeight);
  }
  if (imageHeight != potHeight) {
    qglCopyTexSubImage2D(GL_TEXTURE_2D, 0, 0, imageHeight, x,
                         y + imageHeight - 1, imageWidth, 1);
  }
...
void idImage::CopyDepthbuffer(int x, int y, int imageWidth, int imageHeight) {
  ...
  if (uploadWidth != potWidth || uploadHeight != potHeight) {
    ...
    uploadHeight = potHeight;
    if (potWidth == imageWidth && potHeight == imageHeight) {
      qglCopyTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT, x, y, imageWidth,
                        imageHeight, 0);
    } else {
      // we need to create a dummy image with power of two dimensions,
      // then do a qglCopyTexSubImage2D of the data we want
      qglTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT, potWidth, potHeight,
...
void idImage::UploadScratch(const byte* data, int cols, int rows) {
  ...
  if (rows == cols * 6) {
    ...
    if (cols != uploadWidth || rows != uploadHeight) {
      ...
      // upload the base level
      for (i = 0; i < 6; i++) {
        qglTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X_EXT + i, 0, GL_RGB8, cols,
                      rows, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                      data + cols * rows * 4 * i);
      }
    } else {
      ...
      for (i = 0; i < 6; i++) {
        qglTexSubImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X_EXT + i, 0, 0, 0, cols,
                         rows, GL_RGBA, GL_UNSIGNED_BYTE,
                         data + cols * rows * 4 * i);
      }
    }
    qglTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);

...
void idGuiModel::EmitSurface(guiModelSurface_t* surf,
  ...
  memcpy(tri->indexes, &indexes[surf->firstIndex],
  ...
  viewEntity_t* guiSpace =
      (viewEntity_t*)R_ClearedFrameAlloc(sizeof(*guiSpace));
  memcpy(guiSpace->modelMatrix, modelMatrix, sizeof(guiSpace->modelMatrix));
  memcpy(guiSpace->modelViewMatrix, modelViewMatrix,
         sizeof(guiSpace->modelViewMatrix));
  guiSpace->weaponDepthHack = depthHack;

  // add the surface, which might recursively create another gui
  R_AddDrawSurf(tri, guiSpace, &renderEntity, surf->material,
                tr.viewDef->scissor);
}
...
void idGuiModel::EmitFullScreen(void) {
  ...
  if (!tr.viewDef || !tr.viewDef->isEditor) {
    ...
    viewDef->viewport.x2 =
        tr.viewDef->renderView.x + tr.viewDef->renderView.width;
    viewDef->viewport.y1 = tr.viewDef->renderView.y;
    viewDef->viewport.y2 =
        tr.viewDef->renderView.y + tr.viewDef->renderView.height;
  ...
  viewDef->drawSurfs = (drawSurf_t**)R_FrameAlloc(
      viewDef->maxDrawSurfs * sizeof(viewDef->drawSurfs[0]));
  ...
  for (int i = 0; i < surfaces.Num(); i++) {
    EmitSurface(&surfaces[i], viewDef->worldSpace.modelMatrix,
                viewDef->worldSpace.modelViewMatrix, false);
  }
...
void idGuiModel::SetColor(float r, float g, float b, float a) {
  ...
  if (r == surf->color[0] && g == surf->color[1] && b == surf->color[2] &&
      a == surf->color[3]) {
    return;  // no change
  }
...
=============
*/
void idGuiModel::DrawStretchPic(const idDrawVert* dverts,
                                const glIndex_t* dindexes,
                                int vertCount,
                                int indexCount,
  ...
  if (hShader != surf->material) {
    ...
    const_cast<idMaterial*>(hShader)
        ->EnsureNotPurged();  // in case it was a gui item started before a
                              // level change
  ...
  if (clip) {
    ...
    for (i = 0; i < indexCount; i += 3) {
      w.Clear();
      w.AddPoint(idVec5(dverts[dindexes[i]].xyz.x, dverts[dindexes[i]].xyz.y,
                        dverts[dindexes[i]].xyz.z, dverts[dindexes[i]].st.x,
                        dverts[dindexes[i]].st.y));
      w.AddPoint(
          idVec5(dverts[dindexes[i + 1]].xyz.x, dverts[dindexes[i + 1]].xyz.y,
                 dverts[dindexes[i + 1]].xyz.z, dverts[dindexes[i + 1]].st.x,
                 dverts[dindexes[i + 1]].st.y));
      w.AddPoint(
          idVec5(dverts[dindexes[i + 2]].xyz.x, dverts[dindexes[i + 2]].xyz.y,
                 dverts[dindexes[i + 2]].xyz.z, dverts[dindexes[i + 2]].st.x,
                 dverts[dindexes[i + 2]].st.y));

      for (j = 0; j < 3; j++) {
        if (w[j].x < min_x || w[j].x > max_x || w[j].y < min_y ||
            w[j].y > max_y) {
          break;
        }
      }
...
void idGuiModel::DrawStretchPic(float x,
  ...
  DrawStretchPic(&verts[0], &indexes[0], 4, 6, hShader, false, 0.0f, 0.0f,
                 640.0f, 480.0f);
}

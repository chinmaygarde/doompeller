...
srfTriangles_t* R_CreateVertexProgramTurboShadowVolume(
  ...
  if (cullInfo.cullBits == LIGHT_CULL_ALL_FRONT ||
    ...
    // make all triangles that are outside the light frustum "facing", so they
    // won't cast shadows
    indexes = tri->indexes;
  ...
  // shadowVerts will be NULL on these surfaces, so the shadowVerts will be
  // taken from the ambient surface
  ...
  R_AllocStaticTriSurfIndexes(newTri,
                              (numShadowingFaces + tri->numSilEdges) * 6);
  glIndex_t* tempIndexes = newTri->indexes;
  glIndex_t* shadowIndexes = newTri->indexes;
#else
  glIndex_t* tempIndexes =
      (glIndex_t*)_alloca16(tri->numSilEdges * 6 * sizeof(tempIndexes[0]));
  ...
  newTri->numIndexes = newTri->numShadowIndexesNoFrontCaps =
      numShadowIndexes + numShadowingFaces * 6;
  ...
  SIMDProcessor->Memcpy(newTri->indexes, tempIndexes,
                        numShadowIndexes * sizeof(tempIndexes[0]));
#endif
...
srfTriangles_t* R_CreateTurboShadowVolume(const idRenderEntityLocal* ent,
...
#else
  shadowCache_t* shadowVerts =
      (shadowCache_t*)_alloca16(tri->numVerts * 2 * sizeof(shadowVerts[0]));
#endif
  ...
  newTri->numVerts = SIMDProcessor->CreateShadowCache(
      &shadowVerts->xyz, vertRemap, localLightOrigin, tri->verts,
      tri->numVerts);
  ...
  SIMDProcessor->Memcpy(newTri->shadowVertexes, shadowVerts,

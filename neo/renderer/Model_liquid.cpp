...
modelSurface_t idRenderModelLiquid::GenerateSurface(float lerp) {
  ...
  SIMDProcessor->Memcpy(tri->verts, verts.Ptr(),
                        deformInfo->numSourceVerts * sizeof(tri->verts[0]));
  ...
  for (cy = top; cy < bottom; cy++) {
    for (cx = left; cx < right; cx++) {
      ...
      if (square < radsquare) {
        ...
        page[verts_x * (cy + y) + cx + x] +=
            idMath::Cos16(dist * idMath::PI * 0.5f) * drop_height;
      }
    }
  }
}
    ...
    case 0:
      for (y = 1; y < verts_y - 1; y++) {
        ...
        for (x = 1; x < verts_x - 1; x++) {
          value = (p2[x + verts_x] + p2[x - verts_x] + p2[x + 1] + p2[x - 1] +
                   p2[x - verts_x - 1] + p2[x - verts_x + 1] +
                   p2[x + verts_x - 1] + p2[x + verts_x + 1] + p2[x]) *
                      (2.0f / 9.0f) -
                  p1[x];
...
void idRenderModelLiquid::InitFromFile(const char* fileName) {
  ...
  while (parser.ReadToken(&token)) {
    if (!token.Icmp("seed")) {
      ...
      if ((rate <= 0.0f) || (rate > 60.0f)) {
        parser.Warning(
            "Invalid update_rate.  Must be between 0 and 60.  Using default "
            "model.");
        MakeDefaultModel();
        return;
      }
      ...
      parser.Warning("Unknown parameter '%s'.  Using default model.",
                     token.c_str());
      MakeDefaultModel();
      return;
    }
  ...
  for (i = 0, y = 0; y < verts_y; y++) {
    for (x = 0; x < verts_x; x++, i++) {
      ...
      verts[i].st.Set((float)x / (float)(verts_x - 1),
                      (float)-y / (float)(verts_y - 1));
    }
  }
  ...
  deformInfo =
      R_BuildDeformInfo(verts.Num(), verts.Ptr(), tris.Num(), tris.Ptr(), true);

  bounds.Clear();
  bounds.AddPoint(idVec3(0.0f, 0.0f, drop_height * -10.0f));
  bounds.AddPoint(idVec3((verts_x - 1) * scale_x, (verts_y - 1) * scale_y,
                         drop_height * 10.0f));
...
====================
*/
idRenderModel* idRenderModelLiquid::InstantiateDynamicModel(

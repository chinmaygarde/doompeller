...
idRenderModel* idRenderWorldLocal::ParseShadowModel(idLexer* src) {
  ...
  // we do NOT do a model->FinishSurfaceces, because we don't need sil edges,
  // planes, tangents, etc.
  //	model->FinishSurfaces();
...
void idRenderWorldLocal::ParseInterAreaPortals(idLexer* src) {
  ...
  portalAreas = (portalArea_t*)R_ClearedStaticAlloc(numPortalAreas *
                                                    sizeof(portalAreas[0]));
  areaScreenRect = (idScreenRect*)R_ClearedStaticAlloc(numPortalAreas *
                                                       sizeof(idScreenRect));
  ...
  doublePortals = (doublePortal_t*)R_ClearedStaticAlloc(
      numInterAreaPortals * sizeof(doublePortals[0]));
...
void idRenderWorldLocal::ParseNodes(idLexer* src) {
  ...
  areaNodes =
      (areaNode_t*)R_ClearedStaticAlloc(numAreaNodes * sizeof(areaNodes[0]));
...
bool idRenderWorldLocal::InitFromMap(const char* name) {
  ...
  while (1) {
    ...
    src->Error("idRenderWorldLocal::InitFromMap: bad token \"%s\"",
               token.c_str());
  }
  ...
  // find the points where we can early-our of reference pushing into the BSP
  // tree
...
void idRenderWorldLocal::AddWorldModelEntities() {
  ...
  for (i = 0; i < numPortalAreas; i++) {
    ...
    if (def->parms.hModel->IsDefaultModel() ||
        !def->parms.hModel->IsStaticWorldModel()) {
    ...
    def->parms.shaderParms[0] = def->parms.shaderParms[1] =
        def->parms.shaderParms[2] = def->parms.shaderParms[3] = 1;
...
bool idRenderWorldLocal::CheckAreaForPortalSky(int areaNum) {
  ...
  for (ref = portalAreas[areaNum].entityRefs.areaNext; ref->entity;
       ref = ref->areaNext) {

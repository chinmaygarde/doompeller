...
void idRenderWorldLocal::WriteVisibleDefs(const viewDef_t* viewDef) {
  ...
  for (viewEntity_t* viewEnt = viewDef->viewEntitys; viewEnt;
  ...
  for (viewLight_t* viewLight = viewDef->viewLights; viewLight;
...
void idRenderWorldLocal::ReadRenderLight() {
  ...
  if (light.prelightModel) {
    light.prelightModel =
        renderModelManager->FindModel(session->readDemo->ReadHashString());
  }
  if (light.shader) {
    light.shader =
        declManager->FindMaterial(session->readDemo->ReadHashString());
  }
  if (light.referenceSound) {
    int index;
    session->readDemo->ReadInt(index);
  ...
  */
  ...
  if (r_showDemo.GetBool()) {
    common->Printf("write DC_UPDATE_ENTITYDEF: %i = %s\n", handle,
                   ent->hModel ? ent->hModel->Name() : "NULL");
  }
}
...
void idRenderWorldLocal::ReadRenderEntity() {
  ...
  if (ent.numJoints) {
    ent.joints =
        (idJointMat*)Mem_Alloc16(ent.numJoints * sizeof(ent.joints[0]));
    for (int i = 0; i < ent.numJoints; i++) {
      float* data = ent.joints[i].ToFloatPtr();
      for (int j = 0; j < 12; ++j)

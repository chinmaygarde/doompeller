...
void GL_SelectTexture(int unit) {
  ...
  if (unit < 0 || unit >= glConfig.maxTextureUnits &&
                      unit >= glConfig.maxTextureImageUnits) {
...
void GL_State(int stateBits) {
  ...
  //
  if (diff &
      (GLS_DEPTHFUNC_EQUAL | GLS_DEPTHFUNC_LESS | GLS_DEPTHFUNC_ALWAYS)) {
...
static void RB_SetBuffer(const void* data) {
  ...
  if (r_clear.GetFloat() || idStr::Length(r_clear.GetString()) != 1 ||
      r_lockSurfaces.GetBool() || r_singleArea.GetBool() ||
      r_showOverDraw.GetBool()) {
    float c[3];
...
void RB_ExecuteBackEndCommands(const emptyCommand_t* cmds) {
  ...
  int c_draw3d = 0, c_draw2d = 0, c_setBuffers = 0, c_swapBuffers = 0,
      c_copyRenders = 0;
  ...
  if (r_debugRenderToTexture.GetInteger() == 1) {
    common->Printf(
        "3d: %i, 2d: %i, SetBuf: %i, SwpBuf: %i, CpyRenders: %i, CpyFrameBuf: "
        "%i\n",
        c_draw3d, c_draw2d, c_setBuffers, c_swapBuffers, c_copyRenders,

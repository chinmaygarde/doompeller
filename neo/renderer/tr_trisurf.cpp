...
static idDynamicBlockAlloc<shadowCache_t, 1 << 18, 1 << 10>
...
static idDynamicBlockAlloc<dominantTri_t, 1 << 16, 1 << 10>
...
void R_ShowTriSurfMemory_f(const idCmdArgs& args) {
  ...
  common->Printf(
      ...
      triVertexAllocator.GetBaseBlockMemory() >> 10,
      triVertexAllocator.GetFreeBlockMemory() >> 10,
      triVertexAllocator.GetNumFreeBlocks(),
      triVertexAllocator.GetNumEmptyBaseBlocks());

  common->Printf(
      "%6d kB index memory (%d kB free in %d blocks, %d empty base blocks)\n",
      triIndexAllocator.GetBaseBlockMemory() >> 10,
      triIndexAllocator.GetFreeBlockMemory() >> 10,
      triIndexAllocator.GetNumFreeBlocks(),
      triIndexAllocator.GetNumEmptyBaseBlocks());

  common->Printf(
      "%6d kB shadow vert memory (%d kB free in %d blocks, %d empty base "
      "blocks)\n",
      triShadowVertexAllocator.GetBaseBlockMemory() >> 10,
      triShadowVertexAllocator.GetFreeBlockMemory() >> 10,
      triShadowVertexAllocator.GetNumFreeBlocks(),
      triShadowVertexAllocator.GetNumEmptyBaseBlocks());

  common->Printf(
      "%6d kB tri plane memory (%d kB free in %d blocks, %d empty base "
      "blocks)\n",
      triPlaneAllocator.GetBaseBlockMemory() >> 10,
      triPlaneAllocator.GetFreeBlockMemory() >> 10,
      triPlaneAllocator.GetNumFreeBlocks(),
      triPlaneAllocator.GetNumEmptyBaseBlocks());

  common->Printf(
      "%6d kB sil index memory (%d kB free in %d blocks, %d empty base "
      "blocks)\n",
      triSilIndexAllocator.GetBaseBlockMemory() >> 10,
      triSilIndexAllocator.GetFreeBlockMemory() >> 10,
      triSilIndexAllocator.GetNumFreeBlocks(),
      triSilIndexAllocator.GetNumEmptyBaseBlocks());

  common->Printf(
      "%6d kB sil edge memory (%d kB free in %d blocks, %d empty base "
      "blocks)\n",
      triSilEdgeAllocator.GetBaseBlockMemory() >> 10,
      triSilEdgeAllocator.GetFreeBlockMemory() >> 10,
      triSilEdgeAllocator.GetNumFreeBlocks(),
      triSilEdgeAllocator.GetNumEmptyBaseBlocks());

  common->Printf(
      "%6d kB dominant tri memory (%d kB free in %d blocks, %d empty base "
      "blocks)\n",
      triDominantTrisAllocator.GetBaseBlockMemory() >> 10,
      triDominantTrisAllocator.GetFreeBlockMemory() >> 10,
      triDominantTrisAllocator.GetNumFreeBlocks(),
      triDominantTrisAllocator.GetNumEmptyBaseBlocks());

  common->Printf(
      "%6d kB mirror vert memory (%d kB free in %d blocks, %d empty base "
      "blocks)\n",
      triMirroredVertAllocator.GetBaseBlockMemory() >> 10,
      triMirroredVertAllocator.GetFreeBlockMemory() >> 10,
      triMirroredVertAllocator.GetNumFreeBlocks(),
      triMirroredVertAllocator.GetNumEmptyBaseBlocks());

  common->Printf(
      "%6d kB dup vert memory (%d kB free in %d blocks, %d empty base "
      "blocks)\n",
      triDupVertAllocator.GetBaseBlockMemory() >> 10,
      triDupVertAllocator.GetFreeBlockMemory() >> 10,
      triDupVertAllocator.GetNumFreeBlocks(),
      triDupVertAllocator.GetNumEmptyBaseBlocks());
...
void R_CheckStaticTriSurfMemory(const srfTriangles_t* tri) {
  ...
  if (tri->shadowVertexes != NULL) {
    const char* error =
        triShadowVertexAllocator.CheckMemory(tri->shadowVertexes);
    assert(error == NULL);
  }
}
...
srfTriangles_t* R_CopyStaticTriSurf(const srfTriangles_t* tri) {
  ...
  memcpy(newTri->indexes, tri->indexes,
         tri->numIndexes * sizeof(newTri->indexes[0]));
...
#ifdef USE_TRI_DATA_ALLOCATOR
  tri->shadowVertexes =
      triShadowVertexAllocator.Resize(tri->shadowVertexes, numVerts);
#else
  assert(false);
#endif
}
...
void R_BoundTriSurf(srfTriangles_t* tri) {
  SIMDProcessor->MinMax(tri->bounds[0], tri->bounds[1], tri->verts,
                        tri->numVerts);
}
...
static int* R_CreateSilRemap(const srfTriangles_t* tri) {
  ...
  for (i = 0; i < tri->numVerts; i++) {
    ...
    for (j = hash.First(hashKey); j >= 0; j = hash.Next(j)) {
      v2 = &tri->verts[j];
      if (v2->xyz[0] == v1->xyz[0] && v2->xyz[1] == v1->xyz[1] &&
          v2->xyz[2] == v1->xyz[2]) {
        c_removed++;
        remap[i] = j;
        break;
...
void R_CreateDupVerts(srfTriangles_t* tri) {
  ...
  int* tempDupVerts =
      (int*)_alloca16(tri->numVerts * 2 * sizeof(tempDupVerts[0]));
  tri->numDupVerts = 0;
  for (i = 0; i < tri->numVerts; i++) {
    if (remap[i] != i) {
  ...
  memcpy(tri->dupVerts, tempDupVerts,
         tri->numDupVerts * 2 * sizeof(tri->dupVerts[0]));
}
...
void R_DeriveFacePlanes(srfTriangles_t* tri) {
  ...
  SIMDProcessor->DeriveTriPlanes(planes, tri->verts, tri->numVerts,
                                 tri->indexes, tri->numIndexes);
...
static void R_DefineEdge(int v1, int v2, int planeNum) {
  ...
  for (i = silEdgeHash.First(hashKey); i >= 0 && i < MAX_SIL_EDGES;
...
void R_IdentifySilEdges(srfTriangles_t* tri, bool omitCoplanarEdges) {
  ...
  if (c_duplicatedEdges || c_tripledEdges) {
    common->DWarning("%i duplicated edge directions, %i tripled edges",
                     c_duplicatedEdges, c_tripledEdges);
  }
  ...
  if (omitCoplanarEdges) {
    for (i = 0; i < numSilEdges; i++) {
      ...
      plane.FromPoints(tri->verts[i1].xyz, tri->verts[i2].xyz,
                       tri->verts[i3].xyz);
      ...
      if (j == 3) {
        ...
        memmove(&silEdges[i], &silEdges[i + 1],
                (numSilEdges - i - 1) * sizeof(silEdges[i]));
        c_coplanarCulled++;
        numSilEdges--;
        i--;
      }
...
static void R_DeriveFaceTangents(const srfTriangles_t* tri,
...
void R_DeriveTangentsWithoutNormals(srfTriangles_t* tri) {
  ...
  faceTangents =
      (faceTangents_t*)_alloca16(sizeof(faceTangents[0]) * tri->numIndexes / 3);
...
void R_BuildDominantTris(srfTriangles_t* tri) {
  ...
  dominantTri_t* dt;
  indexSort_t* ind =
      (indexSort_t*)R_StaticAlloc(tri->numIndexes * sizeof(*ind));
  ...
  for (i = 0; i < tri->numIndexes; i += j) {
    ...
    for (j = 0; i + j < tri->numIndexes && ind[i + j].vertexNum == vertNum;
         j++) {
      float d0[5], d1[5];
      idDrawVert *a, *b, *c;
      ...
      dt[vertNum].normalizationScale[0] =
          (area > 0 ? 1 : -1) / len;  // tangents[0]
...
#else
      dt[vertNum].normalizationScale[1] =
          (area > 0 ? 1 : -1) / len;  // tangents[1]
#endif
    }
  }
...
void R_DeriveUnsmoothedTangents(srfTriangles_t* tri) {
  ...
  SIMDProcessor->DeriveUnsmoothedTangents(tri->verts, tri->dominantTris,
                                          tri->numVerts);
  ...
  for (int i = 0; i < tri->numVerts; i++) {
    ...
    a->tangents[0][0] =
        dt->normalizationScale[0] * (d0[0] * d1[4] - d0[4] * d1[0]);
    a->tangents[0][1] =
        dt->normalizationScale[0] * (d0[1] * d1[4] - d0[4] * d1[1]);
    a->tangents[0][2] =
        dt->normalizationScale[0] * (d0[2] * d1[4] - d0[4] * d1[2]);

#ifdef DERIVE_UNSMOOTHED_BITANGENT
    // derive the bitangent for a completely orthogonal axis,
    // instead of using the texture T vector
    a->tangents[1][0] =
        dt->normalizationScale[1] *
        (a->normal[2] * a->tangents[0][1] - a->normal[1] * a->tangents[0][2]);
    a->tangents[1][1] =
        dt->normalizationScale[1] *
        (a->normal[0] * a->tangents[0][2] - a->normal[2] * a->tangents[0][0]);
    a->tangents[1][2] =
        dt->normalizationScale[1] *
        (a->normal[1] * a->tangents[0][0] - a->normal[0] * a->tangents[0][1]);
#else
    // calculate the bitangent from the texture T vector
    a->tangents[1][0] =
        dt->normalizationScale[1] * (d0[3] * d1[0] - d0[0] * d1[3]);
    a->tangents[1][1] =
        dt->normalizationScale[1] * (d0[3] * d1[1] - d0[1] * d1[3]);
    a->tangents[1][2] =
        dt->normalizationScale[1] * (d0[3] * d1[2] - d0[2] * d1[3]);
#endif
  }
...
void R_DeriveTangents(srfTriangles_t* tri, bool allocFacePlanes) {
  ...
  SIMDProcessor->DeriveTangents(planes, tri->verts, tri->numVerts, tri->indexes,
                                tri->numIndexes);
  ...
  for (i = 0; i < tri->numIndexes; i += 3) {
    for (r = 0; r < 3; r++) {
      ...
      for (j = i + 3; j < tri->numIndexes; j += 3) {
        if (tri->silIndexes[j] == a && tri->silIndexes[j + 1] == b &&
            tri->silIndexes[j + 2] == c) {
          c_removed++;
          memmove(tri->indexes + j, tri->indexes + j + 3,
                  (tri->numIndexes - j - 3) * sizeof(tri->indexes[0]));
          memmove(tri->silIndexes + j, tri->silIndexes + j + 3,
                  (tri->numIndexes - j - 3) * sizeof(tri->silIndexes[0]));
          tri->numIndexes -= 3;
          j -= 3;
        }
      }
...
void R_RemoveDegenerateTriangles(srfTriangles_t* tri) {
  ...
  for (i = 0; i < tri->numIndexes; i += 3) {
    ...
    if (a == b || a == c || b == c) {
      c_removed++;
      memmove(tri->indexes + i, tri->indexes + i + 3,
              (tri->numIndexes - i - 3) * sizeof(tri->indexes[0]));
      if (tri->silIndexes) {
        memmove(tri->silIndexes + i, tri->silIndexes + i + 3,
...
srfTriangles_t* R_MergeSurfaceList(const srfTriangles_t** surfaces,
  ...
  for (i = 0; i < numSurfaces; i++) {
    tri = surfaces[i];
    memcpy(newTri->verts + totalVerts, tri->verts,
           tri->numVerts * sizeof(*tri->verts));
...
void R_CleanupTriangles(srfTriangles_t* tri,
  ...
  if (identifySilEdges) {
    R_IdentifySilEdges(
        tri, true);  // assume it is non-deformable, and omit coplanar edges
  }
...
deformInfo_t* R_BuildDeformInfo(int numVerts,
  ...
  R_DuplicateMirroredVertexes(
      &tri);  // split mirror points into multiple points
...
int R_DeformInfoMemoryUsed(deformInfo_t* deformInfo) {
  ...
  if (deformInfo->mirroredVerts != NULL) {
    total +=
        deformInfo->numMirroredVerts * sizeof(deformInfo->mirroredVerts[0]);
  }
  if (deformInfo->dupVerts != NULL) {

...
static bool
    callOptimizer;  // call the preprocessor optimizer after clipping occluders
...
===================
*/
static void R_ProjectPointsToFarPlane(const idRenderEntityLocal* ent,
...
===================
*/
static bool R_ClipLineToLight(const idVec3& a,
                              const idVec3& b,
                              const idPlane frustum[4],
                              idVec3& p1,
                              idVec3& p2) {
  float* clip;
  int j;
...
static void R_AddSilEdges(const srfTriangles_t* tri,
  ...
  for (i = 0; i < tri->numSilEdges; i++) {
    sil = tri->silEdges + i;
    if (sil->p1 < 0 || sil->p1 > numPlanes || sil->p2 < 0 ||
        sil->p2 > numPlanes) {
...
================
*/
static void R_CalcPointCull(const srfTriangles_t* tri,
                            const idPlane frustum[6],
                            unsigned short* pointCull) {
  int i;
  int frontBits;
  ...
  for (i = 0; i < 6; i++) {
    ...
    SIMDProcessor->CmpLT(side1, i, planeSide, LIGHT_CLIP_EPSILON,
                         tri->numVerts);
    SIMDProcessor->CmpGT(side2, i, planeSide, -LIGHT_CLIP_EPSILON,
                         tri->numVerts);
  }
  for (i = 0; i < tri->numVerts; i++) {
...
static void R_CreateShadowVolumeInFrustum(const idRenderEntityLocal* ent,
  ...
  for (i = 0; i < numTris; i++) {
    ...
    if (TRIANGLE_CLIPPED(i1, i2, i3)) {
      cullBits = ((pointCull[i1] ^ 0xfc0) | (pointCull[i2] ^ 0xfc0) |
                  (pointCull[i3] ^ 0xfc0)) >>
                 6;
  ...
  if (callOptimizer) {
    ...
    opt = SuperOptimizeOccluders(shadowVerts, shadowIndexes + firstShadowIndex,
                                 numCapIndexes, farPlane, lightOrigin);
  ...
  // project all of the vertexes to the shadow plane, generating
  // an equal number of back vertexes
  R_ProjectPointsToFarPlane(ent, light, farPlane, firstShadowVert,
                            numShadowVerts);
    ...
    for (int side = 0; side < 6; side++) {
      ...
      for (int edge = 0; edge < 4; edge++) {
        ...
        if (centerOutside) {
          ...
          light->shadowFrustums[light->numShadowFrustums].makeClippedPlanes =
              true;
        }
      }
      light->numShadowFrustums++;
...
srfTriangles_t* R_CreateShadowVolume(const idRenderEntityLocal* ent,
  ...
  if (tri->numIndexes < 0) {
    common->Error("R_CreateShadowVolume: tri->numIndexes = %i",
                  tri->numIndexes);
  }
  ...
  faceCastsShadow = (byte*)_alloca16(tri->numIndexes / 3 +
                                     1);  // + 1 for fake dangling edge face
  ...
  for (int frustumNum = 0; frustumNum < light->numShadowFrustums;
       frustumNum++) {
    ...
    R_CreateShadowVolumeInFrustum(ent, tri, light, lightOrigin, frustum,
                                  frustum[5], frust->makeClippedPlanes);
  ...
  if (numShadowVerts > MAX_SHADOW_VERTS ||
      numShadowIndexes > MAX_SHADOW_INDEXES) {
  ...
  SIMDProcessor->Memcpy(newTri->shadowVertexes, shadowVerts,
                        newTri->numVerts * sizeof(newTri->shadowVertexes[0]));
  ...
  if (1 /* sortCapIndexes */) {
    ...
    for (i = 0; i < indexFrustumNumber; i++) {
      ...
      SIMDProcessor->Memcpy(newTri->indexes + newTri->numShadowIndexesNoCaps,
                            shadowIndexes + indexRef[i].silStart,
                            c * sizeof(newTri->indexes[0]));
    ...
    for (i = 0; i < indexFrustumNumber; i++) {
      ...
      SIMDProcessor->Memcpy(
          newTri->indexes + newTri->numShadowIndexesNoFrontCaps,
          shadowIndexes + indexRef[i].rearCapStart,
          c * sizeof(newTri->indexes[0]));
    ...
    for (i = 0; i < indexFrustumNumber; i++) {
      ...
      SIMDProcessor->Memcpy(newTri->indexes + newTri->numIndexes,
                            shadowIndexes + indexRef[i].frontCapStart,
                            c * sizeof(newTri->indexes[0]));
      newTri->numIndexes += c;
    }
    ...
    SIMDProcessor->Memcpy(newTri->indexes, shadowIndexes,

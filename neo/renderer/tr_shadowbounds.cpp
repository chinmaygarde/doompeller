...
struct polyhedron {
  ...
  void recompute_planes() {
    // for each polygon
    for (unsigned int i = 0; i < p.size(); i++) {
      p[i].plane = compute_homogeneous_plane(v[p[i].vi[0]], v[p[i].vi[1]],
                                             v[p[i].vi[2]]);
    }
  }
...
idMat4 make_idMat4(const float* m) {
  return idMat4(m[0], m[4], m[8], m[12], m[1], m[5], m[9], m[13], m[2], m[6],
...
void draw_polyhedron(const viewDef_t* viewDef,
                     const polyhedron& p,
                     idVec4 color) {
  for (unsigned int i = 0; i < p.e.size(); i++) {
    viewDef->renderWorld->DebugLine(color, v4to3(p.v[p.e[i].vi[0]]),
                                    v4to3(p.v[p.e[i].vi[1]]));
  }
}

void draw_segments(const viewDef_t* viewDef,
                   const MySegments& s,
                   idVec4 color) {
...
void world_to_hclip(const viewDef_t* viewDef,
                    const idVec4& global,
                    idVec4& clip) {
  int i;
  idVec4 view;
...
idScreenRect R_CalcIntersectionScissor(const idRenderLightLocal* lightDef,
  ...
  // ); viewDef->renderWorld->DebugBox( colorBlue, idBox( model->Bounds(),
  // entityDef->parms.origin, entityDef->parms.axis ) );
  ...
  idVec4 lightpos =
      idVec4(lightDef->globalLightOrigin.x, lightDef->globalLightOrigin.y,
             lightDef->globalLightOrigin.z, 1.0f);
  ...
  // limit the bounds to avoid an inside out scissor rectangle due to floating
  // point to short conversion

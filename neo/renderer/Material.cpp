...
idImage* idMaterial::GetEditorImage(void) const {
  ...
  if (!editorImageName.Length()) {
    ...
    editorImage = globalImages->ImageFromFile(editorImageName, TF_DEFAULT, true,
...
static infoParm_t infoParms[] = {
    ...
    {"moveableclip", 0, 0,
     CONTENTS_MOVEABLECLIP},                 // solid to moveable entities
    ...
    {"aasobstacle", 0, 0,
     CONTENTS_AAS_OBSTACLE},  // used to compile an obstacle into AAS that can
                              // be enabled/disabled
    {"flashlight_trigger", 0, 0,
     CONTENTS_FLASHLIGHT_TRIGGER},  // used for triggers that are activated by
                                    // the flashlight
    {"nonsolid", 1, 0, 0},          // clears the solid flag
    {"nullNormal", 0, SURF_NULLNORMAL,
     0},  // renderbump will draw as 0x80 0x80 0x80

    // utility relevant attributes
    {"areaportal", 1, 0, CONTENTS_AREAPORTAL},  // divides areas
    {"qer_nocarve", 1, 0, CONTENTS_NOCSG},      // don't cut brushes in editor

    {"discrete", 1, SURF_DISCRETE,
     0},  // surfaces should not be automatically merged together or
    ...
    {"ricochet", 0, SURFTYPE_RICOCHET,
     0},  // behaves like metal but causes a ricochet sound
...
expOp_t* idMaterial::GetExpressionOp(void) {
  if (numOps == MAX_EXPRESSION_OPS) {
    common->Warning("GetExpressionOp: material '%s' hit MAX_EXPRESSION_OPS",
                    GetName());
    SetMaterialFlag(MF_DEFAULTED);
...
int idMaterial::EmitOp(int a, int b, expOpType_t opType) {
  ...
  if (opType == OP_TYPE_ADD) {
    ...
    if (!pd->registerIsTemporary[a] && !pd->registerIsTemporary[b]) {
      return GetExpressionConstant(pd->shaderRegisters[a] +
                                   pd->shaderRegisters[b]);
    }
  }
...
void idMaterial::ClearStage(shaderStage_t* ss) {
  ...
  ss->color.registers[0] = ss->color.registers[1] = ss->color.registers[2] =
...
int idMaterial::NameToSrcBlendMode(const idStr& name) {
  ...
  common->Warning("unknown blend mode '%s' in material '%s'", name.c_str(),
                  GetName());
  SetMaterialFlag(MF_DEFAULTED);
...
void idMaterial::ParseStage(idLexer& src, const textureRepeat_t trpDefault) {
  ...
  if (numStages >= MAX_SHADER_STAGES) {
    SetMaterialFlag(MF_DEFAULTED);
    common->Warning("material '%s' exceeded %i stages", GetName(),
                    MAX_SHADER_STAGES);
  }
  ...
  while (1) {
    ...
    if (!token.Icmp("texGen")) {
      ...
      if (!token.Icmp("normal")) {
        ...
        common->Warning("bad texGen '%s' in material %s", token.c_str(),
                        GetName());
        SetMaterialFlag(MF_DEFAULTED);
      }
      continue;
    ...
    if (!token.Icmp("centerScale")) {
      ...
      matrix[0][1] = GetExpressionConstant(0);
      matrix[0][2] =
          EmitOp(GetExpressionConstant(0.5),
                 EmitOp(GetExpressionConstant(0.5), a, OP_TYPE_MULTIPLY),
                 OP_TYPE_SUBTRACT);
      matrix[1][0] = GetExpressionConstant(0);
      matrix[1][1] = b;
      matrix[1][2] =
          EmitOp(GetExpressionConstant(0.5),
                 EmitOp(GetExpressionConstant(0.5), b, OP_TYPE_MULTIPLY),
                 OP_TYPE_SUBTRACT);
    ...
    if (!token.Icmp("rotate")) {
      ...
      table = static_cast<const idDeclTable*>(
          declManager->FindType(DECL_TABLE, "sinTable", false));
      if (!table) {
      ...
      table = static_cast<const idDeclTable*>(
          declManager->FindType(DECL_TABLE, "cosTable", false));
      if (!table) {
      ...
      matrix[0][2] = EmitOp(
          EmitOp(EmitOp(GetExpressionConstant(-0.5), cosReg, OP_TYPE_MULTIPLY),
      ...
      matrix[1][1] = cosReg;
      matrix[1][2] = EmitOp(
          EmitOp(EmitOp(GetExpressionConstant(-0.5), sinReg, OP_TYPE_MULTIPLY),
    ...
    if (!token.Icmp("program")) {
      if (src.ReadTokenOnLine(&token)) {
        newStage.vertexProgram =
            R_FindARBProgram(GL_VERTEX_PROGRAM_ARB, token.c_str());
        newStage.fragmentProgram =
            R_FindARBProgram(GL_FRAGMENT_PROGRAM_ARB, token.c_str());
      }
      continue;
    }
    ...
    if (!token.Icmp("megaTexture")) {
      if (src.ReadTokenOnLine(&token)) {
        ...
        newStage.vertexProgram =
            R_FindARBProgram(GL_VERTEX_PROGRAM_ARB, "megaTexture.vfp");
        newStage.fragmentProgram =
            R_FindARBProgram(GL_FRAGMENT_PROGRAM_ARB, "megaTexture.vfp");
        continue;
      }
    }
    ...
    common->Warning("unknown token '%s' in material '%s'", token.c_str(),
                    GetName());
    SetMaterialFlag(MF_DEFAULTED);
    return;
  }
...
==============
*/
void idMaterial::AddImplicitStages(
  ...
  if (!hasBump) {
    ...
    newSrc.SetFlags(LEXFL_NOFATALERRORS | LEXFL_NOSTRINGCONCAT |
...
void idMaterial::ParseMaterial(idLexer& src) {
  ...
  numOps = 0;
  numRegisters =
      EXP_REG_NUM_PREDEFINED;  // leave space for the parms to be copied in
  for (i = 0; i < numRegisters; i++) {
    pd->registerIsTemporary[i] =
        true;  // they aren't constants that can be folded
  }
  ...
  while (1) {
    ...
    // light volumes
      ...
      lightFalloffImage = globalImages->ImageFromFile(
          copy, TF_DEFAULT, false, TR_CLAMP /* TR_CLAMP_TO_ZERO */, TD_DEFAULT);
      continue;
    }
    ...
    // DECAL_MACRO for backwards compatibility with the preprocessor macros
      ...
      common->Warning("unknown general material parameter '%s' in '%s'",
                      token.c_str(), GetName());
      SetMaterialFlag(MF_DEFAULTED);
      return;
    }
  ...
  for (i = 0; i < numStages; i++) {
    if (pd->parseStages[i].texture.texgen != TG_EXPLICIT) {
      if (firstGen == TG_EXPLICIT) {
        ...
        common->Warning("material '%s' has multiple stages with a texgen",
                        GetName());
        break;
      }
    }
...
bool idMaterial::Parse(const char* text, const int textLength) {
  ...
  if (coverage == MC_BAD) {
    ...
    if (!numStages) {
      ...
      coverage = MC_OPAQUE;
    } else if ((pd->parseStages[0].drawStateBits & GLS_DSTBLEND_BITS) !=
                   GLS_DSTBLEND_ZERO ||
               (pd->parseStages[0].drawStateBits & GLS_SRCBLEND_BITS) ==
  ...
  for (i = 0; i < numStages; i++) {
    ...
    if (pStage->newStage) {
      for (int j = 0; j < pStage->newStage->numFragmentProgramImages; j++) {
        if (pStage->newStage->fragmentProgramImages[j] ==
  ...
  */
  ...
  if (numRegisters) {
    expressionRegisters =
        (float*)R_StaticAlloc(numRegisters * sizeof(expressionRegisters[0]));
    memcpy(expressionRegisters, pd->shaderRegisters,
           numRegisters * sizeof(expressionRegisters[0]));
  }
...
===================
*/
char* opNames[] = {"OP_TYPE_ADD",    "OP_TYPE_SUBTRACT", "OP_TYPE_MULTIPLY",
                   "OP_TYPE_DIVIDE", "OP_TYPE_MOD",      "OP_TYPE_TABLE",
                   "OP_TYPE_GT",     "OP_TYPE_GE",       "OP_TYPE_LT",
                   "OP_TYPE_LE",     "OP_TYPE_EQ",       "OP_TYPE_NE",
                   "OP_TYPE_AND",    "OP_TYPE_OR"};
...
void idMaterial::Print() const {
  ...
  for (i = 0; i < numOps; i++) {
    ...
    if (op->opType == OP_TYPE_TABLE) {
      common->Printf("%i = %s[ %i ]\n", op->c,
                     declManager->DeclByIndex(DECL_TABLE, op->a)->GetName(),
                     op->b);
    } else {
      common->Printf("%i = %i %s %i\n", op->c, op->a, opNames[op->opType],
                     op->b);
    }
  }
}
...
void idMaterial::CheckForConstantRegisters() {
  ...
  constantRegisters =
      (float*)R_ClearedStaticAlloc(GetNumRegisters() * sizeof(float));

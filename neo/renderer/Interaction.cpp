...
void R_CalcInteractionFacing(const idRenderEntityLocal* ent,
  ...
  cullInfo.facing =
      (byte*)R_StaticAlloc((numFaces + 1) * sizeof(cullInfo.facing[0]));
...
void R_CalcInteractionCullBits(const idRenderEntityLocal* ent,
  ...
  for (i = 0; i < 6; i++) {
    R_GlobalPlaneToLocal(ent->modelMatrix, -light->frustum[i],
                         cullInfo.localClipPlanes[i]);

    // get front bits for the whole surface
    if (tri->bounds.PlaneDistance(cullInfo.localClipPlanes[i]) >=
        LIGHT_CLIP_EPSILON) {
      frontBits |= 1 << i;
    }
  }
  ...
  cullInfo.cullBits =
      (byte*)R_StaticAlloc(tri->numVerts * sizeof(cullInfo.cullBits[0]));
  SIMDProcessor->Memset(cullInfo.cullBits, 0,
                        tri->numVerts * sizeof(cullInfo.cullBits[0]));
  ...
  for (i = 0; i < 6; i++) {
    ...
    SIMDProcessor->Dot(planeSide, cullInfo.localClipPlanes[i], tri->verts,
                       tri->numVerts);
    SIMDProcessor->CmpLT(cullInfo.cullBits, i, planeSide, LIGHT_CLIP_EPSILON,
                         tri->numVerts);
  }
}
...
=============
*/
static int R_ChopWinding(clipTri_t clipTris[2],
                         int inNum,
                         const idPlane plane) {
  clipTri_t *in, *out;
...
===================
*/
static bool R_ClipTriangleToLight(const idVec3& a,
                                  const idVec3& b,
                                  const idVec3& c,
                                  int planeBits,
...
static srfTriangles_t* R_CreateLightTris(const idRenderEntityLocal* ent,
  ...
  if (cullInfo.cullBits == LIGHT_CULL_ALL_FRONT) {
    ...
    if (includeBackFaces) {
      ...
      SIMDProcessor->MinMax(bounds[0], bounds[1], tri->verts, indexes,
                            numIndexes);
...
void idInteraction::UnlinkAndFree(void) {
  ...
  if (renderWorld->interactionTable) {
    int index = this->lightDef->index * renderWorld->interactionTableWidth +
                this->entityDef->index;
...
idScreenRect idInteraction::CalcInteractionScissorRectangle(
  ...
  if (r_useInteractionScissors.GetInteger() > 1) {
    ...
    if (frustumState == idInteraction::FRUSTUM_VALID) {
      ...
      for (areaReference_t* ref = entityDef->entityRefs; ref;
           ref = ref->ownerNext) {
      ...
      frustumAreas =
          tr.viewDef->renderWorld->FloodFrustumAreas(frustum, frustumAreas);
  ...
  if (lightDef->parms.pointLight) {
    ...
    viewFrustum.ClippedProjectionBounds(
        frustum, idBox(lightDef->frustumTris->bounds), projectionBounds);
  }
...
bool idInteraction::CullInteractionByViewFrustum(const idFrustum& viewFrustum) {
  ...
  if (frustumState == idInteraction::FRUSTUM_UNINITIALIZED) {
    frustum.FromProjection(
        idBox(entityDef->referenceBounds, entityDef->parms.origin,
              entityDef->parms.axis),
        lightDef->globalLightOrigin, MAX_WORLD_SIZE);
    ...
    if (lightDef->parms.pointLight) {
      frustum.ConstrainToBox(idBox(lightDef->parms.origin,
  ...
  if (r_showInteractionFrustums.GetInteger()) {
    static idVec4 colors[] = {colorRed,    colorGreen,   colorBlue,
                              colorYellow, colorMagenta, colorCyan,
                              colorWhite,  colorPurple};
    tr.viewDef->renderWorld->DebugFrustum(
        colors[lightDef->index & 7], frustum,
        (r_showInteractionFrustums.GetInteger() > 1));
    if (r_showInteractionFrustums.GetInteger() > 2) {
      tr.viewDef->renderWorld->DebugBox(
          colorWhite, idBox(entityDef->referenceBounds, entityDef->parms.origin,
                            entityDef->parms.axis));
    }
  }
...
void idInteraction::CreateInteraction(const idRenderModel* model) {
  ...
  surfaces = (surfaceInteraction_t*)R_ClearedStaticAlloc(sizeof(*surfaces) *
                                                         numSurfaces);
  ...
  for (int c = 0; c < model->NumSurfaces(); c++) {
    ...
    shader = surf->shader;
    shader = R_RemapShaderBySkin(shader, entityDef->parms.customSkin,
    ...
    if (R_CullLocalBox(tri->bounds, entityDef->modelMatrix, 6,
                       lightDef->frustum)) {
      continue;
    }
    ...
    if (HasShadows() && shader->SurfaceCastsShadow() && tri->silEdges != NULL) {
      ...
      if (lightDef->parms.prelightModel == NULL ||
          !model->IsStaticWorldModel() || !r_useOptimizedShadows.GetBool()) {
        // this is the only place during gameplay (outside the utilities) that
        // R_CreateShadowVolume() is called
        sint->shadowTris = R_CreateShadowVolume(entityDef, tri, lightDef,
                                                shadowGen, sint->cullInfo);
        if (sint->shadowTris) {
          if (shader->Coverage() != MC_OPAQUE ||
              (!r_skipSuppress.GetBool() &&
            ...
            sint->shadowTris->numShadowIndexesNoCaps =
                sint->shadowTris->numIndexes;
            sint->shadowTris->numShadowIndexesNoFrontCaps =
                sint->shadowTris->numIndexes;
          }
        }
        interactionGenerated = true;
...
void idInteraction::AddActiveInteraction(void) {
  ...
  if (!IsDeferred() &&
      entityDef->dynamicModelFrameCount != dynamicModelFrameCount) {
    FreeSurfaces();
  }
  ...
  R_GlobalPointToLocal(vEntity->modelMatrix, lightDef->globalLightOrigin,
                       localLightOrigin);
  R_GlobalPointToLocal(vEntity->modelMatrix, tr.viewDef->renderView.vieworg,
                       localViewOrigin);
  ...
  for (int i = 0; i < numSurfaces; i++) {
    ...
    // if empty
    if (!lightScissorsEmpty && sint->ambientTris &&
        sint->ambientTris->ambientViewCount == tr.viewCount) {
      // make sure we have created this interaction, which may have been
      // deferred on a previous use that only needed the shadow
      if (sint->lightTris == LIGHT_TRIS_DEFERRED) {
        sint->lightTris =
            R_CreateLightTris(vEntity->entityDef, sint->ambientTris,
                              vLight->lightDef, sint->shader, sint->cullInfo);
        R_FreeInteractionCullInfo(sint->cullInfo);
      }
      ...
      if (lightTris) {
        ...
        // more
        if (!R_CullLocalBox(lightTris->bounds, vEntity->modelMatrix, 5,
                            tr.viewDef->frustum)) {
          ...
          } else if (!lightDef->parms.noShadows &&
                     sint->shader->TestMaterialFlag(MF_NOSELFSHADOW)) {
            R_LinkLightSurf(&vLight->localInteractions, lightTris, vEntity,
                            lightDef, shader, lightScissor, false);
          } else {
            R_LinkLightSurf(&vLight->globalInteractions, lightTris, vEntity,
                            lightDef, shader, lightScissor, false);
          }
        }
      }
    ...
    if (shadowTris) {
      ...
      if (!r_skipSuppress.GetBool()) {
        if (entityDef->parms.suppressShadowInViewID &&
            entityDef->parms.suppressShadowInViewID ==
      ...
      if (r_useShadowCulling.GetBool() && !shadowTris->bounds.IsCleared()) {
        if (R_CullLocalBox(shadowTris->bounds, vEntity->modelMatrix, 5,
                           tr.viewDef->frustum)) {
          continue;
        }
      }
      ...
      bool inside = R_PotentiallyInsideInfiniteShadow(
          sint->ambientTris, localViewOrigin, localLightOrigin);

      if (sint->shader->TestMaterialFlag(MF_NOSELFSHADOW)) {
        R_LinkLightSurf(&vLight->localShadows, shadowTris, vEntity, lightDef,
                        NULL, shadowScissor, inside);
      } else {
        R_LinkLightSurf(&vLight->globalShadows, shadowTris, vEntity, lightDef,
                        NULL, shadowScissor, inside);
      }
    }
  }
...
void R_ShowInteractionMemory_f(const idCmdArgs& args) {
  ...
  for (int i = 0; i < tr.primaryWorld->entityDefs.Num(); i++) {
    ...
    for (idInteraction* inter = def->firstInteraction; inter != NULL;

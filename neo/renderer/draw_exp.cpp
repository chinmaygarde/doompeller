...
really huge lights, like sunlight, are going to be problematic with fixed
projections we could tile the projections and let the auto-resize cut them down
as necessary

It sucks that depth buffers are non-linear, because the bias and compares change
with distance

polygon offset factor causes occasional texture holes from highly angled
textures
...
static bool nativeViewBuffer =
    false;  // true if viewBufferSize is the viewport width
...
idCVar r_sb_lightResolution(
    "r_sb_lightResolution",
    "1024",
    CVAR_RENDERER | CVAR_INTEGER,
    "Pixel dimensions for each shadow buffer, 64 - 2048");
idCVar r_sb_viewResolution("r_sb_viewResolution",
                           "1024",
                           CVAR_RENDERER | CVAR_INTEGER,
                           "Width of screen space shadow sampling");
idCVar r_sb_noShadows("r_sb_noShadows",
                      "0",
                      CVAR_RENDERER | CVAR_BOOL,
                      "don't draw any occluders");
idCVar r_sb_usePbuffer("r_sb_usePbuffer",
                       "1",
                       CVAR_RENDERER | CVAR_BOOL,
                       "draw offscreen");
idCVar r_sb_jitterScale("r_sb_jitterScale",
                        "0.006",
                        CVAR_RENDERER | CVAR_FLOAT,
                        "scale factor for jitter offset");
idCVar r_sb_biasScale("r_sb_biasScale",
                      "0.0001",
                      CVAR_RENDERER | CVAR_FLOAT,
                      "scale factor for jitter bias");
idCVar r_sb_samples("r_sb_samples",
                    "4",
                    CVAR_RENDERER | CVAR_INTEGER,
                    "0, 1, 4, or 16");
idCVar r_sb_randomize("r_sb_randomize",
                      "1",
                      CVAR_RENDERER | CVAR_BOOL,
                      "randomly offset jitter texture each draw");
// polyOfsFactor causes holes in low res images
idCVar r_sb_polyOfsFactor("r_sb_polyOfsFactor",
                          "2",
                          CVAR_RENDERER | CVAR_FLOAT,
                          "polygonOffset factor for drawing shadow buffer");
idCVar r_sb_polyOfsUnits("r_sb_polyOfsUnits",
                         "3000",
                         CVAR_RENDERER | CVAR_FLOAT,
                         "polygonOffset units for drawing shadow buffer");
idCVar r_sb_occluderFacing(
    "r_sb_occluderFacing",
    "0",
    CVAR_RENDERER | CVAR_INTEGER,
...
idCVar r_sb_frustomFOV("r_sb_frustomFOV",
                       "92",
                       CVAR_RENDERER | CVAR_FLOAT,
                       "oversize FOV for point light side matching");
idCVar r_sb_showFrustumPixels("r_sb_showFrustumPixels",
                              "0",
                              CVAR_RENDERER | CVAR_BOOL,
                              "color the pixels contained in the frustum");
idCVar r_sb_singleSide("r_sb_singleSide",
                       "-1",
                       CVAR_RENDERER | CVAR_INTEGER,
                       "only draw a single side (0-5) of point lights");
idCVar r_sb_useCulling("r_sb_useCulling",
                       "1",
                       CVAR_RENDERER | CVAR_BOOL,
                       "cull geometry to individual side frustums");
idCVar r_sb_linearFilter("r_sb_linearFilter",
                         "1",
                         CVAR_RENDERER | CVAR_BOOL,
                         "use GL_LINEAR instead of GL_NEAREST on shadow maps");

idCVar r_sb_screenSpaceShadow(
    "r_sb_screenSpaceShadow",
    "1",
    CVAR_RENDERER | CVAR_BOOL,
    "build shadows in screen space instead of on surfaces");

idCVar r_hdr_useFloats("r_hdr_useFloats",
                       "0",
                       CVAR_RENDERER | CVAR_BOOL,
                       "use a floating point rendering buffer");
idCVar r_hdr_exposure("r_hdr_exposure",
                      "1.0",
                      CVAR_RENDERER | CVAR_FLOAT,
                      "maximum light scale");
idCVar r_hdr_bloomFraction("r_hdr_bloomFraction",
                           "0.1",
                           CVAR_RENDERER | CVAR_FLOAT,
                           "fraction to smear across neighbors");
idCVar r_hdr_gamma("r_hdr_gamma",
                   "1",
                   CVAR_RENDERER | CVAR_FLOAT,
                   "monitor gamma power");
idCVar r_hdr_monitorDither("r_hdr_monitorDither",
                           "0.01",
                           CVAR_RENDERER | CVAR_FLOAT,
...
static void R_CreateShadowBufferImage(idImage* image) {
  ...
  image->GenerateImage((byte*)data, 4, 4, TF_LINEAR, false, TR_CLAMP_TO_BORDER,
                       TD_HIGH_QUALITY);

  // now reset it to a shadow depth image
  GL_CheckErrors();
  image->uploadWidth = image->uploadHeight = lightBufferSize;
  qglTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT24_ARB, lightBufferSize,
                lightBufferSize, 0, GL_DEPTH_COMPONENT, GL_UNSIGNED_BYTE, data);
...
static void R_CreateViewAlphaImage(idImage* image) {
  ...
  image->GenerateImage((byte*)data, viewBufferSize, viewBufferSize, TF_LINEAR,
...
static void R_CreateStubImage(idImage* image) {
  ...
  glTexImage2D(GL_TEXTURE_RECTANGLE_NV, 0, GL_FLOAT_RGBA16_NV, 4, 3, 0, GL_RGBA,
               GL_FLOAT, &data);
}
...
static void R_CreateJitterImage16(idImage* image) {
  ...
  image->GenerateImage((byte*)data, JITTER_SIZE * 16, JITTER_SIZE, TF_NEAREST,
...
static void R_CreateJitterImage4(idImage* image) {
  ...
  image->GenerateImage((byte*)data, JITTER_SIZE * 4, JITTER_SIZE, TF_NEAREST,
                       false, TR_REPEAT, TD_HIGH_QUALITY);
}
...
static void R_CreateJitterImage1(idImage* image) {
  ...
  image->GenerateImage((byte*)data, JITTER_SIZE, JITTER_SIZE, TF_NEAREST, false,
...
static void R_CreateRandom256Image(idImage* image) {
  ...
  image->GenerateImage((byte*)data, 256, 256, TF_NEAREST, false, TR_REPEAT,
                       TD_HIGH_QUALITY);
}
...
void R_PrintPixelFormat(int pixelFormat) {
  ...
  for (int i = 1; i < NUM_WGL_STRINGS; i++) {
    ...
    res = wglGetPixelFormatAttribivARB(win32.hDC, pixelFormat, 0, 1,
                                       &iAttribute, &iValue);
    if (res && iValue) {
...
void R_Exp_Allocate(void) {
  ...
  *atr_p++ = 0;
  ...
  int pbiAttributesTexture[] = {
      ...
      WGL_TEXTURE_RECTANGLE_NV,  // WGL_TEXTURE_2D_ARB,
      0,
      0};

  floatPbuffer =
      wglCreatePbufferARB(win32.hDC, pixelformats[0], glConfig.vidWidth,
  ...
  floatPbufferImage =
      globalImages->ImageFromFunction("_floatPbuffer", R_CreateStubImage);
  ...
  floatPbuffer2Image =
      globalImages->ImageFromFunction("_floatPbuffer2", R_CreateStubImage);

  // create a third buffer for down sampling operations
  floatPbufferQuarter =
      wglCreatePbufferARB(win32.hDC, pixelformats[0], glConfig.vidWidth / 4,
  ...
  floatPbufferQuarterImage =
      globalImages->ImageFromFunction("floatPbufferQuarter", R_CreateStubImage);
  ...
  *atr_p++ = 0;
  ...
  // allocate a pbuffer with this pixel format
  shadowPbuffer =
      wglCreatePbufferARB(win32.hDC, pixelformats[0], lightBufferSize,
  ...
  // generate the texture number
  shadowImage[0] = globalImages->ImageFromFunction(
      va("_shadowBuffer%i_0", lightBufferSize), R_CreateShadowBufferImage);
  shadowImage[1] = globalImages->ImageFromFunction(
      va("_shadowBuffer%i_1", lightBufferSize), R_CreateShadowBufferImage);
  shadowImage[2] = globalImages->ImageFromFunction(
      va("_shadowBuffer%i_2", lightBufferSize), R_CreateShadowBufferImage);
  ...
  // allocate a pbuffer with this pixel format
  viewPbuffer =
      wglCreatePbufferARB(win32.hDC, pixelformats[0], maxViewBufferSize,
  ...
  viewDepthImage =
      globalImages->ImageFromFunction("_viewDepth", R_CreateShadowBufferImage);

  // create the image space shadow alpha buffer for subsampling the shadow
  // calculation
  viewAlphaImage =
      globalImages->ImageFromFunction("_viewAlpha", R_CreateViewAlphaImage);

  //-----------------------------------

  // generate the jitter image
  jitterImage16 =
      globalImages->ImageFromFunction("_jitter16", R_CreateJitterImage16);
  jitterImage4 =
      globalImages->ImageFromFunction("_jitter4", R_CreateJitterImage4);
  jitterImage1 =
      globalImages->ImageFromFunction("_jitter1", R_CreateJitterImage1);

  depthMidpointVertexProgram =
      R_FindARBProgram(GL_VERTEX_PROGRAM_ARB, "depthMidpoint.vfp");
  depthMidpointFragmentProgram =
      R_FindARBProgram(GL_FRAGMENT_PROGRAM_ARB, "depthMidpoint.vfp");

  shadowResampleVertexProgram =
      R_FindARBProgram(GL_VERTEX_PROGRAM_ARB, "shadowResample.vfp");
  shadowResampleFragmentProgram =
      R_FindARBProgram(GL_FRAGMENT_PROGRAM_ARB, "shadowResample.vfp");

  screenSpaceShadowVertexProgram =
      R_FindARBProgram(GL_VERTEX_PROGRAM_ARB, "screenSpaceShadow1.vfp");

  screenSpaceShadowFragmentProgram0 =
      R_FindARBProgram(GL_FRAGMENT_PROGRAM_ARB, "screenSpaceShadow0.vfp");
  screenSpaceShadowFragmentProgram1 =
      R_FindARBProgram(GL_FRAGMENT_PROGRAM_ARB, "screenSpaceShadow1.vfp");
  screenSpaceShadowFragmentProgram4 =
      R_FindARBProgram(GL_FRAGMENT_PROGRAM_ARB, "screenSpaceShadow4.vfp");
  screenSpaceShadowFragmentProgram16 =
      R_FindARBProgram(GL_FRAGMENT_PROGRAM_ARB, "screenSpaceShadow16.vfp");

  shadowVertexProgram =
      R_FindARBProgram(GL_VERTEX_PROGRAM_ARB, "shadowBufferInteraction1.vfp");

  shadowFragmentProgram0 =
      R_FindARBProgram(GL_FRAGMENT_PROGRAM_ARB, "shadowBufferInteraction0.vfp");
  shadowFragmentProgram1 =
      R_FindARBProgram(GL_FRAGMENT_PROGRAM_ARB, "shadowBufferInteraction1.vfp");
  shadowFragmentProgram4 =
      R_FindARBProgram(GL_FRAGMENT_PROGRAM_ARB, "shadowBufferInteraction4.vfp");
  shadowFragmentProgram16 = R_FindARBProgram(GL_FRAGMENT_PROGRAM_ARB,
                                             "shadowBufferInteraction16.vfp");

  gammaDitherVertexProgram =
      R_FindARBProgram(GL_VERTEX_PROGRAM_ARB, "gammaDither.vfp");
  gammaDitherFragmentProgram =
      R_FindARBProgram(GL_FRAGMENT_PROGRAM_ARB, "gammaDither.vfp");

  downSampleVertexProgram =
      R_FindARBProgram(GL_VERTEX_PROGRAM_ARB, "downSample.vfp");
  downSampleFragmentProgram =
      R_FindARBProgram(GL_FRAGMENT_PROGRAM_ARB, "downSample.vfp");
  ...
  random256Image =
      globalImages->ImageFromFunction("_random256", R_CreateRandom256Image);
}

//===========================================================================================

static const int CULL_RECEIVER =
    1;  // still draw occluder, but it is out of the view
static const int CULL_OCCLUDER_AND_RECEIVER =
    2;  // the surface doesn't effect the view at all
...
void RB_EXP_CullInteractions(viewLight_t* vLight, idPlane frustumPlanes[6]) {
  for (idInteraction* inter = vLight->lightDef->firstInteraction; inter;
    ...
    if (r_sb_useCulling.GetBool()) {
      // transform light frustum into object space, positive side points outside
      // the light
      idPlane localPlanes[6];
      int plane;
      for (plane = 0; plane < 6; plane++) {
        R_GlobalPlaneToLocal(entityDef->modelMatrix, frustumPlanes[plane],
                             localPlanes[plane]);
      }
      ...
      for (plane = 0; plane < 6; plane++) {
        int j;
        for (j = 0; j < 8; j++) {
          // if a corner is on the negative side (inside) of the frustum, the
          // surface is not culled by this plane
          if (corners[j] * localPlanes[plane].ToVec4().ToVec3() +
                  localPlanes[plane][3] <
              0) {
            break;
          }
...
void RB_RenderShadowBuffer(viewLight_t* vLight, int side) {
  ...
  GL_State(GLS_DEPTHFUNC_LESS | GLS_SRCBLEND_ONE |
           GLS_DSTBLEND_ZERO);  // make sure depth mask is off before clear
  ...
  viewMatrix[12] = -origin[0] * viewMatrix[0] + -origin[1] * viewMatrix[4] +
                   -origin[2] * viewMatrix[8];
  viewMatrix[13] = -origin[0] * viewMatrix[1] + -origin[1] * viewMatrix[5] +
                   -origin[2] * viewMatrix[9];
  viewMatrix[14] = -origin[0] * viewMatrix[2] + -origin[1] * viewMatrix[6] +
                   -origin[2] * viewMatrix[10];
  ...
  globalFrustum[0][3] =
      -(origin[0] * globalFrustum[0][0] + origin[1] * globalFrustum[0][1] +
        origin[2] * globalFrustum[0][2]);
  ...
  // FIXME: we want to skip the sampling as well as the generation when not
  // casting shadows
  if (!r_sb_noShadows.GetBool() && vLight->lightShader->LightCastsShadows()) {
    ...
    switch (r_sb_occluderFacing.GetInteger()) {
      case 0:  // front sides
        qglPolygonOffset(r_sb_polyOfsFactor.GetFloat(),
                         r_sb_polyOfsUnits.GetFloat());
      ...
      case 2:  // both sides
        ...
        GL_Cull(CT_FRONT_SIDED);
        shadowImage[2]->Bind();
        qglCopyTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 0, 0, lightBufferSize,
                             lightBufferSize);
...
void RB_EXP_DrawInteraction(const drawInteraction_t* din) {
  ...
  qglProgramEnvParameter4fvARB(GL_VERTEX_PROGRAM_ARB, PP_LIGHT_ORIGIN,
                               din->localLightOrigin.ToFloatPtr());
  qglProgramEnvParameter4fvARB(GL_VERTEX_PROGRAM_ARB, PP_VIEW_ORIGIN,
                               din->localViewOrigin.ToFloatPtr());
  qglProgramEnvParameter4fvARB(GL_VERTEX_PROGRAM_ARB, PP_LIGHT_PROJECT_S,
                               din->lightProjection[0].ToFloatPtr());
  qglProgramEnvParameter4fvARB(GL_VERTEX_PROGRAM_ARB, PP_LIGHT_PROJECT_T,
                               din->lightProjection[1].ToFloatPtr());
  qglProgramEnvParameter4fvARB(GL_VERTEX_PROGRAM_ARB, PP_LIGHT_PROJECT_Q,
                               din->lightProjection[2].ToFloatPtr());
  qglProgramEnvParameter4fvARB(GL_VERTEX_PROGRAM_ARB, PP_LIGHT_FALLOFF_S,
                               din->lightProjection[3].ToFloatPtr());
  qglProgramEnvParameter4fvARB(GL_VERTEX_PROGRAM_ARB, PP_BUMP_MATRIX_S,
                               din->bumpMatrix[0].ToFloatPtr());
  qglProgramEnvParameter4fvARB(GL_VERTEX_PROGRAM_ARB, PP_BUMP_MATRIX_T,
                               din->bumpMatrix[1].ToFloatPtr());
  qglProgramEnvParameter4fvARB(GL_VERTEX_PROGRAM_ARB, PP_DIFFUSE_MATRIX_S,
                               din->diffuseMatrix[0].ToFloatPtr());
  qglProgramEnvParameter4fvARB(GL_VERTEX_PROGRAM_ARB, PP_DIFFUSE_MATRIX_T,
                               din->diffuseMatrix[1].ToFloatPtr());
  qglProgramEnvParameter4fvARB(GL_VERTEX_PROGRAM_ARB, PP_SPECULAR_MATRIX_S,
                               din->specularMatrix[0].ToFloatPtr());
  qglProgramEnvParameter4fvARB(GL_VERTEX_PROGRAM_ARB, PP_SPECULAR_MATRIX_T,
  ...
  if (r_testARBProgram.GetBool()) {
    qglProgramEnvParameter4fvARB(GL_FRAGMENT_PROGRAM_ARB, 2,
                                 din->localLightOrigin.ToFloatPtr());
    qglProgramEnvParameter4fvARB(GL_FRAGMENT_PROGRAM_ARB, 3,
                                 din->localViewOrigin.ToFloatPtr());
  }
  ...
  switch (din->vertexColor) {
    case SVC_IGNORE:
      qglProgramEnvParameter4fvARB(GL_VERTEX_PROGRAM_ARB, PP_COLOR_MODULATE,
                                   zero);
  ...
  qglProgramEnvParameter4fvARB(GL_FRAGMENT_PROGRAM_ARB, 0,
...
void RB_EXP_CreateDrawInteractions(const drawSurf_t* surf) {
  ...
  if (r_testARBProgram.GetBool()) {
    globalImages->specular2DTableImage
        ->Bind();  // variable specularity in alpha channel
  } else {
  ...
  for (; surf; surf = surf->nextOnLight) {
    ...
    qglVertexAttribPointerARB(11, 3, GL_FLOAT, false, sizeof(idDrawVert),
                              ac->normal.ToFloatPtr());
    qglVertexAttribPointerARB(10, 3, GL_FLOAT, false, sizeof(idDrawVert),
                              ac->tangents[1].ToFloatPtr());
    qglVertexAttribPointerARB(9, 3, GL_FLOAT, false, sizeof(idDrawVert),
                              ac->tangents[0].ToFloatPtr());
    qglVertexAttribPointerARB(8, 2, GL_FLOAT, false, sizeof(idDrawVert),
                              ac->st.ToFloatPtr());
    qglVertexPointer(3, GL_FLOAT, sizeof(idDrawVert), ac->xyz.ToFloatPtr());
...
const srfTriangles_t* RB_Exp_TrianglesForFrustum(viewLight_t* vLight,
  ...
  static glIndex_t indexes[18] = {0, 1, 2, 0, 2, 3, 0, 3, 4,
                                  0, 4, 1, 2, 1, 4, 2, 4, 3};
...
void RB_Exp_SelectFrustum(viewLight_t* vLight, int side) {
  ...
  // draw front faces of the light frustum, incrementing the stencil buffer on
  // depth fail so we can't draw on those pixels
  ...
  */
  ...
  /*
PARAM	positionToDepthTexScale		= program.local[0];	#
fragment.position to screen depth texture transformation PARAM	zProject
= program.local[1];	# projection[10], projection[14], 0, 0 PARAM
positionToViewSpace			= program.local[2];	# X add, Y add,
X mul, Y mul PARAM	viewToLightS				=
program.local[3]; PARAM	viewToLightT				=
program.local[4]; PARAM	viewToLightR				=
program.local[5]; PARAM	viewToLightQ				=
program.local[6]; PARAM	positionToJitterTexScale	= program.local[7];
# fragment.position to jitter texture PARAM	jitterTexScale
= program.local[8]; PARAM	jitterTexOffset				=
program.local[9];
*/
  float parm[4];
  int pot;
  ...
  */
  ...
  // we need the inverse of the viewMatrix to go from view (looking down
  // negative Z) to world
  ...
  // convert 0..viewport-1 sizes to fractions inside the POT screen depth
  // texture
  ...
  // after all the frustums have been drawn, the surfaces that have been drawn
  // on will get interactions scissor may still be a win even with the stencil
  // test for very fast rejects
  qglStencilFunc(GL_EQUAL, 0, 255);
  qglStencilOp(GL_KEEP, GL_KEEP, GL_KEEP);

  // we can avoid clearing the stencil buffer by changing the hasLight value for
  // each light
}
...
float R_EXP_CalcLightAxialSize(viewLight_t* vLight) {
  ...
  if (!vLight->lightDef->parms.pointLight) {
    idVec3 dir =
        vLight->lightDef->parms.target - vLight->lightDef->parms.origin;
    max = dir.Length();
    return max;
  }
...
This could be avoided by drop sampling the native view depth buffer with render
to texture Bilerp might even be aprorpiate, although it would cause issues at
edges
==================
*/
...
void R_EXP_RenderViewDepthImage(void) {
  ...
  if (0) {  // nativeViewBuffer ) {
    viewDepthImage->CopyDepthbuffer(
    ...
    RB_RenderDrawSurfListWithFunction(backEnd.viewDef->drawSurfs,
    ...
    //
    viewDepthImage->Bind();
    qglCopyTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 0, 0, viewBufferSize,
                         viewBufferHeight);
...
void RB_shadowResampleAlpha(void) {
  viewAlphaImage->Bind();
  // we could make this a subimage, but it isn't relevent once we have
  // render-to-texture
  qglCopyTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 0, 0, viewBufferSize,
                       viewBufferHeight);
  ...
  // depth test greater
  // stencil test of equal 1
  // zero stencil stencil when depth test passes, so subsequent interaction
  // drawing can occur on those pixels
  ...
  if (r_sb_showFrustumPixels.GetBool()) {
    ...
    // convert 0..viewport-1 sizes to fractions inside the POT screen depth
    // texture shrink by one unit for bilerp
    float parm[4];
    parm[0] =
        1.0 / (maxViewBufferSize + 1) * viewBufferSize / maxViewBufferSize;
    parm[1] = parm[0];
    parm[2] = 0;
    parm[3] = 1;
    qglProgramLocalParameter4fvARB(GL_FRAGMENT_PROGRAM_ARB, 0, parm);
...
void RB_EXP_ReadFloatBuffer(void) {
  ...
  qglReadPixels(0, 0, glConfig.vidWidth, glConfig.vidHeight, GL_RGBA, GL_FLOAT,
                buf);
...
multiply HDR value by this to get screen pixels PARAM	gammaPower =
program.local[1]; PARAM	monitorDither =		program.local[2]; PARAM
positionToDitherScale =		program.local[3]; PARAM	bloomFraction =
program.local[4]; PARAM	positionToBloomScale =		program.local[5];
...
PARAM	step =		program.local[0];		# { 1, 0 } or { 0, 1 }
for horizontal / vertical separation
  */
...
void RB_Exp_DrawInteractions(void) {
  ...
  if (r_sb_samples.GetInteger() != 16 && r_sb_samples.GetInteger() != 4 &&
      r_sb_samples.GetInteger() != 1) {
  ...
  // set up for either point sampled or percentage-closer filtering for the
  // shadow sampling
  ...
  if (r_hdr_useFloats.GetBool()) {
    ...
    // we need to set a lot of things, because this is a completely different
    // context
    ...
    RB_BeginDrawingView();
    RB_STD_FillDepthBuffer((drawSurf_t**)&backEnd.viewDef->drawSurfs[0],
                           backEnd.viewDef->numDrawSurfs);
  }
  ...
  //
  for (viewLight_t* vLight = backEnd.viewDef->viewLights; vLight;
       vLight = vLight->next) {
    ...
    if (!vLight->frustumTris->ambientCache) {
      R_CreateAmbientCache(const_cast<srfTriangles_t*>(vLight->frustumTris),
                           false);
    }
    ...
    for (; side < sideStop; side++) {
      ...
      if (nativeViewBuffer || !r_sb_screenSpaceShadow.GetBool()) {
        ...
        qglViewport(0, 0, viewBufferSize, viewBufferHeight);
        qglScissor(0, 0, viewBufferSize,
                   viewBufferHeight);  // !@# FIXME: scale light scissor
      }

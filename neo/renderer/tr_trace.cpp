...
localTrace_t R_LocalTrace(const idVec3& start,
  ...
  SIMDProcessor->TracePointCull(cullBits, totalOr, radius, planes, tri->verts,
                                tri->numVerts);
  ...
  for (i = 0, j = 0; i < tri->numIndexes; i += 3, j++) {
    ...
    if (d > 0.0f) {
      ...
      edge = tri->verts[tri->indexes[i + 0]].xyz -
      ...
      if (d < 0.0f) {
        ...
        edge = tri->verts[tri->indexes[i + 1]].xyz -
               tri->verts[tri->indexes[i + 2]].xyz;
        d = edge * dir[1];
        if (d < 0.0f) {
    ...
    if (d > 0.0f) {
      ...
      if (d < 0.0f) {
        ...
        edge = tri->verts[tri->indexes[i + 2]].xyz -
               tri->verts[tri->indexes[i + 0]].xyz;
        d = edge * dir[2];
        if (d < 0.0f) {
  ...
  common->Printf(
      ...
      tri->numVerts, c_testPlanes, c_testEdges, c_intersect,
...
void RB_DrawExpandedTriangles(const srfTriangles_t* tri,
  ...
  for (i = 0; i < tri->numIndexes; i += 3) {
    idVec3 p[3] = {tri->verts[tri->indexes[i + 0]].xyz,
                   tri->verts[tri->indexes[i + 1]].xyz,
                   tri->verts[tri->indexes[i + 2]].xyz};

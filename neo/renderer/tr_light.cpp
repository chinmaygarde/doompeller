...
bool R_CreateAmbientCache(srfTriangles_t* tri, bool needsLighting) {
  ...
  vertexCache.Alloc(tri->verts, tri->numVerts * sizeof(tri->verts[0]),
                    &tri->ambientCache);
  if (!tri->ambientCache) {
    return false;
  }
...
bool R_CreateLightingCache(const idRenderEntityLocal* ent,
  ...
  R_GlobalPointToLocal(ent->modelMatrix, light->globalLightOrigin,
                       localLightOrigin);
  ...
  SIMDProcessor->CreateTextureSpaceLightVectors(
      &cache[0].localLightVector, localLightOrigin, tri->ambientSurface->verts,
      tri->ambientSurface->numVerts, tri->indexes, tri->numIndexes);

#else

  bool* used =
      (bool*)_alloca16(tri->ambientSurface->numVerts * sizeof(used[0]));
...
void R_CreatePrivateShadowCache(srfTriangles_t* tri) {
  ...
  vertexCache.Alloc(tri->shadowVertexes,
                    tri->numVerts * sizeof(*tri->shadowVertexes),
                    &tri->shadowCache);
}
...
void R_CreateVertexProgramShadowCache(srfTriangles_t* tri) {
  ...
  shadowCache_t* temp =
      (shadowCache_t*)_alloca16(tri->numVerts * 2 * sizeof(shadowCache_t));

#if 1

  SIMDProcessor->CreateVertexProgramShadowCache(&temp->xyz, tri->verts,
                                                tri->numVerts);
  ...
  vertexCache.Alloc(temp, tri->numVerts * 2 * sizeof(shadowCache_t),
                    &tri->shadowCache);
}
...
=================
*/
static void R_SpecularTexGen(drawSurf_t* surf,
  ...
  R_GlobalPointToLocal(surf->space->modelMatrix, globalLightOrigin,
                       localLightOrigin);
  R_GlobalPointToLocal(surf->space->modelMatrix, viewOrg, localViewOrigin);
  ...
  SIMDProcessor->CreateSpecularTextureCoords(
      texCoords, localLightOrigin, localViewOrigin, tri->verts, tri->numVerts,
...
viewEntity_t* R_SetEntityDefViewEntity(idRenderEntityLocal* def) {
  ...
  // creation time
  if (tr.viewDef) {
    myGlMultMatrix(vModel->modelMatrix, tr.viewDef->worldSpace.modelViewMatrix,
...
// plane
static bool R_TestPointInViewLight(const idVec3& org,
                                   const idRenderLightLocal* light) {
  int i;
  idVec3 local;
...
viewLight_t* R_SetLightDefViewLight(idRenderLightLocal* light) {
  ...
  vLight->viewInsideLight =
      R_TestPointInViewLight(tr.viewDef->renderView.vieworg, light);
  if (!vLight->viewInsideLight) {
    vLight->viewSeesShadowPlaneBits = 0;
    for (int i = 0; i < light->numShadowFrustums; i++) {
      float d = light->shadowFrustums[i].planes[5].Distance(
  ...
  vLight->shaderRegisters =
      NULL;  // allocated and evaluated in R_AddLightSurfaces
...
void idRenderWorldLocal::CreateLightDefInteractions(idRenderLightLocal* ldef) {
  ...
  for (lref = ldef->references; lref; lref = lref->ownerNext) {
    ...
    for (eref = area->entityRefs.areaNext; eref != &area->entityRefs;
         eref = eref->areaNext) {
      edef = eref->entity;
      ...
      if (tr.viewDef && edef->viewCount != tr.viewCount) {
        ...
        if (!r_skipSuppress.GetBool()) {
          if (edef->parms.suppressShadowInViewID &&
              edef->parms.suppressShadowInViewID ==
                  tr.viewDef->renderView.viewID) {
            continue;
          }
          if (edef->parms.suppressShadowInLightID &&
              edef->parms.suppressShadowInLightID == ldef->parms.lightId) {
            continue;
          }
        }
      }
      ...
      if (r_useInteractionTable.GetBool() && this->interactionTable) {
        ...
        // fairly close).
        for (inter = edef->firstInteraction; inter != NULL;
...
void R_LinkLightSurf(const drawSurf_t** link,
  ...
  if (!shader) {
    ...
    if (constRegs) {
      ...
      float* regs =
          (float*)R_FrameAlloc(shader->GetNumRegisters() * sizeof(float));
      drawSurf->shaderRegisters = regs;
      shader->EvaluateRegisters(regs, space->entityDef->parms.shaderParms,
                                tr.viewDef,
    ...
    if (!tr.backEndRendererHasVertexPrograms && !r_skipSpecular.GetBool() &&
      ...
      R_SpecularTexGen(drawSurf, light->globalLightOrigin,
                       tr.viewDef->renderView.vieworg);
...
idScreenRect R_ClippedLightScissorRectangle(viewLight_t* vLight) {
  ...
  for (i = 0; i < 6; i++) {
    ...
    for (j = 0; j < w.GetNumPoints(); j++) {
      ...
      R_TransformModelToClip(w[j].ToVec3(),
      ...
      float windowX = 0.5f * (1.0f + ndc[0]) *
                      (tr.viewDef->viewport.x2 - tr.viewDef->viewport.x1);
      float windowY = 0.5f * (1.0f + ndc[1]) *
...
idScreenRect R_CalcLightScissorRectangle(viewLight_t* vLight) {
  ...
  if (vLight->lightDef->parms.pointLight) {
    ...
    tr.viewDef->viewFrustum.ProjectionBounds(
        idBox(lightDef->parms.origin, lightDef->parms.lightRadius,
              lightDef->parms.axis),
        bounds);
  ...
  for (int i = 0; i < tri->numVerts; i++) {
    R_TransformModelToClip(tri->verts[i].xyz,
                           tr.viewDef->worldSpace.modelViewMatrix,
                           tr.viewDef->projectionMatrix, eye, clip);
...
void R_AddLightSurfaces(void) {
  ...
  while (*ptr) {
    ...
    float* lightRegs =
        (float*)R_FrameAlloc(lightShader->GetNumRegisters() * sizeof(float));
    vLight->shaderRegisters = lightRegs;
    lightShader->EvaluateRegisters(lightRegs, light->parms.shaderParms,
                                   tr.viewDef, light->parms.referenceSound);
    ...
    if (!lightShader->IsFogLight() && !lightShader->IsBlendLight()) {
      int lightStageNum;
      for (lightStageNum = 0; lightStageNum < lightShader->GetNumStages();
           lightStageNum++) {
        ...
        if (lightRegs[registers[0]] > 0.0f || lightRegs[registers[1]] > 0.0f ||
            lightRegs[registers[2]] > 0.0f) {
          break;
        }
      }
    ...
    if (r_lightSourceRadius.GetFloat() != 0.0f) {
      for (int i = 0; i < 3; i++) {
        light->globalLightOrigin[i] +=
            r_lightSourceRadius.GetFloat() *
    ...
    if (light->parms.prelightModel && r_useOptimizedShadows.GetBool()) {
      if (!light->parms.prelightModel->NumSurfaces()) {
        common->Error("no surfs in prelight model '%s'",
      ...
      if (r_useShadowCulling.GetBool()) {
        if (R_CullLocalBox(tri->bounds, tr.viewDef->worldSpace.modelMatrix, 5,
                           tr.viewDef->frustum)) {
          continue;
        }
      }
      ...
      R_LinkLightSurf(&vLight->globalShadows, tri, NULL, light, NULL,
                      vLight->scissorRect, true /* FIXME? */);
    }
  }
}
...
idRenderModel* R_EntityDefDynamicModel(idRenderEntityLocal* def) {
  ...
  if (!def->dynamicModel) {
    ...
    def->cachedDynamicModel = model->InstantiateDynamicModel(
        &def->parms, tr.viewDef, def->cachedDynamicModel);
  ...
  if (def->dynamicModel && model->DepthHack() != 0.0f && tr.viewDef) {
    idPlane eye, clip;
    idVec3 ndc;
    R_TransformModelToClip(def->parms.origin,
                           tr.viewDef->worldSpace.modelViewMatrix,
                           tr.viewDef->projectionMatrix, eye, clip);
    R_TransformClipToDevice(clip, tr.viewDef, ndc);
...
void R_AddDrawSurf(const srfTriangles_t* tri,
  ...
  static float refRegs[MAX_EXPRESSION_REGISTERS];  // don't put on stack, or
                                                   // VC++ will do a page touch
  ...
  if (tr.viewDef->numDrawSurfs == tr.viewDef->maxDrawSurfs) {
    ...
    tr.viewDef->drawSurfs = (drawSurf_t**)R_FrameAlloc(
        tr.viewDef->maxDrawSurfs * sizeof(tr.viewDef->drawSurfs[0]));
  ...
  if (constRegs) {
    ...
    if (renderEntity->referenceShader) {
      ...
      renderEntity->referenceShader->EvaluateRegisters(
          refRegs, renderEntity->shaderParms, tr.viewDef,
          renderEntity->referenceSound);
      pStage = renderEntity->referenceShader->GetStage(0);

      memcpy(generatedShaderParms, renderEntity->shaderParms,
    ...
    if (space->entityDef && space->entityDef->parms.timeGroup) {
      ...
      tr.viewDef->floatTime =
          game->GetTimeGroupTime(space->entityDef->parms.timeGroup) * 0.001;
      tr.viewDef->renderView.time =
          game->GetTimeGroupTime(space->entityDef->parms.timeGroup);
    }

    shader->EvaluateRegisters(regs, shaderParms, tr.viewDef,
                              renderEntity->referenceSound);
...
static void R_AddAmbientDrawsurfs(viewEntity_t* vEntity) {
  ...
  for (i = 0; i < total; i++) {
    ...
    if (r_singleSurface.GetInteger() >= 0 &&
        i != r_singleSurface.GetInteger()) {
      continue;
    }
    ...
    shader = surf->shader;
    shader = R_RemapShaderBySkin(shader, def->parms.customSkin,
                                 def->parms.customShader);
    ...
    if (r_checkBounds.GetBool()) {
      ...
      for (j = 0; j < tri->numVerts; j++) {
        for (k = 0; k < 3; k++) {
          ...
          if (tri->verts[j].xyz[k] >
                  def->referenceBounds[1][k] + CHECK_BOUNDS_EPSILON ||
              tri->verts[j].xyz[k] <
                  def->referenceBounds[0][k] - CHECK_BOUNDS_EPSILON) {
    ...
    if (!R_CullLocalBox(tri->bounds, vEntity->modelMatrix, 5,
                        tr.viewDef->frustum)) {
      ...
      R_AddDrawSurf(tri, vEntity, &vEntity->entityDef->parms, shader,
                    vEntity->scissorRect);
...
void R_AddModelSurfaces(void) {
  ...
  tr.viewDef->maxDrawSurfs =
      0;  // will be set to INITIAL_DRAWSURFS on R_AddDrawSurf
  ...
  for (vEntity = tr.viewDef->viewEntitys; vEntity; vEntity = vEntity->next) {
    if (r_useEntityScissors.GetBool()) {
      ...
      if (r_showEntityScissors.GetBool()) {
        R_ShowColoredScreenRect(vEntity->scissorRect,
                                vEntity->entityDef->index);
      }
    }
    ...
    if (vEntity->entityDef->parms.timeGroup) {
      ...
      tr.viewDef->floatTime =
          game->GetTimeGroupTime(vEntity->entityDef->parms.timeGroup) * 0.001;
      tr.viewDef->renderView.time =
          game->GetTimeGroupTime(vEntity->entityDef->parms.timeGroup);
    }
    ...
    if (tr.viewDef->isXraySubview && vEntity->entityDef->parms.xrayIndex == 1) {
      ...
      continue;
    } else if (!tr.viewDef->isXraySubview &&
               vEntity->entityDef->parms.xrayIndex == 2) {
      if (vEntity->entityDef->parms.timeGroup) {
        tr.viewDef->floatTime = oldFloatTime;
        tr.viewDef->renderView.time = oldTime;
      }
    ...
    if (tr.viewDef->isXraySubview) {
      if (vEntity->entityDef->parms.xrayIndex == 2) {
        for (inter = vEntity->entityDef->firstInteraction;
             inter != NULL && !inter->IsEmpty(); inter = next) {
...
void R_RemoveUnecessaryViewLights(void) {
  ...
  if (r_useShadowSurfaceScissor.GetBool()) {
    ...
    for (vLight = tr.viewDef->viewLights; vLight; vLight = vLight->next) {
      ...
      for (surf = vLight->translucentInteractions; surf;

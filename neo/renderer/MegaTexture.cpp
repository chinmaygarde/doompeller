...
idCVar idMegaTexture::r_megaTextureLevel("r_megaTextureLevel",
                                         "0",
                                         CVAR_RENDERER | CVAR_INTEGER,
                                         "draw only a specific level");
idCVar idMegaTexture::r_showMegaTexture("r_showMegaTexture",
                                        "0",
...
idCVar idMegaTexture::r_skipMegaTexture("r_skipMegaTexture",
                                        "0",
                                        CVAR_RENDERER | CVAR_INTEGER,
                                        "only use the lowest level image");
idCVar idMegaTexture::r_terrainScale("r_terrainScale",
                                     "3",
                                     CVAR_RENDERER | CVAR_INTEGER,
...
static byte colors[8][4] = {{0, 0, 0, 255},     {255, 0, 0, 255},
...
static void R_EmptyLevelImage(idImage* image) {
  ...
  image->GenerateImage(data, MAX_LEVEL_WIDTH, MAX_LEVEL_WIDTH, TF_DEFAULT,
...
bool idMegaTexture::InitFromMegaFile(const char* fileBase) {
  ...
  while (1) {
    ...
    levels[numLevels].image =
        globalImages->ImageFromFunction(str, R_EmptyLevelImage);
    numLevels++;
...
void idTextureLevel::UpdateTile(int localX,
  ...
  if ((globalX & (TILE_PER_LEVEL - 1)) != localX ||
      (globalY & (TILE_PER_LEVEL - 1)) != localY) {
  ...
  if (globalX >= tilesWide || globalX < 0 || globalY >= tilesHigh ||
      globalY < 0) {
    // off the map
  ...
  if (idMegaTexture::r_showMegaTextureLabels.GetBool()) {
    ...
    byte color[4] = {255 * localX / TILE_PER_LEVEL,
                     255 * localY / TILE_PER_LEVEL, 0, 0};
    for (int x = 0; x < 8; x++) {
      for (int y = 0; y < 8; y++) {
        *(int*)&data[((y + TILE_SIZE / 2 - 4) * TILE_SIZE + x + TILE_SIZE / 2 -
                      4) *
                     4] = *(int*)color;
      }
    }
  ...
  int size = TILE_SIZE;
  while (1) {
    qglTexSubImage2D(GL_TEXTURE_2D, level, localX * size, localY * size, size,
                     size, GL_RGBA, GL_UNSIGNED_BYTE, data);
    size >>= 1;
    level++;
    ...
    for (int y = 0; y < size; y++) {
      ...
      for (int x = 0; x < size; x++) {
        out[x * 4 + 0] = (in[x * 8 + 0] + in[x * 8 + 4 + 0] + in2[x * 8 + 0] +
                          in2[x * 8 + 4 + 0]) >>
                         2;
        out[x * 4 + 1] = (in[x * 8 + 1] + in[x * 8 + 4 + 1] + in2[x * 8 + 1] +
                          in2[x * 8 + 4 + 1]) >>
                         2;
        out[x * 4 + 2] = (in[x * 8 + 2] + in[x * 8 + 4 + 2] + in2[x * 8 + 2] +
                          in2[x * 8 + 4 + 2]) >>
                         2;
        out[x * 4 + 3] = (in[x * 8 + 3] + in[x * 8 + 4 + 3] + in2[x * 8 + 3] +
                          in2[x * 8 + 4 + 3]) >>
                         2;
      }
    }
...
void idTextureLevel::UpdateForCenter(float center[2]) {
  ...
  for (int x = 0; x < TILE_PER_LEVEL; x++) {
    for (int y = 0; y < TILE_PER_LEVEL; y++) {
      ...
      globalTile[0] = globalTileCorner[0] +
                      ((x - localTileOffset[0]) & (TILE_PER_LEVEL - 1));
      globalTile[1] = globalTileCorner[1] +
          ...
          for (int xx = 0; xx < 2; xx++) {
            ...
            for (int yyy = 0; yyy < TILE_SIZE / 2; yyy++) {
              for (int xxx = 0; xxx < TILE_SIZE / 2; xxx++) {
                ...
                byte* out =
                    &newBlock[(((TILE_SIZE / 2 * yy) + yyy) * TILE_SIZE +
                               (TILE_SIZE / 2 * xx) + xxx) *
                              4];
                out[0] = (in[0] + in[4] + in[0 + TILE_SIZE * 4] +
                          in[4 + TILE_SIZE * 4]) >>
                         2;
                out[1] = (in[1] + in[5] + in[1 + TILE_SIZE * 4] +
                          in[5 + TILE_SIZE * 4]) >>
                         2;
                out[2] = (in[2] + in[6] + in[2 + TILE_SIZE * 4] +
                          in[6 + TILE_SIZE * 4]) >>
                         2;
                out[3] = (in[3] + in[7] + in[3 + TILE_SIZE * 4] +
                          in[7 + TILE_SIZE * 4]) >>
                         2;
              }
            }
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      ...
      for (int yy = 0; yy < tileSize; yy++) {
        memcpy(
            pic + ((y * tileSize + yy) * width * tileSize + x * tileSize) * 4,
...
void idMegaTexture::MakeMegaTexture_f(const idCmdArgs& args) {
  ...
  common->Printf("Writing %i x %i size %i tiles to %s.\n", mtHeader.tilesWide,
                 mtHeader.tilesHigh, mtHeader.tileSize, outName.c_str());
  ...
  while (blockRowsRemaining--) {
    ...
    for (int rowBlock = 0; rowBlock < mtHeader.tilesWide; rowBlock++) {
      for (int y = 0; y < TILE_SIZE; y++) {
        out->Write(
            targa_rgba + (y * targa_header.width + rowBlock * TILE_SIZE) * 4,
            TILE_SIZE * 4);
      }
    }
  }

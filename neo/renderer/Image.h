...
typedef enum { TT_DISABLED, TT_2D, TT_3D, TT_CUBIC, TT_RECT } textureType_t;
...
class idImage {
 public:
  ...
  void Generate3DImage(const byte* pic,
                       int width,
                       int height,
                       int depth,
                       textureFilter_t filter,
  ...
  void GenerateCubeImage(const byte* pic[6],
                         int size,
                         textureFilter_t filter,
                         bool allowDownSize,
                         textureDepth_t depth);

  void CopyFramebuffer(int x,
                       int y,
                       int width,
                       int height,
  ...
  void UploadCompressedNormalMap(int width,
                                 int height,
                                 const byte* rgba,
                                 int mipLevel);
  GLenum SelectInternalFormat(const byte** dataPtrs,
                              int numDataPtrs,
                              int width,
                              int height,
                              textureDepth_t minimumDepth,
                              bool* monochromeResult) const;
  void ImageProgramStringToCompressedFileName(const char* imageProg,
                                              char* fileName) const;
  ...
  GLuint
      texnum;  // gl texture binding, will be TEXTURE_NOT_LOADED if not loaded
  textureType_t type;
...
class idImageManager {
 public:
  ...
  static idCVar image_writeNormalTGA;    // debug tool to write out .tgas of the
                                         // final normal maps
  static idCVar
      image_writeNormalTGAPalletized;  // debug tool to write out palletized
                                       // versions of the final normal maps
  static idCVar
      image_writeTGA;  // debug tool to write out .tgas of the non normal maps
  static idCVar image_useNormalCompression;  // 1 = use 256 color compression
                                             // for normal maps if available, 2
                                             // = use rxgb compression
  static idCVar
      image_useOffLineCompression;  // will write a batch file with commands for
  ...
  idImage*
      specularTableImage;  // 1D intensity texture with our specular function

...
void R_ListRenderLightDefs_f(const idCmdArgs& args) {
  ...
  for (i = 0; i < tr.primaryWorld->lightDefs.Num(); i++) {
    ...
    int iCount = 0;
    for (idInteraction* inter = ldef->firstInteraction; inter != NULL;
...
void R_ListRenderEntityDefs_f(const idCmdArgs& args) {
  ...
  for (i = 0; i < tr.primaryWorld->entityDefs.Num(); i++) {
    ...
    // count up the interactions
    int iCount = 0;
    for (idInteraction* inter = mdef->firstInteraction; inter != NULL;
...
void idRenderWorldLocal::ResizeInteractionTable() {
  ...
  common->Printf(
      "idRenderWorldLocal::ResizeInteractionTable: overflowed "
...
void idRenderWorldLocal::UpdateEntityDef(qhandle_t entityHandle,
  ...
  if (def) {
    if (!re->forceUpdate) {
      ...
      if (!re->joints && !re->callbackData && !def->dynamicModel &&
...
void idRenderWorldLocal::UpdateLightDef(qhandle_t lightHandle,
  ...
  if (light) {
    ...
    if (rlight->axis == light->parms.axis && rlight->end == light->parms.end &&
        ...
        rlight->up == light->parms.up && rlight->shader == light->lightShader &&
...
==================
*/
const renderLight_t* idRenderWorldLocal::GetRenderLight(
    qhandle_t lightHandle) const {
...
void idRenderWorldLocal::ProjectDecalOntoWorld(const idFixedWinding& winding,
  ...
  if (!idRenderModelDecal::CreateProjectionInfo(info, winding, projectionOrigin,
                                                parallel, fadeDepth, material,
                                                startTime)) {
    return;
  }
  ...
  for (i = 0; i < numAreas; i++) {
    ...
    for (ref = area->entityRefs.areaNext; ref != &area->entityRefs;
         ref = ref->areaNext) {
      def = ref->entity;
      ...
      if (def->parms.customShader != NULL &&
          !def->parms.customShader->AllowOverlays()) {
        continue;
      }

      idBounds bounds;
      bounds.FromTransformedBounds(model->Bounds(&def->parms),
                                   def->parms.origin, def->parms.axis);
      ...
      idRenderModelDecal::GlobalProjectionInfoToLocal(
          localInfo, info, def->parms.origin, def->parms.axis);
...
void idRenderWorldLocal::ProjectDecal(qhandle_t entityHandle,
  ...
  idBounds bounds;
  bounds.FromTransformedBounds(model->Bounds(&def->parms), def->parms.origin,
                               def->parms.axis);
...
void idRenderWorldLocal::ProjectOverlay(qhandle_t entityHandle,
  ...
  if (model->IsDynamicModel() !=
      DM_CACHED) {  // FIXME: probably should be MD5 only
    return;
  }
...
void idRenderWorldLocal::RenderScene(const renderView_t* renderView) {
  ...
  if (renderView->fov_x <= 0 || renderView->fov_y <= 0) {
    common->Error("idRenderWorld::RenderScene: bad FOVs: %f, %f",
...
int idRenderWorldLocal::BoundsInAreas(const idBounds& bounds,
  ...
  assert(bounds[1][0] - bounds[0][0] < 1e4f &&
         bounds[1][1] - bounds[0][1] < 1e4f &&
         bounds[1][2] - bounds[0][2] < 1e4f);
...
guiPoint_t idRenderWorldLocal::GuiTrace(qhandle_t entityHandle,
  ...
  model = def->parms.hModel;
  if (def->parms.callback || !def->parms.hModel ||
      def->parms.hModel->IsDynamicModel() != DM_STATIC) {
    return pt;
  }
  ...
  for (j = 0; j < model->NumSurfaces(); j++) {
    ...
    shader = R_RemapShaderBySkin(surf->shader, def->parms.customSkin,
...
bool idRenderWorldLocal::ModelTrace(modelTrace_t& trace,
  ...
  for (i = 0; i < model->NumBaseSurfaces(); i++) {
    ...
    if (collisionSurface) {
      ...
      if (!shader->IsDrawn() || (shader->Coverage() != MC_OPAQUE &&
    ...
    if (localTrace.fraction < trace.fraction) {
      ...
      trace.jointNumber = refEnt->hModel->NearestJoint(i, localTrace.indexes[0],
...
const char* playerModelExcludeList[] = {
    ...
    "models/md5/weapons/pistol_world/worldpistol.md5mesh", NULL};

const char* playerMaterialExcludeList[] = {"muzzlesmokepuff", NULL};
  ...
  for (i = 0; i < numAreas; i++) {
    ...
    for (ref = area->entityRefs.areaNext; ref != &area->entityRefs;
      ...
      if (model->IsDynamicModel() != DM_STATIC) {
        ...
        model = R_EntityDefDynamicModel(def);
        if (!model) {
          continue;  // can happen with particle systems, which don't
                     // instantiate without a valid view
        }
      }
      ...
      if (!traceBounds.IntersectsBounds(bounds) ||
          !bounds.LineIntersection(start, trace.point)) {
        continue;
      }
      ...
      for (j = 0; j < model->NumSurfaces(); j++) {
        ...
        bounds.FromTransformedBounds(tri->bounds, def->parms.origin,
                                     def->parms.axis);
        ...
        if (localTrace.fraction < trace.fraction) {
          ...
          trace.entity = &def->parms;
          trace.jointNumber =
              model->NearestJoint(j, localTrace.indexes[0],
                                  localTrace.indexes[1], localTrace.indexes[2]);
...
void idRenderWorldLocal::RecurseProcBSP_r(modelTrace_t* results,
  ...
  RecurseProcBSP_r(results, nodeNum, node->children[side ^ 1], midf, p2f, mid,
                   p2);
}
...
==================
*/
bool idRenderWorldLocal::FastWorldTrace(modelTrace_t& results,
                                        const idVec3& start,
                                        const idVec3& end) const {
...
void idRenderWorldLocal::GenerateAllInteractions() {
  ...
  common->Printf(
      "idRenderWorld::GenerateAllInteractions, msec = %i, staticAllocCount = "
      "%i.\n",
  ...
  if (r_useInteractionTable.GetBool()) {
    ...
    int size = interactionTableWidth * interactionTableHeight *
    ...
    for (int i = 0; i < this->lightDefs.Num(); i++) {
      ...
      idInteraction* inter;
      for (inter = ldef->firstInteraction; inter != NULL;
           inter = inter->lightNext) {
...
void idRenderWorldLocal::DebugSphere(const idVec4& color,
  ...
  for (i = 15; i <= 360; i += 15) {
    ...
    for (n = 0, j = 15; j <= 360; j += 15, n++) {
      p[0] = sphere.GetOrigin()[0] +
             idMath::Sin16(DEG2RAD(j)) * sphere.GetRadius() * s;
      p[1] = sphere.GetOrigin()[1] +
             idMath::Cos16(DEG2RAD(j)) * sphere.GetRadius() * s;
      p[2] = lastp[2];
...
void idRenderWorldLocal::DebugCone(const idVec4& color,
  ...
  if (radius1 == 0.0f) {
    for (i = 20; i <= 360; i += 20) {
      d = idMath::Sin16(DEG2RAD(i)) * axis[0] +
          idMath::Cos16(DEG2RAD(i)) * axis[1];
      p2 = top + d * radius2;
...
void idRenderWorldLocal::DrawText(const char* text,
                                  ...
                                  const int lifetime,
                                  const bool depthTest) {
  RB_AddDebugText(text, origin, scale, color, viewAxis, align, lifetime,
                  depthTest);
}

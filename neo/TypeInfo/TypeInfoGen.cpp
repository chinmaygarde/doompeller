...
idClassTypeInfo* idTypeInfoGen::ParseClassType(const char* scope,
  ...
  if (src.CheckTokenString(":")) {
    ...
    while (token == "public" || token == "protected" || token == "private") {
      ...
      src.Warning("multiple inheritance not supported for '%s%s'",
  ...
  ParseScope(typeInfo->scope + typeInfo->typeName + "::", typeInfo->isTemplate,
             src, typeInfo);
...
void idTypeInfoGen::ParseScope(const char* scope,
  ...
  while (indent) {
    ...
    } else if (token == "public" || token == "protected" ||
               token == "private") {
...
void idTypeInfoGen::CreateTypeInfo(const char* path) {
  ...
  common->Printf("Type Info Generator v" TYPE_INFO_GEN_VERSION
                 " (c) 2004 id Software\n");
  common->Printf("%s\n", path);
...
void idTypeInfoGen::WriteTypeInfo(const char* fileName) const {
  ...
  file->WriteFloatString(
      ...
      "*/\n"
      "\n",
      define.c_str(), define.c_str(), constants.Num(), enums.Num(),
      classes.Num(), numTemplates, maxInheritance, maxInheritanceClass.c_str());
  ...
  for (i = 0; i < constants.Num(); i++) {
    ...
    file->WriteFloatString("\t{ \"%s\", \"%s\", \"%s\" },\n",
                           info->type.c_str(), info->name.c_str(),
                           info->value.c_str());
  }
  ...
  for (i = 0; i < classes.Num(); i++) {
    ...
    for (j = 0; j < info->variables.Num(); j++) {
      ...
      file->WriteFloatString(
          ...
          "},\n",
          varType, varName, typeName.c_str(), varName, typeName.c_str(),
          varName);
    }
  ...
  for (i = 0; i < classes.Num(); i++) {
    ...
    file->WriteFloatString("\t{ \"%s\", \"%s\", sizeof(%s), %s_typeInfo },\n",
                           typeName.c_str(), info->superType.c_str(),

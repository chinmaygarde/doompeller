...
bool rvDebuggerQuickWatchDlg::DoModal(rvDebuggerWindow* window,
  ...
  DialogBoxParam(window->GetInstance(), MAKEINTRESOURCE(IDD_DBG_QUICKWATCH),
                 window->GetWindow(), DlgProc, (LONG)this);
...
INT_PTR CALLBACK rvDebuggerQuickWatchDlg::DlgProc(HWND wnd,
  ...
  rvDebuggerQuickWatchDlg* dlg =
      (rvDebuggerQuickWatchDlg*)GetWindowLong(wnd, GWL_USERDATA);
  ...
  switch (msg) {
    ...
    case WM_CLOSE:
      ...
      gDebuggerApp.GetOptions().SetColumnWidths(
          "cw_quickwatch", GetDlgItem(wnd, IDC_DBG_CURVALUE));
      EndDialog(wnd, 0);
      break;
    ...
    case WM_SIZE: {
      ...
      MoveWindow(GetDlgItem(wnd, IDC_DBG_RECALC),
                 client.right - dlg->mButtonFromRight, button.top,
                 button.right - button.left, button.bottom - button.top, TRUE);

      GetWindowRect(GetDlgItem(wnd, IDC_DBG_CLOSE), &button);
      ScreenToClient(wnd, (POINT*)&button);
      ScreenToClient(wnd, (POINT*)&button.right);
      MoveWindow(GetDlgItem(wnd, IDC_DBG_CLOSE),
                 client.right - dlg->mButtonFromRight, button.top,
                 button.right - button.left, button.bottom - button.top, TRUE);
      ...
      ScreenToClient(wnd, (POINT*)&button);
      ScreenToClient(wnd, (POINT*)&button.right);
      MoveWindow(GetDlgItem(wnd, IDC_DBG_ADDWATCH),
                 client.right - dlg->mButtonFromRight, button.top,
                 button.right - button.left, button.bottom - button.top, TRUE);
      ...
      ScreenToClient(wnd, (POINT*)&button);
      ScreenToClient(wnd, (POINT*)&button.right);
      MoveWindow(GetDlgItem(wnd, IDC_DBG_VARIABLE), button.left, button.top,
                 client.right - button.left - dlg->mEditFromRight,
                 button.bottom - button.top, TRUE);

      GetWindowRect(GetDlgItem(wnd, IDC_DBG_CURVALUE), &button);
      ScreenToClient(wnd, (POINT*)&button);
      ScreenToClient(wnd, (POINT*)&button.right);
      MoveWindow(GetDlgItem(wnd, IDC_DBG_CURVALUE), button.left, button.top,
                 client.right - button.left - dlg->mEditFromRight,
                 client.bottom - button.top - dlg->mEditFromBottom, TRUE);
    ...
    case WM_INITDIALOG: {
      ...
      gDebuggerApp.GetOptions().GetColumnWidths(
          "cw_quickwatch", GetDlgItem(wnd, IDC_DBG_CURVALUE));
      ...
      switch (LOWORD(wparam)) {
        ...
        case IDC_DBG_VARIABLE:
          ...
          if (HIWORD(wparam) == EN_CHANGE) {
            bool enable = GetWindowTextLength(GetDlgItem(wnd, IDC_DBG_VARIABLE))
                              ? true
                              : false;
...
void rvDebuggerQuickWatchDlg::SetVariable(const char* varname, bool force) {
  ...
  ListView_SetItemText(GetDlgItem(mWnd, IDC_DBG_CURVALUE), 0, 1,
                       (LPSTR)gDebuggerApp.GetClient().GetVariableValue(
                           varname, mCallstackDepth));

  // Give focus back to the variable edit control and set the cursor back to an
  // arrow

...
bool rvDebuggerServer::Initialize(void) {
  ...
  // Get a copy of the game thread handle so we can suspend the thread on a
  // break
  DuplicateHandle(GetCurrentProcess(), GetCurrentThread(), GetCurrentProcess(),
                  &mGameThread, 0, FALSE, DUPLICATE_SAME_ACCESS);
  ...
  while (mPort.GetPacket(adrFrom, msg.data, msg.cursize, msg.maxsize)) {
    ...
    switch (command) {
      ...
      case DBMSG_STEPOVER:
        ...
        mBreakStepOverFunc1 =
            mBreakInterpreter
                ->GetCallstack()[mBreakInterpreter->GetCallstackDepth()]
                .f;
        if (mBreakInterpreter->GetCallstackDepth() > 0) {
          mBreakStepOverFunc2 =
              mBreakInterpreter
                  ->GetCallstack()[mBreakInterpreter->GetCallstackDepth() - 1]
                  .f;
        } else {
          mBreakStepOverFunc2 = NULL;
        }
...
void rvDebuggerServer::Break(idInterpreter* interpreter,
  ...
  // game thread execution wont return until after the thread is resumed

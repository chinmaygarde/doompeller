...
void PDAEditorInit(const idDict* spawnArgs) {
  ...
  if (g_PDAEditorDialog->GetSafeHwnd() == NULL) {
    ...
    /*
                    ...
                    CRect rct;
                    g_PDAEditorDialog->SetWindowPos( NULL, rct.left, rct.top,
       0,0, SWP_NOSIZE );
    */
  }
  ...
  if (spawnArgs) {
    ...
    idDeclPDA* decl = static_cast<idDeclPDA*>(
        const_cast<idDecl*>(declManager->FindType(DECL_PDA, name)));
...
void CDialogPDAEditor::OnActivate(UINT nState,
                                  CWnd* pWndOther,
                                  BOOL bMinimized) {
...
void CDialogPDAEditor::PopulatePDAList() {
  ...
  for (i = 0; i < num; i++) {
    const idDeclPDA* pda =
        dynamic_cast<const idDeclPDA*>(declManager->DeclByIndex(DECL_PDA, i));
...
void CDialogPDAEditor::OnSelChangePDA() {
  ...
  const idDeclPDA* pda =
      dynamic_cast<const idDeclPDA*>(declManager->DeclByIndex(DECL_PDA, index));
  if (!pda) {
    return;
  }
...
void CDialogPDAEditor::OnBtnClickedPDAAdd() {
  ...
  if (dlg.DoModal() == IDOK) {
    dlg.name.MakeLower();
    idDecl* decl = declManager->CreateNewDecl(DECL_PDA, dlg.name,
                                              "newpdas/" + dlg.name + ".pda");
...
void CDialogPDAEditor::OnBtnClickedEmailAdd() {
  ...
  if (pda) {
    ...
    if (addDlg.DoModal() == IDOK) {
      idDeclEmail* email = static_cast<idDeclEmail*>(
          declManager->CreateNewDecl(DECL_EMAIL, name, pda->GetFileName()));
      ...
      const idDeclEmail* emailConst = static_cast<const idDeclEmail*>(
          declManager->FindType(DECL_EMAIL, name));
      emailList.AddString(emailConst->GetSubject());

      // Save the pda to include this email in the list
      // This has a side-effect of saving any other changes, but I don't really
      // care right now
      OnBtnClickedSave();
    }
  }
...
void CDialogPDAEditor::OnBtnClickedEmailEdit() {
  ...
  if (pda) {
    ...
    if (editDlg.DoModal() == IDOK) {
      idDeclEmail* email =
          const_cast<idDeclEmail*>(pda->GetEmailByIndex(index));

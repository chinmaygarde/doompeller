...
const idMaterial* CNewTexWnd::NextPos() {
  ...
  int width = mat->GetEditorImage()->uploadWidth *
              ((float)g_PrefsDlg.m_nTextureScale / 100);
  int height = mat->GetEditorImage()->uploadHeight *
               ((float)g_PrefsDlg.m_nTextureScale / 100);
...
void CNewTexWnd::OnPaint() {
  ...
  if (!qwglMakeCurrent(dc.GetSafeHdc(), win32.hGLRC)) {
    ...
    qglDisable(GL_BLEND);
    qglOrtho(0, rectClient.Width(), origin.y - rectClient.Height(), origin.y,
             -100, 100);
    ...
    while (1) {
      ...
      if ((draw.y - height - FONT_HEIGHT < origin.y) &&
          (draw.y > origin.y - rectClient.Height())) {
        ...
        // Draw the texture
        float fScale = (g_PrefsDlg.m_bHiColorTextures == TRUE)
                           ? ((float)g_PrefsDlg.m_nTextureScale / 100)
                           : 1.0;
        ...
        if (!idStr::Icmp(g_qeglobals.d_texturewin.texdef.name,
                         mat->GetName())) {
          qglLineWidth(3);
          qglColor3f(1, 0, 0);
          globalImages->BindNull();
        ...
        for (name = mat->GetName(); *name && *name != '/' && *name != '\\';
             name++) {
          ;
        }
  ...
  if (g_PrefsDlg.m_bTextureScrollbar &&
      (m_bNeedRange || g_qeglobals.d_texturewin.m_nTotalHeight != nOld)) {
    m_bNeedRange = false;
...
void CNewTexWnd::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) {
  ...
  switch (nSBCode) {
    ...
    case SB_PAGEUP: {
      n = (n - g_qeglobals.d_texturewin.height > 0)
              ? n - g_qeglobals.d_texturewin.height
              : 0;
      break;
    }

    case SB_PAGEDOWN: {
      n = (n + g_qeglobals.d_texturewin.height <
           g_qeglobals.d_texturewin.m_nTotalHeight)
              ? n + g_qeglobals.d_texturewin.height
              : n;
      break;
    }
...
BOOL CNewTexWnd::Create(LPCTSTR lpszClassName,
  ...
  BOOL ret = CWnd::Create(lpszClassName, lpszWindowName, dwStyle, rect,
...
const idMaterial* CNewTexWnd::getMaterialAtPoint(CPoint point) {
  ...
  while (1) {
    ...
    if (point.x > draw.x && point.x - draw.x < width && my < draw.y &&
...
void Texture_SetTexture(texdef_t* texdef,
  ...
  if (g_Inspectors->mediaDlg.IsWindowVisible()) {
    g_Inspectors->mediaDlg.SelectCurrentItem(
        true, g_qeglobals.d_texturewin.texdef.name, CDialogTextures::MATERIALS);
  }
...
void Texture_ShowInuse(void) {
  ...
  brush_t* b;
  for (b = active_brushes.next; b != NULL && b != &active_brushes;
       b = b->next) {
    if (b->pPatch) {
...
void CNewTexWnd::EnsureTextureIsVisible(const char* name) {
  ...
  while (1) {
    ...
    if (!idStr::Icmp(name, mat->GetName())) {
      ...
      if (current.y - height - 2 * FONT_HEIGHT <
          origin.y - rectClient.Height()) {

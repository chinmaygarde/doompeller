...
idCVar radiant_entityMode("radiant_entityMode",
                          "0",
                          CVAR_TOOL | CVAR_ARCHIVE,
                          "");
...
BOOL CRadiantApp::InitInstance() {
  ...
  if (Finder.FindFile(IniPath)) {
    ...
    // look for the registry key for void* buffers storage ( these can't go into
    // .INI files )
    int i = 0;
    CString key;
    HKEY hkResult;
    ...
    do {
      ...
      if (RegOpenKeyEx(HKEY_CURRENT_USER, key, 0, KEY_ALL_ACCESS, &hkResult) !=
          ERROR_SUCCESS) {
        ...
        RegCreateKeyEx(HKEY_CURRENT_USER, key, 0, NULL, REG_OPTION_NON_VOLATILE,
                       KEY_ALL_ACCESS, NULL, &hkResult, &dwDisp);
        RegSetValueEx(hkResult, "RadiantName", 0, REG_SZ,
                      reinterpret_cast<CONST BYTE*>(RadiantPath),
                      strlen(RadiantPath) + 1);
        ...
        RegQueryValueEx(hkResult, "RadiantName", 0, &type,
                        reinterpret_cast<BYTE*>(RadiantAux), &size);
        RegCloseKey(hkResult);
...
void CRadiantApp::OnHelp() {
  ShellExecute(m_pMainWnd->GetSafeHwnd(), "open", "http://www.idDevNet.com",
               NULL, NULL, SW_SHOW);
}
...
void Sys_UpdateStatusBar(void) {
  ...
  sprintf(numbrushbuffer, "Brushes: %d Entities: %d", g_numbrushes,
          g_numentities);

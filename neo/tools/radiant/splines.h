...
class idPointListInterface {
 public:
  ...
  int selectPointByRay(const idVec3& origin,
...
class idSplineList {
 ...
 public:
  ...
  void setActiveSegment(int i) { /* assert(i >= 0 && (splinePoints.Num() > 0 &&
                                    i < splinePoints.Num())); */
    activeSegment = i;
  }
  int numSegments() { return splinePoints.Num(); }

  void setColors(idVec4& path,
                 idVec4& segment,
                 idVec4& control,
                 idVec4& active);
...
class idCameraPosition : public idPointListInterface {
 public:
  ...
  // this can be done with RTTI syntax but i like the derived classes setting a
  // type makes serialization a bit easier to see
  //

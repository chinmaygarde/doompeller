...
void CPropertyList::MeasureItem(LPMEASUREITEMSTRUCT lpMeasureItemStruct) {
  if (measureItem && !measureItem->m_curValue.IsEmpty()) {
    ...
    CDC* dc = GetDC();
    dc->DrawText(measureItem->m_curValue, rect,
                 DT_CALCRECT | DT_LEFT | DT_WORDBREAK);
    ReleaseDC(dc);
    lpMeasureItemStruct->itemHeight =
        (rect.Height() >= 20) ? rect.Height() : 20;  // pixels
  } else {
...
void CPropertyList::DrawItem(LPDRAWITEMSTRUCT lpDIS) {
  ...
  if (nIndex != (UINT)-1) {
    ...
    dc.DrawText(
        ...
        DT_LEFT | (pItem->m_nItemType == PIT_VAR) ? DT_WORDBREAK
                                                  : DT_SINGLELINE);
  }
  dc.Detach();
}
...
void CPropertyList::OnSelchange() {
  ...
  if (pItem->m_nItemType == PIT_COMBO) {
    ...
    if (m_cmbBox) {
      ...
      rect.bottom += 300;
      m_cmbBox.Create(
          CBS_DROPDOWNLIST | WS_VSCROLL | WS_VISIBLE | WS_CHILD | WS_BORDER,
    ...
    if (m_editBox) {
      ...
      m_editBox.Create(
          ES_LEFT | ES_AUTOHSCROLL | WS_VISIBLE | WS_CHILD | WS_BORDER, rect,
...
void CPropertyList::DisplayButton(CRect region) {
  ...
  if (m_btnCtrl) {
    ...
    m_btnCtrl.Create("...", BS_PUSHBUTTON | WS_VISIBLE | WS_CHILD, region, this,
                     IDC_PROPBTNCTRL);
...
void CPropertyList::OnButton() {
  ...
  if (pItem->m_nItemType == PIT_COLOR) {
    ...
    COLORREF cr = (int)(color.x * 255) + (((int)(color.y * 255)) << 8) +
                  (((int)(color.z * 255)) << 16);
    ...
    if (currPath.GetLength() > 0) {
      FileDlg.m_ofn.lpstrInitialDir =
          currPath.Left(currPath.GetLength() - currPath.ReverseFind('\\'));
    }

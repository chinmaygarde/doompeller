...
void Face_TextureVectors(face_t* f, float STfromXYZ[2][4]) {
  ...
  //
  // ++timo when playing with patches, this sometimes get called and the Warning
  // is displayed find some way out ..
  //
  if (g_qeglobals.m_bBrushPrimitMode && !g_qeglobals.bNeedConvert) {
    common->Printf(
        "Warning : illegal call of Face_TextureVectors in brush primitive "
        "mode\n");
  }
#endif
  td = &f->texdef;
...
idWinding* Brush_MakeFaceWinding(brush_t* b,
  ...
  for (clip = b->brush_faces; clip && w; clip = clip->next) {
    ...
    if (DotProduct(face->plane, clip->plane) > 0.999f &&
        idMath::Fabs(face->plane[3] - clip->plane[3]) <
            0.01f) {  // identical plane, use the later one
      if (past) {
        delete w;
...
Brush_Build
  ...
  TTimo added a bConvert flag to convert between old and new brush texture
formats TTimo brush grouping: update the group treeview if necessary
================
*/
void Brush_Build(brush_t* b,
  ...
  if (!g_qeglobals.m_bBrushPrimitMode && bConvert) {
    common->Printf(
        "Warning : conversion from brush primitive to old brush format not "
        "implemented\n");
  }
#endif
  //
  // if bConvert is set and g_qeglobals.bNeedConvert is not, that just means we
  // need convert for this brush only
  //
  ...
  Brush_BuildWindings(b, bSnap, EntityHasModel(b->owner) || b->pPatch,
                      updateLights);
...
Brush_BestSplitFace
  ...
  returns the best face to split the brush with. return NULL if the brush is
convex
================
*/
...
int Brush_Convex(brush_t* b) {
  ...
  for (face1 = b->brush_faces; face1; face1 = face1->next) {
    ...
    for (face2 = b->brush_faces; face2; face2 = face2->next) {
      ...
      if (face1->face_winding->PlanesConcave(
              *face2->face_winding, face1->plane.Normal(),
              face2->plane.Normal(), -face1->plane[3], -face2->plane[3])) {
        return false;
      }
    }
  }
...
int Brush_MoveVertex(brush_t* b,
  ...
  while (!done) {
      ...
      for (i = 0; i < w->GetNumPoints(); i++) {
        if (start.Compare((*w)[i].ToVec3(), TINY_EPSILON)) {
          ...
          if (dot > TINY_EPSILON) {
            ...
            VectorCopy((*w)[(i - 2 + w->GetNumPoints()) % w->GetNumPoints()],
                       tmpw[0]);
            VectorCopy((*w)[(i - 1 + w->GetNumPoints()) % w->GetNumPoints()],
                       tmpw[1]);
    ...
    //
    // now movefaces contains pointers to triangle faces that contain the to be
    // moved vertex
    //
    done = true;
    VectorCopy(end, mid);
    ...
    for (face = b->brush_faces; face; face = face->next) {
      ...
      if (j >= nummovefaces) {
        ...
        if (!plane.FromPoints(tmpw[0].ToVec3(), tmpw[1].ToVec3(),
                              tmpw[2].ToVec3(), false)) {
          VectorCopy((*w)[(k + 2) % w->GetNumPoints()], tmpw[2]);
          if (!plane.FromPoints(tmpw[0].ToVec3(), tmpw[1].ToVec3(),
                                tmpw[2].ToVec3()),
    ...
    if (!result || !Brush_Convex(b)) {
      for (i = 0; i < nummovefaces; i++) {
        ...
        // create new face plane
        for (j = 0; j < 3; j++) {
          VectorCopy((*movefaces[i]->face_winding)[j],
                     movefaces[i]->planepts[j]);
        }
    ...
    for (i = 0; i < nummovefaces; i++) {
      ...
      for (j = 0; j < movefaces[i]->face_winding->GetNumPoints(); j++) {
        EmitTextureCoordinates((*movefaces[i]->face_winding)[j],
                               movefaces[i]->d_texture, movefaces[i]);
      }
    }
    ...
    for (face = b->brush_faces; face; face = nextface) {
      ...
      w = face->face_winding->TryMerge(*face->original->face_winding,
                                       face->plane.Normal(), true);
      if (!w) {
        lastface = face;
        continue;
      }
      ...
      for (j = 0; j < face->original->face_winding->GetNumPoints(); j++) {
        EmitTextureCoordinates((*face->original->face_winding)[j],
                               face->original->d_texture, face->original);
      }
...
int Brush_InsertVertexBetween(brush_t* b, idVec3 p1, idVec3 p2) {
  ...
  for (face = b->brush_faces; face; face = face->next) {
    ...
    for (i = 0; i < w->GetNumPoints(); i++) {
      ...
      if (p2.Compare((*w)[(i + 1) % w->GetNumPoints()].ToVec3(),
                     TINY_EPSILON)) {
        neww = new idWinding(*w);
        neww->InsertPoint(point, (i + 1) % w->GetNumPoints());
        break;
      } else if (p2.Compare(
                     (*w)[(i - 1 + w->GetNumPoints()) % w->GetNumPoints()]
                         .ToVec3(),
                     TINY_EPSILON)) {
        neww = new idWinding(*w);
...
Brush_Parse
  ...
  FIXME: when using old brush primitives, the test loop for "Brush" and
"patchDef2" "patchDef3" run before each face parsing. It works, but it's a
performance hit
================
*/
brush_t* Brush_Parse(idVec3 origin) {
  ...
  do {
    ...
    if (idStr::Icmp(token, "brushDef") == 0 ||
        idStr::Icmp(token, "brushDef2") == 0 ||
      ...
      if (g_qeglobals.m_bBrushPrimitMode) {
        if (g_qeglobals.bOldBrushes) {
          common->Printf(
              "Warning : old brushes and brush primitive in the same file are "
              "not allowed ( Brush_Parse )\n");
        }
      } else {
        // ++Timo write new brush primitive -> old conversion code for Q3->Q2
        // conversions ?
        common->Printf(
            "Warning : conversion code from brush primitive not done ( "
            "Brush_Parse )\n");
      }
    ...
    if (idStr::Icmp(token, "patchDef2") == 0 ||
        idStr::Icmp(token, "patchDef3") == 0) {
      Brush_Free(b);
      ...
      //
      // add the brush to the end of the chain, so loading and saving a map
...
QERApp_MapPrintf_FILE
  ...
  callback for surface properties plugin must fit a PFN_QERAPP_MAPPRINTF ( see
isurfaceplugin.h ) carefully initialize !
================
*/
FILE* g_File;
...
void Brush_Write(brush_t* b, FILE* f, const idVec3& origin, bool newFormat) {
  ...
  if (g_qeglobals.m_bBrushPrimitMode) {
    ...
    for (fa = b->brush_faces; fa; fa = fa->next) {
      ...
      if (newFormat) {
        ...
        if (fa->dirty) {
          ...
          plane.FromPoints(fa->planepts[0], fa->planepts[1], fa->planepts[2],
                           false);
          fa->planepts[0] += origin;
          fa->planepts[1] += origin;
          fa->planepts[2] += origin;
      ...
      WriteFileString(f, "( ( ");
      for (i = 0; i < 3; i++) {
        if (fa->brushprimit_texdef.coords[0][i] ==
            static_cast<int>(fa->brushprimit_texdef.coords[0][i])) {
          WriteFileString(
              f, "%i ", static_cast<int>(fa->brushprimit_texdef.coords[0][i]));
        } else {
      ...
      WriteFileString(f, ") ( ");
      for (i = 0; i < 3; i++) {
        if (fa->brushprimit_texdef.coords[1][i] ==
            static_cast<int>(fa->brushprimit_texdef.coords[1][i])) {
          WriteFileString(
              f, "%i ", static_cast<int>(fa->brushprimit_texdef.coords[1][i]));
        } else {
    ...
    for (fa = b->brush_faces; fa; fa = fa->next) {
      ...
      WriteFileString(f, "%s %i %i %i ", pname, (int)fa->texdef.shift[0],
                      (int)fa->texdef.shift[1], (int)fa->texdef.rotate);
...
void Brush_Write(brush_t* b,
  ...
  if (g_qeglobals.m_bBrushPrimitMode) {
    ...
    for (fa = b->brush_faces; fa; fa = fa->next) {
      if (newFormat) {
        ...
        for (i = 0; i < 3; i++) {
          ...
          for (int j = 0; j < 3; j++) {
            if (fa->planepts[i][j] == static_cast<int>(fa->planepts[i][j])) {
              MemFile_fprintf(pMemFile, "%i ",
                              static_cast<int>(fa->planepts[i][j]));
            } else {
    ...
    for (fa = b->brush_faces; fa; fa = fa->next) {
      ...
      MemFile_fprintf(pMemFile, "%s %i %i %i ", pname, (int)fa->texdef.shift[0],
                      (int)fa->texdef.shift[1], (int)fa->texdef.rotate);
...
brush_t* Brush_Create(idVec3 mins, idVec3 maxs, texdef_t* texdef) {
  ...
  //
  // brush primitive mode : convert texdef to brushprimit_texdef ? most of the
  // time texdef is empty
  //
  for (i = 0; i < 3; i++) {
    if (maxs[i] < mins[i]) {
...
Brush_CreatePyramid
  ...
  Create non-textured pyramid for light entities The brush is NOT linked to any
list
================
*/
...
void Brush_MakeSided(int sides) {
  ...
  for (i = 0; i < sides; i++) {
    ...
    f->planepts[0][(axis + 1) % 3] =
        floor(mid[(axis + 1) % 3] + width * cv + 0.5f);
    f->planepts[0][(axis + 2) % 3] =
        floor(mid[(axis + 2) % 3] + width * sv + 0.5f);
    f->planepts[0][axis] = mins[axis];
    ...
    f->planepts[2][(axis + 1) % 3] =
        floor(f->planepts[0][(axis + 1) % 3] - width * sv + 0.5f);
    f->planepts[2][(axis + 2) % 3] =
        floor(f->planepts[0][(axis + 2) % 3] + width * cv + 0.5f);
    f->planepts[2][axis] = maxs[axis];
  }
...
Brush_Free
  ...
  set bRemoveNode to false to avoid trying to delete the item in group view tree
control
================
*/
...
int Face_MemorySize(face_t* f) {
  ...
  if (f->face_winding) {
    size += sizeof(idWinding) +
            f->face_winding->GetNumPoints() * sizeof((f->face_winding)[0]);
  }
  size += sizeof(face_t);
  return size;
}
...
extern bool Patch_Intersect(patchMesh_t* pm,
                            idVec3 origin,
                            idVec3 direction,
                            float& scale);
extern bool RayIntersectsTri(const idVec3& origin,
  ...
  if (model) {
    if (model->IsDynamicModel() != DM_STATIC) {
      if (dynamic_cast<idRenderModelMD5*>(model)) {
        ...
        model = gameEdit->ANIM_CreateMeshForAnim(md5, classname, anim, frame,
                                                 false);
        if (!model) {
      ...
      if (!Patch_Intersect(b->pPatch, origin, dir, scale)) {
        *dist = 0;
        return NULL;
               ...
               dynamic_cast<idRenderModelPrt*>(b->modelHandle) == NULL &&
...
SetFaceTexdef
  ...
  NOTE: never trust f->d_texture here, f->texdef and f->d_texture are out of
sync when called by Brush_SetTexture use Texture_ForName() to find the right
shader
  ...
  get ->Copy() of it into the face ( and remember to hook ) if NULL, ask for a
default
================
*/
void SetFaceTexdef(brush_t* b,
  ...
  if (g_qeglobals.m_bBrushPrimitMode) {
    f->texdef = *texdef;
    ConvertTexMatWithQTexture(brushprimit_texdef, NULL, &f->brushprimit_texdef,
...
Brush_SelectFaceForDragging
  ...
  Adds the faces planepts to move_points, and rotates and adds the planepts of
adjacent face if shear is set
================
*/
void Brush_SelectFaceForDragging(brush_t* b, face_t* f, bool shear) {
  ...
  for (b2 = selected_brushes.next; b2 != &selected_brushes; b2 = b2->next) {
    ...
    for (f2 = b2->brush_faces; f2; f2 = f2->next) {
      for (i = 0; i < 3; i++) {
        if (idMath::Fabs(DotProduct(f2->planepts[i], f->plane) + f->plane[3]) >
            ON_EPSILON) {
          break;
        }
      }
  ...
  //
  // if shearing, take all the planes adjacent to selected faces and rotate
  // their points so the edge clipped by a selcted face has two of the points
  //
  if (!shear) {
    return;
  }
...
Brush_SideSelect
  ...
  The mouse click did not hit the brush, so grab one or more side planes for
dragging.
================
*/
...
void Brush_UpdateLightPoints(brush_t* b, const idVec3& offset) {
  ...
  idVec3 vCenter;
  idVec3* origin =
      (b->trackLightOrigin) ? &b->owner->lightOrigin : &b->owner->origin;
  ...
  if (str && *str) {
    ...
    UpdateSelectablePoint(b, Brush_TransformedPoint(b, b->lightRight),
                          LIGHT_RIGHT);
    UpdateSelectablePoint(b, Brush_TransformedPoint(b, b->lightTarget),
                          LIGHT_TARGET);
    UpdateSelectablePoint(b, Brush_TransformedPoint(b, b->lightStart),
                          LIGHT_START);
    ...
    if (makeFacePlanes) {
      ...
      if (g_qeglobals.m_bBrushPrimitMode) {
        if (g_qeglobals.bNeedConvert) {
          //
          // we have parsed old brushes format and need conversion convert old
          // brush texture representation to new format
          //
        ...
        //
        // use new texture representation to compute texture coordinates in
        // debug mode we will check against old code and warn if there are
        // differences
        //
...
void Brush_SnapToGrid(brush_t* pb) {
  ...
  if (GetVectorForKey(pb->owner, "origin", v)) {
    ...
    sprintf(str, "%i %i %i", (int)pb->owner->origin.x, (int)pb->owner->origin.y,
...
extern void VectorRotate3Origin(const idVec3& vIn,
...
================
*/
static void FacingVectors(entity_t* e,
                          idVec3& forward,
                          idVec3& right,
                          idVec3& up) {
  idAngles angles;
...
void DrawSpeaker(brush_t* b, bool bSelected, bool twoD) {
  if (!(g_qeglobals.d_savedinfo.showSoundAlways ||
        (g_qeglobals.d_savedinfo.showSoundWhenSelected && bSelected))) {
    return;
  }
  ...
  if (twoD) {
    if (bSelected) {
      qglColor4f(g_qeglobals.d_savedinfo.colors[COLOR_SELBRUSHES].x,
                 g_qeglobals.d_savedinfo.colors[COLOR_SELBRUSHES].y,
                 g_qeglobals.d_savedinfo.colors[COLOR_SELBRUSHES].z, .5);
    } else {
      qglColor4f(b->owner->eclass->color.x, b->owner->eclass->color.y,
                 b->owner->eclass->color.z, .5);
    }
    ...
    if (bSelected) {
      qglColor4f(g_qeglobals.d_savedinfo.colors[COLOR_SELBRUSHES].x,
                 g_qeglobals.d_savedinfo.colors[COLOR_SELBRUSHES].y,
                 g_qeglobals.d_savedinfo.colors[COLOR_SELBRUSHES].z, 1);
    } else {
      qglColor4f(b->owner->eclass->color.x, b->owner->eclass->color.y,
                 b->owner->eclass->color.z, 1);
    }
    ...
    if (bSelected) {
      qglColor4f(g_qeglobals.d_savedinfo.colors[COLOR_SELBRUSHES].x,
                 g_qeglobals.d_savedinfo.colors[COLOR_SELBRUSHES].y,
                 g_qeglobals.d_savedinfo.colors[COLOR_SELBRUSHES].z, 0.35f);
    } else {
      qglColor4f(b->owner->eclass->color.x, b->owner->eclass->color.y,
                 b->owner->eclass->color.z, 0.35f);
    }
    gluSphere(qobj, min, 8, 8);
    if (bSelected) {
      qglColor4f(g_qeglobals.d_savedinfo.colors[COLOR_SELBRUSHES].x,
                 g_qeglobals.d_savedinfo.colors[COLOR_SELBRUSHES].y,
                 g_qeglobals.d_savedinfo.colors[COLOR_SELBRUSHES].z, 0.1f);
    } else {
      qglColor4f(b->owner->eclass->color.x, b->owner->eclass->color.y,
                 b->owner->eclass->color.z, 0.1f);
    }
    gluSphere(qobj, max, 8, 8);
  ...
  if (model) {
    ...
    if (model->IsDynamicModel() != DM_STATIC) {
      if (dynamic_cast<idRenderModelMD5*>(model)) {
        ...
        model2 = gameEdit->ANIM_CreateMeshForAnim(model, classname, anim, frame,
                                                  false);
      } else if (dynamic_cast<idRenderModelPrt*>(model) ||
                 dynamic_cast<idRenderModelLiquid*>(model)) {
        fixedBounds = true;
      }
    ...
    if (bSelected) {
      qglColor3fv(
          g_qeglobals.d_savedinfo.colors[COLOR_SELBRUSHES].ToFloatPtr());
    }
...
void Brush_DrawAxis(brush_t* b) {
  if (g_pParentWnd->ActiveXY()->RotateMode() && b->modelHandle) {
    ...
    idBounds bo;
    bo.FromTransformedBounds(b->modelHandle->Bounds(), b->owner->origin,
                             b->owner->rotation);
    ...
    GLTransformedCircle(0, b->owner->origin, xr, mat, 1.25, idVec3(0, 0, 1),
                        dist);
    GLTransformedCircle(1, b->owner->origin, yr, mat, 1.25, idVec3(0, 1, 0),
                        dist);
    GLTransformedCircle(2, b->owner->origin, zr, mat, 1.25, idVec3(1, 0, 0),
                        dist);
    ...
    GLTransformedCircle(type, org, wr * 1.03f, mat, 1.45f, idVec3(1, 1, 1),
                        dist);
  }
}
...
void Brush_DrawEnv(brush_t* b, bool cameraView, bool bSelected) {
  ...
  idRenderModel* model =
      gameEdit->AF_CreateMesh(b->owner->epairs, origin, axis, poseIsSet);
...
void Brush_Draw(brush_t* b, bool bSelected) {
  ...
  for (face = b->brush_faces, order = 0; face; face = face->next, order++) {
    ...
    if ((nDrawMode == cd_texture || nDrawMode == cd_light) &&
        face->d_texture != prev && !b->forceWireFrame) {
    ...
    if (model) {
      ...
      qglColor4f(face->d_color.x, face->d_color.y, face->d_color.z,
                 face->d_texture->GetEditorAlpha());
    }
    ...
    for (i = 0; i < w->GetNumPoints(); i++) {
      if (!b->forceWireFrame &&
          (nDrawMode == cd_texture || nDrawMode == cd_light)) {
...
void Brush_DrawCurve(brush_t* b, bool bSelected, bool cam) {
  ...
  for (i = 0; i < maxage; i++) {
    ...
    /*
                    if ( cam ) {
                            idSurface_SweptSpline *ss = SplineToSweptSpline(
       b->owner->curve ); if ( ss ) { idMaterial *mat =
       declManager->FindMaterial( "_default" ); mat->GetEditorImage()->Bind();
                                    qglPolygonMode( GL_FRONT_AND_BACK, GL_FILL
       ); qglBegin( GL_TRIANGLES ); const int *indexes = ss->GetIndexes(); const
       idDrawVert *verts = ss->GetVertices(); for ( j = 0; j <
       ss->GetNumIndexes(); j += 3 ) { for ( k = 0; k < 3; k++ ) { int	index =
       indexes[ j + 2 - k ]; float f = ShadeForNormal( verts[index].normal  );
                                                    qglColor3f( f, f, f );
                                                    qglTexCoord2fv(
       verts[index].st.ToFloatPtr() ); qglVertex3fv(
       verts[index].xyz.ToFloatPtr() );
                                            }
                                    }
                                    qglEnd();
                                    delete ss;
    ...
    */
    /*			qglPointSize( 1.0f );
       ...
       { idVec3 v = b->owner->curve->GetCurrentValue( start ); qglVertex3f( v.x,
       v.y, v.z ); start += inc;
                                    }
                            }*/
...
void Brush_DrawXY(brush_t* b,
  ...
  for (face = b->brush_faces, order = 0; face; face = face->next, order++) {
    ...
    /*
                    for (i = 0; i < 3; i++) {
                            glLabeledPoint(idVec4(1, 0, 0, 1),
       face->planepts[i], 3, va("%i", i));
                    }
    */
  }
...
void Brush_Move(brush_t* b, const idVec3 move, bool bSnap, bool updateOrigin) {
  ...
  if (b->owner->eclass->fixedsize || EntityHasModel(b->owner) ||
      (updateOrigin && GetVectorForKey(b->owner, "origin", temp))) {
    //		if (!b->entityModel) {
    bool adjustOrigin = true;
    if (b->trackLightOrigin) {
      b->owner->lightOrigin += move;
      sprintf(text, "%i %i %i", (int)b->owner->lightOrigin[0],
              (int)b->owner->lightOrigin[1], (int)b->owner->lightOrigin[2]);
    ...
    if (adjustOrigin && updateOrigin) {
      b->owner->origin += move;
      if (g_moveOnly) {
        sprintf(text, "%g %g %g", b->owner->origin[0], b->owner->origin[1],
                b->owner->origin[2]);
      } else {
        sprintf(text, "%i %i %i", (int)b->owner->origin[0],
                (int)b->owner->origin[1], (int)b->owner->origin[2]);
      }
    ...
    if (b->owner->eclass->nShowFlags & ECLASS_ENV) {
      ...
      idAngles ang;
      while (arg) {
        sscanf(arg->GetValue(), "%f %f %f %f %f %f", &org.x, &org.y, &org.z,
...
void Brush_Print(brush_t* b) {
  ...
  for (face_t* f = b->brush_faces; f; f = f->next) {
    ...
    common->Printf("%f %f %f\n", f->planepts[0][0], f->planepts[0][1],
                   f->planepts[0][2]);
    common->Printf("%f %f %f\n", f->planepts[1][0], f->planepts[1][1],
                   f->planepts[1][2]);
    common->Printf("%f %f %f\n", f->planepts[2][0], f->planepts[2][1],
                   f->planepts[2][2]);
  }
}
...
Brush_MakeSidedCone
  ...
  Makes the current brush have the given number of 2d sides and turns it into a
cone
================
*/
...
Brush_MakeSidedSphere
  ...
  Makes the current brushhave the given number of 2d sides and turns it into a
sphere
================
*/
void Brush_MakeSidedSphere(int sides) {
  ...
  for (i = 0; i < sides; i++) {
    for (j = 0; j < sides - 1; j++) {
      ...
      f->planepts[0] = idPolar3(radius, idMath::TWO_PI * i / sides,
                                idMath::PI * ((float)(j) / sides - 0.5f))
                           .ToVec3() +
                       mid;
      f->planepts[1] = idPolar3(radius, idMath::TWO_PI * i / sides,
                                idMath::PI * ((float)(j + 1) / sides - 0.5f))
                           .ToVec3() +
                       mid;
      f->planepts[2] = idPolar3(radius, idMath::TWO_PI * (i + 1) / sides,
                                idMath::PI * ((float)(j + 1) / sides - 0.5f))
                           .ToVec3() +
                       mid;
    }
...
extern void Face_FitTexture_BrushPrimit(face_t* f,
                                        idVec3 mins,
                                        idVec3 maxs,
                                        float nHeight,
                                        float nWidth);
...
void Face_FitTexture(face_t* face, float nHeight, float nWidth) {
  if (g_qeglobals.m_bBrushPrimitMode) {
    ...
    /*
     * winding_t *w; idBounds bounds; int i; float width, height, temp; float
     * rot_width, rot_height; float cosv,sinv,ang; float min_t, min_s, max_t,
     * max_s; float s,t; idVec3 vecs[2]; idVec3 coords[4]; texdef_t *td; if
     * (nHeight < 1) { nHeight = 1; } if (nWidth < 1) { nWidth = 1; }
     * bounds.Clear(); td = &face->texdef; w = face->face_winding; if (!w) {
     * return; } for (i=0 ; i<w->numpoints ; i++) { bounds.AddPoint( w->p[i] );
     * } // // get the current angle // ang = td->rotate / 180 * Q_PI; sinv =
     * sin(ang); cosv = cos(ang); // get natural texture axis
     * TextureAxisFromPlane(&face->plane, vecs[0], vecs[1]); min_s = DotProduct(
     * bounds.b[0], vecs[0] ); min_t = DotProduct( bounds.b[0], vecs[1] ); max_s
     * = DotProduct( bounds.b[1], vecs[0] ); max_t = DotProduct( bounds.b[1],
     * vecs[1] ); width = max_s - min_s; height = max_t - min_t; coords[0][0] =
     * min_s; coords[0][1] = min_t; coords[1][0] = max_s; coords[1][1] = min_t;
     * coords[2][0] = min_s; coords[2][1] = max_t; coords[3][0] = max_s;
     * coords[3][1] = max_t; min_s = min_t = 999999; max_s = max_t = -999999;
     * for (i=0; i<4; i++) { s = cosv
     * * coords[i][0] - sinv * coords[i][1]; t = sinv * coords[i][0] + cosv *
     * coords[i][1]; if (i&1) { if (s > max_s) { max_s = s; } } else { if (s <
     * min_s) { min_s = s; } if (i<2) { if (t < min_t) { min_t = t; } } else {
     * if (t > max_t) { max_t = t; } } } } rot_width = (max_s - min_s);
     * rot_height = (max_t - min_t); td->scale[0] =
     ...
     * (temp+1)*face->d_texture->GetEditorImage()->uploadWidth*nWidth;
     * td->shift[0] = (int)(temp -

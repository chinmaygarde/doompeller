...
void CEntityListDlg::OnSelect() {
  ...
  if (index != LB_ERR) {
    entity_t* ent =
        reinterpret_cast<entity_t*>(listEntities.GetItemDataPtr(index));
    if (ent) {
      Select_Deselect();
...
void CEntityListDlg::UpdateList() {
  ...
  for (entity_t* pEntity = entities.next; pEntity != &entities;
  ...
  if (index != LB_ERR) {
    ...
    if (pEntity) {
      ...
      for (int i = 0; i < count; i++) {
        int nParent =
            m_lstEntity.InsertItem(0, pEntity->epairs.GetKeyVal(i)->GetKey());
        m_lstEntity.SetItem(nParent, 1, LVIF_TEXT,
                            pEntity->epairs.GetKeyVal(i)->GetValue(), 0, 0, 0,
                            reinterpret_cast<DWORD>(pEntity));
      }
    }
  }
}

...
void CEditViewDlg::OnSize(UINT nType, int cx, int cy) {
  ...
  wnd->SetWindowPos(NULL, left, 4, crect.Width(), crect.Height(),
                    SWP_SHOWWINDOW);
  wnd = GetDlgItem(IDOK);
  wnd->SetWindowPos(NULL, rect.Width() - crect.Width() - 4, 4, crect.Width(),
                    crect.Height(), SWP_SHOWWINDOW);
  editInfo.SetWindowPos(NULL, 4, 8 + crect.Height(), rect.Width() - 8,
                        rect.Height() - crect.Height() * 2 - 16,
                        SWP_SHOWWINDOW);
  wnd = GetDlgItem(IDC_BUTTON_GOTO);
  wnd->SetWindowPos(NULL, 4, rect.Height() - 4 - crect.Height(), crect.Width(),
                    crect.Height(), SWP_SHOWWINDOW);
  wnd = GetDlgItem(IDC_EDIT_GOTO);
  wnd->SetWindowPos(NULL, 8 + crect.Width(), rect.Height() - 3 - crect.Height(),
                    crect.Width() + 8, crect.Height() - 3, SWP_SHOWWINDOW);
  wnd = GetDlgItem(IDC_STATIC_LINE);
  wnd->SetWindowPos(NULL, 30 + crect.Width() * 2,
                    rect.Height() - crect.Height(), crect.Width() * 2,
                    crect.Height(), SWP_SHOWWINDOW);
  wnd = GetDlgItem(IDC_EDIT_LINE);
  wnd->SetWindowPos(NULL, 40 + crect.Width() * 3,
                    rect.Height() - crect.Height(), crect.Width() + 8,
                    crect.Height(), SWP_SHOWWINDOW);
}
...
void CEditViewDlg::OnBnClickedButtonOpen() {
  ...
  dlg = ((mode == MATERIALS) ? CEntityDlg::ShowMaterialChooser()
                             : CEntityDlg::ShowGuiChooser());
  if (dlg) {
    if (mode == MATERIALS) {
...
void CEditViewDlg::OnBnClickedButtonSave() {
  if (fileName.Length()) {
    ...
    fileSystem->WriteFile(fileName, text.GetBuffer(0), text.GetLength(),
                          "fs_devpath");
    if (mode == MATERIALS) {
...
void CEditViewDlg::UpdateEditPreview() {
  if (GetSafeHwnd() && editInfo.GetSafeHwnd()) {
    ...
    mediaPreview.SetMode((mode == MATERIALS) ? CMediaPreviewDlg::MATERIALS
                                             : CMediaPreviewDlg::GUIS);
    mediaPreview.SetMedia((mode == MATERIALS) ? matName : fileName);
    SetWindowText(va("Editing %s in file <%s>",
                     (mode == MATERIALS) ? matName.c_str() : fileName.c_str(),
                     fileName.c_str()));
    editInfo.SetFocus();
  }
}
...
BOOL CEditViewDlg::OnInitDialog() {
  ...
  if (LoadRegistryInfo("Radiant::EditViewWindow", &rct, &lSize)) {
    SetWindowPos(NULL, rct.left, rct.top, rct.Width(), rct.Height(),
                 SWP_SHOWWINDOW);
  }
...
void CEditViewDlg::SetMaterialInfo(const char* name,
                                   const char* file,
                                   int _line) {
  idStr str;
  void* buf;
...
BOOL CEditViewDlg::PreTranslateMessage(MSG* pMsg) {
  ...
  if (pMsg->hwnd == editInfo.GetSafeHwnd() && (pMsg->message == WM_KEYDOWN) &&
      (pMsg->wParam == VK_TAB)) {

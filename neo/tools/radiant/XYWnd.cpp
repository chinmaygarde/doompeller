...
const char* g_pOrgStrings[] = {"(x:%.f  y:%.f)", "(x:%.f  z:%.f)",
                               "(y:%.f  z:%.f)"};
CString g_strDim;
CString g_strStatus;
...
bool CDragPoint::PointWithin(idVec3 p, int nView) {
  if (nView == -1) {
    if (fDiff(p[0], vec[0]) <= 3 && fDiff(p[1], vec[1]) <= 3 &&
        fDiff(p[2], vec[2]) <= 3) {
      return true;
    }
  } else {
...
/*
 =======================================================================================================================
    RotateLight target is relative to the light origin up and right are relative
 to the target up and right are perpendicular and are on a plane through the
 target with the target vector as normal delta is the movement of the target
...
/*
 ...
 =======================================================================================================================
*/
static void RotateLight(idVec3& target,
                        idVec3& up,
                        idVec3& right,
                        const idVec3& delta) {
...
bool UpdateActiveDragPoint(const idVec3& move) {
  if (activeDrag) {
    ...
    if (activeDrag->nType == LIGHT_TARGET) {
      ...
      target += (activeDrag->pBrush->trackLightOrigin)
                    ? activeDrag->pBrush->owner->lightOrigin
                    : activeDrag->pBrush->owner->origin;
      UpdateSelectablePoint(activeDrag->pBrush,
                            Brush_TransformedPoint(activeDrag->pBrush, target),
                            LIGHT_TARGET);
      up += target;
      UpdateSelectablePoint(activeDrag->pBrush,
                            Brush_TransformedPoint(activeDrag->pBrush, up),
                            LIGHT_UP);
      right += target;
      UpdateSelectablePoint(activeDrag->pBrush,
                            Brush_TransformedPoint(activeDrag->pBrush, right),
                            LIGHT_RIGHT);
      ...
      start += (activeDrag->pBrush->trackLightOrigin)
                   ? activeDrag->pBrush->owner->lightOrigin
                   : activeDrag->pBrush->owner->origin;
      UpdateSelectablePoint(activeDrag->pBrush,
                            Brush_TransformedPoint(activeDrag->pBrush, start),
                            LIGHT_START);
      ...
      end += (activeDrag->pBrush->trackLightOrigin)
                 ? activeDrag->pBrush->owner->lightOrigin
                 : activeDrag->pBrush->owner->origin;
      UpdateSelectablePoint(activeDrag->pBrush,
                            Brush_TransformedPoint(activeDrag->pBrush, end),
                            LIGHT_END);
...
void WXY_Print(void) {
  ...
  if (!PrintDlg(&pd) || !pd.hDC) {
    g_pParentWnd->MessageBox("Could not PrintDlg()", "QE4 Print Error",
                             MB_OK | MB_ICONERROR);
    return;
  }
  ...
  if (StartDoc(pd.hDC, &di) <= 0) {
    g_pParentWnd->MessageBox("Could not StartDoc()", "QE4 Print Error",
                             MB_OK | MB_ICONERROR);
    return;
  }
  ...
  if (StartPage(pd.hDC) <= 0) {
    g_pParentWnd->MessageBox("Could not StartPage()", "QE4 Print Error",
                             MB_OK | MB_ICONERROR);
    return;
  }
  { /* read pixels from the XY window */
    ...
    pwidth = GetDeviceCaps(pd.hDC, PHYSICALWIDTH) -
             GetDeviceCaps(pd.hDC, PHYSICALOFFSETX);
    pheight = GetDeviceCaps(pd.hDC, PHYSICALHEIGHT) -
              GetDeviceCaps(pd.hDC, PHYSICALOFFSETY);

    StretchBlt(pd.hDC, 0, 0, pwidth, pheight, s_hdcXY, 0, 0, bmwidth, bmheight,
               SRCCOPY);
  }

  /* EndPage and EndDoc */
  if (EndPage(pd.hDC) <= 0) {
    g_pParentWnd->MessageBox("QE4 Print Error", "Could not EndPage()",
                             MB_OK | MB_ICONERROR);
    return;
  }

  if (EndDoc(pd.hDC) <= 0) {
    g_pParentWnd->MessageBox("QE4 Print Error", "Could not EndDoc()",
                             MB_OK | MB_ICONERROR);
    return;
  }
}
...
void CXYWnd::DropPathPoint(UINT nFlags, CPoint point) {
  ...
  if (g_pMovingPath) {
    ...
    SnapToPoint(point.x, rctZ.Height() - 1 - point.y,
                g_PathPoints[g_nPathCount]);
    g_nPathCount++;
      ...
      if (selected_brushes.next &&
        ...
        if (g_Clip3.Set() == false) {
          if (m_nViewType == XY) {
            ...
            face.planepts[2][0] =
                Betwixt(g_Clip1.m_ptClip[0], g_Clip2.m_ptClip[0]);
            face.planepts[2][1] =
                Betwixt(g_Clip1.m_ptClip[1], g_Clip2.m_ptClip[1]);
            ...
            face.planepts[2][1] =
                Betwixt(g_Clip1.m_ptClip[1], g_Clip2.m_ptClip[1]);
            face.planepts[2][2] =
                Betwixt(g_Clip1.m_ptClip[2], g_Clip2.m_ptClip[2]);
...
void CXYWnd::ProduceSplitLists() {
  if (AnyPatchesSelected()) {
    ...
    brush_t* next;
    for (brush_t* pb = selected_brushes.next; pb != &selected_brushes;
         pb = next) {
      next = pb->next;
      if (pb->pPatch) {
  ...
  brush_t* pBrush;
  for (pBrush = selected_brushes.next;
       pBrush != NULL && pBrush != &selected_brushes; pBrush = pBrush->next) {
    brush_t* pFront = NULL;
    brush_t* pBack = NULL;
    if (ClipMode()) {
      if (g_Clip1.Set() && g_Clip2.Set()) {
...
void CXYWnd::OnMouseMove(UINT nFlags, CPoint point) {
  ...
  if (g_PrefsDlg.m_bChaseMouse == TRUE &&
      (point.x < 0 || point.y < 0 || point.x > m_nWidth ||
       point.y > m_nHeight) &&
      GetCapture() == this) {
...
void CXYWnd::OnPaint() {
  ...
  if (bPaint) {
    ...
    if (ClipMode()) {
      ...
      CString strMsg;
      if (g_Clip1.Set()) {
        qglRasterPos3f(g_Clip1.m_ptClip[0] + 2, g_Clip1.m_ptClip[1] + 2,
                       g_Clip1.m_ptClip[2] + 2);
        strMsg = "1";
      ...
      if (g_Clip2.Set()) {
        qglRasterPos3f(g_Clip2.m_ptClip[0] + 2, g_Clip2.m_ptClip[1] + 2,
                       g_Clip2.m_ptClip[2] + 2);
        strMsg = "2";
      ...
      if (g_Clip3.Set()) {
        qglRasterPos3f(g_Clip3.m_ptClip[0] + 2, g_Clip3.m_ptClip[1] + 2,
                       g_Clip3.m_ptClip[2] + 2);
        strMsg = "3";
      ...
      if (g_Clip1.Set() && g_Clip2.Set() &&
        ...
        brush_t* pBrush;
        brush_t* pList = ((m_nViewType == XZ) ? !g_bSwitch : g_bSwitch)
                             ? &g_brBackSplits
                             : &g_brFrontSplits;
        for (pBrush = pList->next; pBrush != NULL && pBrush != pList;
             pBrush = pBrush->next) {
          qglColor3f(1, 1, 0);

          face_t* face;
          int order;
          for (face = pBrush->brush_faces, order = 0; face;
               face = face->next, order++) {
    ...
    if (PathMode()) {
      ...
      CString strMsg;
      for (n = 0; n < g_nPathCount; n++) {
        qglRasterPos3f(g_PathPoints[n].m_ptClip[0] + 2,
...
void CreateEntityFromName(char* pName,
  ...
  if (stricmp(pName, "worldspawn") == 0) {
    g_pParentWnd->MessageBox("Can't create an entity with worldspawn.", "info",
                             0);
    return;
  }
  ...
  if (petNew && idStr::Icmp(pName, "light") == 0) {
    ...
    if (rad.x != 0 && rad.y != 0 && rad.z != 0) {
      SetKeyValue(petNew, "light_radius",
                  va("%g %g %g", idMath::Fabs(rad.x), idMath::Fabs(rad.y),
                     idMath::Fabs(rad.z)));
  ...
  if (petNew == NULL) {
    if (!((selected_brushes.next == &selected_brushes) ||
      ...
      if (b->owner != world_entity &&
          ((b->owner->eclass->fixedsize && pecNew->fixedsize) || forceFixed)) {
        idVec3 mins, maxs;
        idVec3 origin;
        for (int i = 0; i < 3; i++) {
...
brush_t* CreateEntityBrush(int x, int y, CXYWnd* pWnd) {
  ...
  int nDim = (pWnd->GetViewType() == XY)   ? 2
             : (pWnd->GetViewType() == YZ) ? 0
                                           : 1;
  mins[nDim] =
...
void CreateSmartEntity(CXYWnd* pWnd, int x, int y, const char* pName) {
  ...
  if (g_strSmartEntity.Find("Smart_Train") >= 0) {
    ShowInfoDialog(
        "Select the path of the train by left clicking in XY, YZ and/or XZ "
    ...
    CreateRightClickEntity(g_pParentWnd->ActiveXY(), g_nSmartX, g_nSmartY,
                           "func_rotating");
...
void FinishSmartCreation() {
  ...
  if (g_strSmartEntity.Find("Smart_Train") >= 0) {
    ...
    for (n = 0; n < g_nPathCount; n++) {
      Select_Deselect();
      CreateRightClickEntity(g_pParentWnd->ActiveXY(),
                             g_PathPoints[n].m_ptScreen.x,
                             g_PathPoints[n].m_ptScreen.y, "path_corner");
      array.Add(reinterpret_cast<void*>(selected_brushes.next));
    }
...
void CXYWnd::OnEntityCreate(unsigned int nID) {
  if (m_mnuDrop.GetSafeHmenu()) {
    ...
    if (strItem.Find("Smart_") >= 0) {
      ...
      CreateRightClickEntity(this, m_ptDown.x, m_ptDown.y,
                             strItem.GetBuffer(0));
    }
...
void CXYWnd::HandleDrop() {
  ...
  if (!m_mnuDrop.GetSafeHmenu()) {  // first time, load it up
    ...
    for (e = eclass; e; e = e->next) {
      ...
      if (n_ > 0) {
        ...
        if (pChild) {
          pMakeEntityPop->AppendMenu(
              MF_POPUP, reinterpret_cast<unsigned int>(pChild->GetSafeHmenu()),
              strActive);
          g_ptrMenus.Add(pChild);
  ...
  m_mnuDrop.TrackPopupMenu(TPM_LEFTALIGN | TPM_LEFTBUTTON | TPM_RIGHTBUTTON,
...
void CXYWnd::XY_ToGridPoint(int x, int y, idVec3& point) {
  if (m_nViewType == XY) {
    ...
    // point[2] = 0;
    point[0] =
        floor(point[0] / g_qeglobals.d_gridsize + 0.5) * g_qeglobals.d_gridsize;
    point[1] =
        floor(point[1] / g_qeglobals.d_gridsize + 0.5) * g_qeglobals.d_gridsize;
  } else if (m_nViewType == YZ) {
    //
    ...
    point[1] =
        floor(point[1] / g_qeglobals.d_gridsize + 0.5) * g_qeglobals.d_gridsize;
    point[2] =
        floor(point[2] / g_qeglobals.d_gridsize + 0.5) * g_qeglobals.d_gridsize;
  } else {
    //
...
void CXYWnd::XY_MouseDown(int x, int y, int buttons) {
  ...
  if ((buttons == MK_LBUTTON) || (buttons == (MK_LBUTTON | MK_SHIFT)) ||
    ...
    Patch_SetView((m_nViewType == XY)   ? W_XY
                  : (m_nViewType == YZ) ? W_YZ
                                        : W_XZ);
  ...
  if ((g_PrefsDlg.m_nMouseButtons == 3 && m_nButtonstate == MK_MBUTTON) ||
      (g_PrefsDlg.m_nMouseButtons == 2 &&
       m_nButtonstate == (MK_SHIFT | MK_CONTROL | MK_RBUTTON))) {
    ...
    if (point[n1] || point[n2]) {
      g_pParentWnd->GetCamera()->Camera().angles[nAngle] =
          RAD2DEG(atan2(point[n1], point[n2]));
...
bool CXYWnd::DragDelta(int x, int y, idVec3& move) {
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (!g_PrefsDlg.m_bNoClamp) {
      delta[i] = floor(delta[i] / g_qeglobals.d_gridsize + 0.5) *
                 g_qeglobals.d_gridsize;
    }
  }
...
void CXYWnd::NewBrushDrag(int x, int y) {
  ...
  g_strStatus.Format("Size X:: %.1f  Y:: %.1f  Z:: %.1f", vSize[0], vSize[1],
                     vSize[2]);
...
bool CXYWnd::XY_MouseMoved(int x, int y, int buttons) {
  ...
  //
  if (m_nButtonstate == MK_LBUTTON && !m_bPress_selection &&
...
void CXYWnd::XY_DrawGrid() {
  ...
  // draw minor blocks
  if (m_fScale > .1 && g_qeglobals.d_showgrid &&
      g_qeglobals.d_gridsize * m_fScale >= 4 &&
      !g_qeglobals.d_savedinfo.colors[COLOR_GRIDMINOR].Compare(
          g_qeglobals.d_savedinfo.colors[COLOR_GRIDBACK])) {
  ...
  if (g_qeglobals.d_savedinfo.show_coordinates) {
    ...
    qglRasterPos2f(m_vOrigin[nDim1] - w + 35 / m_fScale,
...
void CXYWnd::DrawRotateIcon() {
  ...
  if (g_qeglobals.flatRotation) {
    str += g_qeglobals.flatRotation == 2 ? " Flat [center] "
                                         : " Flat [ rot origin ] ";
  }
  qglRasterPos2f(x, y);
...
void CXYWnd::DrawCameraIcon() {
  ...
  float scale =
      1.0 /
      m_fScale;  // jhefty - keep the camera icon proportionally the same size
  ...
  qglBegin(GL_LINE_STRIP);
  qglVertex3f(x + (48 * cos(a + idMath::PI * 0.25f) * scale),
              y + (48 * sin(a + idMath::PI * 0.25f) * scale), 0);
  qglVertex3f(x, y, 0);
  qglVertex3f(x + (48 * cos(a - idMath::PI * 0.25f) * scale),
              y + (48 * sin(a - idMath::PI * 0.25f) * scale), 0);
  qglEnd();
...
bool FilterBrush(brush_t* pb) {
  ...
  if (g_pParentWnd->GetZWnd()
          ->m_pZClip)  // ZClip class up and running? (and hence Z window built)
  {
    if (g_pParentWnd->GetZWnd()->m_pZClip->IsEnabled()) {
      // ZClipping active...
      //
      if (pb->mins[2] >
              g_pParentWnd->GetZWnd()
                  ->m_pZClip->GetTop()  // brush bottom edge is above clip top
          || pb->maxs[2] <
                 g_pParentWnd->GetZWnd()
                     ->m_pZClip
                     ->GetBottom()  // brush top edge is below clip bottom
      ) {
        return TRUE;
      }
    }
  ...
  if (g_qeglobals.d_savedinfo.exclude & EXCLUDE_LIGHTS &&
      pb->owner->eclass->nShowFlags & ECLASS_LIGHT) {
    return true;
  }

  if (g_qeglobals.d_savedinfo.exclude & EXCLUDE_COMBATNODES &&
      pb->owner->eclass->nShowFlags & ECLASS_COMBATNODE) {
    return true;
  }

  if (g_qeglobals.d_savedinfo.exclude & EXCLUDE_PATHS &&
      pb->owner->eclass->nShowFlags & ECLASS_PATH) {
    return true;
  }

  if (g_qeglobals.d_savedinfo.exclude & EXCLUDE_MODELS &&
      (pb->owner->eclass->entityModel != NULL || pb->modelHandle > 0)) {
    return true;
  }
...
void DrawPathLines(void) {
  ...
  num_entities = 0;
  for (te = entities.next; te != &entities && num_entities != MAX_MAP_ENTITIES;
       te = te->next) {
  ...
  for (se = entities.next; se != &entities; se = se->next) {
    ...
    for (k = 0; k < num_entities; k++) {
      ...
      qglColor3f(se->eclass->color[0], se->eclass->color[1],
                 se->eclass->color[2]);
...
void CXYWnd::PaintSizeInfo(int nDim1,
  ...
  if (m_nViewType == XY) {
    ...
    qglRasterPos3f(Betwixt(vMinBounds[nDim1], vMaxBounds[nDim1]),
                   vMinBounds[nDim2] - 20.0 / m_fScale, 0.0f);
    g_strDim.Format(g_pDimStrings[nDim1], vSize[nDim1]);
    qglCallLists(g_strDim.GetLength(), GL_UNSIGNED_BYTE, g_strDim);

    qglRasterPos3f(vMaxBounds[nDim1] + 16.0 / m_fScale,
                   Betwixt(vMinBounds[nDim2], vMaxBounds[nDim2]), 0.0f);
    g_strDim.Format(g_pDimStrings[nDim2], vSize[nDim2]);
    qglCallLists(g_strDim.GetLength(), GL_UNSIGNED_BYTE, g_strDim);

    qglRasterPos3f(vMinBounds[nDim1] + 4, vMaxBounds[nDim2] + 8 / m_fScale,
                   0.0f);
    ...
    qglRasterPos3f(Betwixt(vMinBounds[nDim1], vMaxBounds[nDim1]), 0,
                   vMinBounds[nDim2] - 20.0 / m_fScale);
    g_strDim.Format(g_pDimStrings[nDim1], vSize[nDim1]);
    qglCallLists(g_strDim.GetLength(), GL_UNSIGNED_BYTE, g_strDim);

    qglRasterPos3f(vMaxBounds[nDim1] + 16.0 / m_fScale, 0,
                   Betwixt(vMinBounds[nDim2], vMaxBounds[nDim2]));
    g_strDim.Format(g_pDimStrings[nDim2], vSize[nDim2]);
    qglCallLists(g_strDim.GetLength(), GL_UNSIGNED_BYTE, g_strDim);
    ...
    qglRasterPos3f(0, Betwixt(vMinBounds[nDim1], vMaxBounds[nDim1]),
                   vMinBounds[nDim2] - 20.0 / m_fScale);
    g_strDim.Format(g_pDimStrings[nDim1], vSize[nDim1]);
    qglCallLists(g_strDim.GetLength(), GL_UNSIGNED_BYTE, g_strDim);

    qglRasterPos3f(0, vMaxBounds[nDim1] + 16.0 / m_fScale,
                   Betwixt(vMinBounds[nDim2], vMaxBounds[nDim2]));
    g_strDim.Format(g_pDimStrings[nDim2], vSize[nDim2]);
    qglCallLists(g_strDim.GetLength(), GL_UNSIGNED_BYTE, g_strDim);

    qglRasterPos3f(0, vMinBounds[nDim1] + 4.0,
                   vMaxBounds[nDim2] + 8 / m_fScale);
...
void CXYWnd::XY_Draw() {
  ...
  qglOrtho(mins[0], maxs[0], mins[1], maxs[1], MIN_WORLD_COORD,
           MAX_WORLD_COORD);
  ...
  for (brush = active_brushes.next; brush != &active_brushes;
       brush = brush->next) {
    if (brush->forceVisibile || (brush->owner->eclass->nShowFlags &
                                 (ECLASS_LIGHT | ECLASS_PROJECTEDLIGHT))) {
    } else if (brush->mins[nDim1] > maxs[0] || brush->mins[nDim2] > maxs[1] ||
               brush->maxs[nDim1] < mins[0] || brush->maxs[nDim2] < mins[1]) {
      culled++;
      continue;  // off screen
    }
  ...
  qglPushMatrix();
  qglTranslatef(g_qeglobals.d_select_translate[0],
  ...
  if (!bFixedSize && !RotateMode() && !ScaleMode() && drawn - nSaveDrawn > 0 &&
      g_PrefsDlg.m_bSizePaint) {
  ...
  if (g_qeglobals.d_select_mode == sel_vertex) {
    ...
    for (i = 0; i < g_qeglobals.d_numedges; i++) {
      ...
      qglVertex3f((v1[0] + v2[0]) * 0.5, (v1[1] + v2[1]) * 0.5,
                  (v1[2] + v2[2]) * 0.5);
    }
  ...
  g_splineList->draw(
      static_cast<bool>(g_qeglobals.d_select_mode == sel_editpoint ||
  ...
  if (g_qeglobals.d_select_mode == sel_area) {
    ...
    qglBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    qglColor4f(0.0, 0.0, 1.0, 0.25);
    qglRectf(g_qeglobals.d_vAreaTL[nDim1], g_qeglobals.d_vAreaTL[nDim2],
             g_qeglobals.d_vAreaBR[nDim1], g_qeglobals.d_vAreaBR[nDim2]);
    qglDisable(GL_BLEND);
...
void CXYWnd::Clip() {
  if (ClipMode()) {
    ...
    if (g_PrefsDlg.m_bSwitchClip) {
      pList = ((m_nViewType == XZ) ? g_bSwitch : !g_bSwitch) ? &g_brFrontSplits
                                                             : &g_brBackSplits;
    } else {
      pList = ((m_nViewType == XZ) ? !g_bSwitch : g_bSwitch) ? &g_brFrontSplits
                                                             : &g_brBackSplits;
...
void CXYWnd::Copy() {
  ...
  if (nClipboard > 0) {
    if (OpenClipboard()) {
      ...
      HANDLE h = ::GlobalAlloc(GMEM_ZEROINIT | GMEM_MOVEABLE | GMEM_DDESHARE,
                               lSize + sizeof(long));
      if (h != NULL) {
  ...
  if (!bClipped) {
    common->Printf(
        "Unable to register Windows clipboard formats, copy/paste between "
        "editors will not be possible");
  }
  ...
  CleanCopyEntities();
  for (brush_t* pBrush = selected_brushes.next;
       pBrush != NULL && pBrush != &selected_brushes; pBrush = pBrush->next) {
    if (pBrush->owner == world_entity) {
      ...
      if (!OnList(pBrush->owner, &holdArray)) {
        ...
        for (brush_t* pEB = e->brushes.onext; pEB != &e->brushes;
             pEB = pEB->onext) {
...
void CXYWnd::Paste() {
  ...
  if (nClipboard > 0 && OpenClipboard() &&
      ::IsClipboardFormatAvailable(nClipboard)) {
...
#endif
#else
  if (g_brClipboard.next != &g_brClipboard ||
      g_enClipboard.next != &g_enClipboard) {
    Select_Deselect();

    for (brush_t* pBrush = g_brClipboard.next;
         pBrush != NULL && pBrush != &g_brClipboard; pBrush = pBrush->next) {
      brush_t* pClone = Brush_Clone(pBrush);
    ...
    for (entity_t* pEntity = g_enClipboard.next;
         pEntity != NULL && pEntity != &g_enClipboard;
         pEntity = pEntity->next) {
      entity_t* pEClone = Entity_Clone(pEntity);
      for (brush_t* pEB = pEntity->brushes.onext; pEB != &pEntity->brushes;
           pEB = pEB->onext) {
        brush_t* pClone = Brush_Clone(pEB);
        Brush_AddToList(pClone, &selected_brushes);
        Entity_LinkBrush(pEClone, pClone);
...
void CXYWnd::DrawPrecisionCrosshair(void) {
  ...
  float x, y;
  idVec4 crossEndColor(
      1.0f, 0.0f, 1.0f,
      1.0f);  // the RGBA color of the precision crosshair at its ends
  idVec4 crossMidColor;  // the RGBA color of the precision crosshair at the
                         // crossing point
  ...
  crossEndColor[0] =
      g_qeglobals.d_savedinfo.colors[COLOR_PRECISION_CROSSHAIR][0];
  crossEndColor[1] =
      g_qeglobals.d_savedinfo.colors[COLOR_PRECISION_CROSSHAIR][1];
  crossEndColor[2] =
      g_qeglobals.d_savedinfo.colors[COLOR_PRECISION_CROSSHAIR][2];
  ...
  if (m_precisionCrosshairMode == PRECISION_CROSSHAIR_FREE)
    crossMidColor[3] =
        0.0f;  // intersection-color is 100% transparent (alpha = 0.0f)
  ...
  // Radiant was in opaque, flat-shaded mode by default; restore this to prevent
  // possible slowdown

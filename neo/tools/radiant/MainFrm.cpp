...
bool g_bScreenUpdates =
    true;  // whether window painting is active, used in a few places
...
SCommandInfo g_Commands[] = {
    ...
    {"Texture_AxialArbitrary", 'U', RAD_SHIFT,
    ...
    {"Texture_RotateClock", VK_NEXT, RAD_SHIFT,
     ID_SELECTION_TEXTURE_ROTATECLOCK},
    {"Texture_RotateCounter", VK_PRIOR, RAD_SHIFT,
     ID_SELECTION_TEXTURE_ROTATECOUNTER},
    ...
    {"Texture_ShiftRight", VK_RIGHT, RAD_SHIFT,
     ID_SELECTION_TEXTURE_SHIFTRIGHT},
    ...
    {"Texture_ScaleRight", VK_RIGHT, RAD_CONTROL,
     ID_SELECTION_TEXTURE_SCALERIGHT},
    {"Texture_InvertX", 'I', RAD_CONTROL | RAD_SHIFT,
     ID_CURVE_NEGATIVETEXTUREY},
    ...
    {"FileSaveCopy", 'C', RAD_CONTROL | RAD_ALT | RAD_SHIFT,
     D3XP_ID_FILE_SAVE_COPY},
    ...
    {"Patch_UnFreezeAllVertices", 'F', RAD_CONTROL | RAD_SHIFT,
     ID_CURVE_UNFREEZEALL},
    ...
    {"Selection_SelectCompleteEntity", 'E', RAD_CONTROL | RAD_ALT | RAD_SHIFT,
     ID_SELECT_COMPLETE_ENTITY},
    ...
    {"Show_HideNotSelected", 'H', RAD_CONTROL | RAD_SHIFT,
     ID_VIEW_HIDESHOW_HIDENOTSELECTED},
    ...
    {"_AutoCaulk", 'A', RAD_CONTROL | RAD_SHIFT,
     ID_AUTOCAULK},  // ctrl-shift-a, since SHIFT-A is already taken
};

int g_nCommandCount = sizeof(g_Commands) / sizeof(SCommandInfo);

SKeyInfo g_Keys[] = {{"Space", VK_SPACE},
                     {"Backspace", VK_BACK},
                     {"Escape", VK_ESCAPE},
                     {"End", VK_END},
                     {"Insert", VK_INSERT},
...
ON_COMMAND(ID_CURVE_MOREENDCAPSBEVELS_SQUAREBEVEL,
           OnCurveMoreendcapsbevelsSquarebevel)
ON_COMMAND(ID_CURVE_MOREENDCAPSBEVELS_SQUAREENDCAP,
           OnCurveMoreendcapsbevelsSquareendcap)
...
void HandlePopup(CWnd* pWindow, unsigned int uId) {
  ...
  mnuPopup.GetSubMenu(0)->TrackPopupMenu(
      TPM_LEFTALIGN | TPM_LEFTBUTTON | TPM_RIGHTBUTTON, ptMouse.x, ptMouse.y,
      pWindow);
...
void CMainFrame::SetButtonMenuStates() {
  ...
  if (pMenu) {
    ...
    pMenu->CheckMenuItem(ID_VIEW_SHOW_SHOWVISPORTALS,
    ...
    if (!g_qeglobals.d_savedinfo.show_coordinates) {
      pMenu->CheckMenuItem(ID_VIEW_SHOWCOORDINATES,
    ...
    if (g_qeglobals.d_savedinfo.exclude & EXCLUDE_COMBATNODES) {
      pMenu->CheckMenuItem(ID_VIEW_SHOWCOMBATNODES,
                           MF_BYCOMMAND | MF_UNCHECKED);
    }
    ...
    pMenu->CheckMenuItem(
        ID_TOGGLE_LOCK,
        MF_BYCOMMAND | (g_PrefsDlg.m_bTextureLock) ? MF_CHECKED : MF_UNCHECKED);
    pMenu->CheckMenuItem(
        ID_TOGGLE_ROTATELOCK,
        MF_BYCOMMAND | (g_PrefsDlg.m_bRotateLock) ? MF_CHECKED : MF_UNCHECKED);
    pMenu->CheckMenuItem(ID_VIEW_CUBICCLIPPING,
                         MF_BYCOMMAND | (g_PrefsDlg.m_bCubicClipping)
                             ? MF_CHECKED
                             : MF_UNCHECKED);
    pMenu->CheckMenuItem(
        ID_VIEW_OPENGLLIGHTING,
        MF_BYCOMMAND | (g_PrefsDlg.m_bGLLighting) ? MF_CHECKED : MF_UNCHECKED);
    pMenu->CheckMenuItem(ID_SNAPTOGRID, MF_BYCOMMAND | (!g_PrefsDlg.m_bNoClamp)
                                            ? MF_CHECKED
                                            : MF_UNCHECKED);
    if (m_wndToolBar.GetSafeHwnd()) {
      m_wndToolBar.GetToolBarCtrl().CheckButton(
          ID_VIEW_CUBICCLIPPING, (g_PrefsDlg.m_bCubicClipping) ? TRUE : FALSE);
    }
  ...
  if (g_qeglobals.d_project_entity) {
    ...
    LoadMruInReg(g_qeglobals.d_lpMruMenu,
                 "Software\\" EDITOR_REGISTRY_KEY "\\MRU");
    PlaceMenuMRUItem(g_qeglobals.d_lpMruMenu,
                     ::GetSubMenu(::GetMenu(GetSafeHwnd()), 0), ID_FILE_EXIT);
  }
}
...
void CMainFrame::ShowMenuItemKeyBindings(CMenu* pMenu) {
  ...
  for (i = 0; i < g_nCommandCount; i++) {
    ...
    SetMenuItemInfo(pMenu->m_hMenu, g_Commands[i].m_nCommand, FALSE,
                    &MenuItemInfo);
  }
}
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct) {
  ...
  m_wndToolBar.GetToolBarCtrl().CheckButton(
      ID_SOUND_SHOWSOUNDVOLUMES, g_qeglobals.d_savedinfo.showSoundAlways);
  m_wndToolBar.GetToolBarCtrl().CheckButton(
      ID_SOUND_SHOWSELECTEDSOUNDVOLUMES,
      g_qeglobals.d_savedinfo.showSoundWhenSelected);
  ...
  if (radiant_entityMode.GetBool()) {
    g_qeglobals.d_savedinfo.exclude |=
        (EXCLUDE_PATHS | EXCLUDE_CLIP | EXCLUDE_CAULK | EXCLUDE_VISPORTALS |
         EXCLUDE_NODRAW | EXCLUDE_TRIGGERS);
  }
...
void CMainFrame::LoadCommandMap() {
  ...
  for (int i = 0; i < g_nCommandCount; i++) {
    int nLen = GetPrivateProfileString("Commands", g_Commands[i].m_strCommand,
                                       "", pBuff, 1024, strINI);
    if (nLen > 0) {
...
void CMainFrame::CreateQEChildren() {
  ...
  if (!bProjectLoaded) {
    CFileDialog dlgFile(
        ...
        EDITOR_WINDOWTEXT " Project files (*.qe4, *.prj)|*.qe4|*.prj||", this);
  ...
  if (!bProjectLoaded) {
    Error(
        "Unable to load project file. It was unavailable in the scripts path "
...
void CMainFrame::RoutineProcessing() {
  if (m_bDoLoop) {
    ...
    if (g_PrefsDlg.m_bQE4Painting && g_nUpdateBits) {
      int nBits =
          g_nUpdateBits;  // this is done to keep this routine from being
      g_nUpdateBits = 0;  // re-entered due to the paint process.. only
      UpdateWindows(
          nBits);  // happens in rare cases but causes a stack overflow
    }
  }
}
...
void CMainFrame::OnDestroy() {
  ...
  SaveRegistryInfo("radiant_SavedInfo", &g_qeglobals.d_savedinfo,
...
void CMainFrame::OnFileSaveCopy() {
  ...
  afn.Flags = OFN_SHOWHELP | OFN_PATHMUSTEXIST | OFN_FILEMUSTEXIST |
              OFN_OVERWRITEPROMPT;
...
void CMainFrame::OnViewCenter() {
  ...
  m_pCamWnd->Camera().angles[YAW] =
      22.5 * floor((m_pCamWnd->Camera().angles[YAW] + 11) / 22.5);
...
BOOL DoMru(HWND hWnd, WORD wId) {
  ...
  GetMenuItem(g_qeglobals.d_lpMruMenu, wId, TRUE, szFileName,
              sizeof(szFileName));
...
void RunBsp(const char* command) {
  ...
  if (idStr::Icmpn(command, "bspext", strlen("runbsp")) == 0) {
    ...
    if (strlen(command) > strlen("bspext")) {
      idStr::snPrintf(
          sys, sizeof(sys),
          "%s %s +set r_fullscreen 0 +dmap editorOutput %s %s +quit", buff,
          paths.c_str(), command + strlen("bspext"), in);
    } else {
      idStr::snPrintf(sys, sizeof(sys),
                      "%s %s +set r_fullscreen 0 +dmap editorOutput %s +quit",
                      buff, paths.c_str(), in);
    }

    ::GetStartupInfo(&startupinfo);
    if (!CreateProcess(NULL, sys, NULL, NULL, FALSE, 0, NULL, NULL,
    ...
    if (strlen(command) > strlen("bsp")) {
      idStr::snPrintf(sys, sizeof(sys), "dmap %s %s", command + strlen("bsp"),
                      in);
    } else {
...
void CMainFrame::OnViewShowblocks() {
  ...
  CheckMenuItem(
      ::GetMenu(GetSafeHwnd()), ID_VIEW_SHOWBLOCKS,
...
void CMainFrame::OnViewShowclip() {
  if ((g_qeglobals.d_savedinfo.exclude ^= EXCLUDE_CLIP) & EXCLUDE_CLIP) {
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_SHOWCLIP,
                  MF_BYCOMMAND | MF_UNCHECKED);
  } else {
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_SHOWCLIP,
                  MF_BYCOMMAND | MF_CHECKED);
...
void CMainFrame::OnViewShowTriggers() {
  if ((g_qeglobals.d_savedinfo.exclude ^= EXCLUDE_TRIGGERS) &
      EXCLUDE_TRIGGERS) {
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_SHOWTRIGGERS,
                  MF_BYCOMMAND | MF_UNCHECKED);
  } else {
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_SHOWTRIGGERS,
                  MF_BYCOMMAND | MF_CHECKED);
...
void CMainFrame::OnViewShowcoordinates() {
  ...
  CheckMenuItem(
      ::GetMenu(GetSafeHwnd()), ID_VIEW_SHOWCOORDINATES,
      MF_BYCOMMAND | (g_qeglobals.d_savedinfo.show_coordinates ? MF_CHECKED
                                                               : MF_UNCHECKED));
  Sys_UpdateWindows(W_XY);
}
...
void CMainFrame::OnViewShowent() {
  if ((g_qeglobals.d_savedinfo.exclude ^= EXCLUDE_ENT) & EXCLUDE_ENT) {
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_SHOWENT,
                  MF_BYCOMMAND | MF_UNCHECKED);
  } else {
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_SHOWENT,
                  MF_BYCOMMAND | MF_CHECKED);
...
void CMainFrame::OnViewShowlights() {
  if ((g_qeglobals.d_savedinfo.exclude ^= EXCLUDE_LIGHTS) & EXCLUDE_LIGHTS) {
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_SHOWLIGHTS,
                  MF_BYCOMMAND | MF_UNCHECKED);
  } else {
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_SHOWLIGHTS,
                  MF_BYCOMMAND | MF_CHECKED);
...
void CMainFrame::OnViewShownames() {
  ...
  CheckMenuItem(
      ::GetMenu(GetSafeHwnd()), ID_VIEW_SHOWNAMES,
      MF_BYCOMMAND |
          (g_qeglobals.d_savedinfo.show_names ? MF_CHECKED : MF_UNCHECKED));
  Map_BuildBrushData();
  Sys_UpdateWindows(W_XY);
}
...
void CMainFrame::OnViewShowpath() {
  if ((g_qeglobals.d_savedinfo.exclude ^= EXCLUDE_PATHS) & EXCLUDE_PATHS) {
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_SHOWPATH,
                  MF_BYCOMMAND | MF_UNCHECKED);
  } else {
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_SHOWPATH,
                  MF_BYCOMMAND | MF_CHECKED);
...
void CMainFrame::OnViewShowCombatNodes() {
  if ((g_qeglobals.d_savedinfo.exclude ^= EXCLUDE_COMBATNODES) &
      EXCLUDE_COMBATNODES) {
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_SHOWCOMBATNODES,
                  MF_BYCOMMAND | MF_UNCHECKED);
  } else {
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_SHOWCOMBATNODES,
                  MF_BYCOMMAND | MF_CHECKED);
...
void CMainFrame::OnViewShowwater() {
  if ((g_qeglobals.d_savedinfo.exclude ^= EXCLUDE_DYNAMICS) &
      EXCLUDE_DYNAMICS) {
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_SHOWWATER,
                  MF_BYCOMMAND | MF_UNCHECKED);
  } else {
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_SHOWWATER,
                  MF_BYCOMMAND | MF_CHECKED);
...
void CMainFrame::OnViewShowworld() {
  if ((g_qeglobals.d_savedinfo.exclude ^= EXCLUDE_WORLD) & EXCLUDE_WORLD) {
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_SHOWWORLD,
                  MF_BYCOMMAND | MF_UNCHECKED);
  } else {
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_SHOWWORLD,
                  MF_BYCOMMAND | MF_CHECKED);
...
bool DoColor(int iIndex) {
  ...
  if (dlg.DoModal() == IDOK) {
    ...
    g_qeglobals.d_savedinfo.colors[iIndex][1] =
        ((dlg.GetColor() >> 8) & 255) / 255.0;
    g_qeglobals.d_savedinfo.colors[iIndex][2] =
        ((dlg.GetColor() >> 16) & 255) / 255.0;
...
LPCSTR String_ToLower(LPCSTR psString) {
  ...
  if (strlen(psString) >= iBufferSize) {
    assert(0);
    common->Printf(
        "String_ToLower(): Warning, input string was %d bytes too large, "
        "performing strlwr() inline!\n",
...
bool FindNextBrush(brush_t* pPrevFoundBrush)  // can be NULL for fresh search
{
    ...
    if (ent && ent != world_entity)  // needed!
    {
      ...
      if (!strFindKey.IsEmpty()) {
        ...
        if (strlen(psEntFoundValue) &&
            (
                ...
                ((gbWholeStringMatchOnly &&
                 ...
                 (!gbWholeStringMatchOnly &&
                  strstr(String_ToLower(psEntFoundValue),
        ...
        for (int i = 0; i < iNumEntKeys; i++) {
          ...
          if (psEntFoundValue) {
            if ((strlen(psEntFoundValue) &&
                 strFindValue.IsEmpty())  // if blank <value> search specified
                                          // then any found-value is ok
                || (gbWholeStringMatchOnly &&
                    stricmp(psEntFoundValue, strFindValue) == 0) ||
                (!gbWholeStringMatchOnly &&
                 strstr(String_ToLower(psEntFoundValue),
                        String_ToLower(strFindValue)))) {
  ...
  if (gbSelectAllMatchingEnts) {
    common->Printf(
        "\nBrushes Selected: %d           (Brushes Scanned %d, Ents Scanned "
        "%d)\n",
  ...
  if (bFoundSomething) {
    ...
    if (pLastFoundEnt->origin[0] != 0.0f || pLastFoundEnt->origin[1] != 0.0f ||
        pLastFoundEnt->origin[2] != 0.0f) {
    ...
    g_pParentWnd->GetCamera()->Camera().origin[1] -=
        32;  // back off a touch to look at it
...
void CMainFrame::OnMiscFindOrReplaceEntity() {
  CEntKeyFindReplace FindReplace(&strFindKey, &strFindValue, &strReplaceKey,
                                 &strReplaceValue, &gbWholeStringMatchOnly,
                                 &gbSelectAllMatchingEnts);
  switch (FindReplace.DoModal()) {
    case ID_RET_REPLACE: {
      brush_t* next = NULL;
      int iOccurences = 0;
      for (brush_t* b = active_brushes.next; b != &active_brushes; b = next) {
        next = b->next;  // important to do this here, in case brush gets linked
                         // to a different list
        entity_t* ent = b->owner;
        ...
        if (ent)  // needed!
        {
          ...
          if (stricmp(strFindValue, psEntFoundValue) ==
                  0 ||  // found this exact key/value
              (strlen(psEntFoundValue) &&
               strFindValue.IsEmpty())  // or any value for this key if blank
                                        // value search specified
          ) {
...
void CMainFrame::OnMiscSetViewPos() {
  CString psNewCoords =
      GetString("Input coords (x y z [rot])\n\nUse spaces to seperate numbers");
  if (!psNewCoords.IsEmpty()) {
    idVec3 v3Viewpos;
    float fYaw = 0;

    psNewCoords.Remove(',');
    int iArgsFound = sscanf(psNewCoords, "%f %f %f", &v3Viewpos[0],
                            &v3Viewpos[1], &v3Viewpos[2]);
    if (iArgsFound == 3) {
      // try for an optional 4th (note how this wasn't part of the sscanf()
      // above, so I can check 1st-3, not just any 3)
      //
      int iArgsFound = sscanf(psNewCoords, "%f %f %f %f", &v3Viewpos[0],
                              &v3Viewpos[1], &v3Viewpos[2], &fYaw);
      if (iArgsFound != 4) {
        fYaw = 0;  // jic
      }
...
void CMainFrame::OnBrushFlipx() {
  ...
  Select_FlipAxis(0);
  for (brush_t* b = selected_brushes.next; b != &selected_brushes;
       b = b->next) {
...
void CMainFrame::OnSelectionDelete() {
  ...
  for (brush = selected_brushes.next; brush != &selected_brushes;
       brush = brush->next) {
...
void CMainFrame::OnCameraBack() {
  VectorMA(m_pCamWnd->Camera().origin, -SPEED_MOVE, m_pCamWnd->Camera().forward,
           m_pCamWnd->Camera().origin);
...
void CMainFrame::OnCameraStrafeleft() {
  VectorMA(m_pCamWnd->Camera().origin, -SPEED_MOVE, m_pCamWnd->Camera().right,
           m_pCamWnd->Camera().origin);
...
void CMainFrame::OnPrefs() {
  ...
  if (g_PrefsDlg.DoModal() == IDOK) {
    ...
    if (pMenu) {
      pMenu->CheckMenuItem(
          ID_SNAPTOGRID,
          MF_BYCOMMAND | (!g_PrefsDlg.m_bNoClamp) ? MF_CHECKED : MF_UNCHECKED);
    }
  }
}
...
void CMainFrame::UpdateWindows(int nBits) {
  ...
  if (nBits & W_TEXTURE) {
    g_Inspectors->texWnd.RedrawWindow(NULL, NULL,
...
void CMainFrame::SetGridStatus() {
  ...
  strStatus.Format("G:%1.2f T:%1.2f R:%i C:%i L:%c%c", g_qeglobals.d_gridsize,
...
void CMainFrame::OnFileExportmap() {
  CFileDialog dlgFile(FALSE, "map", NULL,
...
void CMainFrame::OnViewShowcurves() {
  if ((g_qeglobals.d_savedinfo.exclude ^= EXCLUDE_CURVES) & EXCLUDE_CURVES) {
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_SHOWCURVES,
                  MF_BYCOMMAND | MF_UNCHECKED);
  } else {
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_SHOWCURVES,
                  MF_BYCOMMAND | MF_CHECKED);
...
void CMainFrame::NudgeSelection(int nDirection, float fAmount) {
  if (ActiveXY()->RotateMode()) {
    ...
    CString strStatus;
    strStatus.Format("Rotation x:: %.1f  y:: %.1f  z:: %.1f",
    ...
    Select_Scale((g_nScaleHow & SCALE_X) ? v[0] : 1.0f,
...
void CMainFrame::OnSelectBrushesOnly() {
  ...
  m_wndToolBar.GetToolBarCtrl().CheckButton(
      ID_SELECT_BRUSHESONLY, (g_PrefsDlg.m_selectOnlyBrushes) ? TRUE : FALSE);
}
...
void CMainFrame::OnDynamicLighting() {
  ...
  pCam->Create(CAMERA_WINDOW_CLASS, "", WS_OVERLAPPEDWINDOW, rect,
               GetDesktopWindow(), 12345);
...
void CMainFrame::OnPatchToggleBox() {
  ...
  m_wndToolBar.GetToolBarCtrl().CheckButton(
      ID_PATCH_SHOWBOUNDINGBOX, (g_bPatchShowBounds) ? TRUE : FALSE);
  Sys_UpdateWindows(W_ALL);
}
...
void CMainFrame::OnPatchWireframe() {
  g_bPatchWireFrame ^= 1;
  m_wndToolBar.GetToolBarCtrl().CheckButton(ID_PATCH_WIREFRAME,
                                            (g_bPatchWireFrame) ? TRUE : FALSE);
  Sys_UpdateWindows(W_ALL);
}
...
void CMainFrame::OnPatchWeld() {
  g_bPatchWeld ^= 1;
  m_wndToolBar.GetToolBarCtrl().CheckButton(ID_PATCH_WELD,
                                            (g_bPatchWeld) ? TRUE : FALSE);
  Sys_UpdateWindows(W_ALL);
}
...
void CMainFrame::OnPatchDrilldown() {
  g_bPatchDrillDown ^= 1;
  m_wndToolBar.GetToolBarCtrl().CheckButton(ID_PATCH_DRILLDOWN,
                                            (g_bPatchDrillDown) ? TRUE : FALSE);
  Sys_UpdateWindows(W_ALL);
}
...
void CMainFrame::OnPatchBend() {
  Patch_BendToggle();
  m_wndToolBar.GetToolBarCtrl().CheckButton(ID_PATCH_BEND,
                                            (g_bPatchBendMode) ? TRUE : FALSE);
  Sys_UpdateWindows(W_ALL);
}
...
void CMainFrame::OnPatchInsdel() {
  Patch_InsDelToggle();
  m_wndToolBar.GetToolBarCtrl().CheckButton(
      ID_PATCH_INSDEL, (g_bPatchInsertMode) ? TRUE : FALSE);
  Sys_UpdateWindows(W_ALL);
}
...
void CMainFrame::CheckTextureScale(int id) {
  CMenu* pMenu = GetMenu();
  if (pMenu) {
    pMenu->CheckMenuItem(ID_TEXTURES_TEXTUREWINDOWSCALE_10,
                         MF_BYCOMMAND | MF_UNCHECKED);
    pMenu->CheckMenuItem(ID_TEXTURES_TEXTUREWINDOWSCALE_25,
                         MF_BYCOMMAND | MF_UNCHECKED);
    pMenu->CheckMenuItem(ID_TEXTURES_TEXTUREWINDOWSCALE_50,
                         MF_BYCOMMAND | MF_UNCHECKED);
    pMenu->CheckMenuItem(ID_TEXTURES_TEXTUREWINDOWSCALE_100,
                         MF_BYCOMMAND | MF_UNCHECKED);
    pMenu->CheckMenuItem(ID_TEXTURES_TEXTUREWINDOWSCALE_200,
                         MF_BYCOMMAND | MF_UNCHECKED);
...
void CMainFrame::SetEntityCheck() {
  ...
  if (pMenu) {
    pMenu->CheckMenuItem(
        ID_VIEW_ENTITIESAS_WIREFRAME,
        MF_BYCOMMAND | (g_PrefsDlg.m_nEntityShowState == ENTITY_WIRE)
            ? MF_CHECKED
            : MF_UNCHECKED);
    pMenu->CheckMenuItem(
        ID_VIEW_ENTITIESAS_SKINNED,
        MF_BYCOMMAND | (g_PrefsDlg.m_nEntityShowState == ENTITY_SKINNED)
            ? MF_CHECKED
            : MF_UNCHECKED);
  }
}
...
void CMainFrame::OnViewShowhint() {
  if ((g_qeglobals.d_savedinfo.exclude ^= EXCLUDE_HINT) & EXCLUDE_HINT) {
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_SHOWHINT,
                  MF_BYCOMMAND | MF_UNCHECKED);
  } else {
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_SHOWHINT,
                  MF_BYCOMMAND | MF_CHECKED);
...
void CMainFrame::OnViewOpengllighting() {
  ...
  g_PrefsDlg.SavePrefs();
  CheckMenuItem(
      ::GetMenu(GetSafeHwnd()), ID_VIEW_OPENGLLIGHTING,
      MF_BYCOMMAND | (g_PrefsDlg.m_bGLLighting) ? MF_CHECKED : MF_UNCHECKED);
  Sys_UpdateWindows(W_XY | W_CAMERA);
}
...
void CMainFrame::OnViewShowcaulk() {
  if ((g_qeglobals.d_savedinfo.exclude ^= EXCLUDE_CAULK) & EXCLUDE_CAULK) {
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_SHOWCAULK,
                  MF_BYCOMMAND | MF_UNCHECKED);
  } else {
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_SHOWCAULK,
                  MF_BYCOMMAND | MF_CHECKED);
...
void CMainFrame::OnViewShowangles() {
  if ((g_qeglobals.d_savedinfo.exclude ^= EXCLUDE_ANGLES) & EXCLUDE_ANGLES) {
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_SHOWANGLES,
                  MF_BYCOMMAND | MF_UNCHECKED);
  } else {
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_SHOWANGLES,
                  MF_BYCOMMAND | MF_CHECKED);
...
void CMainFrame::OnEditSaveprefab() {
  CFileDialog dlgFile(FALSE, "pfb", NULL,
                      OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,
                      "Prefab files (*.pfb)|*.pfb||", this);
  char CurPath[1024];
...
void CMainFrame::OnEditLoadprefab() {
  CFileDialog dlgFile(TRUE, "pfb", NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,
                      "Prefab files (*.pfb)|*.pfb||", this);
  char CurPath[1024];
  ::GetCurrentDirectory(1024, CurPath);
  dlgFile.m_ofn.lpstrInitialDir = CurPath;
...
void CMainFrame::OnShowLighttextures() {
  ...
  m_wndToolBar.GetToolBarCtrl().CheckButton(
      ID_SHOW_LIGHTTEXTURES, (g_bShowLightTextures) ? TRUE : FALSE);
  Sys_UpdateWindows(W_ALL);
}
...
void CMainFrame::OnShowLightvolumes() {
  ...
  m_wndToolBar.GetToolBarCtrl().CheckButton(
      ID_SHOW_LIGHTVOLUMES, (g_bShowLightVolumes) ? TRUE : FALSE);
  Sys_UpdateWindows(W_ALL);
}
...
void CMainFrame::OnSplineTest() {
  ...
  while (current < start + msecs) {
    float fov;
    g_splineList->getCameraInfo(
        current, g_pParentWnd->GetCamera()->Camera().origin, dir, &fov);
    g_pParentWnd->GetCamera()->Camera().angles[1] =
        atan2(dir[1], dir[0]) * 180 / 3.14159;
    g_pParentWnd->GetCamera()->Camera().angles[0] =
        asin(dir[2]) * 180 / 3.14159;
...
void CMainFrame::OnPopupNewcameraInterpolated() {
  g_qeglobals.d_select_mode = sel_addpoint;
  g_qeglobals.selectObject =
      g_splineList->startNewCamera(idCameraPosition::INTERPOLATED);
  OnSplinesEdit();
}
...
void CMainFrame::OnPopupNewcameraSpline() {
  g_qeglobals.d_select_mode = sel_addpoint;
  g_qeglobals.selectObject =
      g_splineList->startNewCamera(idCameraPosition::SPLINE);
  OnSplinesEdit();
}
...
void CMainFrame::OnPopupNewcameraFixed() {
  g_qeglobals.d_select_mode = sel_addpoint;
  g_qeglobals.selectObject =
      g_splineList->startNewCamera(idCameraPosition::FIXED);
  OnSplinesEdit();
}
...
void CMainFrame::OnSelectionMoveonly() {
  g_moveOnly ^= 1;
  m_wndToolBar.GetToolBarCtrl().CheckButton(ID_SELECTION_MOVEONLY,
                                            (g_moveOnly) ? TRUE : FALSE);
}
...
void CMainFrame::OnSelectionCombine() {
  ...
  if (e1->eclass->nShowFlags & ECLASS_LIGHT) {
    ...
    sprintf(str, "%g %g %g %g %g %g %g %g %g", mat[0][0], mat[0][1], mat[0][2],
    ...
    if (b->owner != e1) {
      if (e1->eclass->nShowFlags & ECLASS_LIGHT) {
        ...
        if (GetMatrixForKey(b->owner, "rotation", mat)) {
          ...
          sprintf(str, "%g %g %g %g %g %g %g %g %g", mat[0][0], mat[0][1],
                  mat[0][2], mat[1][0], mat[1][1], mat[1][2], mat[2][0],
                  mat[2][1], mat[2][2]);
...
void CMainFrame::OnViewRendermode() {
  ...
  CheckMenuItem(
      ::GetMenu(GetSafeHwnd()), ID_VIEW_RENDERMODE,
      MF_BYCOMMAND | (m_pCamWnd->GetRenderMode()) ? MF_CHECKED : MF_UNCHECKED);
  Sys_UpdateWindows(W_ALL);
}
...
void CMainFrame::OnViewRealtimerebuild() {
  ...
  CheckMenuItem(
      ::GetMenu(GetSafeHwnd()), ID_VIEW_REALTIMEREBUILD,
      MF_BYCOMMAND | (m_pCamWnd->GetRebuildMode()) ? MF_CHECKED : MF_UNCHECKED);
  Sys_UpdateWindows(W_ALL);
}
...
void CMainFrame::OnViewRenderentityoutlines() {
  ...
  CheckMenuItem(
      ::GetMenu(GetSafeHwnd()), ID_VIEW_RENDERENTITYOUTLINES,
      MF_BYCOMMAND | (m_pCamWnd->GetEntityMode()) ? MF_CHECKED : MF_UNCHECKED);
  Sys_UpdateWindows(W_ALL);
}

void CMainFrame::OnViewMaterialanimation() {
  m_pCamWnd->ToggleAnimationMode();
  CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_MATERIALANIMATION,
                MF_BYCOMMAND | (m_pCamWnd->GetAnimationMode()) ? MF_CHECKED
                                                               : MF_UNCHECKED);
  Sys_UpdateWindows(W_ALL);
}
...
void CMainFrame::OnViewRenderselection() {
  ...
  CheckMenuItem(
      ::GetMenu(GetSafeHwnd()), ID_VIEW_RENDERSELECTION,
      MF_BYCOMMAND | (m_pCamWnd->GetSelectMode()) ? MF_CHECKED : MF_UNCHECKED);
  Sys_UpdateWindows(W_CAMERA);
}
...
void CMainFrame::OnSelectNomodels() {
  ...
  m_wndToolBar.GetToolBarCtrl().CheckButton(
      ID_SELECT_NOMODELS, (g_PrefsDlg.m_selectNoModels) ? TRUE : FALSE);
}

void CMainFrame::OnViewShowShowvisportals() {
  if ((g_qeglobals.d_savedinfo.exclude ^= EXCLUDE_VISPORTALS) &
      EXCLUDE_VISPORTALS) {
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_SHOW_SHOWVISPORTALS,
                  MF_BYCOMMAND | MF_UNCHECKED);
  } else {
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_SHOW_SHOWVISPORTALS,
                  MF_BYCOMMAND | MF_CHECKED);
...
void CMainFrame::OnViewShowNoDraw() {
  if ((g_qeglobals.d_savedinfo.exclude ^= EXCLUDE_NODRAW) & EXCLUDE_NODRAW) {
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_SHOW_NODRAW,
                  MF_BYCOMMAND | MF_UNCHECKED);
  } else {
    CheckMenuItem(::GetMenu(GetSafeHwnd()), ID_VIEW_SHOW_NODRAW,
                  MF_BYCOMMAND | MF_CHECKED);
...
void CMainFrame::OnViewRendersound() {
  ...
  CheckMenuItem(
      ::GetMenu(GetSafeHwnd()), ID_VIEW_RENDERSOUND,
      MF_BYCOMMAND | (m_pCamWnd->GetSoundMode()) ? MF_CHECKED : MF_UNCHECKED);
  Sys_UpdateWindows(W_CAMERA);
}
...
void CMainFrame::OnGenerateMaterialsList() {
  ...
  g_Inspectors->consoleWnd.SetConsoleText(
      va("condump %s", mtrFileName.c_str()));

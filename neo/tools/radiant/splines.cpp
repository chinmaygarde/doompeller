...
================
*/
void glLabeledPoint(idVec4& color,
                    idVec3& point,
                    float size,
                    const char* label) {
...
void idSplineList::addToRenderer() {
  ...
  for (i = 0; i < controlPoints.Num(); i++) {
    ...
    maxs[1] -= 8;
    maxs[2] += 8;
    debugLine(colorYellow, mins[0], mins[1], mins[2], maxs[0], mins[1],
              mins[2]);
    debugLine(colorYellow, maxs[0], mins[1], mins[2], maxs[0], maxs[1],
              mins[2]);
    debugLine(colorYellow, maxs[0], maxs[1], mins[2], mins[0], maxs[1],
              mins[2]);
    debugLine(colorYellow, mins[0], maxs[1], mins[2], mins[0], mins[1],
              mins[2]);

    debugLine(colorYellow, mins[0], mins[1], maxs[2], maxs[0], mins[1],
              maxs[2]);
    debugLine(colorYellow, maxs[0], mins[1], maxs[2], maxs[0], maxs[1],
              maxs[2]);
    debugLine(colorYellow, maxs[0], maxs[1], maxs[2], mins[0], maxs[1],
              maxs[2]);
    debugLine(colorYellow, mins[0], maxs[1], maxs[2], mins[0], mins[1],
              maxs[2]);
  }
...
================
*/
idCameraPosition* idCameraDef::startNewCamera(
...
void idCameraDef::draw(bool editMode) {
  ...
  if (cameraPosition) {
    ...
    for (int i = 0; i < count; i++) {
      targetPositions[i]->draw((bool)((editMode || cameraRunning) &&
                                      i == activeTarget && !cameraEdit));
    }
  }
}
...
bool idCameraDef::getCameraInfo(long time,
  ...
  if ((time - startTime) / 1000 <= totalTime) {
    for (i = 0; i < events.Num(); i++) {
      if (time >= startTime + events[i]->getTime() &&
          !events[i]->getTriggered()) {
  ...
  for (i = 0; i < events.Num(); i++) {
    ...
    switch (events[i]->getType()) {
      ...
      case idCameraEvent::EVENT_WAIT: {
        ...
        cameraPosition->addVelocity(events[i]->getTime(),
                                    atof(events[i]->getParam()) * 1000, 0);
      ...
      case idCameraEvent::EVENT_SPEED: {
        /*
           ...
           the next five segments float adjust = atof(events[i]->getParam());
                                        int index = events[i]->getSegment();
                                        total = 0;
                                        count = 0;

                                        // get total amount of time over the
           remainder of the segment for (j = index; j <
           cameraSpline.numSegments() - 1; j++) { total +=
           cameraSpline.getSegmentTime(j + 1) - cameraSpline.getSegmentTime(j);
                                                count++;
                                        }
...
bool idCameraDef::load(const char* filename) {
  ...
  src =
      new idParser(filename, LEXFL_NOSTRINGCONCAT | LEXFL_NOSTRINGESCAPECHARS |
...
void idCameraDef::save(const char* filename) {
  ...
  if (f) {
    ...
    for (i = 0; i < numTargets(); i++) {
      targetPositions[i]->write(f,
                                va("target_%s", targetPositions[i]->typeStr()));
    }
...
const char* idCameraEvent::eventStr[] = {
    "NA",         "WAIT",    "TARGETWAIT", "SPEED",   "TARGET",
    "SNAPTARGET", "FOV",     "CMD",        "TRIGGER", "STOP",
    "CAMERA",     "FADEOUT", "FADEIN",     "FEATHER"};
...
float idCameraPosition::getVelocity(long t) {
  ...
  for (int i = 0; i < velocities.Num(); i++) {
    if (check >= velocities[i]->startTime &&
        check <= velocities[i]->startTime + velocities[i]->time) {
...
void idCameraPosition::write(idFile* f, const char* p) {
  ...
  for (int i = 0; i < velocities.Num(); i++) {
    f->Printf("\t\tvelocity %i %i %f\n", velocities[i]->startTime,
              velocities[i]->time, velocities[i]->speed);
  }
}
...
const idVec3* idSplinePosition::getPosition(long t) {
  ...
  if (i >= count - 1) {
    ...
    double percent =
        (lastDistance2 - targetDistance) / (lastDistance2 - lastDistance1);
    v2 *= (1.0f - percent);
    v1 *= percent;
    v2 += v1;
    interpolatedPos = v2;

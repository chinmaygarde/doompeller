...
int g_undoMaxMemorySize = 2 * 1024 * 1024;  // maximum undo memory (default 2
                                            // MB)
...
void Undo_ClearRedo(void) {
  ...
  for (redo = g_redolist; redo; redo = nextredo) {
    nextredo = redo->next;
    for (pBrush = redo->brushlist.next;
         pBrush != NULL && pBrush != &redo->brushlist; pBrush = pNextBrush) {
      pNextBrush = pBrush->next;
      Brush_Free(pBrush);
    }
    for (pEntity = redo->entitylist.next;
         pEntity != NULL && pEntity != &redo->entitylist;
         pEntity = pNextEntity) {
...
void Undo_Clear(void) {
  ...
  for (undo = g_undolist; undo; undo = nextundo) {
    nextundo = undo->next;
    for (pBrush = undo->brushlist.next;
         pBrush != NULL && pBrush != &undo->brushlist; pBrush = pNextBrush) {
      pNextBrush = pBrush->next;
      g_undoMemorySize -= Brush_MemorySize(pBrush);
      Brush_Free(pBrush);
    }
    for (pEntity = undo->entitylist.next;
         pEntity != NULL && pEntity != &undo->entitylist;
         pEntity = pNextEntity) {
      pNextEntity = pEntity->next;
...
void Undo_FreeFirstUndo(void) {
  ...
  for (pEntity = undo->entitylist.next;
       pEntity != NULL && pEntity != &undo->entitylist; pEntity = pNextEntity) {
    pNextEntity = pEntity->next;
    g_undoMemorySize -= Entity_MemorySize(pEntity);
    Entity_Free(pEntity);
  }
...
void Undo_GeneralStart(char* operation) {
  ...
  for (pBrush = active_brushes.next;
       pBrush != NULL && pBrush != &active_brushes; pBrush = pBrush->next) {
  ...
  for (pEntity = entities.next; pEntity != NULL && pEntity != &entities;
...
void Undo_AddBrush(brush_t* pBrush) {
  ...
  if (pBrush->owner &&
      !(pBrush->owner->eclass->nShowFlags & ECLASS_WORLDSPAWN)) {
...
void Undo_AddBrushList(brush_t* brushlist) {
  ...
  for (pBrush = brushlist->next; pBrush != NULL && pBrush != brushlist;
       pBrush = pBrush->next) {
    // if the brush is already in the undo
    if (Undo_BrushInUndo(g_lastundo, pBrush))
      continue;
...
void Undo_EndBrushList(brush_t* brushlist) {
  ...
  for (brush_t* pBrush = brushlist->next; pBrush != NULL && pBrush != brushlist;
       pBrush = pBrush->next) {
    pBrush->undoId = g_lastundo->id;
  }
}
...
void Undo_Undo(void) {
  ...
  for (pBrush = active_brushes.next;
       pBrush != NULL && pBrush != &active_brushes; pBrush = pNextBrush) {
    pNextBrush = pBrush->next;
    if (pBrush->undoId == undo->id) {
      // Brush_Free(pBrush);
  ...
  for (pEntity = entities.next; pEntity != NULL && pEntity != &entities;
    ...
    if (pEntity->undoId == undo->id) {
      ...
      for (pUndoEntity = undo->entitylist.next;
           pUndoEntity != NULL && pUndoEntity != &undo->entitylist;
           pUndoEntity = pUndoEntity->next) {
  ...
  for (pBrush = undo->brushlist.next;
       pBrush != NULL && pBrush != &undo->brushlist;
       pBrush = undo->brushlist.next) {
    g_undoMemorySize -= Brush_MemorySize(pBrush);
    Brush_RemoveFromList(pBrush);
  ...
  for (b = selected_brushes.next; b != NULL && b != &selected_brushes;
       b = next) {
    next = b->next;
    Brush_Build(b, true, false, false);
  }
...
void Undo_Redo(void) {
  ...
  for (pEntity = entities.next; pEntity != NULL && pEntity != &entities;
    ...
    if (pEntity->redoId == redo->id) {
      ...
      for (pRedoEntity = redo->entitylist.next;
           pRedoEntity != NULL && pRedoEntity != &redo->entitylist;
           pRedoEntity = pRedoEntity->next) {
  ...
  for (pBrush = redo->brushlist.next;
       pBrush != NULL && pBrush != &redo->brushlist;
       pBrush = redo->brushlist.next) {
    Brush_RemoveFromList(pBrush);
    Brush_AddToList(pBrush, &active_brushes);
    for (pEntity = entities.next; pEntity != NULL && pEntity != &entities;
         pEntity = pEntity->next) {

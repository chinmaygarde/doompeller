...
void CPreviewDlg::AddCommentedItems() {
  ...
  items.Clear();
  path =
      (currentMode == GUIS) ? "guis/guis.commented" : "models/models.commented";
    ...
    if (src.IsLoaded()) {
      ...
      while (src.ReadToken(&token)) {
        if (token == "{") {
          ...
          CommentedItem ci;
          if (src.ReadToken(&tok1) && src.ReadToken(&tok2) &&
              src.ReadToken(&tok3)) {
            ci.Name = tok1;
            ci.Path = tok2;
            ci.Comments = tok3;
...
void CPreviewDlg::OnTvnSelchangedTreeMedia(NMHDR* pNMHDR, LRESULT* pResult) {
  ...
  if (add) {
    add->EnableWindow(treeMedia.GetItemData(item) == GUIS ||
                      treeMedia.GetItemData(item) == MODELS);
  }
  if (item) {
    editInfo.SetWindowText("No comments for this item");
    int id = treeMedia.GetItemData(item);
    if (id == GUIS || id == MODELS || id == MATERIALS || id == WAVES ||
        id == PARTICLES || id == SKINS) {
...
void CPreviewDlg::OnBnClickedButtonAdd() {
  HTREEITEM item = treeMedia.GetSelectedItem();
  if (treeMedia.ItemHasChildren(item) == FALSE &&
      (treeMedia.GetItemData(item) == GUIS ||
       treeMedia.GetItemData(item) == MODELS)) {
    CCommentsDlg dlg;
    ...
    if (dlg.DoModal()) {
      ...
      const char* path;
      path = (currentMode == GUIS) ? "guis/guis.commented"

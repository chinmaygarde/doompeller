...
CConsoleDlg::CConsoleDlg(CWnd* pParent /*=NULL*/) : CDialog(CConsoleDlg::IDD) {
...
void CConsoleDlg::OnSize(UINT nType, int cx, int cy) {
  ...
  editInput.SetWindowPos(NULL, 4, rect.Height() - 4 - crect.Height(),
                         rect.Width() - 8, crect.Height(), SWP_SHOWWINDOW);
  editConsole.SetWindowPos(NULL, 4, 4, rect.Width() - 8,
                           rect.Height() - crect.Height() - 8, SWP_SHOWWINDOW);
}

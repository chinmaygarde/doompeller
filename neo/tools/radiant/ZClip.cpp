...
CZClip::~CZClip() {
  ...
  SaveRegistryInfo("radiant_ZClipBottom", &m_iZClipBottom,
                   sizeof(m_iZClipBottom));
}
...
void CZClip::Paint(void) {
  float x, y;
  int xCam = z.width / 4;  // hmmm, a rather unpleasant and obscure global name,
                           // but it was already called that so...

...
//++timo TODO : the whole CTextureBar has to be modified for the new texture
//code
...
CTextureBar::CTextureBar() : CDialogBar() {
...
void CTextureBar::OnDeltaposSpinHScale(NMHDR* pNMHDR, LRESULT* pResult) {
  ...
  *pResult = 0;
  if (pNMUpDown->iDelta < 0)
    ...
    Select_ScaleTexture((float)-abs(g_qeglobals.d_savedinfo.m_nTextureTweak),
                        0);
  GetSurfaceAttributes();
}
...
void CTextureBar::OnDeltaposSpinVScale(NMHDR* pNMHDR, LRESULT* pResult) {
  ...
  *pResult = 0;
  if (pNMUpDown->iDelta < 0)
    ...
    Select_ScaleTexture(0,
                        (float)-abs(g_qeglobals.d_savedinfo.m_nTextureTweak));
  GetSurfaceAttributes();
}
...
void CTextureBar::GetSurfaceAttributes() {
  texdef_t* pt =
      (g_ptrSelectedFaces.GetSize() > 0)
          ? &(reinterpret_cast<face_t*>(g_ptrSelectedFaces.GetAt(0)))->texdef
...
void CTextureBar::SetSurfaceAttributes() {
  if (g_ptrSelectedFaces.GetSize() > 0) {
    if (g_qeglobals.m_bBrushPrimitMode) {
      common->Printf(
          "Warning : brush primitive mode not implemented in CTextureBar");
    }

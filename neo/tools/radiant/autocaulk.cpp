...
void Select_AutoCaulk() {
  ...
  brush_t* next;
  for (brush_t* pSelectedBrush = selected_brushes.next;
       pSelectedBrush != &selected_brushes; pSelectedBrush = next) {
    ...
    for (pSelectedFace = pSelectedBrush->brush_faces; pSelectedFace;
         pSelectedFace = pSelectedFace->next) {
    ...
    for (int iBrushListToScan = 0; iBrushListToScan < 2; iBrushListToScan++) {
      brush_t* snext;
      for (brush_t* pScannedBrush =
               (iBrushListToScan ? active_brushes.next : selected_brushes.next);
           pScannedBrush !=
           (iBrushListToScan ? &active_brushes : &selected_brushes);
           pScannedBrush = snext) {
        ...
        if (pScannedBrush->owner->eclass->fixedsize || pScannedBrush->pPatch ||
            pScannedBrush->hiddenBrush)
          continue;
        ...
        for (pSelectedFace = pSelectedBrush->brush_faces; pSelectedFace;
          ...
          for (face_t* pScannedFace = pScannedBrush->brush_faces; pScannedFace;
            ...
            if (!pScannedWinding)
              continue;  // freed face, probably won't happen here, but who
                         // knows with this program?
            ...
            */
            {
              ...
              VectorAdd(pSelectedFace->plane.Normal(),
                        pScannedFace->plane.Normal(), v3ZeroTest);
              if (v3ZeroTest == v3Zero) {
                ...
                for (int _i = 0; _i < 3; _i++) {
                  fTotalDist += fabs(DotProduct(pSelectedFace->plane.Normal(),
                                                (*pSelectedWinding)[0]) -
                                     DotProduct(pSelectedFace->plane.Normal(),
                                                (*pScannedWinding)[i]));
                }
                ...
                int iPoint;
                for (iPoint = 0; iPoint < pScannedWinding->GetNumPoints();
                     iPoint++) {
                  AddPointToBounds((*pScannedWinding)[iPoint].ToVec3(),
                                   v3ScannedBoundsMins, v3ScannedBoundsMaxs);
                }
                ...
                //
                bool bWithin = true;
                for (iPoint = 0; iPoint < pSelectedWinding->GetNumPoints();
                     iPoint++) {
                  for (int iXYZ = 0; iXYZ < 3; iXYZ++) {
                    float f = floor((*pSelectedWinding)[iPoint][iXYZ] + 0.5);
                    if (!(f >= v3ScannedBoundsMins[iXYZ] &&

...
===============
*/
extern void Face_GetScale_BrushPrimit(face_t* face,
                                      float* s,
                                      float* t,
                                      float* rot);
...
#define IDC_VSHIFTA 0
  ...
  if (pBar->GetSafeHwnd() == ::GetDlgItem(GetSafeHwnd(), IDC_ROTATEA)) {
    ...
    Select_ScaleTexture((up) ? -m_horzScale : m_horzScale, 0, true,
                        (m_absolute != FALSE));
  } else if (pBar->GetSafeHwnd() == ::GetDlgItem(GetSafeHwnd(), IDC_VSCALEA)) {
    Select_ScaleTexture(0, (up) ? -m_vertScale : m_vertScale, true,
                        (m_absolute != FALSE));
...
void DoSurface(void) {
  ...
  if (g_surfwin == NULL && g_dlgSurface.GetSafeHwnd() == NULL) {
    ...
    if (LoadRegistryInfo("radiant_SurfaceWindow", &rct, &lSize)) {
      g_dlgSurface.SetWindowPos(NULL, rct.left, rct.top, 0, 0,
...
void CSurfaceDlg::OnBtnCancel() {
  ...
  if (g_changed_surface) {
    ...
    if (!g_qeglobals.m_bBrushPrimitMode) {
      common->Printf(
          "Warning : non brush primitive mode call to CSurfaceDlg::GetTexMods "
          "broken\n");
...
void CSurfaceDlg::OnBtnPatchnatural() {
  Select_SetTexture(&g_qeglobals.d_texturewin.texdef,
                    &g_qeglobals.d_texturewin.brushprimit_texdef, false);
  ...
  if (pMsg->message == WM_KEYDOWN) {
    if (pMsg->wParam == VK_RETURN) {
      if (focusControl) {
        ...
        } else if (focusControl == &m_wndHeight ||

...
void CTearoffContainerWindow::SetDialog(CWnd* dlg, int ID) {
  ...
  rect.OffsetRect(
      point);  // move the window slightly so you can tell it's been popped up
...
BOOL CTearoffContainerWindow::PreTranslateMessage(MSG* pMsg) {
  if (pMsg->message == WM_NCLBUTTONUP) {
    /*		CRect rect;
                    ...
                    rect.DeflateRect( 0,0,0,rect.Height() -
       GetSystemMetrics(SM_CYSMSIZE)); if ( m_DockManager->RectWithinDockManager
       ( rect ))
                    {

...
void SetKeyMat3(entity_t* ent, const char* key, idMat3 m) {
  ...
  sprintf(str, "%g %g %g %g %g %g %g %g %g", m[0][0], m[0][1], m[0][2], m[1][0],
...
bool GetMatrixForKey(entity_t* ent, const char* key, idMat3& mat) {
  ...
  k = ValueForKey(ent, key);
  if (k && strlen(k) > 0) {
    sscanf(k, "%f %f %f %f %f %f %f %f %f ", &mat[0][0], &mat[0][1], &mat[0][2],
           &mat[1][0], &mat[1][1], &mat[1][2], &mat[2][0], &mat[2][1],
           &mat[2][2]);
    return true;
  } else {
    mat.Identity();
...
/*
 =======================================================================================================================
    Entity_Free Frees the entity and any brushes is has. The entity is removed
 from the global entities list.
 =======================================================================================================================
 */
...
const EpairFixup FloatFixups[] = {
    {"origin", 1},       {"rotation", 2},    {"_color", 1},
    {"falloff", 0},      {"light", 0},       {"light_target", 1},
    {"light_up", 1},     {"light_right", 1}, {"light_start", 1},
    {"light_center", 1}, {"light_end", 1},   {"light_radius", 1},
    {"light_origin", 1}};
    ...
    for (int j = 0; j < FixupCount; j++) {
      if (kv->GetKey().Icmp(FloatFixups[j].name) == 0) {
        ...
        if (FloatFixups[j].type == 1) {
          ...
          idMat3 mat;
          sscanf(kv->GetValue().c_str(), "%f %f %f %f %f %f %f %f %f ",
                 &mat[0][0], &mat[0][1], &mat[0][2], &mat[1][0], &mat[1][1],
                 &mat[1][2], &mat[2][0], &mat[2][1], &mat[2][2]);
          sprintf(val, "%g %g %g %g %g %g %g %g %g", mat[0][0], mat[0][1],
...
entity_t* Entity_PostParse(entity_t* ent, brush_t* pList) {
  ...
  if (e->fixedsize || hasModel) {  // fixed size entity
    ...
    if (hasModel) {
      ...
      if (dynamic_cast<idRenderModelPrt*>(modelHandle) ||
          dynamic_cast<idRenderModelLiquid*>(modelHandle)) {
        bo.Zero();
        bo.ExpandSelf(12.0f);
      } else {
      ...
      extern void Brush_Rotate(brush_t * b, idMat3 matrix, idVec3 origin,
                               bool bBuild);
      extern void Brush_Rotate(brush_t * b, idVec3 rot, idVec3 origin,
                               bool bBuild);
    ...
    if (needsOrigin) {
      ...
      sprintf(text, "%i %i %i", (int)ent->origin[0], (int)ent->origin[1],
              (int)ent->origin[2]);
      SetKeyValue(ent, "origin", text);
    }

    if (!(e->nShowFlags & ECLASS_WORLDSPAWN)) {
      if (e->defArgs.FindKey("model") == NULL &&
          (pModel == NULL || (pModel && strlen(pModel) == 0))) {
...
/*
 =======================================================================================================================
    Entity_Parse If onlypairs is set, the classname info will not be looked up,
 and the entity will not be added to the global list. Used for parsing the
 project.
 =======================================================================================================================
 */
...
void Entity_Write(entity_t* e, FILE* f, bool use_region) {
  ...
  for (int j = 0; j < count; j++) {
    fprintf(f, "\"%s\" \"%s\"\n", e->epairs.GetKeyVal(j)->GetKey().c_str(),
            e->epairs.GetKeyVal(j)->GetValue().c_str());
  }
...
bool IsBrushSelected(brush_t* bSel) {
  for (brush_t* b = selected_brushes.next; b != NULL && b != &selected_brushes;
       b = b->next) {
    if (b == bSel) {
      return true;
    }
  ...
  if (!EntityHasModel(e)) {
    ...
    for (b = e->brushes.onext; b != &e->brushes; b = b->onext) {
      ...
      if (IsBrushSelected(b)) {
        ...
        count++;
        Brush_Write(b, pMemFile, e->origin,
                    (g_PrefsDlg.m_bNewMapFormat != FALSE));
      }
    }
  }
...
/*
 =======================================================================================================================
    Entity_Create Creates a new entity out of the selected_brushes list. If the
 entity class is fixed size, the brushes are only used to find a midpoint.
...
void Entity_UpdateSoundEmitter(entity_t* ent) {
  ...
  if (g_pParentWnd->GetCamera()->GetSoundMode() &&
    ...
    if (v[0]) {
      ...
      if (!sound.waitfortrigger) {  // waitfortrigger will not start playing
                                    // immediately
        if (!ent->soundEmitter) {

...
CTabsDlg::CTabsDlg(UINT ID, CWnd* pParent /*=NULL*/) : CDialog(ID, pParent) {
...
void CTabsDlg::DockWindow(int ID, bool dock) {
  ...
  if (!dock) {
    ...
    info->m_Container.CreateEx(
        WS_EX_TOOLWINDOW, classname, info->m_Title,
        WS_THICKFRAME | WS_SYSMENU | WS_POPUP | WS_CAPTION, rect, this, 0);
    ...
    info->m_TabControlIndex =
        m_Tabs.InsertItem(TCIF_TEXT | TCIF_IMAGE | TCIF_PARAM, 0, info->m_Title,
                          info->m_ImageID, (LPARAM)info);

    info->m_Window->SetParent(this);
    info->m_Window->ShowWindow(TRUE);

    info->m_Container.SetDockManager(
        NULL);  // so it doesn't try to call back and redock this window
...
int CTabsDlg::PreTranslateMessage(MSG* msg) {
  ...
  // steal lbutton clicks for the main dialog too, but let the tabs do their
  // default thing as well
...
void CTabsDlg::OnLButtonUp(UINT nFlags, CPoint point) {
  if (m_DragTabActive && ((abs(point.x - m_DragDownPoint.x) > 50) ||
                          (abs(point.y - m_DragDownPoint.y) > 50))) {
    HandleUndock();
    m_DragTabActive = false;
  }
...
void CTabsDlg::SaveWindowPlacement(int ID) {
  ...
  for (pos = m_Windows.GetStartPosition(); pos != NULL;) {
    ...
    if ((info->m_State == DockedWindowInfo::FLOATING) &&
        ((ID == -1) || (ID == info->m_ID))) {

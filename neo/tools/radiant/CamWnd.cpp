...
void CCamWnd::OnPaint() {
  ...
  if (!qwglMakeCurrent(dc.m_hDC, win32.hGLRC)) {
    ...
    common->Printf("Please restart " EDITOR_WINDOWTEXT
                   " if the camera view is not working\n");
  } else {
    ...
    if (g_bClipMode) {
      if (g_Clip1.Set() && g_Clip2.Set()) {
        g_pSplitList =
            ((g_pParentWnd->ActiveXY()->GetViewType() == XZ) ? !g_bSwitch
                                                             : g_bSwitch)
                ? &g_brBackSplits
                : &g_brFrontSplits;
      }
...
void CCamWnd::OnMouseMove(UINT nFlags, CPoint point) {
  ...
  if (GetCapture() == this && (GetAsyncKeyState(VK_MENU) & 0x8000) &&
    ...
    if (GetAsyncKeyState(VK_CONTROL) & 0x8000) {
      ...
      Select_ScaleTexture((float)point.x - m_ptLastCursor.x,
                          (float)m_ptLastCursor.y - point.y);
    } else {
      Select_ShiftTexture((float)point.x - m_ptLastCursor.x,
                          (float)m_ptLastCursor.y - point.y);
    }
  } else {
...
int CCamWnd::OnCreate(LPCREATESTRUCT lpCreateStruct) {
  ...
  if (!qwglUseFontBitmaps(hDC, 0, 255, g_qeglobals.d_font_list)) {
    common->Warning("wglUseFontBitmaps failed (%d).  Trying again.",
                    GetLastError());
    ...
    if (!qwglUseFontBitmaps(hDC, 0, 255, g_qeglobals.d_font_list)) {
      common->Warning("wglUseFontBitmaps failed again (%d).  Trying outlines.",
                      GetLastError());

      if (!qwglUseFontOutlines(hDC, 0, 255, g_qeglobals.d_font_list, 0.0f, 0.1f,
                               WGL_FONT_LINES, NULL)) {
        common->Warning(
            "wglUseFontOutlines also failed (%d), no coordinate text will be "
            "visible.",
            GetLastError());
      }
    }
...
void CCamWnd::Cam_BuildMatrix() {
  ...
  xa = ((renderMode) ? -m_Camera.angles[PITCH] : m_Camera.angles[PITCH]) *
       idMath::M_DEG2RAD;
...
void CCamWnd::Cam_MouseControl(float dtime) {
...
#endif
  {
    ...
    VectorMA(m_Camera.origin, yf * dtime * g_PrefsDlg.m_nMoveSpeed,
...
extern void glLabeledPoint(idVec4& color,
                           idVec3& point,
                           float size,
                           const char* label);
void DrawAxial(face_t* selFace) {
  if (g_bAxialMode) {
    ...
    for (int j = 0; j < selFace->face_winding->GetNumPoints(); j++) {
      glLabeledPoint(idVec4(1, 1, 1, 1), (*selFace->face_winding)[j].ToVec3(),
                     3, va("%i", j));
    }
...
void CCamWnd::SetProjectionMatrix() {
  float xfov = 90;
  float yfov =
      2 * atan((float)m_Camera.height / m_Camera.width) * idMath::M_RAD2DEG;
#if 0
...
void CCamWnd::Cam_Draw() {
  ...
  idMat3 axis = idAngles(-m_Camera.angles.pitch, m_Camera.angles.yaw,
                         m_Camera.angles.roll)
                    .ToMat3();
  ...
  brush_t* pList =
      (g_bClipMode && g_pSplitList) ? g_pSplitList : &selected_brushes;
  ...
  setGLMode(m_Camera.draw_mode);
  qglDisable(GL_LIGHTING);
  qglColor4f(g_qeglobals.d_savedinfo.colors[COLOR_SELBRUSHES][0],
             g_qeglobals.d_savedinfo.colors[COLOR_SELBRUSHES][1],
             g_qeglobals.d_savedinfo.colors[COLOR_SELBRUSHES][2], 0.25f);
  qglEnable(GL_BLEND);
  qglPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
  qglBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
  globalImages->BindNull();
  ...
  if (g_qeglobals.d_select_mode == sel_vertex) {
    ...
    for (i = 0; i < g_qeglobals.d_numedges; i++) {
      ...
      qglVertex3f((v1[0] + v2[0]) * 0.5f, (v1[1] + v2[1]) * 0.5f,
                  (v1[2] + v2[2]) * 0.5f);
    }
  ...
  g_splineList->draw(
      static_cast<bool>(g_qeglobals.d_select_mode == sel_addpoint ||
...
================
*/
int Brush_ToTris(brush_t* brush,
                 idTriList* tris,
                 idMatList* mats,
                 bool models,
                 bool bmodel);
  ...
  if (v && *v) {
    ...
    if (!stricmp(name, v)) {
      ...
      if (md5) {
        ...
        refent.joints =
            (idJointMat*)Mem_Alloc16(refent.numJoints * sizeof(*refent.joints));
        const idMD5Anim* anim = gameEdit->ANIM_GetAnimFromEntityDef(
            spawnArgs.GetString("classname"), str);
        int frame = spawnArgs.GetInt("frame") + 1;
        if (frame < 1) {
          frame = 1;
        }
        const idVec3& offset = gameEdit->ANIM_GetModelOffsetFromEntityDef(
            spawnArgs.GetString("classname"));
        gameEdit->ANIM_CreateAnimFrame(md5, anim, refent.numJoints,
                                       refent.joints, (frame * 1000) / 24,
                                       offset, false);
      }
      if (ent->modelDef >= 0) {
        g_qeglobals.rw->FreeEntityDef(ent->modelDef);
  ...
  if (f) {
    ...
    for (i = 0; i < tris->Num(); i++) {
      ...
      for (j = 0; j < tri->numVerts; j++) {
        f->Printf("v %f %f %f\n", tri->verts[j].xyz.x, tri->verts[j].xyz.z,
                  -tri->verts[j].xyz.y);
      }
      for (j = 0; j < tri->numVerts; j++) {
      ...
      for (j = 0; j < tri->numVerts; j++) {
        f->Printf("vn %f %f %f\n", tri->verts[j].normal.x,
                  tri->verts[j].normal.y, tri->verts[j].normal.z);
      }
    ...
    if (model) {
      ...
      for (int i = 0; i < model->NumSurfaces(); i++) {
        ...
        for (int j = 0; j < tri2->numVerts; j++) {
          idVec3 v;
          if (matrix) {
            v = tri2->verts[j].xyz * brush->owner->rotation +
                brush->owner->origin;
          } else {
            v = tri2->verts[j].xyz;
            ...
            float y = v[1];
            if (a) {
              float x2 = (((x - brush->owner->origin[0]) * c) -
                          ((y - brush->owner->origin[1]) * s)) +
                         brush->owner->origin[0];
              float y2 = (((x - brush->owner->origin[0]) * s) +
                          ((y - brush->owner->origin[1]) * c)) +
...
int Brush_ToTris(brush_t* brush,
  ...
  if (brush->pPatch) {
    ...
    if (pm->explicitSubdivisions) {
      ...
      cp->Subdivide(DEFAULT_CURVE_MAX_ERROR, DEFAULT_CURVE_MAX_ERROR,
...
void Select_ToOBJ() {
  int i;
  CFileDialog dlgFile(FALSE, "obj", NULL,
...
void Select_ToCM() {
  CFileDialog dlgFile(FALSE, "lwo, ase", NULL, 0,
  ...
  if (dlgFile.DoModal() == IDOK) {
    ...
    collisionModelManager->WriteCollisionModelForMapEntity(mapEnt,
                                                           name.c_str());
...
bool CCamWnd::UpdateRenderEntities() {
  ...
  for (entity_t* ent = entities.next; ent != &entities; ent = ent->next) {
    BuildEntityRenderState(
        ent, (ent->lightDef != -1 || ent->modelDef != -1 || ent->soundEmitter)
                 ? true
                 : false);
...
void CCamWnd::FreeRendererState() {
  for (entity_t* ent = entities.next; ent != &entities; ent = ent->next) {
    ...
    if (ent->modelDef >= 0) {
      renderEntity_t* refent = const_cast<renderEntity_t*>(
          g_qeglobals.rw->GetRenderEntity(ent->modelDef));
      if (refent) {
...
void CCamWnd::DrawEntityData() {
  ...
  int pass = 0;
  while (brushList) {
    for (brush_t* brush = brushList->next; brush != brushList;
         brush = brush->next) {
      if (CullBrush(brush, true)) {
        continue;
      }
      ...
      if ((pass == 1 && selectMode) ||
          (entityMode && pass == 0 && brush->owner->lightDef >= 0)) {
...
void CCamWnd::Cam_Render() {
  ...
  // the editor uses opposite pitch convention
  refdef.viewaxis = idAngles(-m_Camera.angles.pitch, m_Camera.angles.yaw,
                             m_Camera.angles.roll)
                        .ToMat3();

...
void idGLDrawableMaterial::draw(int x, int y, int w, int h) {
  ...
  if (mat) {
    ...
    if (worldDirty) {
      ...
      idImage* img = (mat->GetNumStages() > 0) ? mat->GetStage(0)->texture.image
                                               : mat->GetEditorImage();

      if (img == NULL) {
        common->Warning("Unable to load image for preview for %s",
                        mat->GetName());
        return;
      }
      ...
      tris->verts[1].normal = tris->verts[2].normal = tris->verts[3].normal =
          tris->verts[0].normal;
      AddTris(tris, mat);
...
void idGLDrawableMaterial::setMedia(const char* name) {
  ...
  if (name && *name) {
    ...
    if (material) {
      const shaderStage_t* stage =
          (material->GetNumStages() > 0) ? material->GetStage(0) : NULL;
      if (stage) {
  ...
  if (material && img) {
    float size = (img->uploadWidth > img->uploadHeight) ? img->uploadWidth
                                                        : img->uploadHeight;

...
void CEntityDlg::OnSize(UINT nType, int cx, int cy) {
  ...
  staticTitle.SetWindowPos(NULL, 4, 4, rect.Width() - 8, crect.Height(),
                           SWP_SHOWWINDOW);
  int top = 4 + crect.Height() + 4;
  comboClass.GetWindowRect(crect);
  btnCreate.GetWindowRect(crect2);
  comboClass.SetWindowPos(NULL, 4, top, rect.Width() - 12 - crect2.Width(),
                          crect.Height(), SWP_SHOWWINDOW);
  btnCreate.SetWindowPos(NULL, rect.Width() - crect2.Width() - 4, top,
                         crect2.Width(), crect.Height(), SWP_SHOWWINDOW);
  top += crect.Height() + 4;
  ...
  staticKey.SetWindowPos(NULL, 4, top + 2, crect.Width(), crect.Height(),
                         SWP_SHOWWINDOW);
  int left = 4 + crect.Width() + 4;
  int pad = crect.Width();
  editKey.GetWindowRect(crect);
  editKey.SetWindowPos(NULL, left, top, rect.Width() - 12 - pad, crect.Height(),
                       SWP_SHOWWINDOW);
  top += crect.Height() + 4;
  staticVal.GetWindowRect(crect);
  staticVal.SetWindowPos(NULL, 4, top + 2, crect.Width(), crect.Height(),
                         SWP_SHOWWINDOW);
  editVal.GetWindowRect(crect);
  bh = crect.Height();
  editVal.SetWindowPos(NULL, left, top, rect.Width() - 16 - bh - pad,
                       crect.Height(), SWP_SHOWWINDOW);
  btnBrowse.SetWindowPos(NULL, rect.right - 4 - bh, top, bh, bh,
                         SWP_SHOWWINDOW);
  top += crect.Height() + 8;
  btnModel.GetWindowRect(crect);
  btnModel.SetWindowPos(NULL, rect.right - 4 - crect.Width(), top + 8,
                        crect.Width(), crect.Height(), SWP_SHOWWINDOW);
  btnSound.SetWindowPos(NULL, rect.right - 4 - crect.Width(),
                        top + 12 + crect.Height(), crect.Width(),
                        crect.Height(), SWP_SHOWWINDOW);
  btnGui.SetWindowPos(NULL, rect.right - 4 - crect.Width(),
                      top + 16 + crect.Height() * 2, crect.Width(),
                      crect.Height(), SWP_SHOWWINDOW);
  btnParticle.SetWindowPos(NULL, rect.right - 8 - (crect.Width() * 2),
                           top + 16 + crect.Height() * 2, crect.Width(),
                           crect.Height(), SWP_SHOWWINDOW);
  btnSkin.SetWindowPos(NULL, rect.right - 8 - (crect.Width() * 2),
                       top + 12 + crect.Height(), crect.Width(), crect.Height(),
                       SWP_SHOWWINDOW);
  btnCurve.SetWindowPos(NULL, rect.right - 8 - (crect.Width() * 2), top + 8,
                        crect.Width(), crect.Height(), SWP_SHOWWINDOW);

  //*************************************
  // animation controls
  //*************************************
  int rightAnimAreaBorder =
      rect.right - 75 - crect.Width(); /*models, etc button width*/

  btnStopAnim.GetWindowRect(crect);
  btnStopAnim.SetWindowPos(NULL, rightAnimAreaBorder - crect.Width(), top + 8,
                           crect.Width(), crect.Height(), SWP_SHOWWINDOW);

  left = rightAnimAreaBorder - crect.Width() - 4;
  btnPlayAnim.GetWindowRect(crect);
  btnPlayAnim.SetWindowPos(NULL, left - crect.Width(), top + 8, crect.Width(),
                           crect.Height(), SWP_SHOWWINDOW);

  left -= crect.Width() + 4;
  cbAnimations.GetWindowRect(crect);
  cbAnimations.SetWindowPos(NULL, left - crect.Width(), top + 8, crect.Width(),
                            crect.Height(), SWP_SHOWWINDOW);

  staticFrame.GetWindowRect(crect);
  staticFrame.SetWindowPos(NULL, rightAnimAreaBorder - crect.Width(), top + 34,
                           crect.Width(), crect.Height(), SWP_SHOWWINDOW);

  left = rightAnimAreaBorder - crect.Width() - 4;

  slFrameSlider.GetWindowRect(crect);
  slFrameSlider.SetWindowPos(NULL, left - crect.Width(), top + 32,
                             crect.Width(), crect.Height(), SWP_SHOWWINDOW);
  ...
  btnUp.SetWindowPos(NULL, 4 + 2 + 2 + 6 + bh * 3, top + bh / 2, bh, bh,
                     SWP_SHOWWINDOW);
  btnDown.SetWindowPos(NULL, 4 + 2 + 2 + 6 + bh * 3, top + bh / 2 + bh + 2, bh,
                       bh, SWP_SHOWWINDOW);
  top += bh + 2;
  ...
  if (index != LB_ERR) {
    ...
    if (ent) {
      if (selected_brushes.next == &selected_brushes) {
        ...
        for (brush_t* b = selected_brushes.next->next; b != &selected_brushes;
             b = b->next) {
      ...
      CPropertyItem* pi =
          new CPropertyItem("Usage:", ent->desc.c_str(), PIT_VAR, "");
      listVars.AddPropItem(pi);

      int c = ent->vars.Num();
      for (int i = 0; i < c; i++) {
        pi = new CPropertyItem(ent->vars[i].name.c_str(),
                               ent->vars[i].desc.c_str(), PIT_VAR, "");
...
void CEntityDlg::SetKeyValPairs(bool updateAnims) {
  if (editEntity) {
    ...
    for (int i = 0; i < c; i++) {
      ...
      CPropertyItem* pi = new CPropertyItem(
          kv->GetKey().c_str(), kv->GetValue().c_str(), PIT_EDIT, "");
      bool found = false;
    ...
    if (updateAnims) {
      ...
      num =
          gameEdit->ANIM_GetNumAnimsFromEntityDef(&editEntity->eclass->defArgs);
      for (i = 0; i < num; i++) {
        cbAnimations.AddString(gameEdit->ANIM_GetAnimNameFromEntityDef(
            &editEntity->eclass->defArgs, i));
      }
...
static int TabOrder[] = {
    IDC_COMBO_CLASS, IDC_BUTTON_CREATE,
    // IDC_EDIT_INFO,
    IDC_LIST_KEYVAL, IDC_EDIT_KEY, IDC_EDIT_VAL, IDC_BUTTON_BROWSE, IDC_E_135,
    IDC_E_90, IDC_E_45, IDC_E_180, IDC_E_0, IDC_E_225, IDC_E_270, IDC_E_315,
    IDC_E_UP, IDC_E_DOWN, IDC_BUTTON_MODEL, IDC_BUTTON_SOUND, IDC_BUTTON_GUI,
...
BOOL CEntityDlg::PreTranslateMessage(MSG* pMsg) {
  ...
  if (pMsg->message == WM_KEYDOWN && pMsg->wParam == VK_RIGHT &&
      pMsg->hwnd == slFrameSlider.GetSafeHwnd()) {
...
void CEntityDlg::AddProp() {
  ...
  bool isModel =
      static_cast<bool>((stricmp(Key, "model") == 0 && Value.GetLength() > 0));
  ...
  if (multipleEntities) {
    ...
    for (b = selected_brushes.next; b != &selected_brushes; b = b->next) {
      if (isName) {
        ...
        if (!((isModel || isOrigin) &&
              (b->owner->eclass->nShowFlags & ECLASS_WORLDSPAWN))) {
    ...
    if (isName) {
      ...
      if (!((isModel || isOrigin) &&
            (editEntity->eclass->nShowFlags & ECLASS_WORLDSPAWN))) {
    ...
    if (isModel && !(editEntity->eclass->nShowFlags & ECLASS_WORLDSPAWN)) {
      ...
      if (dynamic_cast<idRenderModelPrt*>(selected_brushes.next->modelHandle) ||
          dynamic_cast<idRenderModelLiquid*>(
              selected_brushes.next->modelHandle)) {
        bo.Zero();
        bo.ExpandSelf(12.0f);
      } else {
...
void CEntityDlg::CreateEntity() {
  ...
  if (pWnd) {
    ...
    if (selected_brushes.next == &selected_brushes) {
      pBrush =
          CreateEntityBrush(g_nSmartX, rctZ.Height() - 1 - g_nSmartY, pWnd);
      forceFixed = true;
    }
  } else {
    if (selected_brushes.next == &selected_brushes) {
      MessageBox("You must have a selected brush to create an entity", "info",
                 0);
      return;
    }
  }
...
void CEntityDlg::OnCbnAnimationChange() {
  ...
  if (sel != -1) {
    ...
    if (animName.GetLength() > 0) {
      ...
      // update the slider
      currentAnimation = gameEdit->ANIM_GetAnimFromEntityDef(
          editEntity->eclass->name, animName.GetBuffer(0));
      currentAnimationFrame = 0;

      if (currentAnimation) {
        slFrameSlider.SetRange(1, gameEdit->ANIM_GetNumFrames(currentAnimation),
                               TRUE);
...
void CEntityDlg::OnTimer(UINT nIDEvent) {
  ...
  if (currentAnimation) {
    currentAnimationFrame = ((currentAnimationFrame++) %
                             gameEdit->ANIM_GetNumFrames(currentAnimation));
...
void CEntityDlg::EditCurvePoints() {
  ...
  for (int i = 0; i < c; i++) {
    if (g_qeglobals.d_numpoints < MAX_POINTS - 1) {
      g_qeglobals.d_points[g_qeglobals.d_numpoints++] =
          editEntity->curve->GetValue(i);
    }
  }
    ...
    if (PointInMoveList(editEntity->curve->GetValueAddress(i)) >= 0) {
      if (i == editEntity->curve->GetNumValues() - 1) {
        ...
        for (int j = 0; j < editEntity->curve->GetNumValues(); j++) {
          ...
          newCurve->AddValue(newCurve->GetNumValues() * 100,
                             editEntity->curve->GetValue(j));
        }
...
void CEntityDlg::SelectCurvePointByRay(const idVec3& org,
  ...
  g_qeglobals.d_move_points[g_qeglobals.d_num_move_points++] =
      editEntity->curve->GetValueAddress(besti);
}

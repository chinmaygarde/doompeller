...
double SarrusDet(idVec3D a, idVec3D b, idVec3D c) {
  return (double)a[0] * (double)b[1] * (double)c[2] +
         (double)b[0] * (double)c[1] * (double)a[2] +
         (double)c[0] * (double)a[1] * (double)b[2] -
         (double)c[0] * (double)b[1] * (double)a[2] -
         (double)a[1] * (double)b[0] * (double)c[2] -
         (double)a[0] * (double)b[2] * (double)c[1];
}
...
void FaceToBrushPrimitFace(face_t* f) {
  ...
  f->brushprimit_texdef.coords[0][0] =
      ST[1][3] - f->brushprimit_texdef.coords[0][2];
  f->brushprimit_texdef.coords[1][0] =
      ST[1][4] - f->brushprimit_texdef.coords[1][2];
  f->brushprimit_texdef.coords[0][1] =
      ST[2][3] - f->brushprimit_texdef.coords[0][2];
  f->brushprimit_texdef.coords[1][1] =
      ST[2][4] - f->brushprimit_texdef.coords[1][2];
}
...
void EmitBrushPrimitTextureCoordinates(face_t* f,
  ...
  if (w) {
    for (i = 0; i < w->GetNumPoints(); i++) {
      ...
      (*w)[i][3] = f->brushprimit_texdef.coords[0][0] * x +
                   f->brushprimit_texdef.coords[0][1] * y +
                   f->brushprimit_texdef.coords[0][2];
      (*w)[i][4] = f->brushprimit_texdef.coords[1][0] * x +
                   f->brushprimit_texdef.coords[1][1] * y +
  ...
  if (patch) {
    ...
    for (i = 0; i < patch->width; i++) {
      for (j = 0; j < patch->height; j++) {
        ...
        patch->ctrl(i, j).st.x = f->brushprimit_texdef.coords[0][0] * x +
                                 f->brushprimit_texdef.coords[0][1] * y +
                                 f->brushprimit_texdef.coords[0][2];
        patch->ctrl(i, j).st.y = f->brushprimit_texdef.coords[1][0] * x +
                                 f->brushprimit_texdef.coords[1][1] * y +
                                 f->brushprimit_texdef.coords[1][2];
      }
    }
...
void TexMatToFakeTexCoords(float texMat[2][3],
  ...
  if (idMath::Fabs(texMat[0][0] * texMat[0][1] + texMat[1][0] * texMat[1][1]) >
      ZERO_EPSILON) {
    common->Printf(
...
void ConvertTexMatWithQTexture(float texMat1[2][3],
  ...
  float s1, s2;
  s1 =
      (qtex1 ? static_cast<float>(qtex1->GetEditorImage()->uploadWidth)
             : 2.0f) /
      (qtex2 ? static_cast<float>(qtex2->GetEditorImage()->uploadWidth) : 2.0f);
  s2 = (qtex1 ? static_cast<float>(qtex1->GetEditorImage()->uploadHeight)
              : 2.0f) /
       (qtex2 ? static_cast<float>(qtex2->GetEditorImage()->uploadHeight)
              : 2.0f);
  s1 *= sScale;
  s2 *= tScale;
...
void Face_MoveTexture_BrushPrimit(face_t* f, idVec3 delta) {
  ...
  D[0][1] =
      f->brushprimit_texdef.coords[0][0] + f->brushprimit_texdef.coords[0][2];
  D[0][2] =
      f->brushprimit_texdef.coords[0][1] + f->brushprimit_texdef.coords[0][2];
  D[1][0] = f->brushprimit_texdef.coords[1][2];
  D[1][1] =
      f->brushprimit_texdef.coords[1][0] + f->brushprimit_texdef.coords[1][2];
  D[1][2] =
      f->brushprimit_texdef.coords[1][1] + f->brushprimit_texdef.coords[1][2];
...
void Face_FitTexture_BrushPrimit(face_t* f,
  ...
  Mf[0][0] = N.coords[0][0] * f->brushprimit_texdef.coords[0][0] +
             N.coords[0][1] * f->brushprimit_texdef.coords[1][0];
  Mf[0][1] = N.coords[0][0] * f->brushprimit_texdef.coords[0][1] +
             N.coords[0][1] * f->brushprimit_texdef.coords[1][1];
  Mf[0][2] = N.coords[0][0] * f->brushprimit_texdef.coords[0][2] +
             N.coords[0][1] * f->brushprimit_texdef.coords[1][2] +
             N.coords[0][2];
  Mf[1][0] = N.coords[1][0] * f->brushprimit_texdef.coords[0][0] +
             N.coords[1][1] * f->brushprimit_texdef.coords[1][0];
  Mf[1][1] = N.coords[1][0] * f->brushprimit_texdef.coords[0][1] +
             N.coords[1][1] * f->brushprimit_texdef.coords[1][1];
  Mf[1][2] = N.coords[1][0] * f->brushprimit_texdef.coords[0][2] +
             N.coords[1][1] * f->brushprimit_texdef.coords[1][2] +
             N.coords[1][2];
...
void Face_RotateTexture_BrushPrimit(face_t* face, float amount, idVec3 origin) {
  ...
  if (amount) {
    ...
    pBP->coords[0][0] =
        (((x - origin[0]) * c) - ((y - origin[1]) * s)) + origin[0];
    pBP->coords[0][1] =
        (((x - origin[0]) * s) + ((y - origin[1]) * c)) + origin[1];
    pBP->coords[1][0] =
        (((x1 - origin[0]) * c) - ((y1 - origin[1]) * s)) + origin[0];
    pBP->coords[1][1] =
        (((x1 - origin[0]) * s) + ((y1 - origin[1]) * c)) + origin[1];
    EmitBrushPrimitTextureCoordinates(face, face->face_winding);
  }
}
...
void TextureLockTransformation_BrushPrimit(face_t* f) {
  ...
  idVec3D lOrig, lvecS,
      lvecT;  // [2] are not used ( but usefull for debugging )
  idVec3D M[3];
  double det;
  idVec3D D[2];
...
void Face_GetScale_BrushPrimit(face_t* face, float* s, float* t, float* rot) {
  ...
  *t = idMath::Sqrt(aux[0][1] * aux[0][1] + aux[1][1] * aux[1][1]);
  ...
  if (idMath::Fabs(face->brushprimit_texdef.coords[0][0]) < ZERO_EPSILON) {
    ...
    *rot = RAD2DEG(
        atan2(face->brushprimit_texdef.coords[1][0] / (*s) ? (*s) : 1.0f,
              face->brushprimit_texdef.coords[0][0] / (*t) ? (*t) : 1.0f));
  }
}
...
void Face_SetAxialScale_BrushPrimit(face_t* face, bool y) {
  ...
  if (g_bAxialMode) {
    if (g_axialAnchor >= 0 &&
        g_axialAnchor < face->face_winding->GetNumPoints() &&
        g_axialDest >= 0 && g_axialDest < face->face_winding->GetNumPoints() &&
        g_axialAnchor != g_axialDest) {
      len = (*face->face_winding)[g_axialDest].ToVec3() -
            (*face->face_winding)[g_axialAnchor].ToVec3();
    } else {
      return;
    }
  } else {
    if (y) {
      len =
          (*face->face_winding)[2].ToVec3() - (*face->face_winding)[1].ToVec3();
    } else {
      len =
          (*face->face_winding)[1].ToVec3() - (*face->face_winding)[0].ToVec3();
    }
  }

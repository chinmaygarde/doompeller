...
const char* CDialogTextures::TypeNames[] = {
    "None",   "Textures",    "Materials", "Models",    "Scripts",
    "Sounds", "SoundParent", "Guis",      "Particles", "Fx"};
...
bool CDialogTextures::loadTree(HTREEITEM item,
  ...
  if (m_treeTextures.ItemHasChildren(item)) {
    ...
    while (childItem != NULL) {
      ...
      childName =
          name + "/" + (const char*)m_treeTextures.GetItemText(childItem);
    ...
    while (childItem != NULL && *foundItem == NULL) {
      ...
      if (m_treeTextures.ItemHasChildren(nextItem)) {
        ...
        if (dw == TEXTURES) {
          ...
          if (!idStr::Icmpn(name, "textures/", 9) &&
              stricmp(name + 9, matName) == 0) {
            *foundItem = nextItem;
            return *foundItem;
          }
...
void CDialogTextures::OnRefresh() {
  ...
  if (mode == TEXTURES) {
    ...
    for (i = 0; i < count; i++) {
      ...
      if (mat->IsValid() && mat->TestMaterialFlag(MF_EDITOR_VISIBLE) &&
          !idStr::Icmpn(mat->GetName(), "textures/", 9)) {
    ...
    SelectCurrentItem(false, g_qeglobals.d_texturewin.texdef.name,
                      CDialogTextures::TEXTURES);
...
void CDialogTextures::OnSize(UINT nType, int cx, int cy) {
  ...
  m_btnLoad.SetWindowPos(NULL, rect.left + 4, rect.top + 4, 0, 0,
                         SWP_NOSIZE | SWP_SHOWWINDOW);
  m_btnRefresh.SetWindowPos(NULL, rect.left + rect2.Width() + 4, rect.top + 4,
  ...
  m_chkHideRoot.SetWindowPos(NULL, right - rect3.Width() * 2, rect.top + 4, 0,
                             0, SWP_NOSIZE | SWP_SHOWWINDOW);
  m_chkHideRoot.ShowWindow(SW_HIDE);

  int verticalSpace = (rect.Height() - rect2.Height() - 12) / 2;

  m_treeTextures.SetWindowPos(NULL, rect.left + 4,
                              rect.top + 8 + rect2.Height(), (rect.Width() - 8),
                              verticalSpace, SWP_SHOWWINDOW);
  m_wndPreview.SetWindowPos(NULL, rect.left + 4,
                            rect.top + 12 + rect2.Height() + verticalSpace,
                            (rect.Width() - 8), verticalSpace, SWP_SHOWWINDOW);
...
void CDialogTextures::OnMaterialEdit() {
  ...
  if ((mode == TEXTURES || mode == MATERIALS) && editMaterial) {
    ...
    dlg.SetMaterialInfo(editMaterial->GetName(), editMaterial->GetFileName(),
                        editMaterial->GetLineNum());
    dlg.DoModal();

...
void showCameraInspector() {
  if (g_dlgCamera.GetSafeHwnd() == NULL) {
    ...
    if (LoadRegistryInfo("Radiant::CameraInspector", &rct, &lSize)) {
      g_dlgCamera.SetWindowPos(NULL, rct.left, rct.top, 0, 0,
...
void CDlgCamera::OnBtnAddevent() {
  ...
  if (dlg.DoModal() == IDOK) {
    ...
    g_splineList->addEvent(
        static_cast<idCameraEvent::eventType>(dlg.m_event + 1), dlg.m_strParm,
        n);
    setupFromCamera();
  }
}
...
void CDlgCamera::OnBtnAddtarget() {
  ...
  if (dlg.DoModal() == IDOK) {
    g_splineList->addTarget(
        dlg.m_strName,
        static_cast<idCameraPosition::positionType>(dlg.m_nType));
    setupFromCamera();
...
void CDlgCamera::setupFromCamera() {
  if (m_wndSplines.GetSafeHwnd()) {
    ...
    for (i = 0; i < g_splineList->numEvents(); i++) {
      str = va("%s\t%s", g_splineList->getEvent(i)->typeStr(),
               g_splineList->getEvent(i)->getParam());
...
void CDlgCamera::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) {
  ...
  if (m_trackCamera) {
    ...
    idVec3 dir;
    float fov;
    g_splineList->getCameraInfo(p, g_pParentWnd->GetCamera()->Camera().origin,
                                dir, &fov);

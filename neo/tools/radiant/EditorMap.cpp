...
void Map_BuildBrushData(void) {
  ...
  for (b = active_brushes.next; b != NULL && b != &active_brushes; b = next) {
    ...
    if (!b->brush_faces ||
        (g_PrefsDlg.m_bCleanTiny &&
         CheckForTinyBrush(b, n++, g_PrefsDlg.m_fTinySize))) {
      Brush_Free(b);
...
brush_t* BrushFromMapPatch(idMapPatch* mappatch, idVec3 origin) {
  ...
  for (int i = 0; i < mappatch->GetWidth(); i++) {
    for (int j = 0; j < mappatch->GetHeight(); j++) {
      pm->ctrl(i, j).xyz =
          (*mappatch)[j * mappatch->GetWidth() + i].xyz + origin;
...
idMapPrimitive* BrushToMapPrimitive(const brush_t* b, const idVec3& origin) {
  if (b->pPatch) {
    idMapPatch* patch =
        new idMapPatch(b->pPatch->width * 6, b->pPatch->height * 6);
    patch->SetSize(b->pPatch->width, b->pPatch->height);
    for (int i = 0; i < b->pPatch->width; i++) {
      for (int j = 0; j < b->pPatch->height; j++) {
        (*patch)[j * patch->GetWidth() + i].xyz =
            b->pPatch->ctrl(i, j).xyz - origin;
...
bool Map_SaveFile(const char* filename, bool use_region, bool autosave) {
  ...
  if (filename == NULL || len == 0 ||
      (filename && stricmp(filename, "unnamed.map") == 0)) {
    CFileDialog dlgSave(FALSE, "map", NULL,
  ...
  if (!use_region) {
    ...
    if (_unlink(backup) != 0 &&
        errno != 2) {  // errno 2 means the file doesn't exist, which we don't
                       // care about
      g_pParentWnd->MessageBox(
          va("Unable to delete %s: %s", backup.c_str(), strerror(errno)),
          "File Error");
    }

    if (rename(filename, backup) != 0) {
      g_pParentWnd->MessageBox(va("Unable to rename %s to %s: %s", filename,
  ...
  if (use_region) {
    ...
    idLexer src(LEXFL_NOSTRINGCONCAT | LEXFL_NOSTRINGESCAPECHARS |
  ...
  for (e = entities.next; e != &entities; e = next) {
    ...
    if (e->brushes.onext == &e->brushes) {
      ...
      if (!GetVectorForKey(e, "origin", origin)) {
        ...
        sprintf(text, "%i %i %i", (int)origin[0], (int)origin[1],
                (int)origin[2]);
        SetKeyValue(e, "origin", text);
      }

      if (use_region &&
          !idStr::Icmp(ValueForKey(e, "classname"), "info_player_start")) {
        continue;
      }
...
void Map_New(void) {
  ...
  world_entity->brushes.onext = world_entity->brushes.oprev =
      &world_entity->brushes;
...
/*
 =======================================================================================================================
    AddRegionBrushes a regioned map will have temp walls put up at the region
 boundary
 =======================================================================================================================
 */
void AddRegionBrushes(void) {
...
void Map_RegionXY(void) {
  ...
  region_mins[0] = g_pParentWnd->GetXYWnd()->GetOrigin()[0] -
                   0.5 * g_pParentWnd->GetXYWnd()->Width() /
  ...
  region_mins[1] = g_pParentWnd->GetXYWnd()->GetOrigin()[1] -
                   0.5 * g_pParentWnd->GetXYWnd()->Height() /
                       g_pParentWnd->GetXYWnd()->Scale();
...
void Map_ImportBuffer(char* buf, bool renameEntities) {
  ...
  if (buf) {
    ...
    for (int iKeyVal = 0; iKeyVal < iNumKeyVals; iKeyVal++) {
      ...
      entity_t* pEntOld =
          FindEntity("name", psOldName);  // original ent we cloned from
      ...
      case '\\':
        ...
        switch (*string) {
          ...
          case 'n':
            fprintf(fp, "\n");
          default:
            common->Error("WriteFileString: unknown escape character \'%c\'",
                          *string);
            break;
        }
        string++;
      ...
      case '\\':
        ...
        switch (*string) {
          ...
          case 'n':
            sprintf(buff, "\n");
          default:
            common->Error("MemFile_fprintf: unknown escape character \'%c\'",
                          *string);
            break;
        }
        string++;

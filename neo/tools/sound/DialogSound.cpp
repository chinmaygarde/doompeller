...
extern HTREEITEM FindTreeItem(CTreeCtrl* tree,
                              HTREEITEM root,
                              const char* text,
...
void CDialogSound::AddGroups() {
  ...
  list.SetNum(1024);
  int count = gameEdit->MapGetUniqueMatchingKeyVals("soundgroup", list.Ptr(),
                                                    list.Num());
  for (int i = 0; i < count; i++) {
...
void CDialogSound::AddInUseSounds() {
  ...
  idList<const char*> list;
  list.SetNum(512);
  int i, count = gameEdit->MapGetEntitiesMatchingClassWithString(
             "speaker", "", list.Ptr(), list.Num());
  idStrList list2;
  for (i = 0; i < count; i++) {
...
void CDialogSound::AddSpeakers() {
  ...
  int count = gameEdit->MapGetEntitiesMatchingClassWithString(
      "speaker", group, list.Ptr(), list.Num());
...
void CDialogSound::OnBtnEdit() {
  const idDecl* decl =
      declManager->FindDeclWithoutParsing(DECL_SOUND, strShader, false);
...
void CDialogSound::OnBtnGroup() {
  ...
  if (count) {
    for (int i = 0; i < count; i++) {
      ...
      if (dict) {
        if (MessageBox("Are you Sure?", "Delete Selected Speakers", MB_YESNO) ==
            IDYES) {
...
void CDialogSound::OnSelchangeComboSpeakers() {
  ...
  if (comboSpeakers.GetCurSel() >= 0) {
    ...
    list.SetNum(512);
    int count = gameEdit->MapGetEntitiesMatchingClassWithString(
        "speaker", speaker, list.Ptr(), list.Num());
    for (int i = 0; i < count; i++) {
      idEntity* gameEnt = gameEdit->FindEntity(list[i]);
      if (gameEnt) {
        gameEdit->AddSelectedEntity(gameEnt);

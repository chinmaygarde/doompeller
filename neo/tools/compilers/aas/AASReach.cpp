...
void idAASReach::Reachability_Fly(int areaNum) {
  ...
  for (i = 0; i < area->numFaces; i++) {
    ...
    if (faceNum < 0) {
      reach->end = reach->start + file->planeList[face->planeNum].Normal() *
                                      INSIDEUNITS_FLYEND;
    } else {
      reach->end = reach->start - file->planeList[face->planeNum].Normal() *
                                      INSIDEUNITS_FLYEND;
...
void idAASReach::Reachability_EqualFloorHeight(int areaNum) {
  int i, k, l, m, n, faceNum, face1Num, face2Num, otherAreaNum, edge1Num,
      edge2Num;
...
================
*/
bool idAASReach::Reachability_Step_Barrier_WaterJump_WalkOffLedge(
    int area1num,
    int area2num) {
  int i, j, k, l, edge1Num, edge2Num, areas[10];
  int floor_bestArea1FloorEdgeNum, floor_bestArea2FloorEdgeNum,
      floor_foundReach;
  int water_bestArea1FloorEdgeNum, water_bestArea2FloorEdgeNum,
      water_foundReach;
  int side1, faceSide1, floorFace1Num;
  float dist, dist1, dist2, diff, invGravityDot, orthogonalDot;
  float x1, x2, x3, x4, y1, y2, y3, y4, tmp, y;
  float length, floor_bestLength, water_bestLength, floor_bestDist,
      water_bestDist;
  ...
  for (i = 0; i < area1->numFaces; i++) {
    ...
    if (!(floorFace1->flags & FACE_FLOOR)) {
      ...
      if (CanSwimInArea(area1num)) {
        ...
        if (plane->Normal() * file->settings.invGravityDir <
            file->settings.minFloorCos) {
          continue;
        }
      } else {
    ...
    for (k = 0; k < floorFace1->numEdges; k++) {
      ...
      for (j = 0; j < area2->numFaces; j++) {
        ...
        for (l = 0; l < floorFace2->numEdges; l++) {
          ...
          invGravityDot =
              file->settings.invGravityDir * file->settings.invGravityDir;
  ...
  if (floor_foundReach) {
    ...
    if (floor_bestDist >= 0 && floor_bestDist < file->settings.maxStepHeight) {
      ...
      walkReach->start =
          floor_bestStart + INSIDEUNITS_WALKSTART * floor_bestNormal;
    ...
    if (area1->flags & AREA_LIQUID) {
      ...
      if (water_bestDist < file->settings.maxWaterJumpHeight + 24) {
        ...
        if (!(area1->flags & AREA_CROUCH) && !(area2->flags & AREA_CROUCH)) {
          ...
          waterJumpReach->end =
              water_bestEnd + INSIDEUNITS_WATERJUMP * water_bestNormal;
  ...
  if (floor_foundReach) {
    ...
    if (floor_bestDist > 0 &&
        floor_bestDist < file->settings.maxBarrierHeight) {
      ...
      if (!water_foundReach || (floor_bestDist - water_bestDist < 16)) {
        ...
        if (!(area1->flags & AREA_CROUCH) && !(area2->flags & AREA_CROUCH)) {
          ...
          barrierJumpReach->start =
              floor_bestStart + INSIDEUNITS_WALKSTART * floor_bestNormal;
          barrierJumpReach->end =
              floor_bestEnd + INSIDEUNITS_WALKEND * floor_bestNormal;
  ...
  if (floor_foundReach) {
    if (floor_bestDist < 0) {
      ...
      if (!file->settings.maxFallHeight ||
          idMath::Fabs(floor_bestDist) < file->settings.maxFallHeight) {
          ...
          if (trace.lastAreaNum == area2num) {
            ...
            if (i >= trace.numAreas) {
              ...
              walkOffLedgeReach->travelTime =
                  file->settings.tt_startWalkOffLedge +
                  idMath::Fabs(floor_bestDist) * 50 /
                      file->settings.gravityValue;
...
void idAASReach::Reachability_WalkOffLedge(int areaNum) {
  ...
  for (i = 0; i < area->numFaces; i++) {
    ...
    for (j = 0; j < face->numEdges; j++) {
      ...
      if (idMath::Fabs(mid[2] - trace.endpos[2]) >
      ...
      reach->travelTime = file->settings.tt_startWalkOffLedge +
                          idMath::Fabs(mid[2] - trace.endpos[2]) * 50 /
                              file->settings.gravityValue;
      AddReachabilityToArea(reach, areaNum);
    }
  }

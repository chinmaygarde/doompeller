...
int idBrushBSP::BrushSplitterStats(const idBrush* brush,
  ...
  // plane
  if (stats.numSplits > lastNumSplits &&
      (brush_front < 1.0f || brush_back > -1.0f)) {
...
int idBrushBSP::FindSplitter(idBrushBSPNode* node,
  ...
  for (brush = node->brushList.Head(); brush; brush = brush->Next()) {
    ...
    for (i = 0; i < brush->GetNumSides(); i++) {
      ...
      if (node->volume->Split(planeList[planeNum], planeNum, NULL, NULL) !=
          PLANESIDE_CROSS) {
        continue;
      }
      ...
      for (b = node->brushList.Head(); b; b = b->Next()) {
        ...
        if (b->GetFlags() & BFL_NO_VALID_SPLITTERS) {
          ...
          b->SetPlaneSide(
              BrushSplitterStats(b, planeNum, planeList, testedPlanes, stats));
        }

        numBrushSides -= b->GetNumSides();
        // best value we can get using this plane as a splitter
        value = f * (stats.numFacing + numBrushSides) - 10 * stats.numSplits -
                stats.epsilonBrushes * 1000;
      ...
      value = f * stats.numFacing - 10 * stats.numSplits -
              abs(stats.numFront - stats.numBack) - stats.epsilonBrushes * 1000;
...
idBrushBSPNode* idBrushBSP::BuildBrushBSP_r(idBrushBSPNode* node,
  ...
  node->volume->Split(node->plane, -1, &node->children[0]->volume,
                      &node->children[1]->volume);
  node->brushList.Split(node->plane, -1, node->children[0]->brushList,
                        node->children[1]->brushList, true);
  ...
  // process children
  node->children[0] =
      BuildBrushBSP_r(node->children[0], planeList, testedPlanes, skipContents);
  node->children[1] =
      BuildBrushBSP_r(node->children[1], planeList, testedPlanes, skipContents);
...
void idBrushBSP::BuildGrid_r(idList<idBrushBSPNode*>& gridCells,
  ...
  for (axis = 0; axis < 3; axis++) {
    if (halfSize[axis] > BSP_GRID_SIZE) {
      dist = BSP_GRID_SIZE *
             (floor((bounds[0][axis] + halfSize[axis]) / BSP_GRID_SIZE) + 1);
    } else {
  ...
  node->children[0]->brushList.SetFlagOnFacingBrushSides(node->plane,
                                                         SFL_USED_SPLITTER);
  node->children[1]->brushList.SetFlagOnFacingBrushSides(node->plane,
                                                         SFL_USED_SPLITTER);
  node->children[0]->parent = node->children[1]->parent = node;
...
void idBrushBSP::SplitNodePortals(idBrushBSPNode* node) {
  ...
  for (p = node->portals; p; p = nextPortal) {
    ...
    p->winding->Split(*plane, SPLIT_WINDING_EPSILON, &frontWinding,
                      &backWinding);
...
void idBrushBSP::MakeOutsidePortals(void) {
  ...
  for (i = 0; i < 6; i++) {
    for (j = 0; j < 6; j++) {
      ...
      portals[i]->winding =
          portals[i]->winding->Clip(portals[j]->plane, ON_EPSILON);
    }
  }
}
...
bool idBrushBSP::FloodFromEntities(const idMapFile* mapFile,
  ...
  if (!inside) {
    ...
    common->Warning("reached outside from entity %d (%s)", i,
                    classname.c_str());
  }
...
void idBrushBSP::SetPortalPlanes_r(idBrushBSPNode* node,
  ...
  for (p = node->portals; p; p = p->next[s]) {
    s = (p->nodes[1] == node);
    if (p->planeNum == -1) {
      p->planeNum = planeList.FindPlane(p->plane, PORTAL_PLANE_NORMAL_EPSILON,

...
bool idAASCluster::FloodClusterAreas_r(int areaNum, int clusterNum) {
  ...
  if (area->cluster > 0) {
    ...
    common->Error("cluster %d touched cluster %d at area %d\r\n", clusterNum,
                  file->areas[areaNum].cluster, areaNum);
    return false;
  }
...
void idAASCluster::NumberClusterAreas(int clusterNum) {
  ...
  for (i = 0; i < cluster->numPortals; i++) {
    ...
    if (!(file->areas[portal->areaNum].flags &
...
bool idAASCluster::TestPortals(void) {
  ...
  for (i = 1; i < file->portals.Num(); i++) {
    ...
    for (reach = area->reach; reach; reach = reach->next) {
      ...
      if ((portal2->clusters[0] != portal->clusters[0] &&
           portal2->clusters[0] != portal->clusters[1]) ||
          (portal2->clusters[1] != portal->clusters[0] &&
           portal2->clusters[1] != portal->clusters[1])) {
        area2->contents &= ~AREACONTENTS_CLUSTERPORTAL;
        ok = false;
        continue;
      }
...
void idAASCluster::RemoveInvalidPortals(void) {
  int i, j, k, face1Num, face2Num, otherAreaNum, numOpenAreas,
      numInvalidPortals;

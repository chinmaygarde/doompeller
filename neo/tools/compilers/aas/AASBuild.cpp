...
void idAASBuild::ParseProcNodes(idLexer* src) {
  ...
  idAASBuild::procNodes = (aasProcNode_t*)Mem_ClearedAlloc(
      idAASBuild::numProcNodes * sizeof(aasProcNode_t));
...
bool idAASBuild::LoadProcBSP(const char* name, ID_TIME_T minFileTime) {
  ...
  if (!src->IsLoaded()) {
    common->Warning("idAASBuild::LoadProcBSP: couldn't load %s",
                    fileName.c_str());
    delete src;
    return false;
  }
  ...
  if (!src->ReadToken(&token) || token.Icmp(PROC_FILE_ID)) {
    common->Warning("idAASBuild::LoadProcBSP: bad id '%s' instead of '%s'",
  ...
  do {
    ...
    if (res == SIDE_FRONT) {
      ...
      if (node->children[1] > 0) {
        if (!idAASBuild::ChoppedAwayByProcBSP(node->children[1], &back, normal,
                                              origin, radius)) {
          return false;
        }
      }
...
void idAASBuild::ClipBrushSidesWithProcBSP(idBrushList& brushList) {
  ...
  for (brush = brushList.Head(); brush; brush = brush->Next()) {
    for (i = 0; i < brush->GetNumSides(); i++) {
      ...
      if (ChoppedAwayByProcBSP(0, &neww, brush->GetSide(i)->GetPlane().Normal(),
                               origin, radius)) {
...
idBrushList idAASBuild::AddBrushesForMapBrush(const idMapBrush* mapBrush,
  ...
  if (!brush->FromSides(sideList)) {
    common->Warning("brush primitive %d on entity %d is degenerate",
                    primitiveNum, entityNum);
    delete brush;
    return brushList;
  }
...
idBrushList idAASBuild::AddBrushesForMapPatch(const idMapPatch* mapPatch,
  ...
  if (mapPatch->GetExplicitlySubdivided()) {
    mesh.SubdivideExplicit(mapPatch->GetHorzSubdivisions(),
                           mapPatch->GetVertSubdivisions(), false, true);
  } else {
    mesh.Subdivide(DEFAULT_CURVE_MAX_ERROR_CD, DEFAULT_CURVE_MAX_ERROR_CD,
  ...
  if (!validBrushes) {
    common->Warning("patch primitive %d on entity %d is completely degenerate",
                    primitiveNum, entityNum);
  }
...
idBrushList idAASBuild::AddBrushesForMapEntity(const idMapEntity* mapEnt,
  ...
  for (i = 0; i < mapEnt->GetNumPrimitives(); i++) {
    ...
    if (mapPrim->GetType() == idMapPrimitive::TYPE_BRUSH) {
      brushList = AddBrushesForMapBrush(static_cast<idMapBrush*>(mapPrim),
                                        origin, axis, entityNum, i, brushList);
      continue;
    }
    if (mapPrim->GetType() == idMapPrimitive::TYPE_PATCH) {
      if (aasSettings->usePatches) {
        brushList =
            AddBrushesForMapPatch(static_cast<idMapPatch*>(mapPrim), origin,
                                  axis, entityNum, i, brushList);
      }
      continue;
    }
  }
...
idBrushList idAASBuild::AddBrushesForMapFile(const idMapFile* mapFile,
  ...
  for (i = 1; i < mapFile->GetNumEntities(); i++) {
    const char* classname =
        mapFile->GetEntity(i)->epairs.GetString("classname");
...
bool idAASBuild::CheckForEntities(const idMapFile* mapFile,
                                  idStrList& entityClassNames) const {
  int i;
  idStr classname;
...
bool MergeAllowed(idBrush* b1, idBrush* b2) {
  return (b1->GetContents() == b2->GetContents() &&
          !((b1->GetFlags() | b2->GetFlags()) & BFL_PATCH));
}
...
bool idAASBuild::Build(const idStr& fileName, const idAASSettings* settings) {
  ...
  if (aasSettings->writeBrushMap) {
    bsp.WriteBrushMap(fileName, "_" + aasSettings->fileExtension,
                      AREACONTENTS_SOLID);
  }

  // build BSP tree from brushes
  bsp.Build(brushList, AREACONTENTS_SOLID, ExpandedChopAllowed,
            ExpandedMergeAllowed);

  // only solid nodes with all bits set for all bounding boxes need to stay
  // solid
...
void RunAAS_f(const idCmdArgs& args) {
  ...
  if (args.Argc() <= 1) {
    common->Printf(
        ...
        "  -playerFlood       = use player spawn points as valid AAS "
        "positions.\n");
    return;
  }
  ...
  while (kv != NULL) {
    const idDict* settingsDict =
        gameEdit->FindEntityDefDict(kv->GetValue(), false);
    if (!settingsDict) {
      common->Warning("Unable to find '%s' in def/aas.def",
                      kv->GetValue().c_str());
    } else {
  ...
  for (i = 0; i < mapFiles->GetNumFiles(); i++) {
    ...
    while (kv != NULL) {
      ...
      if (!settingsDict) {
        ...
        settings.FromDict(kv->GetValue(), settingsDict);
        aas.Build(idStr("maps/") + args.Argv(1) + "/" + mapFiles->GetFile(i),
                  &settings);
      }

...
class idAASBuild {
 ...
 private:  // map loading
  ...
  bool ChoppedAwayByProcBSP(int nodeNum,
                            idFixedWinding* w,
                            const idVec3& normal,
                            const idVec3& origin,
                            const float radius);
  ...
  idBrushList AddBrushesForMapBrush(const idMapBrush* mapBrush,
                                    const idVec3& origin,
                                    const idMat3& axis,
                                    int entityNum,
                                    int primitiveNum,
                                    idBrushList brushList);
  idBrushList AddBrushesForMapPatch(const idMapPatch* mapPatch,
                                    const idVec3& origin,
                                    const idMat3& axis,
                                    int entityNum,
                                    int primitiveNum,
                                    idBrushList brushList);
  idBrushList AddBrushesForMapEntity(const idMapEntity* mapEnt,
                                     int entityNum,
                                     idBrushList brushList);
  idBrushList AddBrushesForMapFile(const idMapFile* mapFile,
                                   idBrushList brushList);
  bool CheckForEntities(const idMapFile* mapFile,
 ...
 private:  // ledge subdivision
  ...
  bool IsLedgeSide_r(idBrushBSPNode* node,
                     idFixedWinding* w,
                     const idPlane& plane,
                     const idVec3& normal,
                     const idVec3& origin,
 ...
 private:  // merging
  bool AllGapsLeadToOtherNode(idBrushBSPNode* nodeWithGaps,
 ...
 private:  // storing file
  ...
  void GetSizeEstimate_r(idBrushBSPNode* parent,
                         idBrushBSPNode* node,

...
bool idAASBuild::GetVertex(const idVec3& v, int* vertexNum) {
  ...
  for (vn = aas_vertexHash->First(hashKey); vn >= 0;
...
bool idAASBuild::GetFaceForPortal(idBrushBSPPortal* portal,
  ...
  for (i = 0; i < w->GetNumPoints(); i++) {
    GetEdge((*w)[i].ToVec3(), (*w)[(i + 1) % w->GetNumPoints()].ToVec3(),
            &faceEdges[numFaceEdges], v1num);

    if (faceEdges[numFaceEdges]) {
      // last vertex of this edge is the first vertex of the next edge
      v1num = file->edges[abs(faceEdges[numFaceEdges])]
                  .vertexNum[INTSIGNBITNOTSET(faceEdges[numFaceEdges])];
  ...
  face.planeNum = file->planeList.FindPlane(
      portal->GetPlane(), AAS_PLANE_NORMAL_EPSILON, AAS_PLANE_DIST_EPSILON);
...
int idAASBuild::StoreTree_r(idBrushBSPNode* node) {
  ...
  aasNode.planeNum = file->planeList.FindPlane(
      node->GetPlane(), AAS_PLANE_NORMAL_EPSILON, AAS_PLANE_DIST_EPSILON);
  aasNode.children[0] = aasNode.children[1] = 0;
  nodeNum = file->nodes.Num();
  file->nodes.Append(aasNode);

  // !@#$%^ cause of some bug we cannot set the children directly with the
  // StoreTree_r return value

...
idVec3 idAASFileLocal::AreaReachableGoal(int areaNum) const {
  ...
  if (!(area->flags & (AREA_REACHABLE_WALK | AREA_REACHABLE_FLY)) ||
      (area->flags & AREA_LIQUID)) {
...
int idAASFileLocal::PointReachableAreaNum(const idVec3& origin,
  ...
  if (areaNum) {
    if ((areas[areaNum].flags & areaFlags) &&
  ...
  if (trace.lastAreaNum) {
    if ((areas[trace.lastAreaNum].flags & areaFlags) &&
        ((areas[trace.lastAreaNum].travelFlags & excludeTravelFlags) == 0)) {
  ...
  for (i = 1; i <= 12; i++) {
    ...
    if (areaNum && (areas[areaNum].flags & areaFlags) &&
        ((areas[areaNum].travelFlags & excludeTravelFlags) == 0)) {
      return areaNum;
    }
  }
...
int idAASFileLocal::BoundsReachableAreaNum_r(
  ...
  while (nodeNum) {
    if (nodeNum < 0) {
      if ((areas[-nodeNum].flags & areaFlags) &&
    ...
    if (res == PLANESIDE_BACK) {
      ...
      nodeNum = BoundsReachableAreaNum_r(node->children[1], bounds, areaFlags,
                                         excludeTravelFlags);
      if (nodeNum) {
        return nodeNum;
      }
...
bool idAASFileLocal::Trace(aasTrace_t& trace,
  ...
  while (1) {
    ...
    if (nodeNum < 0) {
      ...
      if ((areas[-nodeNum].flags & trace.flags) ||

...
bool idBrush::CreateWindings(void) {
  ...
  for (i = 0; i < sides.Num(); i++) {
    ...
    for (j = 0; j < sides.Num() && side->winding; j++) {
      ...
      side->winding =
          side->winding->Clip(-sides[j]->plane, BRUSH_EPSILON, true);
    }
...
void idBrush::BoundBrush(const idBrush* original) {
  ...
  if (bounds[0][0] > bounds[1][0]) {
    ...
    common->Error("idBrush::BoundBrush: brush %d on entity %d without windings",
  ...
  for (i = 0; i < 3; i++) {
    if (bounds[0][i] < MIN_WORLD_COORD || bounds[1][i] > MAX_WORLD_COORD) {
      ...
      common->Error("idBrush::BoundBrush: brush %d on entity %d is unbounded",
                    primitiveNum, entityNum);
    }
  }
}
...
bool idBrush::FromWinding(const idWinding& w, const idPlane& windingPlane) {
  ...
  bestAxis = 0;
  for (i = 1; i < 3; i++) {
    if (idMath::Fabs(windingPlane.Normal()[i]) >
        idMath::Fabs(windingPlane.Normal()[bestAxis])) {
      bestAxis = i;
    }
  }
...
bool idBrush::TryMerge(const idBrush* brush, const idPlaneSet& planeList) {
  ...
  for (i = 0; i < brush->GetNumSides(); i++) {
    for (j = 0; j < GetNumSides(); j++) {
      if (!((brush->GetSide(i)->GetPlaneNum() ^ GetSide(j)->GetPlaneNum()) >>
            1)) {
        break;
      }
    }
  ...
  if (mid) {
    if (mid->IsTiny()) {
      ...
      common->Warning(
          ...
          primitiveNum, entityNum, bounds[0][0], bounds[0][1], bounds[0][2],
          bounds[1][0], bounds[1][1], bounds[1][2], bounds[1][0] - bounds[0][0],
          bounds[1][1] - bounds[0][1], bounds[1][2] - bounds[0][2]);
      delete mid;
      mid = NULL;
    }
  }
...
void idBrush::ExpandForAxialBox(const idBounds& bounds) {
  ...
  if (!CreateWindings()) {
    common->Error("idBrush::ExpandForAxialBox: brush %d on entity %d imploded",
                  primitiveNum, entityNum);
  }
    ...
    for (b2 = b1->next; b2; b2 = next) {
      ...
      for (i = 0; i < b1->GetNumSides(); i++) {
        for (j = 0; j < b2->GetNumSides(); j++) {
          if (b1->GetSide(i)->GetPlaneNum() ==
              (b2->GetSide(j)->GetPlaneNum() ^ 1)) {
...
void idBrushList::SetFlagOnFacingBrushSides(const idPlane& plane, int flag) {
  ...
  for (b = head; b; b = b->next) {
    ...
    for (i = 0; i < b->GetNumSides(); i++) {
      ...
      if (!w) {
        if (b->GetSide(i)->GetPlane().Compare(plane, BRUSH_PLANE_NORMAL_EPSILON,
...
void idBrushList::CreatePlaneList(idPlaneSet& planeList) const {
  ...
  for (b = Head(); b; b = b->Next()) {
    for (i = 0; i < b->GetNumSides(); i++) {
      side = b->GetSide(i);
      side->SetPlaneNum(planeList.FindPlane(side->GetPlane(),
...
void idBrushMap::WriteBrush(const idBrush* brush) {
  ...
  for (i = 0; i < brush->GetNumSides(); i++) {
    ...
    fp->WriteFloatString(" ( %f %f %f %f ) ", side->GetPlane()[0],
                         side->GetPlane()[1], side->GetPlane()[2],
                         -side->GetPlane().Dist());

...
void idLedge::Expand(const idBounds& bounds, float maxStepHeight) {
  ...
  planes[numSplitPlanes + 0].SetDist(planes[numSplitPlanes + 0].Dist() +
                                     maxStepHeight);
...
bool idLedge::PointBetweenBounds(const idVec3& v) const {
  return (planes[2].Distance(v) < LEDGE_EPSILON) &&
         (planes[3].Distance(v) < LEDGE_EPSILON);
}
...
void idAASBuild::LedgeSubdiv(idBrushBSPNode* root) {
  ...
  for (i = 0; i < ledgeList.Num(); i++) {
    ...
    ledgeList[i].Expand(aasSettings->boundingBoxes[0],
...
bool idAASBuild::IsLedgeSide_r(idBrushBSPNode* node,
  ...
  while (node->GetChild(0) && node->GetChild(1)) {
    ...
    if (res == SIDE_FRONT) {
      ...
      if (IsLedgeSide_r(node->GetChild(1), &back, plane, normal, origin,
                        radius)) {
        return true;
      }
      node = node->GetChild(0);
...
void idAASBuild::FindLeafNodeLedges(idBrushBSPNode* root,
  ...
  for (p1 = node->GetPortals(); p1; p1 = p1->Next(s1)) {
    ...
    for (i = 0; i < w->GetNumPoints(); i++) {
      ...
      winding += winding[1].ToVec3() +
                 (aasSettings->maxStepHeight + 1.0f) * aasSettings->gravityDir;
      winding += winding[0].ToVec3() +
                 (aasSettings->maxStepHeight + 1.0f) * aasSettings->gravityDir;
      ...
      plane.FitThroughPoint(v1 + aasSettings->maxStepHeight *
                                     aasSettings->gravityDir);

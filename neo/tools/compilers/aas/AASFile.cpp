...
bool idAASSettings::WriteToFile(idFile* fp) const {
  ...
  for (i = 0; i < numBoundingBoxes; i++) {
    fp->WriteFloatString("\t\t(%f %f %f)-(%f %f %f)\n", boundingBoxes[i][0].x,
                         boundingBoxes[i][0].y, boundingBoxes[i][0].z,
                         boundingBoxes[i][1].x, boundingBoxes[i][1].y,
                         boundingBoxes[i][1].z);
  }
  fp->WriteFloatString("\t}\n");
...
bool idAASSettings::ValidEntity(const char* classname) const {
  ...
  const idDeclEntityDef* decl = static_cast<const idDeclEntityDef*>(
      declManager->FindType(DECL_ENTITYDEF, classname, false));
  if (decl && decl->dict.GetString("use_aas", NULL, use_aas) &&
      !fileExtension.Icmp(use_aas)) {
  ...
  for (i = 0; i < areas.Num(); i++) {
    ...
    for (reach = areas[i].reach; reach; reach = reach->next) {
      ...
      switch (reach->travelType) {
        case TFL_SPECIAL:
          Reachability_Special_Write(
              aasFile, static_cast<idReachability_Special*>(reach));
          break;
      }
      aasFile->WriteFloatString("\n");
    }

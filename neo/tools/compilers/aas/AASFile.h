...
#define AREACONTENTS_OBSTACLE \
  BIT(3)  // area contains (part of) a dynamic obstacle
#define AREACONTENTS_TELEPORTER \
  BIT(4)  // area contains (part of) a teleporter trigger
...
class idReachability {
 public:
  ...
  unsigned short*
      areaTravelTimes;  // travel times within the fromAreaNum from
...
typedef struct aasNode_s {
  unsigned short
      planeNum;     // number of the plane that splits the subspace at this node
...
typedef struct aasPortal_s {
  ...
  short clusterAreaNum[2];  // number of this portal area in the front and back
                            // cluster
  unsigned short
      maxAreaTravelTime;  // maximum travel time through the portal area
} aasPortal_t;
...
class idAASFile {
 public:
  ...
  virtual int PointReachableAreaNum(const idVec3& origin,
                                    ...
                                    const int areaFlags,
                                    const int excludeTravelFlags) const = 0;

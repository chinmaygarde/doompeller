...
#define STRIDE_FACTOR \
  ...
  while (stride >
         (STRIDE_FACTOR - 1)) {  // loop to sort for each value of stride
...
void codec::Segment(int* alist, float* flist, int numElements, float rmse) {
  ...
  if (doopen) {
    ...
    cbook =
        (byte*)Mem_ClearedAlloc(3 * image->pixelsWide() * image->pixelsHigh());
    float* snrBook =
        (float*)Mem_ClearedAlloc(image->pixelsWide() * image->pixelsHigh());
    dst = cbook;
    int numEntries = 0;
      ...
      if (qStatus[i].size == 8 && qStatus[i].rsnr >= MIN_SNR * 4) {
        for (y = qStatus[i].yat; y < qStatus[i].yat + 8; y += 4) {
          for (x = qStatus[i].xat; x < qStatus[i].xat + 8; x += 4) {
            ...
            for (yy = y; yy < (y + 4); yy++) {
              for (xx = x; xx < (x + 4); xx++) {
                src = image->bitmapData() + (yy * (bpp * image->pixelsWide())) +
                      (xx * bpp);
                memcpy(dst, src, 3);
                dst += 3;
              }
...
void codec::MakePreviousImage(quadcel* pquad) {
          ...
          for (yy = 0; yy < 4; yy += 2) {
            for (xx = 0; xx < 4; xx += 2) {
              ...
              for (y = yy; y < (yy + 2); y++) {
                for (x = xx; x < (xx + 2); x++) {
                  pluck =
                      (((y + pquad[i].yat) * pWide) + (pquad[i].xat + x)) * 4;
  ...
  if (whichFrame == 0) {
    memcpy(previousImage[1]->bitmapData(), previousImage[0]->bitmapData(),
           pWide * (pixelsHigh & 0xfff0) * 4);
  }
...
void codec::InitImages(void) {
  ...
  numQuadCels =
      ((pixelsWide & 0xfff0) * (pixelsHigh & 0xfff0)) / (MINSIZE * MINSIZE);
  ...
  if (previousImage[0]) {
    ...
    for (y = 0; y < pixelsHigh; y++) {
      for (x = 0; x < pixelsWide; x++) {
        ftemp = RMULT * lutimage[index0 + 0] + GMULT * lutimage[index0 + 1] +
...
void codec::VqData8(byte* cel, quadcel* pquad) {
  ...
  for (y = 0; y < 4; y++) {
    for (x = 0; x < 4; x++) {
      temp = y * 64 + x * 8;
      tempImage[i++] =
          (cel[temp + 0] + cel[temp + 4] + cel[temp + 32] + cel[temp + 36]) / 4;
      tempImage[i++] =
          (cel[temp + 1] + cel[temp + 5] + cel[temp + 33] + cel[temp + 37]) / 4;
      tempImage[i++] =
          (cel[temp + 2] + cel[temp + 6] + cel[temp + 34] + cel[temp + 38]) / 4;
      if (dimension4 == 64)
        tempImage[i++] =
            (cel[temp + 3] + cel[temp + 7] + cel[temp + 35] + cel[temp + 39]) /
            4;
    }
  }
...
void codec::VqData2(byte* cel, quadcel* pquad) {
  ...
  for (yy = 0; yy < 4; yy += 2) {
    for (xx = 0; xx < 4; xx += 2) {
      ...
      pquad->patten[j++] = best =
          BestCodeword(tempImage, dimension2, codebook2);
      i = 0;
      for (y = yy; y < (yy + 2); y++) {
        for (x = xx; x < (xx + 2); x++) {
...
void codec::FvqData(byte* bitmap,
  ...
  for (x = 0; x < (size * size); x++) {
    fmblur0 = RMULT * bitmap[x * 4 + 0] + GMULT * bitmap[x * 4 + 1] +
              BMULT * bitmap[x * 4 + 2];
    luty[x] = (byte)fmblur0;
  }
  if (!luti) {
  ...
  if (((realx - xxMean) + searchX) < 0 ||
      (((realx - xxMean) - searchX) + depthx + size) > xLen ||
      ((realy - yyMean) + searchY) < 0 ||
  ...
  int sPsQ = -1;
  for (sX = (((realx - xxMean) - searchX) + depthx);
       sX <= ((realx - xxMean) + searchX) && !fabort; sX += depthx) {
    for (sY = (((realy - yyMean) - searchY) + depthy);
         sY <= ((realy - yyMean) + searchY) && breakHigh; sY += depthy) {
      temp1 = xLen * sY + sX;
      if (sX >= 0 && (sX + size) <= xLen && sY >= 0 && (sY + size) <= yLen) {
        ...
        bitma2 = bitmap;
        scale1 =
            previousImage[((whichFrame + 1) & 1)]->bitmapData() + temp1 * bpp;
        //		mblur0 = 0;
        //		bitma2 = luty;
        //		scale1 = luti + temp1;
        for (y = 0; y < size; y++) {
...
int codec::AddQuad(quadcel* pquad, int lownum) {
  ...
  if (lownum != -1) {
    ...
    for (i = lownum + 1; i < lownum + (nx * 4) + 1; i += nx) {
      ...
      if (previousImage[0]) {
        FvqData(idataA, pquad[i].size, pquad[i].xat, pquad[i].yat, &pquad[i],
                true);
        GetData(idataB, pquad[i].size, pquad[i].xat, pquad[i].yat,
                previousImage[whichFrame & 1]);
        pquad[i].snr[MOT] = Snr(idataA, idataB, pquad[i].size);
        if (ComputeMotionBlock(idataA, idataB, pquad[i].size) &&
            !theRoQ->IsLastFrame() && !detail) {
          pquad[i].mark = true;
        }
      }
    ...
    Mem_Free(idataB);
    newsnr /= 4;
    LowestQuad(&pquad[lownum], &pquad[lownum].status, &pquad[lownum].rsnr,
               false);

    if (pquad[lownum + nx * 0 + 1].status == MOT &&
        pquad[lownum + nx * 1 + 1].status == MOT &&
        pquad[lownum + nx * 2 + 1].status == MOT &&
        pquad[lownum + nx * 3 + 1].status == MOT && nsize == 4) {
      newsnr = 9999;
    ...
    if (pquad[lownum].rsnr > newsnr) {
      ...
      pquad[lownum].rsnr = 0;
      for (i = lownum + 1; i < lownum + (nx * 4) + 1; i += nx) {
        theRoQ->MarkQuadx(pquad[i].xat, pquad[i].yat, nsize, pquad[i].rsnr,
                          qStatus[i].status);
      }
    } else {
      theRoQ->MarkQuadx(pquad[lownum].xat, pquad[lownum].yat, nsize * 2,
                        pquad[lownum].rsnr, qStatus[lownum].status);
...
void codec::SparseEncode(void) {
  ...
  for (i = 0; i < numQuadCels; i++) {
    ...
    if (qStatus[i].size == osize) {
      if (previousImage[0]) {
        ...
        GetData(idataB, qStatus[i].size, qStatus[i].xat, qStatus[i].yat,
                previousImage[whichFrame & 1]);
        qStatus[i].snr[MOT] = Snr(idataA, idataB, qStatus[i].size);
        if (ComputeMotionBlock(idataA, idataB, qStatus[i].size) &&
            !theRoQ->IsLastFrame()) {
          qStatus[i].mark = true;
        }
        if (!qStatus[i].mark) {
          FvqData(idataA, qStatus[i].size, qStatus[i].xat, qStatus[i].yat,
                  &qStatus[i], false);
        }
      }
      LowestQuad(&qStatus[i], &qStatus[i].status, &qStatus[i].rsnr, wtype);
      if (qStatus[i].rsnr < 9999)
        theRoQ->MarkQuadx(qStatus[i].xat, qStatus[i].yat, qStatus[i].size,
                          qStatus[i].rsnr, qStatus[i].status);
    } else {
      if (qStatus[i].size < osize) {
        qStatus[i].status = 0;
        qStatus[i].size = 0;
  ...
  */
  ...
  temp = 0;
  for (i = 0; i < numQuadCels; i++) {
    if (qStatus[i].size == osize && qStatus[i].mark == false &&
        qStatus[i].snr[MOT] > 0) {
      GetData(idataA, qStatus[i].size, qStatus[i].xat, qStatus[i].yat, image);
      if (osize == 8)
      ...
      if (previousImage[0]) {
        ...
        if (dx < 0 || dx > 15 || dy < 0 || dy > 15) {
          ...
          dy = (qStatus[i].domain & 0xff) - 128 - dyMean + 8;
          if ((dx < 0 || dx > 15 || dy < 0 || dy > 15) &&
              qStatus[i].snr[FCC] != 9999 && qStatus[i].status == FCC) {
            common->Printf(
                "sparseEncode: something is wrong here, dx/dy is %d,%d after "
                "being clamped\n",
                dx, dy);
            common->Printf("xat:    %d\n", qStatus[i].xat);
            common->Printf("yat:    %d\n", qStatus[i].yat);
  ...
  while (GetCurrentQuadOutputSize(qStatus) < fsize && ong < onf &&
         flist[ong] > 0 && qStatus[ilist[ong]].mark == false) {
  ...
  common->Printf("sparseEncode: rmse of frame %d is %f, size is %d\n",
                 whichFrame, GetCurrentRMSE(qStatus),
  ...
  if (theRoQ->IsQuiet() == false) {
    ...
    common->Printf(
        "sparseEncode: for 08x08 CCC = %d, FCC = %d, MOT = %d, SLD = %d, PAT = "
        "%d\n",
    ...
    common->Printf(
        "sparseEncode: for 04x04 CCC = %d, FCC = %d, MOT = %d, SLD = %d, PAT = "
        "%d\n",
        num[CCC], num[FCC], num[MOT], num[SLD], num[PAT]);

    common->Printf(
        "sparseEncode: average RMSE = %f, numActiveQuadCels = %d, estSize = "
        "%d, slop = %d \n",
...
void codec::VQ(const int numEntries,
  ...
  for (i = 0; i < numEntries - 1; i++) {
    for (j = i + 1; j < numEntries; j++) {
      if (inuse[i] && inuse[j]) {
        if (!memcmp(&vectors[i * dimension], &vectors[j * dimension],
                    dimension)) {
          inuse[j] = false;
          numFinalEntries--;
          import[i] += import[j];

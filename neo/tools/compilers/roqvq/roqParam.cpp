...
int parseRange(const char* rangeStr,
               int field,
               int skipnum[],
               int startnum[],
               int endnum[],
               ...
               bool padding[],
               int numpadding[]);
int parseTimecodeRange(const char* rangeStr,
                       int field,
                       int skipnum[],
                       int startnum[],
                       int endnum[],
          ...
          if (arg1[0] != '[') {
            ...
            if (arg1[0] == '[') {
              ...
              if (useTimecodeForRange) {
                realnum +=
                    parseTimecodeRange(arg1, field, skipnum, startnum, endnum,
                                       numfiles, padding, numpadding);
                //								common->Printf("
                //+ reading %s from %d to %d\n", file[field], startnum[field],
                //endnum[field]);
              } else {
                realnum += parseRange(arg1, field, skipnum, startnum, endnum,
                                      numfiles, padding, numpadding);
                //								common->Printf("
                //+ reading %s from %d to %d\n", file[field], startnum[field],
                //endnum[field]);
                       ...
                       (arg3[0] == '[')) {  // a double ranger...
              int files1, files2;

              file2[field] = arg1;
              range[field] = 2;
              files1 = parseRange(arg2, field, skipnum, startnum, endnum,
                                  numfiles, padding, numpadding);
              //							common->Printf("
              //+ reading %s from %d to %d\n", file[field], startnum[field],
              //endnum[field]);
              files2 = parseRange(arg3, field, skipnum2, startnum2, endnum2,
                                  numfiles, padding2, numpadding2);
              //							common->Printf("
              //+ reading %s from %d to %d\n", file2[field], startnum2[field],
              //endnum2[field]);
              if (files1 != files2) {
                common->Error("You had %d files for %s and %d for %s!", files1,
                              arg1.c_str(), files2, arg2.c_str());
              } else {
    ...
    *strp++ = 0;
    ...
    if (padding[myfield] == true) {
      if (useTimecodeForRange) {
        ...
        secs = (index / (30)) % 60;
        frs = index % 30;
        sprintf(fileName, "%s%.02d%.02d/%.02d%.02d%.02d%.02d%s", left, hrs,
                mins, hrs, mins, secs, frs, right);
      } else {
        sprintf(tempfile, "%032d", index);
        sprintf(fileName, "%s%s%s", left, &tempfile[32 - numpadding[myfield]],
                right);
      }
    } else {
      if (useTimecodeForRange) {

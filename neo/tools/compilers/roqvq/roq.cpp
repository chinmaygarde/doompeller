...
void roq::WriteLossless(void) {
   ...
   */
  cinfo.image_width =
      image->pixelsWide(); /* image width and height, in pixels */
   ...
   */
  jpeg_set_quality(&cinfo, paramFile->JpegQuality(),
  ...
  while (cinfo.next_scanline < cinfo.image_height) {
     ...
     */
    row_pointer[0] = &pixbuf[((cinfo.image_height - 1) * row_stride) -
  ...
  if (!dataStuff) {
    ...
    for (i = 0; i < 256; i++) {
      if (use4[i]) {
        ...
        for (dx = 0; dx < 4; dx++)
          cccList[j * 4 + code + dx] =
              index2[codes[i * 4 + (dimension * 256) + dx]];
        j++;
      }
    }
    code += j * 4;
        ...
        case FCC:
          ...
          if (dx > 15 || dx < 0 || dy > 15 || dy < 0) {
            common->Error(
                ...
                dx, dy, dxMean, dyMean, pquad[i].xat, pquad[i].yat,
                pquad[i].size, pquad[i].snr[FCC]);
          }
...
void roq::LoadAndDisplayImage(const char* filename) {
  ...
  numQuadCels =
      ((image->pixelsWide() & 0xfff0) * (image->pixelsHigh() & 0xfff0)) /
      (MINSIZE * MINSIZE);

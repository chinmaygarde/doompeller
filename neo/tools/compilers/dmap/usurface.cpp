...
===============
*/
static void AddTriListToArea(uEntity_t* e,
                             mapTri_t* triList,
                             int planeNum,
                             int areaNum,
  ...
  for (group = area->groups; group; group = group->nextGroup) {
    if (group->material == triList->material && group->planeNum == planeNum &&
        group->mergeGroup == triList->mergeGroup) {
      // check the texture vectors
      for (i = 0; i < 2; i++) {
        for (j = 0; j < 3; j++) {
          if (idMath::Fabs(texVec->v[i][j] - group->texVec.v[i][j]) >
              TEXTURE_VECTOR_EQUAL_EPSILON) {
            break;
          }
        }
        if (j != 3) {
        ...
        if (idMath::Fabs(texVec->v[i][3] - group->texVec.v[i][3]) >
            TEXTURE_OFFSET_EQUAL_EPSILON) {
          break;
        }
      }
      if (i == 2) {
...
static void ClipSideByTree_r(idWinding* w, side_t* side, node_t* node) {
  ...
  if (!node->opaque) {
    if (!side->visibleHull) {
      ...
      side->visibleHull->AddToConvexHull(
          w, dmapGlobals.mapPlanes[side->planenum].Normal());
    }
  }
...
====================
*/
void ClipTriIntoTree_r(idWinding* w,
                       mapTri_t* originalTri,
                       uEntity_t* e,
                       node_t* node) {
  idWinding *front, *back;
...
====================
*/
static void PutWindingIntoAreas_r(uEntity_t* e,
                                  const idWinding* w,
                                  side_t* side,
                                  node_t* node) {
  idWinding *front, *back;
...
void PutPrimitivesInAreas(uEntity_t* e) {
  ...
  if (dmapGlobals.entityNum == 0) {
    bool inlineAll =
        dmapGlobals.uEntities[0].mapEntity->epairs.GetBool("inlineAllStatics");
    ...
    for (int eNum = 1; eNum < dmapGlobals.num_entities; eNum++) {
      ...
      if (!entity->mapEntity->epairs.GetMatrix("rotation", "1 0 0 0 1 0 0 0 1",
                                               axis)) {
...
=================
*/
static void ClipTriByLight(const mapLight_t* light,
                           const mapTri_t* tri,
                           mapTri_t** in,
                           mapTri_t** out) {
...
static void BuildLightShadows(uEntity_t* e, mapLight_t* light) {
  ...
  if (light->shadowTris && hasPerforatedSurface) {
    ...
    light->shadowTris->numShadowIndexesNoCaps =
        light->shadowTris->numShadowIndexesNoFrontCaps =
...
static void CarveGroupsByLight(uEntity_t* e, mapLight_t* light) {
  ...
  for (i = 0; i < e->numAreas; i++) {
    ...
    for (group = area->groups; group; group = nextGroup) {
      ...
      if ((light->def.lightShader->IsFogLight() &&
           !group->material->ReceivesFog()) ||
          (!light->def.lightShader->IsFogLight() &&
           !group->material->ReceivesLighting()) ||
      ...
      if (group->numGroupLights == MAX_GROUP_LIGHTS) {
        common->Error("MAX_GROUP_LIGHTS around %f %f %f",
                      group->triList->v[0].xyz[0], group->triList->v[0].xyz[1],
                      group->triList->v[0].xyz[2]);
      }
      ...
      if (!light->def.lightShader->LightEffectsBackSides() &&
          ...
          dmapGlobals.mapPlanes[group->planeNum].Distance(
              light->def.parms.origin) <= 0) {
        group->nextGroup = carvedGroups;
        carvedGroups = group;
        continue;
      }
...
void Prelight(uEntity_t* e) {
  ...
  if (dmapGlobals.shadowOptLevel > 0) {
    ...
    for (i = 0; i < e->numAreas; i++) {
      ...
      for (optimizeGroup_t* group = area->groups; group;

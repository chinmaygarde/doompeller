...
================
*/
static void ParseSurface(const idMapPatch* patch,
                         const idSurface* surface,
...
static void ParsePatch(const idMapPatch* patch, int primitiveNum) {
  ...
  if (patch->GetExplicitlySubdivided()) {
    cp->SubdivideExplicit(patch->GetHorzSubdivisions(),
                          patch->GetVertSubdivisions(), true);
  } else {
...
static bool ProcessMapEntity(idMapEntity* mapEnt) {
  ...
  for (entityPrimitive = 0; entityPrimitive < mapEnt->GetNumPrimitives();
       entityPrimitive++) {
...
static void CreateMapLight(const idMapEntity* mapEnt) {
  ...
  if (!light->name[0]) {
    common->Error("Light at (%f,%f,%f) didn't have a name",
                  light->def.parms.origin[0], light->def.parms.origin[1],
                  light->def.parms.origin[2]);
  }
#if 0
...
bool LoadDMapFile(const char* filename) {
  ...
  size =
      dmapGlobals.dmapFile->GetNumEntities() * sizeof(dmapGlobals.uEntities[0]);

...
void MakeTreePortals_r(node_t* node) {
  ...
  for (i = 0; i < 3; i++) {
    if (node->bounds[0][i] < MIN_WORLD_COORD ||
        node->bounds[1][i] > MAX_WORLD_COORD) {
    ...
    for (b = node->brushlist; b; b = b->next) {
      ...
      for (j = 0; j < orig->numsides; j++) {
        ...
        for (k = 0; k < orig->numsides; k++) {
          ...
          common->Warning("brush has multiple area portal sides at %s",
...
static void FindInterAreaPortals_r(node_t* node) {
  ...
  for (p = node->portals; p; p = p->next[s]) {
    ...
    //		w = p->winding;
    if (!side) {
      common->Warning("FindSideForPortal failed at %s",
                      p->winding->GetCenter().ToString());
      continue;
    }
    w = side->visibleHull;
    if (!w) {
    ...
    for (i = 0; i < numInterAreaPortals; i++) {
      ...
      if (side == iap->side && ((p->nodes[0]->area == iap->area0 &&
                                 p->nodes[1]->area == iap->area1) ||
                                (p->nodes[1]->area == iap->area0 &&
                                 p->nodes[0]->area == iap->area1))) {
        break;
      }
    }

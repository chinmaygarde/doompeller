...
==================
*/
static mapTri_t* FixTriangleAgainstHashVert(const mapTri_t* a,
                                            const hashVert_t* hv) {
  int i;
  ...
  for (i = 0; i < 3; i++) {
    ...
    plane1.FromPoints(new1->hashVert[0]->v, new1->hashVert[1]->v,
                      new1->hashVert[2]->v);
    plane2.FromPoints(new2->hashVert[0]->v, new2->hashVert[1]->v,
                      new2->hashVert[2]->v);
...
void FixGlobalTjunctions(uEntity_t* e) {
  ...
  if (dmapGlobals.entityNum == 0) {
    for (int eNum = 1; eNum < dmapGlobals.num_entities; eNum++) {
      ...
      if (!strstr(modelName, ".lwo") && !strstr(modelName, ".ase") &&
          !strstr(modelName, ".ma")) {
        continue;
      }

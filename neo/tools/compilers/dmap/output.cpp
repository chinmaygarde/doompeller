...
static void WriteShadowTriangles(const srfTriangles_t* tri) {
  ...
  // emit this chain
  procFile->WriteFloatString(
      "/* numVerts = */ %i /* noCaps = */ %i /* noFrontCaps = */ %i /* "
      "numIndexes = */ %i /* planeBits = */ %i\n",
      tri->numVerts, tri->numShadowIndexesNoCaps,
...
=======================
*/
static bool GroupsAreSurfaceCompatible(const optimizeGroup_t* a,
                                       const optimizeGroup_t* b) {
  if (a->material != b->material) {
    return false;
  }
  if (a->mergeGroup != b->mergeGroup) {
  ...
  for (group = area->groups; group; group = group->nextGroup) {
    ...
    for (groupStep = group; groupStep; groupStep = groupStep->nextGroup) {
      ...
      for (i = 0; i < groupStep->numGroupLights; i++) {
        for (checkInter = interactions; checkInter;
        ...
        if (!checkInter) {
          ...
          checkInter =
              (interactionTris_t*)Mem_ClearedAlloc(sizeof(*checkInter));
...
static void WriteOutputPortals(uEntity_t* e) {
  ...
  procFile->WriteFloatString(
      "/* interAreaPortal format is: numPoints positiveSideArea "
      "negativeSideArea ( point) ... */\n");
  for (i = 0; i < numInterAreaPortals; i++) {
    iap = &interAreaPortals[i];
    w = iap->side->winding;
    procFile->WriteFloatString("/* iap %i */ %i %i %i ", i, w->GetNumPoints(),
                               iap->area0, iap->area1);

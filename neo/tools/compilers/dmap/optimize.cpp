...
static bool IsTriangleValid(const optVertex_t* v1,
                            const optVertex_t* v2,
                            const optVertex_t* v3);
static bool IsTriangleDegenerate(const optVertex_t* v1,
                                 const optVertex_t* v2,
                                 const optVertex_t* v3);
...
=================
*/
static bool VertexBetween(const optVertex_t* p1,
                          const optVertex_t* v1,
                          const optVertex_t* v2) {
  idVec3 d1, d2;
  float d;
...
====================
*/
static optVertex_t* EdgeIntersection(const optVertex_t* p1,
                                     const optVertex_t* p2,
                                     const optVertex_t* l1,
                                     const optVertex_t* l2,
                                     optimizeGroup_t* opt) {
  float f;
  idDrawVert* v;
...
====================
*/
static bool PointsStraddleLine(optVertex_t* p1,
                               optVertex_t* p2,
                               optVertex_t* l1,
                               optVertex_t* l2) {
  bool t1, t2;
...
====================
*/
static bool EdgesCross(optVertex_t* a1,
                       optVertex_t* a2,
                       optVertex_t* b1,
                       optVertex_t* b2) {
...
====================
*/
static bool TryAddNewEdge(optVertex_t* v1,
                          optVertex_t* v2,
                          optIsland_t* island) {
  optEdge_t* e;
...
==================
*/
static bool PointInTri(const idVec3& p,
                       const mapTri_t* tri,
                       optIsland_t* island) {
  idVec3 d1, d2, normal;
...
===============
*/
static void CreateOptTri(optVertex_t* first,
                         optEdge_t* e1,
                         optEdge_t* e2,
                         optIsland_t* island) {
  optEdge_t* opposite;
  ...
  optTri->v[2] = third;
  optTri->midpoint =
      (optTri->v[0]->pv + optTri->v[1]->pv + optTri->v[2]->pv) * (1.0f / 3.0f);
...
// debugging tool
static void ReportNearbyVertexes(const optVertex_t* v,
                                 const optIsland_t* island) {
...
static void RegenerateTriangles(optIsland_t* island) {
  ...
  for (optTri = island->tris; optTri; optTri = optTri->next) {
    ...
    if (plane.Normal() *
            dmapGlobals.mapPlanes[island->group->planeNum].Normal() <=
        0) {
...
=================
*/
static void DrawOriginalEdges(int numOriginalEdges,
...
static void AddOriginalEdges(optimizeGroup_t* opt) {
  ...
  originalEdges =
      (originalEdges_t*)Mem_Alloc(numTris * 3 * sizeof(*originalEdges));
  numOriginalEdges = 0;
...
void SplitOriginalEdgesAtCrossings(optimizeGroup_t* opt) {
  ...
  crossings =
      (edgeCrossing_t**)Mem_ClearedAlloc(numOriginalEdges * sizeof(*crossings));
...
common->Printf( "lines %i (%i to %i) and %i (%i to %i) intersect at old point %i\n", i, v1 - optVerts, v2 - optVerts, 
  ...
  for (i = 0; i < numOptEdges; i++) {
    for (j = i + 1; j < numOptEdges; j++) {
      if ((optEdges[i].v1 == optEdges[j].v1 &&
           optEdges[i].v2 == optEdges[j].v2) ||
          (optEdges[i].v1 == optEdges[j].v2 &&
           optEdges[i].v2 == optEdges[j].v1)) {
...
static void OptimizeOptList(optimizeGroup_t* opt) {
  ...
  dmapGlobals.mapPlanes[opt->planeNum].Normal().NormalVectors(opt->axis[0],
                                                              opt->axis[1]);

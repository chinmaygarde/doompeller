...
typedef struct {
  ...
  idPlane plane;  // positive side is forward for the triangle, which is away
                  // from the light
  int planeNum;   // from original triangle, not calculated from the clipped
                  // verts
} shadowTri_t;
...
static void ClipTriangle_r(const shadowTri_t* tri,
                           int startTri,
                           int skipTri,
                           int numTris,
...
====================
*/
static void ClipOccluders(idVec4* verts,
                          glIndex_t* indexes,
                          int numIndexes,
...
static void OptimizeOutputTris(void) {
  ...
  for (i = 0; i < numOutputTris; i++) {
    ...
    for (checkGroup = optGroups; checkGroup;
...
static void GenerateSilEdges(void) {
  ...
  unsigned* edges =
      (unsigned*)_alloca((numOutputTris * 3 + 1) * sizeof(*edges));
  int numEdges = 0;
  ...
  edges[numEdges] =
      -1;  // force the last to make an edge if no matched to previous
...
===================
*/
static void FragmentSilQuad(silQuad_t quad,
                            silPlane_t* silPlane,
                            shadowOptEdge_t* startEdge,
                            shadowOptEdge_t* skipEdge) {
  ...
  for (shadowOptEdge_t* check = startEdge; check; check = check->nextEdge) {
    ...
    for (int i = 0; i < 2; i++) {
      ...
      if ((d1 > EDGE_PLANE_EPSILON && d3 > EDGE_PLANE_EPSILON &&
           d2 < -EDGE_PLANE_EPSILON && d4 < -EDGE_PLANE_EPSILON) ||
          (d2 > EDGE_PLANE_EPSILON && d4 > EDGE_PLANE_EPSILON &&
           d1 < -EDGE_PLANE_EPSILON && d3 < -EDGE_PLANE_EPSILON)) {
        float f = d1 / (d1 - d2);
        float f2 = d3 / (d3 - d4);
        f = f2;
        ...
        idVec3 nearMid =
            (1 - f) * uniqued[quad.nearV[0]] + f * uniqued[quad.nearV[1]];
        int nearMidIndex = FindUniqueVert(nearMid);
        idVec3 farMid =
            (1 - f) * uniqued[quad.farV[0]] + f * uniqued[quad.farV[1]];
...
optimizedShadow_t SuperOptimizeOccluders(idVec4* verts,
  ...
  if (dmapGlobals.shadowOptLevel >= SO_CLIP_SILS) {
    ret.numSilPlaneIndexes =
        numSilQuads * 12;  // this is the worst case with clipping
  } else {
    ret.numSilPlaneIndexes =
        numSilEdges * 6;  // this is the worst case with clipping
  }
...
srfTriangles_t* CreateLightShadow(optimizeGroup_t* shadowerGroups,
  ...
  combined = NULL;
  for (optimizeGroup_t* group = shadowerGroups; group;
  ...
  if (dmapGlobals.shadowOptLevel == SO_MERGE_SURFACES) {
    ...
    shadowTris = R_CreateShadowVolume(&space, occluders, &light->def,
                                      SG_OFFLINE, cullInfo);
  }

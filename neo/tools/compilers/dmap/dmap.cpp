...
bool ProcessModels(void) {
  ...
  for (dmapGlobals.entityNum = 0;
       dmapGlobals.entityNum < dmapGlobals.num_entities;
...
void Dmap(const idCmdArgs& args) {
  ...
  dmapGlobals.shadowOptLevel =
      SO_MERGE_SURFACES;  // create shadows by merging all surfaces, but no
                          // super optimization
  ...
  idStr::Copynz(dmapGlobals.mapFileBase, stripped,
                sizeof(dmapGlobals.mapFileBase));

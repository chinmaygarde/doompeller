...
================
*/
static void OutlineNormalMap(byte* data,
                             int width,
                             int height,
                             int emptyR,
                             int emptyG,
    ...
    for (j = 0; j < height; j++) {
      ...
      for (k = -1; k < 2; k++) {
        for (l = -1; l < 2; l++) {
          ...
          in = orig +
               (((j + l) & (height - 1)) * width + ((i + k) & (width - 1))) * 4;
...
================
*/
static void OutlineColorMap(byte* data,
                            int width,
                            int height,
                            int emptyR,
                            int emptyG,
...
static triHash_t* CreateTriHash(const srfTriangles_t* highMesh) {
  ...
  hash->linkBlocks[hash->numLinkBlocks] =
      (triLink_t*)Mem_Alloc(MAX_LINKS_PER_BLOCK * sizeof(triLink_t));
  hash->numLinkBlocks++;
    ...
    for (j = iBounds[0][0]; j <= iBounds[1][0]; j++) {
      for (k = iBounds[0][1]; k <= iBounds[1][1]; k++) {
        for (l = iBounds[0][2]; l <= iBounds[1][2]; l++) {
          ...
          triLink_t* link = &hash->linkBlocks[numLinks / MAX_LINKS_PER_BLOCK]
                                             [numLinks % MAX_LINKS_PER_BLOCK];
          link->faceNum = i / 3;
...
static float TraceToMeshFace(const srfTriangles_t* highMesh,
                             int faceNum,
                             float minDist,
                             float maxDist,
                             const idVec3& point,
  ...
  for (j = 0; j < 3; j++) {
    sampledNormal +=
        bary[j] * highMesh->verts[highMesh->indexes[faceNum * 3 + j]].normal;
  }
  ...
  for (int i = 0; i < 4; i++) {
    ...
    for (j = 0; j < 3; j++) {
      color += bary[j] *
               highMesh->verts[highMesh->indexes[faceNum * 3 + j]].color[i];
    }
...
#define RAY_STEPS 100
  for (i = 0; i < RAY_STEPS; i++) {
    p = point - rb->hash->bounds[0] +
        normal * (-1.0 + 2.0 * i / RAY_STEPS) * rb->traceDist;
    ...
    for (; linkNum != -1; linkNum = link->nextLink) {
      link = &rb->hash->linkBlocks[linkNum / MAX_LINKS_PER_BLOCK]
                                  [linkNum % MAX_LINKS_PER_BLOCK];

      faceNum = link->faceNum;
      dist = TraceToMeshFace(rb->mesh, faceNum, bestDist, maxDist, point,
                             normal, sampledNormal, sampledColor);
...
=============
*/
static float TriTextureArea(const float a[2],
                            const float b[2],
                            const float c[2]) {
  idVec3 d1, d2;
  idVec3 cross;
...
================
*/
static void RasterizeTriangle(const srfTriangles_t* lowMesh,
  ...
  verts[0][0] =
      lowMesh->verts[lowMesh->indexes[lowFaceNum * 3 + 0]].st[0] * rb->width -
      0.5;
  verts[1][0] =
      lowMesh->verts[lowMesh->indexes[lowFaceNum * 3 + 1]].st[0] * rb->width -
      0.5;
  verts[2][0] =
      lowMesh->verts[lowMesh->indexes[lowFaceNum * 3 + 2]].st[0] * rb->width -
      0.5;
  verts[0][1] =
      lowMesh->verts[lowMesh->indexes[lowFaceNum * 3 + 0]].st[1] * rb->height -
      0.5;
  verts[1][1] =
      lowMesh->verts[lowMesh->indexes[lowFaceNum * 3 + 1]].st[1] * rb->height -
      0.5;
  verts[2][1] =
      lowMesh->verts[lowMesh->indexes[lowFaceNum * 3 + 2]].st[1] * rb->height -
      0.5;
...
#define SKIP_MIRRORS
      ...
      if (!((dists[0] >= -edgeOverlap && dists[1] >= -edgeOverlap &&
             dists[2] >= -edgeOverlap) ||
            (dists[0] <= edgeOverlap && dists[1] <= edgeOverlap &&
             dists[2] <= edgeOverlap))) {
        continue;
      }
      ...
      if (!SampleHighMesh(rb, point, traceNormal, sampledNormal,
                          sampledColor)) {
#if 0
...
static void RenderBumpTriangles(srfTriangles_t* lowMesh, renderBump_t* rb) {
  ...
  // to the true surface.
  idVec3* lowMeshNormals =
      (idVec3*)Mem_ClearedAlloc(lowMesh->numVerts * sizeof(*lowMeshNormals));
  R_DeriveFacePlanes(lowMesh);
  R_CreateSilIndexes(
      lowMesh);  // recreate, merging the mirrored verts back together
  ...
  for (i = 0; i < lowMesh->numIndexes; i++) {
    lowMeshNormals[lowMesh->indexes[i]] =
        lowMeshNormals[lowMesh->silIndexes[i]];
  ...
  for (j = 0; j < lowMesh->numIndexes; j += 3) {
    ...
    qglClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    qglRasterPos2f(0, 1);
    qglPixelZoom(glConfig.vidWidth / (float)rb->width,
                 glConfig.vidHeight / (float)rb->height);
    qglDrawPixels(rb->width, rb->height, GL_RGBA, GL_UNSIGNED_BYTE,
                  rb->localPic);
    qglPixelZoom(1, 1);
    qglFlush();
    GLimp_SwapBuffers();
...
static void InitRenderBump(renderBump_t* rb) {
  ...
  for (i = 0; i < c; i += 4) {
    ...
    rb->localPic[i + 2] = 128;
    rb->localPic[i + 3] =
        0;  // the artists use this for masking traced pixels sometimes
...
void RenderBump_f(const idCmdArgs& args) {
  ...
  renderBumps = (renderBump_t*)R_StaticAlloc(lowPoly->NumSurfaces() *
                                             sizeof(*renderBumps));
  numRenderBumps = 0;
  for (i = 0; i < lowPoly->NumSurfaces(); i++) {
    ...
    if (j != (localArgs.Argc() - 2)) {
      common->Error(
          "usage: renderBump [-size width height] [-aa <1-2>] [globalMap] "
          "[colorMap] [-trace <0.01 - 1.0>] normalMapImageFile "
          "highPolyAseFile");
    }
    ...
    for (j = 0; j < numRenderBumps; j++) {
      ...
      if (idStr::Icmp(rb->highName, opt.highName) || rb->width != opt.width ||
        ...
        common->Error("mismatched renderbump parameters on image %s",
                      rb->outputName);
        continue;
      }
...
void RenderBumpFlat_f(const idCmdArgs& args) {
  ...
  qglMatrixMode(GL_PROJECTION);
  qglLoadIdentity();
  qglOrtho(bounds[0][0], bounds[1][0], bounds[0][2], bounds[1][2],
          ...
          for (j = 0; j < mesh->numIndexes; j += 3) {
            if (flat) {
              ...
              if (surf->shader->GetSurfaceFlags() & SURF_NULLNORMAL) {
                ...
                qglColor3f(0.5 + 0.5 * plane[0], 0.5 - 0.5 * plane[2],
                           0.5 - 0.5 * plane[1]);
              }
              ...
              for (k = 0; k < 3; k++) {
                ...
                if (surf->shader->GetSurfaceFlags() & SURF_NULLNORMAL) {
                  ...
                  qglColor3f(0.5 + 0.5 * n[0], 0.5 - 0.5 * n[2],
                             0.5 - 0.5 * n[1]);
                }

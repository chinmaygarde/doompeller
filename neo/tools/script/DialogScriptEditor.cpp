...
toolTip_t DialogScriptEditor::toolTips[] = {{IDOK, "save"},
                                            {IDCANCEL, "cancel"},
                                            {0, NULL}};
...
void DialogScriptEditor::UpdateStatusBar(void) {
  ...
  statusBar.SetWindowText(
      va("Line: %d, Column: %d, Character: %d", line, column, character));
}
...
BOOL DialogScriptEditor::OnInitDialog() {
  ...
  m_hAccel = ::LoadAccelerators(AfxGetResourceHandle(),
                                MAKEINTRESOURCE(IDR_ACCELERATOR_SCRIPTEDITOR));
...
LRESULT DialogScriptEditor::OnFindDialogMessage(WPARAM wParam, LPARAM lParam) {
  ...
  if (findDlg->ReplaceAll()) {
    ...
    int numReplaces =
        scriptEdit.ReplaceAll(findStr, replaceStr, matchCase, matchWholeWords);
    if (numReplaces == 0) {
      AfxMessageBox("The specified text was not found.",
                    MB_OK | MB_ICONINFORMATION, 0);
    } else {
...
void DialogScriptEditor::OnBnClickedOk() {
  ...
  if (fileSystem->WriteFile(fileName, scriptText, scriptText.Length(),
                            "fs_devpath") == -1) {
    MessageBox(va("Couldn't save: %s", fileName.c_str()),
               va("Error saving: %s", fileName.c_str()), MB_OK | MB_ICONERROR);
    return;
  }

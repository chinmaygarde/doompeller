...
const int EditEnableID[] = {
    IDC_BUTTON_XDN,         IDC_BUTTON_XUP,    IDC_BUTTON_YUP,
    IDC_BUTTON_YDN,         IDC_BUTTON_ZUP,    IDC_BUTTON_ZDN,
    IDC_BUTTON_DROPEMITTER, IDC_BUTTON_VECTOR, IDC_BUTTON_BROWSECOLOR_ENTITY};
...
void ParticleEditorInit(const idDict* spawnArgs) {
  ...
  if (spawnArgs) {
    ...
    g_ParticleDialog->SetParticleVisualization(
        static_cast<int>(CDialogParticleEditor::SELECTED));
  }
...
void CDialogParticleEditor::OnBnClickedButtonSaveAs() {
  ...
  if (dlg.DoModal() == IDOK) {
    ...
    CFileDialog dlgSave(TRUE, "prt", NULL, OFN_CREATEPROMPT,
                        "Particle Files (*.prt)|*.prt||All Files (*.*)|*.*||",
                        AfxGetMainWnd());
    if (dlgSave.DoModal() == IDOK) {
      idStr fileName;
      fileName = fileSystem->OSPathToRelativePath(dlgSave.m_ofn.lpstrFile);
      idDeclParticle* decl = dynamic_cast<idDeclParticle*>(
          declManager->CreateNewDecl(DECL_PARTICLE, dlg.m_strName, fileName));
      if (decl) {
...
void CDialogParticleEditor::OnBnClickedButtonBrowsecolor() {
  ...
  if (DoNewColor(&r, &g, &b, &ob)) {
    color.Format("%f %f %f %f", (float)r / 255.0f, (float)g / 255.0f,
                 (float)b / 255.0f, 1.0f);
    DlgVarsToCurStage();
    CurStageToDlgVars();
  }
        ...
        for (int i = 0; i < count; i++) {
          ...
          if (ent) {
            gameEdit->EntitySetColor(
                ent, idVec3((float)r / 255.0f, (float)g / 255.0f,
                            (float)b / 255.0f));
            str = va("%f %f %f", (float)r / 255.0f, (float)g / 255.0f,
                     (float)b / 255.0f);
            dict2.Clear();
...
void CDialogParticleEditor::OnBnClickedButtonBrowsefadecolor() {
  ...
  ob = 1.0f;
  if (DoNewColor(&r, &g, &b, &ob)) {
    fadeColor.Format("%f %f %f %f", (float)r / 255.0f, (float)g / 255.0f,
                     (float)b / 255.0f, 1.0f);
    DlgVarsToCurStage();
    CurStageToDlgVars();
  }
...
idDeclParticle* CDialogParticleEditor::GetCurParticle() {
  ...
  return static_cast<idDeclParticle*>(
      const_cast<idDecl*>(declManager->DeclByIndex(DECL_PARTICLE, index)));
}
...
void CDialogParticleEditor::RemoveStage() {
  ...
  if (MessageBox("Are you sure you want to remove this stage?", "Remove Stage",
...
void CDialogParticleEditor::DlgVarsToCurStage() {
  ...
  sscanf(color, "%f %f %f %f", &ps->color.x, &ps->color.y, &ps->color.z,
         &ps->color.w);
  sscanf(fadeColor, "%f %f %f %f", &ps->fadeColor.x, &ps->fadeColor.y,
         &ps->fadeColor.z, &ps->fadeColor.w);
  ...
  sscanf(customParms, "%f %f %f %f %f %f %f %f", &ps->customPathParms[0],
         &ps->customPathParms[1], &ps->customPathParms[2],
         &ps->customPathParms[3], &ps->customPathParms[4],
         &ps->customPathParms[5], &ps->customPathParms[6],
  ...
  if (dlg.DoModal() == IDOK) {
    ...
    if (dlgSave.DoModal() == IDOK) {
      ...
      idDecl* decl =
          declManager->CreateNewDecl(DECL_PARTICLE, dlg.m_strName, fileName);
      if (decl) {
        if (MessageBox("Copy current particle?", "Copy current",
                       MB_YESNO | MB_ICONQUESTION) == IDYES) {
...
void CDialogParticleEditor::OnBnClickedButtonSave() {
  ...
  if (strstr(idp->GetFileName(), "implicit")) {
    ...
    CFileDialog dlgSave(FALSE, "prt", NULL, OFN_OVERWRITEPROMPT,
                        "Particle Files (*.prt)|*.prt||All Files (*.*)|*.*||",
                        AfxGetMainWnd());
    if (dlgSave.DoModal() == IDOK) {
      idStr fileName;
...
void CDialogParticleEditor::OnBtnDrop() {
  ...
  while (!nameValid) {
    ...
    if (dlg.DoModal() == IDOK) {
      ...
      if (gameEnt) {
        if (MessageBox("Please choose another name", "Duplicate Entity Name!",

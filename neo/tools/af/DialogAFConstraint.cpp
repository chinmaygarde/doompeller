...
void DialogAFConstraint::InitConstraintTypeDlg(void) {
  ...
  if (constraintDlg) {
    ...
    constraintDlg->MoveWindow(0, 117, rect.right - rect.left,
...
void DialogAFConstraint::InitBodyLists(void) {
  ...
  m_comboBody2List.AddString(
      "world");  // FIXME: we currently assume this is the last body in the list
}
...
void DialogAFConstraint::LoadConstraint(const char* name) {
  ...
  SetSafeComboBoxSelection(&m_comboConstraintType,
                           ConstraintTypeToString(constraint->type), -1);

  // load constrained bodies from the current idDeclAF_Constraint
  s1 = SetSafeComboBoxSelection(&m_comboBody1List, constraint->body1.c_str(),
                                -1);
  s2 = SetSafeComboBoxSelection(&m_comboBody2List, constraint->body2.c_str(),
                                s1);
...
void DialogAFConstraint::OnBnClickedButtonDeleteconstraint() {
  ...
  i = m_comboConstraintList.GetCurSel();
  if (i != CB_ERR) {
    if (MessageBox("Are you sure you want to delete this constraint ?",
...
void DialogAFConstraint::OnDeltaposSpinConstraintFriction(NMHDR* pNMHDR,
  ...
  m_friction = EditSpinFloat((CEdit*)GetDlgItem(IDC_EDIT_CONSTRAINT_FRICTION),
                             pNMUpDown->iDelta < 0);
  UpdateFile();
  *pResult = 0;
}

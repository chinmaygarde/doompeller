...
toolTip_t DialogAFConstraintSpring::toolTips[] = {
    {IDC_RADIO_ANCHOR_JOINT,
     "use the position of a joint for the first anchor"},
    {IDC_COMBO_ANCHOR_JOINT, "first anchor joint name"},
    {IDC_RADIO_ANCHOR_COORDINATES,
     "use absolute coordinates for the first anchor"},
    ...
    {IDC_RADIO_ANCHOR2_JOINT,
     "use the position of a joint for the second anchor"},
    {IDC_COMBO_ANCHOR2_JOINT, "second anchor joint name"},
    {IDC_RADIO_ANCHOR2_COORDINATES,
     "use absolute coordinates for the second anchor"},
...
void DialogAFConstraintSpring::LoadConstraint(idDeclAF_Constraint* c) {
  ...
  SetSafeComboBoxSelection(&m_comboAnchor2Joint,
                           constraint->anchor2.joint1.c_str(), -1);
  ...
  CheckRadioButton(IDC_RADIO_SPRING_NO_MIN_LENGTH, IDC_RADIO_SPRING_MIN_LENGTH,
                   i);
  ...
  CheckRadioButton(IDC_RADIO_SPRING_NO_MAX_LENGTH, IDC_RADIO_SPRING_MAX_LENGTH,
                   i);
...
ON_NOTIFY(UDN_DELTAPOS,
          IDC_SPIN_SPRING_REST_LENGTH,
          OnDeltaposSpinSpringRestLength)
...
ON_NOTIFY(UDN_DELTAPOS,
          IDC_SPIN_SPRING_MAX_LENGTH,
          OnDeltaposSpinLimitMaxLength)
END_MESSAGE_MAP()
...
void DialogAFConstraintSpring::OnDeltaposSpinSpringStretch(NMHDR* pNMHDR,
                                                           LRESULT* pResult) {
  LPNMUPDOWN pNMUpDown = reinterpret_cast<LPNMUPDOWN>(pNMHDR);
  m_stretch = EditSpinFloat((CEdit*)GetDlgItem(IDC_EDIT_SPRING_STRETCH),
                            pNMUpDown->iDelta < 0);
  UpdateFile();
  *pResult = 0;
}
...
void DialogAFConstraintSpring::OnDeltaposSpinSpringCompress(NMHDR* pNMHDR,
                                                            LRESULT* pResult) {
  LPNMUPDOWN pNMUpDown = reinterpret_cast<LPNMUPDOWN>(pNMHDR);
  m_compress = EditSpinFloat((CEdit*)GetDlgItem(IDC_EDIT_SPRING_COMPRESS),
                             pNMUpDown->iDelta < 0);
  UpdateFile();
  *pResult = 0;
}
...
void DialogAFConstraintSpring::OnDeltaposSpinSpringDamping(NMHDR* pNMHDR,
                                                           LRESULT* pResult) {
  LPNMUPDOWN pNMUpDown = reinterpret_cast<LPNMUPDOWN>(pNMHDR);
  m_damping = EditSpinFloat((CEdit*)GetDlgItem(IDC_EDIT_SPRING_DAMPING),
                            pNMUpDown->iDelta < 0);
  UpdateFile();
  *pResult = 0;
}
...
void DialogAFConstraintSpring::OnDeltaposSpinSpringRestLength(
    NMHDR* pNMHDR,
    LRESULT* pResult) {
  LPNMUPDOWN pNMUpDown = reinterpret_cast<LPNMUPDOWN>(pNMHDR);
  if (pNMUpDown->iDelta < 0) {

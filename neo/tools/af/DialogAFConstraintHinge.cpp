...
toolTip_t DialogAFConstraintHinge::toolTips[] = {
    ...
    {IDC_RADIO_HINGE_AXIS_ANGLES,
     "use angles to set the orientation of the hinge axis"},
...
void DialogAFConstraintHinge::LoadConstraint(idDeclAF_Constraint* c) {
  ...
  // hinge axis
  s1 = SetSafeComboBoxSelection(&m_comboAxisJoint1,
                                constraint->axis.joint1.c_str(), -1);
  s2 = SetSafeComboBoxSelection(&m_comboAxisJoint2,
                                constraint->axis.joint2.c_str(), s1);
...
void DialogAFConstraintHinge::SaveConstraint(void) {
  ...
  if (constraint->axis.type == idAFVector::VEC_BONEDIR) {
    ...
    constraint->axis.ToVec3() =
        idAngles(m_axisPitch, m_axisYaw, 0.0f).ToForward();
  }
...
ON_CBN_SELCHANGE(IDC_COMBO_HINGE_AXIS_JOINT1,
                 OnCbnSelchangeComboHingeAxisJoint1)
...
ON_NOTIFY(UDN_DELTAPOS,
          IDC_SPIN_HINGE_LIMIT_ANGLE1,
          OnDeltaposSpinHingeLimitAngle1)
ON_EN_CHANGE(IDC_EDIT_HINGE_LIMIT_ANGLE2, OnEnChangeEditHingeLimitAngle2)
ON_NOTIFY(UDN_DELTAPOS,
          IDC_SPIN_HINGE_LIMIT_ANGLE2,
          OnDeltaposSpinHingeLimitAngle2)
ON_EN_CHANGE(IDC_EDIT_HINGE_LIMIT_ANGLE3, OnEnChangeEditHingeLimitAngle3)
ON_NOTIFY(UDN_DELTAPOS,
          IDC_SPIN_HINGE_LIMIT_ANGLE3,
          OnDeltaposSpinHingeLimitAngle3)
END_MESSAGE_MAP()
...
void DialogAFConstraintHinge::OnEnChangeEditHingeAxisPitch() {
  if (EditControlEnterHit((CEdit*)GetDlgItem(IDC_EDIT_HINGE_AXIS_PITCH))) {
    ...
    m_axisPitch =
        EditVerifyFloat((CEdit*)GetDlgItem(IDC_EDIT_HINGE_AXIS_PITCH));
  }
}
...
void DialogAFConstraintHinge::OnEnChangeEditHingeLimitAngle1() {
  if (EditControlEnterHit((CEdit*)GetDlgItem(IDC_EDIT_HINGE_LIMIT_ANGLE1))) {
    ...
    m_limitAngle1 =
        EditVerifyFloat((CEdit*)GetDlgItem(IDC_EDIT_HINGE_LIMIT_ANGLE1));
  }
}
...
void DialogAFConstraintHinge::OnEnChangeEditHingeLimitAngle2() {
  if (EditControlEnterHit((CEdit*)GetDlgItem(IDC_EDIT_HINGE_LIMIT_ANGLE2))) {
    ...
    m_limitAngle2 =
        EditVerifyFloat((CEdit*)GetDlgItem(IDC_EDIT_HINGE_LIMIT_ANGLE2), false);
  }
}
...
void DialogAFConstraintHinge::OnEnChangeEditHingeLimitAngle3() {
  if (EditControlEnterHit((CEdit*)GetDlgItem(IDC_EDIT_HINGE_LIMIT_ANGLE3))) {
    ...
    m_limitAngle3 =
        EditVerifyFloat((CEdit*)GetDlgItem(IDC_EDIT_HINGE_LIMIT_ANGLE3));
  }
}

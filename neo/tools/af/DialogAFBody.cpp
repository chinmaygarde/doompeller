...
cm_type_t modelTypes[] = {{TRM_INVALID, "invalid"},
                          {TRM_BOX, "box"},
...
toolTip_t DialogAFBody::toolTips[] = {
    ...
    {IDC_RADIO_ORIGIN_COORDINATES,
     "use absolute coordinates for the body origin"},
    {IDC_RADIO_ORIGIN_BONECENTER,
     "use the center of a bone for the body origin"},
    ...
    {IDC_CHECK_SELFCOLLISION,
     "collide with other bodies of this articulated figure"},
    ...
    {IDC_COMBO_MODIFIEDJOINT,
     "the joint modified by the relative rotation of the body"},
...
void DialogAFBody::DoDataExchange(CDataExchange* pDX) {
  ...
  DDX_Control(pDX, IDC_COMBO_ORIGIN_BONECENTER_JOINT1,
              cm_originBoneCenterJoint1);
  DDX_Control(pDX, IDC_COMBO_ORIGIN_BONECENTER_JOINT2,
              cm_originBoneCenterJoint2);
...
void DialogAFBody::InitCollisionModelType(void) {
  ...
  if (type == TRM_BONE) {
    ...
    CheckRadioButton(IDC_RADIO_ORIGIN_COORDINATES, IDC_RADIO_ORIGIN_JOINT,
                     IDC_RADIO_ORIGIN_BONECENTER);
  } else {
    showBone = SW_HIDE;
    showOther = SW_SHOW;
...
void DialogAFBody::LoadBody(const char* name) {
  ...
  SetSafeComboBoxSelection(&cm_comboType, ModelTypeToString(body->modelType),
                           -1);
  if (body->modelType == TRM_BONE) {
    s1 = SetSafeComboBoxSelection(&cm_comboBoneJoint1, body->v1.joint1.c_str(),
                                  -1);
    s2 = SetSafeComboBoxSelection(&cm_comboBoneJoint2, body->v2.joint1.c_str(),
                                  s1);
    cm_width = body->width;
    cm_length = cm_height = 20.0f;
    s1 = SetSafeComboBoxSelection(&cm_originBoneCenterJoint1,
                                  body->v1.joint1.c_str(), -1);
    s2 = SetSafeComboBoxSelection(&cm_originBoneCenterJoint2,
                                  body->v2.joint1.c_str(), s1);
  } else {
    ...
    s1 = SetSafeComboBoxSelection(&cm_originBoneCenterJoint1,
                                  body->origin.joint1.c_str(), -1);
    s2 = SetSafeComboBoxSelection(&cm_originBoneCenterJoint2,
                                  body->origin.joint2.c_str(), s1);
    s1 = SetSafeComboBoxSelection(&cm_originJoint, body->origin.joint1.c_str(),
                                  -1);
  ...
  if (body->frictionDirection.ToVec3() != vec3_origin) {
    ...
    file.WriteFloatString("%f %f %f", body->frictionDirection.ToVec3().x,
                          body->frictionDirection.ToVec3().y,
                          body->frictionDirection.ToVec3().z);
  ...
  if (body->contactMotorDirection.ToVec3() != vec3_origin) {
    ...
    file.WriteFloatString("%f %f %f", body->contactMotorDirection.ToVec3().x,
                          body->contactMotorDirection.ToVec3().y,
                          body->contactMotorDirection.ToVec3().z);
...
void DialogAFBody::SaveBody(void) {
  ...
  if (str.GetLength() != 0) {
    ...
    sscanf(str, "%f %f %f", &body->frictionDirection.ToVec3().x,
           &body->frictionDirection.ToVec3().y,
           &body->frictionDirection.ToVec3().z);
  }
  ...
  if (str.GetLength() != 0) {
    ...
    sscanf(str, "%f %f %f", &body->contactMotorDirection.ToVec3().x,
           &body->contactMotorDirection.ToVec3().y,
           &body->contactMotorDirection.ToVec3().z);
  }
...
ON_CBN_SELCHANGE(IDC_COMBO_ORIGIN_BONECENTER_JOINT1,
                 OnOnCbnSelchangeComboOriginBoneCenterJoint1)
ON_CBN_SELCHANGE(IDC_COMBO_ORIGIN_BONECENTER_JOINT2,
                 OnOnCbnSelchangeComboOriginBoneCenterJoint2)
...
void DialogAFBody::OnEnChangeEditCmDensity() {
  if (EditControlEnterHit((CEdit*)GetDlgItem(IDC_EDIT_CM_DENSITY))) {
    ...
    cm_density =
        EditVerifyFloat((CEdit*)GetDlgItem(IDC_EDIT_CM_DENSITY), false);
  }
}

void DialogAFBody::OnDeltaposSpinCmDensity(NMHDR* pNMHDR, LRESULT* pResult) {
  LPNMUPDOWN pNMUpDown = reinterpret_cast<LPNMUPDOWN>(pNMHDR);
  cm_density = EditSpinFloat((CEdit*)GetDlgItem(IDC_EDIT_CM_DENSITY),
                             pNMUpDown->iDelta < 0);
  ValidateCollisionModelDensity(false);
  UpdateFile();
  *pResult = 0;
...
void DialogAFBody::OnEnChangeEditAnglesPitch() {
  if (EditControlEnterHit((CEdit*)GetDlgItem(IDC_EDIT_ANGLES_PITCH))) {
    ...
    cm_angles_pitch =
        EditVerifyFloat((CEdit*)GetDlgItem(IDC_EDIT_ANGLES_PITCH));
  }
}
...
void DialogAFBody::OnEnChangeEditLinearfriction() {
  if (EditControlEnterHit((CEdit*)GetDlgItem(IDC_EDIT_LINEARFRICTION))) {
    ...
    m_linearFriction =
        EditVerifyFloat((CEdit*)GetDlgItem(IDC_EDIT_LINEARFRICTION), false);
  }
}
...
void DialogAFBody::OnDeltaposSpinLinearfriction(NMHDR* pNMHDR,
                                                LRESULT* pResult) {
  LPNMUPDOWN pNMUpDown = reinterpret_cast<LPNMUPDOWN>(pNMHDR);
  m_linearFriction = EditSpinFloat((CEdit*)GetDlgItem(IDC_EDIT_LINEARFRICTION),
                                   pNMUpDown->iDelta < 0);
  UpdateFile();
  *pResult = 0;
}
...
void DialogAFBody::OnEnChangeEditAngularfriction() {
  if (EditControlEnterHit((CEdit*)GetDlgItem(IDC_EDIT_ANGULARFRICTION))) {
    ...
    m_angularFriction =
        EditVerifyFloat((CEdit*)GetDlgItem(IDC_EDIT_ANGULARFRICTION), false);
  }
}
...
void DialogAFBody::OnDeltaposSpinAngularfriction(NMHDR* pNMHDR,
                                                 LRESULT* pResult) {
  LPNMUPDOWN pNMUpDown = reinterpret_cast<LPNMUPDOWN>(pNMHDR);
  m_angularFriction = EditSpinFloat(
      (CEdit*)GetDlgItem(IDC_EDIT_ANGULARFRICTION), pNMUpDown->iDelta < 0);
  UpdateFile();
  *pResult = 0;
}
...
void DialogAFBody::OnEnChangeEditContactfriction() {
  if (EditControlEnterHit((CEdit*)GetDlgItem(IDC_EDIT_CONTACTFRICTION))) {
    ...
    m_contactFriction =
        EditVerifyFloat((CEdit*)GetDlgItem(IDC_EDIT_CONTACTFRICTION), false);
  }
}
...
void DialogAFBody::OnDeltaposSpinContactfriction(NMHDR* pNMHDR,
                                                 LRESULT* pResult) {
  LPNMUPDOWN pNMUpDown = reinterpret_cast<LPNMUPDOWN>(pNMHDR);
  m_contactFriction = EditSpinFloat(
      (CEdit*)GetDlgItem(IDC_EDIT_CONTACTFRICTION), pNMUpDown->iDelta < 0);
  UpdateFile();
  *pResult = 0;
}

...
toolTip_t DialogAFProperties::toolTips[] = {
    ...
    {IDC_CHECK_SELFCOLLISION,
     "allow bodies to collide with other bodies of this articulated figure"},
    ...
    {IDC_EDIT_LINEARVELOCITY,
     "do not suspend simulation if the linear velocity is higher than this "
     "value"},
    {IDC_EDIT_ANGULARVELOCITY,
     "do not suspend simulation if the angular velocity is higher than this "
     "value"},
    {IDC_EDIT_LINEARACCELERATION,
     "do not suspend simulation if the linear acceleration is higher than this "
     "value"},
    {IDC_EDIT_ANGULARACCELERATION,
     "do not suspend simulation if the angular acceleration is higher than "
     "this value"},
    {IDC_EDIT_NO_MOVE_TIME,
     "suspend simulation if hardly any movement for this many seconds"},
    {IDC_EDIT_MAXIMUM_MOVE_TIME,
     "always suspend simulation after running for this many seconds"},
...
ON_NOTIFY(UDN_DELTAPOS,
          IDC_SPIN_CONSTRAINTFRICTION,
          OnDeltaposSpinConstraintfriction)
...
void DialogAFProperties::OnBnClickedCheckSelfcollision() {
  UpdateFile();
  if (file && file->bodies.Num() &&
      MessageBox("Apply to all bodies ?", "Self Collision",
...
void DialogAFProperties::OnEnChangeEditContents() {
  if (EditControlEnterHit(&m_editContents)) {
    UpdateFile();
    if (file && file->bodies.Num() &&
        MessageBox("Apply to all bodies ?", "Contents",
                   MB_YESNO | MB_ICONQUESTION) == IDYES) {
      for (int i = 0; i < file->bodies.Num(); i++) {
...
void DialogAFProperties::OnEnChangeEditClipmask() {
  if (EditControlEnterHit(&m_editClipMask)) {
    UpdateFile();
    if (file && file->bodies.Num() &&
        MessageBox("Apply to all bodies ?", "Clip Mask",
                   MB_YESNO | MB_ICONQUESTION) == IDYES) {
      for (int i = 0; i < file->bodies.Num(); i++) {
...
void DialogAFProperties::OnEnChangeEditLinearfriction() {
  if (EditControlEnterHit((CEdit*)GetDlgItem(IDC_EDIT_LINEARFRICTION))) {
    UpdateFile();
    if (file && file->bodies.Num() &&
        MessageBox("Apply to all bodies ?", "Linear Friction",
                   MB_YESNO | MB_ICONQUESTION) == IDYES) {
      for (int i = 0; i < file->bodies.Num(); i++) {
...
void DialogAFProperties::OnEnChangeEditAngularfriction() {
  if (EditControlEnterHit((CEdit*)GetDlgItem(IDC_EDIT_ANGULARFRICTION))) {
    UpdateFile();
    if (file && file->bodies.Num() &&
        MessageBox("Apply to all bodies ?", "Angular Friction",
                   MB_YESNO | MB_ICONQUESTION) == IDYES) {
      for (int i = 0; i < file->bodies.Num(); i++) {
...
void DialogAFProperties::OnEnChangeEditContactfriction() {
  if (EditControlEnterHit((CEdit*)GetDlgItem(IDC_EDIT_CONTACTFRICTION))) {
    UpdateFile();
    if (file && file->bodies.Num() &&
        MessageBox("Apply to all bodies ?", "Contact Friction",
                   MB_YESNO | MB_ICONQUESTION) == IDYES) {
      for (int i = 0; i < file->bodies.Num(); i++) {
...
void DialogAFProperties::OnEnChangeEditConstraintfriction() {
  if (EditControlEnterHit((CEdit*)GetDlgItem(IDC_EDIT_CONSTRAINTFRICTION))) {
    UpdateFile();
    if (file && file->constraints.Num() &&
        MessageBox("Apply to all constraints ?", "Constraint Friction",
                   MB_YESNO | MB_ICONQUESTION) == IDYES) {
...
void DialogAFProperties::OnDeltaposSpinConstraintfriction(NMHDR* pNMHDR,
                                                          LRESULT* pResult) {
  LPNMUPDOWN pNMUpDown = reinterpret_cast<LPNMUPDOWN>(pNMHDR);
  m_constraintFriction = EditSpinFloat(
      (CEdit*)GetDlgItem(IDC_EDIT_CONSTRAINTFRICTION), pNMUpDown->iDelta < 0);
  UpdateFile();
  *pResult = 0;
}
...
void DialogAFProperties::OnDeltaposSpinTotalmass(NMHDR* pNMHDR,
                                                 LRESULT* pResult) {
  LPNMUPDOWN pNMUpDown = reinterpret_cast<LPNMUPDOWN>(pNMHDR);
  m_totalMass = EditSpinFloat((CEdit*)GetDlgItem(IDC_EDIT_TOTALMASS),
                              pNMUpDown->iDelta < 0);
  UpdateFile();
  *pResult = 0;
}

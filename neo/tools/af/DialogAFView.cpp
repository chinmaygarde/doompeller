...
toolTip_t DialogAFView::toolTips[] = {
    ...
    {IDC_CHECK_VIEW_CONSTRAINEDBODIES,
     "show bodies constrained by current constraint (body1 = cyan, body2 = "
     "blue)"},
    ...
    {IDC_CHECK_PHYSICS_SHOW_DRAG_SELECTION,
     "show selection box around the entity selected for dragging"},
    {0, NULL}};
...
ON_BN_CLICKED(IDC_CHECK_VIEW_CONSTRAINTNAMES,
              OnBnClickedCheckViewConstraintnames)
...
ON_BN_CLICKED(IDC_CHECK_VIEW_CONSTRAINEDBODIES,
              OnBnClickedCheckViewConstrainedBodies)
...
ON_BN_CLICKED(IDC_CHECK_PHYSICS_NOSELFCOLLISION,
              OnBnClickedCheckPhysicsNoselfcollision)
ON_BN_CLICKED(IDC_CHECK_PHYSICS_TIMING, OnBnClickedCheckPhysicsTiming)
ON_BN_CLICKED(IDC_CHECK_PHYSICS_DRAG_ENTITIES,
              OnBnClickedCheckPhysicsDragEntities)
ON_BN_CLICKED(IDC_CHECK_PHYSICS_SHOW_DRAG_SELECTION,
              OnBnClickedCheckPhysicsShowDragSelection)
END_MESSAGE_MAP()
...
void DialogAFView::OnBnClickedCheckViewConstraintnames() {
  UpdateData(TRUE);
  cvarSystem->SetCVarBool("af_showConstraintNames",
                          m_showConstraintNames != FALSE);
}
...
void DialogAFView::OnBnClickedCheckViewConstrainedBodies() {
  UpdateData(TRUE);
  cvarSystem->SetCVarBool("af_showConstrainedBodies",
                          m_showConstrainedBodies != FALSE);
}
...
void DialogAFView::OnBnClickedCheckLinesDepthtest() {
  UpdateData(TRUE);
  cvarSystem->SetCVarBool("r_debugLineDepthTest",
                          m_debugLineDepthTest != FALSE);
}

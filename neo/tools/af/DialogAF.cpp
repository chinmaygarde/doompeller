...
toolTip_t DialogAF::toolTips[] = {
    ...
    {IDC_BUTTON_AF_SPAWN,
     "spawn ingame entity using the selected articulated figure"},
    {IDC_BUTTON_AF_TPOSE,
     "set ingame entity using the selected articulated figure back into "
     "T-Pose"},
    {IDC_BUTTON_AF_KILL,
     "kill ingame entity using the selected articulated figure"},
...
DialogAF::DialogAF(CWnd* pParent /*=NULL*/)
    : CDialog(DialogAF::IDD, pParent), file(NULL) {
  wndTabs = NULL;
  wndTabDisplay = NULL;
}
...
void DialogAF::InitAFList(void) {
  ...
  for (i = 0; i < c; i++) {
    AFList.AddString(static_cast<const idDeclAF*>(
                         declManager->DeclByIndex(DECL_AF, i, false))
                         ->GetName());
  }
}
...
void AFEditorInit(const idDict* spawnArgs) {
  ...
  if (spawnArgs) {
    ...
    idDeclAF* decl = static_cast<idDeclAF*>(
        const_cast<idDecl*>(declManager->FindType(DECL_AF, name)));
    if (decl) {
...
void DialogAF::OnCbnSelchangeComboAf() {
  ...
  if (index != CB_ERR) {
    ...
    LoadFile(static_cast<idDeclAF*>(
        const_cast<idDecl*>(declManager->FindType(DECL_AF, str))));
  }
}
...
void DialogAF::OnBnClickedButtonAfNew() {
  ...
  CFileDialog dlgSave(FALSE, "map", NULL, OFN_OVERWRITEPROMPT,
                      "AF Files (*.af)|*.af|All Files (*.*)|*.*||",
                      AfxGetMainWnd());
  ...
  idDeclAF* decl = static_cast<idDeclAF*>(
      declManager->CreateNewDecl(DECL_AF, name, fileName));
  LoadFile(decl);
...
void DialogAF::OnBnClickedButtonAfSave() {
  ...
  if (file) {
    if (file->Save()) {
      ...
      MessageBox("Saving the file failed. Make sure the file is not read-only.",
...
void DialogAF::OnBnClickedCancel() {
  ...
  c = declManager->GetNumDecls(DECL_AF);
  for (i = 0; i < c; i++) {
    if (static_cast<const idDeclAF*>(declManager->DeclByIndex(DECL_AF, i))
            ->modified) {

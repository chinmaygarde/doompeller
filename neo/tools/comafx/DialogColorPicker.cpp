...
class CMyColorDialog : public CColorDialog {
 ...
 public:
  CMyColorDialog(COLORREF clrInit = 0,
                 DWORD dwFlags = 0,
...
CMyColorDialog::CMyColorDialog(COLORREF clrInit,
                               DWORD dwFlags,
                               CWnd* pParentWnd)
...
void CDialogColorPicker::OnMouseMove(UINT nFlags, CPoint point) {
  if (GetCapture() == this && m_nMouseIn) {
    ...
    if (nIndex == RED) {
      ...
      rects[RED] = CRect(pt.x - RECT_WIDTH, pt.y - RECT_WIDTH,
                         pt.x + RECT_WIDTH, pt.y + RECT_WIDTH);
      CalcCuboid();
      DrawLines(&dc);
      bChange = TRUE;
      ...
      rects[GREEN] = CRect(pt.x - RECT_WIDTH, pt.y - RECT_WIDTH,
                           pt.x + RECT_WIDTH, pt.y + RECT_WIDTH);
      CalcCuboid();
      DrawLines(&dc);
      bChange = TRUE;
      ...
      rects[BLUE] = CRect(pt.x - RECT_WIDTH, pt.y - RECT_WIDTH,
                          pt.x + RECT_WIDTH, pt.y + RECT_WIDTH);
      CalcCuboid();
      DrawLines(&dc);
      bChange = TRUE;
...
BOOL CDialogColorPicker::OnInitDialog() {
  ...
  brightRect =
      CRect(hsbRect.left + hsbWidth + 20, hsbRect.top,
            hsbRect.left + hsbWidth + 20 + 20, hsbRect.top + hsbHeight);
...
void CDialogColorPicker::CalcRects() {
  ...
  m_CurrentRect = CRect(pt.x - RECT_WIDTH, pt.y - RECT_WIDTH, pt.x + RECT_WIDTH,
                        pt.y + RECT_WIDTH);
...
void CDialogColorPicker::DrawHSB(CDC* pDC) {
  if (m_HsbBitmap.GetSafeHandle()) {
    ...
    pDC->BitBlt(hsbRect.left, hsbRect.top, hsbWidth, hsbHeight, &memDC, 0, 0,
                SRCCOPY);
    m_BrightDIB.BitBlt(pDC->m_hDC, brightRect.left, brightRect.top,
                       brightRect.Width(), brightRect.Height(), 0, 0);
    DrawMarkers(pDC);
...
void CDialogColorPicker::DrawRGB(CDC* pDC) {
  if (m_RgbBitmap.GetSafeHandle()) {
    ...
    pDC->BitBlt(rgbRect.left, rgbRect.top, rgbWidth, rgbHeight, &memDC, 0, 0,
                SRCCOPY);
    DrawLines(pDC);
    memDC.SelectObject(pOldBitmap);
  }
...
void CDialogColorPicker::DrawLines(CDC* pDC) {
  ...
  for (int i = 0; i < 3; i++) {
    cr = CRect(pt[i].x - RECT_WIDTH, pt[i].y - RECT_WIDTH, pt[i].x + RECT_WIDTH,
               pt[i].y + RECT_WIDTH);
    rects[i] = cr;
    DrawXorRect(pDC, cr);
  }

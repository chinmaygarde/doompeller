...
void CVectorCtl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) {
  ...
  if (!m_bSelected && lpDrawItemStruct->itemState & ODS_SELECTED) {
    // Just got re-selected (user starts a new mouse dragging session)
             ...
             !(lpDrawItemStruct->itemState &
               ODS_SELECTED) &&  // New state is NOT selected
             (lpDrawItemStruct->itemState &
              ODS_FOCUS) &&        // New state is still in focus
  ...
  m_bSelected =
      lpDrawItemStruct->itemState & ODS_SELECTED;  // Update selection status
...
BOOL CVectorCtl::Normalize() {
  double Norm =
      m_dVec[0] * m_dVec[0] + m_dVec[1] * m_dVec[1] + m_dVec[2] * m_dVec[2];
...
COLORREF CVectorCtl::CalcLight(double dx, double dy, double dz) {
  double NL = dx * m_dVec[0] + dy * m_dVec[1] + dz * m_dVec[2], RV = 2.0 * NL,
         rx = m_dVec[0] - (dx * RV), ry = m_dVec[1] - (dy * RV),
         rz = m_dVec[2] - (dz * RV);
...
void CVectorCtl::RotateByXandY(double XRot,
                               double YRot) {  // Angles are in radians

  if (XRot == 0.0 && YRot == 0.0) {
    return;
  }

  double cx = cos(XRot), sx = sin(XRot), cy = cos(YRot), sy = sin(YRot),
  ...
  if (!m_bFrontVector || dz >= 0.0) {  // Vector is bounds free
    ...
    m_dVec[1] = dy;
    m_dVec[2] = dz;
  } else {  // Otherwise, do not allow Z to be negative (light shines from
            // behind)
    m_dVec[2] = 0.0;
    m_dVec[0] = dx;
    m_dVec[1] = dy;
...
void CVectorCtl::CreateBackground() {
  if (!m_bBmpCreated)
    return;                        //  No image yet
  if (!m_bBackgroundBitmapUsed) {  // No background used - fill with gradient
                                   // color

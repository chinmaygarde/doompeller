...
BOOL CDIB::Create(BITMAPINFOHEADER& bmInfo) {
  ...
  int size;
  size = sizeof(BITMAPINFOHEADER) + sizeof(RGBQUAD) * GetPaletteSize(bmInfo) +
         bytes * height;
...
CBitmap* CDIB::GetTempBitmap(CDC& dc) {
  ...
  CBitmap* temp;
  ASSERT(m_pVoid != NULL);
  hBitmap = CreateDIBitmap(dc.m_hDC, (PBITMAPINFOHEADER)m_pInfo, CBM_INIT,
                           (const void*)m_pBits, m_pInfo, DIB_RGB_COLORS);
...
void CDIB::BitBlt(HDC hDest,
  ...
  SetDIBitsToDevice(hDest, nXDest, nYDest, nWidth, nHeight, xSrc,
                    Height() - ySrc - nHeight, 0, Height(), m_pBits, m_pInfo,
                    DIB_RGB_COLORS);
}
...
void CDIB::StretchBlt(HDC hDest,
  ...
  int err;
  err = StretchDIBits(
      hDest, nXDest, nYDest, nDWidth, nDHeight, xSrc, ySrc, nSWidth, nSHeight,
      m_pBits, (CONST BITMAPINFO*)&m_pInfo->bmiHeader, DIB_RGB_COLORS, SRCCOPY);
}
...
void CDIB::StretchBlt(int nXDest,
  ...
  if (!xMod && !yMod && xDiv > 0 && yDiv > 0) {
    ExpandBlt(nXDest, nYDest, xDiv, yDiv, dibSrc, xSrc, ySrc, nSWidth,
              nSHeight);
    return;
  }
...
DWORD CDIB::GetDIBSize() {
  return sizeof(BITMAPINFOHEADER) + sizeof(RGBQUAD) * GetPaletteSize() +
         bytes * height;
}
...
HANDLE CDIB::DIBHandle() {
  int nSize;
  HANDLE hMem;
  nSize = sizeof(BITMAPINFOHEADER) + sizeof(RGBQUAD) * GetPaletteSize() +
          bytes * height;
  ...
  pBits =
      lpVoid + sizeof(BITMAPINFOHEADER) + sizeof(RGBQUAD) * GetPaletteSize();
...
BOOL CDIB::SaveBMP(CString& csFileName) {
  ...
  bFile.bfOffBits = sizeof(BITMAPINFOHEADER) +

...
// or
//      static void CALLBACK MyClass::MyCallBack (double dVecX, double dVecY,
//      double dVecZ);
...
#define DEFAULT_VEC \
  { 0.00, 0.00, 1.00 }                    // Default start vector
...
#define DEFAULT_START_BACKGROUND_COLOR \
  RGB(0, 0, 0)  // Default gradient background start color
#define DEFAULT_END_BACKGROUND_COLOR \
  RGB(140, 0, 120)             // Default gradient background end color
...
#define VAL_NOT_IN_USE -50000  // Internal use
 ...
 private:
  ...
  BOOL m_bBmpCreated,           // Was the bitmap created ?
      ...
      m_bFrontVector,  // Is the vector constrained to be facing front (positive
                       // Z) ?
  ...
  double m_dSpecularExponent,  // Specularity effect intensity
      m_dSensitivity;  // The bigger the number the less sensitive the mouse
                       // gets Valid ranges are 1..MAX_UINT

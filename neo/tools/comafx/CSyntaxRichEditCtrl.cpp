...
void CSyntaxRichEditCtrl::InitFont(void) {
  ...
  defaultCharFormat.dwMask = CFM_CHARSET | CFM_FACE | CFM_SIZE | CFM_BOLD |
                             CFM_COLOR | CFM_PROTECTED | CFM_BACKCOLOR;
...
void CSyntaxRichEditCtrl::Init(void) {
  ...
  if (iu == NULL ||
      iu->QueryInterface(tom::IID_ITextDocument, (void**)&m_TextDoc) != S_OK) {
    m_TextDoc = NULL;
  }
  ...
  SetEventMask(GetEventMask() | ENM_CHANGE | ENM_KEYEVENTS | ENM_MOUSEEVENTS |
               ENM_PROTECTED);  // ENM_SCROLLEVENTS

  EnableToolTips(TRUE);

  // create auto complete list box
  CRect rect(0, 0, AUTOCOMPLETE_WIDTH, AUTOCOMPLETE_HEIGHT);
  autoCompleteListBox.Create(
      WS_DLGFRAME | WS_VISIBLE | WS_VSCROLL | LBS_SORT | LBS_NOTIFY, rect, this,
      IDC_LISTBOX_AUTOCOMPLETE);
  autoCompleteListBox.SetFont(GetParent()->GetFont());
  autoCompleteListBox.ShowWindow(FALSE);

  // create function parameter tool tip
  funcParmToolTip.Create(WS_VISIBLE | WS_BORDER, rect, this,
                         IDC_EDITBOX_FUNCPARMS);
...
================
*/
void CSyntaxRichEditCtrl::SetObjectMemberCallback(
    objectMemberCallback_t callback) {
...
void CSyntaxRichEditCtrl::HighlightSyntax(int startCharIndex,
                                          int endCharIndex) {
  int c, t, line, charIndex, textLength, syntaxStart, keyWordLength,
      keyWordIndex;
  const char* keyWord;
  CHARRANGE visRange;
  CString text;
  ...
  if (startCharIndex < visRange.cpMin) {
    SetColor(startCharIndex, visRange.cpMin - 1, SRE_COLOR_BLACK,
             INVALID_BACK_COLOR, false);
    startCharIndex = visRange.cpMin;
  }
  if (visRange.cpMax < endCharIndex) {
    SetColor(visRange.cpMax, endCharIndex, SRE_COLOR_BLACK, INVALID_BACK_COLOR,
             false);
    ...
    switch (t) {
      ...
      case CT_COMMENT: {
        ...
        if (c == '/') {
          ...
          SetColor(syntaxStart, charIndex + 1, singleLineCommentColor,
          ...
          charIndex++;
          SetColor(syntaxStart, charIndex + 1, multiLineCommentColor,
                   MULTILINE_COMMENT_BACK_COLOR, false);
        }
        break;
      }
      case CT_STRING: {
        ...
        SetColor(syntaxStart, charIndex + 1, stringColor[stringColorIndex],
                 DEFAULT_BACK_COLOR, false);
        stringColorIndex ^= 1;
        break;
      }
      case CT_LITERAL: {
        ...
        SetColor(syntaxStart, charIndex + 1, literalColor, DEFAULT_BACK_COLOR,
                 false);
        break;
      }
      case CT_NUMBER: {
      ...
      case CT_NAME: {
        ...
        if (keyWordIndex != -1) {
          SetColor(syntaxStart, syntaxStart + keyWordLength,
                   keyWordColors[keyWordIndex], DEFAULT_BACK_COLOR, false);
        }
        break;
      }
      case CT_PUNCTUATION: {
...
void CSyntaxRichEditCtrl::BracedSectionShow(void) {
  for (int i = 0; i < 2; i++) {
    if (bracedSection[i] >= 0) {
      SetColor(bracedSection[i], bracedSection[i] + 1, braceHighlightColor,
               DEFAULT_BACK_COLOR, true);
    }
  }
}
...
void CSyntaxRichEditCtrl::BracedSectionHide(void) {
  for (int i = 0; i < 2; i++) {
    if (bracedSection[i] >= 0) {
      SetColor(bracedSection[i], bracedSection[i] + 1, defaultColor,
               DEFAULT_BACK_COLOR, false);
      bracedSection[i] = -1;
    }
  }
...
bool CSyntaxRichEditCtrl::GetNameForMousePosition(idStr& name) const {
  ...
  for (endCharIndex = charIndex; endCharIndex < GetTextLength();
       endCharIndex++) {
...
BOOL CSyntaxRichEditCtrl::OnToolTipNotify(UINT id,
  ...
  *pResult = 0;
  ...
  if (GetNameForMousePosition(name)) {
    ...
    // disappears
    state->m_pToolTip->SetDelayTime(TTDT_AUTOPOP,
                                    5000 + toolTip.GetLength() * 50);
  ...
  if (autoCompleteStart >= 0) {
    ...
    switch (nKey) {
      ...
      case VK_DOWN: {  // down arrow
        sel = Min(autoCompleteListBox.GetCount() - 1,
                  autoCompleteListBox.GetCurSel() + 1);
        autoCompleteListBox.SetCurSel(sel);
        return;
      }
      ...
      case VK_NEXT: {  // page down key
        sel = Min(autoCompleteListBox.GetCount() - 1,
                  autoCompleteListBox.GetCurSel() + 10);
        autoCompleteListBox.SetCurSel(sel);
        return;
      }
...
void CSyntaxRichEditCtrl::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags) {
  ...
  if (keyWordAutoCompletion && charType[nChar] == CT_NAME &&
      funcParmToolTipStart < 0) {
    long selStart, selEnd;
...
BOOL CSyntaxRichEditCtrl::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt) {
  if (autoCompleteStart >= 0) {
    ...
    if (zDelta > 0) {
      ...
      sel = Min(autoCompleteListBox.GetCount() - 1,
                autoCompleteListBox.GetCurSel() - (zDelta / WHEEL_DELTA));
    }
    autoCompleteListBox.SetCurSel(sel);
    return TRUE;
  }
...
void CSyntaxRichEditCtrl::OnChange() {
  ...
  GetParent()->SendMessage(WM_NOTIFY, (EN_CHANGE << 16) | GetDlgCtrlID(),
                           (LPARAM)&pNMHDR);
}

...
void SpinButton_SetRange(HWND hWnd, float minRange, float maxRange) {
  SendMessage(hWnd, UDM_SETRANGE32, (LONG)(minRange * 100.0f),
...
void SpinButton_HandleNotify(NMHDR* hdr) {
  ...
  char strValue[64];
  float value;
  GetWindowText((HWND)SendMessage(hdr->hwndFrom, UDM_GETBUDDY, 0, 0), strValue,
                63);
  ...
  SetWindowText((HWND)SendMessage(hdr->hwndFrom, UDM_GETBUDDY, 0, 0),
                va("%g", value));
}

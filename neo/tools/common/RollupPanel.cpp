...
bool rvRollupPanel::Create(DWORD dwStyle,
  ...
  mWindow = CreateWindowEx(WS_EX_TOOLWINDOW, "ROLLUP_PANEL", "",
                           dwStyle | WS_CLIPSIBLINGS, rect.left, rect.top,
                           rect.right - rect.left, rect.bottom - rect.top,
                           parent, NULL, win32.hInstance, this);
...
int rvRollupPanel::InsertItem(const char* caption,
  ...
  HWND groupbox = CreateWindow("BUTTON", "", WS_CHILD | BS_GROUPBOX, r.left,
                               r.top, r.right - r.left, r.bottom - r.top,
                               mWindow, 0, win32.hInstance, NULL);

  // Create the expand button
  HWND button = CreateWindow("BUTTON", caption,
...
void rvRollupPanel::RecallLayout(void) {
  ...
  for (i = 0; i < mItems.Num(); i++) {
    ...
    SendMessage(item->mButton, BM_SETCHECK,
                (item->mEnable & item->mExpanded) ? BST_CHECKED : BST_UNCHECKED,
                0);
    ...
    // collapsed
...
#endif
                     item->mGroupBox, 0, 2, posy,
                     (r.right - r.left) - 3 - RP_SCROLLBARWIDTH, 16,
                     SWP_NOZORDER | SWP_SHOWWINDOW);
      ...
      SetWindowPos(
#endif
                     item->mDialog, 0, RP_GRPBOXINDENT, 0, 0, 0,
                     SWP_NOZORDER | SWP_HIDEWINDOW | SWP_NOSIZE | SWP_NOMOVE);
...
LRESULT CALLBACK rvRollupPanel::DialogProc(HWND hWnd,
  ...
  rvRollupPanel* _this =
      (rvRollupPanel*)GetWindowLong(GetParent(hWnd), GWL_USERDATA);
...
int rvRollupPanel::HandlePaint(WPARAM wParam, LPARAM lParam) {
  ...
  if (mItemHeight > (r.bottom - r.top)) {
    sbSize =
        clientHeight -
        (((mItemHeight - (r.bottom - r.top)) * clientHeight) / mItemHeight);
...
int rvRollupPanel::HandleContextMenu(WPARAM wParam, LPARAM lParam) {
  ...
  for (i = 0; i < mItems.Num(); i++) {
    ...
    TrackPopupMenu(menu, TPM_LEFTALIGN | TPM_LEFTBUTTON, point.x, point.y, 0,
                   mWindow, NULL);
  }
...
void rvRollupPanel::AutoSize(void) {
  ...
  SetWindowPos(mWindow, NULL, 0, 0, width, cr.bottom - cr.top,

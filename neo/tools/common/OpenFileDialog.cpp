...
bool rvOpenFileDialog::DoModal(HWND parent) {
  ...
  return DialogBoxParam(mInstance, MAKEINTRESOURCE(IDD_TOOLS_OPEN), parent,
                        DlgProc, (LPARAM)this)
             ? true
             : false;
}
...
void rvOpenFileDialog::HandleCommandOK(void) {
  ...
  // If the item is a file then build the filename and end the dialog
    ...
    if (mFlags & OFD_MUSTEXIST) {
      ...
      if (!file) {
        MessageBox(mWnd,
                   va("%s\nFile not found.\nPlease verify the correct file "
                      "name was given",
                      mFilename.c_str()),
...
void rvOpenFileDialog::HandleInitDialog(void) {
  ...
  mImageList = ImageList_LoadBitmap(mInstance, MAKEINTRESOURCE(IDB_TOOLS_OPEN),
                                    16, 1, RGB(255, 255, 255));
  mBackBitmap = (HBITMAP)LoadImage(mInstance, MAKEINTRESOURCE(IDB_TOOLS_BACK),
                                   IMAGE_BITMAP, 16, 16,
  ...
  SendMessage(GetDlgItem(mWnd, IDC_TOOLS_BACK), BM_SETIMAGE, IMAGE_BITMAP,
              (LONG)mBackBitmap);
  ...
  switch (msg) {
    ...
    case WM_COMMAND:
      switch (LOWORD(wparam)) {
        ...
        case IDC_TOOLS_BACK: {
          int sel = SendMessage(GetDlgItem(wnd, IDC_TOOLS_LOOKIN), CB_GETCURSEL,
                                0, 0);
          if (sel > 0) {
            sel--;
            SendMessage(GetDlgItem(wnd, IDC_TOOLS_LOOKIN), CB_SETCURSEL, sel,
                        0);

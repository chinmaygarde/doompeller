...
bool rvRegistryOptions::Save(void) {
  ...
  if (ERROR_SUCCESS != RegCreateKeyEx(HKEY_LOCAL_MACHINE, mBaseKey, 0, NULL, 0,
  ...
  for (i = 0; i < mValues.GetNumKeyVals(); i++) {
    ...
    assert(key);
    RegSetValueEx(hKey, key->GetKey().c_str(), 0, REG_SZ,
                  (BYTE*)key->GetValue().c_str(), key->GetValue().Length());
  }

  // Write Recent Files
  for (i = 0; i < mRecentFiles.Num(); i++) {
    RegSetValueEx(hKey, va("mru%d", i), 0, REG_SZ,
                  (BYTE*)mRecentFiles[i].c_str(), mRecentFiles[i].Length());
  }
...
bool rvRegistryOptions::Load(void) {
  ...
  if (ERROR_SUCCESS !=
      RegOpenKeyEx(HKEY_LOCAL_MACHINE, mBaseKey, 0, KEY_READ, &hKey)) {
    return false;
  }
  ...
  keyname[0] = 0;
  dwSize = MAX_PATH;
  for (i = 0; RegEnumValue(hKey, i, keyname, &dwSize, NULL, NULL, NULL, NULL) ==
              ERROR_SUCCESS;
       i++) {
    temp[0] = '\0';
    dwSize = MAX_PATH;
    ...
    if (ERROR_SUCCESS !=
        RegQueryValueEx(hKey, keyname, NULL, &dwType, (LPBYTE)temp, &dwSize)) {
      continue;
    }
  ...
  for (i = 0; i < MAX_MRU_SIZE; i++) {
    dwSize = MAX_PATH;
    if (ERROR_SUCCESS != RegQueryValueEx(hKey, va("mru%d", i), NULL, &dwType,
                                         (LPBYTE)temp, &dwSize)) {
      continue;
    }
...
void rvRegistryOptions::SetWindowPlacement(const char* name, HWND hwnd) {
  ...
  out = va("%d %d %d %d %d %d %d %d %d %d", wp.flags, wp.ptMaxPosition.x,
           wp.ptMaxPosition.y, wp.ptMinPosition.x, wp.ptMinPosition.y,
           wp.rcNormalPosition.left, wp.rcNormalPosition.top,
           wp.rcNormalPosition.right, wp.rcNormalPosition.bottom, wp.showCmd);
...
bool rvRegistryOptions::GetWindowPlacement(const char* name, HWND hwnd) {
  ...
  sscanf(key->GetValue().c_str(), "%d %d %d %d %d %d %d %d %d %d", &wp.flags,
         &wp.ptMaxPosition.x, &wp.ptMaxPosition.y, &wp.ptMinPosition.x,
         &wp.ptMinPosition.y, &wp.rcNormalPosition.left,
         &wp.rcNormalPosition.top, &wp.rcNormalPosition.right,
         &wp.rcNormalPosition.bottom, &wp.showCmd);

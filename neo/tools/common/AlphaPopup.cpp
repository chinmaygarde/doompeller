  ...
  switch (msg) {
    case WM_LBUTTONDOWN: {
      ...
      SetWindowLong(hwnd, GWL_USERDATA,
                    MAKELONG(0x8000, (unsigned short)(255.0f * v)));
    ...
    case WM_LBUTTONUP:
      if (LOWORD(GetWindowLong(hwnd, GWL_USERDATA)) & 0x8000) {
        ...
        SendMessage(GetParent(hwnd), WM_COMMAND,
                    MAKELONG(GetWindowLong(hwnd, GWL_ID), 0), 0);
      }
      break;
    ...
    case WM_PAINT: {
      ...
      for (int i = 0; i < parts; i++) {
        ...
        HBRUSH brush =
            CreateSolidBrush(RGB((int)color, (int)color, (int)color));
...
INT_PTR CALLBACK AlphaSelectDlg_WndProc(HWND hwnd,
  ...
  switch (msg) {
    case WM_INITDIALOG: {
      ...
      gAlphaDlg = hwnd;
      gAlphaHook = SetWindowsHookEx(WH_GETMESSAGE, AlphaSelectDlg_GetMsgProc,
                                    NULL, GetCurrentThreadId());
      color = GetRValue(ColorButton_GetColor((HWND)lParam));

      // The lParam for the alpha select dialog is the window handle of the
      // button pressed
      SetWindowLong(hwnd, GWL_USERDATA, lParam);

      // Subclass the alpha
      SetWindowLong(GetDlgItem(hwnd, IDC_GUIED_ALPHASLIDER), GWL_USERDATA,
                    MAKELONG(0, color));

      // Numbers only on the edit box and start it with the current alpha value.
      NumberEdit_Attach(GetDlgItem(hwnd, IDC_GUIED_ALPHA));
      SetWindowText(GetDlgItem(hwnd, IDC_GUIED_ALPHA),
                    va("%.3f", ((float)color / 255.0f)));
      break;
    }
    ...
    case WM_COMMAND:
      switch (LOWORD(wParam)) {
        case IDC_GUIED_ALPHA: {
          ...
          SetWindowLong(GetDlgItem(hwnd, IDC_GUIED_ALPHASLIDER), GWL_USERDATA,
                        MAKELONG(0, (255.0f * value)));
          break;
        }

        case IDC_GUIED_ALPHASLIDER:
        case IDOK: {
          int color = (short)HIWORD(GetWindowLong(
              GetDlgItem(hwnd, IDC_GUIED_ALPHASLIDER), GWL_USERDATA));
          ColorButton_SetColor((HWND)GetWindowLong(hwnd, GWL_USERDATA),
...
void AlphaButton_OpenPopup(HWND button) {
  ...
  dlg = CreateDialogParam(win32.hInstance, MAKEINTRESOURCE(IDD_GUIED_ALPHA),
                          GetParent(button), AlphaSelectDlg_WndProc,
                          (LPARAM)button);

  SetWindowPos(dlg, NULL, rWindow.left, rWindow.bottom + 1, 0, 0,

...
CPropTreeItemCombo::CPropTreeItemCombo()
    : m_lComboData(0), m_nDropHeight(DROPDOWN_HEIGHT) {}
...
void CPropTreeItemCombo::DrawAttribute(CDC* pDC, const RECT& rc) {
  ...
  if (!IsWindow(m_hWnd)) {
    TRACE0(
        "CPropTreeItemCombo::DrawAttribute() - The window has not been "
        "created\n");
    return;
  }

  pDC->SelectObject(IsReadOnly() ? m_pProp->GetNormalFont()
                                 : m_pProp->GetBoldFont());
...
void CPropTreeItemCombo::OnMove() {
  if (IsWindow(m_hWnd) && IsWindowVisible())
    SetWindowPos(NULL, m_rc.left, m_rc.top, m_rc.Width() + 1, m_rc.Height(),
...
void CPropTreeItemCombo::OnActivate(int activateType, CPoint point) {
  ...
  SetWindowPos(NULL, m_rc.left, m_rc.top, m_rc.Width() + 1,
               m_rc.Height() + m_nDropHeight, SWP_NOZORDER | SWP_SHOWWINDOW);
  SetFocus();
...
BOOL CPropTreeItemCombo::CreateComboBox(DWORD dwStyle) {
  ...
  if (!Create(dwStyle, CRect(0, 0, 0, 0), m_pProp->GetCtrlParent(),
              GetCtrlID())) {
    TRACE0(
...
BOOL CPropTreeItemCombo::CreateComboBoxBool() {
  ...
  if (!Create(dwStyle, CRect(0, 0, 0, 0), m_pProp->GetCtrlParent(),
              GetCtrlID())) {
    TRACE0(
        "CPropTreeItemCombo::CreateComboBoxBool() - failed to create combo "
        "box\n");
    return FALSE;
  }

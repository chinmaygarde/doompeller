...
static void _DrawExpand(HDC hdc, LONG x, LONG y, BOOL bExpand, BOOL bFill) {
  ...
  oBrush = (HBRUSH)SelectObject(
      hdc, GetStockObject(bFill ? WHITE_BRUSH : NULL_BRUSH));
...
LONG CPropTreeItem::DrawItem(CDC* pDC, const RECT& rc, LONG x, LONG y) {
  ...
  m_rc.SetRect(m_pProp->GetOrigin().x + PROPTREEITEM_SPACE, m_loc.y, rc.right,
               m_loc.y + GetHeight() - 1);
  ...
  if (IsRootLevel())
    drc.SetRect(pt.x + PROPTREEITEM_EXPANDCOLUMN, pt.y, rc.right,
                pt.y + nTotal);
  else
  ...
  if (IsRootLevel()) {
    ...
    pDC->PatBlt(rc.left, drc.top, rc.right - rc.left + 1, drc.Height(),
                PATCOPY);
  ...
  if (GetChild()) {
    m_rcExpand.left =
        PROPTREEITEM_EXPANDCOLUMN / 2 - PROPTREEITEM_EXPANDBOXHALF;
  ...
  if (IsCheckBox()) {
    ir = m_rcCheckbox;
    ir.OffsetRect(0, -m_pProp->GetOrigin().y);
    pDC->DrawFrameControl(&ir, DFC_BUTTON,
                          DFCS_BUTTONCHECK | (IsChecked() ? DFCS_CHECKED : 0));
  }
  ...
  // draw horzontal sep
  _DotHLine(pDC->m_hDC, PROPTREEITEM_EXPANDCOLUMN, pt.y + nTotal - 1,
            rc.right - PROPTREEITEM_EXPANDCOLUMN + 1);
  ...
  if (GetChild() && IsExpanded()) {
    ...
    for (pNext = GetChild(); pNext; pNext = pNext->GetSibling()) {
      LONG nHeight =
          pNext->DrawItem(pDC, rc, x + (IsRootLevel() ? 0 : PNINDENT), y);
      nTotal += nHeight;
      y += nHeight;
    }
  }

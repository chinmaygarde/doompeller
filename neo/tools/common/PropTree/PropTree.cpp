...
static int CALLBACK FontFamilyProcFonts(const LOGFONT FAR* lplf,
...
BOOL CPropTree::Create(DWORD dwStyle,
                       const RECT& rect,
                       CWnd* pParentWnd,
                       UINT nID) {
  CWnd* pWnd = this;

  LPCTSTR pszCreateClass = AfxRegisterWndClass(CS_HREDRAW | CS_VREDRAW,
...
void CPropTree::InitGlobalResources() {
  ...
  if (!fUseSystemFont) {
    ...
    if (::EnumFontFamilies(dc.GetSafeHdc(), NULL, FontFamilyProcFonts, 0) ==
        0) {
...
void CPropTree::SetColumn(LONG nColumn) {
  ...
  if (rc.IsRectEmpty())
    ...
    nColumn = __min(__max(PROPTREEITEM_EXPANDCOLUMN, nColumn),
                    rc.Width() - PROPTREEITEM_EXPANDCOLUMN);
...
BOOL CPropTree::IsItemVisible(CPropTreeItem* pItem) {
  ...
  for (CPropTreeItem* pNext = m_pVisbleList; pNext;
       pNext = pNext->GetNextVisible()) {
    if (pNext == pItem)
      return TRUE;
  }
...
LONG CPropTree::HitTest(const POINT& pt) {
  ...
  if ((pItem = FindItem(pt)) != NULL) {
    if (!pItem->IsRootLevel() && pt.x >= m_Origin.x - PROPTREEITEM_COLRNG &&
...
BOOL CALLBACK CPropTree::EnumFindItem(CPropTree*,
                                      CPropTreeItem* pItem,
                                      LPARAM lParam) {
  ASSERT(pItem != NULL);
...
BOOL CALLBACK CPropTree::EnumSelectAll(CPropTree*,
                                       CPropTreeItem* pItem,
                                       LPARAM lParam) {
  if (!pItem)
    return FALSE;
...
BOOL CALLBACK CPropTree::EnumRefreshAll(CPropTree*,
                                        CPropTreeItem* pItem,
                                        LPARAM) {
  if (!pItem)
    return FALSE;

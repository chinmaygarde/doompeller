...
CPropTreeList::CPropTreeList()
    : m_pProp(NULL), m_BackBufferSize(0, 0), m_bColDrag(FALSE), m_nPrevCol(0) {}
...
BOOL CPropTreeList::Create(DWORD dwStyle,
  ...
  LPCTSTR pszCreateClass = AfxRegisterWndClass(
...
void CPropTreeList::OnPaint() {
  ...
  rc.DeflateRect(2, 2);
  memdc.PatBlt(rc.left, rc.top, rc.Width(), rc.Height(),
  ...
  // draw all items
  for (pItem = m_pProp->GetRootItem()->GetChild(); pItem;
  ...
  dc.GetClipBox(&rc);
  dc.BitBlt(rc.left, rc.top, rc.Width(), rc.Height(), &memdc, rc.left, rc.top,
            SRCCOPY);
  memdc.DeleteDC();
}
...
void CPropTreeList::OnLButtonDown(UINT, CPoint point) {
  ...
  switch (nHit) {
    case HTCOLUMN:
      ...
      pDC = GetDC();
      GetClientRect(rc);
      pDC->PatBlt(m_nPrevCol - PROPTREEITEM_COLRNG / 2, 0, PROPTREEITEM_COLRNG,
                  rc.bottom, PATINVERT);
      ReleaseDC(pDC);
      break;
    ...
    case HTEXPAND:
      if ((pItem = m_pProp->FindItem(point)) != NULL) {
        if (pItem->GetChild() &&
            !m_pProp->SendNotify(PTN_ITEMEXPANDING, pItem)) {
...
void CPropTreeList::OnMouseMove(UINT, CPoint point) {
  if (m_bColDrag) {
    ...
    pDC->PatBlt(m_nPrevCol - PROPTREEITEM_COLRNG / 2, 0, PROPTREEITEM_COLRNG,
                rc.bottom, PATINVERT);
    pDC->PatBlt(point.x - PROPTREEITEM_COLRNG / 2, 0, PROPTREEITEM_COLRNG,
                rc.bottom, PATINVERT);
    m_nPrevCol = point.x;
    ReleaseDC(pDC);
  }
  ...
  switch (nChar) {
    case VK_RETURN:
      if ((pItem = m_pProp->GetFocusedItem()) != NULL &&
          !pItem->IsRootLevel() && !pItem->IsReadOnly()) {

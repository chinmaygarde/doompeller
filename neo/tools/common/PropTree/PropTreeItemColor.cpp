...
void CPropTreeItemColor::DrawAttribute(CDC* pDC, const RECT& rc) {
  ...
  r = rc;
  r.left += r.Height();
  s.Format(_T("R = %d, G = %d, B = %d"), GetRValue(m_cColor),
           GetGValue(m_cColor), GetBValue(m_cColor));
...
void CPropTreeItemColor::OnActivate(int activateType, CPoint point) {
  ...
  SetWindowPos(NULL, r.left, r.top, r.Width() + 1, r.Height(),
...
void CPropTreeItemColor::OnPaint() {
  ...
  dc.DrawEdge(&m_rcButton, m_bButton ? BDR_SUNKENOUTER : BDR_RAISEDINNER,
              BF_RECT);
}

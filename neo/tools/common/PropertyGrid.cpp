...
bool rvPropertyGrid::Create(HWND parent, int id, int style) {
  ...
  mWindow =
      CreateWindowEx(0, "LISTBOX", "",
                     WS_VSCROLL | WS_CHILD | WS_VISIBLE | LBS_OWNERDRAWFIXED |
                         LBS_NOINTEGRALHEIGHT | LBS_NOTIFY,
                     0, 0, 0, 0, parent, (HMENU)id, win32.hInstance, 0);
  ...
  mEdit = CreateWindowEx(0, "RichEdit20A", "", WS_CHILD, 0, 0, 0, 0, mWindow,
                         (HMENU)999, win32.hInstance, NULL);
...
void rvPropertyGrid::StartEdit(int item, bool label) {
  ...
  MoveWindow(mEdit, rItem.left, rItem.top + 2, rItem.right - rItem.left,
...
void rvPropertyGrid::FinishEdit(void) {
  ...
  update = false;
  item =
      (rvPropertyGridItem*)SendMessage(mWindow, LB_GETITEMDATA, mEditItem, 0);
  assert(item);
  ...
  if (!mEditLabel && item->mValue.Cmp(value)) {
    ...
    // The item may have been destroyed and recreated in the notify call so get
    // it again
    item =
        (rvPropertyGridItem*)SendMessage(mWindow, LB_GETITEMDATA, mEditItem, 0);
    if (item) {
...
int rvPropertyGrid::AddItem(const char* name,
  ...
  insert = SendMessage(mWindow, LB_GETCOUNT, 0, 0) -
           ((mStyle & PGS_ALLOWINSERT) ? 1 : 0);
      ...
      switch (wParam) {
        case VK_RETURN:
          if (grid->mSelectedItem >= 0) {
            grid->StartEdit(
                grid->mSelectedItem,
                (*grid->GetItemName(grid->mSelectedItem)) ? false : true);
          }
          break;
      }
      break;
...
int rvPropertyGrid::HandleDrawItem(WPARAM wParam, LPARAM lParam) {
  ...
  DrawText(dis->hDC, item->mName, item->mName.Length(), &rText,
           DT_LEFT | DT_VCENTER | DT_SINGLELINE);

  rText.left = dis->rcItem.left + mSplitter + 2;
  rText.right = dis->rcItem.right;
  DrawText(dis->hDC, item->mValue, item->mValue.Length(), &rText,
           DT_LEFT | DT_VCENTER | DT_SINGLELINE);

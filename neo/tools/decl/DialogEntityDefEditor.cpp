...
DialogEntityDefEditor::DialogEntityDefEditor(CWnd* pParent /*=NULL*/)
    : CDialog(DialogEntityDefEditor::IDD, pParent), decl(NULL), firstLine(0) {}
...
bool DialogEntityDefEditor::TestDecl(const idStr& declText) {
  ...
  if (indent < 0) {
    MessageBox("Missing opening brace!",
               va("Error saving %s", decl->GetFileName()),
               MB_OK | MB_ICONERROR);
    return false;
  }
  if (indent > 0) {
    MessageBox("Missing closing brace!",
               va("Error saving %s", decl->GetFileName()),
               MB_OK | MB_ICONERROR);
    return false;
  }
...
void DialogEntityDefEditor::LoadDecl(idDeclEntityDef* decl) {
  ...
  */
  ...
  SetWindowText(va("EntityDef Editor (%s, line %d)", decl->GetFileName(),
                   decl->GetLineNum()));
...
=================
*/
void DialogEntityDefEditor::PopulateLists(const char* declText,
  ...
  // Get the parent, and remove the 'inherit' key so it doesn't show up in the
  // list We currently don't support multiple inheritence properly, but nothing
  // uses it anyway
  idStr inherit;
  ...
  for (unsigned int i = 0; i < numPairs; i++) {
    ...
    if (keyVal) {
      keyValsList.AddPropItem(new CPropertyItem(
          keyVal->GetKey().c_str(), keyVal->GetValue().c_str(), PIT_EDIT, ""));
    }
  }
      ...
      for (unsigned int i = 0; i < numPairs; i++) {
        ...
        if (keyVal) {
          if (spawnclass.IsEmpty() && keyVal->GetKey() == "spawnclass") {
            ...
            key = "*" + key;
            keyValsList.AddPropItem(new CPropertyItem(
                key, keyVal->GetValue().c_str(), PIT_EDIT, ""));
          }
        }
      }
    }
...
BOOL DialogEntityDefEditor::OnInitDialog() {
  ...
  m_hAccel = ::LoadAccelerators(AfxGetResourceHandle(),
                                MAKEINTRESOURCE(IDR_ACCELERATOR_DECLEDITOR));
...
void DialogEntityDefEditor::BuildDeclText(idStr& declText) {
  ...
  for (int i = 0; i < keyValsList.GetCount(); i++) {
    ...
    if (pItem) {
      ...
      declText +=
          "\"" + pItem->m_propName + "\"\t\t\t\"" + pItem->m_curValue + "\"\r";
    }
  }
  declText += "}\r";
  ...
  if (decl) {
    ...
    if (decl->SourceFileChanged()) {
      if (MessageBox(va("Declaration file %s has been modified outside of the "
                        ...
                        decl->GetFileName()),
                     va("Warning saving: %s", decl->GetFileName()),
...
void DialogEntityDefEditor::OnBnClickedCancel() {
  if (okButton.IsWindowEnabled()) {
    if (MessageBox("Cancel changes?", "Cancel", MB_YESNO | MB_ICONQUESTION) !=
        IDYES) {
      return;
    }
  }

...
toolTip_t DialogDeclNew::toolTips[] = {
    ...
    {IDC_DECLNEW_EDIT_NEW_FILE,
     "enter the name of the file to add the declaration to"},
    {IDC_DECLNEW_BUTTON_NEW_FILE,
     "select existing file to add the declaration to"},
...
DialogDeclNew::DialogDeclNew(CWnd* pParent /*=NULL*/)
    : CDialog(DialogDeclNew::IDD, pParent), declTree(NULL), newDecl(NULL) {}
...
void DialogDeclNew::OnBnClickedOk() {
  ...
  if (!declTree) {
    MessageBox("No declaration tree available.", errorTitle,
               MB_OK | MB_ICONERROR);
    return;
  }

  if (GetSafeComboBoxSelection(&typeList, typeName, -1) == -1) {
    MessageBox("No declaration type selected.", errorTitle,
               MB_OK | MB_ICONERROR);
    return;
  }
  ...
  if (declName.GetLength() == 0) {
    MessageBox("No declaration name specified.", errorTitle,
               MB_OK | MB_ICONERROR);
    return;
  }

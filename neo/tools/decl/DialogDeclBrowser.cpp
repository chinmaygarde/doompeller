...
toolTip_t DialogDeclBrowser::toolTips[] = {
    ...
    {IDC_DECLBROWSER_EDIT_SEARCH_NAMES,
     "search for declarations with matching name, use meta characters: *, ? "
     "and [abc...]"},
    {IDC_DECLBROWSER_EDIT_SEARCH_TEXT,
     "search for declarations containing text"},
    {IDC_DECLBROWSER_BUTTON_FIND,
     "find declarations matching the search strings"},
...
void DialogDeclBrowser::AddDeclTypeToTree(declType_t type,
  ...
  for (i = 0; i < decls.Num(); i++) {
    ...
    tree.AddPathToTree(declName, GetIdFromTypeAndIndex(type, decls[i]->Index()),
                       stack);
  }
}
...
void DialogDeclBrowser::AddScriptsToTree(CPathTreeCtrl& tree) {
  ...
  for (i = 0; i < files->GetNumFiles(); i++) {
    ...
    tree.AddPathToTree(scriptName, GetIdFromTypeAndIndex(DECLTYPE_SCRIPT, i),
                       stack);
  }
...
void DialogDeclBrowser::AddGUIsToTree(CPathTreeCtrl& tree) {
  ...
  for (i = 0; i < files->GetNumFiles(); i++) {
    ...
    tree.AddPathToTree(scriptName, GetIdFromTypeAndIndex(DECLTYPE_GUI, i),
                       stack);
  }
...
void DialogDeclBrowser::InitBaseDeclTree(void) {
  ...
  for (i = 0; i < declManager->GetNumDeclTypes(); i++) {
    AddDeclTypeToTree((declType_t)i,
                      declManager->GetDeclNameFromType((declType_t)i),
                      baseDeclTree);
  }
...
void DialogDeclBrowser::GetDeclName(HTREEITEM item,
                                    idStr& typeName,
                                    idStr& declName) const {
  HTREEITEM parent;
  idStr itemName;

  declName.Clear();
  for (parent = declTree.GetParentItem(item); parent;
       parent = declTree.GetParentItem(parent)) {
...
void DialogDeclBrowser::EditSelected(void) const {
  ...
  switch (type) {
    ...
    case DECLTYPE_GUI: {
      ...
      scriptEditor->OpenFile(typeName + "/" + declName +
                             ((type == DECLTYPE_SCRIPT) ? ".script" : ".gui"));
  ...
  if (findTextString.Length()) {
    ...
    if (type == DECLTYPE_SCRIPT || type == DECLTYPE_GUI) {
      ...
      if (fileSystem->ReadFile(
              idStr(name) + ((type == DECLTYPE_SCRIPT) ? ".script" : ".gui"),
              &buffer) == -1) {
        return false;
      }
      text = (char*)buffer;
      ...
      char* declText =
          (char*)_alloca((decl->GetTextLength() + 1) * sizeof(char));
...
BOOL DialogDeclBrowser::OnInitDialog() {
  ...
  statusBar.CreateEx(SBARS_SIZEGRIP, WS_CHILD | WS_VISIBLE | CBRS_BOTTOM,
                     initialRect, this, AFX_IDW_STATUS_BAR);
  ...
  numListedDecls =
      baseDeclTree.SearchTree(DeclBrowserCompareDecl, this, declTree);
...
void DialogDeclBrowser::OnTreeSelChanged(NMHDR* pNMHDR, LRESULT* pResult) {
  ...
  if (decl) {
    statusBar.SetWindowText(va("%d decls listed    -    %s, line: %d",
                               numListedDecls, decl->GetFileName(),
                               decl->GetLineNum()));
    findNameEdit.SetWindowText(
        va("%s/%s", declManager->GetDeclNameFromType(decl->GetType()),
           decl->GetName()));
  } else {
    ...
    GetDeclName(item, typeName, declName);
    findNameEdit.SetWindowText(
        va("%s/%s*", typeName.c_str(), declName.c_str()));
    statusBar.SetWindowText(va("%d decls listed", numListedDecls));
  }

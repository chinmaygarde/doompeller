...
void DialogDeclEditor::UpdateStatusBar(void) {
  ...
  if (decl) {
    ...
    statusBar.SetWindowText(va("Line: %d, Column: %d, Character: %d",
                               decl->GetLineNum() + line, column, character));
  }
}
...
void DialogDeclEditor::LoadDecl(idDecl* decl) {
  ...
  switch (decl->GetType()) {
    ...
    case DECL_MODELDEF:
      ...
      break;
    default:
      declEdit.LoadKeyWordsFromFile(va(
          "editors/%s.def", declManager->GetDeclNameFromType(decl->GetType())));
      break;
  }
  ...
  SetWindowText(va("Declaration Editor (%s, line %d)", decl->GetFileName(),
                   decl->GetLineNum()));
...
void DialogDeclEditor::OnEditFindNext() {
  if (declEdit.FindNext(findStr, matchCase, matchWholeWords, searchForward)) {
    ...
    AfxMessageBox("The specified text was not found.",
...
LRESULT DialogDeclEditor::OnFindDialogMessage(WPARAM wParam, LPARAM lParam) {
  ...
  if (findDlg->ReplaceAll()) {
    ...
    int numReplaces =
        declEdit.ReplaceAll(findStr, replaceStr, matchCase, matchWholeWords);
    if (numReplaces == 0) {
      AfxMessageBox("The specified text was not found.",
                    MB_OK | MB_ICONINFORMATION, 0);
    } else {
      AfxMessageBox(va("Replaced %d occurances.", numReplaces),
                    MB_OK | MB_ICONINFORMATION, 0);
    }
  }

...
void MaterialDoc::SetAttribute(int stage,
  ...
  if (orig.Icmp(value)) {
    ...
    if (addUndo) {
      ...
      AttributeMaterialModifierString* mod =
          new AttributeMaterialModifierString(manager, name, stage, attribName,
                                              value, orig);
...
void MaterialDoc::SetAttributeBool(int stage,
  ...
  if (orig != value) {
    ...
    if (addUndo) {
      // Create a new Modifier for this change so we can undo and redo later
      AttributeMaterialModifierBool* mod = new AttributeMaterialModifierBool(
          manager, name, stage, attribName, value, orig);
      manager->AddMaterialUndoModifier(mod);
    }
...
void MaterialDoc::SetMaterialName(const char* materialName, bool addUndo) {
  ...
  if (addUndo) {
    RenameMaterialModifier* mod =
        new RenameMaterialModifier(manager, name, oldName);
    manager->AddMaterialUndoModifier(mod);
  }
...
void MaterialDoc::ApplySourceModify(idStr& text) {
  if (sourceModify) {
    ...
    src.SetFlags(
        LEXFL_NOSTRINGCONCAT |  // multiple strings seperated by whitespaces are
                                // not concatenated
...
void MaterialDoc::AddStage(int stageType, const char* stageName, bool addUndo) {
  ...
  if (addUndo) {
    StageInsertModifier* mod =
        new StageInsertModifier(manager, name, index, stageType, stageName);
    manager->AddMaterialUndoModifier(mod);
  }
...
void MaterialDoc::InsertStage(int stage,
  ...
  if (addUndo) {
    StageInsertModifier* mod =
        new StageInsertModifier(manager, name, stage, stageType, stageName);
    manager->AddMaterialUndoModifier(mod);
  }
...
void MaterialDoc::RemoveStage(int stage, bool addUndo) {
  ...
  if (addUndo) {
    ...
    StageDeleteModifier* mod = new StageDeleteModifier(
        manager, name, stage, editMaterial.stages[stage]->stageData);
    manager->AddMaterialUndoModifier(mod);
  }
...
void MaterialDoc::MoveStage(int from, int to, bool addUndo) {
  ...
  if (addUndo) {
    StageMoveModifier* mod =
        new StageMoveModifier(manager, name, origFrom, origTo);
    manager->AddMaterialUndoModifier(mod);
  }
...
void MaterialDoc::Save() {
  ...
  if (renderMaterial->Save()) {
    ...
    MessageBox(GetMaterialEditorWindow(),
               va("Unable to save '%s'. It may be read-only", name.c_str()),
...
void MaterialDoc::ParseMaterial(idLexer* src) {
  ...
  while (1) {
    ...
    if (ParseMaterialDef(&token, src, MaterialDefManager::MATERIAL_DEF_MATERIAL,
...
void MaterialDoc::ParseStage(idLexer* src) {
  ...
  while (1) {
    ...
    if (ParseMaterialDef(&token, src, MaterialDefManager::MATERIAL_DEF_STAGE,
                         &newStage->stageData)) {
      continue;
    }

...
MaterialDefList
    MaterialDefManager::materialDefs[MaterialDefManager::MATERIAL_DEF_NUM];
...
void MaterialDefManager::InitializeMaterialDefLists() {
  ...
  if (length == -1) {
    common->Error("Couldn't load material editor definition: %s",
                  MATERIAL_DEF_FILE);
    return;
  }
  ...
  InitializeMaterialDefList(&src, "materialprops",
                            &materialDefs[MATERIAL_DEF_MATERIAL]);
  InitializeMaterialDefList(&src, "stageprops",
                            &materialDefs[MATERIAL_DEF_STAGE]);
  InitializeMaterialDefList(&src, "specialmapstageprops",
                            &materialDefs[MATERIAL_DEF_SPECIAL_STAGE]);

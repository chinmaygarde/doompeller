 ...
 */
void MaterialTreeView::InitializeMaterialList(bool includeFile,
                                              const char* filename) {
 ...
 * @param filename The file to load or NULL to load all files.
 */
void MaterialTreeView::BuildMaterialList(bool includeFile,
                                         const char* filename) {
  CTreeCtrl& tree = GetTreeCtrl();
  ...
  if (count > 0) {
    for (int i = 0; i < count; i++) {
      ...
      if (includeFile) {
        filename.StripPath();
        temp = idStr(mat->GetFileName()) + "/" + idStr(mat->GetName()) + "|" +
               filename;
      } else {
        temp = mat->GetName();
      }
...
void MaterialTreeView::MV_OnMaterialChange(MaterialDoc* pMaterial) {
  ...
  tree.SetItemImage(*materialItem, IMAGE_MATERIAL_MOD_APPLY,
...
void MaterialTreeView::MV_OnMaterialSaved(MaterialDoc* pMaterial) {
  ...
  if (treeWithFile) {
    if (!materialDocManager->IsFileModified(
            pMaterial->renderMaterial->GetFileName())) {
      HTREEITEM* fileItem = NULL;
      idStr file = pMaterial->renderMaterial->GetFileName();
...
bool MaterialTreeView::FindNextMaterial(MaterialSearchData_t* searchData) {
  ...
  while ((search = GetNextSeachItem(search, (searchData->searchScope == 0))) !=
         NULL) {
...
void MaterialTreeView::DeleteFolder(HTREEITEM item, bool addUndo) {
  ...
  for (int i = 0; i < materialsToDelete.Num(); i++) {
    ...
    const idMaterial* material =
        declManager->FindMaterial(materialsToDelete[i].materialName);

    MaterialDoc* pMaterial = NULL;
    pMaterial = materialDocManager->CreateMaterialDoc(
  ...
  if (addUndo) {
    DeleteMaterialFolderModifier* mod = new DeleteMaterialFolderModifier(
        materialDocManager, tree.GetItemText(item), this,
        tree.GetParentItem(item), &affectedMaterials);
...
int MaterialTreeView::OnCreate(LPCREATESTRUCT lpCreateStruct) {
  lpCreateStruct->style |= TVS_HASLINES | TVS_HASBUTTONS | TVS_LINESATROOT |
                           TVS_EDITLABELS | TVS_SHOWSELALWAYS | TVS_INFOTIP;
...
void MaterialTreeView::OnTvnEndlabeledit(NMHDR* pNMHDR, LRESULT* pResult) {
  ...
  *pResult = 0;
  ...
  if (pTVDispInfo->item.pszText) {
    ...
    newLabel.ToLower();
    strncpy(pTVDispInfo->item.pszText, newLabel.c_str(),
            pTVDispInfo->item.cchTextMax);
    ...
    if (type == TYPE_MATERIAL) {
      ...
      if (declManager->FindMaterial(material, false)) {
        ...
        MessageBox(
            "Unable to rename material because it conflicts with another "
            "material",
            "Error");
      } else {
      ...
      RenameMaterialFolderModifier* mod = new RenameMaterialFolderModifier(
          materialDocManager, pTVDispInfo->item.pszText, this,
          pTVDispInfo->item.hItem, tree.GetItemText(pTVDispInfo->item.hItem));
      materialDocManager->AddMaterialUndoModifier(mod);
      ...
      if (dragItemParent != item) {
        ...
        if (!dragFile.Icmp(filename)) {
          materialDocManager->CopyMaterial(
              materialDocManager->GetCurrentMaterialDoc(), true);
        } else {
          materialDocManager->CopyMaterial(
              materialDocManager->GetCurrentMaterialDoc(), false);
        }
...
void MaterialTreeView::OnApplyMaterial() {
  materialDocManager->ApplyMaterial(
      materialDocManager->GetCurrentMaterialDoc());
}
...
void MaterialTreeView::OnAddFolder() {
  ...
  AddMaterialFolderModifier* mod = new AddMaterialFolderModifier(
      materialDocManager, newFolder, this, newItem, item);
  materialDocManager->AddMaterialUndoModifier(mod);
}
...
void MaterialTreeView::OnDeleteMaterial() {
  ...
  if (itemType == TYPE_MATERIAL_FOLDER) {
    int result = MessageBox("Are you sure you want to delete this folder?",
                            "Delete?", MB_ICONQUESTION | MB_YESNO);
    if (result == IDYES) {
      DeleteFolder(item);
    }
  } else if (itemType == TYPE_MATERIAL) {
    int result = MessageBox("Are you sure you want to delete this material?",
                            "Delete?", MB_ICONQUESTION | MB_YESNO);
    if (result == IDYES) {
  ...
  if (itemType == TYPE_MATERIAL || itemType == TYPE_FILE ||
    ...
    if (materialDocManager->IsFileModified(filename)) {
      int result = MessageBox(
          "This file has been modified. Are you sure you want to reload this "
          "file?",
...
void MaterialTreeView::RenameMaterial(HTREEITEM item,
  ...
  // if(!pMaterial) {
  pMaterial =
      materialDocManager->CreateMaterialDoc(const_cast<idMaterial*>(material));
  //}
      ...
      if (index >= 0) {
        ...
        if (newItem == NULL) {
          ...
          if (!afterFile || thisisfile) {
            ...
            treeMedia.SetItemImage(newItem, IMAGE_MATERIAL_FOLDER,
...
void MaterialTreeView::PopupMenu(CPoint* pt) {
  ...
  if (pDoc && pDoc->applyWaiting) {
    pPopupMenu->EnableMenuItem(ID_POPUP_APPLYMATERIAL,
  ...
  if (GetFileName(item, filename)) {
    if (materialDocManager->DoesFileNeedApply(filename.c_str()))
      ...
      pPopupMenu->EnableMenuItem(ID_POPUP_APPLYFILE,
                                 MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
  } else {
    pPopupMenu->EnableMenuItem(ID_POPUP_APPLYFILE,
                               MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
  ...
  if (materialDocManager->DoesAnyNeedApply()) {
    ...
    pPopupMenu->EnableMenuItem(ID_POPUP_APPLYALL,
                               MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
  }

  // Save Material
  if (pDoc && pDoc->modified) {
    pPopupMenu->EnableMenuItem(ID_POPUP_SAVEMATERIAL,
                               MF_BYCOMMAND | MF_ENABLED);
  } else {
    pPopupMenu->EnableMenuItem(ID_POPUP_SAVEMATERIAL,
                               MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
  ...
  if (GetFileName(item, filename)) {
    if (materialDocManager->IsFileModified(filename.c_str()))
      ...
      pPopupMenu->EnableMenuItem(ID_POPUP_SAVEFILE,
                                 MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
  } else {
    pPopupMenu->EnableMenuItem(ID_POPUP_SAVEFILE,
                               MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
  ...
  if (materialDocManager->IsAnyModified()) {
    ...
    pPopupMenu->EnableMenuItem(ID_POPUP_SAVEALL,
                               MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
  }

  if (itemType == TYPE_MATERIAL || itemType == TYPE_MATERIAL_FOLDER) {
    pPopupMenu->EnableMenuItem(ID_POPUP_RENAMEMATERIAL,
                               MF_BYCOMMAND | MF_ENABLED);
    pPopupMenu->EnableMenuItem(ID_POPUP_DELETEMATERIAL,
                               MF_BYCOMMAND | MF_ENABLED);
  } else {
    pPopupMenu->EnableMenuItem(ID_POPUP_RENAMEMATERIAL,
                               MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
  ...
  if (itemType == TYPE_FILE || itemType == TYPE_MATERIAL_FOLDER ||
    ...
    pPopupMenu->EnableMenuItem(ID_POPUP_ADDMATERIAL,
                               MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
    pPopupMenu->EnableMenuItem(ID_POPUP_ADDFOLDER,
                               MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
  }
  ...
  if (itemType == TYPE_MATERIAL) {
    ...
    pPopupMenu->EnableMenuItem(ID_POPUP_CUT,
                               MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
    pPopupMenu->EnableMenuItem(ID_POPUP_COPY,
                               MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
  }
  ...
  if ((itemType == TYPE_MATERIAL || itemType == TYPE_FILE ||
    ...
    pPopupMenu->EnableMenuItem(ID_POPUP_PASTE,
                               MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
  }
  ...
  if (itemType == TYPE_MATERIAL || itemType == TYPE_FILE ||
    ...
    pPopupMenu->EnableMenuItem(ID_POPUP_RELOADFILE,
                               MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
  }

  pPopupMenu->TrackPopupMenu(TPM_LEFTALIGN | TPM_RIGHTBUTTON, pt->x, pt->y,
                             &GetTreeCtrl());
}

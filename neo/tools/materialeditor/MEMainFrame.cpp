...
BOOL MEMainFrame::OnCreateClient(LPCREATESTRUCT lpcs,
  ...
  m_tabs.Create(TCS_BOTTOM | TCS_FLATBUTTONS | WS_CHILD | WS_VISIBLE,
                CRect(0, 0, 0, 0), this, TAB_CONTROL);
  ...
  m_editSplitter.CreateStatic(&m_splitterWnd, 1, 2,
                              WS_CHILD | WS_VISIBLE | WS_BORDER,
                              m_splitterWnd.IdFromRowCol(0, 0));

  if (!m_editSplitter.CreateView(0, 0, RUNTIME_CLASS(MaterialTreeView),
                                 CSize(300, 200), pContext)) {
    TRACE0("Failed to create material list pane\n");
    return FALSE;
  }

  if (!m_editSplitter.CreateView(0, 1, RUNTIME_CLASS(MaterialEditView),
                                 CSize(200, 200), pContext)) {
    TRACE0("Failed to create stage property pane\n");
    return FALSE;
  }

  m_previewSplitter.CreateStatic(&m_splitterWnd, 1, 2,
                                 WS_CHILD | WS_VISIBLE | WS_BORDER,
                                 m_splitterWnd.IdFromRowCol(1, 0));

  if (!m_previewSplitter.CreateView(0, 0,
                                    RUNTIME_CLASS(MaterialPreviewPropView),
                                    CSize(300, 200), pContext)) {
    TRACE0("Failed to create preview property pane\n");
    return FALSE;
  }

  if (!m_previewSplitter.CreateView(0, 1, RUNTIME_CLASS(MaterialPreviewView),
  ...
  m_previewPropertyView =
      (MaterialPreviewPropView*)m_previewSplitter.GetPane(0, 0);
...
void MEMainFrame::OnSize(UINT nType, int cx, int cy) {
  ...
  m_splitterWnd.MoveWindow(
      0, toolbarRect.Height(), cx,
      cy - statusRect.Height() - toolbarRect.Height() - tabHeight);

  m_tabs.MoveWindow(0, cy - statusRect.Height() - tabHeight, cx, tabHeight);

  m_consoleView->MoveWindow(
      0, toolbarRect.Height(), cx,
      cy - statusRect.Height() - toolbarRect.Height() - tabHeight);
}
...
void MEMainFrame::OnApplyFileUpdate(CCmdUI* pCmdUI) {
  ...
  if (pDoc && materialDocManager.DoesFileNeedApply(
                  pDoc->renderMaterial->GetFileName())) {
    pCmdUI->Enable(TRUE);
  } else {
    pCmdUI->Enable(FALSE);
  }
...
void MEMainFrame::FindNext(MaterialSearchData_t* search) {
  ...
  if (!m_materialTreeView->FindNextMaterial(&searchData)) {
    MessageBox(va("Unable to find '%s'.", searchData.searchText.c_str()),
               "Find");
  }

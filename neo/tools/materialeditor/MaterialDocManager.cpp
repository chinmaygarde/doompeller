...
void MaterialDocManager::SetSelectedMaterial(idMaterial* material) {
  ...
  if (material) {
    if (currentMaterial) {
      if (strcmp(material->GetName(),
                 currentMaterial->renderMaterial->GetName())) {
        change = true;
      }
    } else {
      change = true;
...
void MaterialDocManager::AddMaterial(const char* name,
  ...
  idMaterial* rendMat =
      (idMaterial*)declManager->CreateNewDecl(DECL_MATERIAL, name, filename);
...
void MaterialDocManager::RedoAddMaterial(const char* name, bool clearData) {
  ...
  idMaterial* rendMat =
      const_cast<idMaterial*>(declManager->FindMaterial(name, false));
...
void MaterialDocManager::DeleteMaterial(MaterialDoc* material, bool addUndo) {
  ...
  if (addUndo) {
    DeleteMaterialModifier* mod =
        new DeleteMaterialModifier(this, material->name);
    AddMaterialUndoModifier(mod);
  }
...
MaterialDoc* MaterialDocManager::CreateMaterialDoc(idMaterial* material) {
  ...
  if (currentMaterial && material &&
      !currentMaterial->name.Icmp(material->GetName())) {
...
void MaterialDocManager::PasteMaterial(const char* name, const char* filename) {
  ...
  if (cutMaterial) {
    ...
    MoveMaterialModifier* mod =
        new MoveMaterialModifier(this, name, filename, copyMaterial);
    AddMaterialUndoModifier(mod);
...
bool MaterialDocManager::FindMaterial(const char* name,
  ...
  const idMaterial* material = static_cast<const idMaterial*>(
      declManager->FindDeclWithoutParsing(DECL_MATERIAL, name, false));
  ...
  for (int i = 0; i < c; i++) {
    ...
    switch (notifyType) {
      ...
      case MATERIAL_ADD_STAGE:
        materialViews[i]->MV_OnMaterialStageAdd(materialDoc,
                                                va_arg(argptr, int));
        break;
      case MATERIAL_DELETE_STAGE:
        materialViews[i]->MV_OnMaterialStageDelete(materialDoc,
                                                   va_arg(argptr, int));
        break;
      ...
      case MATERIAL_ATTRIBUTE_CHANGE: {
        ...
        materialViews[i]->MV_OnMaterialAttributeChanged(materialDoc, stage,
                                                        attribName);
      } break;

...
int MaterialPreviewView::OnCreate(LPCREATESTRUCT lpCreateStruct) {
  ...
  renderWindow.SetWindowPos(NULL, 0, 0, rc.Width(), rc.Height(),
                            SWP_SHOWWINDOW);
...
void idGLDrawableView::ResetView(void) {
  ...
  worldModel =
      renderModelManager->FindModel("models/materialeditor/cube128.ase");
  ...
  if (handleMove) {
    ...
    if (button == MK_LBUTTON) {
      ...
      if (doZoom) {
        ...
        viewRotation.pitch =
            idMath::ClampFloat(-89.9f, 89.9f, viewRotation.pitch);
      }
...
void idGLDrawableView::addLight(void) {
  ...
  viewLight.shader =
      declManager->FindMaterial("lights/defaultPointLight", false);
...
void idGLDrawableView::UpdateModel(void) {
  switch (objectId) {
    ...
    case 1:
      worldModel =
          renderModelManager->FindModel("models/materialeditor/box128x64.ase");
      break;
    case 2:
      worldModel =
          renderModelManager->FindModel("models/materialeditor/box128x32.ase");
      break;
    case 3:
      worldModel =
          renderModelManager->FindModel("models/materialeditor/box64x128.ase");
      break;
    case 4:
      worldModel =
          renderModelManager->FindModel("models/materialeditor/box32x128.ase");
      break;
    case 5:
      worldModel =
          renderModelManager->FindModel("models/materialeditor/cylinder_v.ase");
      break;
    case 6:
      worldModel =
          renderModelManager->FindModel("models/materialeditor/cylinder_h.ase");
      break;
    case 7:
      worldModel =
          renderModelManager->FindModel("models/materialeditor/sphere64.ase");
      break;
    case -1:
...
void idGLDrawableView::drawLights(renderView_t* refdef) {
  ...
  for (i = 0; i < viewLights.Num(); i++) {
    ...
    session->rw->DrawText(va("%d", i + 1),
                          vLight->renderLight.origin + idVec3(0, 0, 5), 0.25f,
                          idVec4(1, 1, 0, 1), refdef->viewaxis, 1, 0, true);
  }
}
...
void idGLDrawableView::setMedia(const char* name) {
  ...
  if (material->GetStage(0)->texture.image) {
    ratio = (float)((float)material->GetImageWidth() /
                    (float)material->GetImageHeight());
  }

 ...
 */
MaterialEditView::MaterialEditView() : CFormView(MaterialEditView::IDD) {
  initHack = false;
  sourceInit = false;
  sourceChanged = false;
}
...
void MaterialEditView::ApplyMaterialSource() {
  ...
  MaterialDoc* material =
      materialDocManager->CreateMaterialDoc(currentMaterialName);
...
int MaterialEditView::OnCreate(LPCREATESTRUCT lpCreateStruct) {
  ...
  m_nameEdit.Create(
      WS_CHILD | WS_VISIBLE | WS_TABSTOP | WS_BORDER | ES_READONLY,
      CRect(0, 0, 0, 0), this, NAME_CONTROL);

  m_editSplitter.CreateStatic(this, 1, 2);

  if (!m_editSplitter.CreateView(0, 0, RUNTIME_CLASS(StageView),
                                 CSize(200, 200), NULL)) {
    TRACE0("Failed to create stage property pane\n");
    return -1;
  }

  if (!m_editSplitter.CreateView(0, 1, RUNTIME_CLASS(MaterialPropTreeView),
                                 CSize(500, 200), NULL)) {
  ...
  m_tabs.Create(TCS_BOTTOM | TCS_FLATBUTTONS | WS_CHILD | WS_VISIBLE,
                CRect(0, 0, 0, 0), this, EDIT_TAB_CONTROL);
...
/**
 * Called when the user changes the properties/text tab selection. This methods

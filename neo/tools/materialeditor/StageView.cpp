 ...
 */
void StageView::MV_OnMaterialStageMove(MaterialDoc* pMaterial,
                                       int from,
                                       int to) {
  if (!internalChange) {
    from++;
 ...
 */
void StageView::MV_OnMaterialAttributeChanged(MaterialDoc* pMaterial,
                                              int stage,
                                              const char* attribName) {
  // Refresh this stage list if a material name has changed
  if (!internalChange && currentMaterial == pMaterial && stage >= 0 &&
      attribName && !strcmp(attribName, "name")) {
    CListCtrl& list = GetListCtrl();
    list.SetItemText(stage + 1, 0,
                     currentMaterial->GetAttribute(stage, attribName));
  }
}

/**
 * Returns true if the current state of the stage view will allow a copy
 * operation
 */
...
/**
 * Returns true if the current state of the stage view will allow a paste
 * operation
 */
...
/**
 * Returns true if the current state of the stage view will allow a delete
 * operation
 */
...
/**
 * Returns true if the current state of the stage view will allow a rename
 * operation
 */
bool StageView::CanRename() {
  ...
  if (nItem > 0) {
    ...
    if (nItem > 0 && material->GetAttributeInt(nItem - 1, "stagetype") ==
...
void StageView::RefreshStageList() {
  ...
  if (currentMaterial) {
    ...
    if (selectedItem < 0) {
      // Select the material
      list.SetItemState(0, LVIS_SELECTED | LVIS_FOCUSED,
                        LVIS_SELECTED | LVIS_FOCUSED);
    } else {
      list.SetItemState(selectedItem, LVIS_SELECTED | LVIS_FOCUSED,
                        LVIS_SELECTED | LVIS_FOCUSED);
    }
  }
}
...
int StageView::OnCreate(LPCREATESTRUCT lpCreateStruct) {
  ...
  SetToggleIcons(MAKEINTRESOURCE(IDI_ME_DISABLED_ICON),
                 MAKEINTRESOURCE(IDI_ME_ON_ICON),
                 MAKEINTRESOURCE(IDI_ME_OFF_ICON));

  return 0;
}

/**
 * Called when the user changes the selection in the list box. This method will
 * notify the property view of the change so that it can display the appropriate
 * properties.
 */
void StageView::OnLvnItemchanged(NMHDR* pNMHDR, LRESULT* pResult) {
  ...
  if (!bDragging) {
    ...
    if (pNMLV->uChanged && LVIF_STATE && pNMLV->uNewState & LVIS_SELECTED) {
      ...
      if (pNMLV->iItem >= 0) {
        if (pNMLV->iItem == 0)
          ...
          int stageType =
              currentMaterial->GetAttributeInt(pNMLV->iItem - 1, "stagetype");
          switch (stageType) {
            case MaterialDoc::STAGE_TYPE_NORMAL:
              type = MaterialDefManager::MATERIAL_DEF_STAGE;
              break;
    ...
    if (pNMLV->uChanged && LVIF_STATE && pNMLV->uOldState & LVIS_SELECTED &&
...
void StageView::OnDeleteStage() {
  ...
  if (pos) {
    nItem = list.GetNextSelectedItem(pos);
    if (nItem > 0) {
      int result = MessageBox("Are you sure you want to delete this stage?",
...
void StageView::OnDeleteAllStages() {
  int result = MessageBox("Are you sure you want to delete all stages?",
                          "Delete?", MB_ICONQUESTION | MB_YESNO);
  if (result == IDYES) {
    MaterialDoc* material = materialDocManager->GetCurrentMaterialDoc();
...
void StageView::OnPaste() {
  if (materialDocManager->IsCopyStage()) {
    ...
    if (type != MaterialDoc::STAGE_TYPE_SPECIALMAP || existingIndex == -1) {
      ...
      if (MessageBox(va("Do you want to replace '%s' stage?", name.c_str()),
...
void StageView::OnLvnBeginlabeledit(NMHDR* pNMHDR, LRESULT* pResult) {
  ...
  MaterialDoc* material = materialDocManager->GetCurrentMaterialDoc();
  if (index <= 0 || material->GetAttributeInt(index - 1, "stagetype") !=
                        MaterialDoc::STAGE_TYPE_NORMAL) {
    *pResult = 1;
    return;
  }
...
void StageView::OnLvnEndlabeledit(NMHDR* pNMHDR, LRESULT* pResult) {
  ...
  if (pDispInfo->item.pszText) {
    MaterialDoc* material = materialDocManager->GetCurrentMaterialDoc();
    internalChange = true;
    material->SetAttribute(pDispInfo->item.iItem - 1, "name",
...
void StageView::PopupMenu(CPoint* pt) {
  ...
  if (nItem <= 0) {
    pPopupMenu->EnableMenuItem(ID_STAGEPOPUP_RENAMESTAGE,
                               MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
    pPopupMenu->EnableMenuItem(ID_STAGEPOPUP_DELETESTAGE,
                               MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);

    pPopupMenu->EnableMenuItem(ID_STAGEPOPUP_CUT,
                               MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
    pPopupMenu->EnableMenuItem(ID_STAGEPOPUP_COPY,
                               MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
  } else {
    MaterialDoc* material = materialDocManager->GetCurrentMaterialDoc();
    if (material->GetAttributeInt(nItem - 1, "stagetype") !=
        MaterialDoc::STAGE_TYPE_NORMAL) {
      pPopupMenu->EnableMenuItem(ID_STAGEPOPUP_RENAMESTAGE,
                                 MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
    }
  ...
  if (material->FindStage(MaterialDoc::STAGE_TYPE_SPECIALMAP, "bumpmap") >= 0) {
    pPopupMenu->EnableMenuItem(ID_STAGEPOPUP_ADDBUMPMAP,
                               MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
  }
  if (material->FindStage(MaterialDoc::STAGE_TYPE_SPECIALMAP, "diffusemap") >=
      0) {
    pPopupMenu->EnableMenuItem(ID_STAGEPOPUP_ADDDIFFUSEMAP,
                               MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
  }
  if (material->FindStage(MaterialDoc::STAGE_TYPE_SPECIALMAP, "specularmap") >=
      0) {
    pPopupMenu->EnableMenuItem(ID_STAGEPOPUP_ADDSPECULAR,
                               MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
  }
  ...
  if (materialDocManager->IsCopyStage()) {
    ...
    pPopupMenu->EnableMenuItem(ID_STAGEPOPUP_PASTE,
                               MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
  }
...
void StageView::DropItemOnList() {
  ...
  // Ignore the drop if the index is the same or they are droping on the
  // material

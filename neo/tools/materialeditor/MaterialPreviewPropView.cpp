...
void MaterialPreviewPropView::AddLight(void) {
  ...
  pCombo =
      (CPropTreeItemCombo*)m_Tree.InsertItem(new CPropTreeItemCombo(), pRoot);
  ...
  pColor =
      (CPropTreeItemColor*)m_Tree.InsertItem(new CPropTreeItemColor(), pRoot);
  pColor->SetLabelText(_T("Color"));
  pColor->SetInfoText(_T("Color of the light."));
  pColor->SetItemValue(
      (LPARAM)RGB(0xff, 0xff, 0xff));  // default as color white
  ...
  pCheck =
      (CPropTreeItemCheck*)m_Tree.InsertItem(new CPropTreeItemCheck(), pRoot);
...
void MaterialPreviewPropView::InitializePropTree(void) {
  ...
  pCombo->SetInfoText(
      _T("Select the type of model on which to preview the material."));
  pCombo->CreateComboBox();
  ...
  // Custom model entry
  /*pEdit = (CPropTreeItemEdit*)m_Tree.InsertItem( new CPropTreeItemEdit(),
  pRoot ); pEdit->SetLabelText(_T("Custom Model"));
  ...
  pCutomButton = (CPropTreeItemEditButton*)m_Tree.InsertItem(
      new CPropTreeItemEditButton(), pRoot);
  pCutomButton->SetButtonText(_T("..."));
  pCutomButton->SetLabelText(_T("Custom Model"));
  pCutomButton->SetInfoText(
      _T("Specify any model to display the current material."));
  ...
  pCheck->SetInfoText(
      _T("Show the light origin sphere and number in the preview."));
  pCheck->CreateCheckBox();
  pCheck->SetCheckState(BST_CHECKED);
  ...
  pParmRoot->SetInfoText(
      _T("Local shaderparms for the model being displayed."));
  ...
  pParmRoot->SetInfoText(
      _T("Global shaderparms for the renderworld being displayed."));
  pParmRoot->Expand(FALSE);  // have this item NOT expanded by default
...
void MaterialPreviewPropView::OnPropertyChangeNotification(NMHDR* nmhdr,
                                                           LRESULT* lresult) {
  idVec3 testColor;
  int lightId = 0;
  COLORREF color;
  ...
  if (itemLabel == "Model Type") {
    ...
    materialPreview->OnLightRadiusChange(lightId,
    ...
    materialPreview->OnLightAllowMoveChange(
        lightId, item->GetItemValue() ? true : false);
    ...
    materialPreview->OnLocalParmChange(index,
                                       atof((char*)item->GetItemValue()));
    ...
    materialPreview->OnGlobalParmChange(index,
                                        atof((char*)item->GetItemValue()));
  }
}

void MaterialPreviewPropView::OnPropertyButtonClick(NMHDR* nmhdr,
                                                    LRESULT* lresult) {
  NMPROPTREE* nmProp;
  CPropTreeItem* item;
  ...
  if (itemLabel == "Preview Lights") {
    ...
    for (light = m_Tree.GetRootItem()->GetChild(); light != NULL;
         light = light->GetSibling()) {
      ...
      if (label.Left(5) == "Light") {
        ...
        light->SetInfoText(
            _T(va("Parameters for light number %d.", testLightNum)));
    ...
    if (dlg.DoModal() == IDOK) {
      item->Check(FALSE);
      item->SetItemValue(
          (LPARAM)fileSystem->OSPathToRelativePath(dlg.m_ofn.lpstrFile));

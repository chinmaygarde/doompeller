...
void MaterialPropTreeView::SetPropertyListType(int listType, int stageNum) {
  ...
  MaterialDefList* propList =
      MaterialDefManager::GetMaterialDefs(currentListType);
  ...
  for (int i = 0; i < propList->Num(); i++) {
    switch ((*propList)[i]->type) {
      case MaterialDef::MATERIAL_DEF_TYPE_GROUP: {
        ...
        if (!registry.GetBool(va("Expand%d%s", currentListType,
                                 (*propList)[i]->displayName.c_str())))
          pCurrentGroup->Expand();
      } break;
      case MaterialDef::MATERIAL_DEF_TYPE_BOOL: {
        CPropTreeItemCheck* pCheck;
        pCheck = (CPropTreeItemCheck*)m_Tree.InsertItem(
            new CPropTreeItemCheck(), pCurrentGroup);
      ...
      case MaterialDef::MATERIAL_DEF_TYPE_STRING: {
        ...
        // pCurrentGroup);
        pCurrentNode =
            m_Tree.InsertItem(new CPropTreeItemFileEdit(), pCurrentGroup);
  ...
  if (propItem) {
    ...
    switch (propItem->type) {
      case MaterialDef::MATERIAL_DEF_TYPE_BOOL: {
        ...
        materialDoc->SetAttributeBool(currentStage, propItem->dictName,
                                      val ? true : false);
      } break;
      case MaterialDef::MATERIAL_DEF_TYPE_STRING: {
...
void MaterialPropTreeView::OnPropertyItemExpanding(NMHDR* nmhdr,
  ...
  // current state.
  registry.SetBool(va("Expand%d%s", currentListType, item->GetLabelText()),
                   item->IsExpanded() ? true : false);
  registry.Save();
...
void MaterialPropTreeView::RefreshProperties() {
  ...
  for (int i = 0; i < propList->Num(); i++) {
    switch ((*propList)[i]->type) {
      case MaterialDef::MATERIAL_DEF_TYPE_BOOL: {
        bool val = materialDoc->GetAttributeBool(currentStage,
                                                 (*propList)[i]->dictName);
        CPropTreeItemCheck* item = (CPropTreeItemCheck*)m_Tree.FindItem(
            (*propList)[i]->GetViewData(PROP_TREE_VIEW));
        item->SetCheckState(val ? TRUE : FALSE);
      } break;
      case MaterialDef::MATERIAL_DEF_TYPE_STRING: {
        idStr val =
            materialDoc->GetAttribute(currentStage, (*propList)[i]->dictName);
        CPropTreeItemEdit* item = (CPropTreeItemEdit*)m_Tree.FindItem(
            (*propList)[i]->GetViewData(PROP_TREE_VIEW));

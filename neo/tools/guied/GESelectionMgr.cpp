...
void rvGESelectionMgr::Render(void) {
  ...
  for (i = 0; i < mSelections.Num(); i++) {
    ...
    qglPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
    qglBegin(GL_QUADS);
    qglVertex2f(rect.x + (rect.w - GUIED_CENTERSIZE) / 2,
                rect.y + (rect.h - GUIED_CENTERSIZE) / 2);
    qglVertex2f(rect.x + (rect.w + GUIED_CENTERSIZE) / 2,
                rect.y + (rect.h - GUIED_CENTERSIZE) / 2);
    qglVertex2f(rect.x + (rect.w + GUIED_CENTERSIZE) / 2,
                rect.y + (rect.h + GUIED_CENTERSIZE) / 2);
  ...
  // Left
  qglVertex2f(mRect.x - GUIED_GRABSIZE,
              mRect.y + mRect.h / 2 - GUIED_GRABSIZE / 2);
  ...
  // Bottom
  qglVertex2f(mRect.x - GUIED_GRABSIZE / 2 + mRect.w / 2,
              mRect.y + mRect.h + 1);
  qglVertex2f(mRect.x + GUIED_GRABSIZE / 2 + mRect.w / 2,
              mRect.y + mRect.h + 1);
  qglVertex2f(mRect.x + GUIED_GRABSIZE / 2 + mRect.w / 2,
              mRect.y + mRect.h + GUIED_GRABSIZE);
  qglVertex2f(mRect.x - GUIED_GRABSIZE / 2 + mRect.w / 2,
              mRect.y + mRect.h + GUIED_GRABSIZE);
  ...
  qglVertex2f(mRect.x + mRect.w + GUIED_GRABSIZE,
              mRect.y + mRect.h + GUIED_GRABSIZE);
  qglVertex2f(mRect.x + mRect.w + 1, mRect.y + mRect.h + GUIED_GRABSIZE);

  // Right
  qglVertex2f(mRect.x + mRect.w + 1,
              mRect.y + mRect.h / 2 - GUIED_GRABSIZE / 2);
  qglVertex2f(mRect.x + mRect.w + GUIED_GRABSIZE,
              mRect.y + mRect.h / 2 - GUIED_GRABSIZE / 2);
  qglVertex2f(mRect.x + mRect.w + GUIED_GRABSIZE,
...
Test to see if the given coordinate is within the selection rectangle and if it
is see what its over.
================
*/
rvGESelectionMgr::EHitTest rvGESelectionMgr::HitTest(float x, float y) {
  ...
  if (idRectangle(mRect.x - GUIED_GRABSIZE, mRect.y - GUIED_GRABSIZE,
                  GUIED_GRABSIZE, GUIED_GRABSIZE)
          .Contains(x, y)) {
    return HT_SIZE_TOPLEFT;
  }

  // Check for left sizing
  if (idRectangle(mRect.x - GUIED_GRABSIZE,
                  mRect.y + mRect.h / 2 - GUIED_GRABSIZE / 2, GUIED_GRABSIZE,
                  GUIED_GRABSIZE)
          .Contains(x, y)) {
    return HT_SIZE_LEFT;
  }
  ...
  if (idRectangle(mRect.x - GUIED_GRABSIZE, mRect.y + mRect.h, GUIED_GRABSIZE,
                  GUIED_GRABSIZE)
          .Contains(x, y)) {
  ...
  if (idRectangle(mRect.x + mRect.w / 2 - GUIED_GRABSIZE / 2, mRect.y + mRect.h,
                  GUIED_GRABSIZE, GUIED_GRABSIZE)
          .Contains(x, y)) {
  ...
  if (idRectangle(mRect.x + mRect.w, mRect.y + mRect.h, GUIED_GRABSIZE,
                  GUIED_GRABSIZE)
          .Contains(x, y)) {
  ...
  if (idRectangle(mRect.x + mRect.w, mRect.y + mRect.h / 2 - GUIED_GRABSIZE / 2,
                  GUIED_GRABSIZE, GUIED_GRABSIZE)
          .Contains(x, y)) {
  ...
  if (idRectangle(mRect.x + mRect.w, mRect.y - GUIED_GRABSIZE, GUIED_GRABSIZE,
                  GUIED_GRABSIZE)
          .Contains(x, y)) {
  ...
  if (idRectangle(mRect.x + mRect.w / 2 - GUIED_GRABSIZE / 2,
                  mRect.y - GUIED_GRABSIZE, GUIED_GRABSIZE, GUIED_GRABSIZE)
          .Contains(x, y)) {
    return HT_SIZE_TOP;
  }
...
idWindow* rvGESelectionMgr::GetBottomMost(void) {
  ...
  for (i = 0; i < mSelections.Num(); i++) {
    ...
    // Calculate the depth of the window by iterating back through the windows
    // parents
    for (tempDepth = 0, parent = mSelections[i]; parent;
         parent = parent->GetParent(), tempDepth++)
      ;

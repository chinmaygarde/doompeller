...
rvGEHideModifier::rvGEHideModifier(const char* name,
                                   idWindow* window,
                                   bool hide)
  ...
  // If unhiding then find any parent window along the way that may be hidden
  // and prevent this window from being visible
  if (!hide) {
    mParent = mWindow;

...
void rvGEItemPropsImagePage::UpdateCheckedStates(void) {
  ...
  state = IsDlgButtonChecked(mPage, IDC_GUIED_USEBACKCOLOR) &&
          IsWindowEnabled(GetDlgItem(mPage, IDC_GUIED_USEBACKCOLOR));
  ...
  EnableWindow(GetDlgItem(mPage, IDC_GUIED_ITEMBORDERCOLOR),
               rstate && state && enable);
  EnableWindow(GetDlgItem(mPage, IDC_GUIED_ITEMBORDERCOLORALPHA),
               rstate && state && enable);
  EnableWindow(GetDlgItem(mPage, IDC_GUIED_ITEMBORDERMATERIAL),
               !rstate && state);

  state =
      (state && !rstate) || IsDlgButtonChecked(mPage, IDC_GUIED_USEMATERIAL);
  enable = !IsExpression(mDict->GetString("matcolor", "1,1,1,1"));
  EnableWindow(GetDlgItem(mPage, IDC_GUIED_ITEMMATCOLORSTATIC),
               state && enable);
    ...
    case WM_COMMAND:
      switch (LOWORD(wParam)) {
        ...
        case IDC_GUIED_ITEMMATCOLOR: {
          ...
          col.rgbResult =
              ColorButton_GetColor(GetDlgItem(mPage, LOWORD(wParam)));
          if (ChooseColor(&col)) {
            ColorButton_SetColor(GetDlgItem(mPage, LOWORD(wParam)),
                                 col.rgbResult);
          }
          break;
        }
...
bool rvGEItemPropsImagePage::SetActive(void) {
  ...
  ColorButton_SetColor(GetDlgItem(mPage, IDC_GUIED_ITEMBACKCOLOR),
                       mDict->GetString("backcolor", "1,1,1,1"));
  AlphaButton_SetColor(GetDlgItem(mPage, IDC_GUIED_ITEMBACKCOLORALPHA),
                       mDict->GetString("backcolor", "1,1,1,1"));

  ColorButton_SetColor(GetDlgItem(mPage, IDC_GUIED_ITEMMATCOLOR),
                       mDict->GetString("matcolor", "1,1,1,1"));
  AlphaButton_SetColor(GetDlgItem(mPage, IDC_GUIED_ITEMMATCOLORALPHA),
                       mDict->GetString("matcolor", "1,1,1,1"));

  ColorButton_SetColor(GetDlgItem(mPage, IDC_GUIED_ITEMBORDERCOLOR),
                       mDict->GetString("bordercolor", "0,0,0,1"));
  AlphaButton_SetColor(GetDlgItem(mPage, IDC_GUIED_ITEMBORDERCOLORALPHA),
                       mDict->GetString("bordercolor", "0,0,0,1"));

  SetWindowText(GetDlgItem(mPage, IDC_GUIED_ITEMBACKGROUND),
                idStr(mDict->GetString("background", "")).StripQuotes());
  SetWindowText(GetDlgItem(mPage, IDC_GUIED_ITEMBORDERMATERIAL),
                idStr(mDict->GetString("borderShader", "")).StripQuotes());
  SetWindowText(GetDlgItem(mPage, IDC_GUIED_ITEMBORDERSIZE),
                idStr(mDict->GetString("bordersize", "0")).StripQuotes());

  SetWindowText(GetDlgItem(mPage, IDC_GUIED_ITEMMATSCALEX),
                idStr(mDict->GetString("matscalex", "1")).StripQuotes());
  SetWindowText(GetDlgItem(mPage, IDC_GUIED_ITEMMATSCALEY),
                idStr(mDict->GetString("matscaley", "1")).StripQuotes());

  CheckDlgButton(mPage, IDC_GUIED_ITEMVARIABLEBACKGROUND,
                 mDict->GetBool("variablebackground", "0"));

  CheckDlgButton(
      mPage, IDC_GUIED_USEMATERIAL,
      idStr(mDict->GetString("background", "")).StripQuotes().Length()
          ? BST_CHECKED
          : BST_UNCHECKED);
  CheckDlgButton(mPage, IDC_GUIED_USEBACKCOLOR,
                 idStr(mDict->GetString("backcolor", "")).StripQuotes().Length()
                     ? BST_CHECKED
                     : BST_UNCHECKED);

  CheckRadioButton(mPage, IDC_GUIED_USEBORDERCOLOR, IDC_GUIED_USEBORDERMATERIAL,
                   idStr(mDict->GetString("borderShader", "")).Length()
                       ? IDC_GUIED_USEBORDERMATERIAL
...
Applys the settings currently stored in the property page back into the attached
window
================
*/
bool rvGEItemPropsImagePage::KillActive(void) {
  ...
  if (IsDlgButtonChecked(mPage, IDC_GUIED_USEBACKCOLOR)) {
    if (IsWindowEnabled(GetDlgItem(mPage, IDC_GUIED_ITEMBACKCOLOR))) {
      COLORREF color =
          ColorButton_GetColor(GetDlgItem(mPage, IDC_GUIED_ITEMBACKCOLOR));
      COLORREF alpha =
          ColorButton_GetColor(GetDlgItem(mPage, IDC_GUIED_ITEMBACKCOLORALPHA));
      mDict->Set("backcolor",
                 StringFromVec4(idVec4((float)GetRValue(color) / 255.0f,
  ...
  if (atoi(temp)) {
    ...
    if (IsDlgButtonChecked(mPage, IDC_GUIED_USEBORDERCOLOR)) {
      if (IsWindowEnabled(GetDlgItem(mPage, IDC_GUIED_ITEMBORDERCOLOR))) {
        COLORREF color =
            ColorButton_GetColor(GetDlgItem(mPage, IDC_GUIED_ITEMBORDERCOLOR));
        COLORREF alpha = ColorButton_GetColor(
            GetDlgItem(mPage, IDC_GUIED_ITEMBORDERCOLORALPHA));
      ...
      GetWindowText(GetDlgItem(mPage, IDC_GUIED_ITEMBORDERMATERIAL), temp,
                    1024);
      s = "\"";
      s.Append(temp);
      s.Append("\"");
  ...
  if (matcolor) {
    if (IsWindowEnabled(GetDlgItem(mPage, IDC_GUIED_ITEMMATCOLOR))) {
      COLORREF color =
          ColorButton_GetColor(GetDlgItem(mPage, IDC_GUIED_ITEMMATCOLOR));
      COLORREF alpha =
          ColorButton_GetColor(GetDlgItem(mPage, IDC_GUIED_ITEMMATCOLORALPHA));
      mDict->Set("matcolor",
                 StringFromVec4(idVec4((float)GetRValue(color) / 255.0f,
                                       (float)GetGValue(color) / 255.0f,
                                       (float)GetBValue(color) / 255.0f,
...
bool rvGEItemPropsTextPage::Init(void) {
  ...
  SendMessage(GetDlgItem(mPage, IDC_GUIED_ITEMTEXTALIGN), CB_ADDSTRING, 0,
              (LPARAM) "Left");
  SendMessage(GetDlgItem(mPage, IDC_GUIED_ITEMTEXTALIGN), CB_ADDSTRING, 0,
              (LPARAM) "Center");
  SendMessage(GetDlgItem(mPage, IDC_GUIED_ITEMTEXTALIGN), CB_ADDSTRING, 0,
...
void rvGEItemPropsTextPage::UpdateCheckedStates(void) {
  ...
  EnableWindow(GetDlgItem(mPage, IDC_GUIED_ITEMFORECOLORALPHA),
               state && enable);
...
bool rvGEItemPropsTextPage::SetActive(void) {
  ...
  ColorButton_SetColor(GetDlgItem(mPage, IDC_GUIED_ITEMFORECOLOR),
                       mDict->GetString("forecolor", "1,1,1,1"));
  AlphaButton_SetColor(GetDlgItem(mPage, IDC_GUIED_ITEMFORECOLORALPHA),
                       mDict->GetString("forecolor", "1,1,1,1"));

  SetWindowText(GetDlgItem(mPage, IDC_GUIED_ITEMTEXT),
                idStr(mDict->GetString("text", "")).StripQuotes());
  SetWindowText(GetDlgItem(mPage, IDC_GUIED_ITEMTEXTSCALE),
                mDict->GetString("textscale", "1.0"));
  SetWindowText(GetDlgItem(mPage, IDC_GUIED_ITEMTEXTALIGNX),
                mDict->GetString("textalignx", "0"));
  SetWindowText(GetDlgItem(mPage, IDC_GUIED_ITEMTEXTALIGNY),
                mDict->GetString("textaligny", "0"));

  SendMessage(GetDlgItem(mPage, IDC_GUIED_ITEMTEXTALIGN), CB_SETCURSEL,
              atoi(mDict->GetString("textalign", "0")), 0);
  ...
  font.StripQuotes();
  font.StripPath();
  fontSel = SendMessage(GetDlgItem(mPage, IDC_GUIED_ITEMTEXTFONT),
                        CB_FINDSTRING, -1, (LONG)font.c_str());
  SendMessage(GetDlgItem(mPage, IDC_GUIED_ITEMTEXTFONT), CB_SETCURSEL, 0, 0);
  SendMessage(GetDlgItem(mPage, IDC_GUIED_ITEMTEXTFONT), CB_SETCURSEL,
              fontSel == -1 ? 0 : fontSel, 0);

  CheckDlgButton(mPage, IDC_GUIED_USETEXT,
                 idStr(mDict->GetString("text", "")).Length() ? BST_CHECKED
                                                              : BST_UNCHECKED);
  CheckDlgButton(mPage, IDC_GUIED_ITEMTEXTNOWRAP,
                 atoi(idStr(mDict->GetString("nowrap", "0")).StripQuotes())
                     ? BST_CHECKED
                     : BST_UNCHECKED);
...
bool rvGEItemPropsTextPage::KillActive(void) {
  ...
  if (IsDlgButtonChecked(mPage, IDC_GUIED_USETEXT)) {
    if (IsWindowEnabled(GetDlgItem(mPage, IDC_GUIED_ITEMFORECOLOR))) {
      COLORREF color =
          ColorButton_GetColor(GetDlgItem(mPage, IDC_GUIED_ITEMFORECOLOR));
      COLORREF alpha =
          ColorButton_GetColor(GetDlgItem(mPage, IDC_GUIED_ITEMFORECOLORALPHA));
      mDict->Set("forecolor",
                 StringFromVec4(idVec4((float)GetRValue(color) / 255.0f,
                                       (float)GetGValue(color) / 255.0f,
                                       (float)GetBValue(color) / 255.0f,
    ...
    i = SendMessage(GetDlgItem(mPage, IDC_GUIED_ITEMTEXTALIGN), CB_GETCURSEL, 0,
                    0);
    s = va("%d", i);
    if (i != 0) {
    ...
    int fontSel = SendMessage(GetDlgItem(mPage, IDC_GUIED_ITEMTEXTFONT),
                              CB_GETCURSEL, 0, 0);
    if (fontSel == 0) {
      mDict->Delete("font");
    } else {
...
rvGEItemPropsKeysPage::rvGEItemPropsKeysPage(idDict* dict,
...
INT_PTR CALLBACK ModifyItemKeyDlg_WndProc(HWND hwnd,
  ...
  switch (msg) {
    case WM_INITDIALOG: {
      ...
      SetWindowText(GetDlgItem(hwnd, IDC_GUIED_ITEMVALUE),
                    idStr(keyValue->GetValue()).StripQuotes());
      if (idStr::Icmp(keyValue->GetKey(), "guied_temp")) {
        if (!idStr::Icmp(keyValue->GetKey(), "name") ||
            !idStr::Icmp(keyValue->GetKey(), "rect")) {
    ...
    case WM_COMMAND:
      switch (LOWORD(wParam)) {
        case IDC_GUIED_ADDKEY: {
          ...
          while (1) {
            if (DialogBoxParam(gApp.GetInstance(),
                               MAKEINTRESOURCE(IDD_GUIED_ITEMKEY), mPage,
                               ModifyItemKeyDlg_WndProc, (LPARAM)key)) {
              idStr finalValue;
              ...
              if (!mWrapper->VerfiyStateKey(key->GetKey(), finalValue)) {
                ...
                if (!mWrapper->VerfiyStateKey(key->GetKey(), finalValue)) {
                  gApp.MessageBox(
                      va("Invalid property value '%s' for property '%s'",
                         key->GetValue().c_str(), key->GetKey().c_str()),
                      MB_OK | MB_ICONERROR);
                  continue;
                }
              }
          ...
          if (index != -1) {
            ...
            while (1) {
              if (DialogBoxParam(gApp.GetInstance(),
                ...
                ListView_SetItemText(list, index, 0,
                                     (LPSTR)key->GetKey().c_str());
...
rvGEItemPropsGeneralPage::rvGEItemPropsGeneralPage(
    idDict* dict,
...
bool rvGEItemPropsGeneralPage::SetActive(void) {
  ...
  SetWindowText(GetDlgItem(mPage, IDC_GUIED_ITEMNAME),
                idStr(mDict->GetString("name", "unnamed")).StripQuotes());

  enable = !IsExpression(mDict->GetString("visible", "1"));
  CheckDlgButton(mPage, IDC_GUIED_ITEMVISIBLE,
                 atol(idStr(mDict->GetString("visible", "1")).StripQuotes())
                     ? BST_CHECKED
                     : BST_UNCHECKED);
  EnableWindow(GetDlgItem(mPage, IDC_GUIED_ITEMVISIBLE), enable);

  enable = !IsExpression(mDict->GetString("notime", "0"));
  CheckDlgButton(mPage, IDC_GUIED_ITEMNOTIME,
                 atol(idStr(mDict->GetString("notime", "0")).StripQuotes())
                     ? BST_CHECKED
                     : BST_UNCHECKED);
  EnableWindow(GetDlgItem(mPage, IDC_GUIED_ITEMNOTIME), enable);

  enable = !IsExpression(mDict->GetString("noevents", "0"));
  CheckDlgButton(mPage, IDC_GUIED_ITEMNOEVENTS,
                 atol(idStr(mDict->GetString("noevents", "0")).StripQuotes())
                     ? BST_CHECKED
                     : BST_UNCHECKED);
  EnableWindow(GetDlgItem(mPage, IDC_GUIED_ITEMNOEVENTS), enable);

  enable = !IsExpression(mDict->GetString("noclip", "0"));
  CheckDlgButton(mPage, IDC_GUIED_ITEMNOCLIP,
                 atol(idStr(mDict->GetString("noclip", "0")).StripQuotes())
                     ? BST_CHECKED
                     : BST_UNCHECKED);
  ...
  CheckDlgButton(mPage, IDC_GUIED_ITEMNOCURSOR,
                 atol(idStr(mDict->GetString("nocursor", "0")).StripQuotes())
                     ? BST_CHECKED
                     : BST_UNCHECKED);
...
bool GEItemPropsDlg_DoModal(HWND parent, idWindow* window, idDict& dict) {
  ...
  propsp[RVITEMPROPS_GENERAL].pszTemplate =
      MAKEINTRESOURCE(IDD_GUIED_ITEMPROPS_GENERAL);
  propsp[RVITEMPROPS_GENERAL].pfnDlgProc = rvGEPropertyPage::WndProc;
  propsp[RVITEMPROPS_GENERAL].pszTitle = "General";
  propsp[RVITEMPROPS_GENERAL].lParam =
      (LONG) new rvGEItemPropsGeneralPage(&dict, wrapper->GetWindowType());
  ...
  propsp[RVITEMPROPS_IMAGE].pszTemplate =
      MAKEINTRESOURCE(IDD_GUIED_ITEMPROPS_IMAGE);
  ...
  propsp[RVITEMPROPS_TEXT].pszTemplate =
      MAKEINTRESOURCE(IDD_GUIED_ITEMPROPS_TEXT);
  ...
  propsp[RVITEMPROPS_KEYS].pszTemplate =
      MAKEINTRESOURCE(IDD_GUIED_ITEMPROPS_KEYS);
  propsp[RVITEMPROPS_KEYS].pfnDlgProc = rvGEPropertyPage::WndProc;
  propsp[RVITEMPROPS_KEYS].pszTitle = "Keys";
  propsp[RVITEMPROPS_KEYS].lParam =
      (LONG) new rvGEItemPropsKeysPage(&dict, wrapper);

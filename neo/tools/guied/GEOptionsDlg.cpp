...
================
*/
static INT_PTR CALLBACK GEOptionsDlg_GeneralProc(HWND hwnd,
                                                 UINT msg,
                                                 WPARAM wParam,
                                                 LPARAM lParam) {
  switch (msg) {
    case WM_INITDIALOG:
      ...
      CheckDlgButton(hwnd, IDC_GUIED_IGNOREDESKTOP,
                     gApp.GetOptions().GetIgnoreDesktopSelect()
                         ? BST_CHECKED
                         : BST_UNCHECKED);
      break;
    ...
    case WM_COMMAND:
      switch (LOWORD(wParam)) {
        case IDC_GUIED_SELECTIONCOLOR: {
          ...
          col.rgbResult =
              ColorButton_GetColor(GetDlgItem(hwnd, IDC_GUIED_SELECTIONCOLOR));
          if (ChooseColor(&col)) {
            ColorButton_SetColor(GetDlgItem(hwnd, IDC_GUIED_SELECTIONCOLOR),
                                 col.rgbResult);
          }
    ...
    case WM_NOTIFY:
      switch (((NMHDR FAR*)lParam)->code) {
        case PSN_APPLY:
          gApp.GetOptions().SetLastOptionsPage(PropSheet_HwndToIndex(
              GetParent(hwnd), PropSheet_GetCurrentPageHwnd(GetParent(hwnd))));
          gApp.GetOptions().SetSelectionColor(
              ColorButton_GetColor(GetDlgItem(hwnd, IDC_GUIED_SELECTIONCOLOR)));
          gApp.GetOptions().SetIgnoreDesktopSelect(
              IsDlgButtonChecked(hwnd, IDC_GUIED_IGNOREDESKTOP) != 0);
          break;
      }
      break;
  }
...
================
*/
static INT_PTR CALLBACK GEOptionsDlg_GridProc(HWND hwnd,
                                              UINT msg,
                                              WPARAM wParam,
                                              LPARAM lParam) {
  switch (msg) {
    case WM_INITDIALOG:
      ...
      ColorButton_SetColor(GetDlgItem(hwnd, IDC_GUIED_GRIDCOLOR),
                           RGB(gApp.GetOptions().GetGridColor()[0] * 255,
                               gApp.GetOptions().GetGridColor()[1] * 255,
                               gApp.GetOptions().GetGridColor()[2] * 255));
      SetWindowText(GetDlgItem(hwnd, IDC_GUIED_SPACINGWIDTH),
                    va("%d", gApp.GetOptions().GetGridWidth()));
      SetWindowText(GetDlgItem(hwnd, IDC_GUIED_SPACINGHEIGHT),
                    va("%d", gApp.GetOptions().GetGridHeight()));
      CheckDlgButton(
          hwnd, IDC_GUIED_GRIDVISIBLE,
          gApp.GetOptions().GetGridVisible() ? BST_CHECKED : BST_UNCHECKED);
      CheckDlgButton(
          hwnd, IDC_GUIED_GRIDSNAP,
          gApp.GetOptions().GetGridSnap() ? BST_CHECKED : BST_UNCHECKED);
      return TRUE;
    ...
    case WM_NOTIFY:
      switch (((NMHDR FAR*)lParam)->code) {
        case PSN_APPLY: {
          ...
          gApp.GetOptions().SetGridVisible(
              IsDlgButtonChecked(hwnd, IDC_GUIED_GRIDVISIBLE) != 0);
          gApp.GetOptions().SetGridSnap(
              IsDlgButtonChecked(hwnd, IDC_GUIED_GRIDSNAP) != 0);
          gApp.GetOptions().SetGridColor(
              ColorButton_GetColor(GetDlgItem(hwnd, IDC_GUIED_GRIDCOLOR)));
          break;
        }
      }
      break;
    ...
    case WM_COMMAND:
      switch (LOWORD(wParam)) {
        case IDC_GUIED_GRIDCOLOR: {
          ...
          col.hInstance = NULL;
          col.Flags = CC_RGBINIT;
          col.rgbResult = RGB(gApp.GetOptions().GetGridColor()[0] * 255,
                              gApp.GetOptions().GetGridColor()[1] * 255,
                              gApp.GetOptions().GetGridColor()[2] * 255);
          if (ChooseColor(&col)) {
            ColorButton_SetColor(GetDlgItem(hwnd, IDC_GUIED_GRIDCOLOR),

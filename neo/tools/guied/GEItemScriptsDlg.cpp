...
LRESULT CALLBACK GEScriptEdit_WndProc(HWND hwnd,
                                      UINT msg,
                                      WPARAM wParam,
                                      LPARAM lParam) {
...
bool GEItescriptsDlg_Init(HWND hwnd) {
  ...
  SendMessage(script, EM_SETMARGINS, EC_LEFTMARGIN | EC_RIGHTMARGIN,
              MAKELONG(10, 10));
  ...
  for (i = 0; i < wrapper->GetScriptDict().GetNumKeyVals(); i++) {
    ...
    SendMessage(script, EM_SETSEL, -1, -1);
    SendMessage(script, EM_REPLACESEL, FALSE,
                (LPARAM)va("%s\r\n", key->GetKey().c_str()));
    SendMessage(script, EM_SETSEL, -1, -1);
    SendMessage(script, EM_REPLACESEL, FALSE, (LPARAM)key->GetValue().c_str());
    SendMessage(script, EM_SETSEL, -1, -1);
...
bool GEItescriptsDlg_Apply(HWND hwnd) {
  ...
  try {
    idParser src(parse, parse.Length(), "",
...
INT_PTR CALLBACK GEItescriptsDlg_WndProc(HWND hwnd,
                                         UINT msg,
                                         WPARAM wParam,
                                         LPARAM lParam) {
  switch (msg) {
    ...
    case WM_SIZE: {
      ...
      MoveWindow(GetDlgItem(hwnd, IDC_GUIED_SCRIPT), rClient.left, rClient.top,
...
bool GEItemScriptsDlg_DoModal(HWND parent, idWindow* window) {
  ...
  if (DialogBoxParam(gApp.GetInstance(), MAKEINTRESOURCE(IDD_GUIED_SCRIPTS),
                     parent, GEItescriptsDlg_WndProc, (LPARAM)window)) {
    return true;
  }

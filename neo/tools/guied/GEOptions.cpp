...
Writes the options to the registry so they can later be read using the Load
method
================
*/
bool rvGEOptions::Save(void) {
  ...
  mRegistry.SetVec4("gridColor",
                    idVec4(mGridColor[0], mGridColor[1], mGridColor[2], 1.0f));
...
void rvGEOptions::SnapRectToGrid(idRectangle& rect,
  ...
  if (snapLeft) {
    float offset =
        (int)(rect.x + GetGridWidth() / 2) / GetGridWidth() * GetGridWidth();
    offset -= rect.x;
    rect.x += offset;
    rect.w -= offset;
  }
  ...
  if (snapWidth) {
    float offset = (int)(rect.x + rect.w + GetGridWidth() / 2) /
  ...
  if (snapTop) {
    float offset =
        (int)(rect.y + GetGridHeight() / 2) / GetGridHeight() * GetGridHeight();
    offset -= rect.y;
    rect.y += offset;
    rect.h -= offset;
  }
  ...
  if (snapHeight) {
    float offset = (int)(rect.y + rect.h + GetGridHeight() / 2) /

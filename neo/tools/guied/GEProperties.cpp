...
bool rvGEProperties::Create(HWND parent, bool visible) {
  ...
  mWnd = CreateWindowEx(
      WS_EX_TOOLWINDOW, "GUIEDITOR_PROPERTIES_CLASS", "Properties",
      WS_SYSMENU | WS_THICKFRAME | WS_CAPTION | WS_POPUP | WS_OVERLAPPED |
  ...
  if (!gApp.GetOptions().GetWindowPlacement("properties", mWnd)) {
    ...
    SetWindowPos(mWnd, NULL,
                 ...
                 rParent.bottom - 10 - (rClient.bottom - rClient.top), 0, 0,
...
bool rvGEProperties::AddModifier(const char* name, const char* value) {
  ...
  if (!mWrapper->VerfiyStateKey(name, tempvalue)) {
    ...
    if (!mWrapper->VerfiyStateKey(name, tempvalue)) {
      gApp.MessageBox(
          va("Invalid property value '%s' for property '%s'", value, name),
          MB_OK);
      return false;
    }
  }
  ...
  mWorkspace->GetModifierStack().Append(new rvGEStateModifier(
      "Property Change", mWrapper->GetWindow(), tempstate));
        ...
        switch (hdr->code) {
          case PGN_ITEMCHANGED:
            ...
            /*
               ...
               || !idStr::Icmp ( prop, "visible" )	|| !idStr::Icmp ( prop,
               "name" ) )
                                                                            {

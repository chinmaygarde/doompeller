...
================
*/
static INT_PTR CALLBACK GECheckInDlg_GeneralProc(HWND hwnd,
                                                 UINT msg,
                                                 WPARAM wParam,
                                                 LPARAM lParam) {
    ...
    case WM_COMMAND:
      switch (LOWORD(wParam)) {
        case IDOK: {
          ...
          GetWindowText(GetDlgItem(hwnd, IDC_GUIED_COMMENT), temp,
                        tempsize + 1);
...
bool GECheckInDlg_DoModal(HWND parent, const char* filename, idStr* comment) {
  ...
  if (!DialogBoxParam(gApp.GetInstance(), MAKEINTRESOURCE(IDD_GUIED_CHECKIN),
                      parent, GECheckInDlg_GeneralProc, (LPARAM)&dlg)) {
    return false;
  }

...
bool rvGENavigator::Create(HWND parent, bool visible) {
  ...
  mVisibleIcon = (HICON)LoadImage(
      win32.hInstance, MAKEINTRESOURCE(IDI_GUIED_NAV_VISIBLE), IMAGE_ICON, 16,
      16, LR_DEFAULTCOLOR | LR_LOADMAP3DCOLORS);
  mScriptsIcon =
      (HICON)LoadImage(win32.hInstance, MAKEINTRESOURCE(IDI_GUIED_NAV_SCRIPTS),
                       IMAGE_ICON, 16, 16, LR_DEFAULTCOLOR);
  mScriptsLightIcon = (HICON)LoadImage(win32.hInstance,
                                       MAKEINTRESOURCE(IDI_GUIED_NAV_SCRIPTSHI),
                                       IMAGE_ICON, 16, 16, LR_DEFAULTCOLOR);
  mVisibleIconDisabled = (HICON)LoadImage(
      win32.hInstance, MAKEINTRESOURCE(IDI_GUIED_NAV_VISIBLEDISABLED),
      IMAGE_ICON, 16, 16, LR_DEFAULTCOLOR | LR_LOADMAP3DCOLORS);
  mExpandIcon = (HICON)LoadImage(
      win32.hInstance, MAKEINTRESOURCE(IDI_GUIED_NAV_EXPAND), IMAGE_ICON, 16,
      16, LR_DEFAULTCOLOR | LR_LOADMAP3DCOLORS);
  mCollapseIcon = (HICON)LoadImage(
      win32.hInstance, MAKEINTRESOURCE(IDI_GUIED_NAV_COLLAPSE), IMAGE_ICON, 16,
      16, LR_DEFAULTCOLOR | LR_LOADMAP3DCOLORS);

  mWnd =
      CreateWindowEx(WS_EX_TOOLWINDOW, "GUIEDITOR_NAVIGATOR_CLASS", "Navigator",
                     WS_SYSMENU | WS_THICKFRAME | WS_CAPTION | WS_POPUP |
                         WS_OVERLAPPED | WS_BORDER | WS_CLIPSIBLINGS | WS_CHILD,
                     0, 0, 200, 300, parent, NULL, win32.hInstance, this);
  ...
  if (!gApp.GetOptions().GetWindowPlacement("navigator", mWnd)) {
    ...
    SetWindowPos(mWnd, NULL, rParent.right - 10 - (rNav.right - rNav.left),
                 rParent.bottom - 10 - (rNav.bottom - rNav.top), 0, 0,
  ...
  switch (msg) {
    ...
    case WM_DRAWITEM: {
      ...
      if (window) {
        ...
        Draw3dRect(dis->hDC, &rDraw, GetSysColorBrush(COLOR_3DHILIGHT),
                   GetSysColorBrush(COLOR_3DSHADOW));

        InflateRect(&rDraw, -3, -3);
        Draw3dRect(dis->hDC, &rDraw, GetSysColorBrush(COLOR_3DSHADOW),
                   GetSysColorBrush(COLOR_3DHILIGHT));

        if (!wrapper->IsHidden()) {
          DrawIconEx(dis->hDC, rDraw.left, rDraw.top,
                     disabled ? nav->mVisibleIconDisabled : nav->mVisibleIcon,
        ...
        if (wrapper->CanHaveChildren() && window->GetChildCount()) {
          if (wrapper->IsExpanded()) {
            DrawIconEx(dis->hDC, rDraw.left + offset, rDraw.top + 3,
                       nav->mCollapseIcon, 16, 16, 0, NULL, DI_NORMAL);
          } else {
            DrawIconEx(dis->hDC, rDraw.left + offset, rDraw.top + 3,
                       nav->mExpandIcon, 16, 16, 0, NULL, DI_NORMAL);
          }
        }
        ...
        int colorIndex = ((dis->itemState & ODS_SELECTED) ? COLOR_HIGHLIGHTTEXT
                                                          : COLOR_WINDOWTEXT);
        SetTextColor(dis->hDC, GetSysColor(colorIndex));
        DrawText(dis->hDC, name, name.Length(), &rDraw,
                 DT_LEFT | DT_VCENTER | DT_SINGLELINE);

        if (wrapper->GetVariableDict().GetNumKeyVals() ||
            wrapper->GetScriptDict().GetNumKeyVals()) {
          DrawIconEx(dis->hDC, dis->rcItem.right - 16,
                     (dis->rcItem.bottom + dis->rcItem.top) / 2 - 6,
                     (dis->itemState & ODS_SELECTED) ? nav->mScriptsLightIcon
                                                     : nav->mScriptsIcon,
                     13, 13, 0, NULL, DI_NORMAL);
        }
      }
    ...
    case WM_CREATE: {
      ...
      nav->mTree = CreateWindowEx(
          0, "SysListView32", "",
          WS_VSCROLL | WS_CHILD | WS_VISIBLE | LVS_REPORT | LVS_OWNERDRAWFIXED |
              LVS_NOCOLUMNHEADER | LVS_SHOWSELALWAYS,
          0, 0, 0, 0, hWnd, (HMENU)IDC_GUIED_WINDOWTREE, win32.hInstance, 0);
      ...
      switch (nh->code) {
        ...
        case NM_DBLCLK: {
          ...
          if (index != -1) {
            ...
            if (info.pt.x < GENAV_ITEMHEIGHT) {
            ...
            } else if (info.pt.x > GENAV_ITEMHEIGHT + offset &&
                       info.pt.x < GENAV_ITEMHEIGHT + offset + 16) {
              if (wrapper->CanHaveChildren() && window->GetChildCount()) {
                if (wrapper->IsExpanded()) {
                  wrapper->Collapse();
                  nav->Update();
              ...
              SendMessage(gApp.GetMDIFrame(), WM_COMMAND,
                          MAKELONG(ID_GUIED_ITEM_PROPERTIES, 0), 0);
            }
          }
        ...
        case NM_RCLICK: {
          ...
          if (index != -1) {
            ...
            HMENU menu =
                GetSubMenu(LoadMenu(gApp.GetInstance(),
                                    MAKEINTRESOURCE(IDR_GUIED_ITEM_POPUP)),
                           0);
            TrackPopupMenu(menu, TPM_RIGHTBUTTON | TPM_LEFTALIGN, info.pt.x,
                           info.pt.y, 0, gApp.GetMDIFrame(), NULL);
            DestroyMenu(menu);
          }
        ...
        case LVN_ITEMCHANGED: {
          ...
          if ((nml->uNewState & LVIS_SELECTED) !=
            ...
            if (nml->uNewState & LVIS_SELECTED) {
              nav->mWorkspace->GetSelectionMgr().Add((idWindow*)item.lParam,
                                                     false);
            } else {
...
void rvGENavigator::AddWindow(idWindow* window) {
  ...
  item.iImage = 0;
  item.state =
      rvGEWindowWrapper::GetWrapper(window)->IsSelected() ? LVIS_SELECTED : 0;
...
void rvGENavigator::Update(void) {
  ...
  // For some reason the horizontal scrollbar wants to show up initially after
  // an update so this forces it not to
  RECT rClient;
...
void rvGENavigator::UpdateSelections(void) {
  ...
  for (i = 0; i < count; i++) {
    ...
    ListView_SetItemState(mTree, i, wrapper->IsSelected() ? LVIS_SELECTED : 0,
                          LVIS_SELECTED);

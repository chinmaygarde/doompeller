...
bool rvGEViewer::Create(HWND parent) {
  ...
  mWnd =
      CreateWindowEx(WS_EX_TOOLWINDOW, "GUIED_VIEWER", "GUI Viewer",
                     WS_SYSMENU | WS_THICKFRAME | WS_CAPTION | WS_POPUP |
                         WS_OVERLAPPED | WS_BORDER | WS_CLIPSIBLINGS | WS_CHILD,
                     CW_USEDEFAULT, CW_USEDEFAULT, SCREEN_WIDTH / 2,
                     SCREEN_HEIGHT / 2, parent, NULL, gApp.GetInstance(), this);
...
void rvGEViewer::Play(void) {
  ...
  SendMessage(mToolbar, TB_SETBUTTONINFO, ID_GUIED_VIEWER_PLAY,
              (LPARAM)&tbinfo);
}
...
void rvGEViewer::Pause(void) {
  ...
  SendMessage(mToolbar, TB_SETBUTTONINFO, ID_GUIED_VIEWER_PAUSE,
              (LPARAM)&tbinfo);
}
...
bool rvGEViewer::OpenFile(const char* filename) {
  ...
  mInterface = reinterpret_cast<idUserInterfaceLocal*>(
      uiManager->FindGui(tempfile, true, true));
...
LRESULT CALLBACK rvGEViewer::WndProc(HWND hwnd,
  ...
  switch (msg) {
    ...
    case WM_MOUSEMOVE:
      if (viewer->mInterface) {
        float x = (float)(LOWORD(lParam)) / (float)viewer->mWindowWidth *
                  SCREEN_WIDTH;
        float y = (float)(HIWORD(lParam)) /
                  (float)(viewer->mWindowHeight - viewer->mToolbarHeight) *
                  SCREEN_HEIGHT;
        sysEvent_t event;
        bool visuals;
    ...
    case WM_CREATE: {
      ...
      viewer->mToolbar = CreateWindowEx(
          0, TOOLBARCLASSNAME, "", CCS_BOTTOM | WS_CHILD | WS_VISIBLE, 0, 0, 0,
          0, hwnd, (HMENU)IDR_GUIED_VIEWERTOOLBAR, gApp.GetInstance(), NULL);

      // Send the TB_BUTTONSTRUCTSIZE message, which is required for backward
      // compatibility.
      SendMessage(viewer->mToolbar, TB_BUTTONSTRUCTSIZE,
                  (WPARAM)sizeof(TBBUTTON), 0);

      SendMessage(viewer->mToolbar, TB_SETBUTTONSIZE, 0, MAKELONG(16, 15));

      SendMessage(
          viewer->mToolbar, TB_SETSTYLE, 0,
          SendMessage(viewer->mToolbar, TB_GETSTYLE, 0, 0) | TBSTYLE_FLAT);

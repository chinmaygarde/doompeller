...
static INT_PTR CALLBACK AboutDlg_WndProc(HWND hwnd,
                                         UINT msg,
                                         WPARAM wParam,
                                         LPARAM lParam) {
  switch (msg) {
...
bool rvGEApp::Initialize(void) {
  ...
  mAccelerators =
      LoadAccelerators(mInstance, MAKEINTRESOURCE(IDR_GUIED_ACCELERATORS));
  ...
  wndClass.style =
      CS_OWNDC | CS_DBLCLKS | CS_BYTEALIGNWINDOW | CS_VREDRAW | CS_HREDRAW;
  wndClass.hbrBackground = (HBRUSH)GetStockObject(LTGRAY_BRUSH);
  RegisterClassEx(&wndClass);

  // Create the main window
  mMDIFrame = CreateWindow("QUAKE4_GUIEDITOR_CLASS", "Quake IV GUI Editor",
                           WS_OVERLAPPEDWINDOW | WS_CLIPSIBLINGS, CW_USEDEFAULT,
                           CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL,
                           NULL, mInstance, (LPVOID)this);

  if (!mMDIFrame) {
    return false;
  }

  SetClassLong(mMDIFrame, GCL_HICON,
               (LONG)LoadIcon(win32.hInstance, MAKEINTRESOURCE(IDI_GUIED)));
  ...
  mMDIClient = CreateWindow("MDICLIENT", NULL,
                            ...
                            0, 0, 1000, 1000, mMDIFrame, NULL, mInstance, &ccs);
...
LRESULT CALLBACK rvGEApp::FrameWndProc(HWND hWnd,
  ...
  switch (uMsg) {
    case WM_SIZE: {
      ...
      MoveWindow(app->mMDIClient, 0, 0, rClient.right - rClient.left,
                 rClient.bottom - rClient.top, FALSE);
...
void rvGEApp::HandleCommandSave(rvGEWorkspace* workspace,
  ...
  if (workspace->IsNew() || filename == NULL) {
    ...
    if (_taccess(realFilename, 0) == 0) {
      if (IDNO ==
          MessageBox(va("File '%s' already exists. Do you want to replace it?",
                        realFilename.c_str()),
  ...
  if (workspace->SaveFile(realFilename)) {
    ...
    MessageBox(va("Could not write file '%s'", workspace->GetFilename()),
               MB_OK | MB_ICONERROR);
  }
}
...
int rvGEApp::HandleCommand(WPARAM wParam, LPARAM lParam) {
  ...
  if (LOWORD(wParam) >= ID_GUIED_FILE_MRU1 &&
      LOWORD(wParam) < ID_GUIED_FILE_MRU1 + rvGEOptions::MAX_MRU_SIZE) {
    OpenFile(mOptions.GetRecentFile(mOptions.GetRecentFileCount() -
                                    (LOWORD(wParam) - ID_GUIED_FILE_MRU1) - 1));
    return 0;
  }
  ...
  switch (LOWORD(wParam)) {
    ...
    case ID_GUIED_SOURCECONTROL_UNDOCHECKOUT:
      ...
      if (IDYES ==
          MessageBox(
              va("Are you sure you want to undo the checkout of the file '%s'?",
    ...
    case ID_GUIED_HELP_ABOUT:
      DialogBox(GetInstance(), MAKEINTRESOURCE(IDD_GUIED_ABOUT), mMDIFrame,
                AboutDlg_WndProc);
      break;
    ...
    case ID_GUIED_TOOLS_VIEWER: {
      ...
      if (workspace) {
        if (!workspace->IsModified() ||
            HandleCommand(MAKELONG(ID_GUIED_FILE_SAVE, 0), 0)) {
    ...
    case ID_GUIED_VIEW_STATUSBAR: {
      ...
      SendMessage(
          mMDIFrame, WM_SIZE, 0,
          MAKELONG(rWindow.right - rWindow.left, rWindow.bottom - rWindow.top));
      break;
    }
...
int rvGEApp::HandleInitMenu(WPARAM wParam, LPARAM lParam) {
  ...
  for (nPos = 0; nPos < cMenuItems; nPos++) {
    ...
    switch (id) {
      case ID_GUIED_VIEW_STATUSBAR:
        flags = MF_BYCOMMAND |
                (mOptions.GetStatusBarVisible() ? MF_CHECKED : MF_UNCHECKED);
        CheckMenuItem(hmenu, id, flags);
        break;

      case ID_GUIED_WINDOW_SHOWNAVIGATOR:
        flags = MF_BYCOMMAND |
                (mOptions.GetNavigatorVisible() ? MF_CHECKED : MF_UNCHECKED);
        CheckMenuItem(hmenu, id, flags);
        break;

      case ID_GUIED_WINDOW_SHOWPROPERTIES:
        flags = MF_BYCOMMAND |
                (mOptions.GetPropertiesVisible() ? MF_CHECKED : MF_UNCHECKED);
        CheckMenuItem(hmenu, id, flags);
        break;

      case ID_GUIED_WINDOW_SHOWTRANSFORMER:
        flags = MF_BYCOMMAND |
                (mOptions.GetTransformerVisible() ? MF_CHECKED : MF_UNCHECKED);
        CheckMenuItem(hmenu, id, flags);
        break;
    }
    ...
    switch (id) {
      ...
      case ID_GUIED_EDIT_UNDO: {
        ...
        if (!workspace->GetModifierStack().CanUndo()) {
          ...
          undo = "Undo ";
          undo.Append(
              workspace->GetModifierStack().GetUndoModifier()->GetName());
          undo.Append("\tCtrl+Z");
      ...
      case ID_GUIED_EDIT_REDO: {
        ...
        if (!workspace || !workspace->GetModifierStack().CanRedo()) {
          ...
          undo = "Redo ";
          undo.Append(
              workspace->GetModifierStack().GetRedoModifier()->GetName());
          undo.Append("\tCtrl+Y");
          info.fState = MFS_ENABLED;
        }
      ...
      case ID_GUIED_VIEW_SHOWGRID:
        flags = MF_BYCOMMAND |
                (mOptions.GetGridVisible() ? MF_CHECKED : MF_UNCHECKED);
        CheckMenuItem(hmenu, id, flags);
        break;

      case ID_GUIED_VIEW_SNAPTOGRID:
        flags =
            MF_BYCOMMAND | (mOptions.GetGridSnap() ? MF_CHECKED : MF_UNCHECKED);
        CheckMenuItem(hmenu, id, flags);
        break;
      ...
      case ID_GUIED_EDIT_DELETE:
      case ID_GUIED_EDIT_COPY:
        EnableMenuItem(hmenu, nPos,
                       MF_BYPOSITION | (workspace->GetSelectionMgr().Num() > 0
                                            ? MF_ENABLED
                                            : MF_GRAYED));
        break;
      ...
      case ID_GUIED_EDIT_PASTE:
        EnableMenuItem(
            hmenu, nPos,
            MF_BYPOSITION |
                (workspace->GetClipboard().Num() > 0 ? MF_ENABLED : MF_GRAYED));
        break;
      ...
      case ID_GUIED_ITEM_SCRIPTS:
        EnableMenuItem(hmenu, nPos,
                       MF_BYPOSITION | (workspace->GetSelectionMgr().Num() == 1
                                            ? MF_ENABLED
                                            : MF_GRAYED));
        break;
      ...
      case ID_GUIED_SOURCECONTROL_UNDOCHECKOUT:
        EnableMenuItem(hmenu, nPos,
                       MF_BYPOSITION | ((workspace->GetSourceControlState() ==
                                         rvGEWorkspace::SCS_CHECKEDOUT)
                                            ? MF_ENABLED
                                            : MF_GRAYED));
        break;
...
bool rvGEApp::NewFile(void) {
  ...
  if (workspace->NewFile()) {
    ...
    child = CreateMDIWindow("QUAKE4_GUIEDITOR_CHILD_CLASS", "Untitled.gui",
                            WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN |
                                WS_HSCROLL | WS_VSCROLL | WS_MAXIMIZE,
                            CW_USEDEFAULT, CW_USEDEFAULT, 640, 480, mMDIClient,
                            mInstance, (LONG)workspace);
...
bool rvGEApp::OpenFile(const char* filename) {
  ...
  for (i = 0; i < mWorkspaces.Num(); i++) {
    if (!idStr::Icmp(mWorkspaces[i]->GetFilename(), filename)) {
      SendMessage(mMDIClient, WM_MDIACTIVATE,
                  (WPARAM)mWorkspaces[i]->GetWindow(), 0);
      return false;
    }
  }
  ...
  if (workspace->LoadFile(filename, &error)) {
    ...
    child = CreateMDIWindow("QUAKE4_GUIEDITOR_CHILD_CLASS", "Unamed.gui",
                            WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN |
                                WS_HSCROLL | WS_VSCROLL | WS_MAXIMIZE,
                            CW_USEDEFAULT, CW_USEDEFAULT, 640, 480, mMDIClient,
                            mInstance, (LONG)workspace);

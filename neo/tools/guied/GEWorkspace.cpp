...
static float g_ZoomScales[rvGEWorkspace::ZOOM_MAX] = {
    0, 0.25f, 0.33f, 0.50f, 0.66f, 1.0f, 1.5f, 2.0f, 3.0f};
...
rvGEWorkspace::~rvGEWorkspace() {
  ...
  rvGEWindowWrapper::GetWrapper(mInterface->GetDesktop())
      ->EnumChildren(CleanupEnumProc, NULL);
...
void rvGEWorkspace::Render(HDC hdc) {
  ...
  qglColor3f(mApplication->GetOptions().GetWorkspaceColor()[0],
             mApplication->GetOptions().GetWorkspaceColor()[1],
             mApplication->GetOptions().GetWorkspaceColor()[2]);
  qglBegin(GL_QUADS);
...
void rvGEWorkspace::UpdateTitle(void) {
  ...
  SetWindowText(mWnd,
                va("%s%s (%d%%)", idStr(mFilename).StripPath().c_str(),
                   mModified ? "*" : "", (int)(g_ZoomScales[mZoom] * 100)));
...
void rvGEWorkspace::UpdateScrollbars(void) {
  ...
  info.nMin = -info.nMax;
  info.nPage =
      (int)((float)info.nMax * (float)((float)mWindowHeight / mRect.h));
  ...
  switch (msg) {
    case WM_CLOSE: {
      if (IsModified()) {
        if (IDYES ==
            gApp.MessageBox(
                va("Save changes to the document \"%s\" before closing?",
                   GetFilename()),
                MB_YESNO | MB_ICONQUESTION)) {
          SendMessage(mApplication->GetMDIFrame(), WM_COMMAND,
                      MAKELONG(ID_GUIED_FILE_SAVE, 0), 0);
        }
      }
...
int rvGEWorkspace::HandleCommand(WPARAM wParam, LPARAM lParam) {
  // Select command
  if (LOWORD(wParam) >= ID_GUIED_SELECT_FIRST &&
      LOWORD(wParam) <= ID_GUIED_SELECT_LAST) {
...
int rvGEWorkspace::HandleRButtonDown(WPARAM wParam, LPARAM lParam) {
  ...
  rvGEWindowWrapper::GetWrapper(mInterface->GetDesktop())
      ->EnumChildren(BuildSelectMenuEnumProc, this);

  // Add the desktop window always
  mSelectMenu.Append(mInterface->GetDesktop());

  //
  menu = GetSubMenu(LoadMenu(mApplication->GetInstance(),
                             MAKEINTRESOURCE(IDR_GUIED_ITEM_POPUP)),
                    0);
  ...
  for (i = 0; i < mSelectMenu.Num(); i++) {
    ...
    AppendMenu(
        popup,
        MF_STRING | MF_ENABLED | (wrapper->IsSelected() ? MF_CHECKED : 0),
        ID_GUIED_SELECT_FIRST + i, mSelectMenu[i]->GetName());
  }
  ...
  TrackPopupMenu(menu, TPM_RIGHTBUTTON | TPM_LEFTALIGN, point.x, point.y, 0,
                 mWnd, NULL);
...
int rvGEWorkspace::HandleLButtonDown(WPARAM wParam, LPARAM lParam) {
  ...
  if (gApp.GetOptions().GetIgnoreDesktopSelect() &&
      window == mInterface->GetDesktop()) {
    window = NULL;
  }

  if (mDragType == rvGESelectionMgr::HT_MOVE ||
      mDragType == rvGESelectionMgr::HT_NONE) {
    if (window) {
      bool selected;
...
int rvGEWorkspace::HandleMouseMove(WPARAM wParam, LPARAM lParam) {
  ...
  // so
  if (mDragType == rvGESelectionMgr::HT_MOVE &&
      Sys_Milliseconds() - mDragTime <= 50) {
    return 0;
  }

  // Handle grid snapping
  if (gApp.GetOptions().GetGridSnap()) {
    cursor.x = (float)(((int)cursor.x + gApp.GetOptions().GetGridWidth() / 2) /
                       gApp.GetOptions().GetGridWidth() *
                       gApp.GetOptions().GetGridWidth());
    cursor.y = (float)(((int)cursor.y + gApp.GetOptions().GetGridWidth() / 2) /
                       gApp.GetOptions().GetGridWidth() *
                       gApp.GetOptions().GetGridWidth());
  }

  // If the cursor hasnt moved then there is nothing to update with the drag
  if ((int)cursor.x == (int)mDragPoint.x &&
      (int)cursor.y == (int)mDragPoint.y) {
    return 0;
  }

  switch (mDragType) {
    case rvGESelectionMgr::HT_MOVE:
      AddModifierMove("Move", cursor.x - mDragPoint.x, cursor.y - mDragPoint.y,
                      mApplication->GetOptions().GetGridSnap());
      break;

    case rvGESelectionMgr::HT_SIZE_BOTTOM:
      AddModifierSize("Size", 0, 0, 0, cursor.y - mDragPoint.y,
                      mApplication->GetOptions().GetGridSnap());
      break;

    case rvGESelectionMgr::HT_SIZE_TOP:
      AddModifierSize("Size", 0, cursor.y - mDragPoint.y, 0, 0,
                      mApplication->GetOptions().GetGridSnap());
      break;

    case rvGESelectionMgr::HT_SIZE_RIGHT:
      AddModifierSize("Size", 0, 0, cursor.x - mDragPoint.x, 0,
                      mApplication->GetOptions().GetGridSnap());
      break;

    case rvGESelectionMgr::HT_SIZE_LEFT:
      AddModifierSize("Size", cursor.x - mDragPoint.x, 0, 0, 0,
                      mApplication->GetOptions().GetGridSnap());
      break;

    case rvGESelectionMgr::HT_SIZE_TOPLEFT:
      AddModifierSize("Size", cursor.x - mDragPoint.x, cursor.y - mDragPoint.y,
                      0, 0, mApplication->GetOptions().GetGridSnap());
      break;

    case rvGESelectionMgr::HT_SIZE_TOPRIGHT:
      AddModifierSize("Size", 0, cursor.y - mDragPoint.y,
                      cursor.x - mDragPoint.x, 0,
                      mApplication->GetOptions().GetGridSnap());
      break;

    case rvGESelectionMgr::HT_SIZE_BOTTOMLEFT:
      AddModifierSize("Size", cursor.x - mDragPoint.x, 0, 0,
                      cursor.y - mDragPoint.y,
                      mApplication->GetOptions().GetGridSnap());
      break;

    case rvGESelectionMgr::HT_SIZE_BOTTOMRIGHT:
      AddModifierSize("Size", 0, 0, cursor.x - mDragPoint.x,
                      cursor.y - mDragPoint.y,
                      mApplication->GetOptions().GetGridSnap());
      break;
  }
...
rvGEModifier* rvGEWorkspace::CreateModifier(EModifierType type,
  ...
  switch (type) {
    ...
    case MOD_SEND_BACKWARD:
      mod = new rvGEZOrderModifier("Send Backward", window,
                                   rvGEZOrderModifier::ZO_BACKWARD);
      break;

    case MOD_SEND_BACK:
      mod = new rvGEZOrderModifier("Send to Back", window,
                                   rvGEZOrderModifier::ZO_BACK);
      break;

    case MOD_BRING_FORWARD:
      mod = new rvGEZOrderModifier("Bring Forward", window,
                                   rvGEZOrderModifier::ZO_FORWARD);
      break;

    case MOD_BRING_FRONT:
      mod = new rvGEZOrderModifier("Bring to Front", window,
                                   rvGEZOrderModifier::ZO_FRONT);
      break;
...
bool rvGEWorkspace::BuildSelectMenuEnumProc(rvGEWindowWrapper* wrapper,
  ...
  if (wrapper->GetScreenRect().Contains(workspace->mSelectMenuPos[0],
                                        workspace->mSelectMenuPos[1])) {
...
bool rvGEWorkspace::ShowAllEnumProc(rvGEWindowWrapper* wrapper, void* data) {
  ...
  if (wrapper->IsHidden()) {
    group->Append(
        new rvGEHideModifier("Show Hidden", wrapper->GetWindow(), false));
  }
...
void rvGEWorkspace::AddModifierShowAll(void) {
  ...
  rvGEWindowWrapper::GetWrapper(mInterface->GetDesktop())
      ->EnumChildren(ShowAllEnumProc, group);
...
idWindow* rvGEWorkspace::NewWindow(idDict* state,
  ...
  idWindow* window =
      new idWindow(mInterface->GetDesktop()->GetDC(), mInterface);
  rvGEWindowWrapper* wrapper;
  int count;
  idStr baseName;
  ...
  switch (type) {
    ...
    case rvGEWindowWrapper::WT_RENDER:
      window =
          new idRenderWindow(mInterface->GetDesktop()->GetDC(), mInterface);
      break;

    case rvGEWindowWrapper::WT_CHOICE:
      window =
          new idChoiceWindow(mInterface->GetDesktop()->GetDC(), mInterface);
      break;
  ...
  if (mInterface->GetDesktop()->FindChildByName(baseName)) {
    count = 1;
    while (1) {
      drawWin_t* dw = mInterface->GetDesktop()->FindChildByName(
          va("%s%d", baseName.c_str(), count));
      if (!dw) {
        break;
      }
      assert(dw->win);
  ...
  idParser src(winTemplate, winTemplate.Length(), "",
               LEXFL_ALLOWMULTICHARLITERALS | LEXFL_NOSTRINGCONCAT |
...
idWindow* rvGEWorkspace::AddWindow(rvGEWindowWrapper::EWindowType type) {
  ...
  mModifiers.Append(
      new rvGEInsertModifier("New", window, mInterface->GetDesktop(), NULL));
...
bool rvGEWorkspace::EditSelectedProperties(void) {
  ...
  if (GEItemPropsDlg_DoModal(mWnd, mSelections[0], dict)) {
    mModifiers.Append(
        new rvGEStateModifier("Item Properties", mSelections[0], dict));
    SetModified(true);
  }
...
void rvGEWorkspace::MakeSelectedSameSize(bool changeWidth, bool changeHeight) {
  ...
  for (i = 1; i < mSelections.Num(); i++) {
    ...
    group->Append(new rvGESizeModifier("Make Same Size", mSelections[i], 0, 0,
                                       width, height));
  }
...
void rvGEWorkspace::AlignSelected(EItemAlign align) {
  static const char* alignNames[] = {"Lefts", "Centers", "Rights",
...
void rvGEWorkspace::AddModifierMove(const char* modName,
  ...
  for (int i = 0; i < mSelections.Num(); i++) {
    ...
    // one.
    if (rvGEWindowWrapper::GetWrapper(mSelections[i]->GetParent())
            ->IsSelected()) {
    ...
    group->Append(new rvGEMoveModifier(modName, mSelections[i],
                                       newRect.x - scaleRect.x,
                                       newRect.y - scaleRect.y));
  }
...
void rvGEWorkspace::AddModifierSize(const char* modName,
  ...
  if (snap) {
    gApp.GetOptions().SnapRectToGrid(newRect, l != 0.0f, t != 0.0f, r != 0.0f,
                                     b != 0.0f);
  }
  ...
  for (int i = 0; i < mSelections.Num(); i++) {
    ...
    l = (newRect.x + ((sizeRect.x - scaleRect.x) / scaleRect.w) * newRect.w) -
        sizeRect.x;
    t = (newRect.y + ((sizeRect.y - scaleRect.y) / scaleRect.h) * newRect.h) -
        sizeRect.y;
    ...
    while (NULL != (parent = parent->GetParent())) {
      ...
      offset = ((newRect.x +
                 ((sizeRect.x - scaleRect.x) / scaleRect.w) * newRect.w) -
                sizeRect.x);
      l -= offset;
      r -= offset;
      ...
      offset = ((newRect.y +
                 ((sizeRect.y - scaleRect.y) / scaleRect.h) * newRect.h) -
                sizeRect.y);
      t -= offset;
      b -= offset;
...
void rvGEWorkspace::MakeSelectedAChild(void) {
  ...
  if (!rvGEWindowWrapper::GetWrapper(mSelections[0])->CanHaveChildren()) {
    gApp.MessageBox("Cannot add children to an htmlDef item",
                    MB_OK | MB_ICONERROR);
    return;
  }
  ...
  for (i = 1; i < mSelections.Num(); i++) {
    ...
    group->Append(new rvGEInsertModifier("Make Child", mSelections[i],
                                         mSelections[0], NULL));
  }
...
void rvGEWorkspace::Copy(void) {
  ...
  for (i = 0; i < mSelections.Num(); i++) {
    ...
    item->mStateDict.Set("windowType", rvGEWindowWrapper::WindowTypeToString(
                                           wrapper->GetWindowType()));
...
void rvGEWorkspace::Paste(void) {
  ...
  for (i = 0; i < mClipboard.Num(); i++) {
    ...
    type = rvGEWindowWrapper::StringToWindowType(
        state.GetString("windowType", "windowDef"));
    state.Delete("windowType");

    idWindow* window = NewWindow(&state, type);
    group->Append(new rvGEInsertModifier("Paste", window,
                                         mInterface->GetDesktop(), NULL));
    mSelections.Add(window);

    rvGEWindowWrapper::GetWrapper(window)->GetScriptDict() =
        mClipboard[i]->mScriptDict;
    rvGEWindowWrapper::GetWrapper(window)->GetVariableDict() =
        mClipboard[i]->mVarDict;
  }

...
bool rvGETransformer::Create(HWND parent, bool visible) {
  ...
  mWnd = CreateWindowEx(WS_EX_TOOLWINDOW, "GUIEDITOR_TRANSFORMER_CLASS",
                        "Transformer",
                        WS_SYSMENU | WS_CAPTION | WS_POPUP | WS_OVERLAPPED |
                            WS_BORDER | WS_CLIPSIBLINGS | WS_CHILD,
                        0, 0, 200, 100, parent, NULL, win32.hInstance, this);
  ...
  if (!gApp.GetOptions().GetWindowPlacement("transformer", mWnd)) {
    ...
    SetWindowPos(mWnd, NULL, rParent.right - 10 - (rTrans.right - rTrans.left),
                 rParent.bottom - 10 - (rTrans.bottom - rTrans.top), 0, 0,
...
LRESULT CALLBACK rvGETransformer::WndProc(HWND hWnd,
  ...
  switch (msg) {
    ...
    case WM_CREATE: {
      ...
      trans->mWnd = hWnd;
      trans->mDlg = CreateDialogParam(gApp.GetInstance(),
                                      MAKEINTRESOURCE(IDD_GUIED_TRANSFORMER),
...
INT_PTR CALLBACK rvGETransformer::DlgProc(HWND hWnd,
  ...
  switch (msg) {
    ...
    case WM_INITDIALOG:
      ...
      gTransDlg = hWnd;
      gTransHook = SetWindowsHookEx(WH_GETMESSAGE, GetMsgProc, NULL,
                                    GetCurrentThreadId());
      break;
    ...
    case WM_COMMAND:
      if (LOWORD(wParam) == IDOK) {
        ...
        // The transformer coords are relative to the botto most selected
        // window's parent so adjust the rect accordingly
        if (trans->mRelative) {
          idRectangle& screenRect =
              rvGEWindowWrapper::GetWrapper(trans->mRelative)->GetScreenRect();
          rect.x -= screenRect.x;
          rect.y -= screenRect.y;
        }
        ...
        switch (LOWORD(wParam)) {
          case IDC_GUIED_ITEMRECTX:
            if (value - rect[0]) {
              trans->mWorkspace->AddModifierMove("Transform Move",
                                                 value - rect[0], 0, false);
            }
            break;

          case IDC_GUIED_ITEMRECTY:
            if (value - rect[1]) {
              trans->mWorkspace->AddModifierMove("Transform Move", 0,
                                                 value - rect[1], false);
            }
            break;
          ...
          case IDC_GUIED_ITEMRECTW:
            if (value - rect[2]) {
              trans->mWorkspace->AddModifierSize("Transform Size", 0, 0,
                                                 value - rect[2], 0, false);
            }
            break;

          case IDC_GUIED_ITEMRECTH:
            if (value - rect[3]) {
              trans->mWorkspace->AddModifierSize("Transform Size", 0, 0, 0,
                                                 value - rect[3], false);
            }
            break;
        }
...
void rvGETransformer::Update(void) {
  ...
  if (mWorkspace && mWorkspace->GetSelectionMgr().Num()) {
    ...
    if (mRelative) {
      idRectangle& screenRect =
          rvGEWindowWrapper::GetWrapper(mRelative)->GetScreenRect();
      rect.x -= screenRect.x;
      rect.y -= screenRect.y;
    }

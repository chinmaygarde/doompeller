...
void rvGEWindowWrapper::SetRect(idRectangle& rect) {
  ...
  s = va("%d,%d,%d,%d", (int)(rect.x + 0.5f), (int)(rect.y + 0.5f),
         (int)((rect.w + 0.5f) * (mFlippedHorz ? -1 : 1)),
         (int)((rect.h + 0.5f) * (mFlippedVert ? -1 : 1)));
...
================
*/
rvGEWindowWrapper::EWindowType rvGEWindowWrapper::StringToWindowType(
    const char* string) {
...
const char* rvGEWindowWrapper::WindowTypeToString(EWindowType type) {
  static const char* typeNames[] = {"Unknown", "windowDef", "editDef",
                                    "htmlDef", "choiceDef", "sliderDef",
                                    "bindDef", "listDef",   "renderDef"};
...
int rvGEWindowWrapper::GetDepth(void) {
  ...
  for (depth = 0, parent = mWindow->GetParent(); parent;
...
void rvGEWindowWrapper::Finish(void) {
  mOldVisible =
      ((bool)*dynamic_cast<idWinBool*>(mWindow->GetWinVarByName("visible")));
...
bool rvGEWindowWrapper::VerfiyStateKey(const char* name,
  ...
  idStr old;
  bool failed;
  idParser src(value, strlen(value), "",
               LEXFL_NOSTRINGCONCAT | LEXFL_ALLOWMULTICHARLITERALS |

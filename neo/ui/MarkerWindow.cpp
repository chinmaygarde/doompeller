...
idMarkerWindow::idMarkerWindow(idDeviceContext* d, idUserInterfaceLocal* g)
    : idWindow(d, g) {
  dc = d;
  gui = g;
  CommonInit();
...
const char* idMarkerWindow::HandleEvent(const sysEvent_t* event,
  ...
  if (event->evValue2 && key == K_MOUSE1) {
    ...
    for (i = 0; i < c; i++) {
      ...
      if (md.rect.Contains(gui->CursorX(), gui->CursorY())) {
        ...
        gui->GetDesktop()->SetChildWinVarVal(
            "markerText", "text",
            va("Marker set at %.2i:%.2i", md.time / 60 / 60,
               (md.time / 60) % 60));
        gui->GetDesktop()->SetChildWinVarVal("markerText", "visible", "1");
        gui->GetDesktop()->SetChildWinVarVal("markerBackground", "matcolor",
                                             "1 1 1 1");
        gui->GetDesktop()->SetChildWinVarVal("markerBackground", "text", "");
        gui->GetDesktop()->SetChildWinVarVal("markerBackground", "background",
                                             md.mat->GetName());
        break;
      }
    }
    if (i == c) {
      ...
      gui->GetDesktop()->SetChildWinVarVal(
          "markerText", "text",
          va("Marker set at %.2i:%.2i", currentTime / 60 / 60,
             (currentTime / 60) % 60));
      gui->GetDesktop()->SetChildWinVarVal("markerText", "visible", "1");
      gui->GetDesktop()->SetChildWinVarVal("markerBackground", "matcolor",
                                           "0 0 0 0");
...
void idMarkerWindow::Draw(int time, float x, float y) {
  ...
  if (numStats > 1) {
    ...
    if (c > 0) {
      for (int i = 0; i < c; i++) {
        ...
        dc->DrawMaterial(md.rect.x, md.rect.y, md.rect.w, md.rect.h, markerMat,
                         markerColor);
      }
    }
  }
  ...
  if (r.w > 0 && r.Contains(gui->CursorX(), gui->CursorY())) {
    ...
    currentTime = len * pct;
    r.x = (gui->CursorX() > r.x + r.w - 40) ? gui->CursorX() - 40
                                            : gui->CursorX();
    r.y = gui->CursorY() - 15;
    r.w = 40;
    r.h = 20;
    dc->DrawText(
        va("%.2i:%.2i", currentTime / 60 / 60, (currentTime / 60) % 60), 0.25,
        0, idDeviceContext::colorWhite, r, false);
  }

...
void SSDCrossHair::InitCrosshairs() {
  crosshairMaterial[CROSSHAIR_STANDARD] =
      declManager->FindMaterial(CROSSHAIR_STANDARD_MATERIAL);
  crosshairMaterial[CROSSHAIR_SUPER] =
      declManager->FindMaterial(CROSSHAIR_SUPER_MATERIAL);
...
void SSDCrossHair::Draw(idDeviceContext* dc, const idVec2& cursor) {
  ...
  dc->DrawMaterial(x, y, crosshairWidth, crosshairHeight,
...
void SSDEntity::Draw(idDeviceContext* dc) {
  ...
  idBounds bounds;
  bounds[0] = idVec3(position.x - (size.x / 2.0f), position.y - (size.y / 2.0f),
                     position.z);
  bounds[1] = idVec3(position.x + (size.x / 2.0f), position.y + (size.y / 2.0f),
                     position.z);
  ...
  x = screenBounds[0].x;
  y = screenBounds[1].y;
  dc->DrawMaterialRotated(x, y, persize.x, persize.y, material, matColor, 1.0f,
                          1.0f, DEG2RAD(rotation));
...
SSDAsteroid* SSDAsteroid::GetNewAsteroid(idGameSSDWindow* _game,
  ...
  for (int i = 0; i < MAX_ASTEROIDS; i++) {
    if (!asteroidPool[i].inUse) {
      asteroidPool[i].Init(_game, startPosition, _size, _speed, rotate,
                           _health);
...
const char* explosionMaterials[] = {"game/SSD/fball", "game/SSD/teleport"};
...
SSDExplosion* SSDExplosion::GetNewExplosion(idGameSSDWindow* _game,
  ...
  for (int i = 0; i < MAX_EXPLOSIONS; i++) {
    if (!explosionPool[i].inUse) {
      explosionPool[i].Init(_game, _position, _size, _length, _type, _buddy,
                            _killBuddy, _followBuddy);
...
SSDProjectile* SSDProjectile::GetNewProjectile(idGameSSDWindow* _game,
  ...
  for (int i = 0; i < MAX_PROJECTILES; i++) {
    if (!projectilePool[i].inUse) {
      projectilePool[i].Init(_game, _beginPosition, _endPosition, _speed,
                             _size);
...
void SSDPowerup::OnHit(int key) {
  if (powerupState == POWERUP_STATE_CLOSED) {
    ...
    SSDExplosion* explosion = SSDExplosion::GetNewExplosion(
        game, position, size * 2.0f, 300, SSDExplosion::EXPLOSION_NORMAL, this,
        false, true);
    ...
    SSDExplosion* explosion = SSDExplosion::GetNewExplosion(
        game, position, size * 2, 300, SSDExplosion::EXPLOSION_NORMAL, this);
    game->entities.Append(explosion);
...
idGameSSDWindow::idGameSSDWindow(idDeviceContext* d, idUserInterfaceLocal* g)
    : idWindow(d, g) {
  dc = d;
  gui = g;
  CommonInit();
...
void idGameSSDWindow::ParseLevelData(int level, const idStr& levelDataString) {
  idParser parser;
  idToken token;
  parser.LoadMemory(levelDataString.c_str(), levelDataString.Length(),
                    "LevelData");
...
void idGameSSDWindow::ParseAsteroidData(int level,
  ...
  idParser parser;
  idToken token;
  parser.LoadMemory(asteroidDataString.c_str(), asteroidDataString.Length(),
                    "AsteroidData");
  ...
  asteroidData[level].rotateMin =
      parser.ParseFloat();  // Rotate Min (rotations per second)
  asteroidData[level].rotateMax =
      parser.ParseFloat();  // Rotate Max (rotations per second)

  asteroidData[level].spawnMin = parser.ParseInt();  // Spawn Min
  asteroidData[level].spawnMax = parser.ParseInt();  // Spawn Max

  asteroidData[level].asteroidHealth =
      parser.ParseInt();  // Health of the asteroid
  asteroidData[level].asteroidDamage = parser.ParseInt();  // Asteroid Damage
  asteroidData[level].asteroidPoints =
      parser.ParseInt();  // Points awarded for destruction
}
...
void idGameSSDWindow::ParsePowerupData(int level,
  ...
  idParser parser;
  idToken token;
  parser.LoadMemory(powerupDataString.c_str(), powerupDataString.Length(),
                    "PowerupData");

  powerupData[level].speedMin = parser.ParseFloat();  // Speed Min
  powerupData[level].speedMax = parser.ParseFloat();  // Speed Max

  powerupData[level].rotateMin =
      parser.ParseFloat();  // Rotate Min (rotations per second)
  powerupData[level].rotateMax =
      parser.ParseFloat();  // Rotate Max (rotations per second)
...
void idGameSSDWindow::ParseWeaponData(int weapon,
  ...
  idParser parser;
  idToken token;
  parser.LoadMemory(weaponDataString.c_str(), weaponDataString.Length(),
                    "WeaponData");
...
void idGameSSDWindow::ParseAstronautData(int level,
  ...
  idParser parser;
  idToken token;
  parser.LoadMemory(astronautDataString.c_str(), astronautDataString.Length(),
                    "AstronautData");

  astronautData[level].speedMin = parser.ParseFloat();  // Speed Min
  astronautData[level].speedMax = parser.ParseFloat();  // Speed Max

  astronautData[level].rotateMin =
      parser.ParseFloat();  // Rotate Min (rotations per second)
  astronautData[level].rotateMax =
      parser.ParseFloat();  // Rotate Max (rotations per second)
...
void idGameSSDWindow::LevelComplete() {
  ...
  if (!gameStats.levelStats.shotCount) {
    ...
    accuracy = (int)(((float)gameStats.levelStats.hitCount /
                      (float)gameStats.levelStats.shotCount) *
                     100.0f);
  }
  ...
  int saveAccuracy;
  int totalAst = gameStats.levelStats.savedAstronauts +
                 gameStats.levelStats.killedAstronauts;
  if (!totalAst) {
    saveAccuracy = 0;
  } else {
    saveAccuracy =
        (int)(((float)gameStats.levelStats.savedAstronauts / (float)totalAst) *
              100.0f);
  }
...
void idGameSSDWindow::SpawnAsteroid() {
  ...
  startPosition.x = random.RandomInt(V_WIDTH + spawnBuffer) -
                    ((V_WIDTH / 2.0f) + spawnBuffer);
  startPosition.y = random.RandomInt(V_HEIGHT + spawnBuffer) -
                    ((V_HEIGHT / 2.0f) + spawnBuffer);
  startPosition.z = ENTITY_START_DIST;

  float speed =
      random.RandomInt(asteroidData[gameStats.currentLevel].speedMax -
                       asteroidData[gameStats.currentLevel].speedMin) +
      asteroidData[gameStats.currentLevel].speedMin;
  float size = random.RandomInt(asteroidData[gameStats.currentLevel].sizeMax -
                                asteroidData[gameStats.currentLevel].sizeMin) +
               asteroidData[gameStats.currentLevel].sizeMin;
  float rotate = (random.RandomFloat() *
                  (asteroidData[gameStats.currentLevel].rotateMax -
                   asteroidData[gameStats.currentLevel].rotateMin)) +
                 asteroidData[gameStats.currentLevel].rotateMin;

  SSDAsteroid* asteroid = SSDAsteroid::GetNewAsteroid(
      this, startPosition, idVec2(size, size), speed, rotate,
      asteroidData[gameStats.currentLevel].asteroidHealth);
  entities.Append(asteroid);

  gameStats.levelStats.nextAsteroidSpawnTime =
      currentTime +
      random.RandomInt(asteroidData[gameStats.currentLevel].spawnMax -
  ...
  if (key == K_MOUSE1) {
    ...
    if (gameStats.levelStats.targetEnt) {
      ...
      if (gameStats.levelStats.targetEnt->type == SSD_ENTITY_ASTEROID) {
        HitAsteroid(static_cast<SSDAsteroid*>(gameStats.levelStats.targetEnt),
                    key);
      ...
      idVec3 vec =
          idVec3(cursorWorld.x, cursorWorld.y, (Z_FAR - Z_NEAR) / 8.0f);
      vec *= 8;
      SSDProjectile* newProj = SSDProjectile::GetNewProjectile(
          this, idVec3(0, -180, 0), vec,
          weaponData[gameStats.currentWeapon].speed,
          weaponData[gameStats.currentWeapon].size);
      entities.Append(newProj);
    }
...
void idGameSSDWindow::HitAsteroid(SSDAsteroid* asteroid, int key) {
  ...
  if (asteroid->health <= 0) {
    ...
    SSDExplosion* explosion = SSDExplosion::GetNewExplosion(
        this, asteroid->position, asteroid->size * 2, 300,
        SSDExplosion::EXPLOSION_NORMAL, asteroid);
    ...
    SSDExplosion* explosion = SSDExplosion::GetNewExplosion(
        this, asteroid->position, asteroid->size / 2.0f, 200,
        SSDExplosion::EXPLOSION_NORMAL, asteroid, false, false);
    entities.Append(explosion);
  }
}
...
void idGameSSDWindow::AddScore(SSDEntity* ent, int points) {
  ...
  if (points > 0) {
    pointsEnt = SSDPoints::GetNewPoints(this, ent, points, 1000, 50,
                                        idVec4(0, 1, 0, 1));
  } else {
    pointsEnt = SSDPoints::GetNewPoints(this, ent, points, 1000, 50,
                                        idVec4(1, 0, 0, 1));
...
void idGameSSDWindow::OnNuke() {
  ...
  for (int i = 0; i < entities.Num(); i++) {
    if (entities[i]->type == SSD_ENTITY_ASTEROID) {
      // The asteroid has been destroyed
      SSDExplosion* explosion = SSDExplosion::GetNewExplosion(
          this, entities[i]->position, entities[i]->size * 2, 300,
          SSDExplosion::EXPLOSION_NORMAL, entities[i]);
      entities.Append(explosion);

      AddScore(entities[i],
               asteroidData[gameStats.currentLevel].asteroidPoints);
...
void idGameSSDWindow::RefreshGuiData() {
  ...
  if (!gameStats.levelStats.shotCount) {
    ...
    accuracy = ((float)gameStats.levelStats.hitCount /
                (float)gameStats.levelStats.shotCount) *
               100.0f;
  }
  ...
  if (!totalAst) {
    ...
    saveAccuracy =
        ((float)gameStats.levelStats.savedAstronauts / (float)totalAst) *
        100.0f;
  }
  ...
  gui->SetStateString("player_health", va("%i", gameStats.health));
  gui->SetStateString("player_score", va("%i", gameStats.score));
  gui->SetStateString("player_prebonusscore",
                      va("%i", gameStats.prebonusscore));
  gui->SetStateString("level_complete",
                      va("%i/%i", gameStats.levelStats.savedAstronauts,
...
void idGameSSDWindow::SpawnAstronaut() {
  ...
  float speed =
      random.RandomInt(astronautData[gameStats.currentLevel].speedMax -
                       astronautData[gameStats.currentLevel].speedMin) +
      astronautData[gameStats.currentLevel].speedMin;
  float rotate = (random.RandomFloat() *
                  (astronautData[gameStats.currentLevel].rotateMax -
                   astronautData[gameStats.currentLevel].rotateMin)) +
                 astronautData[gameStats.currentLevel].rotateMin;

  SSDAstronaut* astronaut = SSDAstronaut::GetNewAstronaut(
      this, startPosition, speed, rotate,
      astronautData[gameStats.currentLevel].health);
  entities.Append(astronaut);

  gameStats.levelStats.nextAstronautSpawnTime =
      currentTime +
      random.RandomInt(astronautData[gameStats.currentLevel].spawnMax -
...
void idGameSSDWindow::HitAstronaut(SSDAstronaut* astronaut, int key) {
  if (key == K_MOUSE1) {
    ...
    if (astronaut->health <= 0) {
      ...
      SSDExplosion* explosion = SSDExplosion::GetNewExplosion(
          this, astronaut->position, astronaut->size * 2, 300,
          SSDExplosion::EXPLOSION_NORMAL, astronaut);
      entities.Append(explosion);
      PlaySound("arcade_explode");
      ...
      // This was a damage hit so create a real small quick explosion
      SSDExplosion* explosion = SSDExplosion::GetNewExplosion(
          this, astronaut->position, astronaut->size / 2.0f, 200,
          SSDExplosion::EXPLOSION_NORMAL, astronaut, false, false);
      entities.Append(explosion);
    }
  }
}
...
void idGameSSDWindow::AstronautStruckPlayer(SSDAstronaut* astronaut) {
  ...
  SSDExplosion* explosion = SSDExplosion::GetNewExplosion(
      this, astronaut->position, astronaut->size * 2, 300,
      SSDExplosion::EXPLOSION_TELEPORT, astronaut);
  entities.Append(explosion);
  ...
  if (gameStats.levelStats.savedAstronauts >=
      levelData[gameStats.currentLevel].needToWin) {
    LevelComplete();
  }
}
...
void idGameSSDWindow::SpawnPowerup() {
  ...
  float speed = random.RandomInt(powerupData[gameStats.currentLevel].speedMax -
                                 powerupData[gameStats.currentLevel].speedMin) +
                powerupData[gameStats.currentLevel].speedMin;
  float rotate =
      (random.RandomFloat() * (powerupData[gameStats.currentLevel].rotateMax -
                               powerupData[gameStats.currentLevel].rotateMin)) +
      powerupData[gameStats.currentLevel].rotateMin;
  ...
  gameStats.levelStats.nextPowerupSpawnTime =
      currentTime +
      random.RandomInt(powerupData[gameStats.currentLevel].spawnMax -

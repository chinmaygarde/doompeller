...
idSliderWindow::idSliderWindow(idDeviceContext* d, idUserInterfaceLocal* g)
    : idWindow(d, g) {
  dc = d;
  gui = g;
  CommonInit();
...
idWinVar* idSliderWindow::GetWinVarByName(const char* _name,
                                          bool fixup,
                                          drawWin_t** owner) {
...
const char* idSliderWindow::HandleEvent(const sysEvent_t* event,
                                        bool* updateVisuals) {
  ...
  if (key == K_RIGHTARROW || key == K_KP_RIGHTARROW ||
      (key == K_MOUSE2 && gui->CursorY() > thumbRect.y)) {
...
void idSliderWindow::InitWithDefaults(const char* _name,
...
void idSliderWindow::Draw(int time, float x, float y) {
  ...
  dc->DrawMaterial(thumbRect.x, thumbRect.y, thumbRect.w, thumbRect.h, thumbMat,
                   color);
  if (flags & WIN_FOCUS) {
    dc->DrawRect(thumbRect.x + 1.0f, thumbRect.y + 1.0f, thumbRect.w - 2.0f,
                 thumbRect.h - 2.0f, 1.0f, color);
  }
}
...
void idSliderWindow::InitCvar() {
  if (cvarStr[0] == '\0') {
    if (!buddyWin) {
      common->Warning(
          "idSliderWindow::InitCvar: gui '%s' window '%s' has an empty cvar "
          "string",
  ...
  if (!cvar) {
    common->Warning(
        "idSliderWindow::InitCvar: gui '%s' window '%s' references undefined "
        "cvar '%s'",

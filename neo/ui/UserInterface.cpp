    ...
    if (guis[i]->GetRefs() == 0) {
      ...
      for (int j = 0; j < declManager->GetNumDecls(DECL_MATERIAL); j++) {
        const idMaterial* material = static_cast<const idMaterial*>(
            declManager->DeclByIndex(DECL_MATERIAL, j, false));
...
void idUserInterfaceManagerLocal::ListGuis() const {
  ...
  for (int i = 0; i < c; i++) {
    ...
    common->Printf("%6.1fk %4i (%s) %s ( %i transitions )\n", sz / 1024.0f,
                   guis[i]->GetRefs(), isUnique ? "unique" : "copy",
                   guis[i]->GetSourceFile(),
                   guis[i]->desktop->NumTransitions());
    total += sz;
  }
  common->Printf(
      "===========\n  %i total Guis ( %i copies, %i unique ), %.2f total "
      "Mbytes",
...
const char* idUserInterfaceLocal::GetStateString(
    const char* varName,

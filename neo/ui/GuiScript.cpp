...
bool idGuiScript::Parse(idParser* src) {
  ...
  if (handler && (parms.Num() < commandList[i].mMinParms ||
                  parms.Num() > commandList[i].mMaxParms)) {
    src->Error("incorrect number of parameters for script %s",
               commandList[i].name);
  }
  //
  ...
  if (handler == &Script_Set) {
    ...
    for (int i = 1; i < parmCount; i++) {
      ...
      if (idStr::Icmpn(*str, "gui::", 5) == 0) {
        ...
        dest = win->GetGui()->GetDesktop()->GetWinVarByName(
            (const char*)(*str) + 1, true);
        //
        if (dest) {
          delete parms[i].var;
          parms[i].var = dest;
      ...
      } else if (idStr::Cmpn(str->c_str(), STRTABLE_ID, STRTABLE_ID_LENGTH) ==
                 0) {
        ...
        // Search for "play <...>"
        idToken token;
        idParser parser(LEXFL_NOSTRINGCONCAT | LEXFL_ALLOWMULTICHARLITERALS |
    ...
    if (dest) {
      ...
      common->Warning(
          "Window %s in gui %s: a transition does not have a valid destination "
          "var %s",
    ...
    for (c = 1; c < 3; c++) {
      ...
      if (dest) {
        ...
        if (owner) {
          ownerparent =
              owner->simp ? owner->simp->GetParent() : owner->win->GetParent();
          destparent = destowner->simp ? destowner->simp->GetParent()
                                       : destowner->win->GetParent();

          // If its the rectangle they are referencing then adjust it
          if (ownerparent && destparent &&
              (dest == (owner->simp ? owner->simp->GetWinVarByName("rect")
                                    : owner->win->GetWinVarByName("rect")))) {
            idRectangle rect;

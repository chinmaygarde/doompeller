...
bool idWindow::registerIsTemporary
    [MAX_EXPRESSION_REGISTERS];  // statics to assist during parsing
...
const idRegEntry idWindow::RegisterVars[] = {
    {"forecolor", idRegister::VEC4},   {"hovercolor", idRegister::VEC4},
    {"backcolor", idRegister::VEC4},   {"bordercolor", idRegister::VEC4},
    {"rect", idRegister::RECTANGLE},   {"matcolor", idRegister::VEC4},
    {"scale", idRegister::VEC2},       {"translate", idRegister::VEC2},
    {"rotate", idRegister::FLOAT},     {"textscale", idRegister::FLOAT},
    {"visible", idRegister::BOOL},     {"noevents", idRegister::BOOL},
    {"text", idRegister::STRING},      {"background", idRegister::STRING},
    {"runscript", idRegister::STRING}, {"varbackground", idRegister::STRING},
    {"cvar", idRegister::STRING},      {"choices", idRegister::STRING},
    {"choiceVar", idRegister::STRING}, {"bind", idRegister::STRING},
    {"modelRotate", idRegister::VEC4}, {"modelOrigin", idRegister::VEC4},
    {"lightOrigin", idRegister::VEC4}, {"lightColor", idRegister::VEC4},
    {"viewOffset", idRegister::VEC4},  {"hideCursor", idRegister::BOOL}};

const int idWindow::NumRegisterVars = sizeof(RegisterVars) / sizeof(idRegEntry);

const char* idWindow::ScriptNames[] = {
    "onMouseEnter",    "onMouseExit", "onAction",      "onActivate",
    "onDeactivate",    "onESC",       "onEvent",       "onTrigger",
    "onActionRelease", "onEnter",     "onEnterRelease"};
...
void idWindow::Draw(int time, float x, float y) {
  ...
  if (textShadow) {
    ...
    dc->DrawText(shadowText, textScale, textAlign, colorBlack, shadowRect,
                 !(flags & WIN_NOWRAP), -1);
  }
  dc->DrawText(text, textScale, textAlign, foreColor, textRect,
               !(flags & WIN_NOWRAP), -1);

  if (gui_edit.GetBool()) {
    dc->EnableClipping(false);
    dc->DrawText(va("x: %i  y: %i", (int)rect.x(), (int)rect.y()), 0.25, 0,
                 dc->colorWhite, idRectangle(rect.x(), rect.y() - 15, 100, 20),
                 false);
    dc->DrawText(
        va("w: %i  h: %i", (int)rect.w(), (int)rect.h()), 0.25, 0,
        dc->colorWhite,
        idRectangle(rect.x() + rect.w(), rect.w() + rect.h() + 5, 100, 20),
        false);
...
const char* idWindow::RouteMouseCoords(float xd, float yd) {
  ...
  while (c > 0) {
    ...
    if (child->visible && !child->noEvents &&
        child->Contains(child->drawRect, gui->CursorX(), gui->CursorY())) {
      ...
      if (event->evValue == K_MOUSE1) {
        ...
        while (--c >= 0) {
          if (children[c]->visible &&
              children[c]->Contains(children[c]->drawRect, gui->CursorX(),
                                    gui->CursorY()) &&
              !(children[c]->noEvents)) {
            idWindow* child = children[c];
            if (event->evValue2) {
            ...
            if (child->Contains(child->clientRect, gui->CursorX(),
                                gui->CursorY())) {
              ...
              if (event->evValue2) {
                ...
                bool capture = true;
                if (capture &&
                    ((child->flags & WIN_MOVABLE) || gui_edit.GetBool())) {
                  SetCapture(child);
                }
                return "";
              } else {
        ...
        if (gui_edit.GetBool()) {
          int c = children.Num();
          for (int i = 0; i < c; i++) {
            if (children[i]->drawRect.Contains(gui->CursorX(),
                                               gui->CursorY())) {
              if (event->evValue2) {
        ...
        if (GetFocusedChild()) {
          const char* childRet =
              GetFocusedChild()->HandleEvent(event, updateVisuals);
          if (childRet && *childRet) {
            return childRet;
          }
...
void idWindow::DebugDraw(int time, float x, float y) {
  ...
  if (dc) {
    ...
    if (gui_debug.GetInteger() == 1) {
      dc->DrawRect(drawRect.x, drawRect.y, drawRect.w, drawRect.h, 1,
      ...
      sprintf(out, "Draw Rect: %0.1f, %0.1f, %0.1f, %0.1f\n", drawRect.x,
              drawRect.y, drawRect.w, drawRect.h);
      strcat(buff, out);
      sprintf(out, "Client Rect: %0.1f, %0.1f, %0.1f, %0.1f\n", clientRect.x,
              clientRect.y, clientRect.w, clientRect.h);
      strcat(buff, out);
  ...
  for (i = 0; i < c; i++) {
    ...
    if (data->interp.IsDone(gui->GetTime()) && data->data) {
      ...
      if (data->data) {
        ...
        common->Warning("Invalid transitional data for window %s in gui %s",
                        GetName(), gui->GetSourceFile());
      }
    }
  }
...
void idWindow::Time() {
  ...
  if (c > 0) {
    for (int i = 0; i < c; i++) {
      if (timeLineEvents[i]->pending &&
          gui->GetTime() - timeLine >= timeLineEvents[i]->time) {
...
float idWindow::EvalRegs(int test, bool force) {
  ...
  if (!force && test >= 0 && test < MAX_EXPRESSION_REGISTERS &&
      lastEval == this) {
    return regs[test];
  }
...
void idWindow::DrawBackground(const idRectangle& drawRect) {
  if (backColor.w()) {
    dc->DrawFilledRect(drawRect.x, drawRect.y, drawRect.w, drawRect.h,
                       backColor);
  }
  ...
  if (background && matColor.w()) {
    ...
    dc->DrawMaterial(drawRect.x, drawRect.y, drawRect.w, drawRect.h, background,
                     matColor, scalex, scaley);
  }
}
...
void idWindow::DrawBorderAndCaption(const idRectangle& drawRect) {
  if (flags & WIN_BORDER && borderSize && borderColor.w()) {
    dc->DrawRect(drawRect.x, drawRect.y, drawRect.w, drawRect.h, borderSize,
                 borderColor);
  }
}
...
void idWindow::CalcRects(float x, float y) {
  ...
  for (int i = 0; i < c; i++) {
    if (drawWindows[i].win) {
      drawWindows[i].win->CalcRects(clientRect.x + xOffset,
                                    clientRect.y + yOffset);
    }
  }
  drawRect.Offset(-x, -y);
  clientRect.Offset(-x, -y);
...
void idWindow::Redraw(float x, float y) {
  ...
  if (flags & WIN_SHOWTIME) {
    dc->DrawText(va(" %0.1f seconds\n%s", (float)(time - timeLine) / 1000,
                    gui->State().GetString("name")),
                 0.35f, 0, dc->colorWhite, idRectangle(100, 0, 80, 80), false);
  }

  if (flags & WIN_SHOWCOORDS) {
    dc->EnableClipping(false);
    sprintf(str, "x: %i y: %i  cursorx: %i cursory: %i", (int)rect.x(),
            (int)rect.y(), (int)gui->CursorX(), (int)gui->CursorY());
    dc->DrawText(str, 0.25f, 0, dc->colorWhite, idRectangle(0, 0, 100, 20),
                 false);
    dc->EnableClipping(true);
  }
  ...
  for (int i = 0; i < c; i++) {
    if (drawWindows[i].win) {
      drawWindows[i].win->Redraw(clientRect.x + xOffset,
                                 clientRect.y + yOffset);
    } else {
      drawWindows[i].simp->Redraw(clientRect.x + xOffset,
                                  clientRect.y + yOffset);
    }
  }
  ...
  if (gui_edit.GetBool() ||
      (flags & WIN_DESKTOP && !(flags & WIN_NOCURSOR) && !hideCursor &&
       (gui->Active() || (flags & WIN_MENUGUI)))) {
  ...
  if (gui_debug.GetInteger() && flags & WIN_DESKTOP) {
    ...
    dc->DrawText(str, 0.25, 0, dc->colorWhite, idRectangle(0, 0, 100, 20),
                 false);
    dc->DrawText(gui->GetSourceFile(), 0.25, 0, dc->colorWhite,
                 idRectangle(0, 20, 300, 20), false);
    dc->EnableClipping(true);
  }
...
void idWindow::SaveExpressionParseState() {
  ...
  memcpy(saveTemps, registerIsTemporary,
         MAX_EXPRESSION_REGISTERS * sizeof(bool));
}
...
void idWindow::RestoreExpressionParseState() {
  memcpy(registerIsTemporary, saveTemps,
         MAX_EXPRESSION_REGISTERS * sizeof(bool));
  Mem_Free(saveTemps);
}
...
idWinVar* idWindow::GetWinVarByName(const char* _name,
  ...
  for (int i = 0; i < c; i++) {
    if (idStr::Icmp(_name, (guiVar) ? va("%s", definedVars[i]->GetName())
                                    : definedVars[i]->GetName()) == 0) {
  ...
  while (token != "}") {
    ...
    //  added new onEvent
      ...
      if (com_editors & EDITOR_GUI) {
        ...
        // Save the script
        rvGEWindowWrapper::GetWrapper(this)->GetScriptDict().Set(
            va("onEvent %s", token.c_str()), out);
      }
#endif
      ...
      if (com_editors & EDITOR_GUI) {
        ...
        rvGEWindowWrapper::GetWrapper(this)->GetVariableDict().Set(
            va("definefloat\t\"%s\"", token.c_str()), str);
      }
#endif
      ...
      gui->GetDesktop()->regList.AddReg(work, idRegister::VEC4, src,
                                        gui->GetDesktop(), var);
      ...
      if (com_editors & EDITOR_GUI) {
        ...
        src->GetStringFromMarker(str, true);
        rvGEWindowWrapper::GetWrapper(this)->GetVariableDict().Set(
            va("definevec4\t\"%s\"", token.c_str()), str);
      }
#endif
    } else if (token == "float") {
      src->ReadToken(&token);
...
void idWindow::AddTransition(idWinVar* dest,
  ...
  data.data = dest;
  data.interp.Init(gui->GetTime(), accelTime * time, decelTime * time, time,
                   from, to);
...
int idWindow::ExpressionConstant(float f) {
  ...
  if (expressionRegisters.Num() == MAX_EXPRESSION_REGISTERS) {
    common->Warning("expressionConstant: gui %s hit MAX_EXPRESSION_REGISTERS",
                    gui->GetSourceFile());
    return 0;
  }
...
int idWindow::ExpressionTemporary() {
  if (expressionRegisters.Num() == MAX_EXPRESSION_REGISTERS) {
    common->Warning("expressionTemporary: gui %s hit MAX_EXPRESSION_REGISTERS",
                    gui->GetSourceFile());
    return 0;
  }
...
wexpOp_t* idWindow::ExpressionOp() {
  if (ops.Num() == MAX_EXPRESSION_OPS) {
    common->Warning("expressionOp: gui %s hit MAX_EXPRESSION_OPS",
                    gui->GetSourceFile());
    return &ops[0];
  }
  wexpOp_t wop;
...
int idWindow::ParseTerm(idParser* src, idWinVar* var, int component) {
  ...
  const idDeclTable* table = static_cast<const idDeclTable*>(
    ...
    switch (op->opType) {
      ...
      case WOP_TYPE_DIVIDE:
        if (registers[op->b] == 0.0f) {
          common->Warning("Divide by zero in window '%s' in %s", GetName(),
                          gui->GetSourceFile());
      ...
      case WOP_TYPE_TABLE: {
        const idDeclTable* table = static_cast<const idDeclTable*>(
            declManager->DeclByIndex(DECL_TABLE, op->a));
      ...
      case WOP_TYPE_COND:
        registers[op->c] =
            (registers[op->a]) ? registers[op->b] : registers[op->d];
        break;
      case WOP_TYPE_AND:
        registers[op->c] = registers[op->a] && registers[op->b];
        break;
...
void idWindow::ReadFromDemoFile(class idDemoFile* f, bool rebuild) {
  ...
  for (i = 0; i < c; i++) {
    ...
    f->ReadVec4(endValue);
    td.interp.Init(startTime, accelTime, decelTime,
                   accelTime + linearTime + decelTime, startValue, endValue);
  ...
  for (i = 0; i < timeLineEvents.Num(); i++) {
    if (timeLineEvents[i]) {
      savefile->Write(&timeLineEvents[i]->pending,
                      sizeof(timeLineEvents[i]->pending));
      savefile->Write(&timeLineEvents[i]->time,
                      sizeof(timeLineEvents[i]->time));
...
void idWindow::ReadFromSaveGame(idFile* savefile) {
  ...
  for (i = 0; i < timeLineEvents.Num(); i++) {
    if (timeLineEvents[i]) {
      savefile->Read(&timeLineEvents[i]->pending,
                     sizeof(timeLineEvents[i]->pending));
...
void idWindow::FixupTransitions() {
  int i, c = transitions.Num();
  for (i = 0; i < c; i++) {
    drawWin_t* dw = gui->GetDesktop()->FindChildByName(
        ((idWinStr*)transitions[i].data)->c_str());
...
bool idWindow::UpdateFromDictionary(idDict& dict) {
  ...
  for (i = 0; i < dict.GetNumKeyVals(); i++) {
    ...
    idParser src(kv->GetValue().c_str(), kv->GetValue().Length(), "",
                 LEXFL_NOFATALERRORS | LEXFL_NOSTRINGCONCAT |

...
void idChoiceWindow::InitVars() {
  if (cvarStr.Length()) {
    ...
    if (!cvar) {
      common->Warning(
          "idChoiceWindow::InitVars: gui '%s' window '%s' references undefined "
          "cvar '%s'",
...
idChoiceWindow::idChoiceWindow(idDeviceContext* d, idUserInterfaceLocal* g)
    : idWindow(d, g) {
  dc = d;
  gui = g;
  CommonInit();
...
const char* idChoiceWindow::HandleEvent(const sysEvent_t* event,
                                        bool* updateVisuals) {
  int key;
...
void idChoiceWindow::UpdateChoice() {
  ...
  if (choiceType == 0) {
    ...
    int i;
    for (i = 0; i < c; i++) {
      if (idStr::Icmp(cvarStr.c_str(),
                      (values.Num()) ? values[i] : choices[i]) == 0) {
        break;
      }
    }
    if (i == c) {
...
idWinVar* idChoiceWindow::GetWinVarByName(const char* _name,
                                          bool fixup,
                                          drawWin_t** owner) {
...
void idChoiceWindow::UpdateChoicesAndVals(void) {
  ...
  if (latchedChoices.Icmp(choicesStr)) {
    choices.Clear();
    src.FreeSource();
    src.SetFlags(LEXFL_NOFATALERRORS | LEXFL_ALLOWPATHNAMES |
  ...
  if (choiceVals.Length() && latchedVals.Icmp(choiceVals)) {
    values.Clear();
    src.FreeSource();
    src.SetFlags(LEXFL_ALLOWPATHNAMES | LEXFL_ALLOWMULTICHARLITERALS |
                 LEXFL_ALLOWBACKSLASHSTRINGCONCAT);
    ...
    if (choices.Num() != values.Num()) {
      common->Warning(
          "idChoiceWindow:: gui '%s' window '%s' has value count unequal to "
          "choices count",
...
void idChoiceWindow::Draw(int time, float x, float y) {
  ...
  dc->DrawText(choices[currentChoice], textScale, textAlign, color, textRect,
               false, -1);
}

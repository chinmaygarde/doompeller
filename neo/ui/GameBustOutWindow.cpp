...
collideDir_t BOBrick::checkCollision(idVec2 pos, idVec2 vel) {
  ...
  if (vel.y < 0 && pos.y > ptA.y) {
    if (pos.x > ptA.x && pos.x < ptB.x) {
      ...
      if ((idMath::Fabs(vec.y) > idMath::Fabs(vec.x)) &&
          (vec.LengthFast() < BALL_RADIUS)) {
        result = COLLIDE_DOWN;
      }
    }
  }
    ...
    if (result == COLLIDE_NONE) {
      ...
      if (vel.x > 0 && pos.x < ptA.x) {
        if (pos.y > ptA.y && pos.y < ptB.y) {
          ...
          if ((idMath::Fabs(vec.x) >= idMath::Fabs(vec.y)) &&
              (vec.LengthFast() < BALL_RADIUS)) {
            result = COLLIDE_LEFT;
          }
        }
...
void idGameBustOutWindow::LoadBoardFiles(void) {
  ...
  for (i = 0; i < numLevels; i++) {
    ...
    if (pic != NULL) {
      if (w != 9 || h != 12) {
        common->DWarning(
            "Hell Bust-Out level image not correct dimensions! (%d x %d)", w,
            h);
      }

...
idWinVar* idEditWindow::GetWinVarByName(const char* _name,
                                        bool fixup,
                                        drawWin_t** owner) {
...
idEditWindow::idEditWindow(idDeviceContext* d, idUserInterfaceLocal* g)
    : idWindow(d, g) {
  dc = d;
  gui = g;
  CommonInit();
...
void idEditWindow::Draw(int time, float x, float y) {
  ...
  dc->DrawText(buffer, scale, 0, color, rect, wrap,
               (flags & WIN_FOCUS) ? cursorPos : -1);
}
...
=============
*/
const char* idEditWindow::HandleEvent(const sysEvent_t* event,
                                      bool* updateVisuals) {
  ...
  if (event->evType == SE_CHAR) {
    ...
    if (key == 'h' - 'a' + 1 || key == K_BACKSPACE) {  // ctrl-h is backspace
      if (cursorPos > 0) {
        if (cursorPos >= len) {
          ...
          memmove(&buffer[cursorPos - 1], &buffer[cursorPos],
                  len + 1 - cursorPos);
          cursorPos--;
        }
    ...
    if (key == K_HOME) {
      if (idKeyInput::IsDown(K_CTRL) || cursorLine <= 0 ||
          (cursorLine >= breaks.Num())) {
        cursorPos = 0;
      } else {
    ...
    if (key == K_END) {
      if (idKeyInput::IsDown(K_CTRL) || (cursorLine < -1) ||
          (cursorLine >= breaks.Num() - 1)) {
        cursorPos = len;
      } else {
...
void idEditWindow::EnsureCursorVisible() {
  ...
  if (!wrap) {
    ...
    idRectangle rect = textRect;
    rect.w -= sizeBias;
    dc->DrawText(text, textScale, textAlign, colorWhite, rect, true,
                 (flags & WIN_FOCUS) ? cursorPos : -1, true, &breaks);
...
void idEditWindow::InitCvar() {
  if (cvarStr[0] == '\0') {
    if (text.GetName() == NULL) {
      common->Warning(
          "idEditWindow::InitCvar: gui '%s' window '%s' has an empty cvar "
          "string",
  ...
  if (!cvar) {
    common->Warning(
        "idEditWindow::InitCvar: gui '%s' window '%s' references undefined "
        "cvar '%s'",

...
struct idTimeLineEvent {
  idTimeLineEvent() { event = new idGuiScriptList; }
...
class rvNamedEvent {
 public:
  ...
  ~rvNamedEvent(void) { delete mEvent; }
...
class idWindow {
 public:
  ...
  void AddTransition(idWinVar* dest,
                     idVec4 from,
                     idVec4 to,
                     int time,
                     float accelTime,
 ...
 protected:
  ...
  int ParseEmitOp(idParser* src,
                  int a,
                  wexpOpType_t opType,
                  int priority,
                  wexpOp_t** opp = NULL);
  int ParseTerm(idParser* src, idWinVar* var = NULL, int component = 0);
  int ParseExpressionPriority(idParser* src,
                              int priority,
                              idWinVar* var = NULL,
                              int component = 0);
  ...
  static bool
      registerIsTemporary[MAX_EXPRESSION_REGISTERS];  // statics to assist
                                                      // during parsing
